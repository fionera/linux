...
#if 1 //merlin4 has secure mode p-channel register, can not access in kernel.
...
{
// 	const unsigned int reg[] = {0xb805B460, 0xb805B464, 0xb805B468, 0xb805B46C}; // mac6
	const unsigned int reg[] = {0xb805B4E4, 0xb805B4E8, 0xb805B4EC, 0xb805B4F0}; //merlin4
	unsigned int val;

	val = rtd_inl(reg[cpu]);

	return (val & 0x1a0); //bit 8,7,5 
}


unsigned int PSTATE_GET(int cpu)
{
	const unsigned int reg = 0xb805B4E0;
	unsigned int data;

	data = rtd_inl(reg);
	data = (data >> (cpu*8)) & 0x03f;
...
unsigned int PSTATE_SET_PREQ(int cpu, int val)
{
	...
	const unsigned int write_en = (1<<7); 

	data = rtd_inl(reg);
	data = (data & ~(0x7f << (cpu*8))) | ((write_en | preq | val) << (cpu*8));
...
unsigned int PREQ_RELEASE(int cpu)
{
	const unsigned int reg = 0xb805B4E0;
	unsigned int data;
	const unsigned int preq = (1<<6)<<(cpu*8); 
	const unsigned int write_en = (1<<7) << (cpu*8); 

	data = rtd_inl(reg);
	data = (data & ~preq) | write_en;
...
retry:
	...
	BUG_ON(status != P_ACCPETED);

	if (PSTATE_GET(cpu)  == P_OFF) {
		nCPUPORESET_SET(cpu);
		printk(" CPU(%d) kill done, pstate(0x%x) \n", cpu, PSTATE_GET(cpu));
		return !0;
	}
