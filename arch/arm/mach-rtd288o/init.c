...
int parse_series_variable(const char *variable_name, char *store_ptr, int max_len, int level, int contiguous, int txt2bin)
{
	...
	for(i=1;i<=level;i++) {
		if(level == 1)
			snprintf(ext_variable_name, sizeof(ext_variable_name), "%s", variable_name);
		else
			snprintf(ext_variable_name, sizeof(ext_variable_name), "%s%d", variable_name, i);
...
{
	extern phys_addr_t reserved_dvr_size;
	extern phys_addr_t reserved_dvr_start;
	char *endp;
...
{
	extern phys_addr_t reserved_last_image_size;
	extern phys_addr_t reserved_last_image_start;
	char *endp;
...
rtk_rtdlog_info_t * rtdlog_info = NULL;

#define UBOOT_RTDLOG_INFO_ADDR    (0x00003000)
...
int rtdlog_resume(void)
{
	...
	//info_p = (rtk_rtdlog_info_t *)phys_to_virt(UBOOT_RTDLOG_INFO_ADDR);
       rtdlog_info =  (rtk_rtdlog_info_t *)phys_to_virt(UBOOT_RTDLOG_INFO_ADDR);
	if ( rtdlog_info != NULL) {
           ...
           // console_rtdlog.rtdlog_option= info_p->rtdlog_option;
            ...
            pr_info(" console_rtdlog_level = 0x%x (%u)\n",  rtdlog_info->rtdlog_level,  rtdlog_info->rtdlog_level);
            pr_info(" console_rtdlog_module = 0x%x (%u)\n", rtdlog_info->rtdlog_module, rtdlog_info->rtdlog_module);
            pr_info(" console_rtdlog_option = 0x%x (%u)\n", rtdlog_info->rtdlog_option, rtdlog_info->rtdlog_option);
...
static int __init early_rtdlog(char *str)
{
	...
	rtdlog_info = (rtk_rtdlog_info_t *)phys_to_virt(rtdlog_addr);
	if (rtdlog_info != NULL) {
            //console_rtdlog.rtdlog_level= info_p->rtdlog_level;
            //console_rtdlog.rtdlog_module= info_p->rtdlog_module;
            //console_rtdlog.rtdlog_option= info_p->rtdlog_option;
...
inline int is_release_mode(void)
{
	return (debugMode == RELEASE_LEVEL);
}
