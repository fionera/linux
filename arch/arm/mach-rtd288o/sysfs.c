...
#if (defined CONFIG_RTK_KDRIVER) || (defined CONFIG_ARCH_RTK287X)
...
{
#if defined(CONFIG_REALTEK_THERMAL) || defined(CONFIG_RTK_KDRV_THERMAL_SENSOR)
...
{
	if(page) {
		sscanf(page,"%hd",&platform_info.sched_log_event_number); //[CID_20603] expects argument of type ‘int *’, but argument 3 has type ‘short int *’
	}
	else
		platform_info.sched_log_event_number=100;
...
{
	if(page) {
		sscanf(page,"%s",platform_info.sched_log_name); //[CID_20604] expects argument of type ‘char *’, but argument 3 has type ‘char (*)[120]’
	}
	else
...
static ssize_t enable_dvs_show(struct kobject *kobj, struct kobj_attribute *attr, char *page)
{
	return sprintf(page, "%x\n", platform_info.enable_dvs);
}

static ssize_t enable_dvs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
    char str[2];
	if(page) {
	    strncpy(str,page,1);
		if(strncmp(str,"1",1)==0)
			...
			platform_info.enable_dvs=0;
	}
	else
		platform_info.enable_dvs=0;
	return count;
}
REALTEK_BOARDS_ATTR_RW(enable_dvs);
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE
extern void cpufreq_interactive_bouncing(int count);
#endif
static ssize_t power_cut_bouncing_show(struct kobject *kobj, struct kobj_attribute *attr, char *page)
{
	return sprintf(page, "%x\n", platform_info.power_cut_bouncing);
}

static ssize_t power_cut_bouncing_store(struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
	if(page) {
	    	sscanf(page,"%d",&platform_info.power_cut_bouncing);
		if(platform_info.power_cut_bouncing<5)
			platform_info.power_cut_bouncing=5;
		if(platform_info.power_cut_bouncing>31)
			platform_info.power_cut_bouncing=31;
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE
		cpufreq_interactive_bouncing(platform_info.power_cut_bouncing);
#endif
	}
	else
		platform_info.power_cut_bouncing=0;
	return count;
}
REALTEK_BOARDS_ATTR_RW(power_cut_bouncing);
...
static ssize_t arm_wrapper_store (struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
    char buffer[128];
    int arm_wrapper_ = 0;
    unsigned int addr_s =0,addr_e=0;
...
#ifdef CONFIG_RTK_KDRV_SB2
__attribute__((weak)) int sb2_dbg_get_info (char *buf) 
{
        return 0;
}
...
void do_dvr_reclaim(int cmd)
{
	...
	if (((cmd == 1) || (cmd == 3)) && (pAnimation)) {
		unsigned long sync_phy = ntohl(*(unsigned long *)phys_to_virt(SYNC_STRUCT_ADDR));
		...
		if (sync_phy) {
			...
			aEnable = (int)*(unsigned long *)(sync_ptr+12);
			vEnable = (int)*(unsigned long *)(sync_ptr+8);
	...
	if ((cmd == 2) && (pLastImage)) {
        ...
        drvif_scalerdisplay_set_bg_color(SLR_MAIN_DISPLAY, _DISPLAY_BG, 0x21, 0x21, 0x21);
        scalerdisplay_force_bg_enable(SLR_MAIN_DISPLAY, TRUE);
#endif
        printk("\033[1;44m [%s] cmd=%d, reclaim pLastImage...\033[0m\n", __FUNCTION__, cmd);
	}
...
void do_rtk_uart2rbus_control(int cmd)
{
	if (cmd == 0)
	{
		printk("disable uart2rbus\n");
		enable_uart2rbus(0);
	}
	else if (cmd == 1)
	{
		printk("enalbe uart2rbus\n");
		enable_uart2rbus(1);
	}
	else
	{
		printk("%s, wrong command\n", __FUNCTION__);
	}
}
...
#ifdef CONFIG_RTK_KDRV_DEMOD
static ssize_t demod_log_time_period_show(struct kobject *kobj, struct kobj_attribute *attr, char *page)
{
       extern int demod_get_time_period(void);
       unsigned int period;
       period = demod_get_time_period();
	return sprintf(page, "%d\n",period);
}

static ssize_t demod_log_time_period_store(struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
        extern void demod_set_time_period(int period);
        unsigned int period;
        if(page) {
		sscanf(page,"%d",&period);
	}
	else
		period=100;
	if(period>0 && period<=100)
		period=100;
        demod_set_time_period(period);
        return count;
}
REALTEK_BOARDS_ATTR_RW(demod_log_time_period);

static ssize_t demod_log_file_start_show(struct kobject *kobj, struct kobj_attribute *attr, char *page)
{
       extern int demod_get_time_period(void);
       unsigned int period;
       period = demod_get_time_period();
...
static ssize_t demod_log_file_start_store(struct kobject *kobj, struct kobj_attribute *attr, const char *page, size_t count)
{
        extern int  demod_log_start(void);
        demod_log_start();
        return count;
}
REALTEK_BOARDS_ATTR_RW(demod_log_file_start);
#endif
...
static ssize_t pll_info_show(struct kobject * kobj, struct kobj_attribute * attr, char * page)
{
	...
	strcat(page_info, msg);
        ...
        tmp = rtd_inl(0xb80be024);
        value_f = ((tmp & 0x07FF0000) >> 16);
        tmp = rtd_inl(0xb80be028);
        value_n = ((tmp & 0x0000FF00) >> 8);
        freq = (27 * (value_n + 3) + ((27 * value_f) >> 11))*2; //      27*(NCODE + 3 + FCODE/2048)

        if (freq != 0)
                sprintf( msg, "DDR    PLL       = %4d MHz", freq);
        else
                sprintf( msg, "DDR    PLL       =--- MHz");
	strcat(page_info, msg);
	strcat(page_info, "\n");
	...
	if(dfs_enable_A53 == 1) // DFS enable
	{
		...
		if(ret > 0)
		{
			...
			sprintf(msg, "SCPU(A55) PLL   = %s MHz\n", buf);
			strcat(page_info, msg);
		}
		else if(ret == 0)
		{
			strcat(page_info, "SCPU(A55) PLL: get fail\n");
		}
		else
		{
			strcat(page_info, "SCPU(A55) PLL: get fail\n");
		...
		//[CID_17520] filp0 would not be null, for we had been used IS_ERR(filp0) to confirm the validity of filp0;
	...
	else // DFS disable
	{
		...
		memset(msg, 0, sizeof(msg));
		sprintf( msg, "SCPU(A55) PLL   = %4d MHz\n", freq );
		strcat(page_info, msg);
...
//	&vout_interface_attr.attr,
	...
	&sched_log_name_attr.attr,
#endif
#ifdef CONFIG_RTK_KDRV_DEMOD
        &demod_log_time_period_attr.attr,
        &demod_log_file_start_attr.attr,
#endif
	NULL
};
...
static int __init realtek_boards_sysfs_init(void)
{
    ...
    if(_LINUX_REVISION == _KDRIVER_REVISION && _KDRIVER_REVISION == _COMMON_REVISION ){
        // OE build case: do nothing, because git hash is OE build git, not linux/kdriver/common git hash
    }else{
        // local build case: show git hash
        #ifdef _LINUX_REVISION
        printk(KERN_EMERG"[Linux   Module Version]  %x \n", _LINUX_REVISION);
        #endif

        #ifdef _KDRIVER_REVISION
        printk(KERN_EMERG"[Kdriver Module Version]  %x \n", _KDRIVER_REVISION);
        #endif

        #ifdef _COMMON_REVISION
        printk(KERN_EMERG"[Common  Module Version]  %x \n", _COMMON_REVISION);
        #endif
    }
...
void *get_platform_throttle_resource(void)  //for thermal build
{
	return NULL;
}
