		...
		struct vm_area_struct *new_vma)
{
	...
	unsigned long vdso_size = vdso32_end - vdso32_start;
...
{
	int ret;
	ret = vdso_mappings_init("vdso32", vdso32_start, vdso32_end,
			&vdso32_mappings);
	vdso32_mappings.code_mapping.mremap = vdso32_mremap;
	return ret;
}
...
static struct page *sigreturn_page;
...
/* kuser helpers page */
#ifdef CONFIG_EKP
struct page *kuser_helpers_page;
#else
static struct page *kuser_helpers_page;
#endif
...
static int __init aarch32_kuser_helpers_init(void)
{
	...
	page = romempool_alloc_pages(GFP_KERNEL | __GFP_ZERO, RMP_SPECIAL, 0);
	if (!page) {
		page = alloc_page(GFP_KERNEL | __GFP_ZERO);
		if (!page)
			return -ENOMEM;
