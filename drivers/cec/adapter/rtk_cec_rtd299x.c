...
#include "rtk_cec_rtd294x-reg.h"
#else
#include "rtk_cec_rtd299x-reg.h"
#endif
...
unsigned int _get_pre_div(void)
{
	...
	tmp = read_reg32(RTD299x_CEC_SYS_PLL_BUS2);
...
unsigned int _get_pre_div(void)  //reference from bootcode:/common/cmd_dssinfo.c  pll_ddr_freq()
{
	unsigned int unRet;

	cec_info("%s bad parameters . please check first \n", __func__  );
	unRet = 0xff;
...
int _read_rx_fifo(cm_buff *cmb)
{
	...
	int len = read_reg32(CEC_RXCR0) & 0x1F;
	int i;
	...
	if (cmb_tailroom(cmb) >= len) {
		while (len) {
			...
			for (i = 0; i < rx_len; i += 4)
				rx_fifo_data[i >> 2] =
				    __cpu_to_be32(read_reg32(CEC_RXDR1 + i));

			cec_rx_dbg("cmb->tail=%p, rx_fifo_data=%p, rx_len=%d\n",
				   cmb->tail, rx_fifo_data, rx_len);

			memcpy(cmb->tail, (unsigned char *)rx_fifo_data,
			       rx_len);

			cmb_put(cmb, rx_len);

			write_reg32(CEC_TXDR0, RX_SUB_CNT | FIFO_CNT(rx_len));	/* fetch next rx data */
...
{
	int remain = 0x1F - (unsigned char)(read_reg32(CEC_TXCR0) & 0x1F);
	int i;
	int len = cmb->len;
	...
	while (len) {
		...
		cec_tx_dbg("_write_tx_fifo : tx data len = %d\n", tx_len);

		memcpy(tx_fifo_data, cmb->data, tx_len);
		cmb_pull(cmb, tx_len);

		for (i = 0; i < tx_len; i += 4)
			write_reg32(CEC_TXDR1 + i,
				    __cpu_to_be32(tx_fifo_data[i >> 2]));

		write_reg32(CEC_TXDR0, TX_ADD_CNT | FIFO_CNT(tx_len));	/* add data to fifo */
...
void rtd299x_cec_rx_reset(rtk_cec *p_this)
{
	...
	wmb();
	write_reg32(CEC_RXCR0, PATCH_RX_SAME_LA_ACK);
	wmb();
...
void rtd299x_cec_tx_start(rtk_cec *p_this)
{
	...
	write_reg32(CEC_RXCR0, RX_EN | PATCH_RX_SAME_LA_ACK);	/* enable RX */
...
void rtd299x_cec_rx_work(struct work_struct *work)
{
	...
	if (p_rcv->state == RCV) {
...
#endif
		{
			if (rx_event & RX_INT) {
				...
				write_reg32(CEC_RXCR0, RX_INT|PATCH_RX_SAME_LA_ACK);
			}
		}
		else
		{
	...
	if (p_rcv->state == IDEL) {
		...
		if (!p_rcv->cmb) {
			...
			if (p_rcv->cmb == NULL) {
				cec_warning
				    ("[CEC] FATAL, something wrong, no rx buffer left\n");
				goto end_proc;
			}
		}
		...
		if (delayed_rx_en == 0) {
			write_reg32(CEC_RXCR0, RX_EN | RX_INT_EN | PATCH_RX_SAME_LA_ACK);
...
int rtd299x_cec_init(rtk_cec *p_this)
{
	...
	if (!p_this->status.init) {
		...
		write_reg32(CEC_CLKEN, CLKEN_CEC);
		udelay(10);
		write_reg32(CEC_SRST, RSTN_CEC);
		udelay(10);
		write_reg32(CEC_SRST, RSTN_CEC | _BIT0 );
		udelay(10);
		write_reg32(CEC_CLKEN, CLKEN_CEC |_BIT0 );
		udelay(10);
	#else
		write_reg32(CEC_CLKEN, (read_reg32(CEC_CLKEN) | _BIT0 ) & (~CLKEN_CEC) );
		udelay(10);
		write_reg32(CEC_SRST, (read_reg32(CEC_SRST)  | _BIT0 ) & (~RSTN_CEC) );
		udelay(10);
		write_reg32(CEC_SRST, read_reg32(CEC_SRST) | RSTN_CEC | _BIT0 );
		udelay(10);
		write_reg32(CEC_CLKEN, read_reg32(CEC_CLKEN) | CLKEN_CEC |_BIT0 );
		udelay(10);
	#endif
		...
		write_reg32(RTK_GDI_POWER_SAVING_MODE, 0x13);
#else
		...
		write_reg32(CEC_RXTCR0,
			    RX_START_LOW(140) | RX_START_PERIOD(188) |
...
#ifdef CEC_OPCODE_COMPARE_ENABLE
		write_reg32(RTK_GDI_CEC_OPCODE_ENABLE, 0);	/* disable opcode compare function */
#endif
		...
		if (request_irq
		    (ISO_IRQ, rtd299x_cec_isr, SA_INTERRUPT | SA_SHIRQ,
		     "RTD293x CEC", p_this) < 0) {
			cec_warning
			    ("cec : open %s cec failed, unable to request irq#%d\n",
			     CEC_MODEL_NAME, ISO_IRQ);
			return -1;
		}
		cec_info("cec : enable CEC SCPU interrupt\n");
		write_reg32(CEC_INT_EN,
			    (read_reg32(CEC_INT_EN) & ~CEC_INT_EN_MASK) |
			    CEC_INT_EN_VAL);
...
int rtd299x_cec_suspend(rtk_cec *p_this)
{
	...
	if (!p_this->status.enable) { /*CEC is off*/
		printk
		    ("[CEC]function is off when power on, doesn't enable!!\n");
		return 0;
	}
	...
	if (rtk_cec_standby_config && rtk_cec_standby_logical_addr < 0xF
...
#ifdef CEC_OPCODE_COMPARE_ENABLE
		write_reg32(RTK_GDI_CEC_OPCODE_ENABLE, 0);	/*disable */

		/*-- compare 1 */
		if (rtk_cec_standby_config &
		    STANBY_RESPONSE_GIVE_POWER_STATUS) {
			write_reg32(CEC_TxRxACKOPTION,
			...
			write_reg32(RTK_GDI_CEC_OPCODE_ENABLE,
				    (read_reg32(RTK_GDI_CEC_OPCODE_ENABLE) | 0x01));
		}
		/*-- compare 2 */
		if (rtk_cec_standby_config &
		...
		if (rtk_cec_standby_config &
			...
			write_reg32(RTK_GDI_CEC_COMPARE_OPCODE_03, CEC_MSG_GET_CEC_VERSION);
			write_reg32(RTK_GDI_CEC_SEND_OPCODE_03, CEC_MSG_CEC_VERSION);
			write_reg32(RTK_GDI_CEC_SEND_OPERAND_NUMBER_03, 1);
			write_reg32(RTK_GDI_CEC_OPERAND_07,
				    rtk_cec_standby_cec_version);
			write_reg32(RTK_GDI_CEC_OPCODE_ENABLE,
				    (read_reg32(RTK_GDI_CEC_OPCODE_ENABLE) | 0x04));
		}
		/*-- compare 4*/
		if (rtk_cec_standby_config &
		    STANBY_RESPONSE_GIVE_DEVICE_VENDOR_ID) {
			write_reg32(RTK_GDI_CEC_COMPARE_OPCODE_04, CEC_MSG_GIVE_DEVICE_VENDOR_ID);
			write_reg32(RTK_GDI_CEC_SEND_OPCODE_04, CEC_MSG_DEVICE_VENDOR_ID);
			write_reg32(RTK_GDI_CEC_SEND_OPERAND_NUMBER_04, 3);
			write_reg32(RTK_GDI_CEC_OPERAND_10,
				    (rtk_cec_standby_vendor_id >> 16) &
				    0xFF);
			write_reg32(RTK_GDI_CEC_OPERAND_11,
				    (rtk_cec_standby_vendor_id >> 8) &
				    0xFF);
			write_reg32(RTK_GDI_CEC_OPERAND_12,
				    (rtk_cec_standby_vendor_id) & 0xFF);
			write_reg32(RTK_GDI_CEC_OPCODE_ENABLE,
				    (read_reg32(RTK_GDI_CEC_OPCODE_ENABLE) | 0x08));
		}
		printk("CEC_COMP_ENABLE = %08x\n", read_reg32(RTK_GDI_CEC_OPCODE_ENABLE));
...
int rtd299x_cec_resume(rtk_cec *p_this)
{
	...
	p_this->status.standby_mode = 0;
		...
		udelay(10);
	#else
		write_reg32(CEC_CLKEN, read_reg32(CEC_CLKEN) & ~CLKEN_CEC);
		udelay(10);
		write_reg32(CEC_SRST, read_reg32(CEC_SRST) & ~RSTN_CEC);
		udelay(10);
		write_reg32(CEC_SRST, read_reg32(CEC_SRST) | RSTN_CEC);
		udelay(10);
		write_reg32(CEC_CLKEN, read_reg32(CEC_CLKEN) | CLKEN_CEC);
	...
	write_reg32(CEC_TXTCR1,
		    TX_DATA_LOW(26) | TX_DATA_01(35) | TX_DATA_HIGH(34));
