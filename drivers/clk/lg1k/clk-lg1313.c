...
static int clk_lg1314_set_rate(struct clk_hw *hw, unsigned long rate,
                               unsigned long parent_rate)
{
	struct clk_lg1313 const *clk = to_clk_lg1313(hw);
	u32 regs[3], v;
	...
	/* switch to stable 660MHz clock */
	v = readl_relaxed(clk->base[1]) | 0x1000;
	writel_relaxed(v, clk->base[1]);
...
static struct clk_ops clk_lg1313_ops = {
...
static void __init __clk_lg1313_of_setup(struct device_node *np)
{
	struct clk *clk;
...
{
	clk_lg1313_ops.set_rate = clk_lg1313_set_rate;
	__clk_lg1313_of_setup(np);
}
CLK_OF_DECLARE(clk_lg1313, "lge,lg1313-clk", clk_lg1313_of_setup);

static void __init clk_lg1314_of_setup(struct device_node *np)
{
	clk_lg1313_ops.set_rate = clk_lg1314_set_rate;
	__clk_lg1313_of_setup(np);
}
CLK_OF_DECLARE(clk_lg1314, "lge,lg1314-clk", clk_lg1314_of_setup);
