...
static unsigned long rtk_i2c_cal_isr_timeout(unsigned char id, unsigned int spd)
{
	...
	{
		if (spd >= 1000) {
			wait_timeout =  jiffies + _msecs_to_jiffies(6);
		}else if (spd >= 600) {
			wait_timeout =  jiffies + _msecs_to_jiffies(9);
		}else if (spd >= 400) {
			wait_timeout =  jiffies + _msecs_to_jiffies(13);
		} else if (spd == 50){
			wait_timeout =  jiffies + _msecs_to_jiffies(100);
		} else {
			wait_timeout =  jiffies + _msecs_to_jiffies(50);
		}
	}
	else
	{
		if (spd >= 1000) {
			wait_timeout =	jiffies + _usecs_to_jiffies(200);
		}else if (spd >= 600) {
			wait_timeout =	jiffies + _usecs_to_jiffies(200);
		}else if (spd >= 400) {
			wait_timeout =	jiffies + _usecs_to_jiffies(300);
		} else if (spd == 50){
			wait_timeout =	jiffies + _usecs_to_jiffies(2100);
		} else {
			wait_timeout =	jiffies + _usecs_to_jiffies(1100);
		}
	}
...
#define TxComplete()              (p_this->xfer.tx_len >= p_this->xfer.tx_buff_len)
	...
	unsigned long wait_timeout  = rtk_i2c_cal_isr_timeout(p_this->id,p_this->spd);
	...
	else if (time_before_eq(jiffies, wait_timeout))
	{
		goto gotoSendCMD;
	}
...
int rtk_i2c_set_spd(rtk_i2c * p_this, int KHz)
{
	...
	*/
	...
	if (KHz >= 1000) {
		...
		SET_IC_SS_SCL_HCNT(p_this, 0x07);
...
int rtk_i2c_start_xfer(rtk_i2c * p_this)
{
	...
	if(p_this->id == 0)
		rtd_outl(RTD299s_ISO_IC0_SDA_HOLD,0x0000000b);


#ifdef CONFIG_CUSTOMER_TV006

	if( 1 == p_this->id )
		rtd_outl(RTD299s_MISC_IC1_SDA_HOLD,0x0000000B);

	if( 2 == p_this->id )
		rtd_outl(RTD299s_MISC_IC2_SDA_HOLD,0x00000008);

	if( 3 == p_this->id )
		rtd_outl(RTD299s_MISC_IC3_SDA_HOLD,0x0000000B);

	if( 4 == p_this->id )
		rtd_outl(RTD299s_MISC_IC4_SDA_HOLD,0x0000000B);
		   ...
		   unsigned char *rx_buf, unsigned short rx_buf_len)
{
	...
	while (retry > 0) {
		...
		retry--;
		if( retry > 0  )
	   		I2C_WARNING(" %s Line%d , read %s detected, do retry ,\n", __func__ , __LINE__ ,
	   			( -ETIMEOUT == ret )?("timeout"):("ECMDSPLIT") );
	  	else
	  		I2C_WARNING(" %s Line%d , read %s detected, retry = 0x%X ,\n", __func__ , __LINE__ ,
	  			( -ETIMEOUT == ret )?("timeout"):("ECMDSPLIT") , retry );
	}
