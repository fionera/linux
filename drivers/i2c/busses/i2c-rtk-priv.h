...
#define rtk_I2C_SLAVE_ENABLE  0x80
	...
	int (*register_slave_ops) (rtk_i2c * p_this,
...
#define I2C_INFO(fmt, args...)		rtd_printk(KERN_INFO,  TAG_NAME , "[Info]" fmt, ## args)
#define I2C_WARNING(fmt, args...)		rtd_printk(KERN_WARNING,  TAG_NAME , "[Warn]" fmt, ## args)
#define I2C_CONDITION_WARNING(level,adb,fmt, args...)		\
					do {						\
							if(adb->i2c_loglevel>=level)		\
							rtd_printk(KERN_WARNING, TAG_NAME , "[Warn]" fmt, ## args);		\
					} while (0)
