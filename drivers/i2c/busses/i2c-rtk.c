...
{
#if defined( CONFIG_CUSTOMER_TV006 ) && ( defined(CONFIG_ARCH_RTK299O) || defined(CONFIG_ARCH_RTK289X) )
	...
	#endif
	{
		...
		while (!rtd_inl (SB2_HD_SEM_NEW_2_reg)  ) {

			pr_err("#@# %s [i2c-%d] wait hw sema ready\n" , __func__ , p_this->id );
...
static void hw_i2c_semaphore_unlock(rtk_i2c *p_this)
{
	...
	#endif
	{
		rtd_outl(SB2_HD_SEM_NEW_2_reg, 0);
	}
#endif
}
...
__init i2c_rtk_module_init(void)
{
	...
	for (i = 0; i < adapter_count; i++) {
		if (adapter_config[i].mode == I2C_MODE) {
			...
			if (p_this == NULL)
				continue;
			I2C_WARNING( "[ch_%d] open successed, reference  id =%d, cnt=%d\n",
				i,
			       adapter_config[i].i2c.phy_id,
