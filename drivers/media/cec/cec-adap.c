...
static int cec_report_features(struct cec_adapter *adap, int la_idx);
...
static int cec_report_features(struct cec_adapter *adap, int la_idx)
{
	...
	const u8 *features = NULL;
	bool op_is_dev_features = false;
	unsigned int idx;

        if(la_idx <0 || la_idx >= CEC_MAX_LOG_ADDRS)
            return -EINVAL;

        features = las->features[la_idx];
