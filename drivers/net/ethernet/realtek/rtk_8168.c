...
static void rtl_read_adc_val(struct rtl8168_private *tp)
{
	...
	for(i = 0; i < 5; i++) {
		...
		eth_info("ADC peak port A = 0x%x\n", phy_read_data);
	}

	rtl8168_xmii_set_page(tp, 0x0a9e);
	for(i = 0; i < 5; i++) {
		phy_read_data = rtl_readphy(tp, 0x11);
		eth_info("ADC peak port B = 0x%x\n", phy_read_data);
	}
	rtl8168_xmii_set_page(tp, 0x0000);
}
...
static void rtl_read_phy_status(struct rtl8168_private *tp)
{
	...
	eth_info("rd_dbg0 = 0x%4x, clk_fail_mac_sys = 0x%04x, mcu = 0x%04x, 0x%04x, lock_main_uc = 0x%4x, %s",
			...
			phyStatus->lock_main, phyStatus->lock_main & BIT(1) ? "lock":"unlock");
...
#if defined(CONFIG_ARCH_RTK299O) || defined(CONFIG_ARCH_RTK293x) || defined(CONFIG_ARCH_RTK289X) || defined(CONFIG_ARCH_RTK287X)
                ...
                char *buf)
{
	...
	//RTL_W32(OCPDR, 0x70030000);
        ...
        return sprintf(buf, "wol_enable = %d, wol_interrupt = %08x	\
