	...
	if (IS_HARDWARE_TYPE_JAGUAR2(adapter)) {
		if (chk_type) {
			...
			if (!reason)
				...
				RTW_PRINT("0x210=0x%08x, 0x80=0x%08x, 0x10f8=0x%08x, 0x1cc=0x%08x, 0x120=0x%08x, 0x130=0x%08x, 0x1120=0x%08x, 0x1128=0x%08x, 0x1130=0x%08x\n"
				, rtw_read32(adapter, 0x210), rtw_read32(adapter, 0x80)
				, rtw_read32(adapter, 0x10f8), rtw_read32(adapter, 0x1cc)
				, rtw_read32(adapter, 0x120), rtw_read32(adapter, 0x130)
				, rtw_read32(adapter, 0x1120), rtw_read32(adapter, 0x1128)
				, rtw_read32(adapter, 0x1130));
				trycnt=10;
				while(trycnt >0) {
					RTW_PRINT(" 0x10fc=0x%08x, 0xc0=0x%08x,", rtw_read32(adapter, 0x10fc), rtw_read32(adapter, 0xc0));
					trycnt--;
				}
				RTW_PRINT("\n");
				//mac_reg_dump_p1p2(RTW_DBGDUMP, adapter);
				res = _FALSE;
			}
		} else {
...
static u8 rtw_hal_set_gpio_custom_cmd(_adapter *adapter, u8 enable)
{
	...
	u8 customid = 0x3F;
	u8 gpio_unit_a = 1;
	u8 special_wakeup_reason_a = RX_MAGIC_PKT; /* magic packet */
	u8 gpio_dur_a = 75;
	u8 gpio_pulse_en_a = 1;
	u8 gpio_pulse_cnt_a = 127;
	u8 ret = _FAIL;
	...
	if(enable) {
		...
		SET_H2CCMD_WOWLAN_CHANGE_UNIT_A(H2CGpioCustomParm, gpio_unit_a);
		SET_H2CCMD_SPECIAL_WAKE_REASON_A(H2CGpioCustomParm, special_wakeup_reason_a);
		SET_H2CCMD_WOWLAN_GPIO_DURATION_A(H2CGpioCustomParm, gpio_dur_a);
		SET_H2CCMD_WOWLAN_GPIO_PULSE_EN_A(H2CGpioCustomParm, gpio_pulse_en_a);
		SET_H2CCMD_WOWLAN_GPIO_PULSE_COUNT_A(H2CGpioCustomParm, gpio_pulse_cnt_a);

		ret = rtw_hal_fill_h2c_cmd(adapter,
						H2C_GPIO_CUSTOM,
						H2C_GPIO_CUSTOM_LEN,
						H2CGpioCustomParm);
		RTW_INFO("%s(): H2C_cmd=%x, cmd=%02x, %02x, %02x, %02x\n", __func__, H2C_GPIO_CUSTOM,
		H2CGpioCustomParm[0], H2CGpioCustomParm[1], H2CGpioCustomParm[2], H2CGpioCustomParm[3]);
	}
...
static u8 rtw_hal_set_wowlan_ctrl_cmd(_adapter *adapter, u8 enable, u8 change_unit)
{
	...
	u8 discont_wake = 1, gpionum = 0, gpio_dur = 25;
	u8 hw_unicast = 0, gpio_pulse_cnt = 127, gpio_pulse_en = 1;
	u8 sdio_wakeup_enable = 1;
	u8 gpio_high_active = 0;
	u8 magic_pkt = 0;
	u8 gpio_unit = 1; /*0: 64ns, 1: 8ms*/
	u8 ret = _FAIL;
...
int check_phy_efuse_tx_power_info_valid(PADAPTER padapter)
{
	...
	case RTL8192E:
		...
		break;
	case RTL8192F:
		tx_index_offset = EEPROM_TX_PWR_INX_8192F;
		break;
	case RTL8821:
