...
)
{
	...
	for (i = 0; i < psd_avg_time; i++) {
		...
		for(j = 0; j < 1000 ; j++) {
			...
			while(!odm_get_bb_reg(dm, R_0xeac, psd_finish_bit[rf_path]));	/*wait finish bit*/
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION,"[TGGC] rf0=0x%x rf56=0x%x rf56_reg=0x%x time=%d psd_vaule=0x%x\n",
			odm_get_rf_reg(dm, rf_path, RF_0x0, 0xff),
			rf56, odm_get_rf_reg(dm, rf_path, RF_0x56, 0xfff), j, psd_vaule[i]);
	}
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,"[TGGC] rf0=0x%x rf56=0x%x rf56_reg=0x%x psd_vaule_temp=0x%x\n",
		odm_get_rf_reg(dm, rf_path, RF_0x0, 0xff),
		rf56, odm_get_rf_reg(dm, rf_path, RF_0x56, 0xfff), psd_vaule_temp);
