...
#ifdef OPROFILE_ARM_DSU_PMU
/* Set at runtime when we know what CPU type we are. */
static struct dsu_pmu *__oprofile_cpu_pmu;
...
int perf_num_counters(void)
{
	...
	if (__oprofile_cpu_pmu != NULL) {
		max_events = __oprofile_cpu_pmu->num_counters;
        pr_info("%s %d, count=%d\n", __func__, __LINE__, max_events);
    }
			  ...
			  void *hcpu)
{
	...
	struct dsu_pmu *dsu_pmu = container_of(b, struct dsu_pmu, hotplug_nb);
...
static int cpu_pmu_init(struct dsu_pmu *dsu_pmu)
{
	int err;
	int cpu = get_cpu();

	dsu_pmu->hotplug_nb.notifier_call = cpu_pmu_notify;
	err = register_cpu_notifier(&dsu_pmu->hotplug_nb);
	if (err)
		goto out_hw_events;

	pr_info("probing arm_dsu_pmu on CPU %d\n", cpu);
...
static void cpu_pmu_destroy(struct dsu_pmu *cpu_pmu)
{
...
out_destroy:
	cpu_pmu_destroy(dsu_pmu);
out_free:
	pr_info("failed to register PMU devices! rc=%d\n", rc);
	devm_kfree(pdev, dsu_pmu);
#endif
    return rc;
}
