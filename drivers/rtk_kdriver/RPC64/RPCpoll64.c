...
int rpc_poll_run(void)
{
	#if 0
	int i;
	RPC_COMMON_Dev *rpc_dev = (RPC_COMMON_Dev *)get_rpc_addr(RPC_POLL_RECORD_ADDR);
	
	for (i = 0; i < RPC_POLL_DEV_TOTAL; i++)
	{
		if(!(ring_empty((RPC_COMMON_Dev *)(&rpc_dev[i]))))
		{
			printk("yan_wang debug poll ringIn == %x, ringOut == %x... \n", rpc_dev[i].ringIn, rpc_dev[i].ringOut);
