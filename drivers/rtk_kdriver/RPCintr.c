...
static int (*vrpc_service)(int portID, int cmd, void *data, int size) = NULL;
...
static inline void rpc_unhandle_packet(RPC_PROCESS *proc, char *out, int minor)
{
       ...
       if (out) {
               ...
               pr_err("%s-rpc(%p),program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx %s\n",
                       extra->name, out, rpc.programID, rpc.versionID, rpc.procedureID, rpc.taskID, (long unsigned int)(rpc.sysPID), rpc.parameterSize,
                       (long unsigned int)rpc.mycontext, in_atomic() ? "atomic" : "");
		#else
               pr_err("%s-rpc(%p),program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx %s\n",
...
void rpc_dispatch(unsigned long data)
{
		...
		extra->name, rpc.programID, rpc.versionID, rpc.procedureID, (long unsigned int)rpc.taskID, rpc.sysPID, rpc.parameterSize,
		(long unsigned int)rpc.mycontext, in_atomic() ? "atomic" : "");
	#else
        PDEBUGD("%s: program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx %s\n",
...
#endif
	} else if (rpc.programID == VIDEO_AGENT && rpc.sysPID > pid_max) {
		void *dataBuf = NULL, *dataPtr;
		if ((out + rpc.parameterSize) > dev->ringEnd) {
			dataPtr = dataBuf = kmalloc(rpc.parameterSize, GFP_ATOMIC);
			out = get_ring_data(extra->name, dev, out, dataBuf, rpc.parameterSize);
		} else {
			dataPtr = (void*)out;
		...
		if (vrpc_service)
			vrpc_service (rpc.sysPID, rpc.procedureID, dataPtr, rpc.parameterSize);
		if (dataBuf)
			kfree(dataBuf);
		...
		return;
			...
			#ifdef CONFIG_KDRIVER_USE_NEW_COMMON
                            EMERG_DEBUG("found pid = %d, sysPID = %d, taskID = 0x%lx, programID = %d, procedureID = %d, footprint=%x\n",(proc) ? proc->pid:-1,(int)rpc.sysPID, (long unsigned int)rpc.taskID, (int)rpc.programID,(int)rpc.procedureID,extra->footprint);
			#else
...
void rpc_setVRPCService(int (*funptr)(int portID, int cmd, void *data, int size))
{
	vrpc_service = funptr;
}
EXPORT_SYMBOL(rpc_setVRPCService);
