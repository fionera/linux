...
#endif
KRPC_INFO krpcinfo[RPC_NR_KERN_DEVS/RPC_NR_PAIR];
...
RPC_COMMON_Dev record_timeout_readbuf[RPC_NR_KERN_DEVS/RPC_NR_PAIR]= {0};
RPC_COMMON_Dev record_timeout_writebuf[RPC_NR_KERN_DEVS/RPC_NR_PAIR]= {0};
...
static void dump_trash_kernel_rpc_buffer(int opt)
{
	...
	pr_err("show_buffer(0x%8x, 0x%8x) InOut(0x%8x, 0x%8x)\n",
...
//		printk(" #@# wakeup %s \n", current->comm);
			...
			||(htonl(rpc->programID) == VIDEO_AGENT) || (htonl(rpc->programID)== KERNELID)) //RPC data
		{
			...
			*(tmp+1) = htonl(ret);
			...
			if(countack == LOOPCOUNT){
				...
				pr_err("kernel rpc ack: program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx",
					rpc->programID, rpc->versionID, rpc->procedureID, rpc->taskID, (long unsigned int)(rpc->sysPID), rpc->parameterSize, (long unsigned int)(rpc->mycontext));
				#else
				pr_err("kernel rpc ack: program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx",
			...
			*((unsigned long *)ntohl(rpc->mycontext)) = ntohl(*(tmp+1));
			...
			wake_up(krpcinfo[f].wq); // ack the sync...
...
void dump_kernel_rpc_data(int opt)
{
	...
	if(opt == RPC_VIDEO)
		pr_err("kernel rpc timeout! current %s dummy register info:0x%08x\n", rpc_kthread_name[opt], kernel_rpc_video_dummy);
    ...
    unsigned long *retvalue)
{
    ...
    if( opt >= RPC_MAX)
	{
		pr_err("BUG!!! NO this(%d) CPU (command:%lx)...\n",opt,command);
		return RPC_FAIL;
	}
	...
	if( opt == RPC_VIDEO2)
	{
		...
		{
			pr_err("RPCkern: VCPU2 not finished init RPC ...\n");
			up(&send_kernel_rpc_sem[opt]);
			return RPC_VCPU_NOT_READY;
		}
	}
#endif
//	printk(" #@# sendbuf: %d \n", sizeof(sendbuf));
	krpcinfo[opt].wq = &rpc_wq[opt];
	krpcinfo[opt].reply = 0;
	// fill the RPC_STRUCT...
	...
	rpc->taskID = (send_mode & KRPC_BLOCK_MODE) ? htonl((unsigned long)&krpcinfo[opt]) : 0 ;
#ifdef CONFIG_REALTEK_RPC_MULTIPROCESS
	rpc->sysPID = 0;
#endif
...
//	printk(" aaa: %x bbb: %x \n", sendbuf, tmp);
	...
	*(tmp+2) = htonl(param2);
	...
	if(send_mode & KRPC_BLOCK_MODE) //for block mode
   	{
       	// wait the result...
	       if (!wait_event_timeout(rpc_wq[opt], krpcinfo[opt].reply, TIMEOUT)) {
		pr_err("[krpc timeout v1.2]kernel rpc to %s wait ack timeout ->disable %s thread:  command:%lx, parm1: %lx, parm2: %lx ...\n", rpc_kthread_name[opt],
