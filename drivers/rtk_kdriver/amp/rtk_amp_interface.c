                           ...
                           const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "amp_mute") == 0) {
        ...
        if (kstrtouint(buf, 10, &on_off)!=0)
        {
            AMP_WARN("%s error , buff = %s \n" , __func__ , buf );
            return 0;
        }
    ...
    else if (strcmp(attr->attr.name, "amp_param") == 0)
    {
        ...
        else if (memcmp(buf, "set", strlen("set")) == 0)
        {
        ...
        else if (memcmp(buf, "init", strlen("init")) == 0)
        {
           rtk_amp.ops(rtk_amp.amp_i2c_id,rtk_amp.slave_addr);
        }
        else
...
{
    char *amp_boot_sel_str , *amp_boot_addr_str ;
    char *cur;
    int  i = 0;
    ...
    else
    {
        ...
        // parsing options string to amp_boot_sel/amp_boot_addr  , original format ==> (sscanf(options, "%2d-%2x", &amp_boot_sel, &amp_boot_addr)
        while (NULL != (cur = strsep(&options, "-")))
        {
            if (i == 0)
            {
                amp_boot_sel_str = cur;

                if (kstrtouint(amp_boot_sel_str, 10, &amp_boot_sel) != 0)
                {
                    AMP_ERR("%s, unknown options - %s , amp_boot_sel_str = %s  \n", __func__ ,options , amp_boot_sel_str );
                    return 1;
                }
            }
            else if (i == 1)
            {
                amp_boot_addr_str = cur;

                if (kstrtouint(amp_boot_addr_str, 16, &amp_boot_addr) != 0)
                {
                    AMP_ERR("%s, unknown options - %s , amp_boot_addr_str = %s  \n", __func__ ,options , amp_boot_addr_str );
                    return 1;
                }
            }
