...
static UINT32 InitInRingBuf(Base* pBaseObj, UINT32 pinID)
{
    ...
    /* Didn't need reset the ring between ain and decoder, so it set listSize > 2 */
    if ((pBaseObj->inPin->ResetPin) && (header.listSize > 2)) {
        pBaseObj->inPin->ResetPin(pBaseObj->inPin);
    }
...
UINT32 InitOutRingBuf(Base* pBaseObj)
{
    ...
    /* Didn't need reset the ring between ain and decoder, so it set listSize > 2 */
    if ((pBaseObj->outPin->ResetPin) && (header.listSize > 2)) {
        pBaseObj->outPin->ResetPin(pBaseObj->outPin);
    }
    pBaseObj->outPin->GetBufHeader(pBaseObj->outPin, (void**)&vAddr, &pAddr);
    if (ntohl(vAddr->numOfReadPtr) > 1) {
        DEBUG("[%s][%s] header numOfReadPtr is %d, reset to 1\n",pBaseObj->name,__FUNCTION__, ntohl(vAddr->numOfReadPtr));
        vAddr->numOfReadPtr = htonl(1);
    }
    header.pRingBufferHeaderList[0] = (unsigned long)pAddr;
    if(header.listSize > 1)
    {
...
static UINT32 Disconnect(Base* pForeBaseObj, Base* pBackBaseObj)
{
    ...
    {
        DEBUG("[%s] no send RPC ForeBase = %x pBackBaseObj = %x \n", __FUNCTION__,(int)pForeBaseObj, (int)pBackBaseObj);
    }
