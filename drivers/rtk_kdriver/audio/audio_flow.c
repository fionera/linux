...
static int CheckEventProcess(void* pData)
{
	...
	for (;;)
    {
        ...
        {
            UINT32 eventID = (UINT32)pEventQueue->GetQueue(pEventQueue);
            switch(eventID)
            {
                case EVENT_EOS:
                    DEBUG("[Flow] Get Event(%d)\n", eventID);
...
UINT32 Flow_DirectRun(FlowManager* pFlow)
{
    ...
    list_for_each_prev(list, &pFlowBaseObj->flowList)
    {
        ...
        if (((pli_getPTS() - get_ms)) / 90000 > DEADLOCK_TIMEOUT)
        {
...
UINT32  Flow_Flush(FlowManager* pFlow)
{
    ...
    list_for_each(list, &pFlowBaseObj->flowList)
    {
        if(list == &pFlowBaseObj->flowList) continue;
        pBaseObj = container_of(list, Base, flowList);
        if (pFlow->SkipDECFlush == TRUE)
        {
            if (strncmp(pBaseObj->name, "DEC", 3) == 0)
            {
                DEBUG("Skip %s Flush\n", pBaseObj->name);
            }
            else
...
UINT32  Flow_Connect(FlowManager* pFlow, Base* pForeBase, Base* pBackBase)
{
    ...
    if(!list_empty(&pFlowBaseObj->flowList))
    {
        ...
        list_for_each(list, &pFlowBaseObj->flowList)
        {
            ...
            {
                ERROR("[FLOW] connect %s exit in flow (%x = %x)\n", pBackBase->name, (UINT32)pBaseObj , (UINT32)pBackBase);
                INFO("[FLOW] connect %s exit in flow (%x = %x)\n", pBackBase->name, (UINT32)pBaseObj , (UINT32)pBackBase);
...
UINT32  Flow_Remove(FlowManager* pFlow, Base* pBaseObj)
{
    ...
    list_for_each(list, &pFlowBaseObj->flowList)
    {
       ...
       DEBUG("[FLOW] (%x)  find  Module(%s) (%x) prev (%x) next (%x)\n", (UINT32)&pFlowBaseObj->flowList, pTempBase->name, (UINT32)&(pTempBase->flowList), (UINT32)(pTempBase->flowList.prev), (UINT32)(pTempBase->flowList.next));
...
UINT32  Flow_SetSkipDECFlush(FlowManager* pFlow, UINT32 isSkip)
{
    pFlow->SkipDECFlush = isSkip;
    DEBUG("[FLOW][%s] isSkip:%x\n", __FUNCTION__, pFlow->SkipDECFlush);
    return S_OK;
}
...
FlowManager* new_flow()
{
    ...
    pFlow->DirectRun             = Flow_DirectRun;
    pFlow->SetSkipDECFlush       = Flow_SetSkipDECFlush;
    ...
    pFlow->SkipDECFlush      = FALSE;


#if 0
    if(pFlow->pEventThread && pFlow->pEventThread->IsRun(pFlow->pEventThread) == false)
    {
        //AUDIO_INFO("ask flow event to run\n");
        pFlow->pEventThread->Run(pFlow->pEventThread);
    }
    else
    {
        //AUDIO_INFO("flow event create failed\n");
        /*assert(0);*/
    }
#endif
