...
static UINT32 ConfigBBADC(Base* pBaseObj/*, AUDIO_BBADC_CONFIG* pBBADC_cfg*/)
{
    AUDIO_BBADC_CONFIG cfg_bbadc;
    AIN* pAINObj = (AIN*)pBaseObj->pDerivedObj;
    //AUDIO_BBADC_CONFIG* m_pBBADC_cfg = &(((AIN*)pBaseObj->pDerivedObj)->m_pBBADC_cfg);
    ...
    //memcpy(m_pBBADC_cfg, &cfg_bbadc, sizeof(AUDIO_BBADC_CONFIG));
    memcpy(&(pAINObj->m_pBBADC_cfg), &cfg_bbadc, sizeof(AUDIO_BBADC_CONFIG));

    return S_OK;
}
static UINT32 ConfigI2S(Base* pBaseObj/*, AUDIO_I2SI_CONFIG* pI2S_cfg*/)
{
    AUDIO_I2SI_CONFIG cfg_i2s;
    AIN* pAINObj = (AIN*)pBaseObj->pDerivedObj;
    //AUDIO_I2SI_CONFIG* m_pI2S_cfg = &(((AIN*)pBaseObj->pDerivedObj)->m_pI2S_cfg);
    ...
    //memcpy(m_pI2S_cfg, &cfg_i2s, sizeof(AUDIO_I2SI_CONFIG));
    memcpy(&(pAINObj->m_pI2S_cfg), &cfg_i2s, sizeof(AUDIO_I2SI_CONFIG));

    return S_OK;
}
static UINT32 ConfigSPDIF(Base* pBaseObj/*, AUDIO_SPDIFI_CONFIG* pSPDIF_cfg*/)
{
    AUDIO_SPDIFI_CONFIG cfg_spdif;
    AIN* pAINObj = (AIN*)pBaseObj->pDerivedObj;
    //AUDIO_SPDIFI_CONFIG* m_pSPDIF_cfg = &(((AIN*)pBaseObj->pDerivedObj)->m_pSPDIF_cfg);
    ...
    //memcpy(m_pSPDIF_cfg, &cfg_spdif, sizeof(AUDIO_SPDIFI_CONFIG));
...
UINT32 AIN_GetAudioFormatInfo(Base* pBaseObj, AUDIO_RPC_AIN_FORMAT_INFO* formatInfo)
{
    ...
    long instanceID = pBaseObj->GetAgentID(pBaseObj);
...
UINT32 SetATVClock(Base* pBaseObj, SINT32 atv_type)
{
    ...
    DEBUG("[AIN] AIN_RPC_TOAGENT_SET_ATV_CLOCK: %d......\n", atv_type);

    pRes = AUDIO_RPC_ToAgent_SET_ATV_CLOCK_0(&pInfo, &clnt);

    if( (pRes == (HRESULT *)-1 ) ||  (pRes== 0))
           MY_AUDIO_RPC_ERROR("return value = %x\n", (unsigned int) pRes);
       MY_AUDIO_ASSERT((pRes != ((HRESULT *)-1 )));

    if (((int)pRes == 0) || ((int)pRes== -1))
    {
       res = S_FALSE;
    }
    else
...
UINT32 SetSIFADCInit(Base* pBaseObj, SINT32 enable)
{
    ...
    DEBUG("[AIN] AIN_RPC_TOAGENT_SET_SIF_ADC_INIT: %d......\n", enable);

    pRes = AUDIO_RPC_ToAgent_SET_SIF_ADC_INIT_0(&pInfo, &clnt);
...
void delete_AIN(Base* pBaseObj)
{
    ...
    if(g_instance_idx >= 0) {
        ...
        if((long)pBaseObj->instanceID != g_instance_list[g_instance_idx]) {
            SINT32 i;
...
Base* new_AIN()
{
    ...
    pObj->outPin = new_multiPin(AUDIO_IN_OUTPUT_BUF_SIZE, 2);
