...
UINT32 AO_GetTrsencAudioFormatInfo(Base* pBaseObj, AUDIO_RPC_AO_TRSENC_FORMAT_INFO* format_info)
{
    AUDIO_RPC_AO_TRSENC_FORMAT_INFO pInfo = {0};
    UINT32 instanceID = pBaseObj->GetAgentID(pBaseObj);
    UINT32 res;

    res = RTKAUDIO_RPC_AO_TOAGENT_GETTRSENCAUDIOFORMATINFO_SVC(&instanceID, &pInfo);
    if(res != S_OK) {
...
Base* new_PPAO(PPAO_MODE mode)
{
    ...
    pPPAOObj->GetTrsencAudioFormatInfo = AO_GetTrsencAudioFormatInfo;
    /* agent init */
