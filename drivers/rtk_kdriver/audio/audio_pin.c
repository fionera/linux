...
extern long rtkaudio_free(unsigned int phy_addr);
...
static UINT32 Constructor(ALLOCATOR* alloc)
{
    ...
    //MY_LIBK_INFO("[%s] allocate RINGBUFFER_HEADER \n", __FUNCTION__);

    /*alloc->m_pBufferHeaderAddr = (UINT8*)RTKAudio_Malloc(sizeof(RINGBUFFER_HEADER),*/
            /*(BYTE**)&(alloc->m_pBufferHeader),(UINT32*)&(alloc->m_bufferHeaderPhyAddr));*/
    alloc->m_bufferHeaderPhyAddr = rtkaudio_alloc(sizeof(RINGBUFFER_HEADER), NULL,
                                            &(alloc->m_pBufferHeaderAddr));
    ...
    alloc->m_headerPhyAddrDiff = alloc->m_bufferHeaderPhyAddr - (UINT32)alloc->m_pBufferHeader;
...
static UINT32 SetBuffer(ALLOCATOR* alloc, UINT32 size)
{
    ...
    // set write ptr
    IPC_WriteU32((BYTE*)&(alloc->m_pBufferHeader->writePtr), (UINT32)0+alloc->m_headerPhyAddrDiff);
...
static UINT32 Commit(ALLOCATOR* alloc)
{
    ...
    //MY_LIBK_INFO("[%s] allocate m_bufferSize \n", __FUNCTION__);

    /*alloc->m_pBufferLower = (UINT8*)RTKAudio_Malloc(alloc->m_bufferSize*sizeof(UINT8),*/
            /*(BYTE**)&(alloc->m_pBufferNonCachedLower),(UINT32*)&(alloc->m_bufferPhyAddr));*/
    alloc->m_bufferPhyAddr = rtkaudio_alloc(alloc->m_bufferSize*sizeof(UINT8), NULL,
                                        &(alloc->m_pBufferNonCachedLower));
    alloc->m_pBufferLower = alloc->m_pBufferNonCachedLower;

    alloc->m_pBufferUpper = alloc->m_pBufferLower + alloc->m_bufferSize;
    alloc->m_bufferPhyAddrDiff = alloc->m_bufferPhyAddr - (UINT32)alloc->m_pBufferNonCachedLower;
    ...
    IPC_WriteU32((BYTE*)&(alloc->m_pBufferHeader->writePtr),
                      (UINT32)alloc->m_pBufferNonCachedLower+alloc->m_bufferPhyAddrDiff);
...
static ALLOCATOR* CreateAllocator(UINT32 size)
{
    ...
    //MY_LIBK_INFO("[%s] create allocate  \n", __FUNCTION__);
...
UINT8* GetReadPtr(Pin* pPinObj, UINT32 Idx)// function return virtual address
{
    ALLOCATOR *alloc = (ALLOCATOR*)pPinObj->allocator;
    RINGBUFFER_HEADER* bufHeader = alloc->m_pBufferHeader;
    return (UINT8*)(IPC_ReadU32((BYTE*)&(bufHeader->readPtr[Idx])) - alloc->m_bufferPhyAddrDiff);
}
...
UINT32 SetReadPtr(Pin* pPinObj, UINT8* readPtr, UINT32 Idx)// function accept virtual address
{
    ALLOCATOR *alloc = (ALLOCATOR*)pPinObj->allocator;
    RINGBUFFER_HEADER* bufHeader = alloc->m_pBufferHeader;
    IPC_WriteU32((BYTE*)&(bufHeader->readPtr[Idx]), (UINT32)readPtr + alloc->m_bufferPhyAddrDiff);
    return S_OK;
}
...
UINT8* GetWritePtr(Pin* pPinObj)// function return virtual address
{
    ALLOCATOR *alloc = (ALLOCATOR*)pPinObj->allocator;
    RINGBUFFER_HEADER* bufHeader = alloc->m_pBufferHeader;
    return (UINT8*)(IPC_ReadU32((BYTE*)&(bufHeader->writePtr)) - alloc->m_bufferPhyAddrDiff);
}
...
UINT8*  ConvertPhy2Virtual(Pin* pPinObj, int phyAddress)
{
    ...
    ALLOCATOR *alloc = (ALLOCATOR*)pPinObj->allocator;
    return (UINT8*) (phyAddress - alloc->m_bufferPhyAddrDiff);
}
...
UINT8*  ConvertVirtual2Phy(Pin* pPinObj, int VirtualAddress)
{
    ...
    ALLOCATOR *alloc = (ALLOCATOR*)pPinObj->allocator;
    return (UINT8*) (VirtualAddress + alloc->m_bufferPhyAddrDiff);
}
...
UINT32 SetWritePtr(Pin* pPinObj, UINT8* writePtr)// function accept virtual address
{
    ALLOCATOR *alloc = (ALLOCATOR*)pPinObj->allocator;
    RINGBUFFER_HEADER* bufHeader = alloc->m_pBufferHeader;
    IPC_WriteU32((BYTE*)&(bufHeader->writePtr), (UINT32)writePtr + alloc->m_bufferPhyAddrDiff);
    return S_OK;
}
...
static void MyMemReadNoRevert(BYTE* pCurr, BYTE* Data, long Size)
{
	/*long value;*/
	//assert((Size % 4) == 0);
...
void ShowPinInfo(Pin* pPinObj)
{
    ...
    pr_info("7base = %x sz = %d  rp %x wp %x \n", (UINT32)base, bufferSize, (UINT32)readPtr, (UINT32)writePtr);
}
...
UINT32 MemCopyFromReadPtr(Pin* pPinObj, UINT8* dest, UINT32 size, UINT32 type)
{
    ...
    readPtr = pPinObj->GetReadPtr(pPinObj, 0);
    if ((int)readPtr%4 != 0 && type != AUDIO_BS)
    {
        pr_alert("[%s] wrong ptr:%x\n", __FUNCTION__, (UINT32)readPtr);
        ShowPinInfo(pPinObj);
        //while(1);
...
UINT32 IsPtrInReage(int ptr, int base , int limit)
{
...
UINT32 IsPtrValid(int ptr, Pin* pPinObj)
{
    ...
    return IsPtrInReage(ptr, (int)(&(*base)), (int)(&(*limit)));
}
...
UINT32 AddReadPtr(Pin* pPinObj, UINT32 size)
{
    ...
    //FIXME: pointer to int is incorrect.
    if(!IsPtrValid((int)(&(*readPtr)), pPinObj))
    {
        pr_alert("error ptr b %x \n", (UINT32)readPtr);
        ShowPinInfo(pPinObj);
        /*assert(1==0);*/
    }
...
Pin* new_multiPin(UINT32 bufSize, UINT32 listSize)
{
    ...
    Pin **pinArr = (Pin**)kmalloc(sizeof(UINT32)*(listSize-1), GFP_KERNEL);
    if(pinArr == NULL)
    {
        return NULL;
    }
...
void InitPacketPin(Pin *pPinObj, int isChild)
{
    ...
    InitHeader.pRingBufferHeaderList[0] = (unsigned long)pAddr;
    //MY_LIBK_INFO("[%s] ch0 pAddr:%x\n", __FUNCTION__, pAddr);
    ...
    if(InitHeader.listSize > 1) {
        ...
        for(i = 1; i < InitHeader.listSize; i++) {
            ...
            if(isChild==0) InitHeader.pRingBufferHeaderList[i] = (unsigned long)pAddr;
            //MY_LIBK_INFO("[%s] ch[%d] pAddr:%x,cmdsize:%x\n", __FUNCTION__, i, pAddr,sizeof(SYS_PROCESS_INIT_RING_HEADER));
        }
    }
