...
void IPCWrite_U32(unsigned long *src, unsigned long x)
{
  return IPC_WriteU32((BYTE*)src, x);
}
void IPCWrite_S32(long *src, long x)
{
  return IPC_WriteU32((BYTE*)src, x);
}
...
{
    if(pObj == NULL) return;
    if(pObj->m_phyAddr == 0) return;

    rtkaudio_free(pObj->m_phyAddr);
    kfree(pObj);
    return;
}
ReferenceClock* new_ReferenceClock()
{
    ...
    pObj->m_phyAddr = rtkaudio_alloc(sizeof(REFCLOCK), NULL, &pObj->m_virtualAddr);
    ...
    IPCWrite_U32(&pObj->m_core->videoContext,   (unsigned long)-1);
    IPCWrite_U32(&pObj->m_core->audioContext,   (unsigned long)-1);
    IPCWrite_S32(&pObj->m_core->videoEndOfSegment, (long)-1);
