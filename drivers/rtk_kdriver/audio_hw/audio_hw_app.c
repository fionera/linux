...
static int32_t g_adc1_offset = 15; //+0.5dB
#if defined(SUPPORT_2_SET_ADC) && defined(SUPPORT_ADC_DVC)
...
int32_t Audio_HwpSetBalanceLR(int32_t  balance)
{
	...
	if (balance >= APP_MAX_BAL) {
		balance = APP_MAX_BAL;
		Audio_HwpSetMute(AIO_DISABLE, APP_CH_ID_R, BAL_MUTE_ID);
		Audio_HwpSetMute(AIO_ENABLE, APP_CH_ID_L, BAL_MUTE_ID);
	} else if (balance <= APP_MIN_BAL) {
		balance = APP_MIN_BAL;
		Audio_HwpSetMute(AIO_DISABLE, APP_CH_ID_L, BAL_MUTE_ID);
		Audio_HwpSetMute(AIO_ENABLE, APP_CH_ID_R, BAL_MUTE_ID);
	} else {
		Audio_HwpSetMute(AIO_DISABLE, APP_CH_ID_L|APP_CH_ID_R, BAL_MUTE_ID);
	}
...
int32_t Audio_HwpSetBalanceTS(int32_t  balance)
{
	...
	if (balance >= APP_MAX_BAL) {
		balance = APP_MAX_BAL;
		Audio_HwpSetMuteTS(AIO_ENABLE, APP_CH_ID_L, BAL_MUTE_ID);
	} else if (balance <= APP_MIN_BAL) {
		balance = APP_MIN_BAL;
		Audio_HwpSetMuteTS(AIO_ENABLE, APP_CH_ID_R, BAL_MUTE_ID);
	} else {
		Audio_HwpSetMuteTS(AIO_DISABLE, APP_CH_ID_L|APP_CH_ID_R, BAL_MUTE_ID);
	}

	m_balance_ts_lr = balance;

	if (m_balance_ts_lr > 0) {   // attenuate L channel
		HwpSetVolumeTS(APP_CH_ID_L, m_volume_ts[CH_L] - m_balance_ts_lr);
	} else {                 // attenuate R channel
		HwpSetVolumeTS(APP_CH_ID_R, m_volume_ts[CH_R] + m_balance_ts_lr);
	}
...
void Audio_HwpPrescale(int vol, int ch_id)
{
	int32_t a;
	int32_t b = 0;
	//APP_CH_ID ch_id = (APP_CH_ID)(APP_CH_ID_L|APP_CH_ID_R);
...
int32_t Audio_HwpSetVolume(uint32_t ch_id, int32_t vol)
{
	...
	/*
	    ...
	    {
	    ch_id = ch_id & (~APP_CH_ID_L);
	    ch_id |= APP_CH_ID_C;
	    }
	
	    if(ch_id& APP_CH_ID_R)
	    {
	    ch_id = ch_id & (~APP_CH_ID_R);
	    ch_id |= APP_CH_ID_SW;
	    }
	    }
	*/
...
int32_t Audio_HwpSetVolumeTS(uint32_t ch_id, int32_t vol)
{
	...
	if (ch_id&APP_CH_ID_L) {
		...
		if (vol != APP_MIN_VOL) {
			if (m_balance_ts_lr > 0) // attenuate L channel
				HwpSetVolumeTS(APP_CH_ID_L, vol - m_balance_ts_lr);
			else
				HwpSetVolumeTS(APP_CH_ID_L, vol);
		}
	}
	...
	if (ch_id&APP_CH_ID_R) {
		...
		if (vol != APP_MIN_VOL) {
			if (m_balance_ts_lr < 0) // attenuate R channel
				HwpSetVolumeTS(APP_CH_ID_R, vol + m_balance_ts_lr);
			else
				HwpSetVolumeTS(APP_CH_ID_R, vol);
		}
	}
...
static int set_mute(unsigned int para, unsigned int ch_id)
{
	...
	if (ch_id&APP_CH_ID_LS) {
		...
		DVOL_CH_CSW_REG.hpf_c_enable = AIO_ENABLE; //AIO_ENABLE for RTD2874
	...
	if (ch_id&APP_CH_ID_RS) {
		...
		DVOL_CH_CSW_REG.hpf_sw_enable = AIO_ENABLE; //AIO_ENABLE for RTD2874
		AIO_WriteRegister(AUDIO_CSW_Digital_Volume_Control_reg, DVOL_CH_CSW_REG.regValue);
	}
...
{
//	audio_adc_digital_volume_control_0_RBUS adc_digital_volume_control_0;
	audio_hw_process_control_RBUS            HWP_CTRL_REG;
	...
	/*
	adc_digital_volume_control_0.regValue = Audio_HwpReadRegister(AUDIO_ADC_Digital_Volume_Control_0_reg);
	adc_digital_volume_control_0.adc4_l_inverter_en = 1;
	adc_digital_volume_control_0.adc4_r_inverter_en = 1;
	Audio_HwpWriteRegister(AUDIO_ADC_Digital_Volume_Control_0_reg, adc_digital_volume_control_0.regValue);
	*/
	...
	//Audio_HW_Earc_Arc_Switch(AIO_AUDIO_ARC);
	m_app_inited = true;
...
{
	audio_hw_usleep(40); //Due to some HWP reg control has double buffer, in line with samples, so keep at leaset 20us(48kHz) delay before read reg for safety
	return AIO_ReadRegister(addr);
}

void Audio_HwpWriteRegister(unsigned int addr, unsigned int value)
{
	AIO_WriteRegister(addr, value);
	audio_hw_usleep(40); //Due to some HWP reg control has double buffer, in line with samples, so keep at leaset 20us(48kHz) delay after write reg for safety
}
...
{	
	...
	switch(band_id){
		...
		default:	alog("[%s] over eq bfp num\n",__FUNCTION__);	return AIO_ERROR;
	}
