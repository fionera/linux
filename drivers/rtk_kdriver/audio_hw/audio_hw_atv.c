...
#define NOAUDIOSIGNALCNT (1)
	...
	if((g_user_config == AUDIO_USER_CONFIG_TV001) ||(g_user_config == AUDIO_USER_CONFIG_TV002))	// TV001 start to auto std change when 2019 project
	{
	//	ROS_ENTER_CRITICAL();
...
void AtvChageA2SoundMode(ATV_SOUND_MODE sound_mode)
{
	...
	if(g_CurSifType == ATV_AUDIO_SIF_KOREA_A2_SELECT){
		alog_info("Soundmode = %x, Language = %x.\n",g_A2_soundmode, g_UserTVA2Language);

// Clayton: 2019/9/12, No need to compensate anything due to down 6dB remove
/*
		if(g_A2_soundmode ==  ATV_SOUND_MODE_DUAL && g_UserTVA2Language == 0){
			alog_info("Compenate for Dual Main.\n");
			// [Fix me]
			//HwpSetVolume_SIF(g_sif_offset+3);
		}else{
			HwpSetVolume_SIF(g_sif_offset);
		}
*/			
...
#else
#define  FIRSTTIMECHECKTIME (5)  //8->6
#endif
...
#define NICAM_SOUNDMODE_OUT_CNT 3
			...
			if (g_errorRateDebunce >= g_NicamMaxErrorDebounce) {
				...
				if (g_NicamSignalStable == 0) {
					...
					#if 0 //disable unused code by ryanlan
					if ((audioStatusChanageCallBack != NULL)) {
...
#define  SAP_NOISE_GAP (0)
	...
	if (btsc_sap_status.w1sapqualitygood == 1) {
...
#define BTSC_SOUNDMODE_OUT_CNT 4
	...
	//USER:Peter_Lin DATE:2010/10/12
	// fix btsc sap disapper noise
	#if 0 //ryanlan, disable un-used code
	...
	AudioSifBtscLpfSel(g_btsc_soundmode);
	#if 0 //mark un-used code by ryanlan
...
void  Audio_HwpSIFSetA2StereoDualTH(uint32_t a2_th)
{
	...
	AtvSetA2Parameter();
	alog_info("[Clayton-1015]Set A2 TH %d\n", g_A2_TH_sel);
...
else
{
	...
	audio_hw_usleep(350000);  //delay 300ms
	...
	if(main_sys == ATV_SOUND_AUTO_SYSTEM)
	{
		ATV_SOUND_STD	recheck_sound_std = ATV_SOUND_STD_UNKNOWN;
		ATV_SOUND_STD_MAIN_SYSTEM	recheck_main_sys = ATV_SOUND_UNKNOWN_SYSTEM;

		SIF_4p5 = AIO_ReadRegister(AUDIO_msd_mag_4_5_reg);
		SIF_5p25 = AIO_ReadRegister(AUDIO_msd_mag_5_25_reg);

		if(real_main_sys == ATV_SOUND_I_SYSTEM && abs(SIF_4p5 - SIF_5p25) > ATV_LOW_TH_THRESHOLD_HIGH) //check MN system existed or not
		{
			audio_ifd_set_audio_bp(ATV_AUDIO_BP_SEL_4P000M_5P000M);
			audio_hw_usleep(100000);  //delay 100ms, wait SIF energy stable
			AtvScanSoundStd_LG(&p_sound_std, 1); //Get current HW detect STD
			...
			if(recheck_main_sys == ATV_SOUND_MN_SYSTEM)
			{
	...
	SIF_4p5 = AIO_ReadRegister(AUDIO_msd_mag_4_5_reg);
	SIF_4p28 = AIO_ReadRegister(AUDIO_msd_mag_4_28_reg);
	SIF_4p72 = AIO_ReadRegister(AUDIO_msd_mag_4_72_reg);
	alog_info("SIF_4p5 SIF_4p28 SIF_4p72 %x %x %x\n", SIF_4p5, SIF_4p28, SIF_4p72);

	SIF_5p5 = AIO_ReadRegister(AUDIO_msd_mag_5_5_reg);
	SIF_5p25 = AIO_ReadRegister(AUDIO_msd_mag_5_25_reg);
	SIF_5p74 = AIO_ReadRegister(AUDIO_msd_mag_5_74_reg);
	SIF_5p85 = AIO_ReadRegister(AUDIO_msd_carrier_5_85_reg);
	alog_info("SIF_5p5 SIF_5p25 SIF_5p74 SIF_5p85 %x %x %x %x\n", SIF_5p5, SIF_5p25, SIF_5p74, SIF_5p85);

	SIF_6p0 = AIO_ReadRegister(AUDIO_msd_mag_6_reg);
	SIF_6p25 = AIO_ReadRegister(AUDIO_msd_mag_6_25_reg);
	alog_info("SIF_6p0 SIF_6p25 %x %x\n", SIF_6p0, SIF_6p25);

	SIF_6p5 = AIO_ReadRegister(AUDIO_msd_carrier_6_5_reg);
	SIF_6p552 = AIO_ReadRegister(AUDIO_msd_mag_6_552_reg);
	SIF_6p74 = AIO_ReadRegister(AUDIO_msd_carrier_6_74_reg);
	alog_info("SIF_6p5 SIF_6p552 SIF_6p72 %x %x %x\n", SIF_6p5, SIF_6p552, SIF_6p74);

	msd_result.regValue = AIO_ReadRegister(AUDIO_msd_result_reg);
	alog_info("HW detect STD= %x\n", msd_result.hw_detected_std);
	...
	if (main_sys == ATV_SOUND_AUTO_SYSTEM) {  // unstable signal
		if (real_main_sys == ATV_SOUND_MN_SYSTEM) {
			...
			Audio_HwpSIFGetMainToneMag(real_main_sys, &ToneMag);
			noise_tone[0] = AIO_ReadRegister(AUDIO_msd_mag_4_5_reg);
			noise_tone[1] = AIO_ReadRegister(AUDIO_msd_mag_6_552_reg);
		} else if (real_main_sys == ATV_SOUND_DK_SYSTEM || real_main_sys == ATV_SOUND_L_SYSTEM) {
			Audio_HwpSIFGetMainToneMag(real_main_sys, &ToneMag);
			noise_tone[0] = AIO_ReadRegister(AUDIO_msd_mag_4_5_reg);
			noise_tone[1] = AIO_ReadRegister(AUDIO_msd_mag_4_72_reg);
		} else if (real_main_sys == ATV_SOUND_I_SYSTEM) {
			Audio_HwpSIFGetMainToneMag(real_main_sys, &ToneMag);
			noise_tone[0] = AIO_ReadRegister(AUDIO_msd_mag_4_5_reg);
			noise_tone[1] = AIO_ReadRegister(AUDIO_msd_mag_5_5_reg);
		} else {
	...
	if (main_sys == real_main_sys || main_sys == ATV_SOUND_AUTO_SYSTEM) {
		...
		*pToneSNR = ToneMag;    // Return SIF magnitude
		...
		if (AtvCheckVideoTiming(real_main_sys) != 1) {
			*pToneSNR = 0;
			real_main_sys=ATV_SOUND_UNKNOWN_SYSTEM; //if main_sys == real_main_sys,but SNR=0, return unknown
...
int32_t Audio_HwpSetAtvAudioBand(SIF_INPUT_SOURCE sif_source, ATV_SOUND_STD_MAIN_SYSTEM atv_main_system)
{
	...
	if (sif_source == SIF_FROM_IFDEMOD) { // sap
		...
		case ATV_SOUND_DK_SYSTEM:
			bp_sel = ATV_AUDIO_BP_SEL_6P200M_6P800M;
			break;
...
static int set_sif_mute(unsigned int para)
{
	...
	#endif
	alog_info("%s() Clayton7--set_sif_mute Apply\n", __FUNCTION__);
