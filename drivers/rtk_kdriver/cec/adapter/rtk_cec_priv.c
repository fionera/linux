...
static unsigned long long cec_cfg = CEC_PULL_UP_VALID | CEC_PULL_UP_VALUE(0x17) ; // for backward compatibility


char cec_pull_up_value_Str[32][5]=
{
    // 0
...
char * cec_pull_up_value_string(unsigned int value)
{
    if( value < 32 )
        return cec_pull_up_value_Str[value];
...
{
    unsigned int NCODE, FCODE, PREDIV, O, rbus, pll, pll2;
    ...
    FCODE = PLL_REG_SYS_PLL_BUS2_pllbus_fcode(pll2);
    NCODE = PLL_REG_SYS_PLL_BUS2_pllbus_ncode(pll2);
    PREDIV = PLL_REG_SYS_PLL_BUS1_get_pllbus_prediv(pll);
    O = PLL_REG_SYS_PLL_BUS1_get_pllbus_o(pll);

    pll = (unsigned int) (27*(NCODE+3+FCODE/2048)/(PREDIV+1)/(1<<O));
    rbus = pll/2;
...
static int _read_rx_fifo(char* rx_buff, int rx_buff_size, int eom)
{
    ...
    unsigned long data_len = rtd_inl(RTK_CEC_RXCR0) & 0x1F;
    int rx_entry_cnt = 0;
    int i;

    cec_rx_dbg("rx_fifo_count = %lu, RXCR0=%x, eom=%d\n",
...
static void rtk_cec_pcb_config(void)
{
    unsigned long long param =0 ;

    if (pcb_mgr_get_enum_info_byname("CEC_CFG", &param) == 0)
    {
        cec_cfg = param & 0xFFFFFFFFFFFFFFFF;
        cec_warn("cec_cfg = 0x%llx\n", (unsigned long long)cec_cfg);

        if( PCB_CEC_TX_RETRY_LOAD(cec_cfg) )
        {
            rtk_cec_retry_num = PCB_CEC_TX_RETRY_NUMBER_CHECK(cec_cfg);
        }
    }
    else
    {
        cec_warn("not get pcb cec_cfg value, using default cec_cfg = 0x%llx \n" , cec_cfg);
    }

    cec_warn("cec retry number = %lu \n" , rtk_cec_retry_num );

    if( CEC_PULL_UP_OPTION(cec_cfg) )
    {
        cec_warn("cec enable internal pull up resistor = %s kohms \n", cec_pull_up_value_string( CEC_PULL_UP_VALUE(cec_cfg) ) );
    }
    else
    {
        cec_warn("cec disable internal pull up resistor \n");
    }
...
static void  rtd299x_cec_intial_hw_setting(void)
{
    ...
    write_reg32(RTK_POWER_SAVING_MODE, 0 | CEC_CLK_POL(CEC_POSITIVE_CLOCK) |
        RES_PULL_UP_EN( CEC_PULL_UP_OPTION( cec_cfg ) ) | RES_PULL_UP_SEL( CEC_PULL_UP_VALUE( cec_cfg ) ) );
 ...
 * Retn : 0 : success, others fail
 *------------------------------------------------------------------*/
int rtd299x_cec_set_hotplug(rtk_cec *p_this, unsigned long onoff)
{
    cec_info("rtd299x_cec_set_hotplug, onoff=%lu\n", onoff);
    if (onoff) {
...
static cm_buff *rtd299x_cec_read_message(rtk_cec *p_this,
        unsigned char flags)
{
    while (!(flags & NONBLOCK) && p_this->status.enable& !cmb_queue_len(&p_this->rx_queue) && !pm_freezing)
        wait_event_interruptible(p_this->rcv.wq, !p_this->status.enable|| cmb_queue_len(&p_this->rx_queue) || pm_freezing);
...
{
    /*general setup*/
    write_reg32(RTK_CEC_CR0, CEC_MODE(1) | TEST_MODE_PAD_EN |
                LOGICAL_ADDR(0) | TIMER_DIV(25) |
                PRE_DIV(_get_pre_div()) | UNREG_ACK_EN);
    /*reset engine*/
    write_reg32(RTK_CEC_RTCR0, RETRY_NO(rtk_cec_retry_num));
    write_reg32(RTK_CEC_RXCR0, RX_RST);
    write_reg32(RTK_CEC_RXCR0, 0);
    write_reg32(RTK_CEC_TXCR0, TX_RST);
    write_reg32(RTK_CEC_TXCR0, 0);
    ...
    /*timing setup*/
    write_reg32(RTK_CEC_RXTCR0, RX_START_LOW(0x60) | RX_START_PERIOD(0xC8) |
                RX_DATA_SAMPLE(42) | RX_DATA_PERIOD(PATCH_RX_DATA_PERIOD));
    ...
    write_reg32(RTK_CEC_TXTCR0, TX_START_LOW(149) | TX_START_HIGH(31));
    write_reg32(RTK_CEC_TXTCR1,
...
void rtd299x_cec_dump_current_setting(char *buf, int in_standby)
{
    if(buf == NULL) {
        ...
        cec_info("CEC_RXTCR1(0x%08x) = 0x%08x\n", RTK_CEC_RXTCR1, read_reg32(RTK_CEC_RXTCR1));
        ...
        sprintf(buf + strlen(buf), "CEC_RXTCR1(0x%08x) = 0x%08x\n", RTK_CEC_RXTCR1, read_reg32(RTK_CEC_RXTCR1));
...
static unsigned int rtd299x_cec_setting_time_to_val(unsigned int setting_val)
{
    unsigned int sample_unit = 25;/*us*/
    return setting_val * sample_unit;
}

static int rtd299x_cec_get_register_part(unsigned long reg,
...
static int rtd299x_cec_get_timing(char *attr_name, unsigned int *val)
{
    int ret = 0;
    if(attr_name == NULL || val == NULL)
        return -1;
    if(strcmp(attr_name, "rx_start_low") == 0)
        *val = rtd299x_cec_get_register_part(RTK_CEC_RXTCR0,
    ...
    else if(strcmp(attr_name, "rx_data_period_chk") == 0)
        *val = rtd299x_cec_get_register_part(RTK_CEC_RXTCR1,
                                             CEC_RxTCR1_rx_data_period_chk_mask,
                                             CEC_RxTCR1_rx_data_period_chk_shift);

    else if(strcmp(attr_name, "rx_start_high") == 0)
        *val = rtd299x_cec_get_register_part(RTK_CEC_RXTCR1,
                                             CEC_RxTCR1_rx_start_high_mask,
                                             CEC_RxTCR1_rx_start_high_shift);

    else if(strcmp(attr_name, "tx_start_low") == 0)
        *val = rtd299x_cec_get_register_part(RTK_CEC_TXTCR0,
        ...
        *val = rtd299x_cec_get_register_part(RTK_CEC_TXTCR1,
        ...
        *val = rtd299x_cec_get_register_part(RTK_CEC_RxACKTCR1,
        ...
        *val = rtd299x_cec_get_register_part(RTK_CEC_RxACKTCR1,
        ...
        ret = -1;
    if(ret == 0)
        *val = rtd299x_cec_setting_time_to_val(*val);
    return ret;
}

static unsigned int rtd299x_cec_time_to_setting_val(unsigned int time_usec)
{
    unsigned int sample_unit = 25;/*us*/
    return time_usec / sample_unit;
}
static int rtd299x_cec_set_register_part(unsigned int reg,
...
static int rtd299x_cec_set_timing(char *attr_name, unsigned int val)
{
    int ret = 0;
    unsigned int setting_val;
    if(attr_name == NULL)
        return -1;
    setting_val = rtd299x_cec_time_to_setting_val(val);
    if(strcmp(attr_name, "rx_start_low") == 0)
        ret = rtd299x_cec_set_register_part(RTK_CEC_RXTCR0,
        ...
        ret = rtd299x_cec_set_register_part(RTK_CEC_RXTCR1,
                                             CEC_RxTCR1_rx_data_period_chk_mask,RX_DATA_PERIOD_CHK(setting_val));
    else if(strcmp(attr_name, "rx_start_high") == 0)
        ret = rtd299x_cec_set_register_part(RTK_CEC_RXTCR1,
                                             CEC_RxTCR1_rx_start_high_mask,RX_START_HIGH(setting_val));
    else if(strcmp(attr_name, "tx_start_low") == 0)
        ret = rtd299x_cec_set_register_part(RTK_CEC_TXTCR0,
                                            CEC_TxTCR0_tx_start_low_mask, TX_START_LOW(setting_val));
    else if(strcmp(attr_name, "tx_start_high") == 0)
        ret = rtd299x_cec_set_register_part(RTK_CEC_TXTCR0,
                                            CEC_TxTCR0_tx_start_high_mask, TX_START_HIGH(setting_val));
    else if(strcmp(attr_name, "tx_data_low") == 0)
        ret = rtd299x_cec_set_register_part(RTK_CEC_TXTCR1,
        ...
        ret = rtd299x_cec_set_register_part(RTK_CEC_RxACKTCR1,
...
{
    return rtd299x_cec_set_hotplug((rtk_cec *) cec_get_drvdata(dev), onoff);
}
...
{
    return rtd299x_cec_get_timing(name, p_value);
}

static int ops_set_timing(cec_device *dev, unsigned char *name, unsigned int value)
{
    return rtd299x_cec_set_timing(name, value);
}
