...
unsigned int cec_tx_dbg_en;
unsigned int cec_rx_dbg_en;
static unsigned int cec_record_en;
static cec_recorder recorder;
unsigned int cec_dump_str_setting_en;

extern void rtd299x_cec_dump_current_setting(char *buf, int in_standby);
...
static ssize_t cec_tx_dbg_en_show(struct kobject * kobj, struct kobj_attribute * attr, char *buf)
{
    return sprintf(buf, "cec_tx_dbg_en = %s\n", cec_tx_dbg_en ? "enable" : "disable");
}

static ssize_t cec_tx_dbg_en_store(struct kobject * kobj, struct kobj_attribute * attr, const char *buf, size_t n)
{
    unsigned int val;
...
cec_attr(cec_tx_dbg_en);

static ssize_t cec_rx_dbg_en_show(struct kobject * kobj, struct kobj_attribute * attr, char *buf)
{
    return sprintf(buf, "cec_rx_dbg_en = %s\n", cec_rx_dbg_en ? "enable" : "disable");
}

static ssize_t cec_rx_dbg_en_store(struct kobject * kobj, struct kobj_attribute * attr, const char *buf, size_t n)
{
    unsigned int val;
    if (sscanf(buf, "%u", &val) == 1) {
        cec_rx_dbg_en = val;
...
static ssize_t cec_dump_str_setting_en_show(struct kobject * kobj, struct kobj_attribute * attr, char *buf)
{
    return sprintf(buf, "cec_rx_dbg_en = %s\n", cec_dump_str_setting_en ? "enable" : "disable");
}

static ssize_t cec_dump_str_setting_en_store(struct kobject * kobj, struct kobj_attribute * attr, const char *buf, size_t n)
{
    unsigned int val;
    if (sscanf(buf, "%u", &val) == 1) {
        cec_dump_str_setting_en = val;
        return n;
    }
    return -EINVAL;
}
cec_attr(cec_dump_str_setting_en);

static ssize_t cec_dump_current_setting_show(struct kobject * kobj, struct kobj_attribute * attr, char *buf)
{
    rtd299x_cec_dump_current_setting(buf, 0);
    return strlen(buf);
}
