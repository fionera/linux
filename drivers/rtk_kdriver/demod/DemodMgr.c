...
static long rtk_demod_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_RTK_DEMOD_SETDISEQC_CMD:
		...
		if (copy_from_user((void *)&pDemod_globe->set_diseqc_cmd_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_DISEQC_CMD))) {
			...
			if(pDemod_globe->set_diseqc_cmd_data.length > 8)
			{
				pDemod_globe->set_diseqc_cmd_data.length = 8;
			}
	...
	case IOCTL_RTK_DEMOD_SETDISEQC_2P0_CMD:
		...
		if (copy_from_user((void *)&pDemod_globe->set_diseqc_cmd_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_DISEQC_CMD))) {
			...
			pDemod_globe->m_pDemod->SetDiseqc2p0Cmd(pDemod_globe->m_pDemod, &(pDemod_globe->set_diseqc_cmd_data.length), pDemod_globe->set_diseqc_cmd_data.data);
			if (copy_to_user((void __user *)arg, &pDemod_globe->set_diseqc_cmd_data, sizeof(RTK_DEMOD_SET_DISEQC_CMD)))
