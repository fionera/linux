...
void REALTEK_A_DVBSX_Constructors(DEMOD* pDemodKernel, unsigned char addr, unsigned char output_mode, unsigned char output_freq, COMM* pComm)
{
	...
	pDemodKernel->private_data = kmalloc(sizeof(REALTEK_A_DVBSX), GFP_KERNEL);
	...
	pDemodKernel->GetTsClkRate = REALTEK_A_DVBSX_GetTsClkRate;
...
int REALTEK_A_DVBSX_GetSignalQuality(DEMOD* pDemodKernel, ENUM_TV_QUAL id, TV_SIG_QUAL* pQuality)
{
	...
	case TV_QUAL_SIGNAL_QUALITY:
		#ifdef CONFIG_CUSTOMER_TV006
		RTK_A_DVBSX_GetDVBSxSignalQuality(pDemod, &Sqi);
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_A_DVBSX_BlindScan_AcquireSignal(DEMOD* pDemodKernel, unsigned int *SymbolRateKhz, int *Fcfo, unsigned char *bTpLock)
{
...
re_bs_acq:
	...
	*Fcfo = LockedCFO*1000;
	...
	#ifdef DYNAMIC_TPO_CLK
	REALTEK_A_DVBSX_INFO("\033[1;32;31m" "BS Acquire %u   "PT_U32BITS"    Demod Lock = %u, Global Lock Time = %lu, Local Lock Time = %lu, TPO Time = %lu, Snr = %d, CoarseCFO = %d KHz, LockedCFO = %d KHz\n" "\033[m", 
		re_acq_cnt, pRealtekIDVBSX->m_info.dvbs.symbol_rate/1000, (unsigned int)lock, etime-acquire_sig_stime_s, etime-stime, tpo_etime-tpo_stime, Snr, CoarseCFO, LockedCFO);
	#else
	REALTEK_A_DVBSX_INFO("\033[1;32;31m" "BS Acquire %u   "PT_U32BITS"    Demod Lock = %u, Global Lock Time = %lu, Local Lock Time = %lu, Snr = %d, CoarseCFO = %d KHz, LockedCFO = %d KHz\n" "\033[m", 
		re_acq_cnt, pRealtekIDVBSX->m_info.dvbs.symbol_rate/1000, (unsigned int)lock, etime-acquire_sig_stime_s, etime-stime, Snr, CoarseCFO, LockedCFO);
	#endif
...
{
	REALTEK_A_DVBSX_DRIVER_DATA* pDriver = (REALTEK_A_DVBSX_DRIVER_DATA*)kmalloc(
...
int REALTEK_A_DVBSX_GetTsClkRate(DEMOD* pDemodKernel, unsigned int *pTsClkRate)
{
	int ret = TUNER_CTRL_OK;
	REALTEK_A_DVBSX* pRealtekIDVBSX;
	DVBSX_DEMOD_MODULE* pDemod;
	...
	pDemodKernel->Lock(pDemodKernel);
	*pTsClkRate = pRealtekIDVBSX->m_ts_clk_khz;
	REALTEK_A_DVBSX_DBG("\033[1;32;33m" "[REALTEK_A_DVBSX_GetTsClkRate] TsClkKHz = %u\n" "\033[m", *pTsClkRate);
	pDemodKernel->Unlock(pDemodKernel);
