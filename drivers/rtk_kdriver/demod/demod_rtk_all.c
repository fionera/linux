...
int REALTEK_ALL_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO* pSigInfo)
{
	...
	if(rtd_inl(DTV_FRONTEND_DTV_FRONTEND_DUMMY5_reg) == 0x0A0A0000) {
		DEMOD_INFO("\033[0;32;31m""DEMOD into wintool mode. Disable WebOS control !!!!!\n""\033[m");
		return TUNER_CTRL_OK;
	}
		...
		if (pRealtekAll != NULL) {
			if (pRealtekAll->m_kernel_auto_tune_en == 1) {
				...
				if (pRealtekAll->m_pDemod) {
					...
					if ((PLL27X_REG_IFADC_1_get_pll27x_ifadc_dtv_cko_en(rtd_inl(PLL27X_REG_IFADC_1_reg)) == 0x1) && (!(IsDvbs2Sys(((DEMOD*)pDemodKernel)->m_tv_sys) || IsDvbsSys(((DEMOD*)pDemodKernel)->m_tv_sys))))
#else
					if (PLL27X_REG_IFADC_1_get_pll27x_ifadc_dtv_cko_en(rtd_inl(PLL27X_REG_IFADC_1_reg)) == 0x1)
#endif
					{
				...
				if (pRealtekAll->m_KernelAutoTuneThread_start == 0)//if remove, it would cause STR fail
					break;
