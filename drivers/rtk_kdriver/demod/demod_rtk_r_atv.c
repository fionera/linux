...
int REALTEK_R_ATV_GetLockStatus(DEMOD* pDemodKernel, unsigned char *pLock)
{
	...
	int  FreqOffset = 0, PreFreqOffset = 0;
	...
	if (1) {
		...
		if ((lock_count > 7) && (ABS(FreqOffset, PreFreqOffset) < 5000)) {
			*pLock = 1;
		} else {
			*pLock = 0;
		}
...
{
	unsigned long   stime, stime_start, stime_CrLock;
...
atv_detect_end:
		...
		if (1) {
			...
			while ((tv_osal_time() - stime) <= 60) {
				tv_osal_msleep(5);
				pDemodKernel->GetLockStatus(pDemodKernel, &lock);
				//DEMOD_INFO("signal lock = %d, time=%ld\n", lock, tv_osal_time() - stime);
				if (lock)
					break;
			...
			if (Param.id == TV_TUNER_SONY_SUTRX243) {
				if (lock) {
				...
				DEMOD_INFO("cr_lock = %d, cr_lock_time=%ld, agc_lock = %d, agc_lock_time=%ld\n", lock, stime_CrLock, AgcLock, tv_osal_time() - stime);
			} else {

				DEMOD_INFO("cr_lock = %d, cr_lock_time=%ld\n", lock, stime_CrLock);
			}
			...
			drv_ifd_set_power_diff_cr_lock(0);
                ...
                DEMOD_INFO("signal lock(final) = %d \n", lock);
		if (lock) {
