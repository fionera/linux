...
void REALTEK_R_DVBC_Constructors(DEMOD* pDemodKernel, unsigned char addr, unsigned char output_mode, unsigned char output_freq, COMM* pComm)
{
	...
	pDemodKernel->private_data = kmalloc(sizeof(REALTEK_R_DVBC), GFP_KERNEL);
	...
	pDemodKernel->GetTsClkRate = REALTEK_R_DVBC_GetTsClkRate;
...
//	pDemodKernel->Unlock = Demod_Unlock;
	...
	pRealtekRDVBC->Activate = REALTEK_R_DVBC_Activate;
	pRealtekRDVBC->Destory = REALTEK_R_DVBC_Destructors;
}
...
int REALTEK_R_DVBC_Init(DEMOD* pDemodKernel)
{
	...
	//FUNCTION_NAME_PRINT("REALTEK_R_DVBC::Init\n");

	//////////////////////////////////////////////////
	//QAM_DEMOD_MODULE* pDemod = ((REALTEK_R_DVBC_DRIVER_DATA*) pRealtekRDVBC->m_private)->pDemod;
	//////////////////////////////////////////////////
...
int REALTEK_R_DVBC_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO* pSigInfo)
{
	...
	tv_osal_msleep(40);//AGC stable issue
...
int REALTEK_R_DVBC_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	...
	if(cnt == 5)
	{
		...
		kp_cr = DVBC_KP_CR_EXP_INC_get_kp_cr_exp_inc(rtd_inl(0xB8158C64));
		if ((kp_cr & 0x8) == 0x8)
			cr_rst = -(0x10-kp_cr);
		else
			cr_rst = kp_cr;
	...
	cnt++;
	REALTEK_R_DVBC_INFO("\033[1;32;36m" "lock1 = %u, lock2 = %u, lock3 = %u fsm_state_o=%u, kp_cr=%d\n" "\033[m", Val1, Val2, Val3, fsm_state_o, cr_rst);
	...
	if (*pLock == 1) {
		pRealtekRDVBC->m_unlock_cnt = 0;
		pRealtekRDVBC->m_autotune_stime = tv_osal_time() + AUTODETECT_DVBC_SIG_UPDATE_INTERVAL_MS;
	} else {
		pRealtekRDVBC->m_unlock_cnt = pRealtekRDVBC->m_unlock_cnt + 1;
	}
...
int REALTEK_R_DVBC_AcquireSignalThread(DEMOD* pDemodKernel)
{
	...
	if(IsNoSignal == 1)
	{
		REALTEK_R_DVBC_INFO("\033[1;32;31m" "DVBC NO SIGNAL!!! Total Acq Time="PT_U32BITS", AutoMode Time="PT_U32BITS", IsNoSignal=%u, CSR="PT_S32BITS" KHz, FSR="PT_S32BITS" Hz, CCFO="PT_S32BITS" Hz, QamMode=0x%x\n" "\033[m",
			tv_osal_time() - pRealtekRDVBC->m_acquire_stime, auto_mode_etime - pRealtekRDVBC->m_acquire_stime, IsNoSignal, Coarse_SymbolRate, Fine_SymbolRate, est_ccfo, Qamd);
	...
	while ((tv_osal_time() - stime) <= 1500) {
		...
		if(lock == 1)
		{
			...
			if(pDemodKernel->m_ScanMode != CH_MANUAL_MODE)
			{
				REALTEK_R_DVBC_INFO("\033[1;32;36m" "DVBC LOCK!!! Total Acq Time="PT_U32BITS", AutoMode Time="PT_U32BITS", Local Time="PT_U32BITS", IsNoSignal=%u, CSR="PT_S32BITS" KHz, FSR="PT_S32BITS" Hz, CCFO="PT_S32BITS" Hz, QamMode=0x%x, Snr="PT_S32BITS"/10000, fsm_state=%u, tr=%u\n" "\033[m", 
					tv_osal_time() - pRealtekRDVBC->m_acquire_stime, auto_mode_etime - pRealtekRDVBC->m_acquire_stime, tv_osal_time() - stime, IsNoSignal, Coarse_SymbolRate, Fine_SymbolRate, est_ccfo, Qamd, snr, fsm_state_o, tr_ld_o);
			}
			else
			{
				REALTEK_R_DVBC_INFO("\033[1;32;36m" "DVBC LOCK!!! Total Acq Time="PT_U32BITS", Local Time="PT_U32BITS", Snr="PT_S32BITS"/10000, fsm_state=%u, tr=%u\n" "\033[m", 
					tv_osal_time() - pRealtekRDVBC->m_acquire_stime, tv_osal_time() - stime, snr, fsm_state_o, tr_ld_o);
			}
	...
	if(lock == 0)
	{
		if(pDemodKernel->m_ScanMode != CH_MANUAL_MODE)
		{
			REALTEK_R_DVBC_INFO("\033[1;32;31m" "DVBC UNLOCK!!! Total Acq Time="PT_U32BITS", AutoMode Time="PT_U32BITS", Local Time="PT_U32BITS", IsNoSignal=%u, CSR="PT_S32BITS" KHz, FSR="PT_S32BITS" Hz, CCFO="PT_S32BITS" Hz, QamMode=0x%x, Snr="PT_S32BITS"/10000, fsm_state=%u, tr=%u\n" "\033[m", 
				tv_osal_time() - pRealtekRDVBC->m_acquire_stime, auto_mode_etime - pRealtekRDVBC->m_acquire_stime, tv_osal_time() - stime, IsNoSignal, Coarse_SymbolRate, Fine_SymbolRate, est_ccfo, Qamd, snr, fsm_state_o, tr_ld_o);
		}
		else
		{
			REALTEK_R_DVBC_INFO("\033[1;32;31m" "DVBC UNLOCK!!! Total Acq Time="PT_U32BITS", Local Time="PT_U32BITS", Snr="PT_S32BITS"/10000, fsm_state=%u, tr=%u\n" "\033[m", 
				tv_osal_time() - pRealtekRDVBC->m_acquire_stime, tv_osal_time() - stime, snr, fsm_state_o, tr_ld_o);
		}
		//pRealtekRDVBC->m_auto_tune_en = 1;
	}
...
AcquireSignal_fail:
	REALTEK_R_DVBC_INFO("\033[1;32;31m" "DVBC UNLOCK!!! Total Acq Time="PT_U32BITS", Local Time="PT_U32BITS", fsm_state=%u, tr=%u\n" "\033[m", tv_osal_time() - pRealtekRDVBC->m_acquire_stime, tv_osal_time() - stime, fsm_state_o, tr_ld_o);
	DtvDemodTPOutEn(1); //enable TS output
	pDemodKernel->Unlock(pDemodKernel);
	return TUNER_CTRL_FAIL;
}
...
int REALTEK_R_DVBC_KernelAutoTune(DEMOD* pDemodKernel)
{
	...
	//if(m_auto_tune_en)
	//{
		if (((cur_time > pRealtekRDVBC->m_autotune_stime) || ((pRealtekRDVBC->m_autotune_stime - cur_time) > AUTODETECT_DVBC_SIG_UPDATE_INTERVAL_MS)) && pRealtekRDVBC->m_unlock_cnt > 0)
		{
			REALTEK_R_DVBC_INFO("[DVBC] AUTO DETECT FLOW !!!\n");
			pRealtekRDVBC->m_acquire_sig_en = 1;
			REALTEK_R_DVBC_AcquireSignalThread(pDemodKernel);
			REALTEK_R_DVBC_TSBitRateCheck(pDemodKernel);
			pRealtekRDVBC->m_autotune_stime = tv_osal_time() + AUTODETECT_DVBC_SIG_UPDATE_INTERVAL_MS;
...
{
	REALTEK_R_DVBC_DRIVER_DATA* pDriver = (REALTEK_R_DVBC_DRIVER_DATA*) kmalloc(
...
int REALTEK_R_DVBC_GetTsClkRate(DEMOD* pDemodKernel, unsigned int *pTsClkRate)
{
	int ret = TUNER_CTRL_OK;
	REALTEK_R_DVBC* pRealtekRDVBC;
	QAM_DEMOD_MODULE* pDemod;
	...
	pDemodKernel->Lock(pDemodKernel);
	*pTsClkRate = pRealtekRDVBC->m_ts_clk_KHz;
	REALTEK_R_DVBC_DBG("\033[1;32;33m" "[REALTEK_R_DVBC_GetTsClkRate] TsClkKHz = %u\n" "\033[m", *pTsClkRate);
	pDemodKernel->Unlock(pDemodKernel);
