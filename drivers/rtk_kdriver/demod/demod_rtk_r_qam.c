...
int REALTEK_R_QAM_AcquireSignalThread(DEMOD* pDemodKernel, unsigned char WaitSignalLock)
{
	...
	unsigned char IsNoSignal, pre_64qam = 0, lock;
	unsigned long stime;
	int pre_qam = 0, IsNoSignalTr = 0;
	S32BITS Powdiff = 0, SymbolRateDet = 0;
	...
	pDemodKernel->Lock(pDemodKernel);
	pDemod->GetQamMode(pDemod, &pre_qam);

	if (pre_qam == QAM_QAM_64) pre_64qam = 1;
	...
	REALTEK_R_QAM_INFO("[SRS]IsNoSignal = %u, SymbolRateDet = "PT_S32BITS", pre_64qam = %u, SRS Time = %lu\n", IsNoSignal, SymbolRateDet, pre_64qam, tv_osal_time() - stime);
