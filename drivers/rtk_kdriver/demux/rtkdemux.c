...
do {                                                                             \
    if (pCh->lumpsumErrCount > 0) {                                              \
        ...
        for (i = 0; i < pCh->lumpsumErrCount; i++) {                             \
            ...
            dmx_crit(pCh->tunerHandle, "Err: '%s' (%d times)\n", pCh->lumpsumMsg[i], repeat);      \
            repeat = 1;                                                          \
        }                                                                        \
    }                                                                            \
...
void DEMUX_IPCCopyMemory(char *des, char *src, unsigned long len)
{
	...
	if ((((long)src & 0x3) != 0) || (((long)des & 0x3) != 0) || ((len & 0x3) != 0))
		dmx_err(CHANNEL_UNKNOWN, "error in pli_IPCCopyMemory()...\n") ;
...
inline long DEMUX_checkTaskCompletion(uint64_t serialNumber)
{
	...
	while (smd_waitComplete(0, timeout, &unused) == false) {
		dmx_crit(CHANNEL_UNKNOWN,"Demux: waitComplete more than %lu ms\n", timeout);
	}
#endif
	return 1;
}
...
inline int DeliverInBandCommand(DEMUX_BUF_PIN_T *pPinInfo, unsigned char *Data, long size)
{
	...
	*pPinInfo->pWrPtr = DEMUX_reverseInteger(phyWr) ;
	//dmx_mask_print(DEMUX_NOMAL_DEBUG, CHANNEL_UNKNOWN, "%s, %d,  ------+++++++++++++----\n", __func__, __LINE__);
	return 0;
}
/*
*   checkfilterCount
...
{
    void *context = (void *)(ULONG)((channel->tunerHandle << 8) | index) ;
    mutex_lock(&channel->section_mutex);
...
int DEMUX_GetSecRingBuffer(void *pContext, size_t count)
{
    ...
    if ((demux_device->secBufCnt + count) > SEC_FILTER_BUFFER_UNIT_CNT) {
        ...
        dmx_err(CHANNEL_UNKNOWN,"[%s %d] section buffer is enough(used %d, new %ld)!!\n", __func__, __LINE__, demux_device->secBufCnt, (ULONG)count);
        return -1;
    }
    ...
    dmx_err(CHANNEL_UNKNOWN, "[%s %d] something wrong, need debugging!! (requested %zu, gotten %zu)\n", __func__, __LINE__, count, acquired);
    return -1;
}
...
void DEMUX_FreeSecRingBuffer(void *pContext, size_t count)
{
    ...
    for (;;) {
        if (demux_device->secMap[released].occupied &&
            ...
            dmx_err(CHANNEL_UNKNOWN, "[%s %d] free sec buffer fail!\n", __func__, __LINE__);
            return;
        }
    }
...
int DEMUX_SendAudioDesc(AUDIO_AD_INFO *pAdInfo)
{
	...
	if (adBuf.virAddr == 0)
	{
		dmx_err(CHANNEL_UNKNOWN, "[%s %d] out of memory\n", __func__, __LINE__);
		return -1;
	}
	...
	dmx_dbg(CHANNEL_UNKNOWN, "[%s %d] send_rpc_command start\n", __func__, __LINE__);
	dmx_dbg(CHANNEL_UNKNOWN, "[%s %d] msgID = 0x%x, value[0] = 0x%x, value[1] = 0x%x, value[2] = 0x%x, value[3] = 0x%x, value[4] = 0x%x\n",
		__func__, __LINE__, pConfig->msgID, pConfig->value[0], pConfig->value[1], pConfig->value[2], pConfig->value[3], pConfig->value[4]);
	if (send_rpc_command(RPC_AUDIO, 0x204, adBuf.phyAddr, adBuf.size, &ret) == RPC_FAIL)
		dmx_err(CHANNEL_UNKNOWN, "[%s %d] RPC fail\n", __func__, __LINE__);

	dmx_dbg(CHANNEL_UNKNOWN, "[%s %d] send_rpc_command end\n", __func__, __LINE__);
...
int DeliverPrivateInfo(demux_channel *pCh, DEMUX_BUF_PIN_T *pPinInfo, int infoId, char *pInfo, int length, DEMUX_BUF_PIN_T *pBSPinInfo)
{
	...
	{
		dmx_err(CHANNEL_UNKNOWN, "%s, %d, Param Check FAIL \n", __func__, __LINE__);
		return -1;
	}
	if (pCh->isECP)
	{
	...
	if (pCh->state == DEMUX_STATE_RUNNING && pPinInfo->pRBH) {
		...
		if (phyWr < (uintptr_t)pPinInfo->pBufferLower
			...
			dmx_err(pCh->tunerHandle, "func %s, line %d, infoId %d, r 0x%lx, w 0x%lx, l 0x%lx, u 0x%lx\n",
...
*/
		...
		if (infoId == PRIVATEINFO_PTS) {
			...
			//dmx_err(pCh->tunerHandle, " r 0x%lx, w 0x%lx, l 0x%lx, u 0x%lx : pts:%lld\n", phyRd, phyWr, (unsigned long)(uintptr_t)pPinInfo->pBufferLower, (unsigned long)(uintptr_t)pPinInfo->pBufferUpper, (*(int64_t *)pInfo));
		...
		if (infoId == PRIVATEINFO_VIDEO_NEW_SEG) {
			...
			cmd.wPtr = DEMUX_reverseInteger(*pBSPinInfo->pWrPtr) ;
			dmx_crit(pCh->tunerHandle, "[%s %d]CH%d NEW_SEG pointer to BS 0x%x\n", __func__, __LINE__, pCh->tunerHandle, cmd.wPtr);
			res =  DeliverInBandCommand(pPinInfo, (unsigned char *)&cmd, cmd.header.size) ;
		}
		...
		if (infoId == PRIVATEINFO_AUDIO_FORMAT) {
			...
			dmx_crit(pCh->tunerHandle, "[%s %d]CH%d Aduio NEW_FORMAT pointer to BS 0x%lx\n", __func__, __LINE__, (int)pCh->tunerHandle, ((AUDIO_DEC_NEW_FORMAT *)pInfo)->wPtr);
	...
	if (res <0 ){
		dmx_err(pCh->tunerHandle, " [%s %d] DeliverPrivateInfo FAIL infoId = %d , ret=%d\n"DGB_COLOR_NONE , __FILE__, __LINE__, infoId, res) ;
	}
	return res ;
}
...
int DEMUX_DeliverNavBufCommands(demux_channel *pCh, NAVBUF *pNavBuffer)
{
	...
	default:
		dmx_err(pCh->tunerHandle, "Unknown bufferType : %d\n", pNavBuffer->type) ;
	}
...
int SyncWritePointers(demux_channel *pCh, unsigned int WP_update_flags, unsigned char *WP[])
{
	...
	if (pCh->bUseDDRCopy && pCh->latestDDRCopyHandle != (uint64_t)-1) {
		...
		if (((++g_stat_count)&0xFFF) == 0)
			dmx_notice(pCh->tunerHandle,"DEMUX_DDRCopy_Stat (0/100/1000/10000/slp) : %d/%d/%d/%d/%d\n", g_stat[0], g_stat[1], g_stat[2], g_stat[3], g_stat[4]);
		/* ------------------------------------------------ */
	}
...
#if defined(CONFIG_RTK_KDRV_RPC) && defined(CONFIG_RTK_KDRV_VDEC) && defined(CONFIG_RTK_KDRV_AUDIO)
int DEMUX_DeliverData(demux_channel *pCh, NAVDEMUXOUT *pDemuxOut)
{
	...
	if (pCh->isECP){
		return DEMUX_TA_ECP_DeliverData(pCh, (unsigned long)pDemuxOut - (unsigned long)pCh);
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////
			...
			if ((pCh->avSyncMode == NAV_AVSYNC_FREE_RUN || (pCh->avSyncMode==NAV_AVSYNC_PCR_MASTER && pCh->pcrProcesser.bVideoFreerun)) &&
				...
				if (writableSize <= (1*1024*1024 +512*1024))   {
					...
					unsigned char videoport = (i ==VIDEO_PIN ?0 : 1);
					long ignore;
					videoport = demux_device->VTP_Info[videoport].destPort;
					dmx_mask_print(DEMUX_VOFLUSH_DEBUG, pCh->tunerHandle,"flush vo -----------(videopin:%d, vdecPort:%d)>\n", i, videoport);
...
//////////////////////////////////////////////////////////////////////////////////////////////
			if (writableSize < totalPayloadSize[i]) {
				...
				if (++g_debug_ring_buffer_full_count > 0 && abs64(debug_time-ring_buffer_debug_time) > 3*DEMUX_CLOCKS_PER_SEC) {
					dmx_alert(pCh->tunerHandle, DGB_COLOR_RED_WHITE" RTKDemux: (CH%d)(%s) ring buffer full (%d times), Drop = %d, WP = %lxh, RP = %lxh, size = %ld, req-size = %d\n"DGB_COLOR_NONE,
						...
						if (pCh->tp_src != MTP) {
							dmx_alert(pCh->tunerHandle, "Please check %s decoder or pcr-master logic\n", rtkdemux_outputNames[i]);
				...
				if (pCh->tp_src == MTP && rtkdemux_dbg_esbufmonitor_en) {
					if (IS_AUDIO_PIN(pNavBuffer->data.pinIndex) || pCh->pcrProcesser.lastDemuxAudioPTS == -1) {
						if (PVR_CheckESBufStable(pCh)) {
							...
							dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle,  "ch%d, deliverdata in stepMode: deliverNum:%d \n", pCh->tunerHandle,pDemuxOut->numBuffers );
						}
					}
				}
...
/*			if (!got) {
			...
			if (!got) {
				...
				if(pCh->active_video_pid == pCh->pidActiveList[i].PID){
					...
					dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle,  "%s_%d ch:%d, target:%d, pid:0x%x, dest:%d flushed\n" ,
		...
		for (i = 0; i < pCh->bypassPidFilterCount; i++) {
			...
			if (j == count) {
				...
				count++ ;
				//dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle,  "ch:%d, bypass Pid:0x%x\n" , pCh->tunerHandle, pCh->bypassPidFilterList[i]);
		...
		/*	if ( (pCh->tp_src== MTP)&&(pCh->activeTarget[i].pid == -1 )){
				//removed
				Demux_Flush(pCh, i);
				//dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle,  "ch:%d, target:%d flushed\n" , pCh->tunerHandle, i);
			}*/
		}
		...
		if (rm_count) {
			dmx_notice(pCh->tunerHandle,"%s,(CH%d)UpdateDemuxPID(Remove):\n", __func__, pCh->tunerHandle) ;
			for (i = 0 ; i < rm_count ; i++) {
				dmx_notice(pCh->tunerHandle,"PID = %d\n", rm_pid[i]) ;
			}
		}

		dmx_notice(pCh->tunerHandle,"%s,(CH%d)UpdateDemuxPID(All):\n", __func__, pCh->tunerHandle) ;
		for (i = 0 ; i < count ; i++) {
			dmx_notice(pCh->tunerHandle,"PID = %d, KeySel = %d\n", add_pidEntry[i].PID, add_pidEntry[i].KeySel) ;
		}
...
/*
		if (HAS_FLAG(pCh->events, EVENT_UPDATE_PCR)) {
            ...
            dmx_mask_print(DEMUX_PCRTRACK_DEBUG,  pCh->tunerHandle,  "ch:%d, pcr pid:%d \n", pCh->tunerHandle, pCh->PCRPID);
...
{/*handle mutex  each item self , do not lock whole function*/
	...
	if (HAS_FLAG(pCh->status, STATUS_PCRTAK_START)){
		if (Demux_MW_enablePCRTracking(pCh)<0){
			dmx_mask_print(DEMUX_PCRTRACK_DEBUG,  pCh->tunerHandle, "\033[1;36m [%s %d] ch:%d, fail to start PcrTracking \033[m\n", __func__ , __LINE__ ,pCh->tunerHandle );
		}
		else{
			RESET_FLAG(pCh->status, STATUS_PCRTAK_START);
			dmx_mask_print(DEMUX_PCRTRACK_DEBUG|DEMUX_NOTICE_PRINT,  pCh->tunerHandle, "%s-%d, ch:%d, start PcrTracking....\n", __func__, __LINE__,pCh->tunerHandle);
		}
	}
	...
	if (HAS_FLAG(pCh->status, STATUS_SET_AUDIO_PIN_INFO)) {
		if (DEMUX_BUF_UpdateChannelAudioPinStatus(pCh)<0){
			dmx_mask_print(DEMUX_NOTICE_PRINT|DEMUX_BUFFER_DEBUG, pCh->tunerHandle, "[%s, %d] ch:%d  update pin status fail ,need retry\n", __func__, __LINE__, pCh->tunerHandle);
		}
		else
		{
			dmx_mask_print(DEMUX_NOTICE_PRINT|DEMUX_BUFFER_DEBUG, pCh->tunerHandle, "[%s, %d] ch:%d finish update pin status \n", __func__, __LINE__, pCh->tunerHandle);
			RESET_FLAG(pCh->status, STATUS_SET_AUDIO_PIN_INFO);
		}
	}
#endif
	if (HAS_FLAG(pCh->status, STATUS_SET_VIDEO_PIN_INFO)) {
		if (DEMUX_BUF_UpdatePinInfo(pCh, 1)<0){
			dmx_mask_print(DEMUX_NOTICE_PRINT|DEMUX_BUFFER_DEBUG,  pCh->tunerHandle,  "[%s, %d] ch:%d  update VIDEO pin status fail ,need retry\n", __func__, __LINE__, pCh->tunerHandle);
		}
		else
		{
			dmx_mask_print(DEMUX_NOTICE_PRINT|DEMUX_BUFFER_DEBUG,  pCh->tunerHandle,  "[%s, %d] ch:%d finish update pin status \n", __func__, __LINE__, pCh->tunerHandle);
			RESET_FLAG(pCh->status, STATUS_SET_VIDEO_PIN_INFO);
		}
	}

	if (HAS_FLAG(pCh->status, STATUS_FLUSH_VIDEO)) {
		DEMUX_EVENT_FlushVideo(pCh, VIDEO_PIN);
		RESET_FLAG(pCh->status, STATUS_FLUSH_VIDEO);
	}

	if (HAS_FLAG(pCh->status, STATUS_FLUSH_VIDEO2)) {
		DEMUX_EVENT_FlushVideo(pCh, VIDEO2_PIN);
		RESET_FLAG(pCh->status, STATUS_FLUSH_VIDEO2);
	}
	if (HAS_FLAG(pCh->status, STATUS_FLUSH_AUDIO)) {
		DEMUX_EVENT_FlushAudio(pCh, VIDEO_PIN);
		RESET_FLAG(pCh->status, STATUS_FLUSH_AUDIO);
	}

	if (HAS_FLAG(pCh->status, STATUS_FLUSH_AUDIO2)) {
		DEMUX_EVENT_FlushAudio(pCh, VIDEO2_PIN);
    ...
    if (HAS_FLAG(pCh->status, STATUS_FLUSH_MTP)) {
		...
		dmx_notice(CHANNEL_UNKNOWN, "[%s %d] RHAL_MTPStreamControl start\n", __func__, __LINE__);
		RHAL_MTPStreamControl(mtp_id, MTP_STREAM_START);
		dmx_notice(CHANNEL_UNKNOWN, "[%s %d] RHAL_MTPStreamControl end\n", __func__, __LINE__);
...
void HandleEvents(demux_channel *pCh, NAVBUF *pBuffer)
{
	if (HAS_FLAG(pCh->events, EVENT_VIDEO0_NEW_DECODE_MODE)) {
		if (DeliverPrivateInfo(pCh, &pCh->pinInfo[VIDEO_PIN].bufInfo[DEMUX_STREAM_TYPE_IB], PRIVATEINFO_VIDEO_NEW_DECODE_MODE, (char *)&pCh->videoDecodeMode[0], sizeof(pCh->videoDecodeMode[0]), &pCh->pinInfo[VIDEO_PIN].bufInfo[DEMUX_STREAM_TYPE_BS])<0){
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_NEW_DECODE_MODE FAIL \n", __func__, __LINE__, pCh->tunerHandle);
			return;
		}
	...
	if (HAS_FLAG(pCh->events, EVENT_VIDEO1_NEW_DECODE_MODE)) {
		if (DeliverPrivateInfo(pCh, &pCh->pinInfo[VIDEO2_PIN].bufInfo[DEMUX_STREAM_TYPE_IB], PRIVATEINFO_VIDEO_NEW_DECODE_MODE, (char *)&pCh->videoDecodeMode[1], sizeof(pCh->videoDecodeMode[1]), &pCh->pinInfo[VIDEO2_PIN].bufInfo[DEMUX_STREAM_TYPE_BS])<0){
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_NEW_DECODE_MODE FAIL \n", __func__, __LINE__, pCh->tunerHandle);
			return;
		}
	...
	if (HAS_FLAG(pCh->events, EVENT_SET_VIDEO_START)) {
		if (demux_device->pinMap[VIDEO_PIN].chIndex != pCh->tunerHandle){
			...
			dmx_warn(pCh->tunerHandle, "%s,%d, ch:%d,invalid command, channel do not owner pin[%d], giveup videoFlush\n", __func__, __LINE__, pCh->tunerHandle, VIDEO_PIN);
			RESET_FLAG(pCh->events, EVENT_SET_VIDEO_START);
		}
		else
		{
			if (DEMUX_EVENT_StartVideo(pCh, pCh->videoDecodeMode[0], VIDEO_PIN)<0){
				dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d, ch:%d, FlushVideo (%d)FAIL, try latter  \n", __func__,__LINE__, pCh->tunerHandle, VIDEO_PIN);
			}
			else
				RESET_FLAG(pCh->events, EVENT_SET_VIDEO_START);
		}
	...
	if (HAS_FLAG(pCh->events, EVENT_SET_AUDIO_FORMAT)) {
		if (demux_device->pinMap[AUDIO_PIN].chIndex != pCh->tunerHandle){
			dmx_warn(pCh->tunerHandle, "%s,%d, ch:%d,invalid command, channel do not owner pin[%d], giveup audioFormat Set\n", __func__, __LINE__, pCh->tunerHandle, AUDIO_PIN);
			...
			if (DEMUX_EVENT_AudioNewFormat(pCh, AUDIO_PIN, pCh->audioFormat, pCh->audioPrivateInfo)<0){
				dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d, ch:%d, SetAudioFormat (%d)FAIL, try latter  \n", __func__,__LINE__,pCh->tunerHandle, AUDIO_PIN);
			}
			else
				RESET_FLAG(pCh->events, EVENT_SET_AUDIO_FORMAT);
		}
	...
	if (HAS_FLAG(pCh->events, EVENT_FLUSH_AD_INFO)) {
		dmx_notice(CHANNEL_UNKNOWN, "[%s %d] handle EVENT_FLUSH_AD_INFO\n", __func__, __LINE__);
	...
	if (HAS_FLAG(pCh->events, EVENT_ENTER_ECP_MODE) && (pCh->isECP ==0)) {
		int pin = 0;
		dmx_notice(pCh->tunerHandle, "[%s %d] handle EVENT_ENTER_ECP_MODE\n", __func__, __LINE__);

 		DEMUX_TA_EnterECP(pCh);
 		...
 		if (pCh->tpBuffer.phyAddr){
			...
			//todo: set protection
			RHAL_VBM_EnableProtect(pCh->tunerHandle, 0, 0);
			RHAL_TP_EnableProtect(pCh->tunerHandle, pCh->tpBuffer.phyAddr,  pCh->tpBuffer.phyAddr + pBuf->allocSize);
 		}
 		//unmap video es. set protection
 		for (pin=0;pin< MAX_VDEC_NUM;pin++){
 			if (demux_device->VTP_Info[pin].SrcCh == pCh->tunerHandle && demux_device->VTP_Info[pin].bsBuf.phyAddr)
 			{
				DEMUX_BUF_T *pBuf  = &demux_device->VTP_Info[pin].bsBuf;
 				dvr_unmap_memory(pBuf->isCache ? (void *)pBuf->cachedaddr : (void *)pBuf->nonCachedaddr, pBuf->size);
				pBuf->virAddr = pBuf->nonCachedaddr = NULL;
				//TODO, SEC PROTECTION
				RHAL_ES_EnableProtect(pCh->tunerHandle, demux_device->VTP_Info[pin].bsBuf.phyAddr,  demux_device->VTP_Info[pin].bsBuf.phyAddr+pBuf->size);
 			}
 		}
 		pCh->isECP = 1;
	...
	if (HAS_FLAG(pCh->events, EVENT_EXIT_ECP_MODE)) {
		unsigned int pin = 0;
		dmx_notice(pCh->tunerHandle, "[%s %d] handle EVENT_EXIT_ECP_MODE\n", __func__, __LINE__);
 		DEMUX_TA_ExitECP(pCh);
 		//MAP TP BUFFER
 		if (pCh->tpBuffer.phyAddr){
			DEMUX_BUF_T *pBuf   = &pCh->tpBuffer;
			//TODO, CANCEL PROTECTION
			RHAL_TP_DisableProtect(pCh->tunerHandle);
			RHAL_VBM_DisableProtect(pCh->tunerHandle);
...
#else
				if (pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB].pRBH)
					DEMUX_Buf_SetPinInfo(&pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB], (RINGBUFFER_HEADER *)pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB].pRBH, &pCh->ibAudioBuf[pin/2]);
				if (pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS].pRBH)
					DEMUX_Buf_SetPinInfo(&pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS], (RINGBUFFER_HEADER *)pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS].pRBH, &pCh->bsAudioBuf[pin/2]);
#endif
			}
			if (IS_VIDEO_PIN(pin) &&(demux_device->VTP_Info[pin/2].SrcCh == pCh->tunerHandle)){
				if (pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB].pRBH)
					DEMUX_Buf_SetPinInfo(&pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB], (RINGBUFFER_HEADER *)pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB].pRBH, &demux_device->VTP_Info[pin/2].ibBuf);
				if (pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS].pRBH)
					DEMUX_Buf_SetPinInfo(&pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS], (RINGBUFFER_HEADER *)pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS].pRBH, &demux_device->VTP_Info[pin/2].bsBuf);

			}
			//dmx_emerg(pCh->tunerHandle, "%s_%d: pin:%d, buf:%d, w:%p, r:%p, offset:%ld\n", __func__, __LINE__, pin, DEMUX_STREAM_TYPE_BS,pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS].pWrPtr, pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS].pRdPtr, pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS].phyAddrOffset );
			//dmx_emerg(pCh->tunerHandle, "%s_%d: pin:%d, buf:%d, w:%p, r:%p, offset:%ld\n", __func__, __LINE__, pin, DEMUX_STREAM_TYPE_IB,pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB].pWrPtr, pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB].pRdPtr, pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB].phyAddrOffset );
...
static int DEMUX_GetWaitTime(demux_channel *pCh)
{
	...
	if (byteRate) {
        ...
        if (pPinInfo->pRBH != NULL) {
                const uintptr_t phyWr = DEMUX_reverseInteger(pPinInfo->pRBH->writePtr);
                const uintptr_t phyRd = DEMUX_reverseInteger(pPinInfo->pRBH->readPtr[0]);
...
int DEMUX_EVENT_FlushVideo(demux_channel *pCh,  int pin)
{
	if (pCh == NULL)
	{
		dmx_err(CHANNEL_UNKNOWN, "%s_%d: ERROR!!!, invalid param\n", __func__, __LINE__);
		return -1;
	}
...
int DEMUX_EVENT_StartVideo(demux_channel *pCh, DECODE_MODE decMode, int pin)
{
	...
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_VIDEO_NEW_SEG, NULL, sizeof(NEW_SEG), pPinInfo)<0){
		dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_NEW_SEG FAIL \n", __func__, __LINE__,pCh->tunerHandle);
		return -1;
	}
	/* ANDROIDTV-227 issue */
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_VIDEO_INBAND_CMD_TYPE_SOURCE_DTV, NULL, sizeof(INBAND_SOURCE_DTV), pPinInfo)<0){
		dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_INBAND_CMD_TYPE_SOURCE_DTV FAIL \n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_VIDEO_DECODE_COMMAND, (char *)&cmd, sizeof(DECODE), pPinInfo)<0){
		dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_DECODE_COMMAND FAIL \n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
...
int DEMUX_EVENT_StartAudio(demux_channel *pCh,  int pin)
{
	...
	{
		dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_AUDIO_NEW_SEG FAIL \n", __func__, __LINE__,pCh->tunerHandle);
		return -1;
	}
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_AUDIO_DECODE_COMMAND, (char *)&cmd, sizeof(AUDIO_DEC_DECODE) , pPinInfo)<0)
	{
		dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_AUDIO_DECODE_COMMAND FAIL \n", __func__, __LINE__,pCh->tunerHandle);
		return -1;
	}
...
int DEMUX_EVENT_AudioNewFormat(demux_channel *pCh, int pin, DEMUX_AUDIO_DEC_T audioFormat, s32 privateInfo[8])
{
	...
	{
		dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "%s, %d: ch:%d Deliver PRIVATEINFO_AUDIO_FORMAT FAIL !!!!\n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
	dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG|DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle, "%s,%d, ch:%d, pin:%d, audioFormat:%d\n", __func__, __LINE__, pCh->tunerHandle, pin, audioFormat);
	...
	*/
	if (pCh->tp_src == MTP && rtkdemux_dbg_tpratecontrol_en) {
		...
		if (videoSystemPTS != -1) {
			...
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle,  "[%s %d] Check audio stable start ....\n", __func__, __LINE__);
		}
...
bool DEMUX_channelStatusCheck(demux_channel *pCh, NAVBUF *pBuffer, bool isProhibited){
	...
	if (pCh->bUpdateDemuxTarget) {
		...
		if (pCh->bUpdateDemuxTarget) {
			...
			if (DEMUX_BUF_UpdatePinInfo(pCh, 1) < 0)
				SET_FLAG(pCh->status, STATUS_SET_VIDEO_PIN_INFO);
...
int DEMUX_ReadData(demux_channel *pCh, NAVBUF *pBuffer)
{
	...
	if (pBuffer->data.payloadSize) {
		...
		//dmx_info(pCh->tunerHandle, "%s_%d: payload:%p, payloadSize:%d\n", __func__, __LINE__,pBuffer->data.payloadData, pBuffer->data.payloadSize);
	...
	if (pLastReadAddr && pBuffer->data.payloadSize > 0 && pLastReadAddr != pBuffer->data.payloadData)
		dmx_crit(pCh->tunerHandle, "[%s %d] ERROR: read ptr shift : last = 0x%p, now = 0x%p, tp = 0x%p/0x%lx\n", __func__, __LINE__, pLastReadAddr, pBuffer->data.payloadData, pCh->tpBuffer.nonCachedaddr, (ULONG)pCh->tpBuffer.phyAddr);
...
void ParsePCR(demux_channel *pCh, unsigned char *pTsPacket)
{
	...
	//dmx_crit(pCh->tunerHandle, "[%s]PCR = %lld\n", __func__, PCR);
}
...
void DEMUX_UpdateADInfo(demux_channel *pCh, int target, unsigned char AD_fade_byte, unsigned char AD_pan_byte, char AD_gain_byte_center, char AD_gain_byte_front, char AD_gain_byte_surround)
{
	if(pCh->adInfo.AD_fade_byte != AD_fade_byte || pCh->adInfo.AD_pan_byte != AD_pan_byte
		...
		dmx_notice(pCh->tunerHandle,"[ad] %s :AD_fade_byte %d AD_pan_byte %d AD_gain_byte_center %d AD_gain_byte_front %d AD_gain_byte_surround %d\n",
...
bool ParsePacketCB(demux_channel *pCh, int target, unsigned char *pPacketStart,  int phyAddrOffset)
{
	...
	if (pTarget->contiCounter == -1)
		...
		if (pTarget->contiCounter == count) {/* duplicated packet */
			...
			if (pTarget->contiCounter != count) { /* continuity_counter is not continuous */
				DMX_WARNING_CC((pCh->tunerHandle,"ParsePacketCB: [target:%d] detect discontinuous continuity-counter (%d != %d)\n",
	...
	if (size > 0) {
		...
		space = BS_DISTANCE(pCurWr, rd, pMap->pBufferLower, pMap->pBufferUpper);
		...
		if (space <= size +DEMUX_PESDATA_PREFIX_LENGTH) { /* ringbuffer full, re-sync StartUnit */
			dmx_warn(pCh->tunerHandle, "func %s, PES ringbuffer full, skip this unit(space %u, size %d)!!\n", __func__, space, size) ;
			dmx_warn(pCh->tunerHandle,
				"func %s, rd 0x%x, wr 0x%x, pLower 0x%x, pUpper 0x%x\n",
				__func__, rd, wr, pMap->pBufferLower,  pMap->pBufferUpper);
			pMap->size = 0;
			return false;
		}
		...
		if (isStartUnit) { /* reset buffer */
			...
			*/
			//pes prefix length
			*(unsigned char *)(pCurWr - pMap->phyAddrOffset) = ((pMap->unitSize-DEMUX_PESDATA_PREFIX_LENGTH)>>8) &0xff;
			if (pCurWr+1 >= pMap->pBufferUpper)
			{
				pCurWr =pMap->pBufferLower;
				*(unsigned char *)(pCurWr - pMap->phyAddrOffset) = (pMap->unitSize-DEMUX_PESDATA_PREFIX_LENGTH) &0xff;
				pCurWr++;
			}
			else{
			 	*(unsigned char *)(pCurWr - pMap->phyAddrOffset +1)= (pMap->unitSize-DEMUX_PESDATA_PREFIX_LENGTH) &0xff;
				pCurWr+=2;
			}
		...
		if (pMap->size > pMap->unitSize) { /* skip this pes */
			dmx_err(pCh->tunerHandle, "func %s, PES size is unmatch (accumulated %d,unit %d)!!\n", __func__, pMap->size, pMap->unitSize) ;
		...
		if (pCurWr + size >= pMap->pBufferUpper) {
			...
			memcpy((unsigned char *)(pCurWr - pMap->phyAddrOffset), pData, space) ;
			memcpy((unsigned char *)(pMap->pBufferLower - pMap->phyAddrOffset), pData + space, size - space) ;
		} else {
			memcpy((unsigned char *)(pCurWr - pMap->phyAddrOffset), pData, size) ;
		}
		...
		if (pMap->size == pMap->unitSize) {
			...
			if(wr > (uintptr_t)pCurWr){
				...
				DEMUX_flushMemory((unsigned char *)(wr - pMap->phyAddrOffset), (unsigned char *)(unsigned long)wr, space);
				DEMUX_flushMemory((unsigned char *)(pMap->pBufferLower - pMap->phyAddrOffset),(unsigned char *) (unsigned long)pMap->pBufferLower, pMap->unitSize - space);
			} else {
				rtkdemux_dbg_dump_pes(pCh->tunerHandle, (char *)(wr - pMap->phyAddrOffset), pMap->unitSize, pCh->activePESTarget[target].pid);
				DEMUX_flushMemory((unsigned char *)(wr - pMap->phyAddrOffset), (unsigned char *)(unsigned long)wr, pMap->unitSize);
			}
...
/*    #endif */
		...
		if (/*target == TARGET_AUDIO_2ND && */HAS_FLAG(pCh->demuxTargetInfo[target].events, EVENT_UPDATE_AD_INFO)
			...
			dmx_notice(pCh->tunerHandle,"[ad] EVENT_UPDATE_AD_INFO AD_fade_byte = 0x%x\n",pCh->adInfo.AD_fade_byte);
			/*
	...
	if (pCh->demuxTargetInfo[target].contiCounter == -1)
		...
		if (pCh->demuxTargetInfo[target].contiCounter == count) { /* duplicated packet*/
			...
			if (pCh->demuxTargetInfo[target].contiCounter != count) { /* continuity_counter is not continuous */
				DMX_WARNING_CC((pCh->tunerHandle, "[CH%d]ParsePacket: [target:%d] [target_pid:%d]detect discontinuous continuity-counter (%d != %d)\n",
...
void Demux_Flush(demux_channel *pCh, int target)
{
	...
	if (pCh == NULL || target<0 ||target >= DEMUX_NUM_OF_TARGETS){
		dmx_err(CHANNEL_UNKNOWN, "%s_%d: invalid param target:%d\n", __func__, __LINE__, target);
		return;
	}
	...
	for ( i=0;i<pCh->demuxOutReserved.numBuffers; i++)
	{
		if (pCh->demuxOutReserved.pNavBuffers[i].data.pinIndex == target){
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle,  "%s_%d, flush dirty data of pin %d ,pts:%lld\n", __func__, __LINE__, target, pCh->demuxOutReserved.pNavBuffers[i].data.pts);
	...
	if(pCh->tp_src == MTP){
		if (DEMUX_IS_AUDIO_TARGET(target)){
			REFCLOCK * const pRefClock = DEMUX_GetReferenceClock(pCh);
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle,  "%s_%d, reset pin:%d,  \n", __func__, __LINE__, target);
...
int Demux_Parse(demux_channel *pCh, NAVDEMUXIN *pDemuxIn, NAVDEMUXOUT *pDemuxOut)
{
	...
	if (pCh->isECP){
		int ret = -1;
		//dmx_emerg(pCh->tunerHandle, "%s_%d: ++++  (phy:%x size:%x)begin:%p, end:%p, cur:%p\n", __func__, __LINE__, virt_to_phys((void*)pCh), sizeof(demux_channel),pDemuxIn->pBegin, pDemuxIn->pEnd, pDemuxIn->pCurr);
...
void DEMUX_Read(demux_channel *pCh)
{
	...
	while (1) {
		...
		//dmx_info(pCh->tunerHandle,"%s_%d: beginning:%p, end:%p\n", __func__, __LINE__, pDemuxIn->pCurr,pDemuxIn->pEnd);
...
int DEMUX_ThreadProc(demux_channel *pCh)
{
	...
	while (!kthread_should_stop()) {
		...
		if (pCh->startStreaming && !pCh->bIsSDT && DEMUX_HasReferenceClock(pCh)) {
			...
			if (rtkdemux_dbg_tpratecontrol_en && HAS_FLAG(pCh->status, STATUS_CHECK_PVR_AUDIO_STABLE)) {
				...
				if (CLOCK_GetPTS() - pCh->startCheckAudioStable >= 18000
					...
					dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle,  "[%s %d] disable TP rate control!", __func__, __LINE__);
				}
				/* if audio becomes master, no need to check */
				if (refClock->mastership.masterState == AUTOMASTER_IS_MASTER) {
					dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle,  "[%s %d] audio becomes master!", __func__, __LINE__);
					RESET_FLAG(pCh->status, STATUS_CHECK_PVR_AUDIO_STABLE);
				}
...
#define _IS_VIDEO_PIN_(x) (x == VIDEO_PIN || x == VIDEO2_PIN)
	...
	if (pCh->bPendingOnFullness) {
		...
		if ((pCh->avSyncMode == NAV_AVSYNC_AUDIO_ONLY && _IS_AUDIO_PIN_(pin)) ||
			...
			if (pCh->avSyncStartupFullness > 0 && bEnough) {
				...
				dmx_crit(pCh->tunerHandle, "[%s %d] BufferingEnd\n", __func__, __LINE__);
			}
		...
		if (pCh->bPendingOnFullness) {
			if (REFCLK_GetRCD(DEMUX_GetReferenceClock(pCh)) != -1) {
				PVR_PvrStartToPlay(pCh);
				dmx_crit(pCh->tunerHandle, "[%s %d] Buffering Timeout\n", __func__, __LINE__);
			}
		}
	}
	if (rtkdemux_dbg_tpratecontrol_en
	&& REFCLK_GetRCD(DEMUX_GetReferenceClock(pCh)) != -1
	...
	&& !HAS_FLAG(pCh->status, STATUS_APVR_PLAYBACK_START)) {
		...
		if (refClock->mastership.masterState == AUTOMASTER_IS_MASTER && pCh->pcrProcesser.lastDemuxAudioPTS - audioSystemPTS >= 4500) {
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle, "[%s %d] enable rate control again!\n", __func__, __LINE__);
...
void DEMUX_UpdateAFMode(demux_channel *pCh)
{
	if(pCh->avSyncMode == NAV_AVSYNC_AUDIO_MASTER_AF && pCh->bCheckAFState == true) {
		...
		if(REFCLK_GetRCD(pRefClock) != -1) {
			...
			dmx_crit(pCh->tunerHandle, "[%s %d] audio is master\n", __func__, __LINE__);
...
void DEMUX_ResetAVSync(demux_channel *pCh, AV_SYNC_MODE avSyncMode)
{
	...
	dmx_crit(pCh->tunerHandle, "[%s %d] (CH%d) change avsync mode = %d\n", __func__, __LINE__, pCh->tunerHandle, pCh->avSyncMode);
}
...
)
{
    ...
    if (pCh == NULL)
        return;
    processor = &pCh->pcrProcesser;

	if (!PCRSYNC_IsUnderFreeRunMode(processor))
		return;

    if (RHAL_GetPCRTrackingStatus(pCh->tunerHandle, &newPcr, &newStc, &ignore, &ignore ) != TPK_SUCCESS)
        return;

    //const REFCLOCK * const refClock =    (REFCLOCK *)pCh->refClockBuffer.virAddr;
    if (REFCLK_GetRCD(DEMUX_GetReferenceClock(pCh)) == -1)
        return;
...
void DEMUX_CheckPrintDemuxInfo(demux_channel *pCh)
{
	...
	if(TIMEOUT_CHECK_DEMUX_INFO * DEMUX_CLOCKS_PER_SEC <= passed__ ) {
		...
		if(pCh->read_TP_size == 0){
			if((pCh->active_video_pid > 0) && (pCh->active_video_pid != 0x1FFF) ){
				dmx_notice(pCh->tunerHandle,"(CH%d)RTKDEMUX ERROR(video PID:0x%x): should check TP side\n", pCh->tunerHandle, pCh->active_video_pid);
			}
			if((pCh->active_audio_pid > 0) && (pCh->active_audio_pid  != 0x1FFF)){
				dmx_notice(pCh->tunerHandle,"(CH%d)RTKDEMUX ERROR(audio PID:0x%x): should check TP side\n", pCh->tunerHandle, pCh->active_audio_pid);
			}
		}
		pCh->read_TP_size = 0;
		...
		if((pCh->preAudioSystemPTS == curAudioSystemPTS) && (pCh->pcrProcesser.lastDemuxAudioPTS > 0)){
			dmx_notice(pCh->tunerHandle,"(CH%d)RTKDEMUX ERROR : should check ADEC side, audio system PTS(%lld) no change during %d sec\n", pCh->tunerHandle, curAudioSystemPTS, TIMEOUT_CHECK_DEMUX_INFO);
		}
		if((pCh->preVideoSystemPTS == curVideoSystemPTS) && (pCh->pcrProcesser.lastDemuxVideoPTS > 0)){
			dmx_notice(pCh->tunerHandle,"(CH%d)RTKDEMUX ERROR : should check VDEC side, video system PTS(%lld) no change during %d sec\n", pCh->tunerHandle, curVideoSystemPTS, TIMEOUT_CHECK_DEMUX_INFO);
		}

		if(curRCD == -1 && curRCD == pCh->preRCD)
			dmx_notice(pCh->tunerHandle,"(CH%d)RTKDEMUX: RCD(%lld) no delivered during %d sec\n", pCh->tunerHandle, curRCD, TIMEOUT_CHECK_DEMUX_INFO);
	...
	if (pCh->preMasterState != curMasterState) {
		dmx_notice(pCh->tunerHandle,"(CH%d)RTKDEMUX: mastership mode %d, s %d, v %d, a %d, st %d, PcrCnt %d\n"
			, pCh->tunerHandle
			, pCh->avSyncMode
