...
{\
	...
	if (CurrentPTS - LastPrintPTS > 90090)	{\
		dmx_warn(CHANNEL_UNKNOWN,"WARNING[%d]:", WarningCount+1);	\
		dmx_warn(CHANNEL_UNKNOWN,format, ##__VA_ARGS__);	\
...
typedef struct {
	...
	unsigned int  *pWrPtr ;   /* pointer to ringbuffer wrPtr */
	unsigned int  *pRdPtr ;   /* pointer to ringbuffer rdPtr */
	unsigned int   pBufferLower ; /*  lower bound of buffer (physical)*/
	unsigned int   pBufferUpper ; /*  upper bound of buffer (physical)*/
	long           phyAddrOffset; /*  physical address - non cached address (or cached address for bitstream ringbuffer)*/
...
typedef struct 
{
	...
	UINT8   isECP;
	//=====
	DEMUX_BUF_STAT_T 	vBufStatus;
	DEMUX_BUF_T         bsBuf;  /* BS ringbuffer of video*/
	DEMUX_BUF_T         bsBufH; /* BS RINGBUFFER_HEADER of video*/
	DEMUX_BUF_T         ibBuf;  /* IB ringbuffer of video*/
	DEMUX_BUF_T         ibBufH; /* IB RINGBUFFER_HEADER of video*/
}DEMUX_VTP_T;
...
typedef struct {
	...
	int active_video_pid;
	DEMUX_PES_DEST_T  active_video_dest;
	unsigned long video_deliver_size;
	int active_audio_pid;
	DEMUX_PES_DEST_T  active_audio_dest;
	unsigned long audio_deliver_size;
	unsigned long read_TP_size;
	...
	/* Bypass PID list*/
	short bypassPidFilterList[DEMUX_MAX_NUM_TO_SET];
	unsigned int   bypassPidFilterCount ;

	struct { /* record set tp input config */
...
typedef struct {
	...
	DEMUX_VTP_T     VTP_Info[MAX_VDEC_NUM];
...
int IOCTL_DeliverySetMode(DEMUX_DELIVERY_SET_MODE_T *pClockRecovery);
int IOCTL_DeliveryRequestData(DEMUX_DELIVERY_CHANNEL_T *pClockRecovery);
int IOCTL_DeliveryCancelData(DEMUX_DELIVERY_CHANNEL_T *pClockRecovery);
int IOCTL_DeliveryGetData(DEMUX_DELIVERY_READ_DATA_T *pClockRecovery);
int IOCTL_SetDDRProtect(DEMUX_DDR_PROTECT *pdp);

//int IOCTL_VTP_SetECPMode(DEMUX_VTP_ECP_MODE_T vtpEcp);
