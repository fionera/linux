...
void LRegress_Reset(DEMUX_LINEARREGRESSION_T *pLRegress)
{
    ...
    dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, CHANNEL_UNKNOWN, "%s %d LRegress_Reset !!!\n",__func__, __LINE__);
}
...
int LRegress_InsertDataPair(DEMUX_LINEARREGRESSION_T *pLRegress , LINEARPAIR pair)
{
        ...
        (pLRegress->m_dataPair[(pLRegress->m_pairIdx-1+LINEAR_REGRESSION_SAMPLE_NUM)%LINEAR_REGRESSION_SAMPLE_NUM].y== pLRegress->m_dataPair[pLRegress->m_pairIdx].y))
    {
        ...
        dmx_mask_print(DEMUX_LR_DEBUG, CHANNEL_UNKNOWN, "%s,%d :: pcr not change drop it: pcr:%lld, stc:%lld \n", __func__, __LINE__, pair.y, pair.x);
        return -1;
    }
...
void LRegress_PopDataPair(DEMUX_LINEARREGRESSION_T *pLRegress)
{
    if (pLRegress->m_pairNum >0)
    {        
        dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, CHANNEL_UNKNOWN,"[LRegress] [%s_%d] LR drop data: (%lld,%lld), base(%lld, %lld)\n",
	...
	*/
    ...
    else
    {
        ...
        if(abs64(pLRegress->m_a - MULTIPLE_PRECISION) > SLOPE_PRECISION){
            tmp_y = 0;
            dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, CHANNEL_UNKNOWN,"LRegress-> abnomal ma \n");
        }
        else {
...
{
    int index = 0;
    dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, CHANNEL_UNKNOWN,"LRegress ->LRegress_dumpDatabase callerInfomation:%s %d\n", caller, callLine);
    dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, CHANNEL_UNKNOWN,"LRegress ->ma:%lld, mb:%lld, size:%lld, base:(%lld,%lld)\n", pLRegress->m_a, pLRegress->m_b, pLRegress->m_pairNum, pLRegress->m_BasePair.x, pLRegress->m_BasePair.y);
    for (index = pLRegress->m_pairIdx - pLRegress->m_pairNum; index<pLRegress->m_pairIdx; index++)
    {
        int id = (index+LINEAR_REGRESSION_SAMPLE_NUM)%LINEAR_REGRESSION_SAMPLE_NUM;
        dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, CHANNEL_UNKNOWN,"LRegress-> (%lld, %lld)\n", pLRegress->m_dataPair[id].x, pLRegress->m_dataPair[id].y)
    }
}