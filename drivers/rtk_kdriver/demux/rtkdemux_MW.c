...
int Demux_MW_enablePCRTracking(demux_channel *pCh)
{
	...
	if (pCh->pPcrRecoveryManager) {
		if( 1 == pCh->bPcrTrackEnable) {
			...
			dmx_mask_print(DEMUX_PCRTRACK_DEBUG,  pCh->tunerHandle, "\033[1;36m [%s %d] (dtv%d)Set tp&ref clk=%d\033[m\n", __func__ , __LINE__ , pCh->tunerHandle,clk_src );
			...
			if( TPK_SUCCESS == iClkRet ) {
				...
				if (PCRTRACK_Start(pCh->pPcrRecoveryManager, pCh->tunerHandle) ==false ){
					dmx_mask_print(DEMUX_PCRTRACK_DEBUG,  pCh->tunerHandle, "\031[1;36m [%s %d] ch:%d, fail to start PcrTracking \033[m\n", __func__ , __LINE__ ,pCh->tunerHandle );
					return -1;
				}
			}
			else{
				dmx_notice(pCh->tunerHandle, "\033[1;31m [%s %d](dtv%d)Set tp clk=%d Failed,ret=%d,PID=0x%x\033[m\n", __func__ , __LINE__ , pCh->tunerHandle , clk_src , iClkRet , pCh->PCRPID);
				return -1;
			}
		...
		dmx_err(pCh->tunerHandle, "[%s,%d] fail to enable pcrtracking  , no support pcrtracking in ch: %d \n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
	return 0;
}
...
int Demux_MW_disablePCRTracking(demux_channel *pCh)
{
	...
	if(pCh->pcrProcesser.bUseFixedRCD){
		clk_src = REFCLK_GetClockMode(DEMUX_GetReferenceClock(pCh));
		dmx_must_print("\033[1;36m [%s %d] bUseFixedRCD =true RCD=%lld\033[m\n", __func__ , __LINE__ , REFCLK_GetRCD(pCh->pcrProcesser.pRefClock));
	}
	dmx_mask_print(DEMUX_PCRTRACK_DEBUG,  pCh->tunerHandle, "\033[1;36m [%s %d] Ch =%d\033[m\n", __func__ , __LINE__ , pCh->tunerHandle );
