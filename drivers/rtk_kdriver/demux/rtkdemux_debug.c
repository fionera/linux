...
enum {
	...
	DEMUX_DBG_CMD_ECP_MODE = 23,
	DEMUX_DBG_CMD_TP_RATE_CONTROL = 24,
...
static const char *rtkdemux_cmd_str[] = {
	...
	"tpratecontrol=",	/* DEMUX_DBG_CMD_TP_RATE_CONTROL */
	"log_onoff=",		/* DEMUX_DBG_CMD_LOG_ON_OFF */
};
...
unsigned char rtkdemux_dbg_tpratecontrol_en = 0;
...
*/
			...
			for (j = 0 ; j < demux_device->numberOfVD ; j++) {
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d BS: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, demux_device->VTP_Info[j].bsBuf.allocSize, demux_device->VTP_Info[j].bsBuf.size, demux_device->VTP_Info[j].bsBuf.size - free_size, free_size);
				total_allocated += demux_device->VTP_Info[j].bsBuf.allocSize;
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d IB: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, demux_device->VTP_Info[j].ibBuf.allocSize, demux_device->VTP_Info[j].ibBuf.size, demux_device->VTP_Info[j].ibBuf.size - free_size, free_size);
				total_allocated += demux_device->VTP_Info[j].ibBuf.allocSize;
			}
...
static inline void rtkdemux_dbg_EXECUTE(const int cmd_index, char **cmd_pointer)
{
	...
	case DEMUX_DBG_CMD_PCR_TRACK:
	{
		...
		if (pcrtack_en == 1) {
			...
			Demux_MW_enablePCRTracking(pCh);
	...
	case DEMUX_DBG_CMD_ECP_MODE:
	{
		DEMUX_DDR_PROTECT info={};
		int ecpEn = -1;
		if (*cmd_pointer == NULL) {
			DMX_DBG_LOG(DGB_COLOR_RED"rtkdemux_debug:%d ECP SET COMMAND is incorrect.\n"DGB_COLOR_NONE, __LINE__);
			break;
		}
		...
		token = strsep(cmd_pointer, delim);
		if (token == NULL || kstrtoint(token, 0, &ecpEn) != 0) {
			DMX_DBG_LOG(DGB_COLOR_RED"rtkdemux_debug:%d failed to parse tp index.\n"DGB_COLOR_NONE, __LINE__);
			break;
		}
		info.ch = tp_index;
		info.val = ecpEn;
		DMX_DBG_LOG("CH:%d ,ecp mode =%d\n",  info.ch, info.val);
		IOCTL_SetDDRProtect(&info);
		break;
	}
	case DEMUX_DBG_CMD_TP_RATE_CONTROL:
	{
		rtkdemux_dbg_parse_value(*cmd_pointer, &parsed_data);
		rtkdemux_dbg_tpratecontrol_en = (unsigned char)parsed_data;
		DMX_DBG_LOG("rtkdemux_dbg_tpratecontrol_en=%d\n", rtkdemux_dbg_tpratecontrol_en);
		break;
	}
	default:
	...
	loff_t *ppos)
{
	...
	if (copy_from_user(str, buf, count)) {
		DMX_DBG_LOG("rtkdemux_debug:%d copy_from_user failed! (buf=%p, count=%zu)\n", __LINE__, buf, count);
		return -EFAULT;
	}
	...
	if (cmd_index < DEMUX_DBG_CMD_NUMBER && strlen(rtkdemux_cmd_str[cmd_index])<  RTKDEMUX_MAX_CMD_LENGTH) {
		cmd_pointer = &str[strlen(rtkdemux_cmd_str[cmd_index])];
	}

	if (cmd_pointer == NULL || cmd_pointer > &str[RTKDEMUX_MAX_CMD_LENGTH-1] ){
		DMX_DBG_LOG("rtkdemux_debug:%d >>%s\n", __LINE__, str);
		return -EFAULT;
	}
...
static ssize_t rtkdemux_status_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
{
    ...
    printk(KERN_WARNING "rtkdemux_status_read = %d\n", proc_out_size);
