...
#define dmx_common_print(level, ch, fmt, ...)  \
	{if (ch<DEMUX_TP_NUM){rtd_printk(level, DEMUX_TAG_NAME"-%d", fmt, ch, ##__VA_ARGS__); } \
	else{rtd_printk(level, DEMUX_TAG_NAME, fmt, ##__VA_ARGS__); }}

#define dmx_emerg(ch, fmt, ...)			dmx_common_print(KERN_EMERG, 	ch, fmt, ##__VA_ARGS__);
#define dmx_alert(ch, fmt, ...)			dmx_common_print(KERN_ALERT,	ch, fmt, ##__VA_ARGS__);
#define dmx_crit(ch, fmt, ...)			dmx_common_print(KERN_CRIT,		ch, fmt, ##__VA_ARGS__);
#define dmx_err(ch, fmt, ...)			dmx_common_print(KERN_ERR,		ch, fmt, ##__VA_ARGS__);
#define dmx_warn(ch, fmt, ...)			dmx_common_print(KERN_WARNING,	ch, fmt, ##__VA_ARGS__);
#define dmx_notice(ch, fmt, ...)		dmx_common_print(KERN_NOTICE,	ch, fmt, ##__VA_ARGS__);
#define dmx_info(ch, fmt, ...)    		dmx_common_print(KERN_INFO,		ch, fmt, ##__VA_ARGS__);
#define dmx_dbg(ch, fmt, ...)     		dmx_common_print(KERN_DEBUG,	ch, fmt, ##__VA_ARGS__);
...
enum DEMUX_LOG_MASK_BIT{
	...
	DEMUX_LOG_MASK_BIT_16 = 1<< 16,
		DEMUX_AUDIOPTS_DEBUG = DEMUX_LOG_MASK_BIT_16,
	DEMUX_LOG_MASK_BIT_17 = 1<< 17,
		DEMUX_VOFLUSH_DEBUG = DEMUX_LOG_MASK_BIT_17,
	DEMUX_LOG_MASK_BIT_18 = 1<< 18,
	DEMUX_LOG_MASK_BIT_19 = 1<< 19,
};
#define dmx_mask_print(BIT, ch, fmt, ...)	\
	{if ((BIT) & rtkdemux_dbg_log_mask) {dmx_emerg(ch, fmt, ##__VA_ARGS__);} \
	else {if ((BIT) & DEMUX_NOTICE_PRINT)		dmx_notice(ch, fmt, ##__VA_ARGS__); }}
...
extern unsigned char rtkdemux_dbg_log_onoff;
...
extern unsigned char rtkdemux_dbg_tpratecontrol_en;
