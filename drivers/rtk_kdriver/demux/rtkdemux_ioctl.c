...
{\
	if (x > 0x1fff) { \
		dmx_warn(CHANNEL_UNKNOWN,"func %s, line %d, Error(pid : 0x%x)!!\n", __func__, __LINE__, x); \
		return -ENOTTY; \
	} \
}
...
static INT32 TP_RequestSecBufferCB(void *pContext, UINT8 **ppBuf, UINT16 size)
{
	...
	if (size <= 0 || size > SEC_FILTER_BUFFER_EACH_SIZE) {
		dmx_err(pCh->tunerHandle, "%s, size(%d) is error!\n", __func__, size);
		return -1;
	}
/*	dmx_info(pCh->tunerHandle, "%s called, line %d, ch %d, index %d\n", __func__, __LINE__, ch, index) ; */
...
/*	dmx_info(pCh->tunerHandle, "%s called, line %d, idxR %d, idxW %d, rd 0x%x, wr 0x%x\n", __func__, __LINE__, idxR, idxW, rd, wr) ; */
	if (rd > wr && idxR == idxW) {
		space = rd - wr;
		if (space >= (8+sz)) { /* done */
			...
			if (demux_device->secMap[idxW].nextBufIndex != idxR) { /* done */
				if ((idxW + 1) == demux_device->secMap[idxW].nextBufIndex) { /* continuous buffer */
					...
					*pBuf = ((demux_device->secMap[idxW].nextBufIndex << 16) | 0) ;
					...
					/* dmx_info(pCh->tunerHandle, "%s called, line %d, idxW %d, pBuf 0x%x\n", __func__, __LINE__, idxW, pBuf) ;*/
				}
					...
					*pBuf = ((idxR << 16) | 0) ;
					...
					if(printTime1 == -1)
						...
						if(CLOCK_GetPTS() - printTime1 > 3*DEMUX_CLOCKS_PER_SEC) {
							dmx_err(pCh->tunerHandle, "%s(%d), sectionBuffer Full(ch:%d, secIndex: %d, pid:0x%x, rd:0x%x, wp: 0x%x, lowp:0x%lx, upper:0x%lx,size %d) !!\n",
								__func__, __LINE__, ch, index, pSec->param.PID, rd, wr,
								(unsigned long)(uintptr_t)pSec->pBufferLower,
								(unsigned long)(uintptr_t)pSec->pBufferUpper,
								size
...
int IOCTL_GetBufferInfoImpl(demux_channel *pCh, int pin, int numOfDec)
{
	...
	dmx_crit(pCh->tunerHandle, "[%s %d] pin = %d, numOfDec = %d\n", __func__, __LINE__, pin, numOfDec);
	...
	if (pBufBSH->virAddr == 0) {
		if (DEMUX_BUF_AllocFromPoll(pBufBSH, sizeof(RINGBUFFER_HEADER), &demux_device->bufPoll, 0) < 0) {
			dmx_crit(pCh->tunerHandle, "[%s %d]CH%d out of memory: allocate buffer header fail\n", __func__, __LINE__, pCh->tunerHandle);
			return -1;
		}
	}
	...
	if (pCh->isECP){
		DEMUX_TA_ECP_UpdatePininfo(pCh, pin, DEMUX_STREAM_TYPE_IB, 1, (unsigned long)pBufIBH - (unsigned long)pCh, (unsigned long)pBufIB - (unsigned long)pCh);
		DEMUX_TA_ECP_UpdatePininfo(pCh, pin, DEMUX_STREAM_TYPE_BS, 1, (unsigned long)pBufBSH - (unsigned long)pCh, (unsigned long)pBufBS - (unsigned long)pCh);
	}
	dmx_notice(pCh->tunerHandle, "(CH%d)isVideo(%d) BS PININFO[%d]: %lx,%lx,%pn",
		pCh->tunerHandle, pin, numOfDec, pPinInfo->phyAddrOffset, (ULONG)pBufBS->phyAddr, pBufBS->virAddr);
	dmx_notice(pCh->tunerHandle,
		"(CH%d)isVideo(%d) IB PININFO[%d]: %lx,%lx,%p\n",
		pCh->tunerHandle, pin, numOfDec, pPinIBInfo->phyAddrOffset, (ULONG)pBufIB->phyAddr, pBufIB->virAddr);
...
static void IOCTL_CheckAvSync(demux_channel *pCh)
{
	...
	if (pCh->avSyncMode != MTP_AVSYNC_MODE && (pCh->tp_src == MTP)) {
		dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle, "%s_%d, avsynMode \n", __func__, __LINE__);
...
int IOCTL_InitModule ()
{
	...
	///////////////////////////////////////////////////////////////////
	demux_device->pChArray = kmalloc(PAGE_ALIGN(sizeof(demux_channel)*demux_device->chNum), GFP_KERNEL);
//	demux_device->pChArray = dvr_malloc(sizeof(demux_channel)*demux_device->chNum);
	if (!(demux_device->pChArray)) {
		goto demux_init_fail;
	}
	memset(demux_device->pChArray, 0, PAGE_ALIGN(sizeof(demux_channel)*demux_device->chNum));
	///////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////
	/*if (DEMUX_BUF_AllocFromPoll(&demux_device->chBuffer, sizeof(demux_channel)*demux_device->chNum, &demux_device->bufPoll, 0) < 0)
	{
		goto demux_init_fail;

	}
	demux_device->pChArray = (demux_channel *)demux_device->chBuffer.virAddr;
	if (!(demux_device->pChArray)) {
		goto demux_init_fail;
	}
	...
	for (i = 0; i < demux_device->chNum; i++) {
		...
		if (DEMUX_BUF_AllocateTPBuffer(pCh) < 0){
			dmx_err(pCh->tunerHandle, "%s_%d: allocate TP Buffer FAIL!!!1\n", __func__, __LINE__);
			goto demux_init_fail;
		}
		...
		if (RHAL_GetTPBufferStatus(pCh->tunerHandle, &tpBufSize, &tpDataSize) == TPK_SUCCESS && tpBufSize != pCh->tpBuffer.size) {
			...
			dmx_crit(pCh->tunerHandle, "[%s %d] buf size not matched: tp %d, tpBufSize = %d, org = %d\n", __func__, __LINE__, pCh->tunerHandle, tpBufSize,  pCh->tpBuffer.size);
...
demux_init_fail:
	...
	dmx_err(CHANNEL_UNKNOWN, "DEMUX init Fail @#@#!!!!!!!!!!@#@#\n");
	BUG();
	return ret;
}
...
int IOCTL_ReleaseBuffer(DEMUX_BUF_INFO_T *pInfo)
{
	...
	dmx_info(pCh->tunerHandle, "func %s, line %d, ch %d\n", __func__, __LINE__, pInfo->ch);
...
int IOCTL_GetBufferInfo(DEMUX_BUF_INFO_T *pInfo)
{
	...
	dmx_function_call(pInfo->ch);
	DEMUX_CHECK_CH(pInfo->ch);
	dmx_mask_print(DEMUX_BUFFER_DEBUG|DEMUX_NOTICE_PRINT, pInfo->ch, "[%s, %d]\n", __func__, __LINE__);
	pCh = &demux_device->pChArray[pInfo->ch];
	switch (pInfo->dest) {
	case DEMUX_DEST_ADEC0:
	{
		...
		{
			dmx_err(pCh->tunerHandle, "[%s,%d] ERROR, ch:%d, pin:%d, get buffer fail\n", __func__, __LINE__, pCh->tunerHandle, AUDIO_PIN);
			return -1;
		}
		if (pCh->pinInfo[AUDIO_PIN].pBuffer->status == BUF_STAT_REUSED_II)
		{
			dmx_mask_print(DEMUX_BUFFER_DEBUG|DEMUX_NOTICE_PRINT, pCh->tunerHandle,"[%s, %d] ch:%d, pin:%d this pin still hold a releaseing buffer\n",
				__func__, __LINE__, pCh->tunerHandle, AUDIO_PIN);
	...
	{
	#if defined(DEMUX_AUDIO_USE_GLOBAL_BUFFER)
		dmx_mask_print(DEMUX_BUFFER_DEBUG|DEMUX_NOTICE_PRINT, pCh->tunerHandle,"[%s, %d]\n", __func__, __LINE__);
		mutex_lock(&pCh->mutex);
	...
	{
		if (demux_device->VTP_Info[0].bsBuf.phyAddr){
			pBSH = &demux_device->VTP_Info[0].bsBufH;
			pIBH = &demux_device->VTP_Info[0].ibBufH;
		}
		break;
	}
	case DEMUX_DEST_VDEC1:
	{
		if (demux_device->VTP_Info[1].bsBuf.phyAddr){
			pBSH = &demux_device->VTP_Info[1].bsBufH;
			pIBH = &demux_device->VTP_Info[1].ibBufH;
		}
		break;
	}
	...
	if (pBSH) {
...
#ifdef DEMUX_DEBUG
		{
			...
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)REFCLOCK %x, size %d\n", pCh->tunerHandle, pInfo->refClockPhyAddr, pInfo->refClockHeaderSize);
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)BSH: %x,IBH: %x,BS_size: %x,IB_size: %x\n", pInfo->bsPhyAddr, pInfo->ibPhyAddr, pInfo->bsHeaderSize, pInfo->ibHeaderSize);
			pHeader = (RINGBUFFER_HEADER *)pBSH->virAddr;
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)BS: beginAddr:%x, sz %x\n", pCh->tunerHandle, (unsigned int)DEMUX_reverseInteger(pHeader->beginAddr)   , (unsigned int)DEMUX_reverseInteger(pHeader->size));
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)BS: bufferID: %x, wp %x\n", pCh->tunerHandle, (unsigned int)DEMUX_reverseInteger(pHeader->bufferID)    , (unsigned int)DEMUX_reverseInteger(pHeader->writePtr));
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)BS: numOfRP:  %x, rp %x\n", pCh->tunerHandle, (unsigned int)DEMUX_reverseInteger(pHeader->numOfReadPtr), (unsigned int)DEMUX_reverseInteger(pHeader->readPtr[0]));
			pHeader = (RINGBUFFER_HEADER *)pIBH->virAddr;
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)IB: beginAddr:%x, sz %x\n", pCh->tunerHandle, (unsigned int)DEMUX_reverseInteger(pHeader->beginAddr)   , (unsigned int)DEMUX_reverseInteger(pHeader->size));
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)IB: bufferID: %x, wp %x\n", pCh->tunerHandle, (unsigned int)DEMUX_reverseInteger(pHeader->bufferID)    , (unsigned int)DEMUX_reverseInteger(pHeader->writePtr));
			dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "(CH%d)IB: numOfRP:  %x, rp %x\n", pCh->tunerHandle, (unsigned int)DEMUX_reverseInteger(pHeader->numOfReadPtr), (unsigned int)DEMUX_reverseInteger(pHeader->readPtr[0]));
		}
#endif

		return 0;
	} else {
		dmx_err(pCh->tunerHandle, "func %s, line %d, don't support dest (%d) or alloc buffer fail!!\n", __func__, __LINE__, pInfo->dest);
		return -ENOTTY;
	}
}
...
int IOCTL_StartSCrambleCheck(DEMUX_SCRAMBLE_T *pInfo)
{
	...
	pCh = &demux_device->pChArray[pInfo->ch];
	dmx_mask_print(DEMUX_SCRAMBLE_CHECK_DEBUG, pCh->tunerHandle,
		"DEMUX_StartSCrambleCheck: ch:%d, pid:0x%x, type:%d\n",
...
int IOCTL_StopSCrambleCheck(DEMUX_SCRAMBLE_T *pInfo)
{
	...
	pCh = &demux_device->pChArray[pInfo->ch];
	dmx_mask_print(DEMUX_SCRAMBLE_CHECK_DEBUG, pCh->tunerHandle,
		"DEMUX_StopSCrambleCheck: ch:%d, pid:0x%x, type:%d\n",
		pInfo->ch, pInfo->pid, pInfo->pidType);
	mutex_lock(&pCh->mutex);
	for (i = 0; i < pCh->checkScrambleCount; i++) {
		if (pInfo->pidType == pCh->scrambleList[i].pidType && pInfo->pid == pCh->scrambleList[i].pid) {
...
int IOCTL_CheckSCrambledStatus(DEMUX_SCRAMBLE_T *pInfo)
{
	...
	dmx_mask_print(DEMUX_SCRAMBLE_CHECK_DEBUG, pCh->tunerHandle,
		"DEMUX_CheckSCrambledStatus:ch:%d, pid:0x%x, type:%d,"
		" status:%d(0:unscrambled, 1:scrambled, 2: no data, "
		"3: filter not set yet)\n",
...
static int IOCTL_SetPIDImpl(DEMUX_SET_PID_T *pInfo)
{
	...
	for (i = 0; i < pCh->pidFilterCount; i++) {
		...
		if (haveSamePIDType && haveSameDest) {
			...
			if (currentFilter->pid == pInfo->pid) {
				dmx_info(pCh->tunerHandle, "func %s, line %d, ch %d, pid(%d) has existed in list\n", __func__, __LINE__, pCh->tunerHandle, pInfo->pid);
				return 0;
				...
				if (HAS_FLAG(pCh->status, STATUS_AUDIO_PAUSED) && IOCTL_IsAudioDest(pInfo->dest)) {
					dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle, "[PVR] (CH%d) DONOT RemovePID: pid=%d(0x%x),type=%d,dest=%d\n", pCh->tunerHandle, currentFilter->pid, currentFilter->pid, pInfo->pidType, pInfo->dest);
					return 0;
				} else {
					dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle, "[dtv] (CH%d)RemovePID: pid=%d(0x%x),type=%d,dest=%d\n", pCh->tunerHandle, currentFilter->pid, currentFilter->pid, pInfo->pidType, pInfo->dest);
					...
					if (IOCTL_IsVideoDest(currentFilter->pesDest)) {
						...
						mutex_lock(&demux_device->mutex);
						demux_device->VTP_Info[port].vBufStatus = BUF_STAT_FREE;
						mutex_unlock(&demux_device->mutex);
						dmx_info(pCh->tunerHandle, "[%s %d]Ch%d port %d is waiting to be free\n", __func__, __LINE__, pCh->tunerHandle, port);
					}
				...
				dmx_info(pCh->tunerHandle, "[%s %d] (CH%d)pidType(%d) is already in list\n", __func__, __LINE__, pCh->tunerHandle, pInfo->pidType);
				...
				if (IOCTL_IsVideoDest(pInfo->dest) || IOCTL_IsAudioDest(pInfo->dest)) {
					...
					if( 0 == currentFilter->refCnt )
						...
						dmx_mask_print(DEMUX_PID_DEBUG|DEMUX_NOTICE_PRINT, pCh->tunerHandle, "[dtv] (CH%d)SameTypeA/V,changeToTSO: pid=%d(0x%x),type=%d,dest=%d\n",
							pCh->tunerHandle, currentFilter->pid, currentFilter->pid, pInfo->pidType, pInfo->dest);
						currentFilter->pesDest = DEMUX_DEST_TSO;
					}
				}
			...
			if (currentFilter->pid == pInfo->pid && currentFilter->pesDest == DEMUX_DEST_TSO) {
				if (IOCTL_IsVideoDest(pInfo->dest) || IOCTL_IsAudioDest(pInfo->dest)||IOCTL_IsRecordDest(pInfo->dest)) {
					int test_target = 0;
					dmx_info(pCh->tunerHandle, "func %s, line %d, ch %d, pid(%d) has existed in list, need update dest\n", __func__, __LINE__, pCh->tunerHandle, pInfo->pid);
	...
	if (pInfo->pid == filterRemoveIndicator) { /* only clear PID by setting pInfo->pid to 0x1fff */
		...
		dmx_notice(pCh->tunerHandle,"[dtv] (CH%d)AddPID: pid=%d(0x%x),type=%d,dest=%d\n", pCh->tunerHandle, pInfo->pid, pInfo->pid, pInfo->pidType, pInfo->dest);
	...
	/* debug print */
	for (i = 0 ; i < pCh->pidFilterCount ; i++) {
		dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "[dtv] (CH%d)pidFilterList[%d]:pid=%d,type=%d,dest=%d\n",
			pCh->tunerHandle, i, pCh->pidFilterList[i].pid, pCh->pidFilterList[i].pidType, pCh->pidFilterList[i].pesDest);
	}

	if (pCh->pidFilterCount == 0){
		dmx_notice(pCh->tunerHandle,"set_PID : ch:%d,  DEMUX_PRIVATEINFO_FLUSHTP \n", pInfo->ch);
		dmx_notice(pCh->tunerHandle,"(%s-%d): DEMUX_PRIVATEINFO_FLUSHTP -ch:%d filteramask: %x \n", __FILE__, __LINE__,pInfo->ch,DEMUX_checkFilterCount(pCh));
...
int IOCTL_SetPID(DEMUX_SET_PID_T *pInfo)
{
	...
	DEMUX_CHECK_PID(pInfo->pid);
	pCh = &demux_device->pChArray[pInfo->ch];
	dmx_notice(pCh->tunerHandle,"func %s, line %d, ch %d, pid 0x%x, pidType %d, dest %d\n", __func__, __LINE__, pInfo->ch, pInfo->pid, pInfo->pidType, pInfo->dest);
...
int IOCTL_GetPID(DEMUX_GET_PID_T *pInfo)
{
	...
	dmx_mask_print(DEMUX_FILTER_DEBUG,  pCh->tunerHandle, "func %s, line %d, ch %d, pidType %d, dest %d\n", __func__, __LINE__, pInfo->ch, pInfo->pidType, pInfo->dest);
	mutex_lock(&pCh->mutex);
	//checkpidFilterlist
	for (i = 0; i < pCh->pidFilterCount; i++) {
...
*/
		...
		dmx_mask_print(DEMUX_SECTION_DEBUG,  pCh->tunerHandle, "func %s, line %d, cnt %d, idx %d, first %d, last %d\n", __func__, __LINE__,cnt, i, (i >> 16), (i & 0xffff) ) ;
		if (i < 0 || RHAL_AddSectionFilter(pInfo->ch, pInfo->param, (void *)DEFAULT_FILE_HANDLE, &pSec->handle) != TPK_SUCCESS) {
			...
			pSec->used = 0;
			dmx_err(pCh->tunerHandle, "%s,ch:%d, fail to allocate buffer or add section filter,(ret=%d)!!", __func__,pInfo->ch,i);
			ret = -1;
		} else {
		...
		dmx_err(pCh->tunerHandle, "%s, fail to get section index!!", __func__);
	}
	mutex_unlock(&pCh->section_mutex);
	return ret;
}
int IOCTL_CancelSectionFilter(DEMUX_CANCEL_SEC_FILTER_T *pInfo)
{
	...
	DEMUX_CHECK_CH(pInfo->ch);
	pCh = &demux_device->pChArray[pInfo->ch];
	dmx_mask_print(DEMUX_SECTION_DEBUG,  pCh->tunerHandle, "func %s, line %d, ch %d secfIndex %d\n", __func__, __LINE__, pInfo->ch, pInfo->secfIndex);

	if (pInfo->secfIndex >= MAX_SECTION_FILTER_NUM || !pCh->secListCB[pInfo->secfIndex].used) {
		dmx_warn(CHANNEL_UNKNOWN,"func %s, line %d,ch:%d, wrong index %d\n", __func__, __LINE__,pInfo->ch,  pInfo->secfIndex);
		return -1;
	}
	...
	dmx_err(pCh->tunerHandle, DGB_COLOR_RED"FAIL func %s, line %d, ch %d secfIndex %d\n"DGB_COLOR_NONE, __func__, __LINE__, pInfo->ch, pInfo->secfIndex);
	return -1;
}
int IOCTL_AddPESFilter(DEMUX_ADD_PES_FILTER_T *pInfo)
{
	...
	if (pInfo->size > PES_FILTER_RING_BUFFER_SIZE) {
		dmx_err(pCh->tunerHandle, "%s, size(%d) to be allocated is larger than used!!", __func__, pInfo->size);
	}
	mutex_lock(&pCh->mutex);
	dmx_mask_print(DEMUX_PES_DEBUG|DEMUX_NOTICE_PRINT, pCh->tunerHandle, "DEMUX_Add_PESFilter: ch:%d,pid:0x%x, type:%d, size:0x%x\n", pInfo->ch, pInfo->pid, pInfo->pidType, pInfo->size);
	for (i = 0 ; i < MAX_PID_FILTER_CB_NUM ; i++) {
	...
	if (ret >= 0){
		...
		if (pPes->bufferStatus == BUFFER_OFFLINE){
			if (DEMUX_PESBuffer_Alloc(pPes) < 0){
				dmx_err(pCh->tunerHandle, "%s-%d : alloc pes buffer for pid:%x, fail\n", __func__, __LINE__, pInfo->pid);
   			...
   			dmx_mask_print(DEMUX_PES_DEBUG,  pCh->tunerHandle, "Allocate pesBuffer: ch:%d, pesId:%d, virAddr:%p, phyAddr:%x, limit:%x size: %x\n", pInfo->ch, ret, pPes->virAddr, pPes->phyAddr, pPes->pBufferUpper, PES_FILTER_RING_BUFFER_SIZE);
		}
		else if (pPes->bufferStatus == BUFFER_FREE){
			pPes->bufferStatus = BUFFER_NEW;
			dmx_mask_print(DEMUX_PES_DEBUG,  pCh->tunerHandle, "reuse unfree buffer : ch:%d, pesId:%d, virAddr:%p, phyAddr:%x, limit:%x size: %x\n", pInfo->ch, ret, pPes->virAddr, pPes->phyAddr, pPes->pBufferUpper, PES_FILTER_RING_BUFFER_SIZE);
		}
		else
		{
			dmx_err(pCh->tunerHandle, " %s-%d : PES Filter status not sync !\n", __func__, __LINE__);
			ret = -1;
		}
	}
#endif
	if (ret >= 0) {
		...
		dmx_err(pCh->tunerHandle, "%s(%d) Fail to Add PES, ch:%d, pid: %d, type: %d, size:0x%x \n", __func__, __LINE__, pInfo->ch, pInfo->pid, pInfo->pidType, pInfo->size);
...
int IOCTL_CancelPESFilter(DEMUX_CANCEL_PES_FILTER_T *pInfo)
{
	...
	if (pInfo->pesfIndex >= MAX_PID_FILTER_CB_NUM || !pCh->pidFilterListCB[pInfo->pesfIndex].used) {
		dmx_err(pCh->tunerHandle, "func %s, line %d, Error(pesfIndex = %d)!!\n", __func__, __LINE__, pInfo->pesfIndex);
		return -ENOTTY;
	}
	dmx_mask_print(DEMUX_PES_DEBUG|DEMUX_NOTICE_PRINT, pCh->tunerHandle, "%s_%d, ch:%d, pesId:%d \n", __func__, __LINE__, pInfo->ch, pInfo->pesfIndex);
	mutex_lock(&pCh->mutex);
...
{
	dmx_info(CHANNEL_UNKNOWN, "[%s %d] %s, NOT implemented!!\n", __FILE__, __LINE__, __func__) ;
	return 0;
}
...
int IOCTL_StopPCRRecovery(DEMUX_CHANNEL_T *pInfo)
{
	...
	DEMUX_CHECK_CH(*pInfo);
	pCh = &demux_device->pChArray[*pInfo];
	pCh->pcrProcesser.bUseFixedRCD = true;
...
int IOCTL_ResetFilters(DEMUX_RESET_FILTERS_T *pInfo)
{
	...
	DEMUX_CHECK_CH(pInfo->ch);
	pCh = &demux_device->pChArray[pInfo->ch];
	dmx_notice(pCh->tunerHandle,"%s_%d,ch:%d, scope:%d\n", __func__, __LINE__, pInfo->ch, pInfo->scope);
	mutex_lock(&pCh->mutex);
	switch (pInfo->scope) {
	case DEMUX_CLEAN_AV:
	...
	if (bPES) {
		for (i = 0; i < MAX_PID_FILTER_CB_NUM; i++) {
			...
			#endif
				dmx_mask_print(DEMUX_PES_DEBUG,  pCh->tunerHandle, "%s_%d, canclePesfilter: ch:%d, pesId:%d\n", __func__, __LINE__, pInfo->ch, i);
			}
		}
		SET_FLAG(pCh->events, EVENT_UPDATE_PES_LIST);
	}
	...
	if (bSec) {
		for (i = 0; i < MAX_SECTION_FILTER_NUM; i++) {
			if (pCh->secListCB[i].used) {
				...
				dmx_mask_print(DEMUX_SECTION_DEBUG,  pCh->tunerHandle, "%s_%d, cancleSectionfilter: ch:%d, secId:%d\n", __func__, __LINE__, pInfo->ch, i);
			}
		}
	}
	if (bPVR) {/* TODO */
...
int IOCTL_InitDescrambler(DEMUX_INIT_DESCRAMBLER_T *pInfo)
{
	dmx_function_call(pInfo->ch);
	DEMUX_CHECK_CH(pInfo->ch);
	dmx_notice(CHANNEL_UNKNOWN,"BCAS_IntDescramble:ch:%d, alg:%d,round:%d\n",pInfo->ch, pInfo->alg.algo,pInfo->alg.round)
...
int IOCTL_SetCW(DEMUX_SET_CW_T *pInfo)
{
	...
	if (_bcase_print_key_info)//for debug
	{
		...
		dmx_notice(pCh->tunerHandle,"BCAS:SetCW:ch:%d, keytype(0:iv, 1:key):%d, keyid:(0:even, 1:odd):%d, keysel:%d, key[1]:0x%x\n",
...
int IOCTL_MTPFlush(DEMUX_MTP_FLUSH_T *pInfo)
{
	...
	if (rtkdemux_dbg_tpratecontrol_en)
...
int IOCTL_FreeMTPBuf(DEMUX_CHANNEL_T *pInfo)
{
	...
	DEMUX_CHECK_CH(*pInfo);
	pCh = &demux_device->pChArray[*pInfo];
	dmx_crit(pCh->tunerHandle, "[%s, %d] %s()\n", __FILE__, __LINE__, __func__);
...
int IOCTL_SetTimeStampPadding(DEMUX_TIMESTAMP_SETTING_T *pInfo)
{
	...
	#else
		/* tpStatus.tp_param.prefix_en = false; */ /* timestamp is useless during PVR playback ?! */
		tpStatus.tp_param.frc_en = false; // When config tp status, turn off rate control by default
...
int IOCTL_SetPrivateInfo(DEMUX_PRIVATE_INFO_T *pInfo)
{
	...
	if (pInfo->dest != DEMUX_DEST_ADEC0 && pInfo->dest != DEMUX_DEST_ADEC1 && pInfo->dest != DEMUX_DEST_VDEC0 && pInfo->dest != DEMUX_DEST_VDEC1) {
		dmx_err(pCh->tunerHandle, "func %s, line %d, dest(%d) is wrong!!\n", __func__, __LINE__, pInfo->dest);
		return -1;
	}
	dmx_info(pCh->tunerHandle, "func %s, line %d, ch %d, dest %d, info %u\n",
		__func__, __LINE__, pInfo->ch, pInfo->dest, pInfo->data[0]);
	mutex_lock(&pCh->mutex);
	switch ((DEMUX_PRIVATEINFO_T)pInfo->data[0]) {
	case DEMUX_PRIVATEINFO_NOTIFY_FLUSHED:
		{
			int status;
			dmx_notice(pCh->tunerHandle,"(%s-%d): DEMUX_PRIVATEINFO_NOTIFY_FLUSHED: dest:%d \n", __FILE__, __LINE__, pInfo->dest);
			status = pInfo->dest == DEMUX_DEST_ADEC0 ? STATUS_FLUSH_AUDIO  :
				pInfo->dest == DEMUX_DEST_ADEC1 ? STATUS_FLUSH_AUDIO2 :
				pInfo->dest == DEMUX_DEST_VDEC0 ? STATUS_FLUSH_VIDEO  : STATUS_FLUSH_VIDEO2;
	...
	case DEMUX_PRIVATEINFO_AUDIO_FORMAT:
		{
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle, "(%s-%d): DEMUX_PRIVATEINFO_AUDIO_FORMAT: dest:%d \n", __FILE__, __LINE__, pInfo->dest);
			if (pInfo->dest == DEMUX_DEST_ADEC0) {
	...
	case DEMUX_PRIVATEINFO_VIDEO_DECODE_MODE:
	{
		...
		dmx_info(pCh->tunerHandle, "[%s %d] VDEC%d decode mode = %d\n", __func__, __LINE__, port, pCh->videoDecodeMode[port]);
		break;
	}
	...
	case DEMUX_PRIVATEINFO_AUDIO_PAUSE:
	{
		 ...
		 */
		dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle, "[%s %d] audio paused \n",__func__, __LINE__);
	...
	case DEMUX_PRIVATEINFO_APVR_PLAYBACK_START:
	{
		...
		if(rtkdemux_dbg_tpratecontrol_en)
			PVR_EnableTPRateControl(pCh, false);
		dmx_crit(pCh->tunerHandle, "[%s, %d] DEMUX_PRIVATEINFO_APVR_PLAYBACK_START\n", __func__, __LINE__);
		return ret;
	}
...
int IOCTL_SetTPStreamControl(DEMUX_TP_STREAM_CONTROL_T *pInfo)
{
	...
	if (pCh->startStreaming && pInfo->ctrl == TP_STREAM_STOP) {
		...
		dmx_notice(pCh->tunerHandle,"func %s, line %d, tp %d, donothing, startStreaming %d, ctrl %d\n", __func__, __LINE__, pCh->tunerHandle, pCh->startStreaming, pInfo->ctrl);
	}
...
int IOCTL_SetRecStatus(DEMUX_REC_STATUS_T *pInfo)
{
	...
	if(IOCTL_GetTPInfoBuffer(&bufInfo) >= 0) {
		...
		if (RHAL_SetTPPatternSearchInfoBuffer(pCh->tunerHandle, (UINT8 *)pInfoBuf->phyAddr, (UINT8 *)pInfoBuf->nonCachedaddr, pInfoBuf->size, (UINT8 *)&pHeader->readPtr[0], (UINT8 *)&pHeader->writePtr) != TPK_SUCCESS) {
			dmx_crit(pCh->tunerHandle, "[%s %d] CH%d fail to set TP info buf\n", __func__, __LINE__, pCh->tunerHandle);
			return -1;
		}
	}
	...
	dmx_mask_print(DEMUX_NOMAL_DEBUG,  pCh->tunerHandle, "[%s %d] download buffer is TP buffer. bStartRec = %d\n", __func__, __LINE__, pInfo->bStartRec);
...
int IOCTL_GetBufferInfoEX(DEMUX_BUF_INFO_EX_T *pInfo)
{
	...
	if (pBS) {
		...
		dmx_err(pCh->tunerHandle, "func %s, line %d, don't support filterType (%d)!!\n", __func__, __LINE__, pInfo->bufferType);
		return -ENOTTY;
	}
}
...
int IOCTL_EnableDescrambler(DEMUX_ENABLE_DESCRAMBLER_T *pInfo)
{
	...
	mutex_lock(&pCh->mutex);
	dmx_notice(pCh->tunerHandle,"BCAS_setPID: ch:%d, pid:0x%x,type:%d, enable:%d, key:%d\n",pInfo->ch, pInfo->pid, pInfo->pidType, pInfo->bEnable, pInfo->KeySel);
...
int IOCTL_ClearDescrambler(DEMUX_CLEAR_DESCRAMBLER_T *pInfo)
{
	...
	pCh = &demux_device->pChArray[pInfo->ch];
	mutex_lock(&pCh->mutex);
	dmx_notice(pCh->tunerHandle,"BCAS_ClearPid: ch:%d, type:%d, pid:0x%x,  key:%d\n",pInfo->ch, pInfo->pidType, pInfo->pid, pInfo->KeySel);
	...
	if (pCh->pidFilterCount == 0){
		...
		dmx_notice(pCh->tunerHandle,"BCAS_ClearPid : ch:%d,  DEMUX_PRIVATEINFO_FLUSHTP \n", pInfo->ch);
		dmx_notice(pCh->tunerHandle,"(%s-%d): DEMUX_PRIVATEINFO_FLUSHTP -ch:%d filteramask: %x \n", __FILE__, __LINE__,pInfo->ch,DEMUX_checkFilterCount(pCh));
		SET_FLAG(pCh->status, STATUS_FLUSH_TP);
	}
	mutex_unlock(&pCh->mutex);
...
int IOCTL_SwitchDescrambler(DEMUX_SWITCH_DESCRAMBLER_T *pInfo)
{
	...
	for (i = 0; i < pCh->pidFilterCount; i++) {
		...
		dmx_notice(pCh->tunerHandle,"BCAS_EnableDescramble: ch:%d, pid:0x%x, enable:%d, key:%d\n",pInfo->ch, pInfo->pid, pInfo->bEnable, pInfo->KeySel);
		pCh->pidFilterList[i].cwIndex      = pInfo->KeySel;
		pCh->pidFilterList[i].enDescramble = pInfo->bEnable;
		SET_FLAG(pCh->events, EVENT_UPDATE_TS_LIST);
		pCh->bUpdateDemuxTarget = true;
...
int IOCTL_DebugInfo(DEMUX_DEBUG_INFO_T *pInfo)
{
	...
	case DEMUX_DEBUG_PRINT_PID_LIST:
		{
			dmx_mask_print(DEMUX_PID_DEBUG,  pCh->tunerHandle, "DEMUX_DEBUG_PRINT_PID_LIST:\n");
			for (i = 0; i < pCh->pidFilterCount; i++) {
				dmx_mask_print(DEMUX_PID_DEBUG,  pCh->tunerHandle, "ch:%d, pid 0x%x, pidType %d, pesDest %d, cwIndex %d, enDescramble %d\n",pInfo->ch, pCh->pidFilterList[i].pid, pCh->pidFilterList[i].pidType, pCh->pidFilterList[i].pesDest, pCh->pidFilterList[i].cwIndex, pCh->pidFilterList[i].enDescramble);
			}
			break;
		}
	case DEMUX_DEBUG_PRINT_PID_FOR_PES:
		{
			dmx_notice(pCh->tunerHandle,"DEMUX_DEBUG_PRINT_PID_LIST:\n");
			for (i = 0; i < MAX_PID_FILTER_CB_NUM; i++) {
				if (pCh->pidFilterListCB[i].used) {
					dmx_mask_print(DEMUX_PID_DEBUG,  pCh->tunerHandle, "ch: %d ,idx %d, 0x%x, pidType %d, cwIndex %d\n",pInfo->ch, i, pCh->pidFilterListCB[i].pid, pCh->pidFilterListCB[i].pidType, pCh->pidFilterListCB[i].cwIndex);
				}
			}
			break;
		}
...
int IOCTL_SetPidActive(DEMUX_SET_MULTI_PID_T *pInfo)
{
	...
	for (j = 0; j < pInfo->count; j++) {
		...
		if (i == pCh->pidFilterCount) {
			dmx_err(pCh->tunerHandle, "func %s, line %d, pid(%d) hasn't existed in list\n", __func__, __LINE__, pInfo->pidlist[j].pid);
			continue;
		}
		/*
		   for (i = 0; i < pCh->pidFilterCount; i++)
	...
	for (k = 0; k < pInfo->count; k++) {
		for (i = 0; i < pCh->pidFilterCount; i++) {
			if (pCh->pidFilterList[i].pid == pInfo->pidlist[k].pid && pCh->pidFilterList[i].pidType == pInfo->pidlist[k].pidType && pCh->pidFilterList[i].pesDest == pInfo->pidlist[k].dest) {
				dmx_err(pCh->tunerHandle, "func %s, line %d, pid(%d) has existed in list\n", __func__, __LINE__, pInfo->pidlist[k].pid);
				continue;
			}
		}
...
int IOCTL_ChangeAvSyncMode(DEMUX_SET_SPEED_T *pInfo)
{
	 ...
	 * 3. pause -> fast rewind
	 */
	dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle,  "%s_%d: ch:%d, speed:%d \n", __func__, __LINE__, pCh->tunerHandle, pInfo->speed);
	...
	for (i = 0; i < pCh->pidFilterCount; i++) {
		if (pCh->pidFilterList[i].pidType == DEMUX_PID_VIDEO) {
			...
			long ignore;
			if (videoport >= MAX_VDEC_NUM)
			{
				dmx_err(pCh->tunerHandle, "%s_%d: VTP port invalid:%d (des:%d)\n", __func__, __LINE__, videoport, pCh->pidFilterList[i].pesDest);
				return -1;
			}
			if (send_rpc_command(RPC_VIDEO, VIDEO_RPC_VOUT_ToAgent_SetDemuxSpeed, demux_device->VTP_Info[videoport].destPort, pInfo->speed, &ignore)) {
				pr_err("RPC fail!!\n");
			}
			break;
		}
...
int IOCTL_ChangeAvSyncMode(DEMUX_SET_SPEED_T *pInfo)
{
	...
	DEMUX_CHECK_CH(pInfo->ch);
	pCh = &demux_device->pChArray[pInfo->ch];
	dmx_err(pCh->tunerHandle, "%s_%d: RPC fail!!\n", __func__, __LINE__);
	return -1;
}
#endif
...
int IOCTL_SetTPInputConfig(DEMUX_TP_SOURCE_CONFIG_T *pInfo)
{
	...
	/*  1.  stream control    */
	//dmx_notice(pCh->tunerHandle,"%s ch%d stream control\n", __func__, pInfo->ch);
	if (pInfo->inputType == TPK_INPUT_ENABLE || pInfo->inputType == TPK_INPUT_DISABLE)
	{
		DEMUX_CHANNEL_T tmp_dmx_ch;
		demux_channel *tmp_pCh;
		TPK_INPUT_PORT_T oldPortType;
		...
		if (pInfo->inputType == TPK_INPUT_ENABLE){
			...
			dmx_notice(pCh->tunerHandle,"%s ch%d Start streaming!\n", __func__, pInfo->ch);
		}else{
			streamCtrl.ctrl	= TP_STREAM_STOP;
			dmx_notice(pCh->tunerHandle,"%s ch%d Stop streaming!\n", __func__, pInfo->ch);
		}
		IOCTL_SetTPStreamControl(&streamCtrl);

		oldPortType = pCh->inputConfigInfo.portType;
		...
		if(pInfo->inputType == TPK_INPUT_DISABLE)
		{
			...
			switch(oldPortType) {
				...
				case TPK_PORT_CI_DEMUX_OUT_CI_CH2:
					...
					break;
				default:
					tpp_id = (TPK_TPP_ENGINE_T)MAX_TP_P_COUNT;
			}
			/* stream control for tpp */
	...
	//dmx_notice(pCh->tunerHandle," 2.  set tp param  %s, line %d\n", __func__, __LINE__);
	if(pInfo->casType == TPK_WITH_CIP_CAS)
	{
		switch(pInfo->portType)
		{
			case TPK_PORT_CI_DEMUX_OUT_CI_CH0:
				tpStatus.tp_param.sync_byte = sync_byte[0];
				break;
			case TPK_PORT_CI_DEMUX_OUT_CI_CH1:
				tpStatus.tp_param.sync_byte = sync_byte[1];
				break;
			case TPK_PORT_CI_DEMUX_OUT_CI_CH2:
				tpStatus.tp_param.sync_byte = sync_byte[2];
				break;
			default:
				tpStatus.tp_param.sync_byte = sync_byte[0];
				break;
	...
	if (pInfo ->casType != TPK_WITHOUT_CAS)
	{
		...
		dmx_notice(pCh->tunerHandle,"%s Set TP%d to parallel mode!\n", __func__, tp_id);
	}
	else if (pInfo->inputType == TPK_INPUT_SERIAL)
	{
		tpStatus.tp_param.serial            = 1 ;
		dmx_notice(pCh->tunerHandle,"%s Set TP%d to Serial mode!\n", __func__, tp_id);
	}
	else
	{
		tpStatus.tp_param.serial			= 0 ;
	...
	{
		dmx_notice(pCh->tunerHandle,"%s Set TP%d data from memory!\n", __func__, tp_id);
		/*
		...
		*/
		tpStatus.tp_param.frc_en = false; // When config tp, turn off rate control by default
	...
	{
		dmx_notice(pCh->tunerHandle,"%s Set TP%d data from %d!(1:iD0,2:iD1,3:eI0,4:eI1,5:eI2)\n", __func__, tp_id, pInfo->portType);
	...
	/* 3. set TP source */
        //dmx_notice(pCh->tunerHandle,"%s Ch%d set TP spurce\n", __func__, pInfo->ch);
	...
	if(tp_src == MTP) {
		dmx_notice(pCh->tunerHandle,"\033[1;36m %s tp%d STOP PCR tracking..\033[m\n", __func__, tp_id );
		Demux_MW_disablePCRTracking(pCh);
	}
	else{
		pCh->pcrProcesser.bUseFixedRCD = false;
	}
	dmx_notice(pCh->tunerHandle,"%s Set TP%d Source from %d!(1:MTP,2:iD,3:eI0,4:eI1,5:eI2)\n", __func__, tp_id, tp_src);
	...
	if(pInfo->casType!=TPK_WITHOUT_CAS)
	{
		...
		if(tpout_src == TPK_TPP_TO_TPOUT){
			...
			dmx_notice(pCh->tunerHandle,"%s Set CI channel to TPP%d! tp%d_sync_byte=%X tpp_out_sync_byte=%X\n", __func__, tpp_id, tp_id, tpStatus.tp_param.sync_byte, sync_byte[tpp_id]);
			dmx_notice(pCh->tunerHandle,"%s Set TPO src(%d) remux from %d!(1:MEM,2:MUX)\n", __func__, TPK_TPP_TO_TPOUT, tpp_src);
	...
	if (pInfo->portType == TPK_PORT_FROM_MEM && pCh->mtpBuffer.allocSize == 0) {
		dmx_notice(pCh->tunerHandle,"%s alloc buffer(%d) for mtp\n", __func__, MTP_BUFFER_SIZE);
		if (DEMUX_BUF_MTPBuffer_Create(pCh) < 0)
			return -1;
	}
	DEMUX_BUF_ConfigTPBuffer(pCh);
	...
	for (iChannel =0; iChannel < demux_device->chNum; iChannel++)
	{
		...
		for ( iItem = 0; iItem < MAX_PID_FILTER_CB_NUM; iItem++)
		{
			...
			if (demux_device->pChArray[iChannel].pidFilterListCB[iItem].used && (pRBH->reserve2 == DEMUX_PESBUFFER_OP_INITIALIZED))
				if(BS_DISTANCE_TO_READ(pRBH->readPtr[1], pRBH->writePtr, pPesMap->pBufferLower, pPesMap->pBufferUpper) > 4)
					return true;
		}
	}
	return false;
...
int IOCTL_SetTPPatternSearchPID(DEMUX_TP_PATTERN_SEARCH_T *pInfo)
{
	...
	if(pCh->tunerHandle != 0 && pCh->tunerHandle != 1) {
		dmx_crit(pCh->tunerHandle, "[%s %d]TP%d doesn't support start code search\n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
...
int IOCTL_GetTPInfoBuffer(DEMUX_PVR_BUF_INFO_T *pInfo)
{
	...
	if (pCh->tpInfoBuffer.allocSize == 0) {
		if (DEMUX_BUF_AllocBuffer(&pCh->tpInfoBuffer, size, 0, 0) < 0) {
			dmx_crit(pCh->tunerHandle, "[%s %d]CH%d fail to allocate TP info buffer\n", __func__, __LINE__, pInfo->ch);
			return -1;
		}
	}
...
int IOCTL_GetTPPBufStatus(DEMUX_TPP_BUF_STATUS_T *pInfo)
{
	...
	if (RHAL_TPP_GetBufferStatus(tpp_id, &bufSize, &dataSize) != TPK_SUCCESS)
		return -1;
...
int IOCTL_SetTPPMtpStreaming(DEMUX_MTP_STREAMING_STATUS_T *pInfo)
{
	...
	if(pInfo->bStartStreaming)
	{
		if (pCh->tppMtpBuffer.allocSize == 0) {
			if (DEMUX_BUF_AllocBuffer(&pCh->tppMtpBuffer, TPP_MTP_BUFFER_SIZE, 1, 0) < 0) {
				dmx_crit(pCh->tunerHandle, "[%s %d] fail to allocate TPP_MTP buffer\n", __func__, __LINE__);
				return -1;
			}
			DEMUX_BUF_ConfigTPBuffer(pCh);
		}
...
int IOCTL_TPPMTPWriteBuffer(DEMUX_MTP_WRITE_T *pInfo)
{
	...
	tpp_mtp_id = DEMUX_GET_TPP_MTP_ID(pInfo->ch);
#ifdef CONFIG_ARM64
	if (RHAL_WriteMTPBuffer(tpp_mtp_id, (UINT8 *)(unsigned long)pInfo->writeAddr, pInfo->writeSize) != TPK_SUCCESS)
#else
...
int IOCTL_GetCIDLBufferStatus(DEMUX_CI_DL_BUF_STATUS_T *pInfo)
{
	...
	if(RHAL_CIP_GetBufferStatus(pCh->tunerHandle, &pInfo->phyBase, &pInfo->phyLimit,
...
int IOCTL_SetCIDLBufferRP(DEMUX_CI_DL_BUF_RP_T *pInfo)
{
	...
	if(RHAL_CIP_SetBufferRP(pCh->tunerHandle, pInfo->phyRead) != TPK_SUCCESS)
		return -1;
...
{
	if(RHAL_TPP_EnablePIDFilter((TPK_TP_ENGINE_T)pInfo->tpp_id, pInfo->bEnable) != TPK_SUCCESS)
		   return -1;
...
{
	if(RHAL_TPP_FlushBuffer((TPK_TP_ENGINE_T)*pInfo) != TPK_SUCCESS)
		   return -1;
...
int IOCTL_SetTpSyncCP14(DEMUX_TP_SYNC_CP14_T *pInfo)
{
	...
	DEMUX_CHECK_CH(pInfo->ch);
	pCh = &demux_device->pChArray[pInfo->ch];
	if(RHAL_TPP_SetSyncReplace(pCh->tunerHandle, pInfo->u8SyncByte) != TPK_SUCCESS)
		   return -1;
...
int IOCTL_SetBypassPIDs(DEMUX_PID_TABLE_T *pInfo)
{
	...
	if (pInfo->count > DEMUX_MAX_NUM_TO_SET || (pInfo->count + pCh->bypassPidFilterCount) > DEMUX_MAX_NUM_TO_SET) {
		dmx_warn(CHANNEL_UNKNOWN,"[%s %d] too many pids! already have %d and you want to insert %d pids\n", __func__, __LINE__, pCh->bypassPidFilterCount, pInfo->count);
		return -1;
	}
	mutex_lock(&pCh->mutex);
...
int IOCTL_RemoveBypassPIDs(DEMUX_PID_TABLE_T *pInfo)
{
	...
	pCh = &demux_device->pChArray[pInfo->ch];
	if (pInfo->count > DEMUX_MAX_NUM_TO_SET) {
		dmx_warn(CHANNEL_UNKNOWN,"[%s %d] too many pids! you want to remove %d pids\n", __func__, __LINE__, pInfo->count);
		return -1;
	}
	mutex_lock(&pCh->mutex);
...
{
	if(RHAL_CIP_GetCAMBitRate(&pInfo->inputBitrate,&pInfo->outputBitrate)!= TPK_SUCCESS)
		return -1;

	return 0;
}

int IOCTL_SetTPPMTPStreamControl(DEMUX_TP_STREAM_CONTROL_T *pInfo)
{
	TPK_TPP_ENGINE_T tpp_id;
	DEMUX_CHECK_CH(pInfo->ch);
	...
	if (RHAL_MTPStreamControl((tpp_id+TP_TPP0_MTP), pInfo->ctrl) != TPK_SUCCESS)
		return -1;

	return 0;
}
int IOCTL_VTP_VDEC_Connect(DEMUX_VTP_VDEC_MAP_T mapInfo)
{
	if (mapInfo.vtp_port >= MAX_VDEC_NUM || mapInfo.vdec_port >= MAX_VDEC_NUM)
	{
		dmx_err(CHANNEL_UNKNOWN, "%s_%d: invalid Param, vtp:%d, vdec:%d\n", __func__, __LINE__, mapInfo.vtp_port, mapInfo.vdec_port);
		return -1;
	}
	if ((demux_device->VTP_Info[mapInfo.vtp_port].destPort != 0xff)) {
		if (demux_device->VTP_Info[mapInfo.vtp_port].destPort != mapInfo.vdec_port) {
			dmx_err(CHANNEL_UNKNOWN, "%s_%d:ERROR vtp_%d, is already connect to vdec_%d,(%d)\n",
				__func__, __LINE__, mapInfo.vtp_port, demux_device->VTP_Info[mapInfo.vtp_port].destPort, mapInfo.vdec_port);
			return -1;
		}
	}
	mutex_lock(&demux_device->mutex);
	demux_device->VTP_Info[mapInfo.vtp_port].destPort = mapInfo.vdec_port;
	mutex_unlock(&demux_device->mutex);
	dmx_mask_print(DEMUX_NOTICE_PRINT,  CHANNEL_UNKNOWN, "%s_%d: vdec:%d ---connect----> vtp:%d\n",__func__, __LINE__, mapInfo.vdec_port, mapInfo.vtp_port);
	return 0;
}
int IOCTL_VTP_VDEC_Disconnect(DEMUX_VTP_VDEC_MAP_T mapInfo)
{
	if (mapInfo.vtp_port >= MAX_VDEC_NUM || ((mapInfo.vdec_port >= MAX_VDEC_NUM) && (mapInfo.vdec_port != 0xff))) {
		dmx_err(CHANNEL_UNKNOWN, "%s_%d: invalid Param, vtp:%d, vdec:%d\n", __func__, __LINE__, mapInfo.vtp_port, mapInfo.vdec_port);
		return -1;
	}
	if ((demux_device->VTP_Info[mapInfo.vtp_port].destPort != 0xff)) {
		if (demux_device->VTP_Info[mapInfo.vtp_port].destPort != mapInfo.vdec_port) {
			dmx_err(CHANNEL_UNKNOWN, "%s_%d:ERROR vtp_%d, is NOT connect to vdec_%d,(%d)\n",
				__func__, __LINE__, mapInfo.vtp_port, mapInfo.vdec_port, demux_device->VTP_Info[mapInfo.vtp_port].destPort);
			return -1;
		}
	}
	mutex_lock(&demux_device->mutex);
	demux_device->VTP_Info[mapInfo.vtp_port].destPort = 0xff;
	demux_device->VTP_Info[mapInfo.vtp_port].isECP    = 0;
	mutex_unlock(&demux_device->mutex);
	dmx_mask_print(DEMUX_NOTICE_PRINT, CHANNEL_UNKNOWN, "%s_%d: vdec:%d ---disconnect----> vtp:%d\n",__func__, __LINE__, mapInfo.vdec_port, mapInfo.vtp_port);
	return 0;
}

int IOCTL_SetDDRProtect(DEMUX_DDR_PROTECT *pdp)
{
	demux_channel *pCh = NULL;

	DEMUX_CHECK_CH(pdp->ch);
	pCh = &demux_device->pChArray[pdp->ch];
	dmx_crit(pdp->ch, "%s_%d: IOCTL_SetDDRProtect: %d\n", __func__, __LINE__, pdp->val);
	mutex_lock(&pCh->mutex);
	if (pdp->val){
		RESET_FLAG(pCh->events, EVENT_EXIT_ECP_MODE);
		if (pCh->isECP){
			dmx_crit(pCh->tunerHandle, "%s-%d: already in ECP MODE\n", __func__, __LINE__);
		}
		else
			SET_FLAG(pCh->events, EVENT_ENTER_ECP_MODE);
	}
	else{
		RESET_FLAG(pCh->events, EVENT_ENTER_ECP_MODE);
		if (pCh->isECP == 0){
			dmx_crit(pCh->tunerHandle, "%s-%d: NOT IN  ECP MODE\n", __func__, __LINE__);
		}
		else
