...
TPK_TP_ENGINE_T JAPAN4K_Channel_Mapping(TPK_TP_ENGINE_T input)
{
#if 1
	switch(input)
	{
...
int IOCTL_DeliveryOpen(DEMUX_DELIVERY_CHANNEL_T *pChannel)
{
		...
		else if( delivery_mode == DELIVERY_MODE_JAPAN4K )
		{
...
int IOCTL_DeliverySetTPInputConfig(DEMUX_TP_SOURCE_CONFIG_T *pInfo)
{
		...
		{
			tp_id = JAPAN4K_Channel_Mapping(tp_id);
...
int IOCTL_DeliveryRequestData(DEMUX_DELIVERY_CHANNEL_T *channel)
{
        ...
        DEMUX_CHECK_CH(channel->ch);

        tp_id = DEMUX_GET_DELIVERY_ID(channel->ch);

        if( delivery_mode == DELIVERY_MODE_JAPAN4K) {
#if 0 // USE_RAW_TO_GET_TLV
                tp_id_remapping = JAPAN4K_Channel_Mapping(tp_id);
...
int IOCTL_DeliveryGetData(DEMUX_DELIVERY_READ_DATA_T *pData)
{
        ...
        DEMUX_CHECK_CH(pData->ch);

        tp_id = DEMUX_GET_DELIVERY_ID(pData->ch);
