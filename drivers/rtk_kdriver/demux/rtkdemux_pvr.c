...
void PVR_CheckPvrAudioStable(demux_channel *pCh, int64_t pts)
{
	...
	dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,  pCh->tunerHandle, "(((vSysPTS:%lld, aDemuxPTS:%lld, stable = %d)))\n", videoSystemPTS, pts, HAS_FLAG(pCh->status, STATUS_PVR_AUDIO_IS_STABLE) != 0);
...
void PVR_EnableTPRateControl(demux_channel *pCh, bool bEnable)
{
	...
	if (RHAL_GetTpStatus(pCh->tunerHandle, &tpStatus) == TPK_SUCCESS) {
		...
		RESET_FLAG(pCh->status, STATUS_TP_RATE_CONTROL_ENABLED);
		dmx_crit(pCh->tunerHandle, "[%s %d] Error!RHAL_GetTpStatus fail! Can't start TP Rate Control !\n", __func__, __LINE__);
	}
#else
	RHAL_MTPFrameControl(pCh->tunerHandle, false);
	RESET_FLAG(pCh->status, STATUS_TP_RATE_CONTROL_ENABLED);
...
void PVR_CheckPTSDiff(demux_channel *pCh)
{
	...
	if (((pPcrProcesser->startVideoPts > pPcrProcesser->startAudioPts) && (pPcrProcesser->startVideoPts -pPcrProcesser->startAudioPts) < 2000*DEMUX_CLOCKS_PER_MS && (pPcrProcesser->lastDemuxAudioPTS - pPcrProcesser->startVideoPts) >= pCh->avSyncStartupPTS)
		...
		dmx_crit(pCh->tunerHandle, "[%s %d] Start to play: APTS = %lld, VPTS = %lld\n", __func__, __LINE__, pPcrProcesser->lastDemuxAudioPTS, pPcrProcesser->lastDemuxVideoPTS);
	}

	if (pCh->bPendingOnPTSCheck && REFCLK_GetRCD(refClock) != -1) {
		PVR_PvrStartToPlay(pCh);
		dmx_crit(pCh->tunerHandle, "[%s %d] Check PTS timeout: APTS = %lld, VPTS = %lld\n", __func__, __LINE__, pPcrProcesser->lastDemuxAudioPTS, pPcrProcesser->lastDemuxVideoPTS);
	}

	if ((REFCLK_GetRCD(refClock) == -1 && pPcrProcesser->startVideoPts != -1 && pPcrProcesser->startAudioPts != -1 && abs64(pPcrProcesser->startVideoPts -pPcrProcesser->startAudioPts) >= 2000*DEMUX_CLOCKS_PER_MS)
		...
		pCh->avSyncStartupFullness = 768 * 1024;
		pCh->bPendingOnFullness = true;
		pCh->avSyncStartupPTS = 0;
		pCh->bPendingOnPTSCheck = false;
		dmx_crit(pCh->tunerHandle, "[%s %d] A/V startPTS diff >= 180000 (2 secs)\n", __func__, __LINE__);
	}
}
...
END:
	...
	if (print_info) {
		if (ret) {
			dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle, "\033[1;33m""ch:%d, a:%d ms, v:%d ms, synMode:%d, speed:%d, isPause:%d, pts:(%lld, %lld), sysPts:(%lld, %lld), underflow(%d,%d) ===>ret = %d (%d--%d): (0x%02x) \033[m\n", 
			...
			dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG,  pCh->tunerHandle, "\033[1;33;46m""ch:%d, a:%d ms, v:%d ms, synMode:%d, speed:%d, isPause:%d, pts:(%lld, %lld), sysPts:(%lld, %lld), underflow(%d,%d) ===>ret = %d (%d--%d): (0x%02x) \033[m\n", 
				pCh->tunerHandle,(int)(pCh->pcrProcesser.lastDemuxAudioPTS-aSysPTS)/90,(int)(pCh->pcrProcesser.lastDemuxVideoPTS-vSysPTS)/90,pCh->avSyncMode,pCh->pvrSpeed , HAS_FLAG(pCh->status, STATUS_AUDIO_PAUSED),
				pCh->pcrProcesser.lastDemuxAudioPTS,pCh->pcrProcesser.lastDemuxVideoPTS,aSysPTS,vSysPTS,DEMUX_reverseInteger(pRefClock->AO_Underflow),DEMUX_reverseInteger(pRefClock->VO_Underflow),ret, actionMonitor ,actionMonitor_subCount, conditionMonitor);
		}
