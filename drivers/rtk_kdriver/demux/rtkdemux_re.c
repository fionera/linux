...
static unsigned long dmxDevInfo[]={
	(unsigned long)&(((demux_dev*)0)->VTP_Info),                                  //0
	(unsigned long)&(((demux_dev*)0)->totalDroppingPin),                          //1
	(unsigned long)&(((demux_dev*)0)->pinMap),                                    //2
...
static unsigned long chInfo[]={
	...
	(unsigned long)&(((demux_channel*)0)->video_deliver_size),                     //23
	(unsigned long)&(((demux_channel*)0)->audio_deliver_size),                     //24
	(unsigned long)&(((demux_channel*)0)->startStreaming),                         //25

#ifndef DEMUX_AUDIO_USE_GLOBAL_BUFFER
	(unsigned long)&(((demux_channel*)0)->bsAudioBuf),
	(unsigned long)&(((demux_channel*)0)->bsAudioBufH), 
	(unsigned long)&(((demux_channel*)0)->ibAudioBuf),                             
	(unsigned long)&(((demux_channel*)0)->ibAudioBufH),                     
#endif
...
int DEMUX_TA_ECP_UpdatePininfo(demux_channel *pCh, unsigned int pin, DEMUX_STREAM_TYPE_T streamType, unsigned char bufferType, unsigned long buffer_Header, unsigned long buffer)
{
	...
	dmx_err(pCh->tunerHandle, "%s_%d: pin:%d, stream:%d\n", __func__, __LINE__, pin, streamType);
	rtk_flush_range(pCh, (void*)((long)(pCh) +sizeof(demux_channel)));
	outer_flush_range(virt_to_phys((void*)pCh), virt_to_phys((void*)pCh)+sizeof(demux_channel));	
...
int DEMUX_TA_ECP_UpdatePESBufferinfo(demux_channel *pCh, unsigned int pin, unsigned int isSetup)
{
	...
	dmx_err(pCh->tunerHandle, "%s_%d: pin:%d, \n", __func__, __LINE__, pin);
	rtk_flush_range(pCh, (void*)((long)(pCh) +sizeof(demux_channel)));
	outer_flush_range(virt_to_phys((void*)pCh), virt_to_phys((void*)pCh)+sizeof(demux_channel));	
	rtk_flush_range(demux_device, (void*)((long)demux_device + sizeof(demux_dev)) );
	outer_flush_range(virt_to_phys(demux_device), virt_to_phys(demux_device) + sizeof(demux_dev));
...
int DEMUX_TA_EnterECP(demux_channel *pCh){
	if (Demux_ta_Init()<0)
	{
		dmx_err(CHANNEL_UNKNOWN,"%s_%d: init DEMUX TA FAIL\n",__func__, __LINE__);
	}
	///
	if (pCh == NULL)
		return 0;
	dmx_err(pCh->tunerHandle, "%s_%d: tpaddrssoffset:%ld\n", __func__, __LINE__, pCh->phyAddrOffset);
	rtk_flush_range(pCh, (void*)((long)(pCh) +sizeof(demux_channel)));
	outer_flush_range(virt_to_phys((void*)pCh), virt_to_phys((void*)pCh)+sizeof(demux_channel));
	rtk_flush_range(demux_device, (void*)((long)demux_device + sizeof(demux_dev)) );
	outer_flush_range(virt_to_phys(demux_device), virt_to_phys(demux_device) + sizeof(demux_dev));
	...
	if (Demux_ta_Construct(pCh) < 0){
		dmx_err(CHANNEL_UNKNOWN,"%s_%d: Demux_ta_Construct FAIL\n",__func__, __LINE__);
	}
...
int DEMUX_TA_ExitECP(demux_channel *pCh){
	if (pCh != NULL){
		...
		if (Demux_ta_Deconstruct(pCh) <0){
			dmx_err(CHANNEL_UNKNOWN,"%s_%d: Demux_ta_Deconstruct FAIL\n",__func__, __LINE__);
