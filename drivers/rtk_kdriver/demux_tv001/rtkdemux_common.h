...
struct _tagRefClock {
	...
	u32            videoFreeRunThreshold;
	u32            audioFreeRunThreshold; /* if APTS < LPTS and LPTS - APTS < audioFreeRunThreshold, then drop. Otherwise, output */
	long long      masterGPTS;   /* this is the value of GPTS (hardware PTS) when master set the reference clock*/
	s32            audioFullness;   /* This flag will be turned on when AE's output buffer is almost full.*/
	...
	s32            audioPauseFlag;  /* This flag will be turned on when VE decides to auto-pause.*/
	/* AE which is monitoring this flag will auto-pause itself then.*/
	/* (0: ignore.  1: AE have to pauseEncode immediately)*/

	s32            VO_Underflow; /* (0: VO is working properly; otherwise, VO underflow)*/
	s32            AO_Underflow; /* (0: AO is working properly; otherwise, AO underflow)*/
	s32            videoEndOfSegment; /* set to the flow EOS.eventID by VO after presenting the EOS sample*/
	s32            audioEndOfSegment; /* set to the flow EOS.eventID by AO after presenting the EOS sample*/
	s32            distanceVo;        /* distance between video and master in ms*/
	u32            decodedQueueDepth; /* Number of decoded pictures which is ready to be displayed.*/
	u32            decodingPicturePTS;/* PTS of currently decoded picture.*/
	long long      RCD_Audio;       /* set by audio*/
	long long      RCD_Video;       /* set by video*/
	u32            clockMode;
	u32            audioFreeRunThresholdToWait;	/* if APTS > LPTS and APTS - LPTS < audioFreeRunThresholdToWait, then wait. Otherwise, output */
};
...
typedef struct _tagRingBufferHeader {
	...
	u32           beginAddr;
	u32           size;
	u32           bufferID;  /* RINGBUFFER_TYPE, choose a type from RINGBUFFER_TYPE*/

	u32           writePtr;
	u32           numOfReadPtr;
	u32           reserve2;  /*Reserve for Red Zone*/
	u32           reserve3;  /*Reserve for Red Zone*/

	u32           readPtr[4];

	s32           fileOffset ;
	s32           requestedFileOffset ;
	s32           fileSize ;

	s32           bSeekable ;  /* Can't be sought if data is streamed by HTTP */
	/*
FileOffset:
