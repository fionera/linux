...
int IOCTL_SetTPInputConfig(DEMUX_TP_SOURCE_CONFIG_T *pInfo)
{
	...
	/*  1.  stream control    */
	dmx_notice("1.  stream control  %s, line %d\n", __func__, __LINE__);
	...
	if (pInfo->inputType == TPK_INPUT_SERIAL)
	{
		...
		dmx_notice( "[%s %d] %s - Set TP%d to Serial mode!\n", __FILE__, __LINE__, __func__, tp_id);
	}
	else if (pInfo->inputType == TPK_INPUT_PARALLEL)
	{
		tpStatus.tp_param.serial            = 0 ;
		dmx_notice("[%s %d] %s - Set TP%d to Parallel mode!\n", __FILE__, __LINE__, __func__, tp_id) ;
	}
	...
	*/
	...
	if(tp_src == MTP) {
		...
		for (j = 0; j < DEMUX_NUM_OF_TARGETS; j++) 
			pCh->demuxTargetInfo[j].bCheckContiCounter = 0;
