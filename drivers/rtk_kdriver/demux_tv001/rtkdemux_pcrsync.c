...
{
	pPcrProcesser->rcdUnstableCount      = 0;
	pPcrProcesser->rcdUnstableCheckCount = 0;
...
void PCRSYNC_AdjustPcrOffset(DEMUX_PCRSYNC_T *pPcrProcesser, NAVDTVPCRINFO pcrinfo, int64_t pll)
{
	...
	if (closerInfo.demuxAPTS != -1 || closerInfo.demuxVPTS != -1) {
		...
		/*FIXME start*/
		if (pPcrProcesser->referenceAVPTS == AUDIO_TYPE && pPcrProcesser->lastDemuxSubAudioPTS > 0) {
			audioDiff = pPcrProcesser->lastDemuxAudioPTS - pPcrProcesser->lastDemuxSubAudioPTS;
			pPcrProcesser->bHighWaterLevelMode = bCheckSubAudio = true;
			dmx_mask_print(DEMUX_PCRSYC_DEBUG,"audioDiff = %lld pcr - pts = %lld\n", audioDiff, pcr - pts);
		}
		/*FIXME end*/

		if ((pPcrProcesser->bAdjusted == false) && (pPcrProcesser->rcd != -1) && (pPcrProcesser->referenceAVPTS == beforeReferenceAVPTS) && !bCheckSubAudio) {
			/*
				...
				If there is any AD path, we need to re-calculate pcrOffset again.
			*/
			...
			/*
				No PCR
			*/
			/*FIXME pcrDefaultOffset member variable*/
		...
		/*FIXME start*/
			/*
				High water level
				Compensate by addition DTV_DELAY_TIME
				If it has AD path and audio diff(main-sub) is too lager, we enlarge default pcrOffset to 90K;
				if it has AD path, we set default pcrOffset to 72K;
				otherwise, default pcrOffset is 54K
			*/
			pPcrProcesser->bHighWaterLevelMode = true;
			if (bCheckSubAudio && audioDiff > 2*DTV_DELAY_TIME)
				pPcrProcesser->pcrDefaultOffset = 5*DTV_DELAY_TIME;
			else if (bCheckSubAudio)
				pPcrProcesser->pcrDefaultOffset = 4*DTV_DELAY_TIME;
			else
				pPcrProcesser->pcrDefaultOffset = 3*DTV_DELAY_TIME;

			pPcrProcesser->pcrOffset = pcr - pts + pPcrProcesser->pcrDefaultOffset;

			if (pPcrProcesser->pcrOffset < DTV_DELAY_TIME)
				pPcrProcesser->pcrOffset = DTV_DELAY_TIME;
...
void PCRSYNC_UpdateRCDStableStatus(DEMUX_PCRSYNC_T *pPcrProcesser, NAVDTVPCRINFO pcrinfo)
{
	...
	int64_t rcd_diff = abs(rcd - pPcrProcesser->rcd);

	if (pPcrProcesser->rcd != -1 && (pcrinfo.stc > 0 && pcrinfo.stc != pPcrProcesser->stc)) {
		if (((pPcrProcesser->isTrackingSteady == 1) && (rcd_diff > 400)) || ((pPcrProcesser->isTrackingSteady == 0) && (rcd_diff > 800)))
			pPcrProcesser->rcdUnstableCount++;
		if (pPcrProcesser->rcdUnstableCount)
			pPcrProcesser->rcdUnstableCheckCount++;
	
		//printf("ready=[%d], count=[%d/%d], rcd_diff=[%lld].\n", status.isSteady, m_rcdUnstableCount, m_rcdUnstableCheckCount, (rcd - m_rcd));
	}
}

void PCRSYNC_CheckRCDStable(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	if (pPcrProcesser->rcdUnstableCount > 100) {
		pPcrProcesser->bVirtualPcrMode = true;
		pPcrProcesser->bHighWaterLevelMode = true;
		pPcrProcesser->checkPcrOffset = PREPARE_TO_GET_OFFSET;
		dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d)]:RCD Check: [%d][%d/%d].\n", pPcrProcesser->tunerHandle, pPcrProcesser->bNoPCR, pPcrProcesser->rcdUnstableCount, pPcrProcesser->rcdUnstableCheckCount);
	}
	if (pPcrProcesser->checkPcrOffset != HAVE_STEADY_OFFSET || pPcrProcesser->rcdUnstableCheckCount > 1000 || pPcrProcesser->isTrackingSteady == 2 || pPcrProcesser->isTrackingSteady == 3) {
		if (pPcrProcesser->rcdUnstableCheckCount > 1000)
			dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d)]:RCD Check: [%d][%d/%d].\n", pPcrProcesser->tunerHandle, pPcrProcesser->bNoPCR, pPcrProcesser->rcdUnstableCount, pPcrProcesser->rcdUnstableCheckCount);
...
/*
[ANDROIDTV-2386]
There is a short lag when other sources switch to DTV.
If av diff is lager than 90K(DTV_DELAY_TIME*5), we set checkPcrOffset to PREPARE_TO_GET_OFFSET;
otherwise, there is no need to re-sync again.
test on WOSQRTK-DVBT2-UHD.ts and other ts
*/
	if (bVideo) {
