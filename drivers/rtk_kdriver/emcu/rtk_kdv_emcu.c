...
void emcu_set_boot_cmd_irda(void)
{
	...
	if(irda_cnt >= PARAM_LEN_IRDA){
		...
		if(vaild_irda_cnt > keylist_len)
			vaild_irda_cnt = keylist_len;

		venus_ir_keylist_to_scanlist(
				user_key,
    ...
    const char *buf, size_t count)
{
    ...
    if(emcu_get_param_multi(16,5,&param_cnt,param,buf,count)){
        EMCU_ALERT("\n(%s)get param fail\n",__func__);
        ret = -EINVAL;
        goto FINISH_OUT;
    }
    ...
    const char *buf, size_t count)
{
    ...
    if(emcu_get_param_multi(16,1,&param_cnt,param,buf,count)){
        EMCU_ALERT("\n(%s)get param fail\n",__func__);
        ret = -EINVAL;
        goto FINISH_OUT;
    }
    ...
    const char *buf, size_t count)
{
    ...
    EMCU_DEBUG("\n(%s)count : %d\n",__func__,count);
    EMCU_DEBUG("\n(%s)%s\n",__func__,buf);
    if(emcu_get_param_multi(16,67,&param_cnt,param,buf,count)){
        EMCU_ALERT("\n(%s)get param fail\n",__func__);
        ret = -EINVAL;
        goto FINISH_OUT;
    }
...
{
    return sprintf(buf,
        "please key in \"echo  err_event > "
        "/sys/devices/virtual/misc/emcu_kdv/emcu_set_err_status\"\n");
}
...
#ifdef WAIT_IR_FUNC_READY    //fixme
