...
#define ENABLE_GDMA_HWDESC_PROTECT 1
...
#if defined(GDMA_POWER_AUTO_SAVE)
struct task_struct *gdma_pm_thread=NULL;
static int gdma_pm_func(void*);
#define GDMA_PM_THREAD_INTERVAL  1000000
#define GDMA_PM_ALPHA_ALL_ZERO_NUM (3840*2160)
#define GDMA_PM_ALPHA_CHECK_COUNT 3
...
//RTK mark for netfix
int enable_osd1osd4mixerorder = 0;
...
size_t gdma_dcmt_monitor_end = 0;
#endif
...
int GDMA_AI_SE_draw_block(int s_w,int s_h,int num,unsigned int *color,KGAL_RECT_T *ai_block) ;
...
int g_osd_acc_sync[GDMA_PLANE_MAXNUM] = {0}; // flag
int g_osd_udfl[GDMA_PLANE_MAXNUM] = {0};
int g_sr_udfl[OSD_SR_MAXNUM] = {0};
...
extern int SE_EXT_FilmDetect_test(void);
...
#if defined(CONFIG_ARCH_RTK288O) && defined(THRESHOLD_DETECT)
#if defined(GDMA_POWER_AUTO_SAVE)
static int gdma_pm_func(void *p) {
	//int i;
	//unsigned int total;
	gdma_usleep(GDMA_PM_THREAD_INTERVAL*10);
	while(1) {
		gdma_usleep(GDMA_PM_THREAD_INTERVAL);
#if 0
		printk("\n ================  \n");
		total = 0;
		for(i=0;i<ALPHA_THRESHOLD_NUM;i++){
			printk("[%d] = %d \n",i,alpha_osd_detect_cnt[i]);
			total += alpha_osd_detect_cnt[i];
		}
		printk("\n ====total %d ===  \n",total);
#else
		if(alpha_osd_detect_cnt[0] == GDMA_PM_ALPHA_ALL_ZERO_NUM)
		{
		    if(count_check == GDMA_PM_ALPHA_CHECK_COUNT){
...
#define ALLOW_HW_TOUCH_LENGTH   (256+32)    //for usb minimun touch
...
static void osd_debug_info(void)
{
	...
	printk(KERN_ERR"======================================\n");
	printk(KERN_ERR"\tw, h=%08x\n",rtd_inl(GDMA_DBG_OSD_WI_0_reg));
...
static void gdma_config_fbc(void) {
	...
	dma_swap_reg.afbc_1b = 1;
	dma_swap_reg.afbc_2b = 1;
	dma_swap_reg.afbc_4b = 1;
	dma_swap_reg.afbc_8b = 1;
...
{
    //RTK mark for netfix
	enable_osd1osd4mixerorder = flag;
	return 1;
}
...
static void gdma_config_line_buffer_for_AI(void) {
...
int gdma_osd_writedump(unsigned int type, unsigned int osdn, osd_dmem_info* pinfo,osd_dmem_info* pinfo_stch) {
	...
	if(type == 0) {
		...
		rtd_outl(OSDOVL_Mixer_CRC_Ctrl_reg, OSDOVL_Mixer_CRC_Ctrl_crc_start(1) | OSDOVL_Mixer_CRC_Ctrl_crc_conti(1) |
			OSDOVL_Mixer_CRC_Ctrl_mixer_crc_sel(osdn));
...
extern void kHAL_GAL_GetSurfaceMemInfo(void);
...
#if defined(CONFIG_ARCH_RTK288O) && defined(CONFIG_CUSTOMER_TV006)
				if(picObj->plane == VO_GRAPHIC_OSD2)
				{
					picObj->dst_height = picObj->height;
					picObj->dst_width = picObj->width;
					if(win->dst_x != 0 || win->dst_y != 0)
					{
						picObj->dst_x = (int)win->dst_x;
						picObj->dst_y = (int)win->dst_y;
					}
					else
					{
						picObj->dst_x = osdPlane->OSD_canvas.dispWin.x;
...
#if defined(CONFIG_ARCH_RTK288O) && defined(CONFIG_CUSTOMER_TV006)
		win->dst_x = (unsigned short)cmd->dest_winPos.x;
		win->dst_y = (unsigned short)cmd->dest_winPos.y;
#endif
	}
...
irqreturn_t GDMA_irq_handler(int irq, void *dev_id)
{
	...
	if (OSD_SR_OSD_SR_1_UDFLOW_get_osd_sr_in_udflow(rtd_inl(OSD_SR_OSD_SR_1_UDFLOW_reg))) {
		...
		if (!g_sr_udfl[OSD_SR_1])
			printk(KERN_EMERG"[GDMA][DMA.ERR] OSDSR-%d under-flow!!\n", OSD_SR_1);

		g_sr_udfl[OSD_SR_1] = 1;
#ifdef CONFIG_RTK_KDRIVER_SUPPORT
	...
	if (OSD_SR_OSD_SR_2_UDFLOW_get_osd_sr_in_udflow(rtd_inl(OSD_SR_OSD_SR_2_UDFLOW_reg))) {
		...
		if (!g_sr_udfl[OSD_SR_2])
			printk(KERN_EMERG"[GDMA][DMA.ERR] OSDSR-%d under-flow!!\n", OSD_SR_2);

		g_sr_udfl[OSD_SR_2] = 1;
#ifdef CONFIG_RTK_KDRIVER_SUPPORT
...
int GDMA_init_module(void)
{
...
#if defined(CONFIG_ARCH_RTK288O)
	gdma_devices = kmalloc(gdma_nr_devs * sizeof(gdma_dev) + 12000, GFP_KERNEL); // 58K -> 70K
#else
...
#if defined(CONFIG_RTK_KDRV_DC_MEMORY_TRASH_DETCTER)
#if defined(ENABLE_GDMA_HWDESC_PROTECT)&&ENABLE_GDMA_HWDESC_PROTECT==1
#if defined(CONFIG_ARCH_RTK288O)
	dcmt_monitor_gdma_range(3, (unsigned int)gdma_dev_addr + ALLOW_HW_TOUCH_LENGTH,
		(gdma_nr_devs * sizeof(gdma_dev)) - ALLOW_HW_TOUCH_LENGTH);
	gdma_dcmt_monitor_end = gdma_dev_addr + (gdma_nr_devs * sizeof(gdma_dev))
		- (ALLOW_HW_TOUCH_LENGTH + DCMT_ALIGN_SIZE);
#else
	dcmt_monitor_gdma_range(3, (unsigned int)gdma_dev_addr,gdma_nr_devs * sizeof(gdma_dev));
#endif
#endif
#endif
	...
	g_osdshift_upscale_ctrl.plane = VO_GRAPHIC_OSD1;
#if defined(CONFIG_ARCH_RTK288O)
	g_osdshift_upscale_ctrl.ini_w = 1920;
	g_osdshift_upscale_ctrl.ini_h = 1080;
	g_osdshift_upscale_ctrl.ini_dst_w = 3840;
	g_osdshift_upscale_ctrl.ini_dst_h = 2160;
#endif
...
*/
	...
	*/
...
#if defined(GDMA_POWER_AUTO_SAVE)	
	gdma_pm_thread = kthread_create(gdma_pm_func, NULL, "GDMA_POWER_GUARD");
	if(gdma_pm_thread){
		printk(KERN_ERR"[GDMA] %s , run pm thread..\n", __func__);
		wake_up_process(gdma_pm_thread);
	}
#endif
#endif
	...
	if (request_irq(gic_irq_find_mapping(GDMA_IRQ), GDMA_irq_handler, IRQF_SHARED, "gdma", gdma)) {
	...
	if(gdma_def_thread){
		printk(KERN_ERR"[SE] %s , run gdma def thread..\n", __func__);
...
DCMT_DEBUG_INFO_REGISTER(osd_mdscpu,osd_debug_info);
	...
	if (GAL_MEM_SIZE) {
		...
		printk(KERN_ERR"%s CONST_GAL_MEM_SIZE is empty!\n", __func__);
	}
...
int gdma_suspend_by_vbe(void) {
	...
	for (i = 0; i < gdma_nr_devs; i++) {
		free_irq(gic_irq_find_mapping(GDMA_IRQ), &gdma_devices[i]);
	}
...
static void GDMA_OSDReg_Set(GDMA_DISPLAY_PLANE disPlane)
{
	...
	if (disPlane == GDMA_PLANE_OSD3 && (pReg->osd_wi_reg.regValue == 0 || pReg->osd_wi_reg.regValue > gdma_total_ddr_size)) {
		printk("[GDMA] addr abnormal not updated!\n");
	} else {
...
int GDMA_Debug(KADP_HAL_GAL_CTRL_T *data)
	...
	switch (data->type) {
		...
		case GDMA_DBG_CMD_SET_SHIFT_UPSCALE:
			/*for tv006, need this*/
		break;
		...
		case GDMA_DBG_CMD_OSD_CTRL_EN:
			...
			break;
		case GDMA_DBG_CMD_SE_FILM:
			SE_EXT_FilmDetect_test();
			break;
		...
		case GDMA_DBG_CMD_OSD_BW_GEN_EN:
			...
			break;
		case GDMA_DBG_CMD_OSD_GAL_SURFACE_INFO:
			kHAL_GAL_GetSurfaceMemInfo();
			break;
...
int GDMA_ReceivePicture (PICTURE_LAYERS_OBJECT *data)
{
	...
	if(gosdpattern ==1 && data->layer[0].normal[0].address != gosdpattern_phy){
		return GDMA_SUCCESS;
	...
	for (ilayer = 0; ilayer < data->layer_num; ilayer++) {
		...
		if (picObj->layer_used == 1) {
			...
			*(char *)&pic->status = 0;
			...
			if (osd_gain_info.on_off) {
				pic->plane_ar.plane_alpha_a = picObj->plane_ar.plane_alpha_a;
				pic->plane_ar.plane_alpha_r = osd_gain_info.levelval;
				pic->plane_gb.plane_alpha_g = osd_gain_info.levelval;
				pic->plane_gb.plane_alpha_b = osd_gain_info.levelval;
			} else {
				/*if (com_picObj->normal_num > 0) */{
			...
			g_osdshift_upscale_ctrl.ini_addr = win->top_addr;
...
//for ANDROIDTV-305 test
            ...
            //win->attr.compress = win->attr.IMGcompress ;
			//printk("func=%s line=%d [GDMA] win->attr.IMGcompress = %08x\n ",__FUNCTION__,__LINE__,win->attr.IMGcompress);
			...
			for (idx = 0; idx < com_picObj->fbdc_num; idx++) {
				...
				if (idx == 0) { 
					...
					if (g_osdshift_upscale_ctrl.apply_enable
						&& disPlane == (GDMA_DISPLAY_PLANE)g_osdshift_upscale_ctrl.plane) {
						win->afbc_xy_sta.x_sta = g_osdshift_upscale_ctrl.apply_afbc_x_sta;
						win->afbc_xy_sta.y_sta = g_osdshift_upscale_ctrl.apply_afbc_y_sta;
					} else {
	...
	*/
	...
	if (get_platform() == PLATFORM_K6HP)
	{
		gdma_usleep(6500);//force to 60hz
	}
#endif
						  ...
						  int v_move_range, int apply)
{
	...
	if (disPlane == 1) {
		...
		g_osdshift_upscale_ctrl.plane = VO_GRAPHIC_OSD2;
		win = (GDMA_WIN *)phys_to_virt(rtd_inl(GDMA_OSD2_WI_reg));
	} else if (disPlane == 3) {
		g_osdshift_upscale_ctrl.plane = VO_GRAPHIC_OSD3;
		win = (GDMA_WIN *)phys_to_virt(rtd_inl(GDMA_OSD3_WI_reg));
	} else {
		printk(KERN_ERR"[%s] no OSD-%d, please check the plane setting!!\n", __FUNCTION__, disPlane);
		memset(&g_osdshift_upscale_ctrl, 0x00, sizeof(CONFIG_OSDSHIFT_UPSCALE_STRUCT));
...
int GDMA_AI_SE_draw_block(int s_w,int s_h,int num,
						  unsigned int *color,KGAL_RECT_T *ai_block) 
{
	unsigned int i;
...
static inline void rtkgdma_dbg_EXECUTE(const int cmd_index, char **cmd_pointer)
{
	...
	switch (cmd_index) {
		...
		case GDMA_DBG_CMD_SET_V_SHIFT:
			...
			return;
		case GDMA_DBG_CMD_SET_SHIFT_UPSCALE:
			sscanf(*cmd_pointer, "%d %d %d %d\n", &st.h_shift, &st.v_shift, &st.h_move_range, &st.v_move_range);

			g_osdshift_upscale_ctrl.h_shift_pixel = st.h_shift;
			g_osdshift_upscale_ctrl.v_shift_pixel = st.v_shift;
			
			if (g_osdshift_upscale_ctrl.h_shift_pixel)
				g_osdshift_upscale_ctrl.shift_h_enable = 1;
			else
				g_osdshift_upscale_ctrl.shift_h_enable = 0;
			
			if (g_osdshift_upscale_ctrl.v_shift_pixel)
				g_osdshift_upscale_ctrl.shift_v_enable = 1;
			else
				g_osdshift_upscale_ctrl.shift_v_enable = 0;
			
			GDMA_OSDShiftUpscale_wrapper(1, st.h_shift, st.v_shift, st.h_move_range, st.v_move_range, 0);
...
EXPORT_SYMBOL(GDMA_HideGraphicWindow);
