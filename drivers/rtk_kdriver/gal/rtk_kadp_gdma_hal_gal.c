...
long compat_GDMA_hal_gal_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
#else
int compat_GDMA_hal_gal_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
#endif
#endif
...
#ifdef CONFIG_COMPAT
	.compat_ioctl	= compat_GDMA_hal_gal_ioctl,
#endif
...
int GDMA_hal_gal_setframebuffer(KDRV_GAL_SURFACE_FRAMEBUFFER_INFO_T *data)
{
	...
	if (fb_index == 0) {
		gf_data->viewRect.w = 1920;
		gf_data->viewRect.h = 1080;
		gf_data->dispRect.w = 3840;
		gf_data->dispRect.h = 2160;
		gf_data->bVisibility = 1;
	}
	...
	if (gf_data->bVisibility && (gf_data->viewRect.w != 0)
	   && (gf_data->viewRect.h != 0) && (gf_data->dispRect.w != 0) && (gf_data->dispRect.w != 0)) {
		...
		if (picObj->address < GAL_MEM_ADDR_START || picObj->address > GAL_MEM_ADDR_START + GAL_MEM_SIZE || 
			(picObj->address + (gf_data->viewRect.w * gf_data->viewRect.h * pSurface->bpp / 8)) > GAL_MEM_ADDR_START + GAL_MEM_SIZE) {
			pr_debug("%s, surface addr out of range!\n", __FUNCTION__);
...
int GDMA_hal_gal_captureframebuffer(unsigned int osdn, KADP_HAL_GAL_WIN_INFO_T *pWinfo)
{
	pWinfo->width = GDMA_DBG_OSD_WI_0_get_width(rtd_inl(GDMA_DBG_OSD_WI_0_reg));
	pWinfo->height = GDMA_DBG_OSD_WI_0_get_height(rtd_inl(GDMA_DBG_OSD_WI_0_reg));
	pWinfo->pitch = GDMA_DBG_OSD_WI_4_get_pitch(rtd_inl(GDMA_DBG_OSD_WI_4_reg));
	pWinfo->top_addr = GDMA_DBG_OSD_WI_2_get_top_addr(rtd_inl(GDMA_DBG_OSD_WI_2_reg));
	pWinfo->compress = GDMA_DBG_OSD_WI_1_get_img_compress(rtd_inl(GDMA_DBG_OSD_WI_1_reg));
	...
	if (GDMA_DBG_OSD_WI_1_get_colortype(rtd_inl(GDMA_DBG_OSD_WI_1_reg)) == VO_OSD_COLOR_FORMAT_ARGB8888 ||
		...
		pWinfo->pixelFormat = KADP_HAL_GAL_PIXEL_FORMAT_ARGB;
	} else {
		printk(KERN_ERR"%s: unsupport pixel format %d\n", __func__, GDMA_DBG_OSD_WI_1_get_colortype(rtd_inl(GDMA_DBG_OSD_WI_1_reg)));
		return GDMA_HAL_GAL_FAIL;
	}
	//printk(KERN_ERR"%s win addr = 0x%x %d(%d, %d) compress = %d\n", __func__, pWinfo->top_addr, pWinfo->pixelFormat, pWinfo->width, pWinfo->height, (int)pWinfo->compress);
	return GDMA_HAL_GAL_SUCCESS;
}
...
int GDMA_hal_gal_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GDMA_DRV_GAL_CAPTUREFRAMEFUBBER: {
			KADP_HAL_GAL_WIN_INFO_T winfo;
			unsigned int osdn = 1;

			if (GDMA_hal_gal_captureframebuffer(osdn, &winfo) == GDMA_HAL_GAL_SUCCESS) {
				if (copy_to_user((void __user *)arg, &winfo, sizeof(KADP_HAL_GAL_WIN_INFO_T))) {
					DBG_GAL_PRINT_ERR("GDMA HAL GAL: ioctl GDMA_DRV_GAL_CAPTUREFRAMEFUBBER failed *******\n");
					ret = -EFAULT;
				} else
					ret = GDMA_HAL_GAL_SUCCESS;
			}
...
#if defined(CONFIG_ARM64)
static char *GDMA_hal_gal_devnode(struct device *dev, umode_t *mode)
#else
