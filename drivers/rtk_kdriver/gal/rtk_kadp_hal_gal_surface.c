...
#define gSURFACE_MAX  (512)	// 20150522 LG max index usage is 143


static unsigned int gSURFACE_INDEX = 1;
static unsigned int gSURFACE_dec_INDEX = 1;
static unsigned int gSURFACE_dec_1_INDEX = 1;
static unsigned int gSURFACE_dec_2_INDEX = 1;
static unsigned int gSURFACE_dec_3_INDEX = 1;
...
//tmp_handle = ion_alloc(gHAL_ION_CLIENT, size, 4096,  ION_HEAP_TYPE_CARVEOUT, flags);
		tmp_handle = ion_alloc(gHAL_ION_CLIENT, size, 0,  ION_HEAP_TYPE_CUSTOM_MASK, flags);



		if ( IS_ERR(tmp_handle) ) {
#if defined(CONFIG_ARM64)
			pr_debug(KERN_WARNING " GAL %s fail 0x%x(0x%lx)  size:%d\n", __FUNCTION__,  (unsigned int)PTR_ERR(tmp_handle),  (unsigned long)tmp_handle, size);
#else
...
#if defined(CONFIG_ARM64)
			pr_debug(KERN_WARNING " GAL %s toPhys fail %d handle:0x%lx\n", __FUNCTION__, ret, (unsigned long)handle);
#else
...
#if defined(CONFIG_ARM64)
		pr_debug(KERN_WARNING " GAL err!! size <0 handle:0x%lx\n", (unsigned long) rt_surf->addr_handle);
#else
...
void kHAL_GAL_GetSurfaceMemInfo(void)
{
	unsigned int i=0,totol_size=0,local_count=0;
	RtSurface* rt_surf = NULL;

	//mutex_lock(&gSURFACE_MGR_LOCK);
	printk(KERN_WARNING"\n ======== Surface Mem Info Start  ========\n");
	for( ; i< gSURFACE_MAX; i++) {
		rt_surf = &(gSURFACE_LIST[i]);

		if( rt_surf->valid ) {
			printk(KERN_WARNING"w=%d h=%d size=%d pid=%d\n",rt_surf->width,rt_surf->height,rt_surf->size,rt_surf->pid);
			local_count++;
            totol_size += rt_surf->size;
		}
	}
	printk(KERN_WARNING"total_size=%d gSURFACE_INDEX=%x gSURFACE_dec_INDEX=%x \n",totol_size,gSURFACE_INDEX,gSURFACE_dec_INDEX);
	printk(KERN_WARNING"gSURFACE_dec_1_INDEX =%x gSURFACE_dec_2_INDEX =%x gSURFACE_dec_3_INDEX =%x \n",gSURFACE_dec_1_INDEX,gSURFACE_dec_2_INDEX,gSURFACE_dec_3_INDEX);
	printk(KERN_WARNING"\n gSURFACE_INDEX-gSURFACE_dec_INDEX =%d local_count=%d\n",gSURFACE_INDEX-gSURFACE_dec_INDEX,local_count);
	printk(KERN_WARNING"\n ======== Surface Mem Info End  ========\n");
...
#if defined(CONFIG_ARM64)
		DBG_GAL_PRINT_ERR("[GAL]%s fail !! surfIdx:%u (%u %u)--%u %u) 0x%lx 0x%x\n", __FUNCTION__, pSurfaceInfo->vendorData,
						   ...
						   KADP_HAL_GAL_SURFACE_INFO_T *pSurfaceInfo)
{
	...
	else
	{
		...
		tmp_pitch = ALIGN(tmp_pitch, STRIDE_ALIGN);
#endif
	...
	rt_surf->pid = current->pid;


#ifdef GAL_USE_RESERVE_MEM
#if defined(CONFIG_ARM64)
	rt_surf->virAddr = (unsigned long)phys_to_virt((unsigned long)dvr_addr);
	rt_surf->phyAddr = (unsigned long)dvr_addr;
#else
...
#if defined(CONFIG_ARM64)
	rt_surf->virAddr = (unsigned long)dvr_addr;
	rt_surf->phyAddr =(unsigned long)dvr_to_phys((unsigned long)dvr_addr);
#else
	rt_surf->virAddr = (unsigned int)dvr_addr;
	rt_surf->phyAddr =(unsigned int)dvr_to_phys((unsigned int)dvr_addr);
#endif

	gal_surf->offset = 0; //invalid

#endif
	if(rt_surf->phyAddr < GAL_MEM_ADDR_START || rt_surf->phyAddr > GAL_MEM_ADDR_START+GAL_MEM_SIZE) {
#if defined(CONFIG_ARM64)
		DBG_GAL_PRINT_ERR("%s : error address = 0x%lx\n", __FUNCTION__, rt_surf->phyAddr);
#else
		DBG_GAL_PRINT_ERR("%s : error address = 0x%x\n", __FUNCTION__, rt_surf->phyAddr);
#endif
		error = RT_HAL_GAL_ERR_MEM;
		goto ERR_RT_HAL_GAL_CreateSurface;
	}
...
//gal_surf->paletteInfo = ;
#if defined(CONFIG_ARCH_RTK288O)
	gal_surf->property = size; //LG webos 5.0 use this member for mmap memory length
#else
	gal_surf->property = 0;
#endif
	...
	if((width*height) <  GFX_SPECIAL_SIZE && total_index < GFX_NUM_MAX && (total_size+size) < GFX_CURSOR_SIZE)
	{
		  gfx_data[total_index].phyAddr = gal_surf->physicalAddress;
#if defined(CONFIG_ARM64)
		  gfx_data[total_index].virtAddr = (unsigned long)phys_to_virt(gal_surf->physicalAddress);
#else
...
//pRTGraphicMemInfo->memInfo.largestFreeMemSize =
		...
		for(i=0; i < gSURFACE_MAX; ++i) {
			...
			if( rt_surf->valid ) {
				allo_size += rt_surf->size;
			}
		}
		pRTGraphicMemInfo->memInfo.allocatedMemTotalSize = allo_size;
		pRTGraphicMemInfo->memInfo.freeMemTotalSize = GAL_MEM_SIZE - allo_size;
#else
		for(i=0; i < gSURFACE_MAX; ++i) {
			rt_surf = &(gSURFACE_LIST[i]);
...
static int __init early_parse__gal_fbc(char *str)
{
	...
	printk(KERN_EMERG "[rtkgpu %s:%d] fbcOnOff=%x \n", __FUNCTION__, __LINE__, fbcOnOff);

    return 0;
}

EXPORT_SYMBOL(fbcOnOff);
