...
#define LP_DDR_MAX_ADDR 0x60000000
#define HP_DDR_MAX_ADDR 0x80000000
#define MEMPRISON_MIN_ADDR 0x16000000
#define MEMPRISON_MAX_ADDR 0x16a00000
#endif
	...
	KGAL_BLIT_SETTINGS_T* pBlitSettings)
{
	...
	KGAL_SURFACE_INFO_T* pfSrcSurface = pSrcSurface;
	SE_PROCINFO_T  stSrcProcInfo,stDstProcInfo;
	SEINFO_ROTATE rotmode = SEINFO_ROTATE_90;
	SEINFO_SETUPMODE stMode = SEINFO_BITBLT_SETUP_MANUAL;//dummy
...
static bool SEBoundaryGuard(KGAL_SURFACE_INFO_T* pSurface, KGAL_RECT_T* pRect) {
	if(pSurface == NULL || pRect == NULL)
		return false;
	if (pSurface->width == 0 || pSurface->height == 0 || pRect->w == 0 || pRect->h == 0) {
		printk(KERN_ERR"[%s]=============================================================\n", __func__);
        printk(KERN_ERR"[%s] input size zero!! pSurface->width = %d, pSurface->height = %d, pRect->w = %d, pRect->h = %d\n",
        	__func__, pSurface->width, pSurface->height, pRect->w, pRect->h);
		return false;
	}
	...
	if(pSurface->physicalAddress == 0) {
		printk(KERN_ERR"[%s]=============================================================\n",__func__);
		printk(KERN_ERR"[%s] phyAddr(%08x) is not correct!!\n", __func__, pSurface->physicalAddress);

        return false;
    }
#if defined(CONFIG_ARCH_RTK288O)
	if ((get_platform() == PLATFORM_K6HP && pSurface->physicalAddress > HP_DDR_MAX_ADDR) ||
		(get_platform() == PLATFORM_K6LP && pSurface->physicalAddress > LP_DDR_MAX_ADDR) ||
		(pSurface->physicalAddress >= MEMPRISON_MIN_ADDR && pSurface->physicalAddress <= MEMPRISON_MAX_ADDR)) {
		printk(KERN_ERR"[%s]=============================================================\n",__func__);
		printk(KERN_ERR"[%s] phyAddr(%08x) is not correct!!\n", __func__, pSurface->physicalAddress);
...
static bool SEFormatGuard(KGAL_SURFACE_INFO_T* pSurface, SEINFO_SETUPMODE stpMode, SE_COLOR_FORMAT_EX_T *fmt, bool st /*=false*/)
{
	...
	if(pSurface->width > 4096 || pSurface->width == 0 ||
		pSurface->height > 8096 || pSurface->height == 0 ||
		pSurface->pitch > 4096*4 || pSurface->pitch ==0 ||
