...
static int __init se_init_module(void)
{
	...
	if (request_irq
	    (gic_irq_find_mapping(IRQ_SE), rtk_se_irq_handler, IRQF_SHARED, "se", (void *)se_devices))
...
//#endif	
	...
	//printk("rtd_inl(SEOFFSET(SeCmdReadPtr))=%x \n",rtd_inl(SEOFFSET(SeCmdReadPtr)));
	//printk("rtd_inl(SEOFFSET(SeCmdWritePtr))=%x \n",rtd_inl(SEOFFSET(SeCmdWritePtr)));
	//printk("func=%s line=%d \n",__FUNCTION__,__LINE__);		
#endif
	rtd_outl(SEOFFSET(SeCtrl), 0x7);
	UpdateInstCount(dev);
...
#if 0
	while((rtd_inl(0xb800CE90)&0x1) !=0x01){
		se_usleep(10);
		if(retry++ > 10000)
			break;
	}
	retry = 0;
	printk("func=%s line=%d \n",__FUNCTION__,__LINE__);	
	printk("func=%s line=%d retry=%d\n",__FUNCTION__,__LINE__,retry);
	printk("func=%s line=%d rtd_inl(SEOFFSET(SeCmdReadPtr))=%x \n",__FUNCTION__,__LINE__,rtd_inl(SEOFFSET(SeCmdReadPtr)));
	printk("func=%s line=%d rtd_inl(SEOFFSET(SeCmdWritePtr))=%x \n",__FUNCTION__,__LINE__,rtd_inl(SEOFFSET(SeCmdWritePtr)));
	printk("func=%s line=%d rtd_inl(0xB800CE90)=%x \n",__FUNCTION__,__LINE__,rtd_inl(0xb800CE90));	
    printk("func=%s line=%d rtd_inl(0x1800C3F0)=%x \n",__FUNCTION__,__LINE__,rtd_inl(0xb800C3F0));	
	printk("func=%s line=%d \n",__FUNCTION__,__LINE__);	

    printk("func=%s line=%d rtd_inl(0xb800C180)=%x \n",__FUNCTION__,__LINE__,rtd_inl(0xb800C180));	
	printk("func=%s line=%d rtd_inl(0xb800C3F0)=%x \n",__FUNCTION__,__LINE__,rtd_inl(0xb800C3F0));	
#endif		
	up(&sem_cmdqueue);
}

void WriteCmdISR(struct se_dev *dev, uint8_t* pbyCommandBuffer,
                int32_t lCommandLength) {
        uint32_t dwDataCounter = 0;
        uint32_t wrptr = 0, rdptr = 0;
        uint32_t u32Alignment = 0;
