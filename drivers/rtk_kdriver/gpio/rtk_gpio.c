...
static int __init rtk_gpio_init(void)
{
    ...
    gpio_iowrite32(GPIO_GPIE_1_reg, GPIO_GPIE_1_gp47_mask | GPIO_GPIE_1_get_write_data(1));

    if (request_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_MISC), gpio_interrupt_handler,IRQF_SHARED, "MIS_GPIO", (void *)~MIS_GPIO) < 0)
    {
    ...
    if(ISO_GPIO_CNT)
    {
        if(request_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_ISO), gpio_interrupt_handler, IRQF_SHARED, "ISO_GPIO",(void *)~ISO_GPIO) < 0)
        {
    ...
    if (GPI_ISO_CNT)
    {
        if (request_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_ISO), gpio_interrupt_handler,IRQF_SHARED, "GPI_ISO", (void *)~ISO_GPI) < 0)
        {
            RTK_GPIO_DBG("rtk_gpio_init failed,request iso irq %d failed\n",IRQ_ISO);
            ret = -EIO;
...
err_register_platform_driver_failed:
    ...
    if( ISO_GPIO_CNT || GPI_ISO_CNT )
        free_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_ISO), gpio_interrupt_handler);
#endif
...
static void __exit rtk_gpio_exit(void)
{
    ...
    free_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_MISC), gpio_interrupt_handler);
...
int rtk_gpio_isr_register(RTK_GPIO_ID gid,
                           void (*isr_handler) (RTK_GPIO_ID gid,
                           unsigned char assert, void *dev_id),char *name,
                           unsigned char debounce_time, unsigned char polarity)
{
    int ret = 0;
    ret = rtk_gpio_set_dir(gid, 0);
    ret = rtk_gpio_set_debounce(gid, debounce_time);
    ret = rtk_gpio_set_irq_polarity(gid, polarity);
    ret = rtk_gpio_request_irq(gid, isr_handler, name,isr_handler);
    ret = rtk_gpio_set_irq_enable(gid, 1);
    return ret;
}
EXPORT_SYMBOL(rtk_gpio_isr_register);

int rtk_gpio_isr_unregister(RTK_GPIO_ID gid,void (*isr_handler) (RTK_GPIO_ID gid,
                           unsigned char assert, void *dev_id))
{
    int ret = 0;
    ret = rtk_gpio_set_irq_enable(gid, 0);
    ret = rtk_gpio_clear_isr(gid);
#ifdef CONFIG_ARM64
    rtk_gpio_free_irq(gid, (void*)0);
#else
    rtk_gpio_free_irq(gid, isr_handler);
#endif
    return ret;
}
EXPORT_SYMBOL(rtk_gpio_isr_unregister);
...
int rtk_gpio_get_pcb_info(char* pinName,RTK_GPIO_PCBINFO_T*pInfo){
    ...
    for(i = 0; i < MAX_PCB_GPIO_COUNT; i++){
        if(strncmp(pPcbInfo->pcblist[i].pin_name, pinName, sizeof(pPcbInfo->pcblist[i].pin_name))==0){
