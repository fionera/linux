...
const MODULE_INFO module_info[] =
{
        ...
        {MODULE_MEMC_MC_W0,     "MEMC_MC_W0"},
        {MODULE_MEMC_MC_W1,     "MEMC_MC_W1"},
        {MODULE_MEMC_MC_R0,     "MEMC_MC_R0"},
        {MODULE_MEMC_MC_R1,     "MEMC_MC_R1"},
        {MODULE_MEMC_MC_R2,     "MEMC_MC_R2"},
        {MODULE_MEMC_MC_R3,     "MEMC_MC_R3"},
        {MODULE_MEMC_ME_W1,     "MEMC_ME_W1"},
        {MODULE_MEMC_ME_R1,     "MEMC_ME_R1"},
        {MODULE_MEMC,   "MEMC"},
...
unsigned char key_table_cpus[] = {MODULE_SB2_ACPU,MODULE_SB2_VCPU};
...
//unsigned char trap_table_acpu2[] = {MODULE_SB2_ACPU2};
unsigned char trap_table_vcpu[] = {MODULE_SB2_VCPU};
//unsigned char trap_table_vcpu2[] = {MODULE_SB2_VCPU2};
...
const MODULE_SEARCH_TABLE trap_table[] =
{
        ...
        //{"ACPU2", trap_table_acpu2, sizeof(trap_table_acpu2)},
        {"VCPU", trap_table_vcpu, sizeof(trap_table_vcpu)},
        //{"VCPU2", trap_table_vcpu2, sizeof(trap_table_vcpu2)},
...
int get_dcmt_modues(char * buff)
{
        ...
        for(i = 0;i < 16;++i)
        {
                ...
                offset = snprintf(buff, strlen(FYELLOW_START) + 1, "%s",FYELLOW_START);
                buff += offset;
                ret += offset;
                ...
                offset = snprintf(buff,   strlen(FCOLOR_END) + 1, "%s\n", FCOLOR_END);
                buff += offset;
                ret += offset;
...
int get_module_keys(char * buff)
{
        ...
        for(i = 0;i < total;++i)
        {
                ...
                //DC_MT_ERR("add %s(%x-%x) @ offset:%d\n",name,i,j,offset);
                offset = snprintf(buff,16,  "%-25s", name);
                buff += offset;
                ret +=offset;
        ...
        offset = snprintf(buff,strlen(FCOLOR_END) + 1,  "%s", FCOLOR_END);
        buff += offset;
        ret += offset;
...
void init_dcregs_vari(void)
{
        ...
        for(entry = DC_MT_ENTRY_START; entry < DC_MT2_ENTRY_START; ++entry)
        {
        ...
        for(entry = DC_MT2_ENTRY_START; entry < DC_MTEX_ENTRY_START; ++entry)
        {
                init_dc1_mt2_regs(entry - DC_MT2_ENTRY_START, &dc_regs[DCID_1][entry]);
        }

        for(entry = DC_MTEX_ENTRY_START; entry < DC_MTEX2_ENTRY_START; ++entry)
        {
        ...
        for(entry = DC_MTEX2_ENTRY_START; entry < DC_MT_ENTRY_COUNT; ++entry)
        {
...
#ifdef CONFIG_ARM64
        unsigned long phy_kernel_start = (unsigned long)_text - PAGE_OFFSET;     //kernel text start ;
...
static void dcmt_monitor_over_range(void) //use mem4(mem4 is mtex-0)
{
        ...
        mt_desc.ip_entry[2].module = MODULE_TVSB4_OSD3;
        mt_desc.ip_entry[2].prot = PROT_MODE_RO; //access type read
...
#if 0//def CONFIG_REALTEK_SECURE
//dcmt_monitor_kcpu_text
static void dcmt_monitor_kcpu_text(unsigned int * addr) //use mem5(mem5 is mtex-1)
{
        DC_MT_DESC mt_desc;
        int kcpu_mem_entry = DCMT_DFT_MEM_KCPU;
        __maybe_unused unsigned int pagetable_size = 0x8000;

        memset(&mt_desc, 0, sizeof(mt_desc));

        mt_desc.type = MEMORY_TYPE_SERIAL_BLOCK;
#ifdef CONFIG_OPTEE
        mt_desc.start = addr[DCMT_RANGE_SYNC_SECURE_START];
        mt_desc.end = addr[DCMT_RANGE_SYNC_SECURE_END];
#else
        mt_desc.start = addr[DCMT_RANGE_SYNC_K_START];
        mt_desc.end = addr[DCMT_RANGE_SYNC_K_END];
#endif
        ...
        if((mt_desc.start == 0)||(mt_desc.end == 0))
        {
                ...
                carvedout_buf_query(CARVEDOUT_K_OS_1,(void *)&mt_desc.start);
                ...
                mt_desc.end = mt_desc.start + 0x13d000 - DCMT_ALIGN_SIZE; //monitor K-Linux range
#endif  // CONFIG_OPTEE
#endif  // #ifdef CONFIG_CUSTOMER_TV006
        }

        mt_desc.end -= DCMT_ALIGN_SIZE;
...
static void dcmt_monitor_mmu_table(void) //use mem8
{
        DC_MT_DESC mt_desc;
        int mmu_table_entry = DCMT_DFT_MEM_MMUTABLE;

        unsigned long mmu_table_start = (unsigned long)swapper_pg_dir - PAGE_OFFSET;     //mmu table start
        unsigned long mmu_table_end = (unsigned long)swapper_pg_dir - PAGE_OFFSET - DCMT_ALIGN_SIZE + SWAPPER_DIR_SIZE; //mmu table end
...
static void dcmt_monitor_scaler_range(void) //use mem10-14
{
        ...
        //set dcmt monitor CARVEDOUT_SCALER_MEMC
        memset(pDesc,0,sizeof(DC_MT_DESC));
        index = CARVEDOUT_SCALER_MEMC;
        pDesc->ip_entry[0].module = MODULE_MEMC;
        pDesc->ip_entry[0].prot = PROT_MODE_WO;
        mem_size = carvedout_buf_query(index,(void *)&mem_addr);
        if((mem_size != 0)&&(mem_addr != 0)){
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_MEMC, s_addr : %08x, e_addr : %08x, size : %08x\n", mem_addr, (mem_addr+mem_size), mem_size);
            pDesc->start = mem_addr;
            pDesc->end = mem_addr + mem_size - DCMT_ALIGN_SIZE;
            pDesc->type = MEMORY_TYPE_SERIAL_BLOCK;
            pDesc->mode = 3;
            dcmt_set_monitor(10,pDesc);
        }else{
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_MEMC set fail, addr : %08x, size : %08x\n", mem_addr, mem_size);
        }
        
        //set dcmt monitor CARVEDOUT_SCALER_DI_NR
        memset(pDesc,0,sizeof(DC_MT_DESC));
        index = CARVEDOUT_SCALER_DI_NR;
        pDesc->ip_entry[0].module = MODULE_TVSB2_DIW;
        pDesc->ip_entry[0].prot = PROT_MODE_WO;
        mem_size = carvedout_buf_query(index,(void *)&mem_addr);
        if((mem_size != 0)&&(mem_addr != 0)){
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_DI_NR(W), s_addr : %08x, e_addr : %08x, size : %08x\n", mem_addr, (mem_addr+mem_size), mem_size);
            pDesc->start = mem_addr;
            pDesc->end = mem_addr + mem_size - DCMT_ALIGN_SIZE;
            pDesc->type = MEMORY_TYPE_SERIAL_BLOCK;
            pDesc->mode = 3;
            dcmt_set_monitor(11,pDesc);
        }else{
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_DI_NR(W) set fail, addr : %08x, size : %08x\n", mem_addr, mem_size);
        }
        
        memset(pDesc,0,sizeof(DC_MT_DESC));
        index = CARVEDOUT_SCALER_DI_NR;
        pDesc->ip_entry[0].module = MODULE_TVSB2_DIR;
        pDesc->ip_entry[0].prot = PROT_MODE_RO;
        mem_size = carvedout_buf_query(index,(void *)&mem_addr);
        if((mem_size != 0)&&(mem_addr != 0)){
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_DI_NR(R), s_addr : %08x, e_addr : %08x, size : %08x\n", mem_addr, (mem_addr+mem_size), mem_size);
            pDesc->start = mem_addr;
            pDesc->end = mem_addr + mem_size - DCMT_ALIGN_SIZE;
            pDesc->type = MEMORY_TYPE_SERIAL_BLOCK;
            pDesc->mode = 3;
            dcmt_set_monitor(12,pDesc);
        }else{
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_DI_NR(R) set fail, addr : %08x, size : %08x\n", mem_addr, mem_size);
        }
        
        //set dcmt monitor CARVEDOUT_SCALER_OD
        memset(pDesc,0,sizeof(DC_MT_DESC));
        ...
        mem_size = carvedout_buf_query(index,(void *)&mem_addr);
        if((mem_size != 0)&&(mem_addr != 0)){
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_OD(W), s_addr : %08x, e_addr : %08x, size : %08x\n", mem_addr, (mem_addr+mem_size), mem_size);
            pDesc->start = mem_addr;
            pDesc->end = mem_addr + mem_size - DCMT_ALIGN_SIZE;
            pDesc->type = MEMORY_TYPE_SERIAL_BLOCK;
            pDesc->mode = 3;
            dcmt_set_monitor(13,pDesc);
        }else{
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_OD(W) set fail, addr : %08x, size : %08x\n", mem_addr, mem_size);
        }
        ...
        mem_size = carvedout_buf_query(index,(void *)&mem_addr);
        if((mem_size != 0)&&(mem_addr != 0)){
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_OD(R), s_addr : %08x, e_addr : %08x, size : %08x\n", mem_addr, (mem_addr+mem_size), mem_size);
            pDesc->start = mem_addr;
            pDesc->end = mem_addr + mem_size - DCMT_ALIGN_SIZE;
            pDesc->type = MEMORY_TYPE_SERIAL_BLOCK;
            pDesc->mode = 3;
            dcmt_set_monitor(14,pDesc);
        }else{
            DCMT_LOG(DCMT_LOG_CMD, KERN_ERR, "CARVEDOUT_SCALER_OD(R) set fail, addr : %08x, size : %08x\n", mem_addr, mem_size);
        }
...
#endif
        dcmt_monitor_scaler_range();//use mem10-14 for scaler MEMC/OD/DI
...
int dcmt_monitor_carvedout_mem(char * key)
{
        ...
        else if(isStrFullMatch(key,"MAP_RPC",0) == 0)
        {
                ...
                //pDesc->ip_entry[2].prot = PROT_MODE_RW;
#ifdef DCMT_AV2
                pDesc->ip_entry[3].module = MODULE_SB2_ACPU2;
                pDesc->ip_entry[3].prot = PROT_MODE_RW;
                pDesc->ip_entry[4].module = MODULE_SB2_VCPU2;
                pDesc->ip_entry[4].prot = PROT_MODE_RW;
#endif
        }
#endif
        ...
        else if(isStrFullMatch(key,"VDEC_RINGBUF",0) == 0)
        {
                ...
                pDesc->ip_entry[2].prot = PROT_MODE_RW;
#ifdef DCMT_AV2
                pDesc->ip_entry[3].module = MODULE_SB2_VCPU2;
                pDesc->ip_entry[3].prot = PROT_MODE_RW;
#endif
...
static int __init dc_mt_module_init(void)
{
        ...
        if (!dc_mt_kobj)
	{
        ...
        //dc_mt_irq_init();
        printk("%s %d   xxxxxxxxxxxxxxxxxxxxxxxx\n",__func__,__LINE__);
...
static int dc_mt_probe_remove(struct platform_device *pdev)
{
        return 0;
}
