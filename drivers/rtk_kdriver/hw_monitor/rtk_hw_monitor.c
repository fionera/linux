...
static int is_hwm_desc_invalid(hwm_t *pParam)
{
        ...
        if(pParam == NULL)
        {
                ...
                return -1; //[CID_27428] may dereferencing null pointer pParam.
        }
...
static int translate_desc_to_str(hwm_t *pParam)
{
        ...
        for(i = 0;i < 8;++i)
        {
                ...
                else
                {
                        ...
                        len = snprintf(buf,128-cnt,"%s",tmp);
                        buf += len;
                        cnt += len;
#endif
                }
...
static void __exit hw_monitor_module_exit(void)
{
        return;
}
