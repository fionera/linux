...
{    
    ...
    if( rtd_inl(SB2_INV_INTSTAT_reg) || (rtd_inl(EMCU_wrapper_8051_reg) & EMCU_wrapper_8051_rbus_timeout_int_mask) )
    {
        console_loglevel = 6;
        sb2_timeout_handler(addr, fsr, regs);
        return 0;
    }
    return -1;
}
...
void init_timeout_regs(void)
{
        ...
        rtd_outl(EMCU_wrapper_8051_reg, EMCU_wrapper_8051_val | EMCU_wrapper_8051_rbus_timeout_cycle(5) | 
                        EMCU_wrapper_8051_rbus_timeout_int_en(1) | EMCU_wrapper_8051_get_rbus_timeout_enable(1));
...
bus_err:
	...
	// ack timeout state and print debug info
        ...
        if(panic_on_oops)
        {
                rtd_save_keylog(KERN_ERR,"SB2_TIMEOUT","\n Timeout status 0x%08x:0x%08x, 0xb8060400:0x%08x,module:%s \n",
                        SB2_INV_INTSTAT_reg, rtd_inl(SB2_INV_INTSTAT_reg), rtd_inl(0xb8060400),sb2_module_name_all);
        }
