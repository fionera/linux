...
static irqreturn_t rtk_i2c_isr(int this_irq, void *dev_id)
{
    ...
    if ((GET_IC_CON(p_this) & IC_SLAVE_DISABLE) == 0)
    {
        while (NOT_RXEMPTY(p_this)
                && p_this->slave_rx_len < sizeof(p_this->slave_rx_buffer))
        {
        ...
        if (event & STOP_DET_BIT && p_this->slave_rx_len)
        {
            if (p_this->slave_ops.handle_command)
            {
                p_this->slave_ops.handle_command(
                        p_this->slave_id, p_this->slave_rx_buffer, p_this->slave_rx_len);
            }
...
int rtk_i2c_slave_mode_enable(rtk_i2c * p_this, unsigned char on)
{
    ...
    if (on)
    {
        ...
        {
            wr_reg(p_this->reg_map.IC_ISR_EN, rd_reg(p_this->reg_map.IC_ISR_EN) | p_this->reg_map.IC_ISR_EN_MASK);
        }
...
int rtk_i2c_start_xfer(rtk_i2c * p_this)
{
    ...
    {
        if(p_this->id == 0)
            SET_I2C_SCPU_INT_EN(ISO_MISC_OFF_SYSTEM_i2c0_scpu_int_en(1));
        else if(p_this->id == 5)
            SET_I2C_SCPU_INT_EN(ISO_MISC_OFF_SYSTEM_i2c5_scpu_int_en(1));
