...
typedef struct RPC_STRUCT {
	unsigned int programID;      // program ID defined in IDL file
	unsigned int versionID;      // version ID defined in IDL file
	unsigned int procedureID;    // function ID defined in IDL file

	unsigned int taskID;         // the caller's task ID, assign 0 if NONBLOCK_MODE
	unsigned int sysPID;         // the callee's task ID
	unsigned int parameterSize;  // packet's body size
	unsigned int mycontext;      // return address of reply value
} RPC_STRUCT;
...
#ifndef RPC_RECORD_ADDR_SIZE       /* fixme. it should be sizeof (RPC_RECORE_Dev), not verified yet */
#define RPC_RECORD_ADDR_SIZE 52
#endif
typedef struct RPC_RECORE_Dev {
	...
	u32 dummy[8];
}   RPC_RECORE_Dev; /*struct to record the status of rpc dev*/
...
static inline void peek_rpc_struct(const char *func, RPC_COMMON_Dev *dev)
{
	...
	if (rpc.programID == AUDIO_AGENT && rpc.versionID == 0) {
		...
		if (rpc.procedureID == 1)
			printk("%s: alloc %lu bytes\n", func, arg);
		else
			printk("%s: free addr %lx\n", func, arg);
...
{
#ifdef CONFIG_KDRIVER_USE_NEW_COMMON
	printk(KERN_NOTICE "%s: program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lu size:%lu context:%x %s\n",
		func, rpc->programID, rpc->versionID, rpc->procedureID, rpc->taskID, rpc->sysPID, rpc->parameterSize,
		rpc->mycontext, in_atomic() ? "atomic" : "");

#else
	printk(KERN_NOTICE "%s: program:%u version:%u procedure:%u taskID:%u sysPID:%x size:%u context:%x %s\n",
		func, rpc->programID, rpc->versionID, rpc->procedureID, rpc->taskID, rpc->sysPID, rpc->parameterSize,
		rpc->mycontext, in_atomic() ? "atomic" : "");
#endif
}
...
#ifdef CONFIG_KDRIVER_USE_NEW_COMMON
		pr_err("%s-rpc(%p),program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lu size:%lu context:%x %s\n",
			extra->name, out, rpc.programID, rpc.versionID, rpc.procedureID, rpc.taskID, rpc.sysPID, rpc.parameterSize,
			rpc.mycontext, in_atomic() ? "atomic" : "");
#else
		pr_err("%s-rpc(%p),program:%u version:%u procedure:%u taskID:%u sysPID:%x size:%u context:%x %s\n",
			extra->name, out, rpc.programID, rpc.versionID, rpc.procedureID, rpc.taskID, rpc.sysPID, rpc.parameterSize,
			rpc.mycontext, in_atomic() ? "atomic" : "");
#endif
	}
...
#define RPC_VCPU_SET_DUMPLOG 0x132
...
#ifdef CONFIG_ARM64_RPC
extern RPC_INTR_Dev rpc_intr_devices[RPC_INTR_DEV_TOTAL];
extern RPC_POLL_Dev rpc_poll_devices[RPC_POLL_DEV_TOTAL];
extern RPC_KERN_Dev rpc_kern_devices[RPC_NR_KERN_DEVS];
#else
