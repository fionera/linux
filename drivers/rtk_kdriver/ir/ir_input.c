...
#endif
    keyup_jiffies = jiffies + msecs_to_jiffies(ir_poll_settings[which][1]);
...
void venus_ir_input_report_key(uint32_t repkey)
{
    ...
    if(ir_poll_settings[MAIN1][0] || ir_poll_settings[MAIN0][0]) {
        if (data->prev_keycode && data->prev_keycode != data->keycode && timer_pending(&keyup_timer)) {
            IR_DBG( "[%s,%d,%s]keyup_jiffies==%lu,jiffies==%lu,keycode_up==%x\n",__FILE__,__LINE__,__FUNCTION__,keyup_jiffies,jiffies,data->prev_keycode);
            input_report_key(data->input_dev, data->prev_keycode, 0);
            input_sync(data->input_dev);
            data->prev_keycode = 0;
            data->dataRepeat = 0;
...
u32 venus_ir_scancode_to_keycode(uint32_t value)
{
    ...
    if ((value & 0xFFFFFF) == TV030_JP_CUSTOMER_CODE) {
...
u32 venus_ir_keycode_to_scancode(uint32_t value)
{
	...
	scancode = venus_ir_user_key_table_find_scancode(ir_get_standby_protocol() ,value);
	if(scancode != 0) {
	...
	if(scancode != 0 || custcode != 0) {
		    ...
		    scancode = ((scancode & 0x7f) << 20) | ((custcode & 0xfff) << 27);
                    ...
                    scancode = (scancode & 0x7f) | ((custcode & 0xfff) << 7);
                    scancode = (scancode << 17);
                }
        }
        break;
...
int venus_ir_input_init(void)
{
    ...
    data->input_dev->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_REL) | BIT_MASK(EV_MSC);
#ifdef CONFIG_RTK_KDRV_DYNAMIC_INPUT_DEV_NAME
    ...
    SET_INPUT_DEV_KEY_BITS(&tcl_tv_factory_key_table, data->input_dev);
