...
#ifdef CONFIG_RTK_KDRV_TV043_IR
static struct timer_list data_toggle_timer;
static unsigned long toggle_jiffies = 0;
static u32 data_toggle = 0;
#endif
    ...
    if(srValue & 0x1) {
        ...
        *irrp2Value = rtd_inl(rpReg2);
        ...
        if(ir_protocol == PANASONIC) {
            *irrp2Value = rtd_inl(rpReg2); // read IRRP2
        }
        */
        *regValue = rtd_inl(rpReg); /* read IRRP */

        /* it should be remove in merlin4
...
#ifdef CONFIG_RTK_KDRV_TV043_IR
void ir_check_data_toggle(unsigned long _data)
{
    uint32_t srValue = rtd_inl(REG_IR_SR);

    IR_DBG("%s\n", __func__);
    if(!(srValue & BIT(10))) {
        IR_DBG("%s: no data toggle[0x%08x]\n", __func__, srValue);
...
#ifdef CONFIG_RTK_KDRV_TV043_IR
	if((ir_protocol == SHARP) && ((regValue & (1 << 31)) != 0)) {
		IR_DBG("Sharp ir inverted key , ignored .. \n");
		return 0;
	}	
#endif
    ...
    if(dataRepeat == 1 && (jiffies_to_msecs(jiffies) - lastRecvMs) < debounce) {
...
#ifdef CONFIG_RTK_KDRV_TV043_IR
        else if(ir_protocol == NEC) {
            if(data_toggle == 1 && dataRepeat == 0) {
                IR_DBG("%s: release event\n", __func__);
                venus_ir_input_report_end(0);
            }
        }
#endif
        ...
        if(ir_protocol == PANASONIC && irrp2Value != 0x20020000) {
            ...
            if(!ir_poll_settings[which][0]) {
                ...
                data_toggle = 0;
                toggle_jiffies = jiffies + msecs_to_jiffies(ir_poll_settings[which][1]);
                mod_timer(&data_toggle_timer, toggle_jiffies);
#endif
            } else {
                schedule_venus_ir_keyup_handler(which);
            }
...
int ir_suspend_protocol_multi_irda(void)
{
    /* Pin Mux */
    ...
    /* Use main1 decode in suspend */
      IR_Init(irda_proto_list[ir_support_protocols[0]].proto_standby_id, irda_proto_list[ir_support_protocols[1]].proto_standby_id, OFF);
...
#ifdef CONFIG_RTK_KDRV_TV043_IR
    /* Setup release event timer for Roku */
    setup_timer(&data_toggle_timer, ir_check_data_toggle, 0);
#endif
