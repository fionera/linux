...
{
    lsadc_st_pad0_RBUS st_pad0_reg;
...
int rtk_lsadc_set_lsadc_config(RTK_LSADC_INFO  *lsadc_info)
{
    int ret   = -1;
    lsadc_st_pad0_RBUS st_pad0_reg;
    lsadc_st_lsadc_ctrl_RBUS st_lsadc_ctrl_reg;
    ...
    if(lsadc_info->pinNumber >= 0) {
        ...
        if(!st_lsadc_ctrl_reg.enable && lsadc_info->pad_active) {
            lsadc_st_lsadc_analog_ctrl_RBUS st_ana_ctrl_reg;
...
{
    lsadc_st_lsadc_analog_ctrl_RBUS anaglog;
    anaglog.regValue = rtd_inl(ANALOG_CTRL);
    return anaglog.regValue;
}
int rtk_lsadc_dump_mainLsadcCtrl(void)
{
    lsadc_st_lsadc_ctrl_RBUS ctrl;
    ctrl.regValue = rtd_inl(LSADC_CTRL);
