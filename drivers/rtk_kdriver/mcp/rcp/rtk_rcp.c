...
)
{
    ...
    unsigned char* temp_ptr = NULL; //do not set mode from cw, read it and set by reg
    if(pData != NULL && (unsigned long)pData <= MAX_CW_ENTRY) {
        temp_cw = pData;
        if(Mode == KEY_MODE && DataLen == 7) // des
            temp_len = DataLen + 1;
        else
            temp_len = DataLen;
        temp_ptr = (unsigned char *)kmalloc(temp_len, GFP_ATOMIC);
        if (temp_ptr == NULL)
            return NULL;
        if(RTK_RCP_GET_CW((unsigned long)temp_cw,temp_ptr,temp_len)!=0)
            goto failed;
        if(Mode == KEY_MODE && DataLen == 7) // des CW key we take [7..1]
            pData = temp_ptr + 1;
        else
            pData = temp_ptr;
    }
    ...
    if(temp_ptr) {
        pData = temp_cw + (pData - temp_ptr)/8;
        kfree(temp_ptr);
    }
    return pData;
failed:
                                        ...
                                        unsigned char* pDataIn, unsigned char* pDataOut, unsigned long DataLen)
{
    ...
    unsigned char* temp_cw = NULL; //do not crypt to cw, to reg first then set to cw
    if((unsigned long)pDataOut <= MAX_CW_ENTRY) {
        temp_cw = pDataOut;
        temp_ptr = (unsigned char *)kmalloc(DataLen, GFP_ATOMIC);
        if (temp_ptr == NULL)
            return 0;
        pDataOut = temp_ptr;
    }
    ...
    if(temp_ptr) {
        if(RTK_RCP_SET_CW((unsigned long)temp_cw,temp_ptr,out_len)!=0)
            out_len = 0;
        kfree(temp_ptr);
    }
    return out_len;
...
)
{
    ...
    //if Key from CW ,dereference pKey when set INPUT_ENTRY will error
    if(pDataIn == NULL || pDataOut == NULL || (unsigned long)Key <= MAX_CW_ENTRY) {
        RCP_WARNING("Run RTK_RCP_TDES_EBC_Cipher failed\n");
        return 0;
    }
    RTK_RCP_LOCK(rtk_rcp_lock, flags);
...
)
{
    ...
    if (pDataIn == NULL || pDataOut == NULL || (unsigned long)Key <= MAX_CW_ENTRY || IV == NULL) {
        RCP_WARNING("Run RTK_RCP_DES_CBC_Cipher failed, IV should not be NULL \n");
        return 0;
    }
...
)
{
    ...
    unsigned char  ecnt[32];
    unsigned int* pDI   = (unsigned int*) pDataIn;
    unsigned int* pDO   = (unsigned int*) pDataOut;    
    unsigned int* pEcnt = (unsigned int*) ecnt;    
...
int RTK_RCP_CW_XOR(int CwIn1, int CwIn2, int CwOut)
{
    unsigned int i = 0;
    int ret = 0;
    unsigned char cw_data1[16];
    unsigned char cw_data2[16];
    unsigned char cw_out[16];
    if(CwIn1 <= 0 || CwIn1 > MAX_CW_ENTRY || CwIn2 <= 0 ||
       CwIn2 > MAX_CW_ENTRY || CwOut <= 0 || CwOut > MAX_CW_ENTRY)
        return ret;	
    if(RTK_RCP_GET_CW(CwIn1, cw_data1, 16) != 0) {
        ret = 0;
	 goto EXIT;
    }

    if(RTK_RCP_GET_CW(CwIn2, cw_data2, 16) != 0) {
        ret = 0;
	 goto EXIT;
    }

    for(i = 0; i < 16; i++) {
        cw_out[i] = (cw_data1[i] ^ cw_data2[i]); 
    }

   if(RTK_RCP_SET_CW(CwOut, cw_out, 16) != 0) {
	 ret = 0;
	 goto EXIT;
   }
   ret = 1;
    ...
    switch(RCP_MODE(p_desc->flags)) {
...
#ifdef CONFIG_ARM64
                    ret = RTK_RCP_CW_XOR((int)(unsigned long)p_desc->key, (int)(unsigned long)p_desc->iv, (int)(unsigned long)p_desc->data_out);
#else
                    ret = RTK_RCP_CW_XOR((int)p_desc->key, (int)p_desc->iv, (int)p_desc->data_out);
#endif
                    break;
                default:
                    ret = 0;
        ...
        case RCP_ALGO_CW_OPSEL: {
            switch(RCP_CW_OPSEL(p_desc->flags)) {
                case GET_CW:
#ifdef CONFIG_ARM64
                    ret = _RCP_GET_CW((unsigned int)(unsigned long)p_desc->data_in, p_desc->data_out, p_desc->len);
#else
                    ret = _RCP_GET_CW((unsigned int)p_desc->data_in, p_desc->data_out, p_desc->len);
#endif
                    ret = (ret == 0) ? p_desc->len : 0;
                    break;
                case SET_CW:
#ifdef CONFIG_ARM64
                    ret = _RCP_SET_CW((unsigned int)(unsigned long)p_desc->data_out, p_desc->data_in, p_desc->len);
#else
                    ret = _RCP_SET_CW((unsigned int)p_desc->data_out, p_desc->data_in, p_desc->len);
#endif
                    ret = (ret == 0) ? p_desc->len : 0;
                    break;
            }
