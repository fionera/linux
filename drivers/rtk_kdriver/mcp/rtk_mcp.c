...
static dma_addr_t desc_list_dma_handle_bak = 0;
static mcp_desc* desc_list_bak;
static unsigned long desc_list_size = 0;
...
static int _mcp_phy_init(void)
{
    ...
    SET_MCP_CTRL1(MCP_DMA_BURST_LEN(0x30) | MCP_AES_PAD_OFF(1) |                  /* disable AES_H auto padding*/
...
static int do_mcp_command(mcp_desc* p_desc, int n_desc, int is_user_pointer)
{
    ...
    while (n_desc) {
        int desc_cnt = (n_desc > MCP_ONCE_DESC_ENTRY_COUNT) ? MCP_ONCE_DESC_ENTRY_COUNT : n_desc;
...
int MCP_AES_128_CBC_Update_IV(int EnDe,char* key,
      char* iv, char* p_in, char* p_out, unsigned long len)
{
    mcp_desc desc;
    mcp_desc* pDesc = &desc;
    unsigned char* iv_buff;
    dma_addr_t addr1;
    dma_addr_t addr2;
    dma_addr_t iv_buff_phy;
    int ret = -1;

    if(key == NULL || iv == NULL || p_in == NULL
            || p_out == NULL || len == 0) {
        mcp_warning("MCP_AES_128_CBC_Update_IV failed - input invalid\n");
        return -EINVAL;
    }
    iv_buff = dma_alloc_coherent(mcp_device, 32, &iv_buff_phy, GFP_KERNEL);
    if (!iv_buff) {
        mcp_warning("MCP_AES_128_CBC_Update_IV failed - dma_alloc_coherent fail\n");
        return -ENOMEM;
    }
    memcpy(iv_buff, iv, 16);
    memset(pDesc, 0, sizeof(mcp_desc));
    pDesc->flags = MCP_MODE(MCP_ALGO_AES) | MCP_BCM(MCP_BCM_CBC) | MCP_ENC(EnDe);
    pDesc->flags |= MCP_KEY_SEL(MCP_KEY_SEL_DESC);
    pDesc->key[0] = (key[3] << 24) | (key[2] << 16) | (key[1] << 8) | key[0];
    pDesc->key[1] = (key[7] << 24) | (key[6] << 16) | (key[5] << 8) | key[4];
    pDesc->key[2] = (key[11] << 24) | (key[10] << 16) | (key[9] << 8) | key[8];
    pDesc->key[3] = (key[15] << 24) | (key[14] << 16) | (key[13] << 8) | key[12];

    pDesc->flags |= MCP_IV_SEL3(1) | MCP_IV_SEL(0) | MCP_IV_SEL(0);
    pDesc->iv[0] = iv_buff_phy;
    ...
    desc.length   = len & ~0xFF00000F;

    ret = mcp_do_command(&desc, 1);
    memcpy(iv, iv_buff, 16);

    _mcp_unmap_single(addr1, len, DMA_TO_DEVICE);
    _mcp_unmap_single(addr2, len, DMA_FROM_DEVICE);
    dma_free_coherent(mcp_device, 32, iv_buff, iv_buff_phy);
...
void do_mcp_throughput_calculate(mcp_desc* desc, dma_addr_t* data_in, dma_addr_t* data_out, unsigned long data_len,
                                 int ende, int bcm, char* str)
{
    unsigned long stime = 0;
    ...
    printk("[MCP] %s %s BCM(%d) Throughput %ld Kbytes elapsed %d us, \033[0;32;34m %d ms\n\033[m",
...
static int mcp_dma_init(struct device* dev_device, unsigned long max_desc_count)
{
    unsigned long malloc_size = 0;
    if (!dev_device)
        return -1;

    malloc_size = sizeof(mcp_desc) * max_desc_count  + MCP_DESCRIPTOR_HW_ALIGN;
    desc_list_size = malloc_size;
    desc_list_bak = dma_alloc_coherent(dev_device, desc_list_size, &desc_list_dma_handle_bak, GFP_ATOMIC);

    if (!desc_list_bak)
        return -1;
    if (((unsigned long)desc_list_bak % MCP_DESCRIPTOR_HW_ALIGN) != 0) {
        unsigned  long tmp = MCP_DESCRIPTOR_HW_ALIGN - ((unsigned long)desc_list_bak % MCP_DESCRIPTOR_HW_ALIGN);
        desc_list = (mcp_desc*)((unsigned long)desc_list_bak + tmp);
        desc_list_dma_handle = desc_list_dma_handle_bak + tmp;
    } else {
        desc_list = desc_list_bak;
        desc_list_dma_handle = desc_list_dma_handle_bak;
    }
    return 0;
}

static int mcp_dma_uninit(struct device* dev_device)
{
    dma_free_coherent(dev_device, desc_list_size,
                      desc_list_bak, desc_list_dma_handle_bak);
    return 0;
}

void __weak rtk_mcp_sb2_timeout_debug_info(void)
{
    sb2_timeout_info mcp_timeout_info;
    //check trash module
    if (!isSb2Timeout("CP"))
        return;

    //get sb2 timeout information
    get_sb2_timeout_info(&mcp_timeout_info);

    pr_err("Access CP register timeout,Trash address:0x%lx \n", mcp_timeout_info.timeout_reg);

    //printf debug info
    pr_err("CP clk: (0x%08x) = 0x%08x \n", SYS_REG_SYS_CLKEN1_reg, rtd_inl(SYS_REG_SYS_CLKEN1_reg));
    pr_err("CP reset: (0x%08x) = 0x%08x \n", SYS_REG_SYS_SRST1_reg, rtd_inl(SYS_REG_SYS_SRST1_reg));

    /*print some hw dbg info*/
    rtd_outl(0xb80008f8, (rtd_inl(0xb80008f8) & (~(0xF << 16))) | (0x1 << 16));
    rtd_outl(0xb80008f8, rtd_inl(0xb80008f8) | (0x1 << 20));
    rtd_outl(0xb80008f8, (rtd_inl(0xb80008f8) & (~(0xFF << 24))) | (0x22 << 24));
    rtd_outl(0xb80150cc, 0xB);
    pr_err("(0x%08x) = 0x%08x \n", 0xb80008fc, rtd_inl(0xb80008fc));
    rtd_outl(0xb80150cc, 0x19);
    pr_err("(0x%08x) = 0x%08x \n", 0xb80008fc, rtd_inl(0xb80008fc));
    return;
}

SB2_DEBUG_INFO_DECLARE(sb2_mcp_callback, rtk_mcp_sb2_timeout_debug_info);


int __init mcp_core_init(struct device* dev_device)
{
    int ret = 0;
    SB2_DEBUG_INFO_REGISTER(sb2_mcp_callback, rtk_mcp_sb2_timeout_debug_info);
    if (_mcp_phy_init() < 0) {
        ret = -1;
        goto FAIL_INIT_PHY;
    }
#ifdef MCP_INTERRUPT_ENABLE
    if (request_irq(gic_irq_find_mapping(IRQ_MCP), mcp_isr, IRQF_SHARED, "MCP", &mcp_wait_queue) < 0) {
        mcp_err("Request mcp irq failed\n");
        ret = -ENODEV;
        goto FAIL_REQUEST_IRQ;
    }
#endif
    if (mcp_dma_init(dev_device, MCP_DESC_ENTRY_COUNT) != 0) {
...
#ifdef MCP_INTERRUPT_ENABLE
    free_irq(gic_irq_find_mapping(IRQ_MCP), &mcp_wait_queue);
#endif
    mcp_dma_uninit(mcp_device);
    SB2_DEBUG_INFO_UNREGISTER(sb2_mcp_callback, rtk_mcp_sb2_timeout_debug_info);
