...
__init int rtk_md_init(struct device *md_device)
{
    ...
    /* fixme, %pad is not really supported by gcc. Casting to long to printk for a32/a64. */
    /* PDEIFO("Command Buffer Address = 0x%p, Physical Address = %pad, Size = %08x\n", */
    /*        md_drv_dev->CmdBuf, dwPhysicalAddress, md_drv_dev->size); */
    PDEIFO("Command Buffer Address = 0x%p, Physical Address = 0x%lx, Size = %08x\n",
           md_drv_dev->CmdBuf, (unsigned long)dwPhysicalAddress, md_drv_dev->size);
    md_drv_dev->wrptr = 0;
    md_drv_dev->CmdBase = dwPhysicalAddress;
    md_drv_dev->CmdLimit = dwPhysicalAddress + md_drv_dev->size;
    md_drv_dev->md_device = md_device;

    ret = request_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_MD), rtk_md_irq_handler,
