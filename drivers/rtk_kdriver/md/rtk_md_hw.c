...
//            rp =  (uint32_t*)(rtd_inl(MD_SMQRDPTR_reg) | 0xC0000000);
		rp =  (uint32_t*)(phys_to_virt(rtd_inl(MD_SMQRDPTR_reg)));
                    ...
                    uint32_t lCommandLength, int32_t go)
{
    ...
    uint32_t         pbyWritePointer = 0;
    uint8_t         *pWptr = NULL;
    dma_addr_t         pWptrPhy = 0;
    dma_addr_t      pWptrLimit = 0;
    uint32_t        u32_err_cnt = 0;

    pWptrLimit      = pdev->CmdLimit;
    pbyWritePointer = (uint32_t)rtd_inl(MD_SMQWRPTR_reg);
    pWptr           = (uint8_t *)pdev->CmdBuf + pdev->wrptr;
    pWptrPhy        = pdev->CmdBase + pdev->wrptr;
    ...
    u32_err_cnt = 0;
    while(1) {
        uint32_t pbyReadPointer = (uint32_t )rtd_inl(MD_SMQRDPTR_reg);
...
{
    if(on_off == 1)
        rtd_outl(MD_MULTI_REQ_reg, BIT(MD_MD_MULTI_REQ_multi_req_en_shift));  //Multi-req default value = 0x00000021
    else
        rtd_outl(MD_MULTI_REQ_reg, 0);
}
...
void rtk_md_reset_reg(struct md_dev *pdev, uint32_t set_multi_req)
{
    ...
    rtd_outl(MD_SMQBASE_reg, (uint32_t) pdev->CmdBase);
    rtd_outl(MD_SMQLIMIT_reg, (uint32_t) pdev->CmdLimit);
    rtd_outl(MD_SMQRDPTR_reg, (uint32_t) (pdev->CmdBase + pdev->wrptr));
    rtd_outl(MD_SMQWRPTR_reg, (uint32_t) (pdev->CmdBase + pdev->wrptr));
