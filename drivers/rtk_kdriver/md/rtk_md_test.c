...
#define TIMER_REG_VAL_ADDR  0xb801b690
#define TIMER_REG_CTRL_ADDR  0xb801b688
...
{
    uint64_t handle;
    dma_addr_t src_phys_addr = dma_map_single(pdev->md_device, (void *)srcBuf, length, DMA_BIDIRECTIONAL);
    dma_addr_t dst_phys_addr = dma_map_single(pdev->md_device, (void *)dstBuf, length, DMA_BIDIRECTIONAL);
    PDEWAR("[MD] virtual address: %p, %p\n", srcBuf, dstBuf);

    PDEWAR("[MD] physical address: %x, %x\n", (u32)src_phys_addr, (u32)dst_phys_addr);
...
int rtk_md_memcpy_multi_desc(uint8_t *srcBuf, uint8_t **dstBuf,
...
int rtk_multi_vs_signal(unsigned int length, unsigned int number)
{
    ...
    if(srcBuf != NULL) {
        ...
        for (i = 0; i < number; i++) {
            dstAry[i] = (uint8_t *) kmalloc(length, GFP_KERNEL);
            if(dstAry[i] == NULL) {
                int j = 0;
...
void rtk_md_unit_test(void)
{
    ...
    PDEWAR("[MD]test src addr = %p, dst addr = %p.\n", srcdata, dstdata);
    /*enable timer 90k */
    rtd_outl(TIMER_REG_CTRL_ADDR, 0x1);
    /*CASE - Seq to Seq test*/

    rtk_md_testcases_seq2seq(SPhysicalAddress, DPhysicalAddress, srcdata, dstdata, 0x1000);

    /*CASE - pitch test*/
    rtk_md_testcases_pitch(SPhysicalAddress, DPhysicalAddress, srcdata, dstdata, 0x1000);


err:
    if(srcdata != NULL)
        dma_free_coherent(md_drv_dev->md_device, 0x1000, (void*)srcdata, (dma_addr_t)SPhysicalAddress);
    if(dstdata != NULL)
        dma_free_coherent(md_drv_dev->md_device, 0x1000, (void*)dstdata, (dma_addr_t)DPhysicalAddress);
