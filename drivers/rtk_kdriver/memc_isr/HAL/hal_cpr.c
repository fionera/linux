...
VOID HAL_CPR_Set444Enable(BOOL bEnable)//modify from merlin4, using RTK pqc
{
	if(bEnable){
		...
		if(MEMC_Lib_get_INPUT_COLOR_SPACE_Mode()== 1){ //I3DDMA_COLOR_SPACE_T
			...
			rtd_clearbits(0xb8099284, 0x13); //cmp_data_color(bit:0 & 4)
			rtd_clearbits(0xb8099804, 0x11); //cmp_data_color(bit:0 & 4)
			rtd_clearbits(0xb8099884, 0x13); //cmp_data_color(bit:0 & 4)
	...
	else//Not 444, but 420 or 422
	{
		...
		if(MEMC_Lib_Get_JP_DTV_4K_flag())// PQC 420 mode
			rtd_setbits(0xb8099284, 0x13); //cmp_data_color(bit:0 & 4)
		else// PQC 422 mode
		...
		if(MEMC_Lib_Get_JP_DTV_4K_flag())// PQC 420 mode
		{
			WriteRegister(FRC_TOP__pqc_pqdc__MC_HF_decmp_data_format_ADDR, FRC_TOP__pqc_pqdc__MC_HF_decmp_data_format_BITSTART, FRC_TOP__pqc_pqdc__MC_HF_decmp_data_format_BITEND, 3);
