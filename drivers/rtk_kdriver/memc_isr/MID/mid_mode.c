...
{
	rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "Mid_Mode_SetMEMC_PQC = %d\r\n", (UINT32)emom);
	if(emom == RGB_IN_VIDEO_OUT || emom == YUV_IN_VIDEO_OUT)
	{
		HAL_CPR_Set444Enable(FALSE);
	}
	else if(emom == RGB_IN_PC_OUT || emom == YUV_IN_PC_OUT)
    {
...
VOID Mid_Mode_SetMEMCFrameRepeatEnable(BOOL bEnable, UBYTE *SetMCBlendMode)
{
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "[Mid_Mode_SetMEMCFrameRepeatEnable]%d, %d\n",bEnable,fPcMode);
