...
#ifdef CONFIG_RTK_KDRV_RDVB_CA
RTK_DVB_CA_FUNC rtk_rdvb_func = {NULL, NULL, NULL, NULL};
#endif
...
static int pcmcia_str_state = PCMCIA_STATUS_NORMOL;
#ifdef CONFIG_RTK_KDRV_RDVB_CA
static unsigned long resume_time;
#endif
static int last_card_status = 0;
...
static int rtk_pcmcia_get_boot_parameters( void )
{
    ...
    case PCMCIA_OFF:
        ...
        pcmcia_cfg3 = pcmcia_cfg3 &(~RTK_PCMCIA_CFG_FIFO_FUNC_PIN_SHARE_MODE) & (~RTK_PCMCIA_CFG_AUTO_PIN_SHARE_EN);
    ...
    case PCMCIA_ON:
        pcmcia_cfg |= RTK_PCMCIA_CFG_PCMCIA0_EN;
        pcmcia_cfg3 = pcmcia_cfg3 &(~RTK_PCMCIA_CFG_FIFO_FUNC_PIN_SHARE_MODE) & (~RTK_PCMCIA_CFG_AUTO_PIN_SHARE_EN);
        sprintf(pcmcia_boot_str, "pcmcia_on_without_autoPinShare");
        break;
    ...
    PCMCIA_WARNING("pcmcia_bootcode = %s update pcmcia_cfg= 0x%llX , pcmcia_cfg3= 0x%X \n", pcmcia_boot_str,pcmcia_cfg , pcmcia_cfg3);
    return PCMCIA_OK;
}
...
static irqreturn_t rtk_pcmcia_isr(int this_irq, void *dev_id)
{
    ...
    if (((event & p_this->reg_bits.CD_INSERT_INT) == p_this->reg_bits.CD_INSERT_INT)
        && (GET_PCMCIA_CTRL() | p_this->reg_bits.CD_INSERT_INT)){
        if (rtk_pcmcia_get_card_status(p_this) &RTK_PCMCIA_STS_CARD_PRESENT)
        {
#ifdef CONFIG_RTK_KDRV_RDVB_CA
            if(NULL != rtk_rdvb_func.rtk_pcmcia_change_irq)
                rtk_rdvb_func.rtk_pcmcia_change_irq(1);
#endif
            last_card_status = 1;
            PCMCIA_WARNING("PCMCIA-%d, Card Inserted\n", p_this->id);
            p_this->flags &= ~PCMCIA_CARD_RESET_COMPLETE;
        }
        SET_PCMCIA_STS(p_this->reg_bits.CD_INSERT_INT);
        p_this->event |= RTK_PCMCIA_EVENT_CARD_STATUS_CHANGE;
        wake_up(&p_this->wq);
    }

    else if(last_card_status && (event & p_this->reg_bits.CD_REMOVE_INT)
            && (GET_PCMCIA_CTRL() | p_this->reg_bits.CD_REMOVE_INT)
            && !(rtk_pcmcia_get_card_status(p_this) &RTK_PCMCIA_STS_CARD_PRESENT)){
#ifdef CONFIG_RTK_KDRV_RDVB_CA
            if(NULL != rtk_rdvb_func.rtk_pcmcia_change_irq)
                rtk_rdvb_func.rtk_pcmcia_change_irq(0);
#endif
        last_card_status = 0;
        PCMCIA_WARNING("PCMCIA-%d, Card Removed, disable card\n",
                           p_this->id);
        rtk_pcmcia_card_enable(p_this, 0);
        SET_PCMCIA_STS(p_this->reg_bits.CD_REMOVE_INT);
        p_this->event |= RTK_PCMCIA_EVENT_CARD_STATUS_CHANGE;
        wake_up(&p_this->wq);
    }
...
RTK_PCMCIA *create_rtk_pcmcia(unsigned char id)
{
    ...
    if (p_this)
    {
        ...
        case 0:
            ...
            p_this->reg_bits.CD_INT = (g_cd_seperate2pins)?(PC_CARD_DETECT1_INT | PC_CARD_DETECT2_INT):(PC_CARD_DETECT1_INT);
            p_this->reg_bits.CD_INSERT_INT = (g_cd_seperate2pins)?(PC_PCIIE1 | PC_PCIIE2):(PC_PCIIE1);
            p_this->reg_bits.CD_REMOVE_INT = (g_cd_seperate2pins)?(PC_PCRIE1 | PC_PCRIE2):(PC_PCRIE1);
        ...
        case 1:
            ...
            p_this->reg_bits.CD_INT = (g_cd_seperate2pins)?(PC_CARD_DETECT1_INT | PC_CARD_DETECT2_INT):(PC_CARD_DETECT2_INT);
            p_this->reg_bits.CD_INSERT_INT = (g_cd_seperate2pins)?(PC_PCIIE1 | PC_PCIIE2):(PC_PCIIE2);
            p_this->reg_bits.CD_REMOVE_INT = (g_cd_seperate2pins)?(PC_PCRIE1 | PC_PCRIE2):(PC_PCRIE2);
...
int rtk_pcmcia_card_enable(RTK_PCMCIA *p_this, unsigned char On)
{
    if (On)
    {
        ...
        unsigned long timeout = msecs_to_jiffies(700);
        PCMCIA_WARNING("enable card %d\n", p_this->id);
        while (1) {
            /* read card status and check the card present or not */
            int status = rtk_pcmcia_get_card_status(p_this);
            if(status & RTK_PCMCIA_STS_CARD_PRESENT){
                //PCMCIA_WARNING("[%s], get status ,elase time = %dms\n", __func__,jiffies_to_msecs(jiffies - stime));
                break;
            }
            /* check for timeout */
            if((jiffies - stime) > timeout) {
                PCMCIA_WARNING("[%s], timeout elase time = %ld\n", __func__,jiffies-stime);
                break;
            }
            /* wait for a bit */
            msleep(5);
        }
        //PCMCIA_WARNING("[%s], wait card present for %dms\n", __func__,jiffies_to_msecs(jiffies - stime));	
        if (!(rtk_pcmcia_get_card_status(p_this) & RTK_PCMCIA_STS_CARD_PRESENT))     
        {
...
static long rtk_pcmcia_ioctl(struct file *file, unsigned int cmd,unsigned long arg)                   
{
    RTK_PCMCIA *p_this = (RTK_PCMCIA *) file->private_data;
    ...
    switch (cmd) {
        ...
        case RTK_PCMCIA_IOC_READ_WRITE:	
            ...
            if (rw_cmd.flags & PCMCIA_FLAGS_RD)
            {
                if (pcmcia_fifo_mode_enable)
                    ret = rtk_pcmcia_fifo_read(p_this,rw_cmd.addr, &rw_cmd.data,1, rw_cmd.flags);                         
                else
                    ret = rtk_pcmcia_read(p_this, rw_cmd.addr,&rw_cmd.data,1, rw_cmd.flags);                   
	                                      
                if (ret == PCMCIA_OK)
                {
                    if (copy_to_user(to_user_ptr(arg), &rw_cmd,sizeof(RTK_PCMCIA_RW))){
            ...
            {
                if (pcmcia_fifo_mode_enable)
                    ret = rtk_pcmcia_fifo_write(p_this,rw_cmd.addr,&rw_cmd.data,1, rw_cmd.flags);                            
                else
                    ret = rtk_pcmcia_write(p_this, rw_cmd.addr,&rw_cmd.data,1, rw_cmd.flags);            
            }
            break;
        case RTK_PCMCIA_IOC_MULTI_READ_WRITE:	
            ...
            if (copy_from_user(&multi_rw_cmd, to_user_ptr(arg),sizeof(RTK_PCMCIA_MULTI_RW)))
            {
                PCMCIA_WARNING("read write PCMCIA failed, copy data from user space failed\n");
                ret = -EFAULT;
                break;
            ...
            {
                if (pcmcia_fifo_mode_enable)
                    ret = rtk_pcmcia_fifo_read(p_this,multi_rw_cmd.addr,p_this->buff, multi_rw_cmd.len,multi_rw_cmd.flags);                           
                else
                    ret = rtk_pcmcia_read(p_this,multi_rw_cmd.addr,p_this->buff, multi_rw_cmd.len,multi_rw_cmd.flags);
	                 
                if (ret == PCMCIA_OK)
                {
                    if (copy_to_user(to_user_ptr(multi_rw_cmd.p_data), p_this->buff,multi_rw_cmd.len))
                    {
                        PCMCIA_WARNING("read write PCMCIA failed, copy data to user space failed\n");
                        ret = -EFAULT;
                        break;
            ...
            {
                if (copy_from_user(&p_this->buff, to_user_ptr(multi_rw_cmd.p_data),multi_rw_cmd.len))
                {
                    PCMCIA_WARNING("read write PCMCIA failed, copy data from user space failed\n");
                    ret = -EFAULT;
                    break;
                ...
                if (pcmcia_fifo_mode_enable)
                    ret = rtk_pcmcia_fifo_write(p_this,multi_rw_cmd.addr,p_this->buff,multi_rw_cmd.len,multi_rw_cmd.flags);                             
                else
                    ret = rtk_pcmcia_write(p_this,multi_rw_cmd.addr,p_this->buff,multi_rw_cmd.len,multi_rw_cmd.flags);                    
            }
            break;
                                    ...
                                    unsigned long arg)
{
    ...
    return rtk_pcmcia_ioctl(file, cmd, (unsigned long)compat_arg);
}			
#endif
...
static int pcmcia_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    pcmcia_str_state = PCMCIA_STATUS_SUSPEND;

#ifdef CONFIG_RTK_KDRV_RDVB_CA
	if(NULL != rtk_rdvb_func.rtk_pcmcia_shutdown){
        PCMCIA_WARNING("[%s] suspend to shutdown slot\n", __func__);
        rtk_rdvb_func.rtk_pcmcia_shutdown();
    }
#endif
...
int rtk_pcmcia_str_state_get(void)
{
    return pcmcia_str_state;
}

void rtk_pcmcia_str_state_set(int str)
{
    pcmcia_str_state = str;
}

unsigned long rtk_pcmcia_get_resume_time(void)
{
    return resume_time;
}
...
static int pcmcia_resume(struct platform_device *dev)
{
    ...
    rtk_pcmcia_enable(rtk_pcmcia_ptr, 1);

    pcmcia_str_state = PCMCIA_STATUS_RESUME;
    resume_time = jiffies;
    return 0;
}
