...
#define CONFIG_RTK_KDRV_RDVB_CA
#ifndef UINT8
...
#define PCMCIA_STATUS_SUSPEND  0
#define PCMCIA_STATUS_NORMOL   1
#define PCMCIA_STATUS_RESUME   2
...
#define RTK_PCMCIA_STS_CARD_READY       0x0008
...
typedef struct rdvb_pcmcia_func{
    void (*rtk_pcmcia_change_irq)(int cam_type);
    void (*rtk_pcmcia_ready_irq)(void);
    void (*rtk_pcmcia_frda_irq)(void);
    void (*rtk_pcmcia_shutdown)(void);
}RTK_DVB_CA_FUNC;
#endif
...
#define RTK_PCMCIA_EVENT_IREQ_ASSERT           0x2
    ...
    struct
    {
        ...
        unsigned long CD_INSERT_INT;
        unsigned long CD_REMOVE_INT;
    } reg_bits;
...
/********************** APIs  *************************/
int rtk_pcmcia_init(void);
void rtk_pcmcia_unit(void);
int rtk_pcmcia_card_enable(RTK_PCMCIA *p_this, unsigned char On);
int rtk_pcmcia_get_card_status(RTK_PCMCIA *p_this);
int rtk_pcmcia_card_reset(RTK_PCMCIA *p_this, unsigned long CardResetTimeout);
...
#define RTK_PCMCIA_IOC_ENABLE             		_IOW(RTK_PCMCIA_IOC_MAGIC, 1, unsigned int)
#define RTK_PCMCIA_IOC_CARD_RESET         		_IOW(RTK_PCMCIA_IOC_MAGIC, 2, unsigned int)
#define RTK_PCMCIA_IOC_GET_CARD_STATUS    		_IOR(RTK_PCMCIA_IOC_MAGIC, 3, unsigned int)
#define RTK_PCMCIA_IOC_READ_WRITE         		_IOR(RTK_PCMCIA_IOC_MAGIC, 4, unsigned int)
#define RTK_PCMCIA_IOC_MULTI_READ_WRITE 		_IOR(RTK_PCMCIA_IOC_MAGIC, 5, unsigned int)
#define RTK_PCMCIA_IOC_PROBE_COMMAND			_IOR(RTK_PCMCIA_IOC_MAGIC, 6, unsigned int)
#define RTK_PCMCIA_IOC_POLL_EVENT     			_IOR(RTK_PCMCIA_IOC_MAGIC, 7, unsigned int)
#define RTK_PCMCIA_IOC_INIT_COMMAND  			_IOR(RTK_PCMCIA_IOC_MAGIC, 8, unsigned int)
#define RTK_PCMCIA_IOC_DEINIT_COMMAND   		_IOR(RTK_PCMCIA_IOC_MAGIC, 9, unsigned int)

/********************* DBG  ***************************/
/* #define PCMCIA_DBG_EN */
#ifdef PCMCIA_DBG_EN
#define PCMCIA_DBG(fmt, args...)            rtd_printk(KERN_DEBUG, "PCMCIA","[DBG] " fmt, ## args)
#else

#define PCMCIA_DBG(fmt, args...)
#endif

#define PCMCIA_INFO(fmt, args...)       rtd_printk(  KERN_INFO , "PCMCIA",  "[Info] " fmt, ## args)
#define PCMCIA_WARNING(fmt, args...)    rtd_printk( KERN_WARNING , "PCMCIA", "[Warn] " fmt, ## args)
#define PCMCIA_ERROR(fmt, args...)      rtd_printk( KERN_EMERG , "PCMCIA", "[ERROR] " fmt, ## args)
