...
static DEVICE_ATTR(printk_v4l2_ioctl, S_IWUSR|S_IWGRP|S_IRUGO, rtk_pwm_show_param, rtk_pwm_store_param);
                ...
                char *buf)
{
    ...
    if (strncmp(attr->attr.name, "run", 3) == 0) {
        ...
        ret = sprintf(buf, "%d\n", m_ioctl_disable_get());
    } else if (strncmp(attr->attr.name, "printk_v4l2_ioctl", strlen("printk_v4l2_ioctl")) == 0) {
        ret = sprintf(buf, "%d\n", m_v4l2_pwm_ioctl_printk_get());
        ...
        ret = sprintf(buf, "[%s] en=%d, freq=%d Hz, duty=%d, polarity=%d, vsync_rst=%d, (pos=%d,%08x), adap_freq=%d, adap_en=%d, mPlus=%d\n",
                pchip2->nodename,
                pchip2->rtk_enable,
                pchip2->rtk_freq,
                pchip2->rtk_duty,
                ...
                const char *buf, size_t count)
{
    ...
    if (strncmp(attr->attr.name, "disable_ioctl", strlen("disable_ioctl")) == 0) {
        ...
        if (val == 0 || val == 1)
            m_v4l2_pwm_ioctl_printk_set(val);
        else
            PWM_WARN("v4l2 PWM printk ioctrl Fail!\n");
...
void rtk_pwm_create_attr(struct device *device)
{
    ...
    device_create_file(device,&dev_attr_printk_v4l2_ioctl);
...
void rtk_pwm_remove_attr(struct device *device)
{
    ...
    device_remove_file(device, &dev_attr_printk_v4l2_ioctl);
