...
static int m_v4l2_pwm_ioctl_printk=0;
...
//======================================================================
void m_v4l2_pwm_ioctl_printk_set(int en_printk){    m_v4l2_pwm_ioctl_printk = en_printk;  }
int  m_v4l2_pwm_ioctl_printk_get(void)         {    return m_v4l2_pwm_ioctl_printk;       }
                   ...
                   int pwm_freq_min, int mode)
{
    ...
    if (pwm_freq_hz == (pwm_freq_min*100)) {
        ...
        *totalcnt = duty_max;
        ...
        for (j=10; j<4097; j++) // Start duty_max=255 to find solution
        {
            for (i=0; i<=3; i++)
            {
    ...
    if( m_ioctl_printk_get() > 0 )
        PWM_WARN("%s pwm_freq_hz = 0x%X M_best = 0x%X N_best = 0x%X totalcnt:%d duty_max:%d\n",__func__, pwm_freq_hz , *M_best,*N_best ,*totalcnt,duty_max);
...
void rtk_pwm_hw_chip_init(R_CHIP_T *pwmPin, int ricindex)
{
    ...
    pwmPin->rtk_vsync = 0;//rtk_pwm_vsync_r(pwmPin);
...
void rtk_pwm_ctrl_probe_init(R_CHIP_T *pchip2,int pwm_mode,int invert)
{
    ...
    rtk_pwm_dvs_mux_w(pchip2,PWM_DVS_MUX_DISABLE);
...
//Parameter Function
//======================================================================
void rtk_pwm_polarity_param_set(R_CHIP_T *pchip2,RTK_PWM_PARAM_EX_T* param)
{
...
void rtk_pwm_frequency_param_set(R_CHIP_T *pchip2,RTK_PWM_PARAM_EX_T* param)
{
...
void rtk_pwm_duty_param_set(R_CHIP_T *pchip2,RTK_PWM_PARAM_EX_T* param)
{
...
void rtk_pwm_totalcnt_param_set(R_CHIP_T *pchip2,RTK_PWM_PARAM_EX_T* param)
{
...
void rtk_pwm_vsync_param_set(R_CHIP_T *pchip2,RTK_PWM_PARAM_EX_T* param)
{
...
void rtk_pwm_output_param_set(R_CHIP_T *pchip2,RTK_PWM_PARAM_EX_T* param)
{
...
void rtk_pwm_vs_delay_param_set(R_CHIP_T *pchip2,RTK_PWM_PARAM_EX_T* param)
{
    pchip2->rtk_pos_start_clk = param->val;
...
int rtk_pwm_set_all_param_ex(R_CHIP_T *pchip2,RTK_PWM_INFO_EX_T* param_info)
{
    ...
    strcpy(param_all.pcbname, param_info->pcbname);
    //Set Polarity
...
int rtk_pwm_set_info(RTK_PWM_INFO_T* stpwminfo)
{
    ...
    strcpy(param.pcbname, pchip2->pcbname);
