...
void LRegress_Reset(LINEARREGRESSION_T *pLRegress)
{
	...
	memset(&pLRegress->m_dataPair, 0, sizeof(LINEARPAIR)*LR_SAMPLE_NUM);
	dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, NOCH, "LRegress_Reset !!!\n");
}
...
int LRegress_InsertDataPair(LINEARREGRESSION_T *pLRegress , LINEARPAIR pair)
{
	...
	*/
	...
	if ((pLRegress->m_pairNum > 0) && 
		(pLRegress->m_dataPair[(pLRegress->m_pairIdx-1+LR_SAMPLE_NUM)%LR_SAMPLE_NUM].y
			== pLRegress->m_dataPair[pLRegress->m_pairIdx].y)) {
		...
		dmx_mask_print(DEMUX_LR_DEBUG, NOCH,
			"pcr not change drop it: pcr:%lld, stc:%lld \n", pair.y, pair.x);
		return -1;
	}

	if (pLRegress->m_pairNum < LR_SAMPLE_NUM)
		pLRegress->m_pairNum++;

	pLRegress->m_pairIdx++;
	if (pLRegress->m_pairIdx >= LR_SAMPLE_NUM)
		pLRegress->m_pairIdx = 0;
...
{
	if (pLRegress->m_pairNum >0) {
		dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, NOCH,
			"[LRegress] LR drop data: (%lld,%lld), base(%lld, %lld)\n",
			pLRegress->m_dataPair[pLRegress->m_pairIdx].x,
			pLRegress->m_dataPair[pLRegress->m_pairIdx].y,
			pLRegress->m_BasePair.x, pLRegress->m_BasePair.y);

		pLRegress->m_pairIdx = (pLRegress->m_pairIdx+LR_SAMPLE_NUM -1)%LR_SAMPLE_NUM;
		pLRegress->m_dataPair[pLRegress->m_pairIdx].x =pLRegress->m_restoredPair.x;
		pLRegress->m_dataPair[pLRegress->m_pairIdx].y =pLRegress->m_restoredPair.y;
		if (pLRegress->m_pairNum <LR_SAMPLE_NUM ||
			(pLRegress->m_restoredPair.x==0 && pLRegress->m_restoredPair.y==0))
			/*pop at first round insert*/
			pLRegress->m_pairNum--;
	} 
}

void LRegress_CalculateAB(LINEARREGRESSION_T *pLRegress)
{
/*
least square error :
                       _          _
...
int64_t LRegress_GetPredictY(LINEARREGRESSION_T *pLRegress , int64_t x)
{
	...
	if (pLRegress->m_pairNum < 10)
		...
		if (abs64(pLRegress->m_a - MULTIPLE_PRECISION) > SLOPE_PRECISION) {
			tmp_y = 0;
			dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, NOCH, "LRegress-> abnomal ma \n");
		} else {
...
void LRegress_dumpDatabase(LINEARREGRESSION_T *pLRegress, const char * caller, const int callLine)
{
	int index = 0;
	dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, NOCH,
		"LRegress ->LRegress_dumpDatabase callerInfomation:%s %d\n", caller, callLine);
	dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, NOCH,
		"LRegress ->ma:%lld, mb:%lld, size:%lld, base:(%lld,%lld)\n",
		pLRegress->m_a, pLRegress->m_b, pLRegress->m_pairNum, 
		pLRegress->m_BasePair.x, pLRegress->m_BasePair.y);
	for (index = pLRegress->m_pairIdx - pLRegress->m_pairNum; 
		  index<pLRegress->m_pairIdx; 
		  index++) {
		int id = (index+LR_SAMPLE_NUM)%LR_SAMPLE_NUM;
		dmx_mask_print(DEMUX_LR_DEBUG|DEMUX_NOTICE_PRINT, NOCH,
			"LRegress-> (%lld, %lld)\n",
			 pLRegress->m_dataPair[id].x, pLRegress->m_dataPair[id].y)
	}
}