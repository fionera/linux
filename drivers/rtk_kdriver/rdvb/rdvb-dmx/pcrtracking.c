...
#define PCRTRACK_PREDEFINED_FULL_SPAN                  45
...
static int PCRTRACK_GetNextSpeedPredefinedArrayIdx(PCRTRK_MANAGER_T *pManager, int step)
{
	...
	else
	{
		...
		{
			//dmx_notice(pManager->tunerHandle,"[Trans mode: %d] tuningMode(%d), nextIdx(%d), currentStepIdx(%d)\n", __LINE__, pManager->tuningMode, nextIdx, pManager->currentStepIdx);
	...
	if ((CLOCK_GetPTS()-pManager->sampling.updateIntervalTick > interval) || (pManager->sampling.prevRCD != -1 && pManager->RCD == -1))
	{
		...
		if ((pManager->isAudioMaster != pManager->audioMaster) || (pManager->sampling.prevRCD != -1 && pManager->RCD == -1))
		{
			...
			else
			{
				...
				dmx_mask_print(DMX_PCRTRCK_DBG_01, pManager->tunerHandle,
					"Enter Normal mode, curIdx=%d\n", pManager->currentStepIdx);
			}
#endif
		}
					...
					if (pManager->RCD != -1)
					{
						...
						if (pcr_diff > 0 && pcr_diff1 > 0  && stc_diff > 0 && stc_diff1 > 0 && abs(rcd - rcd1) < 2*DEMUX_CLOCKS_PER_SEC && abs(rcd1 - rcd2) < 2*DEMUX_CLOCKS_PER_SEC)
						{
							...
							*/
							...
							//dmx_notice(pManager->tunerHandle,"\033[1;32m[ADJUST] (mode=%d, cur_step=%d, ti=%lld), (pcr_diff=%lld, stc_diff=%lld, preAdjust=%lld)\033[m\n", pManager->tuningMode, pManager->currentStepIdx, interval, pcr_diff, stc_diff, preAdjust);
							if(pManager->tuningMode == PCRTRK_TM_INIT)
							{
								...
								dmx_mask_print(DMX_PCRTRCK_DBG_01, pManager->tunerHandle,
									"Enter Start mode, preAdjust=%lld\n", preAdjust);
							}
							else
							{
								if(preAdjust > -2 && preAdjust < 2)
								{
									...
									{
										dmx_mask_print(DMX_PCRTRCK_DBG_01, pManager->tunerHandle,
											"BaseRCD reset: 0x%llx/0x%llx, 0x%llx\n", rcd, pManager->sampling.sampleRCD[pManager->sampling.rcdIdx-1], pManager->sampling.baseRCD);
										pManager->sampling.rcdIdx = 0;
										pManager->sampling.baseRCD = 0;
									}
								}
								...
								dmx_mask_print(DMX_PCRTRCK_DBG_01, pManager->tunerHandle,
									"Tune mode:%d, switchMode:%d, preAdjust=%lld, spd_advice:%d :rcd:0x%llx, rcd1:0x%llx, rcd2:0x%llx,basercd:%llx\n",
									pManager->tuningMode, pManager->switchStepSpan, preAdjust, pManager->sampling.speed_advice, rcd, rcd1, rcd2, pManager->sampling.baseRCD);
							}
							pManager->update = false;
							pManager->sampling.updated = true;
						}
...
bool PCRTRACK_REG_Initialize(PCRTRK_REGISTER_T *pRegister, enum clock_type clock_type)
{
...
void PCRTRACK_REG_Set_FNCODE(PCRTRK_REGISTER_T *pRegister, unsigned long n, unsigned long f)
{
	unsigned long  value  = *pRegister->sys_pll_disp_sd2;
	value &= ~(0x7ffff << 8);
	value |= (f &0x7ff)<<16;
	value |= (n&0xff) <<8;
	*pRegister->sys_pll_disp_sd2 = value;
...
void PCRTRACK_REG_ResetClock(PCRTRK_REGISTER_T *pRegister)
{
	...
	PCRTRACK_REG_Set_FNCODE(pRegister, PCRTRACK_PREDEFINED_DEFAULT_NCODE,
										PCRTRACK_PREDEFINED_DEFAULT_FCODE);
...
{
	dmx_notice(pManager->tunerHandle, "[PT] PCR Tracking Manager Initializing...\n");
	...
	sema_init(&pManager->sync_sema,1);
...
void PCRTRACK_Release(PCRTRK_MANAGER_T *pManager)
{
	...
	dmx_notice(pManager->tunerHandle,"[PT] PCR Tracking Manager UnInitialized...\n");
}
bool PCRTRACK_Start(PCRTRK_MANAGER_T *pManager, unsigned char tunerHandle)
{
	...
	if (pManager->bIsSuspending) {
		while(1){
			if (down_timeout(&pManager->sync_sema,msecs_to_jiffies(3*1000))<0){
				dmx_mask_print(DEMUX_PCRTRACK_DEBUG, pManager->tunerHandle,
					 "start pctrakcing sema timeout!!!!!\n")
			}
			else {
				break;
			}
		...
		dmx_mask_print(DEMUX_PCRTRACK_DEBUG, pManager->tunerHandle,
							"start pctrakcing....status not change\n");
		result = true;
	}
	return result;
}
...
{
	mutex_lock(&pManager->mutex);
	if (pManager->state == PCRTRK_SUSPEND &&pManager->bIsSuspending == false ) {
		dmx_mask_print(DEMUX_PCRTRACK_DEBUG, pManager->tunerHandle,
		 "stop pctrakcing....still in suspend mode, not complete start\n");
		up(&pManager->sync_sema);
	}
	pManager->bIsSuspending = true;
	wake_up(&pManager->resumeWQ);
	mutex_unlock(&pManager->mutex);
...
void PCRTRACK_SuspendStateExecute(PCRTRK_MANAGER_T *pManager)
{
	...
	wait_event_timeout(pManager->resumeWQ, (pManager->bIsSuspending == false),
								msecs_to_jiffies(THREAD_SLEEP_TIME_SUSPENDED));
	if (pManager->bIsSuspending == false) {
		mutex_lock(&pManager->mutex);
		if (pManager->bIsSuspending == false) {
			pManager->tunerHandle = pManager->nextTunerHandle;
			pManager->state = PCRTRK_READY;
			dmx_mask_print(DEMUX_PCRTRACK_DEBUG|DEMUX_NOTICE_PRINT, pManager->tunerHandle, 
				"PCRTRACKING   start RUNNING!!!!!\n");
		}
		mutex_unlock(&pManager->mutex);
	}
}
void PCRTRACK_ReadyStateExecute(PCRTRK_MANAGER_T *pManager)
{
	...
	if (pManager->bIsSuspending == true) {
		...
		PCRTRACK_REG_ResetClock(&pManager->registers);
		up(&pManager->sync_sema);
		dmx_mask_print(DEMUX_PCRTRACK_DEBUG|DEMUX_NOTICE_PRINT,
			 pManager->tunerHandle, "PCRTRACKING  be SUSPEND!!!!!.\n");
	} else {
...
void PCRTRACK_SetClockExecute(PCRTRK_MANAGER_T *pManager)
{
	if (pManager->nextStepIdx != pManager->currentStepIdx) {
		...
		PCRTRACK_REG_Set_FNCODE(&pManager->registers, ncode, fcode);
		PCRTRACK_REG_Set_PLLDDS_OC(&pManager->registers, 1);
		dmx_mask_print(DMX_PCRTRCK_DBG_01, pManager->tunerHandle,
			"step change [%d -> %d]@@@@@@@(%ld/%ld)\n",  
			pManager->currentStepIdx, pManager->nextStepIdx, ncode, fcode);
	}
...
void PCRTRACK_WaitForSampleUpdatedStateExecute(PCRTRK_MANAGER_T *pManager)
{
	...
	if (pManager->sampling.updated || pManager->bIsSuspending) {
		...
		wait_event_timeout(pManager->resumeWQ, (pManager->bIsSuspending == true), msecs_to_jiffies(THREAD_SLEEP_TIME_RUNNING));
	}
}
...
int PCRTRACK_ThreadProcEntry(void *pParam)
{
	...
	while (pManager->bIsTerminating == false) {
		...
		if (!pManager->bIsSuspending && pManager->state != PCRTRK_SUSPEND && currentPTS-lastPTS > 3*DEMUX_CLOCKS_PER_SEC) {
			dmx_mask_print(DEMUX_PCRTRACK_DEBUG, pManager->tunerHandle, "\033[1;32m[PT]CH%d: cc=%lld,cur_step=%d,sus=%d,mode=(%d, %d, %d) (adiff %lld, vdiff %lld)\033[m\n",
...
void PCRTRACK_UpdatePosition(PCRTRK_MANAGER_T *pManager, PCRSYNC_T *pPcrProcesser, AV_SYNC_MODE avSyncMode)
{
...
enum clock_type PCRTRACK_ChoosePCRClock (unsigned char tunerHandle)
{
	switch (tunerHandle) {
#ifdef CONFIG_RTK_KDRV_MULTI_TP_CLOCK
	case 0:
