...
#define COMPRESS_TPYE "tar.gz"
//#define RTDLOG_AVCPU2
#define RTDLOG_PLATDEVICE_NAME "rtd-logger"
...
bool exist_reboot_log = false;          // reboot log exist or not
...
int convert_reboot_log(char * src_data_name,char * dst_log_name);

static int wait_compress_ready(char * tar_name)
{
    int i;
    int ret = -1;
    int error;
    ...
    for(i = 0; i <= 15; i++ )
    {
        msleep(20);
        sys_sync();
        error = vfs_stat(tar_name , &stat);
        if( (error == 0) && (stat.size != 0) && (stat.size == size))
        {
            pr_info("[%s] gzip size = %lld , loop = %d\n",tar_name, stat.size, i);
            return 0;
        }
        size = stat.size;
    }
    pr_err("[%s] wait %s timeout!\n",__FUNCTION__,tar_name);

    return ret;   
}

static int call_user_do_compress(char * tar_name, char* src_file_name)
{
        int ret = 0;
        int i;
        char path[] = "/bin/tar"; 
        char *argv[] = {path, "-zcPf", tar_name, src_file_name, NULL};        
        char *envp[] = {NULL};

        ret = call_usermodehelper(path, argv, envp, UMH_NO_WAIT);
        if(ret < 0) {
                pr_err("[%s][call_usermodehelper] excute tar -zcPf  %s failed. ret = %d\n",__FUNCTION__, tar_name, ret);
                return -1;
        }

        for(i=0; i<20; i++){
                msleep(20);
                ret = sys_access((const char __user *) tar_name, 0);
                if(ret ==0){
                        ret = wait_compress_ready(tar_name);
                        break;
                }
                rtd_printk( KERN_WARNING, RTDLOG_TAG, "access %s ret=%d\n",tar_name, ret);
        }

        if(ret != 0)  {
                pr_err("[%s] excute tar -jcPf %s %s failed. ret = %d\n",__FUNCTION__, tar_name, src_file_name, ret);
        }
        pr_info("[%s] excute tar -jcPf %s %s success. ret = %d\n",__FUNCTION__, tar_name, src_file_name, ret);
        return ret;
}


static int call_user_do_remove(char* file_name)
{
        int ret = 0;
        int i = 0;
        char path[] = "/bin/rm"; 
        char *argv[] = {path, "-f", file_name, NULL};        
        char *envp[] = {NULL};

        ret = call_usermodehelper(path, argv, envp, UMH_NO_WAIT);
        if(ret < 0) {
                pr_err("[%s][call_usermodehelper] excute rm -f %s failed. ret = %d\n",__FUNCTION__, file_name, ret);
                return -1;
        }

        for(i=0; i<20; i++){
                msleep(20);
                ret = sys_access((const char __user *) file_name, 0);
        ...
        if(ret == 0) {//file exist
                pr_err("[%s] excute rm -f %s failed. ret = %d\n",__FUNCTION__, file_name, ret);
                return -1;
        }
        pr_info("[%s] excute rm -f %s success. ret = %d\n",__FUNCTION__, file_name, ret);
        return 0;
...
#define K 1024
#define M (1024 * K)
#define NORMAL_MAX_COMPRESSED_FILE_NUM 2
#define FW_MAX_COMPRESSED_FILE_NUM 3

#define LOG_FILE_MAX_SIZE (8 * M)/10 //0.8M
...
#define FW_LOG_MAX_SIZE_SMALL (3 * M)/10   //0.3M
...
static u8 g_keep_compressed_file = 0;
...
int rtd_init_logfile_path(void)
{
        ...
        for(i=0; i<MAX_LOGBUF_NUM; i++){
                ...
                if(rtd_creat_rtdfile(fw_file_path[i], &(gFileInfo.fwfile_fd[i]),MAX_LOGBUF_NUM)){
                        return -1;
                }
...
static void rtd_parse_usb_path(void)
{
        ...
        memset(logbuf_dir,0,LOGBUF_DIR_MAX_SIZE);
        while(rtk_get_usb_path(buf, sizeof(buf))){
                sleep_sec(5);
...
static void rtd_modify_logfile_maxsize(void)
{
        gFileInfo.logfile_max_size = 0x400000;
        gFileInfo.fwfile_max_size[ACPU1] = 0x100000;
        gFileInfo.fwfile_max_size[VCPU1] = 0x100000;    
}
...
static void rtd_init_filesize(void)
{
        ...
        if(g_keep_compressed_file)
        {
                rtd_modify_logfile_maxsize();
        }
...
static int rtd_get_param_logswi(logbuf_cmdline * boot_param)
{
        ...
        if(bootcode_args & (1<<ARG_VALID))
        {
                ...
                if(bootcode_args & (1<<ARG_KEEP_COMPRESSED_FILE))
                {
                        g_keep_compressed_file = 1;
                }
...
static int rtd_remove_compressed_file(EnumLogbufType type, const char* file_name, unsigned long gz_file_num)
{
    int ret = -1;
    char cmd_buf[LOGBUF_DIR_MAX_SIZE] = {0};
    int max_file_num = NORMAL_MAX_COMPRESSED_FILE_NUM;

    if((type == ACPU1_BUF) || (type == VCPU1_BUF)) 
    {
        max_file_num= FW_MAX_COMPRESSED_FILE_NUM;
    }
    if (gz_file_num < max_file_num) {
        return 0;
    }

    snprintf(cmd_buf,LOGBUF_DIR_MAX_SIZE, "%s_%ld.%s", file_name, gz_file_num - max_file_num, COMPRESS_TPYE);
    
    ret =call_user_do_remove(cmd_buf);
    if(ret) {
        rtd_printk( KERN_ERR, RTDLOG_TAG, "[%s,%d,%s] run cmd:%s fail!\n",__FILE__,__LINE__,__FUNCTION__,cmd_buf);
    }
    return ret;
}

int g_wait_ap=0;
static unsigned long g_gz_file_num = 0;
static int rtd_backup_log (loff_t size)
{
    int ret = -1;
    char file_name_buff[LOGBUF_DIR_MAX_SIZE] = {0};
    char cmd_buf[LOGBUF_DIR_MAX_SIZE] = {0};

    //close file
    filp_close(gFileInfo.logfile_fd, NULL);

    snprintf(file_name_buff, LOGBUF_DIR_MAX_SIZE, "%s%s", logbuf_dir, RTD_INTEGRATION_LOG_FILE_NAME);  

    //remove old compressed file
    if(g_keep_compressed_file != 1)
    {
        ret = rtd_remove_compressed_file(KERNEL_BUF, file_name_buff, g_gz_file_num);
        if(ret)
    	{
    	    goto end;
    	}
    ...
    snprintf(cmd_buf,LOGBUF_DIR_MAX_SIZE, "%s_%ld.%s", file_name_buff, g_gz_file_num, COMPRESS_TPYE);
 
    msleep(g_wait_ap);      //wait AP copy
    rtd_printk( KERN_INFO, RTDLOG_TAG, "gzip sleep =%dms\n",g_wait_ap);

    //compressed the rtd_logfile.log    
    mutex_lock(&rtd_logfile_lock);
    ret =call_user_do_compress(cmd_buf, file_name_buff);
    ...
    //create file
    ret = rtd_creat_rtdfile(file_name_buff, &(gFileInfo.logfile_fd),MAX_LOGBUF_NUM);
    return ret;
}

static unsigned long g_fw_gz_file_num[MAX_LOGBUF_NUM] = {0};
static int rtd_backup_fw_log (EnumLogbufType type)
{
    int ret = -1;
    char file_name_buff[LOGBUF_DIR_MAX_SIZE] = {0};
    char cmd_buf[LOGBUF_DIR_MAX_SIZE] = {0};
    ...
    if(type == ACPU1_BUF){
        snprintf(file_name_buff, LOGBUF_DIR_MAX_SIZE, "%s%s", logbuf_dir, RTD_ACPU1_FILE_NAME);
    }
    else if(type == VCPU1_BUF){
        snprintf(file_name_buff, LOGBUF_DIR_MAX_SIZE, "%s%s", logbuf_dir, RTD_VCPU1_FILE_NAME);
    }
    else{
        rtd_printk( KERN_ERR, RTDLOG_TAG, "[%s,%d,%s]  type = %d , File no need compress!\n",__FILE__,__LINE__,__FUNCTION__,type);
        return -1;
    }

    //close file
    filp_close(gFileInfo.fwfile_fd[type], NULL);

    //remove old compressed file
    if(g_keep_compressed_file != 1){
        ret = rtd_remove_compressed_file(type, file_name_buff, g_fw_gz_file_num[type]);
        if(ret)
        {
            goto end;
        }
    ...
    snprintf(cmd_buf, LOGBUF_DIR_MAX_SIZE, "%s_%ld.%s", file_name_buff, g_fw_gz_file_num[type], COMPRESS_TPYE);

    //compressed the fw logfile
    ret =call_user_do_compress(cmd_buf, file_name_buff);
    if(ret == 0)
    {
        g_fw_gz_file_num[type] ++;        
    }

end:
    //create file
    ret = rtd_creat_rtdfile(file_name_buff, &(gFileInfo.fwfile_fd[type]),MAX_LOGBUF_NUM);
    return ret;
}
...
int rtd_select_log_header (EnumLogbufType offset, rtd_msg_header * header, char *msg_tag, int pos)
{
        ...
        log_time = ns_to_timeval((s64)header->timestamp); //[CID_27999] may dereferencing null pointer when header== null;
...
void record_reboot_data(void)
{
        ...
        {
                exist_reboot_log = true;                // reboot log exist
        }
...
static int rtd_write_logfile(rtd_msg_header * header,unsigned char * buffer, int contain_avlog)
{
        ...
        ret_len = header->text_len; //[CID_27998] may dereferencing null pointer when header == null;
...
void write_msg_data (EnumLogbufType type,rtd_msg_header* header,struct kfifo * pKfifo)
{
    ...
    {
        buffer = msg_tmp_buf;
        memset(buffer,0,sizeof(msg_tmp_buf)); //[CID_17529] Dereferencing pointer buffer. not need modify,for The buffer points to an array.
    }
    else
    {
    ...
    //[CID_17529] buffer would not be null.
    if((buffer != msg_tmp_buf))//free malloced buffer
    {
        kfree(buffer);
    }
...
static int parse_rtdlog_sysnode_opera(char * buf, rtdlog_sysnode_param * param)
{
        ...
        if(strcmp(buf, "set") == 0) {
                ...
                param->opera = LOG_COMPRESSED_FILE_KEEP;
        } else {
...
/*
** keep the whole compressed logfile
** when need keep the whole compressed logfile, we would change the the max size value of logfile.
** rtd_logfile.log -> 4M ; rtd_acpu.log -> 1M ; rtd_vcpu.log -> 1M
*/
static int parse_rtdlog_keep_compressed_file(char * buf, rtdlog_sysnode_param * param)
{
        u8 tmp = 0;
        if(sscanf(buf, "%hhu", &tmp) < 1) {
                        rtd_printk( KERN_ERR, RTDLOG_TAG, "parse keep_compressed_file value fail\n");
                        return -1;
        }
        if(tmp == 1) {
            g_keep_compressed_file= 1;
            ...
            g_keep_compressed_file= 0;
            gFileInfo.logfile_max_size = LOG_FILE_MAX_SIZE;
            gFileInfo.fwfile_max_size[ACPU1] = FW_LOG_MAX_SIZE_SMALL;
            gFileInfo.fwfile_max_size[VCPU1] = FW_LOG_MAX_SIZE_SMALL;
        }
        rtd_printk( KERN_ERR, RTDLOG_TAG, "current keep_compressed_file value = %d\n",g_keep_compressed_file);
        return 0;
}
...
static int parse_rtdlog_sysnode_cmdline(const char * buf, rtdlog_sysnode_param * param)
{
        ...
        switch(param->opera) {
                ...
                case LOG_FILE_UNMASK_LEVEL:
                        ...
                        break;
                case LOG_COMPRESSED_FILE_KEEP:
                        parse_rtdlog_keep_compressed_file(buf_field, param);
                        break;
                default:
                        break;
        }
...
static void rtdlog_filter_logdbg(rtdlog_sysnode_param * param)
{
        ...
        rtd_printk( KERN_ERR, RTDLOG_TAG, "g_keep_compressed_file = %d. (%s)\n",
                g_keep_compressed_file, g_keep_compressed_file ? "enabled" : "disabled");
...
#ifndef CONFIG_ARM64
int find_hitory_log_start(EnumLogbufType type, struct kfifo * fifo, unsigned int in_reset)
{
        unsigned int find_pos = 0;
        unsigned int find_offset = 0;
        unsigned int size = fifo->size;
        unsigned char * base = (unsigned char *)fifo->buffer;
        rtd_msg_header tmp;
        rtd_msg_header * pHeader = &tmp;
        //unsigned int head_offset = 0;
        unsigned int head_pos = 0;

        unsigned int type_offset = (unsigned int)(&tmp.cpu_type) - (unsigned int)(&tmp);

        if(fifo->in < sizeof(rtd_msg_header)){
                return -1;
        }

        fifo->in = __kfifo_off(fifo,fifo->in);
        find_pos = fifo->in + 1;
        find_offset = __kfifo_off(fifo,find_pos);
        //make kfifo->in as the end offset of reboot log.
        fifo->in = __kfifo_off(fifo,in_reset);

        while(find_pos - fifo->in < size)
        {
                //.condition1 : header->type should equal type
                  //.find type offset
                if(base[find_offset] != type)
                {
                        goto next_loop;
                }
                  //.get msg header
                head_pos = find_pos - type_offset;
                fifo->out = head_pos;
                __kfifo_vmem_out_data(fifo,pHeader,sizeof(rtd_msg_header),0);

                //.condition2 : msg header verify pass
                if(check_msghead_valid(pHeader)){
                        goto next_loop;
                }else{
                        fifo->in += fifo->size;//make fifo->in larger than out about 1 more circle
                        fifo->out = head_pos;
                        //dump_ddr_range("dump found header",pHeader,sizeof(rtd_msg_header));
                        return 0;
                }
...
void dump_kfifo_status(char *msg,EnumLogbufType type,struct kfifo * fifo)
{
        rtd_printk( KERN_WARNING, RTDLOG_TAG, "%s:\n [type:%d]base:%08x size:%08x in:%08x out:%08x out_cat:%08x\n",
                        msg,type,(unsigned int)fifo->buffer,fifo->size,fifo->in,fifo->out,fifo->out_cat);
}

/*
**when you call this func,you should make sure that:
**. 0 < kfifo->in - kfifo->out < kfifo->size
**. end_pos is between kfifo->in and kfifo->out
**. return 0 if find,and update kfifo->out
*/
int find_next_log_start(EnumLogbufType type,struct kfifo * fifo,unsigned int end_pos)
{
        unsigned int find_pos = 0;
        unsigned int find_offset = 0;
        unsigned char * base = (unsigned char *)fifo->buffer;
        ...
        if((end_pos <= fifo->out)||(end_pos > fifo->in))
        {
                return -1;
        }

        find_pos = fifo->out;
        while(find_pos < end_pos)
        {
                //.condition1 : header->type should equal type
                  //.find type offset
                if(base[find_offset] != type)
...
static char * read_reboot_data(struct file * file_fd)
{
        char * buffer = NULL;
        loff_t offs = 0;

        buffer = vmalloc(rtdlog_bufsize);
        if(!buffer){
                rtd_printk( KERN_ERR, RTDLOG_TAG, "[%s]Error:malloc buffer fail!\n", __FUNCTION__);
                return NULL;
        }

        vfs_read(file_fd, buffer, rtdlog_bufsize, &offs);
        if(offs != rtdlog_bufsize)
        {
                rtd_printk( KERN_ERR, RTDLOG_TAG, "Error:read logbuf header error!\n");
                return NULL;
        }
...
static int rtd_open_file(char *file_path, struct file ** file_fd)
{

        if(!file_path){
                rtd_printk( KERN_ERR, RTDLOG_TAG, "Error:file_path is NULL!\n");
                return -1;
        }

        *file_fd = filp_open(file_path, O_RDONLY, 0644);
        if(IS_ERR(*file_fd))
        {
                rtd_printk( KERN_ERR, RTDLOG_TAG, "Error:opening file %s(%lu,0x%08lx)\n", file_path, (unsigned long)(*file_fd), (unsigned long)(*file_fd));
                *file_fd = NULL;
                return -2;
        }
        else
        {
                //rtd_printk( KERN_WARNING, RTDLOG_TAG, "opening file %s OK(%lu,0x%08l)!\n", file_path, (unsigned long)(*file_fd), (unsigned long)(*file_fd));
        }

        return 0;
}

static int rtd_create_prelogfile(char *file_path, struct file ** file_fd)
{
        if(!file_path){
                *(file_fd)=NULL;
                return -1;
...
int init_prelog_file(char * src_data_name, struct file ** src_fd, char * dst_log_name, struct file ** dst_fd)
{
        char data_file[LOGBUF_DIR_MAX_SIZE] = {0};
        char log_file[LOGBUF_DIR_MAX_SIZE] = {0};

        memset(data_file, 0, sizeof(data_file));
        memset(log_file, 0, sizeof(log_file));

        snprintf(data_file, LOGBUF_DIR_MAX_SIZE, "%s%s", logbuf_dir, src_data_name);
        snprintf(log_file, LOGBUF_DIR_MAX_SIZE, "%s%s", logbuf_dir, dst_log_name);

        /*** open bin source file ***/
        if(rtd_open_file(data_file, src_fd) < 0){
                return -1;
        }
        rtd_printk( KERN_INFO, RTDLOG_TAG, "opening file %s OK!\n", data_file);
        /*** create logfile ***/
        if(rtd_create_prelogfile(log_file, dst_fd)){
                return -2;
        }
        rtd_printk( KERN_INFO, RTDLOG_TAG, "opening file %s OK!\n", log_file);
        return 0;
...
static void save_kfifo_status(EnumLogbufType type,struct kfifo * fifo,struct file * save_file)
{
        char buf[1024];
        memset(buf,0,sizeof(buf));
        snprintf(buf,sizeof(buf),"kfifo status:\n[type:%d]base:%08x size:%08x in:%08x out:%08x out_cat:%08x\n",
                        type,(unsigned int)fifo->buffer,fifo->size,fifo->in,fifo->out,fifo->out_cat);
        buf[sizeof(buf)-1] = '\0';
        rtd_buffer_to_file(buf, strlen(buf), save_file,NULL);
}

static int output_msg_end_process(EnumLogbufType type,struct file * save_file)
{
        switch(type) {
                case KERNEL_BUF:
                        rtd_buffer_to_file("\n", 1, save_file,NULL); //add '\n' for kernel log
                        rtd_write_fw_bin(type,"\n", 1);
                        break;
...
** parse msg header to text
*/
static int output_msg_head_process(EnumLogbufType type, rtd_msg_header * header,struct file * save_file)
{
        char msg_tag[FILE_MSG_TAG_LEN] = {0};
        char cpu_type_str[MAX_LOGBUF_NUM][6] = {
                "ACPU1",
                "ACPU2",
                "VCPU1",
                "VCPU2",
        ...
        if(header->pid != 0xffffffff) {
                len = snprintf(&msg_tag[pos], FILE_MSG_TAG_LEN - pos, "P%04d/",
                               header->pid);
                pos += len;
        } else {
                len = snprintf(&msg_tag[pos], FILE_MSG_TAG_LEN - pos, "P????/");
                pos += len;
        }

        if(header->tid != 0xffffffff) {
                len = snprintf(&msg_tag[pos], FILE_MSG_TAG_LEN - pos, "T%04d)#%-15s#",
                               header->tid,header->name);
                pos += len;
        } else {
                len = snprintf(&msg_tag[pos], FILE_MSG_TAG_LEN - pos, "T????)#%-15s#",
                               header->name);
                pos += len;
        }
...
static void save_kfifo_log(EnumLogbufType type,struct kfifo * fifo,struct file * save_file)
{
        rtd_msg_header tmp;
        rtd_msg_header * pHeader = &tmp;
        unsigned int cnt = 0;
        ...
        char * end_tip = "\n\n\n---------------kfifo log end---------------\n\n\n";
        char * drop_tip = "\n****************kfifo log droped****************\n";
        unsigned int old_out = 0;

        rtd_buffer_to_file(start_tip, strlen(start_tip), save_file,NULL);
        while((fifo->out != fifo->in)&&(cnt <= fifo->size))
        {
                //pop 1 header
                __kfifo_vmem_out_data(fifo,pHeader,sizeof(rtd_msg_header),0);
                //check header
                if(check_msghead_valid(pHeader))
                {
                        rtd_printk( KERN_ERR, RTDLOG_TAG, "Error while save kfifo log,type[%d]\n",type);
                        dump_kfifo_status("drop some logs and dump kfifo status",type,fifo);
                        save_kfifo_status(type,fifo,save_file);
                        rtd_buffer_to_file(drop_tip, strlen(drop_tip), save_file,NULL);
                        old_out = fifo->out;
                        if(find_next_log_start(type,fifo,fifo->in)){
                                goto fail_ret;
                        }else{
                                cnt += fifo->out - old_out;
                                dump_kfifo_status("find next log and dump kfifo status",type,fifo);
                                save_kfifo_status(type,fifo,save_file);
                                continue;
                        }
                }
                //process log header
                output_msg_head_process(type,pHeader,save_file);
                //save msg
                buffer = kmalloc(pHeader->text_len,GFP_KERNEL);
                if(buffer == NULL)
                {
                        goto fail_ret;
                }
                __kfifo_vmem_out_data(fifo,buffer,pHeader->text_len,sizeof(rtd_msg_header));
                rtd_buffer_to_file(buffer, pHeader->text_len, save_file,NULL);
                kfree(buffer);
                fifo->out += pHeader->len;
                cnt += pHeader->len;
                //process log rear
                output_msg_end_process(type,save_file);
        }
        rtd_buffer_to_file(end_tip, strlen(end_tip), save_file,NULL);
        return;
...
/*
function: convert rtd_prelogbuf.bin to rtd_prelogbuf.log

*/
int convert_reboot_log(char * src_data_name,char * dst_log_name)
{
        struct file * reboot_data_file = NULL;
        struct file * reboot_log_file = NULL;
        logbuf_global_status * status = NULL;
        int i, ret=0;
        logbuf_rbcb * pRbcb[MAX_LOGBUF_NUM] = {0};
        struct kfifo * pKfifo[MAX_LOGBUF_NUM] = {NULL};
        char * pLogbuf = NULL;
        mm_segment_t old_fs;

        if(((bootcode_args & ((unsigned long)1<<ARG_VALID)) == 0)||((bootcode_args & ((unsigned long)1<<ARG_REBOOT_LOG_EN)) == 0)){
                rtd_printk( KERN_ERR, RTDLOG_TAG, "[Warning] Reboot switch is off!\n");
                return ret;
        }

        if(!exist_reboot_log){
                rtd_printk( KERN_ERR, RTDLOG_TAG, "[Warning] No reboot log!\n");
                return ret;
        }
        ...
        /*** open/create file ***/
        if(init_prelog_file(src_data_name, &reboot_data_file, dst_log_name, &reboot_log_file) < 0){
                ret = -1;
                goto end;
        }

        /*** read reboot data to pLogbuf ***/
        if((pLogbuf=read_reboot_data(reboot_data_file)) == NULL){
                ret = -2;
                goto end;
        }

        /*** get kfifo info ***/
        status = (logbuf_global_status *)pLogbuf;
        if(check_version_and_magic(status)){
                ret = -3;
                goto end;
        }
        for(i = 0; i < MAX_LOGBUF_NUM; ++i){
                status->rbcb_addr[i] = status->rbcb_addr[i] - RTD_LOGBUF_PHY_START_ADDR + (u32)status;
                pRbcb[i] = (logbuf_rbcb *)status->rbcb_addr[i];
                pKfifo[i] = &(pRbcb[i]->fifo);
                pKfifo[i]->buffer = pKfifo[i]->buffer - RTD_LOGBUF_PHY_START_ADDR + (u32)status;
                //dump kfifo
                rtd_printk(KERN_INFO,RTDLOG_TAG,"HISTORY KFIFO[%d]:buf(%08lx),size(%08x),in(%08x),out(%08x)\n",
                        i,(unsigned long)(pKfifo[i]->buffer),pKfifo[i]->size,pKfifo[i]->in,pKfifo[i]->out);
        }

        /*** step loop buffer ***/
        for(i = 0; i < MAX_LOGBUF_NUM; ++i){
                /** find oldest log **/
                if(find_hitory_log_start(i,pKfifo[i],pRbcb[i]->in_reset) < 0){
                        rtd_printk(KERN_INFO,RTDLOG_TAG,"Found KFIFO[%d] oldest log fail!\n",i);
                        continue;
                }
                /** show kfifo info **/
                rtd_printk(KERN_INFO,RTDLOG_TAG,"Found oldest KFIFO[%d]:buf(%08x),size(%08x),in(%08x),out(%08x)\n",
                        i,(u32)pKfifo[i]->buffer,pKfifo[i]->size,pKfifo[i]->in,pKfifo[i]->out);
                /** step read log line by line **/
                save_kfifo_log(i,pKfifo[i],reboot_log_file);
        }
        ...
        if((reboot_data_file != NULL)&&(!IS_ERR(reboot_data_file))){
                filp_close(reboot_data_file, NULL);
        }
        if((reboot_log_file !=  NULL)&&(!IS_ERR(reboot_log_file))){
                filp_close(reboot_log_file, NULL);
        }
        set_fs(old_fs);

        if(0 == ret){   // convert ok
                rtd_printk(KERN_ERR, RTDLOG_TAG, "%s Finished\n", __FUNCTION__);
        }else{          // convert fail
                rtd_printk(KERN_ERR, RTDLOG_TAG, "%s Failed!, ret=%d.\n", __FUNCTION__, ret);
        }
...
#else
                        convert_reboot_log(RTD_REBOOT_DATA_FILE_NAME, RTD_REBOOT_LOG_FILE_NAME);
#endif
                        break;
                case LOG_DUMP:
                        dump_log_data();
...
int get_msg_header_avk_epc (EnumLogbufType type,rtd_msg_header* header,struct kfifo * pKfifo)
{
    ...
    if (kfifo_len(fifo)> sizeof(rtd_msg_header))
    {
        ...
        if(check_msghead_valid(header)) // if fail
        {
                printk("\n fifo out=0x%x, fifo in=0x%x\n",(unsigned int)fifo->out,  (unsigned int)fifo->in); //[CID_27808] may dereferencing null pointer pParam when pKfifo == null;
                kfifo_drop(fifo);
                gDbgInfo.log_reset_cnt[type]++;
                rtd_printk( KERN_ERR, RTDLOG_TAG, "type[%d] check msg header fail!\n",type);
                return -1;
...
void rtdlog_crash_dump(EnumLogbufType type)
{
        ...
        printk("before decrease Kfifo out=0x%08x, Kfifo in=0x%08x\n",(unsigned int)(pKfifo->out), (unsigned int)(pKfifo->in));
        if(rtdlog_find_header(pKfifo, type)<0){
                if(pKfifo->in == 0){
                        goto RET;
                }
                printk("find header fail\n");
                printk("pKfifo->out=0x%08x, pKfifo->in=0x%08x\n",(unsigned int)(pKfifo->out), (unsigned int)(pKfifo->in));
                goto RET;
        }
        out_tmp_pos = pKfifo->out;
        printk(" after decrease Kfifo out=0x%08x, Kfifo in=0x%08x\n",(unsigned int)(pKfifo->out), (unsigned int)(pKfifo->in));
        ...
        while((pKfifo->out) != (pKfifo->in)){
                ...
                if (header_check == 1){
                        ...
                        printk("Kfifo out=0x%x, Kfifo in=0x%x\n",(unsigned int)(pKfifo->out), (unsigned int)(pKfifo->in));
                        break;
                }
        }
...
#ifdef CONFIG_PM
static int rtdlog_restore(struct device *dev)
{
#if defined(CONFIG_OPTEE)
    struct optee_rpc_param rtc_set_info;
...
static const struct dev_pm_ops rtdlog_pm_ops = {
    .restore  = rtdlog_restore
};
...
#ifdef CONFIG_PM
        rtdlog_platform_devs = platform_device_register_simple(RTDLOG_PLATDEVICE_NAME, -1, NULL, 0);
