...
int get_pcb_names(char *tmp, unsigned int response_buff_size)
{
	...
	snprintf(pattern, sizeof(pattern), "pcb_enum=");
		...
		if(pch) {
			...
			if ((token!=NULL) && (pattern!=NULL)) {
				if (strlen(names)){
					...
					snprintf(names, PCB_NAME_BUFFER_SIZE, "%s", token + strlen(pattern));
				}
			}
		}
	}
	ret = snprintf(tmp, response_buff_size, "%s", names);
	RTICE_DEBUG("[%s] -------ret = %d \n\r", __func__, ret);
	kfree(names);
	return ret;
}
				...
				unsigned int response_buff_size )
{
	...
	case RTICE_SYS_CMD_READ_SYSTEM_INFO:
		...
		case RTICE_SYSTEM_INFO_ID_CPU_ID:
			ret = snprintf(response_data, response_buff_size, "%x",platform_info.cpu_id);
		break;
		case RTICE_SYSTEM_INFO_ID_BOARD_ID:
			ret = snprintf(response_data, response_buff_size, "%x",\
						platform_info.board_id);
		break;
		case RTICE_SYSTEM_INFO_ID_BOOTLOADER_VERSION:
			ret = snprintf(response_data, response_buff_size, "%s",\
					platform_info.bootloader_version);
		break;
		case RTICE_SYSTEM_INFO_ID_AUDIO_FW_VERSION:
			ret = snprintf(response_data, response_buff_size, "Unknown");
		break;
	...
	case RTICE_SYS_CMD_READ_PCB_PARAMETER:
		...
		case RTICE_PCB_INFO_GET_ALL_NAME:
			ret = get_pcb_names(response_data,response_buff_size);
		break;
		default:
