...
#define RTL_SUPPORT_STR_LINK_MODE       1
#define REG_STR_LINK_MODE               0xb8060150
#define STR_LINK_MODE_OFFSET            24
#define REG_WOL_MODE_FLAG               REG_STR_LINK_MODE
#define WOL_MODE_FLAG_OFFSET            27

#if RTL_SUPPORT_STR_LINK_MODE
static int __rtl_support_str_link_mode  = 1;
#else
static int __rtl_support_str_link_mode  = 0;
#endif
#define REG_SHARE_WOL_CAP       0xb8060500
#define REG_SHARE_CLOCK         0xb8060034
#define SHARE_REG_CLOCK_BIT     10
#define WOL_CAP_BIT             24
...
#define RTL8168_RECOVERY_TIMEOUT	(HZ/100)
#define RTL8168_LINK_QUICK_CHECK_TIMEOUT    (HZ/100)
#define RTL8168_LINK_QUICK_CHECK_TIMES   500 
...
// 2bit for present str_link_mode
enum rtl_str_link_mode{
    rtl_str_interrupt_power_mode = 0,
    rtl_str_link_mode = 1,
};
static unsigned int g_in_driver_str = 1;
...
struct rtl8168_private {
	...
	struct timer_list link_recovery_timer;
	struct timer_list link_quick_check_timer;
	...
	u32 wol_mode_of_str;
	u32 link_quick_check_cnt;
	spinlock_t link_check_lock;
};
...
#ifdef ENABLE_WOL
void rtl_speed_down(struct rtl8168_private *tp);
#endif
#ifdef ENABLE_EEEP
...
static void rtl8168_rbus_timeout_debug(void)
{
    ...
    eth_err("%s\n", __func__);
    eth_err("Clock: %08X, reset: %08X\n", rtd_inl(0xb8060040), rtd_inl(0xb8060030));
    eth_err("PLLETN CLK: %08X, RST: %08X, INFO: %08X\n", rtd_inl(0xb8016c04), rtd_inl(0xb8016c00), rtd_inl(0xb8016c3c));

    return;
}
#endif

static void rtl8168_set_str_link_mode(int mode)
{
    unsigned int val = 0;
    if(__rtl_support_str_link_mode)
    {
        val = rtd_inl(REG_STR_LINK_MODE);
        val = (val & (~((0x3 << STR_LINK_MODE_OFFSET))))
                | (0x1 << STR_LINK_MODE_OFFSET);
        rtd_outl(REG_STR_LINK_MODE,val);
    }
}

static int rtl8168_get_str_link_mode(void)
{
    unsigned int mode = 0;
    if(__rtl_support_str_link_mode)
    {
        mode = (rtd_inl(REG_STR_LINK_MODE) & (0x3 << STR_LINK_MODE_OFFSET))
                        >> STR_LINK_MODE_OFFSET;
        if(rtl_str_link_mode == mode)
            return rtl_str_link_mode;
    }
    return rtl_str_interrupt_power_mode;
}

static void rtl8168_set_wol_flag_for_str(int enable)
{
    unsigned int val = 0;
    if(__rtl_support_str_link_mode)
    {
        val = rtd_inl(REG_WOL_MODE_FLAG);
        if(enable)
            val = val | (0x1 << WOL_MODE_FLAG_OFFSET);
        else
            val = val & ~(0x1 << WOL_MODE_FLAG_OFFSET);
        rtd_outl(REG_WOL_MODE_FLAG,val);
    }
}

static int rtl8168_get_wol_flag_for_str(void)
{
    unsigned int mode = 0;
    if(__rtl_support_str_link_mode)
    {
        mode = (rtd_inl(REG_WOL_MODE_FLAG) & (0x1 << WOL_MODE_FLAG_OFFSET))
                            >> WOL_MODE_FLAG_OFFSET;
    }
    return mode;
}

void __dump_phy(char* tag,unsigned int mode)
{
    eth_err("%s mode=0x%x\n",__func__,mode);
    //1 dump afe
    if(mode & 0x1)
    {
        ...
        unsigned int end    = 23;
        unsigned int temp   = 0;
        unsigned int page   = 0xbd0;
        // dump page db0
        rtd_outl(0xb8016060,0x841f0bd0);    // change Page to bd0
        mdelay(100);

        start   = 16;
        end     = 23;
        page    = 0xbd0;
        for(start=16;start<=end;start++)
        {
            temp = 0x04000000+start*0x10000;
            rtd_outl(0xb8016060,temp);
            mdelay(10);
            reg=rtd_inl(0xb8016060);
            mdelay(10);
            eth_err("%s page[%x]reg[0x%x]=0x%08x\n"
                    ,tag
                    ,page
                    ,start
                    ,reg);
        ...
        // dump page bd1
        rtd_outl(0xb8016060,0x841f0bd1);    // change Page to bd1
        mdelay(100);
    ...
    if(mode & 0x10)
    {
        ...
        // dump page c40
        rtd_outl(0xb8016060,0x841f0c40);    // change Page to c40
        mdelay(100);
...
//
        // dump page c41
        rtd_outl(0xb8016060,0x841f0c41);    // change Page to c41
        mdelay(100);
        ...
        // dump page c42
        rtd_outl(0xb8016060,0x841f0c42);    // change Page to c42
        mdelay(100);
        ...
        // dump page c43
        rtd_outl(0xb8016060,0x841f0c43);    // change Page to c43
        mdelay(100);
        ...
        // dump page c44
        rtd_outl(0xb8016060,0x841f0c44);    // change Page to c44
        mdelay(100);
        ...
        // dump page c45
        rtd_outl(0xb8016060,0x841f0c45);    // change Page to c45
        mdelay(100);
        ...
        // dump page c60
        rtd_outl(0xb8016060,0x841f0c60);    // change Page to c60
        mdelay(100);
        ...
        // dump page c61
        rtd_outl(0xb8016060,0x841f0c61);    // change Page to c61
        mdelay(100);
    ...
    if(mode & 0x100)
    {
        ...
        unsigned int end    = 23;
        unsigned int temp   = 0;
        unsigned int page   = 0xd00;

        // dump page d00
        rtd_outl(0xb8016060,0x841f0d00);    // change Page to d00
        mdelay(100);
        end     = 23;
        page    = 0xd00;
        for(start=16;start<=end;start++)
        ...
        // dump page d01
        rtd_outl(0xb8016060,0x841f0d01);    // change Page to d01
        mdelay(100);
        end     = 23;
        page    = 0xd01;
        for(start=16;start<=end;start++)
        ...
        // dump page d04
        rtd_outl(0xb8016060,0x841f0d04);    // change Page to d04
        mdelay(100);
        end     = 20;
        page    = 0xd04;
        for(start=16;start<=end;start++)
        ...
        // dump page d06
        rtd_outl(0xb8016060,0x841f0d06);    // change Page to d06
        mdelay(100);
        end     = 18;
        page    = 0xd06;
        for(start=16;start<=end;start++)
        ...
        // dump page d20
        rtd_outl(0xb8016060,0x841f0d20);    // change Page to d20
        mdelay(100);
        end     = 23;
        page    = 0xd20;
        for(start=16;start<=end;start++)
        ...
        // dump page d21
        rtd_outl(0xb8016060,0x841f0d21);    // change Page to d21
        mdelay(100);
        end     = 22;
        page    = 0xd21;
        for(start=16;start<=end;start++)
    ...
    if(mode & 0x1000)
    {
        ...
        unsigned int page   = 0xd00;
        // dump page bc0
        rtd_outl(0xb8016060,0x841f0bc0);    // change Page to bc0
        mdelay(100);
        end     = 23;
        page    = 0xbc0;
        for(start=16;start<=end;start++)
        ...
        // dump page bc1
        rtd_outl(0xb8016060,0x841f0bc1);    // change Page to bc1
        mdelay(100);
        end     = 23;
        page    = 0xbc1;
        for(start=16;start<=end;start++)
        ...
        // dump page bcd
        rtd_outl(0xb8016060,0x841f0bcd);    // change Page to bcd
        mdelay(100);
        end     = 19;
        page    = 0xbcd;
        for(start=19;start<=end;start++)
        {
            temp = 0x04000000+start*0x10000;
            rtd_outl(0xb8016060,temp);
            mdelay(10);
        ...
        // dump page be0
        rtd_outl(0xb8016060,0x841f0be0);    // change Page to be0
        mdelay(100);
        end     = 23;
        page    = 0xbe0;
        for(start=16;start<=end;start++)
        ...
        // dump page be1
        rtd_outl(0xb8016060,0x841f0be1);    // change Page to be1
        mdelay(100);
        end     = 18;
        page    = 0xbe1;
        for(start=16;start<=end;start++)
    ...
    if(mode & 0x10000)
    {
        ...
        unsigned int page   = 0xd00;
        // dump page a80
        rtd_outl(0xb8016060,0x841f0a80);    // change Page to a80
        mdelay(100);
        end     = 23;
        page    = 0xa80;
        for(start=16;start<=end;start++)
        ...
        // dump page a81
        rtd_outl(0xb8016060,0x841f0a81);    // change Page to a81
        mdelay(100);
        end     = 23;
        page    = 0xa81;
        for(start=16;start<=end;start++)
        ...
        // dump page a82
        rtd_outl(0xb8016060,0x841f0a82);    // change Page to a82
        mdelay(100);
        end     = 23;
        page    = 0xa82;
        for(start=16;start<=end;start++)
        ...
        // dump page a83
        rtd_outl(0xb8016060,0x841f0a83);    // change Page to a83
        mdelay(100);
        end     = 23;
        page    = 0xa83;
        for(start=16;start<=end;start++)
...
//
        // dump page a84
        rtd_outl(0xb8016060,0x841f0a84);    // change Page to a84
        mdelay(100);
        end     = 22;
        page    = 0xa84;
        for(start=16;start<=end;start++)
...
//
        // dump page aa0
        rtd_outl(0xb8016060,0x841f0aa0);    // change Page to aa0
        mdelay(100);
        end     = 23;
        page    = 0xaa0;
        for(start=16;start<=end;start++)
        ...
        // dump page aa1
        rtd_outl(0xb8016060,0x841f0aa1);    // change Page to aa1
        mdelay(100);
        end     = 23;
        page    = 0xaa1;
        for(start=16;start<=end;start++)
        ...
        // dump page aa2
        rtd_outl(0xb8016060,0x841f0aa2);    // change Page to aa2
        mdelay(100);
        end     = 23;
        page    = 0xaa2;
        for(start=16;start<=end;start++)
        ...
        // dump page aa3
        rtd_outl(0xb8016060,0x841f0aa3);    // change Page to aa3
        mdelay(100);
        end     = 23;
        page    = 0xaa3;
        for(start=16;start<=end;start++)
        ...
        // dump page aa4
        rtd_outl(0xb8016060,0x841f0aa4);    // change Page to aa4
        mdelay(100);
        end     = 22;
        page    = 0xaa4;
        for(start=16;start<=end;start++)
    ...
    if(mode & 0x100000)
    {
        ...
        unsigned int page   = 0xd00;
        // dump page c80
        rtd_outl(0xb8016060,0x841f0c80);    // change Page to c80
        mdelay(100);
        end     = 23;
        page    = 0xc80;
        for(start=16;start<=end;start++)
        ...
        // dump page c81
        rtd_outl(0xb8016060,0x841f0c81);    // change Page to c81
        mdelay(100);
        end     = 23;
        page    = 0xc81;
        for(start=16;start<=end;start++)
        ...
        // dump page c82
        rtd_outl(0xb8016060,0x841f0c82);    // change Page to c82
        mdelay(100);
        end     = 16;
        page    = 0xc82;
        for(start=16;start<=end;start++)
...
//
        // dump page ca0
        rtd_outl(0xb8016060,0x841f0ca0);    // change Page to ca0
        mdelay(100);
        end     = 18;
        page    = 0xca0;
        for(start=16;start<=end;start++)
...
static void rtl_recovery_phy_status(struct rtl8168_private *tp)
{
    rtl_phy_status *phyStatus = &tp->phy_status;
//	u32 ioaddr = tp->base_addr;
    rtl_get_phy_status(tp);
    ...
    //0xDE40  bit2:link  bit3:full bit4:100M
    if( phyStatus->link_status & BIT(2)) {
        if( phyStatus->link_speed & BIT(4))
        { //100
            if (phyStatus->link_speed & BIT(3)) //full
                r8168_mac_ocp_write (tp , 0xDE40 , 0x001c);  //100 full
            else // half
                r8168_mac_ocp_write (tp , 0xDE40 , 0x0014);  //100 half
        }
        else
        {
            if (phyStatus->link_speed & BIT(3)) //full
                r8168_mac_ocp_write (tp , 0xDE40 , 0x000c);  //10 full
            else // half
                r8168_mac_ocp_write (tp , 0xDE40 , 0x0004);  //10 half
        }
  }
}
static void __rtl8168_check_do_link_change(struct net_device *dev,
        struct rtl8168_private *tp,
        u32 ioaddr, bool pm, u32 current_link_status)
{
    u32 phy_read_data = 0;
    
    spin_lock(&tp->link_check_lock);
    if (current_link_status == tp->link_status) {
         spin_unlock(&tp->link_check_lock);
        return;
    }
...
#endif
       __rtl8168_check_do_link_change(dev, tp, ioaddr, pm, current_link_status);
}
...
static int rtl8168_set_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	...
	del_timer_sync(&tp->link_recovery_timer);
	del_timer_sync(&tp->link_quick_check_timer);
...
static void rtl8168_recovery_timer(unsigned long __opaque)
{
	struct net_device *dev = (struct net_device *)__opaque;
	struct rtl8168_private *tp = netdev_priv(dev);
	...
	rtl8168_phy_int_disable(tp);	
	rtl_recovery_phy_status(tp);
	rtl8168_phy_int_enable(tp);
	
	mod_timer(&tp->link_recovery_timer, jiffies + RTL8168_RECOVERY_TIMEOUT);
	
}
static int rtl8168_net_and_mac_reset(struct rtl8168_private *tp, unsigned int check_link_status);
static void rtl8168_link_quick_check_timer(unsigned long __opaque)
{
    struct net_device *dev = (struct net_device *)__opaque;
    struct rtl8168_private *tp = netdev_priv(dev);
    u32 current_link_status = RTL8168_LINK_UNKNOWN;
    unsigned int ioaddr = tp->base_addr;
    bool pm = false;
    
    tp->link_quick_check_cnt++;
    if(tp->link_quick_check_cnt == 1)
        rtl8168_net_and_mac_reset(tp, 0);
    ...
    if(tp->link_quick_check_cnt < RTL8168_LINK_QUICK_CHECK_TIMES)
            mod_timer(&tp->link_quick_check_timer, jiffies + RTL8168_LINK_QUICK_CHECK_TIMEOUT);
}
...
void rtl_wol_set(struct rtl8168_private *tp, u32 enable)
{
	u32 ioaddr = tp->base_addr;
	//u32 wol_mode = 1;
...
static void r8168_pll_power_down(struct rtl8168_private *tp)
{
    ...
    eth_info("%s mac link=%u\n",__func__,tp->link_ok(tp->base_addr));
    ...
    else if ((tp->features & RTL_FEATURE_WOL) && tp->link_ok(tp->base_addr)
            //|| ((rtd_inl(REG_SHARE_WOL_CAP) & BIT(WOL_CAP_BIT)) != 0)
            )
	{
		eth_info("%s, Reading WOW/WOL flag from 0x%08x = 0x%08x\n", __func__,REG_SHARE_WOL_CAP, rtd_inl(0xb8060500));
#ifdef ENABLE_WOL
		tp->wol_mode_of_str = 1;
		rtl8168_set_wol_flag_for_str(1);
	...
	{
#ifdef ENABLE_WOL
	    tp->wol_mode_of_str = 0;
	    rtl8168_set_wol_flag_for_str(0);
#endif
	    r8168_phy_power_down(tp);
	}
}
...
void rtl_pll_power_up(struct rtl8168_private *tp)
{
		...
		char *buf)
{
	...
	u32 wol_status = rtd_inl(0xb8060450);
        ...
        return sprintf(buf, "wol_enable = %d, wol_interrupt = 0x%08x	\
				\n%s\n", tp->ap_wol_enable, wol_status, wol_status & BIT(9)? "Receive magic packet!!":"Doesn't receive magic packet!!");
}
...
#define _eBIT(reg,pos)       ((reg&(1<<pos))>>pos)

void __dump_speed(char* tag)
{
    unsigned int reg = 0;
    // Chang page to 0x0
    rtd_outl(0xb8016060,0x801F0a40);
    mdelay(10);
    //read force 100M
    rtd_outl(0xb8016060,0x00000000);
    mdelay(10);
    reg = rtd_inl(0xb8016060);
    eth_debug("%s page0a40 reg0x0=0x%08x speed=0x%08x\n",tag, reg, reg);
}
...
static void rtl8168_net_suspend(struct net_device *dev)
{
	...
	if (!netif_running(dev)|| !test_bit(RTL_FLAG_TASK_ENABLED, tp->wk.flags))
	{
        ...
        // check wol enable/disable
        // read WOW/WOL flag in the share memory
        if ((tp->features & RTL_FEATURE_WOL) && tp->link_ok(ioaddr))
        {
            eth_err("%s std flow set wol flag\n",__func__);
            tp->wol_mode_of_str = 1;
            rtl8168_set_wol_flag_for_str(1);
        }
        else
        {
            eth_err("%s std flow unset wol flag\n",__func__);
            tp->wol_mode_of_str = 0;
            rtl8168_set_wol_flag_for_str(0);
        }
#endif
...
int rtl8168_suspend(struct platform_device *dev, pm_message_t state) // for WOL of the warm standby flow (suspend/resume)
{
	...
	// ac on for std
	rtl8168_set_str_link_mode(rtl_str_link_mode);
    ...
    __dump_speed("suspend end ");
//        __dump_phy("roger dump phy ",0x111000);
//        __dump_phy("roger dump phy ",0x111);
    eth_info(" %s rtl8168_get_str_link_mode=%d rtl8168_get_wol_flag_for_str=%d, g_in_driver_str == %d\n"
            ,__func__
            ,rtl8168_get_str_link_mode()
            ,rtl8168_get_wol_flag_for_str(), g_in_driver_str);
	return 0;
}
...
void __dump_phy_link(char* tag)
{
    unsigned int reg = 0;
    rtd_outl(0xb8016060,0x801f0a43);
    rtd_outl(0xb8016060,0x041A0000);// polling phy_link = 1
    msleep(10);

    reg = rtd_inl(0xb8016060);
    eth_err("%s PHY1 reg0x1A=%08x phy_link=%u\n",tag, reg, _eBIT(reg,2));
}

static int rtl8168_init_phy_for_keep_link(struct rtl8168_private *tp)
{
    u32 ioaddr = tp->base_addr;
    eth_debug("%s\n",__func__);
    RTL_W8(MCU, RTL_R8(MCU) & ~NOW_IS_OOB);
...
void _rtl8168_dump_link(char* tag,struct rtl8168_private *tp)
{
    unsigned int reg    = 0;
    u32 ioaddr          = tp->base_addr;

    rtd_outl(0xb8016060,0x801f0a43);
    rtd_outl(0xb8016060,0x001A0000);// polling phy_link = 1
    msleep(10);

    reg = rtd_inl(0xb8016060);
    eth_debug("%s page0a43 reg0x1A=%08x phy_link=%u\n",tag, reg, _eBIT(reg,2));
    while(0==_eBIT(reg,2))
    {
        rtd_outl(0xb8016060,0x801f0a43);
        rtd_outl(0xb8016060,0x001A0000);// polling phy_link = 1
    ...
    eth_debug("%s page0a43 reg0x1A=%08x phy_link=%u\n",tag, reg, _eBIT(reg,2));
    reg = RTL_R8(PHYstatus);
    eth_debug("%s PHYstatus=0x%x link_sts=%d\n",tag,reg,_eBIT(reg,1));
}
...
int rtl8168_resume(struct platform_device *dev) // for WOL of the warm standby flow (suspend/resume)
{
	...
	struct rtl8168_private *tp = netdev_priv(ndev);
	int ret;
	eth_info("%s wol_mode_of_str=%u rtl8168_get_str_link_mode=%d rtl8168_get_wol_flag_for_str=%d, g_in_driver_str == %d\n"
	        ,__func__
	        ,tp->wol_mode_of_str
	        ,rtl8168_get_str_link_mode()
	        ,rtl8168_get_wol_flag_for_str(), g_in_driver_str);
    ...
    if(g_in_driver_str)
        rtl8168_init_phy_for_keep_link(tp);
    else
        rtl8168_init_phy(ndev, tp);

    rtl8168_phy_int_enable(tp);

    if (g_in_driver_str && rtl8168_get_wol_flag_for_str() && rtl8168_get_str_link_mode())
        mod_timer(&tp->link_recovery_timer, jiffies + RTL8168_RECOVERY_TIMEOUT);
...
//	_rtl8168_dump_link("resume end ",tp);
//    __dump_speed("resume end ");
    if(g_in_driver_str) {
        if(rtl8168_get_wol_flag_for_str() && rtl8168_get_str_link_mode())
        {
            rtl8168_net_and_mac_reset(tp, 1);
        }else {
            tp->link_quick_check_cnt = 0;
            mod_timer(&tp->link_quick_check_timer, jiffies + RTL8168_LINK_QUICK_CHECK_TIMEOUT);
        }
    }
	return 0;
...
static int rtl_init_one(void)
{
	...
	init_timer(&tp->link_recovery_timer);
	tp->link_recovery_timer.data = (unsigned long) ndev;
	tp->link_recovery_timer.function = rtl8168_recovery_timer;

	init_timer(&tp->link_quick_check_timer);
	tp->link_quick_check_timer.data = (unsigned long) ndev;
	tp->link_quick_check_timer.function = rtl8168_link_quick_check_timer;
	spin_lock_init(&tp->link_check_lock);
...
#endif
	rtl8168_set_str_link_mode(rtl_str_link_mode);
    eth_info("[eth] %s rtl8168_set_str_link_mode=%d\n"
            ,__func__
            ,rtl8168_get_str_link_mode());
out:
	return rc;
