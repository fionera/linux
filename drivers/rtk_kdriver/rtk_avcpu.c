...
long avcpu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case AVCPU_IOCTRESET:
		...
		if (!ret) {
			...
			if(stravnoreload == 0) {
                                char *ptr[3] = {rs.audio_image, rs.video_image_1, rs.video_image_2 };
                                char *pathname = kmalloc(sizeof(PATH_MAX)*3, GFP_KERNEL);
                                int i;
                                ...
                                for (i=0; i<3; i++) {
                                        ...
                                        if (ptr[i] == NULL)
                                                continue;
                                        len = strncpy_from_unsafe(pathname+ (PATH_MAX*i), ptr[i], PATH_MAX);
                                        if (len < 0) {
                                                kfree (pathname);
                                                return -EFAULT;
                                        }

                                        ptr[i] = pathname+ (PATH_MAX*i);
                                }
				load_av_image(ptr[0], rs.audio_start, ptr[1], rs.video_start_1, ptr[2], rs.video_start_2);
...
static int load_av_from_file(int cpu, char *load_file, unsigned long load_addr)
{
	struct file *file;
	int file_size;
	...
	printk("avcpu: load cpu %d from file %s to %lx...\n", cpu, load_file, load_addr);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	if (kernel_read(file, (void*)load_addr, file_size, &ofst) != file_size)
		BUG();
#else
	...
	if (audio_image) {
		...
		*((unsigned long *)AUDIO_ENTRY_ADDR) = htonl(audio_fw_entry->target_addr);
		...
		ret = read_emmc_data(audio_fw_entry->offset / EMMC_BLOCK_SIZE, 
				audio_fw_entry->length, (void *)((unsigned long)(audio_fw_entry->target_addr)));
		if (ret < 0)
			goto out;
	...
	if (video_image_1) {
		...
		*((unsigned long *)VIDEO_ENTRY_ADDR_1) = htonl(video_fw_entry_1->target_addr);
		...
		ret = read_emmc_data(video_fw_entry_1->offset / EMMC_BLOCK_SIZE, 
				video_fw_entry_1->length, (void *)((unsigned long)(video_fw_entry_1->target_addr)));
		if (ret < 0)
			goto out;
...
//		// currently we only support align access...
		...
		*((unsigned long *)VIDEO_ENTRY_ADDR_2) = htonl(video_fw_entry_2->target_addr);
		...
		ret = read_emmc_data(video_fw_entry_2->offset / EMMC_BLOCK_SIZE, 
				video_fw_entry_2->length, (void *)((unsigned long)(video_fw_entry_2->target_addr)));
		if (ret < 0)
			goto out;
