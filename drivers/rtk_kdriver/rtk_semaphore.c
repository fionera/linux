...
void dump_sem_data( void )
{
	...
	for (i=0; i<SEMA_HW_SEM_TOP; i++) {
		...
		if (j & 1)
			...
			j);
                RTK_SEM_ERR( " SW sema [0x%p], Owner [0x%x], return address (0x%lx) \n",
			(void *) &sem_record[i],
                        (unsigned int) sem_record[i].sw_sem_uuid,
                        sem_record[i].sw_sem_lr);
#ifdef __linux__
...
inline void _rtd_swsem_lock(int checkid, enum LOCK_UUID uuid, unsigned long ip)
{
	...
	while ( ( swsem = _rtd_swsem_trylock(checkid, uuid, ip) ) != 1 )
	{
		...
		if (--timeout == 0) {
			...
			if ( swsem == -EAGAIN )
			{
				RTK_SEM_ERR( " SW sem %p owned by [0x%x] return address [0x%lx], but try lock by %x \n",
					     (void *) &sem_record[checkid],
					(unsigned int) sem_record[checkid].sw_sem_uuid,
					(unsigned long) sem_record[checkid].sw_sem_lr,
					uuid);
#ifdef __linux__
				RTK_SEM_ERR( " Owner's return address[%pS] \n\n",
		...
		if ( (timeout & 0x0001FFFF) == 0 )
		{
			...
			if ( swsem == -EAGAIN )
			{
				RTK_SEM_WARNING( " sem 0x%p owned by 0x%x, RA [0x%lx], but wait by 0x%x RA [0x%x]",
					&sem_record[checkid],
					(unsigned int) sem_record[checkid].sw_sem_uuid,
					(unsigned long) sem_record[checkid].sw_sem_lr,
					uuid,
...
void _rtd_swsem_unlock( int checkid, enum LOCK_UUID uuid, unsigned long ip )
{
	...
	invalidate_status_flag( );
		...
		RTK_SEM_ERR( "\n\n\n\n"
			"[SW Semaphore] sem 0x%p owned by 0x%x RA [0x%lx], but Unlock by 0x%x"
                        "\n\n\n\n",
                        &sem_record[checkid],
			(unsigned int) value,
			(unsigned long) sem_record[checkid].sw_sem_lr,
