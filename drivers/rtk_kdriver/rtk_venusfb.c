...
static int video_allocMemory(struct venusfb_info *fbi)
{
	...
	if (!(fbi->video_info.videomemory = dvr_malloc_specific(fbi->video_info.videomemorysize, GFP_DCU3))) {
		printk("VenusFB: dvr_malloc(%d) failed ..\n", fbi->video_info.videomemorysizeorder);
		return 1;
	}
#endif
...
// temp disabling DMA map/unmap for Android 64-bit, it suppose be ok due to flush would be performed when using dvr_malloc_
	...
	fbi->fb.screen_base             = fbi->video_info.videomemory;//(char __iomem *)(UNCAC_ADDR((unsigned int)fbi->video_info.videomemory));
		...
		u_int transp, struct fb_info *info)
{
	...
	printk(" #@# %d, r: %d, g: %d, b: %d, %x \n", regno, red, green, blue, (unsigned int)info->pseudo_palette);
//	if (regno == 15)
//		dump_stack();
