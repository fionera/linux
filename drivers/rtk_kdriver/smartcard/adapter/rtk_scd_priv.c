...
int scd_reset(scd *p_this)
{
	...
	wait_event_interruptible_timeout(p_this->wq, p_this->atr.length > 0, (HZ));	/* wait 1000 ms for panasonic ACAS */
	...
	wait_event_interruptible_timeout(p_this->wq, p_this->fsm != IFD_FSM_RESET, (HZ));	/* wait 1000 ms for panasonic ACAS */
...
static void scd_load_gpio_config(scd *p_this)
{
	...
	// PinMux for LG Board
	rtd_outl(0xB8000898, (rtd_inl(0xB8000898) & ~(0xF << 4)) | (0x8 << 4)); /* CD 0xB8000898[7:4] 8: SC_CD_src1 W26*/
	rtd_outl(0xB80008F8, (rtd_inl(0xB80008F8) & ~(0x1 << 8))); /* CD src in input mux, select src1 */

	rtd_outl(0xB8000898, (rtd_inl(0xB8000898) & ~(0xF << 28)) | (0x8 << 28)); /* IO 0xB8000898[31:28]; 8: SC_DATA_src1 Y25*/
	rtd_outl(0x180008F8, (rtd_inl(0x180008F8) & ~(0x1 << 4))); /* IO data in input mux, select src1 */

	rtd_outl(0xB8000898, (rtd_inl(0xB8000898) & ~(0xF << 20)) | (0x8 << 20)); /* RST 0xB8000898[23:20] 8: SC_RST Y26*/
	rtd_outl(0xB8000898, (rtd_inl(0xB8000898) & ~(0xF << 12)) | (0x8 << 12)); /* CLK 0xB8000898[15:12] 8: SC_SCLK W25*/

	rtd_outl(0xB8000894, rtd_inl(0xB8000894) | (0xF << 28)); /* CMD VCC : MISC GPIO 123 AB27*/
	rtd_outl(0xB800083C, rtd_inl(0xB800083C) | (0xF << 24)); /* PWR SEL : MISC GPIO 56 R5*/

	val = 0x8C7B38;

#else
	// PinMux for Merlin5 Board
	rtd_outl(0xB8000898, (rtd_inl(0xB8000898) & ~(0xF << 4)) | (0x8 << 4)); /* CD 0xB8000898[7:4] 8: SC_CD_src1 W26*/
	rtd_outl(0xB80008F8, (rtd_inl(0xB80008F8) & ~(0x1 << 8))); /* CD src in input mux, select src1 */

	rtd_outl(0xB800087C, (rtd_inl(0xB800087C) & ~(0xF << 4)) | (0x8 << 4)); /* IO 0xB800087C[7:4]; 8: SC_DATA_src0 Y22*/
	rtd_outl(0x180008F8, (rtd_inl(0x180008F8) & ~(0x0 << 4))); /* IO data in input mux, select src0 */

	rtd_outl(0xB8000898, (rtd_inl(0xB8000898) & ~(0xF << 20)) | (0x8 << 20)); /* RST 0xB8000898[23:20] 8: SC_RST Y26*/
	rtd_outl(0xB8000888, (rtd_inl(0xB8000888) & ~(0xF << 4)) | (0x8 << 4)); /* CLK 0xB8000888[7:4] 8: SC_SCLK AE26*/

	rtd_outl(0xB8000844, rtd_inl(0xB8000844) | (0xF << 20));    /* CMD VCC : MISC GPIO 62 AA9*/
	rtd_outl(0xB8000894, rtd_inl(0xB8000894) | (0xF << 4));    /* PWR SEL : MISC GPIO 126 AA26*/
	val = 0x8C3E7E;
...
scd *scd_open(unsigned char id)
{
	...
	if (p_this) {
		...
		if (request_irq
		    (IRQ_GET_KERNEL_IRQ_NUM(IRQ_MISC), scd_isr, SA_SHIRQ, "REALTEK SCD",
		     (void *)p_this) < 0) {
			SC_ERR
...
#else
	free_irq(IRQ_GET_KERNEL_IRQ_NUM(IRQ_MISC), p_this);
#endif
