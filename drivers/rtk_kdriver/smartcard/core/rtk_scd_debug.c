...
static ssize_t smc_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
...
static ssize_t smc_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    int ret = 0;
    int len =0;
    char str[512];
    ret= sprintf(str,"SMC kernel driver debug message\n");
    ret += sprintf(str + ret,"Usage:\n");
    ret += sprintf(str + ret,"echo \'debug_on=1\' > dbg  : enable debug log print on\n");
    ret += sprintf(str + ret,"echo \'print_rtx=1\' > dbg : enable rx/tx buffer log print on\n");
    ret += sprintf(str + ret,"Current: debug_on=%d,print_rtx=%d\n",smc_log_onoff,smc_print_rtx);
    len = strlen(str);
