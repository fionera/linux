...
#ifndef CONFIG_CUSTOMER_TV006
	// set FCIC pinmux, sync from fekao
	rtd_part_outl(0xb8000808,15,12,0x1);//SPI_CS
	rtd_part_outl(0xb8000808,7,4,0x1);  //SPI_SCK
	rtd_part_outl(0xb800080c,31,28,0x1);//SPI_DO
	rtd_part_outl(0xb800080c,23,20,0x1);//SPI_DI
#endif

	regval1 = rtd_inl(0xb8000808);
	regval2 = rtd_inl(0xb800080c);

	SPI_INFO("SPI FCIC pinmux setting check, reg-0x%x: 0x%x , reg-0x%x: 0x%x!\r\n", \
		0xb8000808, regval1, 0xb800080c, regval2);
...
static int sfc_check_pin_mux_cs0(void)
{
	...
	regval1 = (rtd_inl(0xb8000850) & 0x0f0f0f0f) | 0x20202820;
...
static int sfc_set_clk(struct spinor_chip_t *spinor_chip, unsigned int clk)
{
	...
	if(clk_div <= support_max_div && clk_div >= support_min_div) {
		...
		for(delay_sel=(DELAY_SEL_STEP_TOTAL_CNT - 1); delay_sel >= 0; delay_sel--){
			if(delay_sel_flag & (1<<delay_sel)){
