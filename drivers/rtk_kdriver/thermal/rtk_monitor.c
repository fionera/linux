...
static unsigned int rtk_monitor_debug = 0; // ms
	...
	if((test_mode & 0x2) == 0x2) { //[1]Main Enable
		...
		if((test_mode & 0x4) == 0x4) { // [2]DCU1 R480 CAl. Enable
			...
			for(loop = 0; loop < R480_max_loop; loop++) { //scan loop
				...
				usleep_range(180, 250); //need to check
