...
#define K6_DDR_DYNAMIC_REFRESH_SETTING
#ifdef K6_DDR_DYNAMIC_REFRESH_SETTING
...
void callback_reg_test(void) //this is just for test callback
{
    ...
    if(!init_flag) {
        ...
        register_temperature_callback(30, callback_fn, (void *)&myint[0], "CPU");
        register_temperature_callback(40, callback_fn, (void *)&myint[1], "GPU");
        register_temperature_callback(50, callback_fn, (void *)&myint[3], "CPU");
...
#ifdef K6_DDR_DYNAMIC_REFRESH_SETTING
/**
 * KGD_Tj = SOC_Tj - 15℃  (HP提供的KGD/SOC的溫度關係)
 * Temperature error tolerance 3℃
 *   if(KGD_Tj <= 82)
 *       0x1800880C=0x282890A5;       //tREFI=7.8us, tRFC=260ns
 *   if(82< KGD_Tj <= 92)
 *       0x1800880C=0x281440A5;       //tREFI=3.9us, tRFC=260ns
 *   if(92< KGD_Tj <= 102)
 *       //0x1800880C=0x280F30A5;       //tREFI=2.925us, tRFC=260ns
 *       0x1800880C=0x280A20A5;       //tREFI=1.95us, tRFC=260ns
 *   if(KGD_Tj > 102)
 *       0x1800880C=0x280A20A5;       //tREFI=1.95us, tRFC=260ns
 */
...
#define L1_REF_SEETING       0x282890A5
#define L2_REF_SETTING       0x281440A5
#define L3_REF_SETTING       0x280A20A5
#define L4_REF_SETTING       0x280A20A5
    ...
    if (kgd_tj <= KGD_L1_UP) {
        if (cur_val != L1_REF_SEETING) {
            rtd_outl(DC1_MC_TMCTRL3_reg, L1_REF_SEETING);
        ...
        if (cur_val != L2_REF_SETTING) {
            rtd_outl(DC1_MC_TMCTRL3_reg, L2_REF_SETTING);
        ...
        if (cur_val != L3_REF_SETTING) {
            rtd_outl(DC1_MC_TMCTRL3_reg, L3_REF_SETTING);
            KGD_L1_UP = KGD_L1_DEFAULT;
        ...
        if (cur_val != L4_REF_SETTING) {
            rtd_outl(DC1_MC_TMCTRL3_reg, L4_REF_SETTING);
            KGD_L1_UP = KGD_L1_DEFAULT;
            KGD_L2_UP = KGD_L2_DEFAULT;
    ...
    if (change)
        pr_info("%s: [tmu] temp:%d, kgd:%d. 0x%x=0x%08x\n", __func__,
...
static void dump_throttle_resource_log(THROTTLE_CONTENT_T *throttle_content)
{
        ...
        for(i=0; i<THROTTLE_LINE_MAX; i++)
        {
                ...
                pr_debug("%s ", throttle_content[i].module_name_content);
                
                for(j=THROTTLE_ENABLE_KEY_E; j<THROTTLE_MAX_KEY_E; j++)
                        pr_debug("%d ", throttle_content[i].content[j-1]);
                pr_debug("\n");
        }
}
#endif
...
{	
...
#endif
					pr_info("Monitor Resource registerd:%s\n",name);
					return;
				}
			}
		}
	...
	{
		if(strcmp(name, "core")==0) 
			rtk_tmu_thermal_zone(90,100,120,150);
		else if(strcmp(name,"pq")==0) {
#ifdef CONFIG_RTK_KDRV_THERMAL_SENSOR                           
#ifdef CONFIG_SUPPORT_SCALER
			Scaler_register_PQ_thermal(115,120,125);
#endif
#endif
		}
#ifdef CONFIG_MEMC
...
#ifdef CONFIG_RTK_KDRV_THERMAL_SENSOR                           
			Scaler_register_MEMC_thermal(115,120,125);
#endif
         
		}
#endif
	}
	pr_info("Monitor Resource not registerd\n");
...
int rtk_tmu_appy_resource(void)
{
	...
	{
		apply_callback("pq",NULL);
		apply_callback("memc",NULL);
	}
	pr_info("Monitor callback is registered\n");
	return 0;		
}
