...
#include "rdvb-buf/dmx_buf_mgr.h"
        ...
        while(!kthread_should_stop()) {
                for (i = 0; i < MAX_TP_COUNT; i++) {
                        ...
                        if (pTp_drv->tp[i].Shared_RWPtr.inited == 1) {
                                ...
                                if(pTp_drv->useECP == 1) {
                                        UINT8 protect_status;
                                        optee_tp_get_channel_protect_data(i, &protect_status);
                                        if(protect_status == 1) {
                                            _tp_unlock();
                                            continue;
                                        }
...
)
{
        ...
        TP_TRACE("exit, TPOUT%u pre_div=%u post_div=%u div16=%u\n", tpout_id, pre_div, post_div, div16);
...
INT32 RHAL_ATSC30_Open(TPK_TP_ENGINE_T tp_id)
{
        ...
        unsigned int   request_buffer_size       = 0;
        int i;
        ...
        if( rdvb_dmxbuf_tpbuf_get(TP_CON_ATSC3, TP_BUF_LEVEL_A, &pTp_drv->tp_atsc[tp_id].bbp_array_phy, &pTp_drv->tp_atsc[tp_id].bbp_array_cached, &request_buffer_size, false) <0 ) {
                TP_WARNING("RHAL_ATSC30_Open dmxbuf_request_tpbuf fail\n");
                return TPK_FAIL;
        }

        if(request_buffer_size < (pTp_drv->tp_atsc[tp_id].bbp_array_size + 3670016 + 524288) ) {
                TP_WARNING("RHAL_ATSC30_Open dmxbuf_request_tpbuf fail size enough\n");
                return TPK_FAIL;
        }

        pTp_drv->tp_atsc[tp_id].bbp_array_cached = dvr_remap_cached_memory(pTp_drv->tp_atsc[tp_id].bbp_array_phy, TP_MAIN_BUFF_SIZE, __builtin_return_address(0));
        ...
        BbfBufferZoneStartVirAddr = pTp_drv->tp_atsc[tp_id].bbp_array_cached + 4202496;
        BbfBufferZoneStartPhyAddr = pTp_drv->tp_atsc[tp_id].bbp_array_phy + 4202496;
...
INT32 RHAL_ATSC30_Close(TPK_TP_ENGINE_T tp_id)
{
        ...
        if(pTp_drv->tp_atsc[tp_id].bbp_array_cached != 0x00) {
            dvr_unmap_memory((UINT8*)(pTp_drv->tp_atsc[tp_id].bbp_array_cached), TP_MAIN_BUFF_SIZE);
            dmxbuf_release_tpbuf(TP_CON_ATSC3);
            pTp_drv->tp_atsc[tp_id].bbp_array_cached = 0x00;
        }
 ...
 * Retn : TPK_SUCCESS
 *------------------------------------------------------------------*/
INT32 RHAL_Delivery_SetMode(UINT8 mode)
{
        _tp_lock();

        pTp_drv->deliveryMode = mode;

        _tp_unlock();
        TP_DBG("RHAL_Delivery_SetMode mode=%d\n", mode);
        return TPK_SUCCESS;
}
...
{
        //TPK_TP_ENGINE_T tp_id_remap;
        //TP_BUFFER_PARAM_T param;
        //unsigned char *tp_buffer;
        UINT32   i;
        UINT8    *tlv_buffer;
        UINT32   tlv_buffer_sz = 0;
        ...
        TP_DBG("#DeliveryPerf# [%d] RHAL_Delivery_RequestData tp_id=%d\n", READ_REG32(TIMER_SCPU_CLK90K_LO_reg) / 90, tp_id);
        ...
        /* save to data structure */
        pTp_drv->tp[tp_id_remap].mass_buffer.OriBufSize = (TLV_RINGBUFFER_SIZE / (188 * 192)) * (188 * 192);
        pTp_drv->tp[tp_id_remap].mass_buffer.BasePhy    = param.Base;
        pTp_drv->tp[tp_id_remap].mass_buffer.LimitPhy   = param.Base + (TLV_RINGBUFFER_SIZE / (188 * 192)) * (188 * 192);
        pTp_drv->tp[tp_id_remap].mass_buffer.BufSize    = param.Limit - param.Base;
        pTp_drv->tp[tp_id_remap].mass_buffer.BaseVir    = (UADDRESS)tp_buffer;
        pTp_drv->tp[tp_id_remap].mass_buffer.LimitVir   = (UADDRESS)tp_buffer + (TLV_RINGBUFFER_SIZE / (188 * 192)) * (188 * 192);
        pTp_drv->tp[tp_id_remap].mass_buffer.RP         = param.Base;
        pTp_drv->tp[tp_id_remap].mass_buffer.WP         = param.Base;
        pTp_drv->tp[tp_id_remap].mass_buffer.RP_Connect[MASS_DATA_BUFFER_RP] = param.Base;
        pTp_drv->tp[tp_id_remap].mass_buffer.RP_Connect[SECTION_BUFFER_RP]   = 0;
        pTp_drv->tp[tp_id_remap].mass_buffer.RP_Connect[CI_BUFFER_RP]        = 0;
        pTp_drv->tp[tp_id_remap].mass_buffer.now_rp = param.Base;
        pTp_drv->tp[tp_id_remap].mass_buffer.RP_Connect_cnt = 1;

        pTp_drv->tp[tp_id_remap].mass_buffer.bock_size = 188;
        Tp_FRC_SetPrefix( tp_id_remap, TP_FRC_NO_PREFIX, 0x00);

        pTp_drv->tp_japan[tp_id].open_count++;

        pTp_drv->tp_japan[tp_id].cb.callback = Tp_japan4k_dmx_callback;

        pTp_drv->tp_japan[tp_id].dmx = create_japan4k_dmx(&pTp_drv->tp_japan[tp_id].cb);
...
#else
        //tlv_buffer = (UINT8 *)dvr_malloc_specific(SIZE_PER_TLV_BUFFER * TLV_DESCRIPTOR_CNT, GFP_DCU1);
        //tlv_buffer_phy = dvr_to_phys( tlv_buffer );
        tlv_buffer_sz = SIZE_PER_TLV_BUFFER * TLV_DESCRIPTOR_CNT;
        if( rdvb_dmxbuf_tpbuf_get(TP_CON_JP, TP_BUF_LEVEL_A, &tlv_buffer_phy, &tlv_buffer, &tlv_buffer_sz, false) <0 )
        {
                TP_WARNING("dmxbuf_request_tpbuf fail\n");
                return TPK_FAIL;
        }

        tlv_buffer = dvr_remap_cached_memory(tlv_buffer_phy, TP_MAIN_BUFF_SIZE, __builtin_return_address(0));
		
        TP_WARNING("tlv_buffer_phy=0x%x tlv_buffer=0x%p\n", tlv_buffer_phy, tlv_buffer);
        ...
        pTp_drv->tp_japan[tp_id].dmx = create_japan4k_dmx(&pTp_drv->tp_japan[tp_id].cb);
#else
        pTp_drv->tp_japan[tp_id].dmx = (struct japan4k_dmx*)(pTp_drv->tp_japan[tp_id].mass_buffer.BaseVir + SIZE_PER_TLV_BUFFER * TLV_DESCRIPTOR_CNT);
        memset((char*)(pTp_drv->tp_japan[tp_id].dmx), 0, sizeof(struct japan4k_dmx));
        pTp_drv->tp_japan[tp_id].dmx->cb.callback = Tp_japan4k_dmx_callback;
#endif
        if (pTp_drv->tp_japan[tp_id].dmx == NULL) {
                TP_ERROR("[%s %d] Error!! malloc japan4k_dmx fail \n", __func__ , __LINE__ );
                _tp_unlock();
        ...
        pTp_drv->tp_japan[tp_id].for_debug_first_tlv_in_time = 0;
...
INT32 RHAL_Delivery_CancelData(TPK_TP_ENGINE_T tp_id)
{
        ...
        if(pTp_drv->tp_japan[tp_id].mass_buffer.BaseVir != 0x00) {
            dvr_unmap_memory((UINT8*)(pTp_drv->tp_japan[tp_id].mass_buffer.BaseVir), TP_MAIN_BUFF_SIZE);
            dmxbuf_release_tpbuf(TP_CON_JP);
...
INT32 RHAL_TP_CheckProtectStatus(TPK_TP_ENGINE_T tp_id, UINT8 *isNeedEnterProtect)
{
        _tp_lock();
        Tp_Get_ProtectStatus( tp_id, isNeedEnterProtect);
        _tp_unlock();
        return TPK_SUCCESS;
}

INT32 RHAL_TP_EnableProtect(TPK_TP_ENGINE_T tp_id)
{
        _tp_lock();
        Tp_Ta_set_mem_prison(1);
        _tp_unlock();
        return TPK_SUCCESS;
}
EXPORT_SYMBOL(RHAL_TP_EnableProtect);

INT32 RHAL_TP_DisableProtect(TPK_TP_ENGINE_T tp_id)
{
        _tp_lock();
        Tp_Ta_set_mem_prison(0);
        _tp_unlock();
        return TPK_SUCCESS;
}
EXPORT_SYMBOL(RHAL_TP_DisableProtect);
