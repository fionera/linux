...
struct  file *rec_tlv_fd1 = NULL;
struct  file *rec_tlv_fd2 = NULL;
...
static const char* cmd_str[TP_PROC_COUNT] = {
        ...
        TP_PROC_PARAM_RECORD_TLV1_EN,
        TP_PROC_PARAM_RECORD_TLV2_EN
};
...
static ssize_t tp_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
        ...
        switch(i) {
                ...
                case TP_PROC_RECORD_TLV1_EN: {
                        if (sscanf(cmd, "%d tp=%u crc=%u tspath=%256s", &enable, &u32_tp_id, &record_tlv_use_crc, rec_file) < 1) {
                                TP_WARNING("sscanf get data failed (%s)\n", cmd);
                                return -EFAULT;
                        } else {
                                TP_DBG_SIMPLE("record_tlv1_en=%d, tp_id=%d, crc=%d rec_file=%s\n", enable, u32_tp_id, record_tlv_use_crc, rec_file);
                                if(enable) {
                                        if(rec_tlv_fd1) {
                                                TP_DBG_SIMPLE("BBP fd exit, BBP Record Stopping...\n");
                                                filp_close(rec_tlv_fd1, NULL);
                                                rec_tlv_fd1 = NULL;
                                        }
                                        rec_tlv_fd1 = filp_open(rec_file, O_RDWR | O_CREAT | O_TRUNC, 0);

                                        if(rec_tlv_fd1)
                                                TP_DBG_SIMPLE("Start Rec BBP to - %s, rec_bbp_fd=%p\n", rec_file, rec_tlv_fd1);
                                        else
                                                TP_DBG_SIMPLE("Start Rec BBP failed, open %s failed\n", rec_file);
                                } else {
                                        if(rec_tlv_fd1) {
                ...
                case TP_PROC_RECORD_TLV2_EN: {
                        if (sscanf(cmd, "%d tp=%u crc=%u tspath=%256s", &enable, &u32_tp_id, &record_tlv_use_crc, rec_file) < 1) {
                                TP_WARNING("sscanf get data failed (%s)\n", cmd);
                                return -EFAULT;
                        } else {
                                TP_DBG_SIMPLE("record_tlv2_en=%d, tp_id=%d, crc=%d rec_file=%s\n", enable, u32_tp_id, record_tlv_use_crc, rec_file);
                                if(enable) {
                                        if(rec_tlv_fd2) {
                                                TP_DBG_SIMPLE("BBP fd exit, BBP Record Stopping...\n");
                                                filp_close(rec_tlv_fd2, NULL);
                                                rec_tlv_fd2 = NULL;
                                        }
                                        rec_tlv_fd2 = filp_open(rec_file, O_RDWR | O_CREAT | O_TRUNC, 0);

                                        if(rec_tlv_fd2)
                                                TP_DBG_SIMPLE("Start Rec BBP to - %s, rec_bbp_fd=%p\n", rec_file, rec_tlv_fd2);
                                        else
                                                TP_DBG_SIMPLE("Start Rec BBP failed, open %s failed\n", rec_file);
                                } else {
                                        if(rec_tlv_fd2) {
