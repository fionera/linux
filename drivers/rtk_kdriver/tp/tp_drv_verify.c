...
long TPVerify_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
        ...
        switch (cmd) {
                ...
                case TP_IOC_TPP_SET_PIDEN:
                        ...
                        break;
                case TP_IOC_TPP_MTP_STREAM_CONTROL:
                        /* copy parameter from user space */
                        if (copy_from_user(&param_tpp_strm_ctrl, (void __user *)arg, sizeof(TP_IOC_TPP_STREAM_CONTROL_T))) {
                                TP_WARNING("TP_IOC_TPP_MTP_STREAM_CONTROL, copy parameter from user space failed\n");
                                return -EFAULT;
                        }

                        ret = RHAL_MTPStreamControl((param_tpp_strm_ctrl.tp_p_id+TP_TPP0_MTP), param_tpp_strm_ctrl.ctrl);
...
int TPVerify_RunDescTestEx2(IN TPK_TP_ENGINE_T tp_id, DESCRAMBLER_TEST_ITEM* pTest, char* pattern_path)
{
        ...
        if (NULL != pbuff) {
                ...
                RHAL_SetTPRingBuffer(tp_id, (UINT8*)&pbuff_phy, pbuff, TS_RING_BUFFER_SIZE);
                RHAL_SetMTPBuffer(tp_id, (UINT8*)((UADDRESS)pbuff_phy + TS_RING_BUFFER_SIZE), (UINT8*)(pbuff + TS_RING_BUFFER_SIZE), TS_RING_BUFFER_SIZE);
