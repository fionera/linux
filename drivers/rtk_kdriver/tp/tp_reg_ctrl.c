...
INT32 Tp_TPO_SetClkDiv( UINT32 pre_div, UINT32 post_div, UINT32 div16)
{
        ...
        /* clock rate = 250 Mhz / ((reg_tp_pre_div+2) *(reg_tp_post_div + 2))      */
        /* if div16 = 1                                                            */
        /* clock rate = 250 Mhz / ((reg_tp_pre_div+2) *(reg_tp_post_div + 2) * 16) */
...
INT32 Tp_SetPidParam(UINT16 pid_idx, TPK_PID_REG_PARAM_T Param)
{
        ...
        pid_data.v        = 0; /* to modify this register, need to set 0 first */
...
)
{
        ...
        if ((pData[1] & 0x80) == 0 && crc_err) {
                if(!(pData[0] == 0x70 || pData[0] == 0x73)) { /* TDT TOT section_syntax_indicator=0 */
        ...
        for( i = 0; i < MAX_SEC_COUNT; i++) {
                ...
                if((pSec->valid) &&
                	...
                	if (pSec->sec_api_setting.dsc){
                        		pSec->sec_api_setting.dsc(pData, ( __kernel_size_t)pPid_sec->sec_data_cnt, NULL, 0, pSec->sec_api_setting.pContext2);
                        }
...
void Tp_Section_assemble(TPK_TP_ENGINE_T tp_id)
{
        ...
        for(Info_process_cnt = 0 ; Info_process_cnt < InfoBufCnt; Info_process_cnt += INFO_PACK_BUFFER_BLOCK_SIZE) {
                ...
                _tp_ecp_lock()
                if(pTp_drv->useECP == 0 || tp_id != TP_TP0 ) {
#ifdef CONFIG_COMPAT
                        ...
                        optee_tp_get_section_data( (UINT32)pInfoBufPhy + Info_process_cnt);
        ...
        while(!kthread_should_stop()) {
                ...
                for( i = 0; i < MAX_TP_COUNT; i++) {
                        ...
                        if(time_cnt >= debug_log_interval_ms) {
                                ...
                                tp_tf_int_reg.regValue  = READ_REG32(tp_tf_int_reg_addr);
                                tp_tf_cntl_reg.regValue = READ_REG32(tp_tf_cntl_reg_addr);
                                if(tp_tf_int_reg.overflow == 1 && tp_tf_cntl_reg.busy == 1 && pTp_drv->mtp[i].mtp_stream_status == MTP_STREAM_STOP) {
                                        RHAL_SetTPHardwareReset(1);
                                }
                        }
                }
                ...
                for( i = 0; i < MAX_TP_COUNT; i++) {
                        if( tp_log_onoff && (time_cnt_status >= status_log_interval_ms)) {
                                ...
                                if(pTp_drv->tp_japan[i].isJAPAN4K == 1) {
                                        ...
                                        printk(KERN_DEBUG "[TP]flowrate=%d full=%u stc(<250ms)=%d stc(>250ms)=%d stc_max=%d\n", pTp_drv->tp_japan[i].for_debug_tlv_total_size / time_cnt_status, pTp_drv->tp_japan[i].for_debug_ring_buffer_full_cnt, pTp_drv->tp_japan[i].timestamp_range1, pTp_drv->tp_japan[i].timestamp_range2, pTp_drv->tp_japan[i].timestamp_max);
                                        pTp_drv->tp_japan[i].timestamp_max = 0;
                                        pTp_drv->tp_japan[i].timestamp_range1 = 0;
                                        pTp_drv->tp_japan[i].timestamp_range2 = 0;
                                }
                        }
                }
        ...
        while(!kthread_should_stop()) {
                ...
                for( tpp_id = TP_TPP0; tpp_id < MAX_TP_P_COUNT; tpp_id++) {
                        if( pTp_drv->tpp[tpp_id].tpp_stream_status == TPP_STREAM_START &&
                            ( pTp_drv->tpp[tpp_id].tpp_src == TS_SRC_0 || pTp_drv->tpp[tpp_id].tpp_src == TS_SRC_2
                              ...
                              || pTp_drv->tpp[tpp_id].tpp_src == TS_SRC_1
                                ...
                                if ( pTp_drv->tpp[tpp_id].tpp_src == Internal_Demod )
                                        ...
                                        tpi_id = TP_TPI0;
                                else if ( pTp_drv->tpp[tpp_id].tpp_src == TS_SRC_2 )
                                        tpi_id = TP_TPI2;
                                else {
#ifdef TP_VERIFY_ENABLE
...
void Tp_japan4k_dmx_callback(        void*    p_ctx, unsigned char *p_data, unsigned int len)
{
        ...
        if( (pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_max_size - pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_size) >= len ) {
                ...
                if(pTp_drv->tp_japan[p_this->tp_id].for_debug_first_tlv_in_time == 0) {
                        pTp_drv->tp_japan[p_this->tp_id].for_debug_first_tlv_in_time = READ_REG32(TIMER_SCPU_CLK90K_LO_reg);
                        TP_WARNING("#DeliveryPerf# [%d] TP%d First TLV data in\n", READ_REG32(TIMER_SCPU_CLK90K_LO_reg) / 90, p_this->tp_id);
                }
                if(copy_to_user((void *)(pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer + pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_size), (const void __user *)p_data, len)) {
                        ...
                        if(rec_tlv_fd2 != NULL) {
                                ...
                                old_fs = get_fs();
                                set_fs(KERNEL_DS);
                                if(record_tlv_use_crc == 0)
                                        vfs_write(rec_tlv_fd2, p_data, len, &rec_tlv_fd2->f_pos);
                                else {
                                        UINT32 crc_value = DoCRC32(p_data, len);
                                        UINT32 timestamp = READ_REG32(TIMER_SCPU_CLK90K_LO_reg);
                                        timestamp = TP_reverseInteger(timestamp);
                                        vfs_write(rec_tlv_fd2, (UINT8*)&timestamp, sizeof(timestamp), &rec_tlv_fd2->f_pos);
                                        vfs_write(rec_tlv_fd2, (UINT8*)&crc_value, sizeof(crc_value), &rec_tlv_fd2->f_pos);
                                }
                                set_fs(old_fs);
                        }
                ...
                TP_WARNING("tlv feed full max=%lu curr=%lu want=%d\n", pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_max_size, pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_size, len);
        }
        return ;
...
INT32 Tp_Ta_set_mem_prison(UINT8 isEnable )
{
        _tp_ecp_lock()
        optee_tp_init();

        RHAL_TP_TA_set_mem_prison(isEnable);

        if (isEnable == 0)
            optee_tp_deinit();
        pTp_drv->useECP = isEnable;
