...
#define TP_CMD_GET_CHANNEL_PROTECT_DATA 7
...
int optee_tp_get_channel_protect_data(UINT32 channel, UINT8 *protect_status)
{
        ...
        memset(&arg, 0, sizeof(arg));
        arg.func = TP_CMD_GET_CHANNEL_PROTECT_DATA;
        arg.session = tp_ta.session;
        arg.num_params = TEE_NUM_PARAM;

        memset(param, 0, sizeof(struct tee_param) * TEE_NUM_PARAM);
        param[0].attr = TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INOUT;
        param[0].u.value.a = channel;
        param[1].attr = TEE_IOCTL_PARAM_ATTR_TYPE_NONE;
        param[2].attr = TEE_IOCTL_PARAM_ATTR_TYPE_NONE;
        param[3].attr = TEE_IOCTL_PARAM_ATTR_TYPE_NONE;
        ...
        *protect_status = param[0].u.value.b;
out:
        if (ret)
                return ret;
        else
