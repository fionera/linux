...
{/* Do YPbPr coneect initial setting */
	...
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_2_0 = 1;/* SOG1 input MUX select (offline) 001:VIN4P */
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_5_4 = 1;/* SOG0 input MUX select (online) 0001:VIN4P */
	...
	if (YPbPr_TimingDetectStage == YPBPR_SOY_STATE) {/* Around 90ms */
		...
		if (YPbPr_DetectTiming_PadCheck() == FALSE) {
			...
			if(ypp_debug_msg_print)
				rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)] Pad no signal!!!\n", __FUNCTION__, __LINE__);
		...
		if (_ENABLE == Scaler_Get_YPbPr_Auto_Soy_Enable()) {
			if (FALSE == YPbPr_AutoSOY_Detect()) {/* Auto restart setting (AutoSOY()) */
				...
				IoReg_Write32(AUTO_SOY_SOYCH0_A0_reg, auto_soy_soych0_a0_reg.regValue);
				if(ypp_debug_msg_print)
					rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]SOY fail, no signal!!!\n", __FUNCTION__, __LINE__);
		...
		if (ret == _MODE_SUCCESS) {
			...
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)] 0xB8020300=0x%08x\n", __FUNCTION__, __LINE__,IoReg_Read32(0xB8020300));
			return _MODE_SUCCESS;
		} else {
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)] ret=0x%08x\n", __FUNCTION__, __LINE__, ret);
		...
		if((adc_adc_dcrestore_ctrl_reg.adc_dcrestore_ctrl_5 == 0)
			...
			rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]re-do timing check,0xB8020018=0x%08x,0xB8021004=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8020018),IoReg_Read32(0xB8021004));
			rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]re-do timing check,0xB8020024=0x%08x,0xB8021070=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8020024),IoReg_Read32(0xB8021070));
			SyncProc_FunctionDisable(_SRC_YPBPR);
			YPbPr_TimingDetectStage = YPBPR_SOY_STATE;/* To do Auto SOY */
			ypp_good_timing_ready = FALSE;
			return ret;
...
void YPbPr_DetectFail_NoSignal_Handler(void)
{
    ...
    if (ypp_debug_msg_print){
		...
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]0xB8020018=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8020018));
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]0xB8021100=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8021100));
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]0xB8021118=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8021118));
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]0xB8020300=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8020300));
		rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]0xB802030C=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB802030C));
    }
}
		  ...
		  if (autosoy_no_trigger_flag_reg.no_trig_r) {/* check notrig flag */
				...
				if (_ENABLE == Scaler_Get_AutoSOY_Aux_Detect_Enable()) {
						...
						if ((autosoy_soych0_a4_reg.cmp_level_r > Scaler_Get_AutoSOY_Aux_Compare_Level_Min())
							...
							rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)], trigger i = %d\n",__func__,__LINE__, i);
				...
				if (FALSE == AutoSOYRestart) {/* Run (False Restart) if notriger=0 in the first time */
					...
					rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)] setting DCLevel level=0x%x,trigger i=%d\n",__func__,__LINE__, DCLevel + AUTO_SOY_COMPARE_LEVEL_OFFSET, i);
...
//	dds_pll_pllphase_ctrl_RBUS dds_pll_pllphase_ctrl_reg;
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "auto phase=%d uc64phase_limited:%d\n", phase, uc64phase_limited);
#if 0
	...
	adc_clock_reg.adc_2x_sample_5 = 0;
	adc_clock_reg.adc_clock_6 = 1;
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "####[%s(%d)]M_VALUE:0x%08x L_VALUE:0x%08x A_temp1:0x%08x A_temp2:0x%08x####\r\n",__func__,__LINE__,M_VALUE,L_VALUE,A_temp1,A_temp2);
	/* Calculate the absolute value from the selected phase to transition */
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "fine-tune phase=%d, PhaseMax=%d\n", phase, PhaseMax);
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC,  "Danger_distance=%d\n", ucDanger_distance);
#endif

	rtd_printk(KERN_NOTICE, TAG_NAME_ADC,  "Danger_position=%d, ucDanger_tolerance=%d\n", ucDanger_position, ucDanger_tolerance);
...
{/* This api is setting after detect timing success  mode setting */
	...
	rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]0xB8020018=0x%08x",__func__, __LINE__,IoReg_Read32(0xB8020018));
...
{	//input: _SRC_YPBPR, _SRC_AV
	...
	if(input == _SRC_YPBPR){
		adc_ctl_rgb12_reg.adc_ctl_rgb1_11_10 = 0;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_8 = 0x1;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_3_2 = 0x1;
	}else{
		adc_ctl_rgb12_reg.adc_ctl_rgb1_11_10 = (_AV_INPUT3 == get_AVD_Input())?0x1:0x2;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_8 = 0;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_3_2 = 0x3;
	}
