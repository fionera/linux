...
extern struct semaphore *get_avd_channelchangeSemaphore(void);
		...
		if (0 == drvif_module_vdc_read_Clamping_path())
		{
...
#endif
			{
				...
				if (vdc_resetTV_Count >= 18)
				{
					...
					rtd_printk(KERN_EMERG, TAG_NAME_AVD, "#####[%s(%d)]sw reset!\n",__func__,__LINE__);
...
extern void Set_Reply_Zero_Timing_Flag(unsigned char src, unsigned char flag);/*If true, let webos get zero timing*/
	...
	if (1 == drvif_module_vdc_GetLockStatus()) {
		...
		case DETECT_STAGE_INIT:
			...
			if (uc625_flag != 0xff)
			{
				...
				Scaler_AVD_SetTimingModeChanged(true);//memset struct when channel changed
				...
				case TV_AllMode:
					...
					break;
				default:
					match_support_color_system = false;
					g_ucVDMode = VDC_MODE_AUTO;
					break;
				}
...
//			drvif_module_vdc_Set_lose_chromalock_count(0x0b);	/* set to default */
			...
			if((g_ucVDMode != g_ucVDPreMode) && match_support_color_system){
				rtd_printk(KERN_INFO, TAG_NAME_AVD, "#####[%s(%d)] g_ucVDMode=%d,g_ucVDPreMode=%d\n",__func__,__LINE__,g_ucVDMode,g_ucVDPreMode);
				g_bChangeSystem = TRUE;
			}

			rtd_printk(KERN_INFO, TAG_NAME_AVD, "#####[%s(%d)] DETECT_STAGE_INIT, g_ucVDMode=%x, g_ucVDPreMode=%x, g_bChangeSystem=%x, 94a4=%x\n", __func__, __LINE__, g_ucVDMode, g_ucVDPreMode, g_bChangeSystem,vdcf_inl(0xb80194a4));
			...
			rtd_printk(KERN_INFO, TAG_NAME_AVD, "#####[%s(%d)] DETECT_STAGE_INIT,g_ucVDMode=%d,g_bMainRunSearchState=%d\n",__func__,__LINE__,g_ucVDMode,g_bMainRunSearchState);
...
#ifdef CONFIG_CUSTOMER_TV010
				//[Start]ryanliao 20181226 bgate_Setting
				module_vdc_Set_bgate_Setting(0);
				//[End]
				drvif_inprocvideo_setup_NotMeasure(g_ucSrcType, ptAVDScalerDispInfo, ptAVDScalerTimingInfo, g_ucVDMode | 0x10, 0);
#endif
#ifndef CONFIG_CUSTOMER_TV010
			...
			{
				if (g_tSupportedColorSys == TV_AllMode){
			...
			Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_AVD, SOURCE_GOOD_TIMING_READY);
		...
		case DETECT_STAGE_FINAL:
			...
			if(!atv_channel_change && atv_channel_change_start && avd_good_timing_ready && (get_AVD_Input_Source() == _SRC_TV)){
				down(get_avd_channelchangeSemaphore());
				atv_channel_change_start = FALSE;
				up(get_avd_channelchangeSemaphore());
		...
		if ((0 == drvif_module_vdc_GetLockStatus()) && (g_ucSrcType == _SRC_TV)) {
			...
			if (((ptAVDScalerDispInfo->ucMode_Curr) == 0 )|| avd_good_timing_ready)
			{
				if((atv_channel_change || atv_channel_change_start || !avd_good_timing_ready)
					...
					ATV_no_signal_Count++;
					if(ATV_no_signal_Count >= 10){//if atv is no signal it will timeout and set scaler timing info
						avd_good_timing_ready= TRUE;
						down(get_avd_channelchangeSemaphore());
						atv_channel_change_start = FALSE;
						up(get_avd_channelchangeSemaphore());
				...
				if(Scaler_AVD_GetIsChannelChange() == 0){
					...
					ClearAVDLGETimingInfo(); //report timing info to WebOS
				}else{
					rtd_printk(KERN_INFO, TAG_NAME_AVD,"#####[%s(%d)] return due to channel changed!\n", __func__, __LINE__);
					return ucRet;
				}
			}
			if(!VDC_get_detect_flag())
