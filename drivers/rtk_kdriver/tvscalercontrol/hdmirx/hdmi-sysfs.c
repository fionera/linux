...
static unsigned char current_add_vic_on_vdb= 0; //
...
extern unsigned int hdmi_amd_vsdb_debug_en;
extern unsigned int hdmi_amd_vsdb_debug_threshold;
    ...
    )
{
    ...
    n = scnprintf(ptr, count, "Gaming: ALLM=%d, VRR_EN=%d, Scaler IVS V Freq=%d\n", allm_mode, newbase_hdmi_get_vrr_enable(port), get_scaler_ivs_framerate()/10);
    ptr+=n; count-=n;
    
    //---------------------------------------
    // Force BG
    //---------------------------------------
    scnprintf(ptr, count,"FOCE_BG : %s\n",  (GET_FORCE_BG_STATUS()) ? "On" : "OFF");
    ptr+=n; count-=n;
#endif
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "debug_ctrl")==0)
    {
        ...
        {
            newbase_hdmi_set_factory_or_power_only_mode_enable((val>=0) ? val : 0);
        }
    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "debug_print")==0)
    {
        ...
        // AMD_FLOW_PRINT_FLAG
        //---------------------------------------------------
        n = scnprintf(ptr, count, "AMD_FREESYNC_FLOW=%d\n", hdmi_amd_vsdb_debug_en);
        ptr+=n; count-=n;
        n = scnprintf(ptr, count, "AMD_FREESYNC_FLOW_MSG_THRESHOLD=%d (ms)\n", hdmi_amd_vsdb_debug_threshold);
        ptr+=n; count-=n;
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "debug_print")==0)
    {
        ...
        else if (sscanf(buf, "AMD_FREESYNC_FLOW=%1d", &val)==1)
        {
            hdmi_amd_vsdb_debug_en = (val) ? 1 : 0;
        }
        else if (sscanf(buf, "AMD_FREESYNC_FLOW_MSG_THRESHOLD=%5d", &val)==1)
        {
            if (val<1)
                return -1;

            hdmi_amd_vsdb_debug_threshold = (unsigned int) val ;
        }
    ...
    )
{
    ...
    {//show raw table
        ...
        for(i = 0; i<256; i++)
        {
            ...
            n = scnprintf(ptr, count, "0x%2X, ", read_edid[i]);
            ptr+=n; count-=n;
        }
        n = scnprintf(ptr, count, "\n");
        ptr+=n; count-=n;
    ...
    {//show video data block
        ...
        for (i=0; i< info.n_cdb; i++)
        {
            ...
            if(info.p_cdb[i]->tag== CEA_861_TAG_2_VIDEO_DATA_BLOCK)
            {
                ...
                {
                    n = scnprintf(ptr, count, "----------Video Data Block------------(length=%d)\n", p_vdb->length);
                    ptr+=n; count-=n;
                    ...
                    for (i=0; i<p_vdb->length; i++)
                    {
                        ...
                        if (p_vic_timing)
                        {
                            n = scnprintf(ptr, count, "[%2d] SVD=0x%2x, VIC=%3d (hactive=%4d, vactive=%4d, vfreq=%4d)\n", i, svd, vic, p_vic_timing->hActive, p_vic_timing->vActive, p_vic_timing->vFreq);
                            ptr+=n; count-=n;
                        }
                        else
                        {
    ...
    ptr+=n; count-=n;
    n = scnprintf(ptr, count, "ADD_VIC=%d (0~255, Add VIC to EDID VDB)\n", current_add_vic_on_vdb);
    ptr+=n; count-=n;
    n = scnprintf(ptr, count, "DEL_VIC=x (x= 0~255,  VIC: erase specified VIC, 255: erase all to 0, 254: erase last one)\n");
    ptr+=n; count-=n;
    ...
    )
{
    ...
    n = scnprintf(ptr, count, "---------------- DRM INFOFRAME (newbase)--------------\n");
    ptr+=n; count-=n;
...
static DEVICE_ATTR(drm_0, 0444, hdmi_drm_info_show, NULL);
static DEVICE_ATTR(drm_1, 0444, hdmi_drm_info_show, NULL);
static DEVICE_ATTR(drm_2, 0444, hdmi_drm_info_show, NULL);
static DEVICE_ATTR(drm_3, 0444, hdmi_drm_info_show, NULL);
    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "flow_cfg")==0)
    {
        ...
        //Decide display page
        //page_num = (HDMI_FLOW_CFG_GROUP_NUM>0 ? ((HDMI_FLOW_CFG_GROUP_NUM -1)/4) : 0);
        //if(m_flow_cfg_page>page_num)
        //{
        //    m_flow_cfg_page = page_num;
        //}
