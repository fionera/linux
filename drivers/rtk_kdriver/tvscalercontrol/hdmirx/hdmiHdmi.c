...
void newbase_hdmi_init(void)
{
	...
	if (irq_init==0)
	{
		...
		{
			HDMI_EMG("!!!!!!!!!!!!!!!!! Register HDMI Interrupt handler failed - IRQ %d !!!!!!!!!!!!!!!!\n", IRQ_HDMI);
		}
		else
		{
			HDMI_EMG("!!!!!!!!!!!!!!!!! Register HDMI Interrupt handler successed - IRQ %d !!!!!!!!!!!!!!!!\n", IRQ_HDMI);
			irq_init=1;
		}
	}
}
...
{//TRUE, timing changed; FALSE, keep the same timing 
	...
	is_vrr = newbase_hdmi_get_vrr_enable(port);
	...
	switch (GET_H_VIDEO_FSM(port)) {
		...
		case MAIN_FSM_HDMI_DISPLAY_ON:
			if (newbase_hdmi_display_on(port, p_phy_st->frl_mode)) {
				...
				else if(get_HDMI_Dolby_VSIF_mode() != DOLBY_HDMI_VSIF_DISABLE)
...
unsigned char newbase_hdmi_measure(unsigned char port, unsigned char frl_mode)
{
	...
	HDMI_PRINTF("FSM_HDMI_MEASURE[12.01][%d][frl_mode:%d][hdmi21en:%d][retry_cnt:%d]\n", port, frl_mode, hdmi_rx[port].hdmi_2p1_en, hdmi_rx[port].meas_retry_cnt);
	...
	newbase_hdmi_vic_correction(port,  &hdmi_rx[port].timing_t, GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_FORCE_VIC_MEASURE));
...
unsigned char newbase_hdmi_get_is_interlace_change(unsigned char port, unsigned char frl_mode)
{
	...
	{//Direct check
		if(frl_mode == MODE_TMDS)
		{
			...
			get_ip_result = newbase_hdmi_get_is_interlace_reg(port, frl_mode, HDMI_MS_CONTINUOUS);
			if(HDMI_IPCHECK_VSYNC_NO_CHANGED == get_ip_result)
			{// v sync change
				result = FALSE;
			}
			else if (GET_H_INTERLACE(port) != get_ip_result)
			{
				result =  TRUE;
			}
			//result = lib_hdmi_get_int_pro_chg_flag(port);
		}
		else
		{
			if (GET_H_INTERLACE(port) != newbase_hdmi_get_is_interlace_reg(port, frl_mode, HDMI_MS_ONESHOT))
			{
				result =  TRUE;
			}
		}
...
unsigned char newbase_hdmi_get_hpd(unsigned char nport)
{
    ...
    if (p_info->dedicate_hpd_det)  // usd hdmi hpd/det io pads
        return lib_hdmi_get_hdmi_hpd(p_info->dedicate_hpd_det_sel);
...
void newbase_hdmi_error_handler(unsigned char port)
{
    ...
    else if (lib_hdmi_ps_measure_complete(port))
    {
        ...
        if (vs2vs > MAX_VS_PERIODIC_CNT || vs2vs < MIN_VS_PERIODIC_CNT)
        {
            ...
            {
                HDMI_WARN("HDMI[p%d] VSync Unlocked = %d (should within %d - %d)\n",
                    port, vs2vs, MAX_VS_PERIODIC_CNT, MIN_VS_PERIODIC_CNT);
            }
        ...
        else
        {
           ...
           // if (hdmi_rx[port].video_t.vs_valid < 0xFF)
            ...
            {
                HDMI_WARN("HDMI[p%d] VSync Freq Changed = %d -> %d (valid_cnt=%d, last_vs_locked=%d, last_stable_cnt=%d))\n",
            ...
            if (vs_pol==hdmi_rx[port].video_t.vs_pol)
            {
                if (hdmi_rx[port].video_t.vs_pol_stable==0) {
                    HDMI_WARN("HDMI[p%d] VSync Polarity = %d\n", port, hdmi_rx[port].video_t.vs_pol);
                }
            ...
            else
            {
                ...
                {
                    HDMI_WARN("HDMI[p%d] VSync Polarity changed = %d -> %d (vs_pol_stable=%d -> 0)\n", 
    ...
    {// v sync vaild and update!!
        ...
        *--------------------------------------------------------------*/
        if((newbase_rxphy_get_frl_mode(port) != MODE_TMDS) && (GET_H_VIDEO_FSM(port) >= MAIN_FSM_HDMI_DISPLAY_ON))
        {
    ...
    if (time_after(jiffies, hdmi_rx[port].err_msg_display_timeout))
    {
        ...
        HDCP_INFO("HDMI[p%d] RxStatus(%02x, %02x), hdcp_enc=%d, bch 2bit error (detect,duration,cnt)=(%d, %d, %d), 1bit=%d\n", 
            port, RxStatus[0], RxStatus[1],
    ...
    if (hdmi_rx[port].no_pkt_continue > 30 && hdmi_rx[port].video_t.vs_valid<5) // no any of info packet received and vsync not stable
    {
        unsigned int no_pkt_and_vsync_unstable_timeout = GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_NO_PKT_AND_VSYNC_UNSTABLE_HDP_TOG_THD);
        HDMI_CHANNEL_T* p_info = newbase_hdmi_get_pcbinfo(port);

        if (p_info->arc_support && hdmi_arc_is_earc_enabled())
            no_pkt_and_vsync_unstable_timeout = 0; // do not toggle hpd if the HPD is used by eARC

        HDMI_WARN("HDMI[p%d] no any packet and vsync unstable !!! Tmds_config=%d, scramble_status=%d, tmds_scramble_off_sync=%d, vs_locked=%d, vs_valid=%d, pkt_cnt=%d, no_pkt_cont=%d, no_pkt_and_vsync_unstable=%d/%d\n",
            port,
        ...
        if (hdmi_rx[port].hdmi2p0_tmds_scramble_off_sync > 30) // if tmes off sync (non scarmbled)
        {
            HDMI_WARN("HDMI[p%d] data abnormal with tmds scramble off sync !!!! reset TMDS config to recover\n", port);
        ...
        if (no_pkt_and_vsync_unstable_timeout &&
            hdmi_rx[port].no_pkt_and_vsync_unstable_continue++ > no_pkt_and_vsync_unstable_timeout)
        {
            HDMI_WARN("HDMI[p%d] no packet and vsync unstable timeout (%d) !!!! do toggle HPD\n",
                port, hdmi_rx[port].no_pkt_and_vsync_unstable_continue);
    ...
    {
        HDMI_WARN("HDMI[p%d] data off sync timeout detected, source may not support Reauth, do HPD toggle to recover\n", port);
        hpd_toggle = 1;
    }
    ...
    if (hpd_toggle)
    {
        if (newbase_hdmi_get_hpd(port))
        {
            ...
            HDMI_WARN("HDMI[p%d] un recoverable error detected, do HPD toggle to recover (round=%d)\n", 
...
//			return _FALSE;
	...
	newbase_hdmi_get_fvs_allm(port, &is_allm);
	HDMI_INFO("Port=%d, VRR_EN=%d, ALLM=%d, Dolby mode=%d, VIC=%d(avi_state:%d)\n",
		port, newbase_hdmi_get_vrr_enable(port), is_allm, get_HDMI_Dolby_VSIF_mode(), avi_info.VIC, avi_state);
