...
static unsigned char HDMI_EDID_FORCE_VDB_VIC = 0;     // 4 Lane 6Gbps

#define HDMI_EDID_FORCE_AMD_FREE_SYNC_ENABLE     (0x1<<22)
#define HDMI_EDID_FORCE_AMD_FREE_SYNC_MIN_FREQ   40
...
static int _check_video_data_block(CEA_DATA_BLOCK *p_vdb, unsigned long flags)
{
    ...
    else if ((flags & HDMI_EDID_FORCE_ADD_VIC_TIMING)|| (flags & HDMI_EDID_FORCE_DEL_VIC_TIMING))
    {
        unsigned char svd_list[HDMI_EDID_VDB_MAX_LEN];
        int i;
        unsigned char is_updated = FALSE;

        memset(svd_list, 0, HDMI_EDID_VDB_MAX_LEN);
        ...
        if(flags & HDMI_EDID_FORCE_ADD_VIC_TIMING)
        {
            for (i=0; i<p_vdb->length; i++)
            {
                ...
                if (i < (p_vdb->length-1) )
                {
                    svd_list[i+1] = svd; // add to next on. 
                }
                else
                {
                    HDMI_EDID_INFO("check_video_data_block, i=%2d, svd=%2d (vic:%3d) Remove last one!\n", i, svd, vic);
                }
            }
 

            svd_list[0] = _vic_to_svd(HDMI_EDID_FORCE_VDB_VIC);
            is_updated = TRUE;
            HDMI_EDID_INFO("ADD_VIC=%d  success, total length =%d\n", HDMI_EDID_FORCE_VDB_VIC, p_vdb->length);
        ...
        else if(flags & HDMI_EDID_FORCE_DEL_VIC_TIMING)
        {
            for (i=0; i<p_vdb->length; i++)
            {
                ...
                else
                {
                    ...
                    if(HDMI_EDID_FORCE_VDB_VIC == 0xFF)
                    {//erase all
                        svd_list[i] = 0;
                        is_updated = TRUE;
                        HDMI_EDID_INFO("Erase index %d to 0\n", i);
                    }
                    else if(HDMI_EDID_FORCE_VDB_VIC == 0xFE)
                    {// Delete last index
                        if(i == (p_vdb->length-1))
                        {
                            int delete_index = i;
                            ...
                            {
                                if(svd_list[delete_index] != 0)
                                {
                                    svd_list[delete_index] = 0;
                                    delete_index = 0;
                                    break;
                                }
                                else
                    ...
                    else if(vic != HDMI_EDID_FORCE_VDB_VIC)
                    {
                        svd_list[i] = svd;
                    }
                    else
                    {
                        ...
                        HDMI_EDID_INFO("VDB Index=%d, Delete VIC=%d \n", i, HDMI_EDID_FORCE_VDB_VIC);
                    }
        ...
        {
            HDMI_EDID_INFO("_check_video_data_block, flag=%lx, svd changed, Original length=%d\n", flags, p_vdb->length);
            memcpy(p_vdb->data, svd_list, p_vdb->length);
            return 1;
        }
    }
...
int _hdmi_check_hdmi_edid(unsigned char port, unsigned char edid[128],  unsigned long check_flags)
{
    ...
    for (i=0; i< info.n_cdb; i++)
    {
        ...
        case CEA_861_TAG_3_VENDOR_SPECIFIC_BLOCK:
            if (_check_vsdb(port, info.p_cdb[i], check_flags, &new_vsdb)) {  // for all VSDB, including HDMI VSDB/HF VSDB ...
                info.p_cdb[i] = &new_vsdb;  // modified, use new vsdb to instead
                edid_modified++;
            }
            break;
    ...
    if ((check_flags & HDMI_EDID_FORCE_AMD_FREE_SYNC_ENABLE))
    {
        ...
        if (cdb_id>=0)
        {
            ...
            edid_modified++;
            HDMI_EDID_EMG("[_hdmi_check_hdmi_edid port: %d] insert AMD VSDB success - block id=%d\n", port, cdb_id);
        }
        else
        {
...
HDMI_EDID_CAPABILITY newbase_hdmi_check_edid_capability(unsigned char* raw_edid, unsigned int len)
{
    ...
    for (i=0; i< info.n_cdb; i++)
    {
        ...
        case CEA_861_EXT_TAG_7_EXTENED_BLOCK:
            if (info.p_cdb[i]->length >= 3)
            {
...
unsigned char newbase_hdmi_write_edid(unsigned char port, unsigned char *edid, unsigned int len)
{
    ...
    else
    {
        ...
        HDMI_EDID_EMG("Write EDID port=%d, result=%d, Checksum=(%02x,%02x),len=%d, ddc_ch=%d, capability=%x\n",
            port, 
            result,
            edid[127],
...
void newbase_hdmi_edid_init(void)
{
    ...
    for (j=0; j<HDMI_PORT_TOTAL_NUM; j++)
    {
        ...
        {
            HDMI_EDID_EMG("%s: init port %d failed - port not used, skip it\n", __func__, j);
            continue;
        }
        p_edid_table = m_edid_status[j].current_edid_table;

        HDMI_EDID_EMG("edid_init EDID, Checksum=(%02x,%02x),Port=%d\n",
            p_edid_table[127], p_edid_table[255], j);
