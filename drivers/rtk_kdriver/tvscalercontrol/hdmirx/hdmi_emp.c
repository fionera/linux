...
int lib_hdmi_set_em_vsem_type(
    unsigned char           nport,
    unsigned char           oui[3],
    unsigned short          data_set_tag,
    unsigned char           sync
    )
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
        hdmi_out(HD21_HDMI_VSEM_EMC_reg,  HDMI21_P0_HD21_HDMI_VSEM_EMC_set_tag_msb((data_set_tag >>8) & 0xFF) |
                                          HDMI21_P0_HD21_HDMI_VSEM_EMC_set_tag_lsb((data_set_tag & 0xFF))|
                                          HDMI21_P0_HD21_HDMI_VSEM_EMC_recognize_tag_en(1));

        hdmi_out(HD21_HDMI_VSEM_EMC2_reg, HDMI21_P0_HD21_HDMI_VSEM_EMC2_sync((sync) ? 1 : 0) |
                                          HDMI21_P0_HD21_HDMI_VSEM_EMC2_recognize_oui_en(1) |
                                          HDMI21_P0_HD21_HDMI_VSEM_EMC2_oui_1st(oui[0])|
                                          HDMI21_P0_HD21_HDMI_VSEM_EMC2_oui_2nd(oui[1])|
                                          HDMI21_P0_HD21_HDMI_VSEM_EMC2_oui_3rd(oui[2]));
    }
    else
    {
        hdmi_out(HDMI_HDMI_VSEM_EMC_reg,  HDMI_HDMI_VSEM_EMC_set_tag_msb((data_set_tag >>8) & 0xFF) |
                                          HDMI_HDMI_VSEM_EMC_set_tag_lsb((data_set_tag & 0xFF))|
                                          HDMI_HDMI_VSEM_EMC_recognize_tag_en(1));

        hdmi_out(HDMI_HDMI_VSEM_EMC2_reg, HDMI_HDMI_VSEM_EMC2_sync((sync) ? 1 : 0) |
                                          HDMI_HDMI_VSEM_EMC2_recognize_oui_en(1) |
                                          HDMI_HDMI_VSEM_EMC2_oui_1st(oui[0])|
                                          HDMI_HDMI_VSEM_EMC2_oui_2nd(oui[1])|
                                          HDMI_HDMI_VSEM_EMC2_oui_3rd(oui[2]));
    }
...
{
    if (hdmi_rx[nport].hdmi_2p1_en)
        return HDMI21_P0_HD21_VSEM_ST_get_em_vsem_no(hdmi_in(HD21_VSEM_ST_reg)) ? 0 : 1;

    return HDMI_VSEM_ST_get_em_vsem_no(hdmi_in(HDMI_VSEM_ST_reg)) ? 0 : 1;
}
...
int lib_hdmi_is_em_hdr_received(unsigned char nport)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
        if (hdmi_in(HD21_HDR_EM_ST_reg)==0)
            return 0;

        if (hdmi_in(HD21_HDR_EM_ST_reg) == 0x80)  // No EMP
        {
            hdmi_out(HD21_HDR_EM_ST_reg, 0x80);
            return 0;
        }
        return 1;
    }
    ...
    {
        if (hdmi_in(HDMI_HDRST_reg)==0)
            return 0;

        if (hdmi_in(HDMI_HDRST_reg) == 0x80)  // No EMP
        {
            hdmi_out(HDMI_HDRST_reg, 0x80);
            return 0;
        }
        return 1;
    }
    ...
    )
{
    ...
    for (i=0; i<len; i+= 32)
    {
        ...
        if (p_emp[3]!=0)        // check emp BCH error
            HDMI_WARN("HDR_CVTEM BCH Error detected (%p)= %02x %02x %02x %02x\n",
                     p_emp, p_emp[0], p_emp[1], p_emp[2], p_emp[3]);
...
void newbase_hdmi_update_emp(unsigned char port)
{
    ...
    lib_hdmi_emp_enable_all_mode(port, 1);   // force use all mode to receive EMP
...
{
    unsigned char pkt_buf[EM_VTEM_INFO_LEN] = {0};
    if(newbase_hdmi_get_vtem_emp(port, pkt_buf)>0)
    {
        p_emp_vtem_t->FVA_Factor_M1 = (hdmi_rx[port].vtem_emp[10] & (_BIT7 | _BIT6 | _BIT5 | _BIT4))>>4;
        p_emp_vtem_t->M_CONST = (hdmi_rx[port].vtem_emp[10] & (_BIT1))>>1;
        p_emp_vtem_t->VRR_EN = (hdmi_rx[port].vtem_emp[10] & (_BIT0));
        p_emp_vtem_t->Base_Vfront = hdmi_rx[port].vtem_emp[11];
        p_emp_vtem_t->RB = (hdmi_rx[port].vtem_emp[12]& (_BIT2))>>2;
        p_emp_vtem_t->Base_Refresh_Rate = ((hdmi_rx[port].vtem_emp[12]& (_BIT1 | _BIT0))<<8) | hdmi_rx[port].vtem_emp[13];
        return TRUE;
    }
    else
    {
