...
#ifdef CONFIG_HDMI_HDCP2_DISABLE
unsigned int hdcp2_disable = 1;
#else
unsigned int hdcp2_disable = 0;
#endif
...
void lib_hdmi_hdcp_init(unsigned char nport)
{
    ...
    memset(hdmi_port_hdcp2_disable, hdcp2_disable, sizeof(hdmi_port_hdcp2_disable));
    lib_hdmi_hdcp_port_write(nport, 0x50, (hdcp2_disable) ? 0x00: 0x04);    //support hdcp2.2
...
#if defined(CONFIG_RTK_KDRV_OTP)
    unsigned char hdcp14_buf[325];
    if (get_hdcp_14_otp(hdcp14_buf, 325) == 0)
    {
        HDCP_INFO("load HDCP1.4 OTP-KEY  OK\n");
        memcpy(hdcpkey.BKsv , hdcp14_buf,5);
        memcpy(hdcpkey.Key , hdcp14_buf+5, 320);
    } else {
        HDCP_WARN("load HDCP1.4 OTP-KEY failed, no otp key present\n");
    }
#else
    HDCP_WARN("NO HDCP1.4 OTP-KEY (UNDEFINED)\n");
#endif
    ...
    else 
    {
        ...
        {//Print reauth status
            ...
            if ((RxStatus[1] & 0x8))
            {
                ...
                {
                    HDCP_INFO("HDMI[p%d] HDCP2 RxReauth is set Status(%02x, %02x) BCHErr(%d,%d), Enc=%d, AVmute=%d, Cnt=%d\n",
            ...
            else
            {
                ...
                {
                    HDCP_INFO("HDMI[p%d] HDCP2 RxReauth is unset Status(%02x, %02x) BitErr(%d,%d), Enc=%d, AVmute=%d, Cnt=%d\n",
                        port, RxStatus[0], RxStatus[1],
                        (hdmi_rx[port].bch_err_detect) ? 1 : 0,
                        (hdmi_rx[port].bch_err_cnt_continue),
                        (p_hdcp->hdcp_enc) ? 1 : 0,
...
void newbase_hdmi_hdcp_disable_hdcp2(unsigned char port, unsigned char disable)
{
  ...
  #ifdef CONFIG_HDMI_HDCP2_DISABLE
        if (disable==0) {
            HDCP_WARN("force disable hdcp2 due to CONFIG_HDMI_HDCP2_DISABLE is set\n");
            disable = 1;   // force disable HDCP2
        }
  #endif
  
        hdcp2_disable = disable;
...
{      
    if (port==EVERY_HDMI_PORT)    
        return hdcp2_disable;  // global hdcp2 disable flag
        
    if (newbase_hdmi_phy_port_status(port)==HDMI_PHY_PORT_NOTUSED)
        return 0;
    
    return (hdcp2_disable || hdmi_port_hdcp2_disable[port]) ? 1 : 0;
}
