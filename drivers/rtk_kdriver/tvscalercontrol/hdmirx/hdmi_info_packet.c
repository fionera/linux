...
static INFO_PACKET_CONFIG info_packet_config_array[INFO_TYPE_NUM] =
{
    ...
    {INFO_TYPE_EMP, TRUE, TYPE_CODE_EMP_PACKET, MAIN_FSM_HDMI_MEASURE},
    ...
    {INFO_TYPE_RSV2, TRUE, TYPE_CODE_VS_PACKET, MAIN_FSM_HDMI_MEASURE},                      // For HDR10+ VSIF Packet
...
{
    if (hdmi_rx[nport].hdmi_2p1_en)
        return HD21_HDMI_GPVS_get_hdr10pvsps_v(hdmi_in(HD21_HDMI_GPVS_reg));
    else
        return HDMI_HDMI_GPVS_get_hdr10pvsps_v(hdmi_in(HDMI_HDMI_GPVS_reg));
}
...
{
    if (FALSE == newbase_hdmi_check_infoframe_config(port, INFO_TYPE_RSV2))
        return 0;
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#if 0//#ifdef CONFIG_SUPPORT_DOLBY_VSIF
unsigned char newbase_hdmi_update_vsi(unsigned char port)
{
    unsigned char sta_pos, len, pkt_buf[RSV_LEN]={0}, len_extend;
    unsigned char Is_DolbyVSIF = _FALSE;
    unsigned char Is_VSIF_14 = FALSE;//check is vsif 1.4 or not
    unsigned char Check_PB6_PB24_result = TRUE; //True: pb6-pb24 all 0. False: not all 0
    unsigned char check_dolby_h14b_result = FALSE; //True: h1.4b dolby format  FALSE: not dolby format
    int idx_PB; // HDMI 1.4b VSIF PB[] index
    
    lib_hdmi_set_rsv_packet_type(port, 2, TYPE_CODE_VS_PACKET);        // use reserved2 to receive Dolby VSIF
    
    if (lib_hdmi_is_fvs_received(port)) 
    {
        lib_hdmi_clear_fvs_received_status(port);
        sta_pos = FVS_INFO_OFST +1;
        len = FVS_INFO_LEN - 1;
    } 
    else if (lib_hdmi_is_vsi_received(port))
    {
        lib_hdmi_clear_vsi_received_status(port);
        sta_pos = VS_INFO_OFST + 1;
        len = VS_INFO_LEN - 1;
        Is_VSIF_14 = TRUE;
    }
    else if(lib_hdmi_is_rsv_packet2_received(port))
    {
        //lib_hdmi_clear_rsv_packet2_status(port); //it will clear later
        Is_DolbyVSIF = _TRUE;
        len = RSV_LEN -1;
    }
    else
    ...
    if(_TRUE == Is_DolbyVSIF)
    {
        if (lib_hdmi_read_rsv2_packet(port, pkt_buf, len)<0)
        {
            return FALSE;
        }

        // DolbyVSIF 24-bit IEEE OUI (=0x00D046)
        if((0x00 == pkt_buf[6]) && (0xD0 == pkt_buf[5]) && (0x46 == pkt_buf[4]))
        {
            hdmi_rx[port].vsi_t.VSIF_TypeCode=TYPE_CODE_VS_PACKET;

            len = pkt_buf[2] + 3;
            if(len > 31)
                len = 31;
            memcpy(&hdmi_rx[port].vsi_t.VSIF_Version, &pkt_buf[1], len); //byte 0 is BCH                    
            if(0x2 == pkt_buf[7])
                {
                set_HDMI_Dolby_VSIF_mode(DOLBY_HDMI_VSIF_STD);//10
                }
            else if(0x3 == pkt_buf[7])
                {
                //if (lib_hdmi_get_fw_debug_bit(DEBUG_16_PRINT_DRM))
                set_HDMI_Dolby_VSIF_mode(DOLBY_HDMI_VSIF_LL);//11
                }
            else
                {
                //if (lib_hdmi_get_fw_debug_bit(DEBUG_16_PRINT_DRM))
                set_HDMI_Dolby_VSIF_mode(DOLBY_HDMI_VSIF_DISABLE);//00 or 01
                }

            if(hdmi_get_infoframe_thread_stop()==_FALSE)
    ...
    lib_hdmi_clear_rsv_packet2_status(port);

    lib_hdmi_read_packet_sram(port, sta_pos, len, pkt_buf);

    //=====willychou 20170712 modify Keep checksume information=======

    //pkt_buf[0];   =>version
    //pkt_buf[1];   =>len
    //pkt_buf[2];   =>checksum

    if((VS_INFO_OFST + 1) == sta_pos)
    {
        if(pkt_buf[1] > 15)//len > 15
        {
            len_extend = pkt_buf[1] - 15; //len_extend = len -15

            if(len_extend >= VS_INFO_CNT_LEN)
                lib_hdmi_read_packet_sram(port, VS_INFO_CNT_OFST, VS_INFO_CNT_LEN, &pkt_buf[18]);
            else// if(len_extend < VSI_PKT_CONT_LEN)
                lib_hdmi_read_packet_sram(port, VS_INFO_CNT_OFST, len_extend, &pkt_buf[18]);

            len = pkt_buf[1] + 3;
        }
    }
    hdmi_rx[port].vsi_t.VSIF_TypeCode = TYPE_CODE_VS_PACKET;
    memcpy(&hdmi_rx[port].vsi_t.VSIF_Version, pkt_buf, len); //use len for memcpy because copy size will be 11 or 18

    if (Is_VSIF_14)
    {
        if(pkt_buf[1] == 24)//lenth is 0x18
        {
              // HDMI1.4b VSIF 24-bit IEEE OUI (=0x000C03)
             if((0x00 == pkt_buf[5]) && (0x0C == pkt_buf[4]) && (0x03 == pkt_buf[3]))
             {
                 for(idx_PB=8 ; idx_PB < 27 ; idx_PB++ )//PB6-PB24
                 {
                     if(pkt_buf[idx_PB]!=0)
                     {
                         Check_PB6_PB24_result = FALSE;
                         break;
                     }
                 }

                 if(Check_PB6_PB24_result)
                 {
                    check_dolby_h14b_result = TRUE;
                 }
             }
        }

        if(check_dolby_h14b_result)
        {//h14b dolby format
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#else //#ifdef CONFIG_SUPPORT_DOLBY_VSIF
unsigned char newbase_hdmi_update_vsi(unsigned char port)
{
