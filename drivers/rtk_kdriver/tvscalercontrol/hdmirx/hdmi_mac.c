...
unsigned int hdmi_ps_mode = HDMI_PS_MODE_AUTO;  // default ps mode
...
#if 0
//---FIXME need to refine code about bring up-----start------------
...
//---FIXME need to refine code about bring up-----end------------
#endif
...
//    lib_hdmi_mac_afifo_enable(nport,HDMI_4LANE);
    ...
    //hdmi physical port2 or port3 need to B/R channel swap    
    g_platform = get_platform();

    if (g_platform == PLATFORM_KXLP)
    {
        if(nport==HDMI_PORT2 || nport==HDMI_PORT3)
            hdmi_mask(HDMI_TMDS_PWDCTL_reg, ~(HDMI_TMDS_PWDCTL_brcw_mask),HDMI_TMDS_PWDCTL_brcw_mask);
    }

    if(g_platform == PLATFORM_KXL)
    {
        if(nport==HDMI_PORT2)
            hdmi_mask(HDMI_TMDS_PWDCTL_reg, ~(HDMI_TMDS_PWDCTL_brcw_mask),HDMI_TMDS_PWDCTL_brcw_mask);
    }
