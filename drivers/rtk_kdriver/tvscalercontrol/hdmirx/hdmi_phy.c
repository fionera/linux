...
static int __init setup_power_only(char *str)
{
    ...
    SET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_MEASURE_ERR_CNT_THD, 1);
    SET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_PHY_STABLE_CNT_THD, 60);
	return 0;
}
...
unsigned char newbase_rxphy_is_clock_stable(unsigned char port)
{
	...
	{// >3G, should wait more time for clock stable.
		...
		// do not increase stable count when scanble is enabled
		// many graphic cards excepts to get SCDC:ScambleStatus=1
		// within specific region. if we wait too long, the source
		// mighe stop sending video.
		if (hdmi_rx[port].scramble_flag==0)   
			phy_stable_cnt +=4;
	}
