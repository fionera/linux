...
static unsigned int resume_hpd_low_start = 0;
static unsigned int resume_hpd_low_end = 0;

int vfe_hdmi_v4l2_set_hpd_flag_port(unsigned char ch, unsigned char state)
{
	unsigned char hdmi_port;
	unsigned long hpd_delay = 0;
	int ret = -1;

	HDMI_INFO("[%s %d] starts, ch=%d, state=%d\n", __func__, __LINE__, ch, state);

	if ((ch>=sizeof(g_hdmi_hpd_flag)) || hdmi_vfe_get_hdmi_port(ch, &hdmi_port)<0)
	{
		HDMI_EMG("[vfe_hdmi_v4l2_set_hpd_flag_port][- ARG ERR] get hdmi port for ch = %d failed\n", ch);
		return -1;
	}

	switch(state)
	{
		case 0: //V4L2_EXT_HDMI_HPD_DISABLE
			lib_hdmi_z0_set(hdmi_port, LN_ALL, 0);   // Q10235 PS4 toggle HPD only, it causes noise
			newbase_hdmi_set_hpd(hdmi_port, 0);
			ret = 0;
			break;
		case 1:
			HDMI_INFO("enable ch%d HPD resume Guard time now=%ld (%d ~%d) ms\n", ch, jiffies, resume_hpd_low_start, resume_hpd_low_end);

			if (newbase_hdmi_get_hpd(hdmi_port))
			{
				HDMI_INFO("enable ch%d HPD, hpd has been initialzed already\n", ch);
				ret = 0;
				break;
			}

			if (jiffies >= resume_hpd_low_start && jiffies < resume_hpd_low_end) {
				hpd_delay = jiffies_to_msecs(resume_hpd_low_end - jiffies);  // to prevent Webos to setup HPD by this function
				HDMI_INFO("enable ch%d HPD while resume hpd low guard time, do delay = %lu ms\n", ch, hpd_delay);		        
			}
			else {
				resume_hpd_low_start = 0;
				resume_hpd_low_end = 0;
			}

			newbase_hdmi_enable_hpd_with_delay(hpd_delay, hdmi_port);
			ret = 0;
			break;
		case 2:  // restart
			ret =vfe_hdmi_drv_restart_hpd(ch);
			break;
		default:
			HDMI_EMG("[vfe_hdmi_v4l2_set_hpd_flag_port][- ARG ERR] ch=%d, state = %d \n", ch, state);
			ret =-1;
			break;
	}

	if(ret ==0)
		g_hdmi_hpd_flag[ch] = state;
	return ret;
}
int vfe_hdmi_v4l2_get_hpd_flag_port(unsigned char ch,unsigned char* state)
{
    unsigned char hdmi_port;

    HDMI_EDID_INFO("[%s %d] starts, ch=%d\n", __func__, __LINE__, ch);
    if (ch >= sizeof(g_hdmi_hpd_flag) || hdmi_vfe_get_hdmi_port(ch, &hdmi_port)<0) {
        HDMI_EDID_EMG("[vfe_hdmi_v4l2_get_hpd_flag_port][- ARG ERR] get hdmi port for ch = %d failed\n", ch);
        return -1;
    }
    *state= g_hdmi_hpd_flag[ch];
	return 0;
}
...
int vfe_hdmi_drv_init(void)
{
	...
	if(GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_V4L2_ENABLE) != HDMI_PCB_FORCE_ENABLE)
	{
		...
		if (1 == lib_hdmi_cec_is_enabled())
		{
			...
			newbase_hdmi_enable_hpd(EVERY_HDMI_PORT); //need to separate AC/DC case, move to resume and init function
		}
		else
		{
			resume_hpd_control = 0;
...
int vfe_hdmi_drv_get_port_spd_info(vfe_hdmi_spd_t *info_frame)
{
	...
	if (newbase_hdmi_get_spd_infoframe(port, &spd_info) && spd_info.ver==1 && spd_info.len==25)  // only report SPD info that followed CTA-861-G standard
	{
...
int vfe_hdmi_drv_get_port_avi_info(vfe_hdmi_avi_t *info_frame)
{
	...
	if (newbase_hdmi_get_avi_infoframe(port, &avi_info))
	{
		...
		info_frame->f14= (vfe_hdmi_avi_additional_colorimetry_e) avi_info.F14;
	...
	)
{
	...
	for (i=0; i<cnt; i++) {
		...
		memcpy(&p_vsi[i], &vsi_info[i], sizeof(HDMI_VSI_T));
	}
...
int vfe_hdmi_drv_enable_hpd(void)
{
	...
	if (resume_hpd_control) {
		...
		if(1 == lib_hdmi_cec_is_enabled())
		{
			//newbase_hdmi_enable_hpd(EVERY_HDMI_PORT); //KTASKWBS-8520 ,enable_hpd will extend hdp low duration when DC on
			return HDMI_DRV_NO_ERR;
		}
#else
		//newbase_hdmi_enable_hpd(EVERY_HDMI_PORT);//KTASKWBS-8520 ,enable_hpd will extend hdp low duration when DC on
	...
	)
{
	...
	if (hdmi_vfe_get_hdmi_port(ch, &port)<0)
	{
		HDMI_EDID_EMG("[WARNING] %s ch%d convert Physical HDMI port fail\n" , __func__ , ch);
		return HDMI_DRV_ARG_ERR;
	}
	...
	)
{
	...
	if (hdmi_vfe_get_hdmi_port(ch, &port)<0)
	{
		HDMI_EDID_EMG("[vfe_hdmi_drv_write_edid]write edid of HDMI-%d failed, get HDMI port failed\n", ch);
		return HDMI_DRV_ARG_ERR;
	}
...
char vfe_hdmi_drv_get_arc_channel(void)
{
    ...
    arc_channel = get_arc_channel();

    if((arc_channel >= 0) && (arc_channel < HDMI_RX_PORT_MAX_NUM))
    {
        if(hdmi_vfe_get_hdmi_channel(arc_channel, &arc_ui_channel) >= 0)
...
int vfe_hdmi_drv_get_HDCPStatus(unsigned char ch, unsigned int* hdcp_status)
{
    ...
    if (hdmi_vfe_get_hdmi_port(ch, &port)<0)
    {
        HDMI_EMG("[WARNING] convert Physical HDMI port fail\n");
        return HDMI_DRV_ARG_ERR;
    }
...
int vfe_hdmi_drv_set_hpd(vfe_hdmi_hpd_t* hpd_value)
{
	...
	{
		HDMI_EDID_EMG("[WARNING] convert Physical HDMI port fail\n");
		return HDMI_DRV_ARG_ERR;
	}
...
int vfe_hdmi_drv_get_currentVRRVFrequency(unsigned char ch, unsigned short *vFreq)
{
	...
	*vFreq = 0;
	...
	{
#if 1   // If no VRR, still reply current v freq
		HDMI_TIMING_T tm;

		if (!newbase_hdmi_get_timing(port, &tm))
		{
			HDMI_PRINTF("get timig info failed, invalid port -%d reply zero vferq\n", port);
			*vFreq = 0;
		}
		else
		{
...
void vfe_hdmi_drv_suspend(void)
{
	...
	if (g_hdmi_drv_state == VFE_HDMI_DRV_SERVICE_STATE) 
	{
		...
		for (ch = 0; ch < HDMI_RX_PORT_MAX_NUM; ch++) 
		{
			...
			newbase_hdmi_release_source(ch);  // release resource
		}
...
void vfe_hdmi_drv_resume_init(void)
{
	unsigned int ch;
	unsigned char port;
	unsigned int hpd_compensation =0;
	unsigned int hdmi_resume_curtime = 0;
	HDMI_INFO("[%s %d] starts\n",__func__,__LINE__);
	...
	hdmi_resume_curtime = (hdmi_in(TIMER_TC7CVR_reg) / (TIMER_CLOCK / CONFIG_SYS_HZ));

	if(newbase_hdmi_get_hpd_low_delay() > (hdmi_resume_curtime - HPD_TOLERANCE))
		hpd_compensation = newbase_hdmi_get_hpd_low_delay() + HPD_TOLERANCE - hdmi_resume_curtime;
	else
		hpd_compensation = 0;

	resume_hpd_low_start = jiffies;
	resume_hpd_low_end = resume_hpd_low_start;

#if HDMI_STR_HPD_SPEED_UP_ENABLE
	// if CEC disable, newbase_hdmi_init_source() will enable hotplug
	if (lib_hdmi_cec_is_enabled()) {
		resume_hpd_control = 1;
		resume_hpd_low_end += msecs_to_jiffies(hpd_compensation);
		newbase_hdmi_enable_hpd_with_delay(hpd_compensation,EVERY_HDMI_PORT);  // use STR PHD delay time delay time
	}
#else //#if HDMI_STR_HPD_SPEED_UP_ENABLE
	resume_hpd_control = 1;
	resume_hpd_low_end += msecs_to_jiffies(hpd_compensation);
	newbase_hdmi_enable_hpd_with_delay(hpd_compensation,EVERY_HDMI_PORT);
#endif //#if HDMI_STR_HPD_SPEED_UP_ENABLE

	HDMI_INFO("[%s] hpd_compensation=%d ms (hdmi_resume_start=%u, hpd_guard_time=%d, HPD_TOLERANCE=%d, (resume_hpd_low_start=%d, resume_hpd_low_end=%d) \n",
		__func__, hpd_compensation, hdmi_resume_curtime, newbase_hdmi_get_hpd_low_delay(), HPD_TOLERANCE,
		resume_hpd_low_start, resume_hpd_low_end);
}
...
unsigned int vfe_hdmi_drv_get_state(void)
{
	return g_hdmi_drv_state;
}
#endif /*CONFIG_PM*/
