 ...
 *  D53:52  : HDMI_HPD_TYPE        (0: on chip, 1: with MUX, 2:Not used)
 *  D51:40  : Reserved
...
int hdmi_vfe_get_rx_port_info(unsigned char port, HDMI_CHANNEL_T_PCB_INFO* p_info)
{
    ...
    {
        HDMI_INFO("HDMI Port[%d] : Phy=%d, ddc_bus=%d, edid_type=%d, hpd=dedicate_hpd_%d, 5v_det=dedicate_det_%d, arc_support=%d\n",
    ...
    {
        HDMI_INFO("HDMI Port[%d] : Phy=%d, ddc_bus=%d, edid_type=%d, hpd=%s_%d, 5v_det=%s_%d, arc_support=%d\n",
            port, p_info->phy_port, p_info->ddc_selected, p_info->edid_type,
...
unsigned int newbase_hdmi_calculate_entry_max_value(HDMI_FLOW_CFG_GROUP group_id, unsigned char entry_id)
{
    ...
    {
        HDMI_EMG("[ERROR][%s] Wrong value , Read group_id %d fail!\n" , __func__ , group_id);
    }
    else
    {
        if(entry_id >= p_flow_cfg_group_table[group_id].cfg_table_length)
        {
            HDMI_EMG("[ERROR][%s] Wrong value , Read entry_id %d fail!\n" , __func__ , entry_id);	
        }
        else
        {
            ...
            if(bit_diff<0)
            {
                HDMI_EMG("[newbase_hdmi_calculate_entry_max_value] ERROR, invalid bit index !! start_bit_index = %d, end_bit_index = %d \n",  p_current_cfg_table[entry_id].start_bit_index, p_current_cfg_table[entry_id].end_bit_index);
            }
            else
            {
...
int  newbase_hdmi_set_flow_cfg_entry(HDMI_FLOW_CFG_GROUP group_id, unsigned char entry_id, unsigned int set_value)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                result = -3;
                HDMI_EMG("[ERROR][%s] Wrong set value ,  %s=%d!\n" , __func__ ,
...
static unsigned int newbase_hdmi_pcbinfo_read(void)
{
    ...
    HDMI_EMG("-------Read HDMI PCB info start------\n"  );
            ...
            ((val&1)==TRUE))
        {
            ...
            for(j = 0; j< p_flow_cfg_group_table[i].cfg_table_length; j++)
            {
                ...
                if(CTRL_PCB_WRITE_ENABLE_FLAG == (p_current_cfg_table[j].ctrl_config & CTRL_PCB_WRITE_ENABLE_FLAG))
                {
                    ...
                    {//Use deault value
                        HDMI_EMG("[%d] Read 0 from PCB , Use driver default value %s =%d\n", j,
                    ...
                    {//Use PCB value
                        ...
                        HDMI_EMG("[%d] Update from pcb info, %s =%d\n", j,
                            GET_FLOW_CFG_NAME(i, j),
                            pcb_value);
                    }
                }
                ...
                {
                    HDMI_EMG("[%d] permission denied. Use default %s =%d\n", j, GET_FLOW_CFG_NAME(i, j), p_current_cfg_table[j].default_value);
                }
            }
        }
        else
        {
            HDMI_EMG("Read [%s] fail ,  not define!valid bit = %d\n" ,  p_flow_cfg_group_table[i].pcb_enum_name , (unsigned char)(val&1) );
            check_result |= (1<<i);
        }
    }
    HDMI_EMG(" -------Read HDMI PCB info End------\n" );
...
unsigned int  newbase_hdmi_flow_cfg_table_default_value_check(HDMI_FLOW_CFG_GROUP group_id)
{
    ...
    for(i = 0; i< p_flow_cfg_group_table[group_id].cfg_table_length ; i++)
    {
        ...
        if(max_value ==0)
        {
            HDMI_EMG("WARNING, invalid bit index !!group_id, entry_id)=(%d, %d), start_bit_index = %d, end_bit_index = %d \n", group_id, i,  p_current_cfg_table[i].start_bit_index, p_current_cfg_table[i].end_bit_index);
            check_result |= 1<<i;
            continue;
        }
        ...
        {
            HDMI_EMG("WARNING, invalid default value make overflow!!defaule = %d, max = %d \n",  p_current_cfg_table[i].default_value, max_value);
            check_result |= 1<<i;
            continue;
        }
    }
...
{
    int i = 0;
    HDMI_EMG("[newbase_hdmi_flow_cfg_info_init] HDMI Compatible Parameter Setting ---------------\n");

    for(i = 0; i < HDMI_FLOW_CFG_GROUP_NUM; i++)
    {
        unsigned int check_result = newbase_hdmi_flow_cfg_table_default_value_check(i);
        HDMI_EMG("check_result = %d\n", check_result);
    }

    if(newbase_hdmi_pcbinfo_read() !=0)
    {
        HDMI_EMG("[newbase_hdmi_flow_pcbinfo_init] cfg0_pcbinfo read fail, use code default!\n");
    }
