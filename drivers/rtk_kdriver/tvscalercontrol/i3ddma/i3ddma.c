...
//	gen_timing->quincunx_en = 0;
	...
	if(drvif_i3ddma_triplebuf_flag()){
		...
		if(tx_timing->v_freq <= 310 && (get_current_driver_pattern(DRIVER_FREERUN_PATTERN))){
			//oled panel set 24.1~24.5 ->24, 30.1~30.5 ->30
			if((webos_tooloption.eBackLight == 2) && (tx_timing->v_freq <= 310)){
				if((tx_timing->v_freq > 240) && (tx_timing->v_freq <= 245))
					gen_timing->v_freq = 240;
				else if((tx_timing->v_freq > 300) && (tx_timing->v_freq <= 305))
					gen_timing->v_freq = 300;
				else
					gen_timing->v_freq = tx_timing->v_freq;
			}
			else{
			...
			else if((tx_timing->v_freq >= 995) && (tx_timing->v_freq <= 1050))
...
//	vodma_vodma_pvs0_gen_RBUS vodma_vodma_pvs0_gen_Reg;
	...
	if (enable == _ENABLE){
		...
		if ((Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC) == 1) && (get_hdmi_4k_hfr_mode() == HDMI_NON_4K120)) {//fix 4k max size
...
//	IoReg_Write32(VODMA_VODMA_PVS0_Gen_reg, vodma_vodma_pvs0_gen_Reg.regValue);
	pr_notice("[Game Mode] i3ddma vodma iv2pv delay = %x\n", id2pv_delay_1);
...
void h3ddma_run_film_mode_capture_config(I3DDMA_TIMING_T *timing)
{
	...
	pr_emerg("NN output size, width=%d, height=%d\n", g_ulNNOutWidth, g_ulNNOutLength);
...
int h3ddma_get_NN_read_buffer(unsigned int *a_pulYAddr, unsigned int *a_pulCAddr)
{
	...
	{
		pr_info("h3ddma_get_NN_read_buffer error, ReadIdx=WriteIdx=%d\n", g_ucNNReadIdx);
		return -1;
	}
	...
	{
		pr_info("h3ddma_get_NN_read_buffer error (getsize = 0), Idx=%d\n", g_ucNNReadIdx);
		return -1;
	}
	...
	*a_pulCAddr = g_tNN_Cap_Buffer[0].phyaddr + ulBufSize * NN_CAP_BUFFER_NUM + (ulBufSize/2)*g_ucNNReadIdx;
	...
	pr_debug("h3ddma_get_NN_read_buffer idx=%d, phyaddr=%x, Y_Addr=%x, C_Addr=%x\n", g_ucNNReadIdx, ulBufAddr, *a_pulYAddr, *a_pulCAddr);
