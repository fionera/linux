...
void drv_I3DDMA_ApplySetting(unsigned char display)
{
	...
	if((i3ddmaCtrl.ptx_timing->h_act_len >= 3840) && (i3ddmaCtrl.ptx_timing->v_act_len > 1080)){
		printk(KERN_EMERG"[I2RND]over 4k1k size need to uzd to 1920x1080\n");
	...
	else if(0)//((i3ddmaCtrl.ptx_timing->h_act_len ==1440) && (DMA_VGIP_DMA_VGIP_CTRL_get_dma_in_sel(rtd_inl(DMA_VGIP_DMA_VGIP_CTRL_reg)) == VGIP_SRC_ATV))
	{
		...
		&& (DMA_VGIP_DMA_VGIP_CTRL_get_dma_in_sel(rtd_inl(DMA_VGIP_DMA_VGIP_CTRL_reg)) == VGIP_SRC_TMDS))
	{
		...
		//printk(KERN_EMERG"HDMI 2880x576i => 1920x576i\n");
		insize.nLength = i3ddmaCtrl.ptx_timing->v_act_len;
		insize.nWidth = i3ddmaCtrl.ptx_timing->h_act_len;
		outsize.nLength = i3ddmaCtrl.ptx_timing->v_act_len;
		outsize.nWidth = 1920;
...
void fw_i3ddma_set_vgip(unsigned char src, unsigned char mode)
{
	...
	{
		//error
		rtd_printk(KERN_NOTICE, TAG_NAME_VSC, "Warning: dma source incorrect %d\n", mode);
	}

	if(u32_cal_ivs_dly>8)	//protection
	{
		pr_emerg("%s(%d)-%s: Warning:dma delay incorrect=%d \n",__FILE__,__LINE__,__FUNCTION__,u32_cal_ivs_dly);
		u32_cal_ivs_dly = 8;
	}
...
{
#if 0
	h3ddma_cap_ctl2_RBUS h3ddma_cap_ctl2_reg;
	h3ddma_cap_ctl2_reg.regValue = IoReg_Read32(H3DDMA_Cap_CTL2_reg);
	h3ddma_cap_ctl2_reg.re_order = enable;
	IoReg_Write32(H3DDMA_Cap_CTL2_reg, h3ddma_cap_ctl2_reg.regValue);
#endif
}
