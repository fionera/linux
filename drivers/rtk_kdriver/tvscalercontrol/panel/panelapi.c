...
//fix me
	...
	if (ret== -1 ) {
		printk( "open /dev/scaler fail, cannot set m_backlightLevelActMax\n" );
		return FALSE;
	}
	*/
	...
	if (ret== -1 ) {
		printk( "open /dev/scaler fail, cannot set m_backlightLevelActMin\n" );
		return FALSE;
	}
	*/
...
void panel_setup_PMIC(void)
{
	...
	rtd_printk(KERN_NOTICE , "VBE" , "panel_setup_PMIC = %d \n", pmicIdx);
	...
	case PMIC_1F_36:
		if(i2c_master_send_ex(PANEL_PMIC_I2C, driver_addr, &pmic_1F_36_data[0][0], 2) < 0){
			rtd_printk(KERN_ERR , "VBE" , "PMIC err @ _line : %d \n", __LINE__);
		}
