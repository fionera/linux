...
HAL_VPQ_ENG_TWOLAYER_STRUCT ENG_TWOLAYER_STRUCT = {
	...
	{	//ENG_ID_STRUCT
		...
		{HAL_VPQ_ENG_HDR,			HAL_VPQ_ENG_HDR10p_EN,		"HDR10+ En",	HAL_VPQ_ENG_combobox,	0,	1,		{{{"OFF"}},{{"ON"}}},},
		{HAL_VPQ_ENG_HDR,			HAL_VPQ_ENG_HDR10p_EOTF_Gain,		"HDR10+ EOTF Gain",	HAL_VPQ_ENG_combobox,	0,	4,		{{{"12000"}},{{"11000"}},{{"10000"}},{{"9000"}},{{"8000"}}},},
		{HAL_VPQ_ENG_HDR,			HAL_VPQ_ENG_HDR10p_EOTF_Linear_W,		"HDR10+ EOTF Linear W",	HAL_VPQ_ENG_combobox,	0,	4,		{{{"500"}},{{"400"}},{{"300"}},{{"200"}},{{"100"}}},},
	}
};
...
unsigned int Scaler_Get_ENG_HDR_ID(unsigned int ID)
{
	...
	//_system_setting_info *VIP_system_info_structure_table = NULL;
	//VIP_system_info_structure_table = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	VIP_HDR10p_CTRL* HDR10p_CTRL;

	HDR10p_CTRL = drvif_fwif_color_Get_HDR10p_Ctrl();

	switch (ID) {
		case HAL_VPQ_ENG_HDR10p_EN:
			ret = drvif_fwif_color_Get_HDR10p_demo_flag(HDR10p_engMenu_layer);
			if (ret == HDR10p_Demo_ON_HDR10)
				ret = 0;
			else
				ret = 1;
			break;

		case HAL_VPQ_ENG_HDR10p_EOTF_Gain:
			if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div == (HDR10p_TBL.EOTF_norm_div + 20000))
				ret = 0;
			else if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div == (HDR10p_TBL.EOTF_norm_div + 10000))
				ret = 1;
			else if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div == (HDR10p_TBL.EOTF_norm_div - 10000))
				ret = 3;
			else if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div == (HDR10p_TBL.EOTF_norm_div - 20000))
				ret = 4;
			else
				ret = 2;
			break;

		case HAL_VPQ_ENG_HDR10p_EOTF_Linear_W:
			if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W == (HDR10p_TBL.EOTF_Linear_Blend_W + 200))
				ret = 0;
			else if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W == (HDR10p_TBL.EOTF_Linear_Blend_W + 100))
				ret = 1;
			else if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W == (HDR10p_TBL.EOTF_Linear_Blend_W - 100))
				ret = 3;
			else if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W == (HDR10p_TBL.EOTF_Linear_Blend_W - 200))
				ret = 4;
			else
				ret = 2;
			break;
...
unsigned int Scaler_Set_ENG_HDR_ID(unsigned int ID, int value)
{
	...
	switch (ID) {
		case HAL_VPQ_ENG_HDR10p_EN:
			if (value == 0)
				drvif_fwif_color_Set_HDR10p_demo_flag(HDR10p_Demo_ON_HDR10, HDR10p_engMenu_layer);
			else
				drvif_fwif_color_Set_HDR10p_demo_flag(HDR10p_Demo_OFF, HDR10p_engMenu_layer);
			break;

		case HAL_VPQ_ENG_HDR10p_EOTF_Gain:
			if (value == 0)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div = HDR10p_TBL.EOTF_norm_div + 20000;
			else if (value == 1)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div = HDR10p_TBL.EOTF_norm_div + 10000;
			else if (value == 3)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div = HDR10p_TBL.EOTF_norm_div - 10000;
			else if (value == 4)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div = HDR10p_TBL.EOTF_norm_div - 20000;
			else
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div = HDR10p_TBL.EOTF_norm_div;
			break;

		case HAL_VPQ_ENG_HDR10p_EOTF_Linear_W:
			if (value == 0)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W = HDR10p_TBL.EOTF_Linear_Blend_W + 200;
			else if (value == 1)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W = HDR10p_TBL.EOTF_Linear_Blend_W + 100;
			else if (value == 3)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W = HDR10p_TBL.EOTF_Linear_Blend_W - 100;
			else if (value == 4)
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W = HDR10p_TBL.EOTF_Linear_Blend_W - 200;
			else
				drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W = HDR10p_TBL.EOTF_Linear_Blend_W;
			break;
