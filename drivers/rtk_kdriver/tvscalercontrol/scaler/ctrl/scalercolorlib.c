...
#ifdef CONFIG_ARM64
static unsigned long od_mem_addr;
#else
	...
	if( input_src_form == _SRC_FROM_VDC )
	{
		...
		if (input_TVD_Form == TVD_INFMT_VODMA1)	{		/* USB source*/
			...
			case _MODE_1080P60:
				...
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_HDR10p)
					which_source = VIP_QUALITY_HDR10p_PVR_1080p;
			...
			case _MODE_4k2kP25:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_HDR10p_PVR_4k2kP_30;
				else if(bDolby_HDR_flag)
			...
			case _MODE_4k2kP60:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_HDR10p_PVR_4k2kP_60;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_PVR_4k2kP;
				else
			...
			case _MODE_1080P60:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_HDR10p_DTV_1080p;
				else if(bDolby_HDR_flag)
			...
			case _MODE_4k2kP25:
                        if (Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_THRIP) || Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_RTNR))
                        {
                                if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
                                        ...
                                        which_source = VIP_QUALITY_HDR10p_DTV_4k2kP_30;
                                else if(bDolby_HDR_flag)
                        ...
                        else //rtnr off  , dut to 4k60 rtnr must be off ,
                        {
                                if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
                                        ...
                                        which_source = VIP_QUALITY_HDR10p_DTV_4k2kP_60;
                                else if(bDolby_HDR_flag)
                                        which_source = VIP_QUALITY_DolbyHDR_DTV_4k2kP;
                                else
	...
	/*==USER: Vincent_Lee  DATE: 2011/08/12  TODO: Add new CASE for 1080P24 mode==*/
			...
			case _MODE_1080P30:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_HDR10p_HDMI_1080p;
				else if(bDolby_HDR_flag)
	...
	/*==USER: Vincent_Lee  DATE: 2011/08/12  TODO: Add new CASE for 1080P24 mode==*/
			...
			case _MODE_4k2kP25:
					if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
						...
						which_source = VIP_QUALITY_HDR10p_HDMI_4k2kP_30;
					else if(bDolby_HDR_flag)
			...
			case _MODE_4k2kP60:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_HDR10p_HDMI_4k2kP_60;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_HDMI_4k2kP;
				else
...
void Scaler_VIP_IP_init(unsigned char Vip_source)
{
	...
	drvif_color_icm_set_bypass_onoff(display, 0);
        ...
        if(BFI_init==0)
        {
                ...
                BFI_init=1;
                if ((strcmp(webos_strToolOption.eBackLight, "oled") == 0)&&(V4l2_memc_motion_pro==1)) //OLED & BFI=on
                {
...
void Scaler_SetLocalContrastEnable(unsigned char enable)
{
	...
	rtd_printk(KERN_DEBUG, TAG_NAME, "[%s] LC_Enable = %d\n", __FUNCTION__, enable);
	pTable = fwif_color_get_color_data(src_idx, 0);

	if (pTable == NULL)
	{
		rtd_printk(KERN_DEBUG, TAG_NAME, "[%s] Get table error\n", __FUNCTION__);
		return;
	}
...
void Scaler_SetLocalContrastTable(unsigned char TableIdx)
{
	...
	rtd_printk(KERN_DEBUG, TAG_NAME, "[%s] LC_Table = %d\n", __FUNCTION__, TableIdx);
	pTable = fwif_color_get_color_data(src_idx, 0);
...
void Scaler_fwif_color_set_LocalContrast_table(unsigned char src_idx, unsigned char TableIdx)
{
	...
	rtd_printk(KERN_DEBUG, TAG_NAME, "[%s] TableIdx = %d\n", __FUNCTION__, TableIdx);
...
{
	#ifdef CONFIG_ARM64
	od_mem_addr = (unsigned long)dvr_malloc_specific(0x76A700, GFP_DCU1);
	#else
...
#if 0	// use "limit to full" and "no gain" for csc1 @ SDR
...
#else		// use "no gain" and "full to limit" for csc1 @ SDR
	// use black level to determine data range
	if (blk_lv == RGB2YUV_BLACK_LEVEL_HIGH) {
		if (colorFMT_info == isRGB || colorFMT_info == COLORSPACE_MAX) {
			ret_idx = HDR_DM_CSC1_YUV2RGB_ByPass_GBRtoRGB_255_to_235;	// data format will be GBR from hdmi input
		} else if (colorFMT_info == is601) {
			ret_idx = HDR_DM_CSC1_YUV2RGB_BT601_255_to_235_240;
		} else if (colorFMT_info == is709) {
			ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_235_240;
		} else {
			ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_235_240;
		}
	} else {
		if (colorFMT_info == isRGB || colorFMT_info == COLORSPACE_MAX) {
			ret_idx = HDR_DM_CSC1_YUV2RGB_ByPass_GBRtoRGB_255_to_255;	// data format will be GBR from hdmi input
...
#endif
	rtd_printk(KERN_INFO, TAG_NAME, "DM_CSC006,dis=%d,mode=%d,srcType=%d,SrcGetFrom=%d,isHD=%d,HDMI_CSC_DataRange_Mode=%d,IsRGB_YUV_RANGE=%d,ucFormat=%d,\n",
		display, mode, srcType, SrcGetFrom, isHD, HDMI_CSC_DataRange_Mode, IsRGB_YUV_RANGE, ucFormat);

	rtd_printk(KERN_INFO, TAG_NAME, "DM_CSC006,video_fmt=%d,isHDMI=%d,.EC=%d,.C=%d,colorFMT_info=%d,data_range=%d,colour_primaries=%d,ret_idx=%d,blk_lv=%d\n",
...
unsigned char Scaler_get_HDR_YUV2RGB_SDR_MAX_RGB(void)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "DM_SMR, dis=%d,mode=%d,srcType=%d,SrcGetFrom=%d,isHD=%d,HDMI_CSC_DataRange_Mode=%d,IsRGB_YUV_RANGE=%d,ucFormat=%d,\n",
		display, mode, srcType, SrcGetFrom, isHD, HDMI_CSC_DataRange_Mode, IsRGB_YUV_RANGE, ucFormat);
...
void Scaler_Set_HDR_YUV2RGB(unsigned char HDR_mode, unsigned char color_format)
{
	...
	blk_lv = fwif_color_get_rgb2yuv_black_level_tv006();
	rtd_printk(KERN_INFO, TAG_NAME, "DM_CSC006,HDR_mode=%d, color_format=%d,blk_lv=%d\n", HDR_mode, color_format, blk_lv);
...
void Scaler_color_set_HDR_AutoRun_TBL(void)
{
	...
	if (HDR_AutoRun_TBL_Flag != NULL) {
		...
		if(vipSys_info == NULL){
			rtd_printk(KERN_EMERG, TAG_NAME, "Scaler_color_set_HDR_AutoRun_TBL, sys inof NULL!!!!\n");
			return;
		}
		...
		if (vpq_hdr_type_active == vipSys_info->HDR_flag) {
			...
			rtd_printk(KERN_INFO, TAG_NAME, "seamless, table done\n");
		} else {
			rtd_printk(KERN_INFO, TAG_NAME, "seamless,HDR flag changed, vipSys_info->HDR_flag=%d, seamless active hdr type=%d,\n",
...
//========check HDR statu ===========
	...
	if (HDR_Type == HDR_DM_DOLBY) {
		...
		VIP_system_info_structure_table ->HDR_flag = HAL_VPQ_HDR_MODE_HDR10p;
		VIP_system_info_structure_table->DolbyHDR_flag = 0;
...
//==========================================
	...
	if(Scaler_color_Get_HDR_AutoRun()) {
		...
		if(HDR_Type == HDR_DM_HDR10 || HDR_Type == HDR_DM_HLG || HDR_Type == HDR_DM_TECHNICOLOR || HDR_Type == HDR_DM_HDR10PLUS) {	//	HDR10
...
void Scaler_color_HDRInfo_Update(UINT32* modeinfo)
{
	...
	switch(modeinfo[0])	//HDR mode
	{
		case HAL_VPQ_HDR_MODE_HDR10:
			...
			fwif_color_set_HDR10p_FrameSync_Enable(0);
		...
		break;
		case HAL_VPQ_HDR_MODE_HDR10p:
			//VIP_RPC_system_info_structure_table->HDR_info.Ctrl_Item[TV006_HDR_En] = 1;
			Scaler_Set_HDR_YUV2RGB(modeinfo[0], modeinfo[1]);
			drvif_DM2_GAMMA_Enable(0);		// gamma 2.2 is embeded in OETF
			fwif_color_DM2_EOTF_OETF_3D_24x24x24_LUT_Handler(HAL_VPQ_HDR_MODE_HDR10p);
			//drvif_HDR_YUV2RGB(1, hdr_YUV2RGB[HDR_DM_CSC1_YUV2RGB_BT2020_Limted_235_to_255]);
			fwif_HDR_RGB2OPT(1, hdr_RGB2OPT[HDR_RGB2OPT_2020_TO_DCI]);
			fwif_color_set_TC_FrameSync_Enable(0);
			fwif_color_set_HDR10p_FrameSync_Enable(1);
			//rtd_printk(KERN_INFO, TAG_NAME, "HDR10 mode EO[1] = %d, OE[1] = %d\n", EOTF_LUT_R[1], OETF_LUT_R[1]);
		break;
		case HAL_VPQ_HDR_MODE_SDR:
		case HAL_VPQ_HDR_MODE_HDREFFECT:
...
HDR_flow_control_timing Scaler_color_get_HDR_timing(void)
{
	...
	if (VIP_system_info_structure_table->DolbyHDR_flag) {	//dolby case
		...
		result = HFC_TC;
	} else if (VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_HDR10p) {	//HDR10+ case
		result = HFC_HDR10P;
	} else {
...
void Scaler_color_HDRInfo_Update_by_HFC(UINT8 HFC_bit, HDR_flow_control_timing HDR_timing, UINT32* modeinfo)
{
	...
	if ((HDR_control[HDR_timing].HDR10_plus_ctrl.IPCtrlBits) & HFC_bit)
		fwif_color_set_HDR10p_FrameSync_Enable(HDR_control[HDR_timing].HDR10_plus_enbale);
		...
		switch(nIdex) {
			...
			case VIP_DITHER_IP_PANEL:
				...
				if (Get_DISPLAY_PANEL_MPLUS_RGBW() == TRUE || Get_DISPLAY_PANEL_BOW_RGBW() == TRUE)
					...
					fwif_color_set_Panel_Dither(TRUE,VIP_PANEL_BIT8,VIP_DITHER_PANELTABLE_BLANCE_ROLL_15);
                                else if (strcmp(webos_strToolOption.eBackLight, "oled") == 0)	// oled use 10 bit
					fwif_color_set_Panel_Dither(nDITHER_ENABLE,VIP_PANEL_BIT10,nDITHER_LUT);
				     else
					fwif_color_set_Panel_Dither(nDITHER_ENABLE,VIP_PANEL_BIT8,nDITHER_LUT);
				break;
		}
	}
}
