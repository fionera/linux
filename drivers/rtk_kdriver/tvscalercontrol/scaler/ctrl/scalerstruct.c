...
#if 1
static unsigned char LivezoomOffMainRunScalerFlag = FALSE;

void Set_LivezoomOffMainRunScaler(unsigned char value)
{
	LivezoomOffMainRunScalerFlag = value;
}

unsigned char Get_LivezoomOffMainRunScaler(void)
{
	return LivezoomOffMainRunScalerFlag;
}
#endif
...
void Scaler_InputRegionType_Set(unsigned char display,SCALER_VSC_INPUT_REGION_TYPE inputType)
{
	if(inputType > INPUT_BASE_ON_HAL_ORIGINAL_INPUT){
		pr_err("[error] wrong input region type:%d\n",inputType);
