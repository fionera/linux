...
extern unsigned short OETF_curve_HDR10plus[1025];	/* in hdr10plus.c*/
extern unsigned int knee_curve_HDR10plus[1025];	/* in hdr10plus.c*/
...
extern  INT32 thl_count_static_time;
extern  INT32 thl_stop_global_drop_count_frame;

extern UINT32 before_phyAddr ;
extern UINT32 after_phyAddr ;
extern unsigned int LD_size;

extern unsigned short pApl[34][60];
extern unsigned short LC_APL[2040];
...
unsigned char gVipDebugLogCtrls[256] = {//Debug log controls for VIPprintf
	...
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0x3, 0x0, 0xff, 0xff, 0x3, 0x0, 0xff, 0xff, 0x3, 0x0, 0x3, 0x0, 0x3, 0x0,
	0x3, 0x0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
#define HDR10p_METADATA_Curve_Info_Max 42
#define HDR10p_Basis_OOTF_Info_Max 20
#define HDR10p_Guided_OOTF_Info_Max 17
#define HDR10p_OETF_CTRL_Info_Max 10
#define HDR10p_OETF_Curve_Max 2050
...
/*SLD*/
int SLD_Y1 = 256, SLD_Y2 = 768, SLD_Y3 = 1023, SLD_gain_low = 512;
//unsigned char SLD_UIsel_L = 0;
//unsigned int SLD_cnt_th = 0;
int SLD_UIsel_L = 1;
int SLD_cnt_th = 0;
...
extern ICM_DMA_DEBUG_INFO g_icm_dma_dbg[ICM_DMA_DEBUG_INFO_SIZE];
extern short xTmp[600];
...
//		memcpy(buf, (unsigned char *) &(smartPic_clue->Adapt_DCC_Info.S_RPC_DCC_LAYER_Array.Apply_Curve), size);
	...
	case TOOLACCESS_PCID_RgnTbl: /*1252 get*/
		...
		if (size > sizeof(unsigned char)*PCID_TBL_LEN)
			return -ID_SIZE_ERR;

		pUcTemp = (unsigned char *)vip_malloc(sizeof(unsigned char)*PCID_TBL_LEN);

		if(pUcTemp == NULL)
			return -ID_SIZE_ERR;

		fwif_color_get_pcid_RgnTableValue(pUcTemp, rtice_param.mode0, rtice_param.mode1);

		memcpy(buf, (unsigned char*)pUcTemp, size);
	...
	case TOOLACCESS_AI_Ctrl_Tbl: /*1260 get*/
		...
		if(pwIntTemp == NULL)
			return -ID_SIZE_ERR;
                ...
                scaler_AI_Ctrl_Get((DRV_AI_Ctrl_table*)pwIntTemp);
		fwif_color_ChangeINT32Endian(pwIntTemp, size>>2, 1);
...
*/
	...
	case TOOLACCESS_ICM_K5_STRUCTURE:
	{
		...
		extern unsigned char cm_data_debug;
		UINT8 i;
		UINT32 index = 0;
		UINT32 icm_DB_size_plus = sizeof(CHIP_CM_REGION_T) + 100;

		cm_data_debug = 1;	/* using tool ==> enable DB debug mode */
		rtd_printk(KERN_INFO, TAG_NAME, "icm_DB_size = %d, size = %d\n", (icm_DB_size_plus - 100), size);
		if (size > icm_DB_size_plus)
			return -ID_SIZE_ERR;

		pwINT16Temp = (short *)vip_malloc(icm_DB_size_plus); /* get bigger DDR */
		...
		for (i = 0; i < CHIP_CM_COLOR_REGION_MAX; i++)
		{
			...
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].hue_x[0]), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].hue_g[0]), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].sat_x[0]), (CHIP_SAT_TBLPOINT + 1) * sizeof(short));
			index = index + (CHIP_SAT_TBLPOINT + 1);
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].sat_g[0]), (CHIP_SAT_TBLPOINT + 1) * sizeof(short));
			index = index + (CHIP_SAT_TBLPOINT + 1);
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].Int_x[0]), (CHIP_INT_TBLPOINT + 1) * sizeof(short));
			index = index + (CHIP_INT_TBLPOINT + 1);
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].Int_g[0]), (CHIP_INT_TBLPOINT + 1) * sizeof(short));
			index = index + (CHIP_INT_TBLPOINT + 1);
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].r_offset_by_hue[0]), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].g_offset_by_hue[0]), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.stColorRegion[i].b_offset_by_hue[0]), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
		}

		memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.satbysat_global[0]), (CHIP_SAT_TBLPOINT + 1) * sizeof(short));
		index = index + (CHIP_SAT_TBLPOINT + 1);
		memcpy((pwINT16Temp + index), &(cm_rgn_tmp->stColorRegionType.satbyInt_global[0]), (CHIP_INT_TBLPOINT + 1) * sizeof(short));
		index = index + (CHIP_INT_TBLPOINT + 1);

		/*copy gain_by_sat(int)_for_rgb_offset*/
		for (i = 0; i < (CHIP_SAT_TBLPOINT + 1); i++) /* 8bit to 16bit */
			pwINT16Temp[index++] = cm_rgn_tmp->stColorRegionType.gain_by_sat_for_rgb_offset[i];
		for (i = 0; i < (CHIP_INT_TBLPOINT + 1); i++) /* 8bit to 16bit */
			pwINT16Temp[index++] = cm_rgn_tmp->stColorRegionType.gain_by_int_for_rgb_offset[i];
		...
		for (i = 0; i < CHIP_CM_COLOR_REGION_MAX; i++)
		{
			pwINT16Temp[index] = cm_ctrl_tmp->stColor[i].masterGain;
			pwINT16Temp[index + 1] = cm_ctrl_tmp->stColor[i].RGBOffset_masterGain;
			pwINT16Temp[index + 2] = cm_ctrl_tmp->stColor[i].stColorGain.stColorGainHue;
			pwINT16Temp[index + 3] = cm_ctrl_tmp->stColor[i].stColorGain.stColorGainSat;
			pwINT16Temp[index + 4] = cm_ctrl_tmp->stColor[i].stColorGain.stColorGainInt;
			index = index + 5;
		}
		fwif_color_ChangeUINT16Endian(pwINT16Temp, index, 1);
		rtd_printk(KERN_INFO, TAG_NAME, "icm_DB item number = %d\n", index);
		//snt to tool
		memcpy(buf, (char *)pwINT16Temp, size);
	...
	case TOOLACCESS_GSR_BLK_APL_GET:
	{
		...
		rtd_printk(KERN_INFO, TAG_NAME, "TOOLACCESS_GSR_BLK_APL_GET\n");
	...
	case TOOLACCESS_HDR10p_CTRL:
		if (size > sizeof(VIP_HDR10p_CTRL))
			return -ID_SIZE_ERR;
		HDR10p_CTRL = drvif_fwif_color_Get_HDR10p_Ctrl();

		HDR10p_CTRL->Debug_Log = RPC_system_setting_info->HDR_info.Debug_Log_3;
		HDR10p_CTRL->Debug_Delay = RPC_system_setting_info->HDR_info.Debug_Log_Delay_Frame;

		memcpy(	&buf[0], (unsigned char*)HDR10p_CTRL, size);
		fwif_color_ChangeUINT32Endian((unsigned int*)&buf[0], size/sizeof(int), 1);
		*num_type = VIP_UINT32;

		break;

	case TOOLACCESS_HDR10p_METADATA:
		if (size > sizeof(HDR10PLUS_metadata))
			return -ID_SIZE_ERR;

		hdr10p_metadata = drvif_fwif_color_Get_HDR10p_METADATA_Buff();
		memcpy(	&buf[0], (unsigned char*)hdr10p_metadata, size);
		fwif_color_ChangeUINT32Endian((unsigned int*)&buf[0], size/sizeof(int), 1);
		*num_type = VIP_UINT32;

		break;

	case TOOLACCESS_HDR10p_METADATA_Curve_Info:
	{
		if (size > sizeof(int) * HDR10p_METADATA_Curve_Info_Max)
			return -ID_SIZE_ERR;

		hdr10p_metadata = drvif_fwif_color_Get_HDR10p_METADATA_Buff();
		pwUINT32Temp = (unsigned int *)vip_malloc(HDR10p_METADATA_Curve_Info_Max * sizeof(int));
		pwUINT32Temp[0] = hdr10p_metadata->knee_point_x;	/* in 12 bits*/
		pwUINT32Temp[1] = hdr10p_metadata->knee_point_y;	/* in 12 bits*/
		pwUINT32Temp[2] = hdr10p_metadata->num_bezier_curve_anchors;
		for(i=0;i<bezier_curve_anchors_num;i++)
			pwUINT32Temp[i+3] = hdr10p_metadata->bezier_curve_anchors[i];	/* in 10 bits */
		pwUINT32Temp[18] = hdr10p_metadata->targeted_system_display_maximum_luminance;	/* unit is nits. */
		pwUINT32Temp[19] = hdr10p_metadata->maxscl[0]; /* unit is nits/10,  ie. maxscl/10 == nits*/
		pwUINT32Temp[20] = hdr10p_metadata->maxscl[1]; /* " unit is nits/10 */
		pwUINT32Temp[21] = hdr10p_metadata->maxscl[2]; /* unit is nits/10 */
		for(i=0;i<distribution_maxrgb_bin_num;i++)
			pwUINT32Temp[i+22] = hdr10p_metadata->distribution_maxrgb_percentages[i];
		for(i=0;i<distribution_maxrgb_bin_num;i++)
			pwUINT32Temp[i+32] = hdr10p_metadata->distribution_maxrgb_percentiles[i];	/* unit is nits/10 */
	...
	case TOOLACCESS_HDR10p_Basis_OOTF_Info:
	{
		if (size > sizeof(int) * HDR10p_Basis_OOTF_Info_Max)
			return -ID_SIZE_ERR;

		HDR10p_CTRL = drvif_fwif_color_Get_HDR10p_Ctrl();
		pwUINT32Temp = (unsigned int *)vip_malloc(HDR10p_Basis_OOTF_Info_Max * sizeof(int));
		pwUINT32Temp[0] = HDR10p_CTRL->Debug_basisOOTF_kneePx;	/* in 12 bits*/
		pwUINT32Temp[1] = HDR10p_CTRL->Debug_basisOOTF_kneePy;	/* in 12 bits*/
		pwUINT32Temp[2] = drvif_fwif_color_Get_HDR10p_TBL()->basisOOTF_order;
		for(i=0;i<bezier_curve_anchors_num;i++)
			pwUINT32Temp[i+3] = HDR10p_CTRL->Debug_basisOOTF_P[i];	/* in 10 bits*/
		pwUINT32Temp[18] = HDR10p_CTRL->Debug_basisOOTF_Fixed_KneePoint;
		pwUINT32Temp[19] = HDR10p_CTRL->Debug_basisOOTF_Fixed_Anchors;
	...
	case TOOLACCESS_HDR10p_Guided_OOTF_Info:
	{
		if (size > sizeof(int) * HDR10p_Guided_OOTF_Info_Max)
			return -ID_SIZE_ERR;

		HDR10p_CTRL = drvif_fwif_color_Get_HDR10p_Ctrl();
		pwUINT32Temp = (unsigned int *)vip_malloc(HDR10p_Guided_OOTF_Info_Max * sizeof(int));
		pwUINT32Temp[0] = HDR10p_CTRL->Debug_GuidedOOTF_kneePx;	/* in 12 bits*/
		pwUINT32Temp[1] = HDR10p_CTRL->Debug_GuidedOOTF_kneePy;	/* in 12 bits*/
		for(i=0;i<bezier_curve_anchors_num;i++)
			pwUINT32Temp[i+2] = HDR10p_CTRL->Debug_GuidedOOTF_P[i];	/* in 10 bits*/
	...
	case TOOLACCESS_HDR10p_OETF_CTRL_Info:
	{
		if (size > sizeof(int) * HDR10p_OETF_CTRL_Info_Max)
			return -ID_SIZE_ERR;
		...
		pwUINT32Temp = (unsigned int *)vip_malloc(HDR10p_OETF_CTRL_Info_Max * sizeof(int));
		pwUINT32Temp[0] = HDR10p_CTRL->Enable;
		pwUINT32Temp[1] = drvif_fwif_color_Get_HDR10p_TBL()->product_peak;
		pwUINT32Temp[2] = drvif_fwif_color_Get_HDR10p_TBL()->product_min;
		pwUINT32Temp[3] = drvif_fwif_color_Get_HDR10p_TBL()->basis_peak;
		pwUINT32Temp[4] = drvif_fwif_color_Get_HDR10p_TBL()->curve_sel;
		pwUINT32Temp[5] = drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div;
		pwUINT32Temp[6] = drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W;
		pwUINT32Temp[7] = drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Gamma22;
		pwUINT32Temp[8] = HDR10p_CTRL->Debug_Log;
		pwUINT32Temp[9] = HDR10p_CTRL->Debug_Delay;
	...
	case TOOLACCESS_HDR10p_OETF_Curve:
	{
		if (size > sizeof(int) * HDR10p_OETF_Curve_Max)
			return -ID_SIZE_ERR;

		HDR10p_CTRL = drvif_fwif_color_Get_HDR10p_Ctrl();
		pwUINT32Temp = (unsigned int *)vip_malloc(HDR10p_OETF_Curve_Max * sizeof(int));
		for(i=0;i<OETF_size;i++)
			pwUINT32Temp[i] = knee_curve_HDR10plus[i];
		for(i=0;i<OETF_size;i++)
			pwUINT32Temp[i+OETF_size] = OETF_curve_HDR10plus[i];
...
*/
	...
	case TOOLACCESS_VIP_LEA:
		...
		pwINT32Temp[12] = (unsigned int)LD_virAddr;
	...
	{
		if(size > sizeof(g_icm_dma_dbg))
			return -ID_SIZE_ERR;

		pwUINT32Temp = (unsigned int *)vip_malloc(sizeof(g_icm_dma_dbg));
		if (pwUINT32Temp != NULL) {
			memcpy(pwUINT32Temp, g_icm_dma_dbg, sizeof(g_icm_dma_dbg));
			fwif_color_ChangeUINT32Endian(pwUINT32Temp, sizeof(g_icm_dma_dbg)/sizeof(int), 1);
			memcpy(buf, pwUINT32Temp, size);
			vip_free(pwUINT32Temp);
		} else
			return -ID_SIZE_ERR;
...
*/
	...
	case TOOLACCESS_PCID_RgnTbl: /*1252 set*/
		...
		memcpy((unsigned char*)pUcTemp, buf, size);

		fwif_color_set_pcid_RgnTableValue(pUcTemp, rtice_param.mode0, rtice_param.mode1);
	...
	case TOOLACCESS_AI_Ctrl_Tbl: /*1260 set*/
		...
		memcpy((unsigned char *)pwIntTemp, (unsigned char *)buf, size);
                ...
                scaler_AI_Ctrl_Set((DRV_AI_Ctrl_table *)pwIntTemp);
...
// lesley 1024
	case TOOLACCESS_AI_Ctrl_Draw: /*1261 set*/
		...
		v4l2_vpq_stereo_face = buf[0];
		vpq_v4l2_ai_ctrl(v4l2_vpq_stereo_face, 0);
...
//			Scaler_SetMA_Chroma_Error(buf[12]); // merlin4 remove
	...
	case TOOLACCESS_ICM_K5_STRUCTURE:
	{
		...
		for (i = 0; i < CHIP_CM_COLOR_REGION_MAX; i++)
		{
			...
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].hue_x[0]), (pwINT16Temp + index), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].hue_g[0]), (pwINT16Temp + index), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].sat_x[0]), (pwINT16Temp + index), (CHIP_SAT_TBLPOINT + 1) * sizeof(short));
			index = index + (CHIP_SAT_TBLPOINT + 1);
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].sat_g[0]), (pwINT16Temp + index), (CHIP_SAT_TBLPOINT + 1) * sizeof(short));
			index = index + (CHIP_SAT_TBLPOINT + 1);
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].Int_x[0]), (pwINT16Temp + index), (CHIP_INT_TBLPOINT + 1) * sizeof(short));
			index = index + CHIP_INT_TBLPOINT + 1;
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].Int_g[0]), (pwINT16Temp + index), CHIP_INT_TBLPOINT + 1 * sizeof(short));
			index = index + CHIP_INT_TBLPOINT + 1;
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].r_offset_by_hue[0]), (pwINT16Temp + index), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].g_offset_by_hue[0]), (pwINT16Temp + index), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
			memcpy(&(cm_rgn_tmp->stColorRegionType.stColorRegion[i].b_offset_by_hue[0]), (pwINT16Temp + index), CHIP_HUE_TBLPOINT * sizeof(short));
			index = index + CHIP_HUE_TBLPOINT;
		}

		memcpy(&(cm_rgn_tmp->stColorRegionType.satbysat_global[0]), (pwINT16Temp + index), (CHIP_SAT_TBLPOINT + 1) * sizeof(short));
		index = index + (CHIP_SAT_TBLPOINT + 1);
		memcpy(&(cm_rgn_tmp->stColorRegionType.satbyInt_global[0]), (pwINT16Temp + index), (CHIP_INT_TBLPOINT + 1) * sizeof(short));
		index = index + (CHIP_INT_TBLPOINT + 1);

		/*copy gain_by_sat(int)_for_rgb_offset*/
		for (i = 0; i < (CHIP_SAT_TBLPOINT + 1); i++) /* 16bit to 8bit */
			cm_rgn_tmp->stColorRegionType.gain_by_sat_for_rgb_offset[i] = pwINT16Temp[index++];
		for (i = 0; i < (CHIP_INT_TBLPOINT + 1); i++) /* 16bit to 8bit */
			cm_rgn_tmp->stColorRegionType.gain_by_int_for_rgb_offset[i] = pwINT16Temp[index++];
		...
		for (i = 0; i < CHIP_CM_COLOR_REGION_MAX; i++)
		{
			cm_ctrl_tmp->stColor[i].masterGain = pwINT16Temp[index];
			cm_ctrl_tmp->stColor[i].RGBOffset_masterGain = pwINT16Temp[index + 1];
			cm_ctrl_tmp->stColor[i].stColorGain.stColorGainHue = pwINT16Temp[index + 2];
			cm_ctrl_tmp->stColor[i].stColorGain.stColorGainSat = pwINT16Temp[index + 3];
			cm_ctrl_tmp->stColor[i].stColorGain.stColorGainInt = pwINT16Temp[index + 4];
			index = index + 5;
		}
	...
	case TOOLACCESS_OD_Table_17x17:
	{
		...
		drvif_color_od_table_seperate(od_delta_table, 0, 255);// 0:delta mode 1:target mode(output mode)
	...
	case TOOLACCESS_HDR10p_CTRL:
		...
		pwUINT32Temp = ((unsigned int*)HDR10p_CTRL) + start_addr;

		memcpy(	(unsigned char*)pwUINT32Temp, &buf[0], size);

		RPC_system_setting_info->HDR_info.Debug_Log_3 = HDR10p_CTRL->Debug_Log;
		RPC_system_setting_info->HDR_info.Debug_Log_Delay_Frame = HDR10p_CTRL->Debug_Delay;

		// for debug
		if (drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Gamma22 == 1)
	...
	case TOOLACCESS_HDR10p_Basis_OOTF_Info:
	{
		...
		HDR10p_CTRL->Debug_basisOOTF_kneePx = pwUINT32Temp[0];	/* in 12 bits*/
		HDR10p_CTRL->Debug_basisOOTF_kneePy = pwUINT32Temp[1];	/* in 12 bits*/
		drvif_fwif_color_Get_HDR10p_TBL()->basisOOTF_order = pwUINT32Temp[2];
		for(i=0;i<bezier_curve_anchors_num;i++)
			HDR10p_CTRL->Debug_basisOOTF_P[i] = pwUINT32Temp[i+3];	/* in 10 bits*/
		HDR10p_CTRL->Debug_basisOOTF_Fixed_KneePoint = pwUINT32Temp[18];
		HDR10p_CTRL->Debug_basisOOTF_Fixed_Anchors= pwUINT32Temp[19];
	...
	case TOOLACCESS_HDR10p_OETF_CTRL_Info:
	{
		...
		HDR10p_CTRL->Enable							= pwUINT32Temp[0];
		drvif_fwif_color_Get_HDR10p_TBL()->product_peak				= pwUINT32Temp[1];
		drvif_fwif_color_Get_HDR10p_TBL()->product_min		= pwUINT32Temp[2];
		drvif_fwif_color_Get_HDR10p_TBL()->basis_peak				= pwUINT32Temp[3];
		drvif_fwif_color_Get_HDR10p_TBL()->curve_sel				= pwUINT32Temp[4];
		drvif_fwif_color_Get_HDR10p_TBL()->EOTF_norm_div			= pwUINT32Temp[5];
		drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Linear_Blend_W		= pwUINT32Temp[6];
		drvif_fwif_color_Get_HDR10p_TBL()->EOTF_Gamma22			= pwUINT32Temp[7];
		HDR10p_CTRL->Debug_Log						= pwUINT32Temp[8];
		HDR10p_CTRL->Debug_Delay				= pwUINT32Temp[9];
		vip_free(pwUINT32Temp);
...
/*
	case TOOLACCESS_VIP_SLD:
		if (size > (sizeof(int)*9))
			return -ID_SIZE_ERR;


		pwINT32Temp = (int *)vip_malloc(sizeof(int)*9);
		...
		SLD_Y1 = pwINT32Temp[0];
		SLD_Y2 = pwINT32Temp[1];
		SLD_Y3 = pwINT32Temp[2];
		SLD_gain_low = pwINT32Temp[3] ;
		SLD_UIsel_L = pwINT32Temp[4];
		thl_count_static_time = pwINT32Temp[5];
		thl_stop_global_drop_count_frame = pwINT32Temp[6];
		SLD_cnt_th = pwINT32Temp[7] ;
		//SLD_en_toolID_2361 = pwINT32Temp[8] ;
...
int  rtice_SetGet_DeMura(ap_data_param *rtice_param, unsigned char *num_type, unsigned char *buf, _system_setting_info *system_setting_info, unsigned char isSet_Flag)
{
	...
	if (base_addr == 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura, addr = NULL\n");
		return -1;
	}
	...
	if (isSet_Flag == 1) {
		if (rtice_param->id == TOOLACCESS_VIP_DeMura_TBL) {
			if (rtice_param->size > sizeDemuraTbl || rtice_param->saddr + rtice_param->size > sizeDemuraTbl) {
				rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura error, size = %d, start addr = %d, end addr = %d\n", rtice_param->size, rtice_param->saddr, rtice_param->eaddr);
				return -1;
			}
			...
			if (rtice_param->size > adaptiveTBL_Size || rtice_param->saddr + rtice_param->size > adaptiveTBL_Size) {
				rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura adative error, size = %d, start addr = %d, end addr = %d\n", rtice_param->size, rtice_param->saddr, rtice_param->eaddr);
				return -1;
			}
			...
			if (rtice_param->size > sizeof(char)) {
				rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura TOOLACCESS_VIP_DeMura_TBL_Mode error, size = %d, start addr = %d, end addr = %d\n", rtice_param->size, rtice_param->saddr, rtice_param->eaddr);
				return -1;
			}
			...
			if (rtice_param->size > sizeof(char)) {
				rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura TOOLACCESS_VIP_DeMura_INI error, size = %d, start addr = %d, end addr = %d\n", rtice_param->size, rtice_param->saddr, rtice_param->eaddr);
				return -1;
			}
			...
			if (rtice_param->size > sizeof(char)) {
				rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura TOOLACCESS_VIP_DeMura_Channel_Mode error, size = %d, start addr = %d, end addr = %d\n", rtice_param->size, rtice_param->saddr, rtice_param->eaddr);
				return -1;
			}
		...
		if (rtice_param->id == TOOLACCESS_VIP_DeMura_TBL) {
			...
			if (pDemura_TBL_tmp == NULL) {
				rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura error, alloc demura tbl tmp fail\n");
				return -ID_SIZE_ERR;
			}
			...
			//rtd_printk(KERN_EMERG, TAG_NAME, "DMA_TBL_Buff.table_height=%d,DMA_TBL_Buff.line_step=%d\n", DMA_TBL_Buff.table_height, DMA_TBL_Buff.line_step);
			...
			*num_type = VIP_UINT8;
			...
			if (rtice_param->size > adaptiveTBL_Size || rtice_param->saddr + rtice_param->size > adaptiveTBL_Size) {
				rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_DeMura adaptive error, size = %d, start addr = %d, end addr = %d\n", rtice_param->size, rtice_param->saddr, rtice_param->eaddr);
				return -1;
			}
...
unsigned char  rtice_SetGet_Froce_vTop_Flag(unsigned char Force_flag, unsigned char isSet_Flag)
{
	...
	if (isSet_Flag == 1) {
		...
		rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_Froce_vTop_Flag, Force_vTop_Flag = %d\n", Force_vTop_Flag);
	}
...
char rtice_SetGet_Froce_vTop_HDMI_DRM(void *pHDMI_DRM, unsigned char isSet_Flag)
{
	...
	if (pHDMI_DRM == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "rtice_SetGet_Froce_vTop_HDMI_DRM, NULL\n");
		return -1;
	}
