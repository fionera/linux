...
unsigned char vbe_orbit_state = 0;
...
void vbe_disp_set_Orbit_State(unsigned char bState)
{
	vbe_orbit_state = bState;
}
unsigned char vbe_disp_get_Orbit_State(void)
{
	return vbe_orbit_state;
}
...
{
	if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC) || vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY))
		return FALSE;
...
//			if(Scaler_ME_CLK_Check() == TRUE)
				...
				{ /* if vr360 run, set_vt_EnableFRCMode(FALSE),always bypass memc -KTASKWBS-13022*/
...
extern unsigned char get_WB_MEMC_forcebg_flag(void);
...
void vbe_disp_set_dtgM2uzu_lowdelay_mode(unsigned char mode)
{
	...
	if ((webos_tooloption.eModelModuleType == module_LGD) && (webos_tooloption.eBackLight == direct ) && (webos_tooloption.eLEDBarType == local_dim_block_32)){
		if(mode == 2){
			pr_notice("LGD LD32 panel, cannot set mode2, due to timing change make panel flicker.\n");
			mode=0;
		}
	}
...
*/
	...
	if(needUpdate){
		...
		if ((webos_tooloption.eModelModuleType == module_LGD) && (webos_tooloption.eBackLight == direct ) && (webos_tooloption.eLEDBarType == local_dim_block_32) ){
				...
				pr_notice("Use memc forcebg for uzudelay garbage.\n");
				//double buffer apply
				ppoverlay_double_buffer_ctrl2_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL2_reg);
...
*/
	...
	//		vbe_dtg_memec_bypass_switch_set(0);
			...
			{
#if 0
				if(check_hdmi_uhd_condition() == TRUE){
					//enable MEMC clock
...
unsigned int Get_Input_vfreq_by_vsync_cnt(void){
	...
	while(count){
		...
		rtd_printk(KERN_INFO, TAG_NAME_VBE,"[WARN] IVS N.\n");
		if(ivs_cnt > 0){
	...
	if(vfreq > 0){
		if((vfreq >= 5999) && (vfreq <= 6001))
			...
			rtd_printk(KERN_ERR, TAG_NAME_VBE,"[WARN] Not normalized ivs freq.(%d)\n", vfreq);
		}
	}
...
//		memcdtg_dv_total_RBUS memcdtg_dv_total_reg;
		...
		{
			vertical_total = ((Get_DISPLAY_CLOCK_TYPICAL()/((Get_DISP_HORIZONTAL_TOTAL()/100)*vFreq))*removeRatio)/multipleRatio;
		}else
		{
			if((Get_DISPLAY_REFRESH_RATE() == 120) &&
			...
			{
				vertical_total = ((Get_DISPLAY_CLOCK_TYPICAL()/((Get_DISP_HORIZONTAL_TOTAL()/100)*(vFreq/2)))*removeRatio)/multipleRatio;
			}
			else{
...
/*extern unsigned char HAL_VT_Finalize(void); WOSQRTK-13830 marked, and just stop dc2h when clock off */
...
void HAL_VBE_DISP_SetDisplayOutput(BOOLEAN bOnOff)
{
	...
	else
	{
		...
		/*if(HAL_VT_Finalize()==TRUE)
			rtd_printk(KERN_EMERG, TAG_NAME_VBE, "%s=%d  (HAL_VT_Finalize()==TRUE)\n", __FUNCTION__, __LINE__);
		else
			rtd_printk(KERN_EMERG, TAG_NAME_VBE, "%s=%d  (HAL_VT_Finalize()==FALSE)\n", __FUNCTION__, __LINE__);*/
...
unsigned char vbe_disp_ap_set_orbit_mode=FALSE;
...
void HAL_VBE_DISP_OLED_SetOrbit(BOOLEAN bEnable, unsigned char mode)
{
	...
	down(&VBE_Semaphore);
	vbe_disp_ap_set_orbit_mode = mode;
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"PC mode need to use justscan mode \n");
		vbe_disp_set_orbit(bEnable, _VBE_PANEL_ORBIT_JUSTSCAN_MODE);
	}
	else
	{
...
void HAL_VBE_DISP_SetBOEMode(unsigned char mode)
{
	down(&VBE_Semaphore);

	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"HAL_VBE_DISP_SetBOEMode, mode:%d \n", mode);

	down(&VBE_RGBW_Resume_Semaphore);
	//vbe_disp_set_BoeMode(pFwData, size);
	fwif_color_set_SiW_ModeChange_TV006(mode);
...
void vbe_disp_panel_boe_resume_flow(void)
{
	...
	// siw_rgbw register
	rtd_outl(0xb802bb04,  0x9000007e);	//0x	0	to 0x	1
	rtd_outl(0xb802bb08,  0x0f000870);	//0x	2	to 0x	3
	rtd_outl(0xb802bb0c,  0x10410100);	//0x	4	to 0x	5
	rtd_outl(0xb802bb10,  0x90000D02);	//0x	6	to 0x	7
	rtd_outl(0xb802bb14,  0x05190002);	//0x	8	to 0x	9
	rtd_outl(0xb802bb18,  0x0001800a);	//0x	10	to 0x	11
	rtd_outl(0xb802bb1c,  0x0200005d);	//0x	12	to 0x	13
	rtd_outl(0xb802bb20,  0x14820004);	//0x	14	to 0x	15
	rtd_outl(0xb802bb24,  0x0bb80000);	//0x	16	to 0x	17
	rtd_outl(0xb802bb28,  0x01550156);	//0x	18	to 0x	19
	rtd_outl(0xb802bb2c,  0x01551a0c);	//0x	20	to 0x	21
	rtd_outl(0xb802bb30,  0x40226050);	//0x	22	to 0x	23
	rtd_outl(0xb802bb34,  0x8070a090);	//0x	24	to 0x	25
	rtd_outl(0xb802bb38,  0xc0b0e0d0);	//0x	26	to 0x	27
	rtd_outl(0xb802bb3c,  0x00f00018);	//0x	28	to 0x	29
	rtd_outl(0xb802bb40,  0x0c00c008);	//0x	30	to 0x	31
	rtd_outl(0xb802bb44,  0x00800180);	//0x	32	to 0x	33
	rtd_outl(0xb802bb48,  0x00800018);	//0x	34	to 0x	35
	rtd_outl(0xb802bb4c,  0x00ff6c24);	//0x	36	to 0x	37
	rtd_outl(0xb802bb50,  0x028a03ff);	//0x	38	to 0x	39
	rtd_outl(0xb802bb54,  0x028a0172);	//0x	40	to 0x	41
	rtd_outl(0xb802bb58,  0x03ff0172);	//0x	42	to 0x	43
	rtd_outl(0xb802bb5c,  0x00ff00e6);	//0x	44	to 0x	45
	rtd_outl(0xb802bb60,  0x00ff0190);	//0x	46	to 0x	47
	rtd_outl(0xb802bb64,  0x01ff0190);	//0x	48	to 0x	49
	rtd_outl(0xb802bb68,  0x00000000);	//0x	50	to 0x	51
	rtd_outl(0xb802bb6c,  0x00000000);	//0x	52	to 0x	53
	rtd_outl(0xb802bb70,  0x00000000);	//0x	54	to 0x	55
	rtd_outl(0xb802bb74,  0x00000000);	//0x	56	to 0x	57
	rtd_outl(0xb802bb78,  0x00000000);	//0x	58	to 0x	59
	rtd_outl(0xb802bb7c,  0x00000000);	//0x	60	to 0x	61
	rtd_outl(0xb802bb80,  0x00000000);	//0x	62	to 0x	63
	rtd_outl(0xb802bb84,  0x02bc0384);	//0x	64	to 0x	65
	rtd_outl(0xb802bb88,  0x03840384);	//0x	66	to 0x	67
	rtd_outl(0xb802bb8c,  0x032002ee);	//0x	68	to 0x	69
	rtd_outl(0xb802bb90,  0x02580064);	//0x	70	to 0x	71
	rtd_outl(0xb802bb94,  0x00640000);	//0x	72	to 0x	73
	rtd_outl(0xb802bb98,  0x00000000);	//0x	74	to 0x	75
	rtd_outl(0xb802bb9c,  0x00000000);	//0x	76	to 0x	77
	rtd_outl(0xb802bba0,  0x00000000);	//0x	78	to 0x	79
	rtd_outl(0xb802bba4,  0x000002E1);	//0x	80	to 0x	81
	rtd_outl(0xb802bba8,  0x10001000);	//0x	82	to 0x	83
	rtd_outl(0xb802bbac,  0x10001000);	//0x	84	to 0x	85
	rtd_outl(0xb802bbb0,  0x10001000);	//0x	86	to 0x	87
	rtd_outl(0xb802bbb4,  0x10001000);	//0x	88	to 0x	89
	rtd_outl(0xb802bbb8,  0x04000000);	//0x	90	to 0x	91
	rtd_outl(0xb802bbbc,  0x7e90007e);	//0x	92	to 0x	93
	rtd_outl(0xb802bbc0,  0x00000000);	//0x	94	to 0x	95
	rtd_outl(0xb802bbc4,  0x00000003);	//0x	96	to 0x	97
	rtd_outl(0xb802bbc8,  0x001e2a30);	//0x	98	to 0x	99
	rtd_outl(0xb802bbcc,  0x02d007d0);	//0x	100 to 0x	101
	rtd_outl(0xb802bbd0,  0x14001000);	//0x	102 to 0x	103
	rtd_outl(0xb802bbd4,  0x00060180);	//0x	104 to 0x	105
	rtd_outl(0xb802bbd8,  0x00000800);	//0x	106 to 0x	107
	rtd_outl(0xb802bbdc,  0x00102000);	//0x	108 to 0x	109
	rtd_outl(0xb802bbe0,  0x00000000);	//0x	110 to 0x	111
	rtd_outl(0xb802bbe4,  0x00190400);	//0x	112 to 0x	113
	rtd_outl(0xb802bbe8,  0x00080000);	//0x	114 to 0x	115
	rtd_outl(0xb802bbec,  0x12491000);	//0x	116 to 0x	117
	rtd_outl(0xb802bbf0,  0x00100249);	//0x	118 to 0x	119
	rtd_outl(0xb802bbf4,  0x02490249);	//0x	120 to 0x	121
	rtd_outl(0xb802bbf8,  0x02490249);	//0x	122 to 0x	123
	rtd_outl(0xb802bbfc,  0x02494650);	//0x	124 to 0x	125
	rtd_outl(0xb802bc00,  0x22000000);	//0x	126 to 0x	127
	rtd_outl(0xb802bc04,  0x40002328);	//0x	128 to 0x	129
	rtd_outl(0xb802bc08,  0x3fff2134);	//0x	130 to 0x	131
	rtd_outl(0xb802bc0c,  0x3fff34bc);	//0x	132 to 0x	133
	rtd_outl(0xb802bc10,  0x09c40000);	//0x	134 to 0x	135
	rtd_outl(0xb802bc14,  0xa800000c);	//0x	136 to 0x	137
	rtd_outl(0xb802bc18,  0x00010000);	//0x	138 to 0x	139
	rtd_outl(0xb802bc1c,  0x44000001);	//0x	140 to 0x	141
	rtd_outl(0xb802bc20,  0x54000006);	//0x	142 to 0x	143
	rtd_outl(0xb802bc24,  0xa800000c);	//0x	144 to 0x	145
	rtd_outl(0xb802bc28,  0xfc000012);	//0x	146 to 0x	147
	rtd_outl(0xb802bc2c,  0x50000019);	//0x	148 to 0x	149
	rtd_outl(0xb802bc30,  0xa400001f);	//0x	150 to 0x	151
	rtd_outl(0xb802bc34,  0xf0f8e0e8);	//0x	152 to 0x	153
	rtd_outl(0xb802bc38,  0xd0d800c8);	//0x	154 to 0x	155
	rtd_outl(0xb802bc3c,  0x0006c8d8);	//0x	156 to 0x	157
	rtd_outl(0xb802bc40,  0xa8b88898);	//0x	158 to 0x	159
	rtd_outl(0xb802bc44,  0x00781b58);	//0x	160 to 0x	161
	rtd_outl(0xb802bc48,  0x1b581b58);	//0x	162 to 0x	163
	rtd_outl(0xb802bc4c,  0x1b581b58);	//0x	164 to 0x	165
	rtd_outl(0xb802bc50,  0x1b581b58);	//0x	166 to 0x	167
	rtd_outl(0xb802bc54,  0x1000b000);	//0x	168 to 0x	169
	rtd_outl(0xb802bc58,  0x50003000);	//0x	170 to 0x	171
	rtd_outl(0xb802bc5c,  0x90007000);	//0x	172 to 0x	173
	rtd_outl(0xb802bc60,  0x70005000);	//0x	174 to 0x	175
	rtd_outl(0xb802bc64,  0xb0009000);	//0x	176 to 0x	177
	rtd_outl(0xb802bc68,  0x30001000);	//0x	178 to 0x	179
	rtd_outl(0xb802bc6c,  0x00004000);	//0x	180 to 0x	181
	rtd_outl(0xb802bc70,  0x03343fff);	//0x	182 to 0x	183
	rtd_outl(0xb802bc74,  0x00018080);	//0x	184 to 0x	185
	rtd_outl(0xb802bc78,  0x80808080);	//0x	186 to 0x	187
	rtd_outl(0xb802bc7c,  0x80808080);	//0x	188 to 0x	189
	rtd_outl(0xb802bc80,  0x80808080);	//0x	190 to 0x	191
	rtd_outl(0xb802bc84,  0x80808080);	//0x	192 to 0x	193
	rtd_outl(0xb802bc88,  0x80808080);	//0x	194 to 0x	195
	rtd_outl(0xb802bc8c,  0x80808080);	//0x	196 to 0x	197
	rtd_outl(0xb802bc90,  0x80808080);	//0x	198 to 0x	199
	rtd_outl(0xb802bc94,  0x80808080);	//0x	200 to 0x	201
	rtd_outl(0xb802bc98,  0x80808080);	//0x	202 to 0x	203
	rtd_outl(0xb802bc9c,  0x80808080);	//0x	204 to 0x	205
	rtd_outl(0xb802bca0,  0x80808080);	//0x	206 to 0x	207
	rtd_outl(0xb802bca4,  0x808003f7);	//0x	208 to 0x	209
	rtd_outl(0xb802bca8,  0x10001000);	//0x	210 to 0x	211
	rtd_outl(0xb802bcac,  0x10001000);	//0x	212 to 0x	213
	rtd_outl(0xb802bcb0,  0x10001000);	//0x	214 to 0x	215
	rtd_outl(0xb802bcb4,  0x10001000);	//0x	216 to 0x	217
	rtd_outl(0xb802bcb8,  0x04000000);	//0x	218 to 0x	219
	rtd_outl(0xb802bcbc,  0x7e90007e);	//0x	220 to 0x	221
	rtd_outl(0xb802bcc0,  0x04000002);	//0x	222 to 0x	223
	rtd_outl(0xb802bcc4,  0x01ff0003);	//0x	224 to 0x	225
	rtd_outl(0xb802bcc8,  0x001e2580);	//0x	226 to 0x	227
	rtd_outl(0xb802bccc,  0x01400700);	//0x	228 to 0x	229
	rtd_outl(0xb802bcd0,  0x20001000);	//0x	230 to 0x	231
	rtd_outl(0xb802bcd4,  0x00060180);	//0x	232 to 0x	233
	rtd_outl(0xb802bcd8,  0x00001000);	//0x	234 to 0x	235
	rtd_outl(0xb802bcdc,  0x00102000);	//0x	236 to 0x	237
	rtd_outl(0xb802bce0,  0x00000000);	//0x	238 to 0x	239
	rtd_outl(0xb802bce4,  0x00190400);	//0x	240 to 0x	241
	rtd_outl(0xb802bce8,  0x00080000);	//0x	242 to 0x	243
	rtd_outl(0xb802bcec,  0x12491000);	//0x	244 to 0x	245
	rtd_outl(0xb802bcf0,  0x00100249);	//0x	246 to 0x	247
	rtd_outl(0xb802bcf4,  0x02490249);	//0x	248 to 0x	249
	rtd_outl(0xb802bcf8,  0x02490249);	//0x	250 to 0x	251
	rtd_outl(0xb802bcfc,  0x02494650);	//0x	252 to 0x	253
	rtd_outl(0xb8026c00,  0x22000000);	//0x	254 to 0x	255
	rtd_outl(0xb8026c04,  0x40002328);	//0x	256 to 0x	257
	rtd_outl(0xb8026c08,  0x3fff2134);	//0x	258 to 0x	259
	rtd_outl(0xb8026c0c,  0x3fff34bc);	//0x	260 to 0x	261
	rtd_outl(0xb8026c10,  0x005a0000);	//0x	262 to 0x	263
	rtd_outl(0xb8026c14,  0xa800000c);	//0x	264 to 0x	265
	rtd_outl(0xb8026c18,  0x00010000);	//0x	266 to 0x	267
	rtd_outl(0xb8026c1c,  0x44000001);	//0x	268 to 0x	269
	rtd_outl(0xb8026c20,  0x54000006);	//0x	270 to 0x	271
	rtd_outl(0xb8026c24,  0xa800000c);	//0x	272 to 0x	273
	rtd_outl(0xb8026c28,  0xfc000012);	//0x	274 to 0x	275
	rtd_outl(0xb8026c2c,  0x50000019);	//0x	276 to 0x	277
	rtd_outl(0xb8026c30,  0xa400001f);	//0x	278 to 0x	279
	rtd_outl(0xb8026c34,  0xf0f8e0e8);	//0x	280 to 0x	281
	rtd_outl(0xb8026c38,  0xd0d800c8);	//0x	282 to 0x	283
	rtd_outl(0xb8026c3c,  0x0006c8d8);	//0x	284 to 0x	285
	rtd_outl(0xb8026c40,  0xa8b88898);	//0x	286 to 0x	287
	rtd_outl(0xb8026c44,  0x00781b58);	//0x	288 to 0x	289
	rtd_outl(0xb8026c48,  0x1b581b58);	//0x	290 to 0x	291
	rtd_outl(0xb8026c4c,  0x1b581b58);	//0x	292 to 0x	293
	rtd_outl(0xb8026c50,  0x1b581b58);	//0x	294 to 0x	295
	rtd_outl(0xb8026c54,  0x1000b000);	//0x	296 to 0x	297
	rtd_outl(0xb8026c58,  0x50003000);	//0x	298 to 0x	299
	rtd_outl(0xb8026c5c,  0x90007000);	//0x	300 to 0x	301
	rtd_outl(0xb8026c60,  0x70005000);	//0x	302 to 0x	303
	rtd_outl(0xb8026c64,  0xb0009000);	//0x	304 to 0x	305
	rtd_outl(0xb8026c68,  0x30001000);	//0x	306 to 0x	307
	rtd_outl(0xb8026c6c,  0x00004000);	//0x	308 to 0x	309
	rtd_outl(0xb8026c70,  0x03343fff);	//0x	310 to 0x	311
	rtd_outl(0xb8026c74,  0x00018080);	//0x	312 to 0x	313
	rtd_outl(0xb8026c78,  0x80808080);	//0x	314 to 0x	315
	rtd_outl(0xb8026c7c,  0x80808080);	//0x	316 to 0x	317
	rtd_outl(0xb8026c80,  0x80808080);	//0x	318 to 0x	319
	rtd_outl(0xb8026c84,  0x80808080);	//0x	320 to 0x	321
	rtd_outl(0xb8026c88,  0x80808080);	//0x	322 to 0x	323
	rtd_outl(0xb8026c8c,  0x80808080);	//0x	324 to 0x	325
	rtd_outl(0xb8026c90,  0x80808080);	//0x	326 to 0x	327
	rtd_outl(0xb8026c94,  0x80808080);	//0x	328 to 0x	329
	rtd_outl(0xb8026c98,  0x80808080);	//0x	330 to 0x	331
	rtd_outl(0xb8026c9c,  0x80808080);	//0x	332 to 0x	333
	rtd_outl(0xb8026ca0,  0x80808080);	//0x	334 to 0x	335
	rtd_outl(0xb8026ca4,  0x808001E3);	//0x	336 to 0x	337
	rtd_outl(0xb8026ca8,  0x10001000);	//0x	338 to 0x	339
	rtd_outl(0xb8026cac,  0x10001000);	//0x	340 to 0x	341
	rtd_outl(0xb8026cb0,  0x10001000);	//0x	342 to 0x	343
	rtd_outl(0xb8026cb4,  0x10001000);	//0x	344 to 0x	345
	rtd_outl(0xb8026cb8,  0x04000000);	//0x	346 to 0x	347
	rtd_outl(0xb8026cbc,  0x7e90000e);	//0x	348 to 0x	349
	rtd_outl(0xb8026cc0,  0x00000000);	//0x	350 to 0x	351
	rtd_outl(0xb8026cc4,  0x00000003);	//0x	352 to 0x	353
	rtd_outl(0xb8026cc8,  0x001e2a30);	//0x	354 to 0x	355
	rtd_outl(0xb8026ccc,  0x02d007d0);	//0x	356 to 0x	357
	rtd_outl(0xb8026cd0,  0x20001000);	//0x	358 to 0x	359
	rtd_outl(0xb8026cd4,  0x00060180);	//0x	360 to 0x	361
	rtd_outl(0xb8026cd8,  0x00000800);	//0x	362 to 0x	363
	rtd_outl(0xb8026cdc,  0x00102000);	//0x	364 to 0x	365
	rtd_outl(0xb8026ce0,  0x00000000);	//0x	366 to 0x	367
	rtd_outl(0xb8026ce4,  0x00190400);	//0x	368 to 0x	369
	rtd_outl(0xb8026ce8,  0x00080000);	//0x	370 to 0x	371
	rtd_outl(0xb8026cec,  0x12491000);	//0x	372 to 0x	373
	rtd_outl(0xb8026cf0,  0x00100249);	//0x	374 to 0x	375
	rtd_outl(0xb8026cf4,  0x02490249);	//0x	376 to 0x	377
	rtd_outl(0xb8026cf8,  0x02490249);	//0x	378 to 0x	379
	rtd_outl(0xb8026cfc,  0x02494650);	//0x	380 to 0x	381
	rtd_outl(0xb8026d00,  0x22000000);	//0x	382 to 0x	383
	rtd_outl(0xb8026d04,  0x40002328);	//0x	384 to 0x	385
	rtd_outl(0xb8026d08,  0x3fff2134);	//0x	386 to 0x	387
	rtd_outl(0xb8026d0c,  0x3fff34bc);	//0x	388 to 0x	389
	rtd_outl(0xb8026d10,  0x09c4005a);	//0x	390 to 0x	391
	rtd_outl(0xb8026d14,  0xa800000c);	//0x	392 to 0x	393
	rtd_outl(0xb8026d18,  0x00010000);	//0x	394 to 0x	395
	rtd_outl(0xb8026d1c,  0x44000001);	//0x	396 to 0x	397
	rtd_outl(0xb8026d20,  0x54000006);	//0x	398 to 0x	399
	rtd_outl(0xb8026d24,  0xa800000c);	//0x	400 to 0x	401
	rtd_outl(0xb8026d28,  0xfc000012);	//0x	402 to 0x	403
	rtd_outl(0xb8026d2c,  0x50000019);	//0x	404 to 0x	405
	rtd_outl(0xb8026d30,  0xa400001f);	//0x	406 to 0x	407
	rtd_outl(0xb8026d34,  0xf0f8e0e8);	//0x	408 to 0x	409
	rtd_outl(0xb8026d38,  0xd0d800c8);	//0x	410 to 0x	411
	rtd_outl(0xb8026d3c,  0x0006c8d8);	//0x	412 to 0x	413
	rtd_outl(0xb8026d40,  0xa8b88898);	//0x	414 to 0x	415
	rtd_outl(0xb8026d44,  0x00781b58);	//0x	416 to 0x	417
	rtd_outl(0xb8026d48,  0x1b581b58);	//0x	418 to 0x	419
	rtd_outl(0xb8026d4c,  0x1b581b58);	//0x	420 to 0x	421
	rtd_outl(0xb8026d50,  0x1b581b58);	//0x	422 to 0x	423
	rtd_outl(0xb8026d54,  0x1000b000);	//0x	424 to 0x	425
	rtd_outl(0xb8026d58,  0x50003000);	//0x	426 to 0x	427
	rtd_outl(0xb8026d5c,  0x90007000);	//0x	428 to 0x	429
	rtd_outl(0xb8026d60,  0x70005000);	//0x	430 to 0x	431
	rtd_outl(0xb8026d64,  0xb0009000);	//0x	432 to 0x	433
	rtd_outl(0xb8026d68,  0x30001000);	//0x	434 to 0x	435
	rtd_outl(0xb8026d6c,  0x00004000);	//0x	436 to 0x	437
	rtd_outl(0xb8026d70,  0x03343fff);	//0x	438 to 0x	439
	rtd_outl(0xb8026d74,  0x00018080);	//0x	440 to 0x	441
	rtd_outl(0xb8026d78,  0x80808080);	//0x	442 to 0x	443
	rtd_outl(0xb8026d7c,  0x80808080);	//0x	444 to 0x	445
	rtd_outl(0xb8026d80,  0x80808080);	//0x	446 to 0x	447
	rtd_outl(0xb8026d84,  0x80808080);	//0x	448 to 0x	449
	rtd_outl(0xb8026d88,  0x80808080);	//0x	450 to 0x	451
	rtd_outl(0xb8026d8c,  0x80808080);	//0x	452 to 0x	453
	rtd_outl(0xb8026d90,  0x80808080);	//0x	454 to 0x	455
	rtd_outl(0xb8026d94,  0x80808080);	//0x	456 to 0x	457
	rtd_outl(0xb8026d98,  0x80808080);	//0x	458 to 0x	459
	rtd_outl(0xb8026d9c,  0x80808080);	//0x	460 to 0x	461
	rtd_outl(0xb8026da0,  0x80808080);	//0x	462 to 0x	463
	rtd_outl(0xb8026da4,  0x808001E7);	//0x	464 to 0x	465
	rtd_outl(0xb8026da8,  0x10001000);	//0x	466 to 0x	467
	rtd_outl(0xb8026dac,  0x10001000);	//0x	468 to 0x	469
	rtd_outl(0xb8026db0,  0x10001000);	//0x	470 to 0x	471
	rtd_outl(0xb8026db4,  0x10001000);	//0x	472 to 0x	473
	rtd_outl(0xb8026db8,  0x04000000);	//0x	474 to 0x	475
	rtd_outl(0xb8026dbc,  0x7e90000e);	//0x	476 to 0x	477
	rtd_outl(0xb8026dc0,  0x04000008);	//0x	478 to 0x	479
	rtd_outl(0xb8026dc4,  0x00400003);	//0x	480 to 0x	481
	rtd_outl(0xb8026dc8,  0x001e2a30);	//0x	482 to 0x	483
	rtd_outl(0xb8026dcc,  0x02d007d0);	//0x	484 to 0x	485
	rtd_outl(0xb8026dd0,  0x20001000);	//0x	486 to 0x	487
	rtd_outl(0xb8026dd4,  0x00060180);	//0x	488 to 0x	489
	rtd_outl(0xb8026dd8,  0x00000800);	//0x	490 to 0x	491
	rtd_outl(0xb8026ddc,  0x00102000);	//0x	492 to 0x	493
	rtd_outl(0xb8026de0,  0x00000000);	//0x	494 to 0x	495
	rtd_outl(0xb8026de4,  0x00190400);	//0x	496 to 0x	497
	rtd_outl(0xb8026de8,  0x00080000);	//0x	498 to 0x	499
	rtd_outl(0xb8026dec,  0x12491000);	//0x	500 to 0x	501
	rtd_outl(0xb8026df0,  0x00100249);	//0x	502 to 0x	503
	rtd_outl(0xb8026df4,  0x02490249);	//0x	504 to 0x	505
	rtd_outl(0xb8026df8,  0x02490249);	//0x	506 to 0x	507
	rtd_outl(0xb8026dfc,  0x02494650);	//0x	508 to 0x	509
	rtd_outl(0xb8026e00,  0x22000000);	//0x	510 to 0x	511
	rtd_outl(0xb8026e04,  0x40002328);	//0x	512 to 0x	513
	rtd_outl(0xb8026e08,  0x3fff2134);	//0x	514 to 0x	515
	rtd_outl(0xb8026e0c,  0x3fff34bc);	//0x	516 to 0x	517
	rtd_outl(0xb8026e10,  0x005a0000);	//0x	518 to 0x	519
	rtd_outl(0xb8026e14,  0xa800000c);	//0x	520 to 0x	521
	rtd_outl(0xb8026e18,  0x00010000);	//0x	522 to 0x	523
	rtd_outl(0xb8026e1c,  0x44000001);	//0x	524 to 0x	525
	rtd_outl(0xb8026e20,  0x54000006);	//0x	526 to 0x	527
	rtd_outl(0xb8026e24,  0xa800000c);	//0x	528 to 0x	529
	rtd_outl(0xb8026e28,  0xfc000012);	//0x	530 to 0x	531
	rtd_outl(0xb8026e2c,  0x50000019);	//0x	532 to 0x	533
	rtd_outl(0xb8026e30,  0xa400001f);	//0x	534 to 0x	535
	rtd_outl(0xb8026e34,  0xf0f8e0e8);	//0x	536 to 0x	537
	rtd_outl(0xb8026e38,  0xd0d800c8);	//0x	538 to 0x	539
	rtd_outl(0xb8026e3c,  0x0006c8d8);	//0x	540 to 0x	541
	rtd_outl(0xb8026e40,  0xa8b88898);	//0x	542 to 0x	543
	rtd_outl(0xb8026e44,  0x00781b58);	//0x	544 to 0x	545
	rtd_outl(0xb8026e48,  0x1b581b58);	//0x	546 to 0x	547
	rtd_outl(0xb8026e4c,  0x1b581b58);	//0x	548 to 0x	549
	rtd_outl(0xb8026e50,  0x1b581b58);	//0x	550 to 0x	551
	rtd_outl(0xb8026e54,  0x1000b000);	//0x	552 to 0x	553
	rtd_outl(0xb8026e58,  0x50003000);	//0x	554 to 0x	555
	rtd_outl(0xb8026e5c,  0x90007000);	//0x	556 to 0x	557
	rtd_outl(0xb8026e60,  0x70005000);	//0x	558 to 0x	559
	rtd_outl(0xb8026e64,  0xb0009000);	//0x	560 to 0x	561
	rtd_outl(0xb8026e68,  0x30001000);	//0x	562 to 0x	563
	rtd_outl(0xb8026e6c,  0x00004000);	//0x	564 to 0x	565
	rtd_outl(0xb8026e70,  0x03343fff);	//0x	566 to 0x	567
	rtd_outl(0xb8026e74,  0x00018080);	//0x	568 to 0x	569
	rtd_outl(0xb8026e78,  0x80808080);	//0x	570 to 0x	571
	rtd_outl(0xb8026e7c,  0x80808080);	//0x	572 to 0x	573
	rtd_outl(0xb8026e80,  0x80808080);	//0x	574 to 0x	575
	rtd_outl(0xb8026e84,  0x80808080);	//0x	576 to 0x	577
	rtd_outl(0xb8026e88,  0x80808080);	//0x	578 to 0x	579
	rtd_outl(0xb8026e8c,  0x80808080);	//0x	580 to 0x	581
	rtd_outl(0xb8026e90,  0x80808080);	//0x	582 to 0x	583
	rtd_outl(0xb8026e94,  0x80808080);	//0x	584 to 0x	585
	rtd_outl(0xb8026e98,  0x80808080);	//0x	586 to 0x	587
	rtd_outl(0xb8026e9c,  0x80808080);	//0x	588 to 0x	589
	rtd_outl(0xb8026ea0,  0x80808080);	//0x	590 to 0x	591
	rtd_outl(0xb8026ea4,  0x80801000);	//0x	592 to 0x	593
	rtd_outl(0xb8026ea8,  0xb0005000);	//0x	594 to 0x	595
	rtd_outl(0xb8026eac,  0x30009000);	//0x	596 to 0x	597
	rtd_outl(0xb8026eb0,  0x70007000);	//0x	598 to 0x	599
	rtd_outl(0xb8026eb4,  0x5000b000);	//0x	600 to 0x	601
	rtd_outl(0xb8026eb8,  0x90003000);	//0x	602 to 0x	603
	rtd_outl(0xb8026ebc,  0x00000000);	//0x	604 to 0x	605
	rtd_outl(0xb8026ec0,  0x40000334);	//0x	606 to 0x	607
	rtd_outl(0xb8026ec4,  0x3fff0001);	//0x	608 to 0x	609
	rtd_outl(0xb8026ec8,  0x12491000);	//0x	610 to 0x	611
	rtd_outl(0xb8026ecc,  0xb0005000);	//0x	612 to 0x	613
	rtd_outl(0xb8026ed0,  0x30009000);	//0x	614 to 0x	615
	rtd_outl(0xb8026ed4,  0x70007000);	//0x	616 to 0x	617
	rtd_outl(0xb8026ed8,  0x5000b000);	//0x	618 to 0x	619
	rtd_outl(0xb8026edc,  0x90003000);	//0x	620 to 0x	621
	rtd_outl(0xb8026ee0,  0x00000000);	//0x	622 to 0x	623
	rtd_outl(0xb8026ee4,  0x40000334);	//0x	624 to 0x	625
	rtd_outl(0xb8026ee8,  0x3fff0001);	//0x	626 to 0x	627
	rtd_outl(0xb8026eec,  0x12491000);	//0x	628 to 0x	629
	rtd_outl(0xb8026ef0,  0xb0005000);	//0x	630 to 0x	631
	rtd_outl(0xb8026ef4,  0x30009000);	//0x	632 to 0x	633
	rtd_outl(0xb8026ef8,  0x70007000);	//0x	634 to 0x	635
	rtd_outl(0xb8026efc,  0x5000b000);	//0x	636 to 0x	637
	rtd_outl(0xb8026f00,  0x90003000);	//0x	638 to 0x	639
	rtd_outl(0xb8026f04,  0x00000000);	//0x	640 to 0x	641
	rtd_outl(0xb8026f08,  0x40000334);	//0x	642 to 0x	643
	rtd_outl(0xb8026f0c,  0x3fff0001);	//0x	644 to 0x	645
	rtd_outl(0xb8026f10,  0x12491000);	//0x	646 to 0x	647
	rtd_outl(0xb8026f14,  0xb0005000);	//0x	648 to 0x	649
	rtd_outl(0xb8026f18,  0x30009000);	//0x	650 to 0x	651
	rtd_outl(0xb8026f1c,  0x70007000);	//0x	652 to 0x	653
	rtd_outl(0xb8026f20,  0x5000b000);	//0x	654 to 0x	655
	rtd_outl(0xb8026f24,  0x90003000);	//0x	656 to 0x	657
	rtd_outl(0xb8026f28,  0x00000000);	//0x	658 to 0x	659
	rtd_outl(0xb8026f2c,  0x40000334);	//0x	660 to 0x	661
	rtd_outl(0xb8026f30,  0x3fff0001);	//0x	662 to 0x	663
	rtd_outl(0xb8026f34,  0x12490000);	//0x	664 to 0x	665
	rtd_outl(0xb8026f38,  0x00000000);	//0x	666 to 0x	667
	rtd_outl(0xb8026f3c,  0x00000000);	//0x	668 to 0x	669
	rtd_outl(0xb8026f40,  0x00000000);	//0x	670 to 0x	671
	rtd_outl(0xb8026f44,  0x00000000);	//0x	672 to 0x	673
	rtd_outl(0xb8026f48,  0x00000000);	//0x	674 to 0x	675
	rtd_outl(0xb8026f4c,  0x00000000);	//0x	676 to 0x	677
	rtd_outl(0xb8026f50,  0x00000000);	//0x	678 to 0x	679
	rtd_outl(0xb8026f54,  0x00000000);	//0x	680 to 0x	681
	rtd_outl(0xb8026f58,  0x00000000);	//0x	682 to 0x	683
	rtd_outl(0xb8026f5c,  0x00000000);	//0x	684 to 0x	685
	rtd_outl(0xb8026f60,  0x00000000);	//0x	686 to 0x	687
	rtd_outl(0xb8026f64,  0x00000000);	//0x	688 to 0x	689
	rtd_outl(0xb8026f68,  0x00000000);	//0x	690 to 0x	691
	rtd_outl(0xb8026f6c,  0x00000000);	//0x	692 to 0x	693
	rtd_outl(0xb8026f70,  0x00000000);	//0x	694 to 0x	695
	rtd_outl(0xb8026f74,  0x00000000);	//0x	696 to 0x	697
	rtd_outl(0xb8026f78,  0x00000000);	//0x	698 to 0x	699
	rtd_outl(0xb8026f7c,  0x00000000);	//0x	700 to 0x	701
	rtd_outl(0xb8026f80,  0x00000000);	//0x	702 to 0x	703
	rtd_outl(0xb8026f84,  0x00000000);	//0x	704 to 0x	705
	rtd_outl(0xb8026f88,  0x00000000);	//0x	706 to 0x	707
	rtd_outl(0xb8026f8c,  0x00000000);	//0x	708 to 0x	709
	rtd_outl(0xb8026f90,  0x00000000);	//0x	710 to 0x	711
	rtd_outl(0xb8026f94,  0x00000000);	//0x	712 to 0x	713
	rtd_outl(0xb8026f98,  0x00000000);	//0x	714 to 0x	715
	rtd_outl(0xb8026f9c,  0x00000000);	//0x	716 to 0x	717
	rtd_outl(0xb8026fa0,  0x00000000);	//0x	718 to 0x	719
	rtd_outl(0xb8026fa4,  0x00000000);	//0x	720 to 0x	721
	rtd_outl(0xb8026fa8,  0x00000000);	//0x	722 to 0x	723
	rtd_outl(0xb8026fac,  0x00000000);	//0x	724 to 0x	725
	rtd_outl(0xb8026fb0,  0x00000000);	//0x	726 to 0x	727
	rtd_outl(0xb8026fb4,  0x00000000);	//0x	728 to 0x	729
	rtd_outl(0xb8026fb8,  0x00000000);	//0x	730 to 0x	731
	rtd_outl(0xb8026fbc,  0x00000000);	//0x	732 to 0x	733
	rtd_outl(0xb8026fc0,  0x00000000);	//0x	734 to 0x	735
	rtd_outl(0xb8026fc4,  0x00000000);	//0x	736 to 0x	737
	rtd_outl(0xb8026fc8,  0x00000000);	//0x	738 to 0x	739
	rtd_outl(0xb8026fcc,  0x00000000);	//0x	740 to 0x	741
	rtd_outl(0xb8026fd0,  0x00000000);	//0x	742 to 0x	743
	rtd_outl(0xb8026fd4,  0x00000000);	//0x	744 to 0x	745
	rtd_outl(0xb8026fd8,  0x00000000);	//0x	746 to 0x	747
	rtd_outl(0xb8026fdc,  0x00000000);	//0x	748 to 0x	749


	//crt
	//rtd_outl(0xb800010c,  0x00008001); // siw rst_n pull high
	//rtd_outl(0xb800011c,  0x00200001); // mplus clken on
	//rtd_outl(0xb800010c,  0x00008000); // siw rst_n pull low
	//rtd_outl(0xb800010c,  0x00008001); // siw rst_n pull high

	//clk_enable
	//rtd_outl(0xb8028000,0x00000000); // disp disable
	//rtd_maskl(0xb8028004,0xFF87FFFF,0x00200000); //mplus enable, reserved, siw-rgbw disable, reserved
	//rtd_maskl(0xb8028004,0xFFFFFFCF,0x00000010); //mplus enable, siw-rgbw disable, reserved
	//rtd_outl(0xb8028000,0x01000000); // disp enable
...
static unsigned char orbit_gamemode_resume=0;
static unsigned char reset_sub=0;
...
void vbe_disp_orbit_set_gamemode_resume(unsigned char bEnable)
{
	orbit_gamemode_resume = bEnable;
}
unsigned char vbe_disp_orbit_get_gamemode_resume(void)
{
	return orbit_gamemode_resume;
}
...
unsigned char vbe_disp_orbit_get_reset_sub(void)
{
	return reset_sub;
}
void vbe_disp_orbit_set_reset_sub(unsigned char bEnable)
{
	reset_sub = bEnable;
}
...
void vbe_disp_orbit_vo_overscan()
{
	...
	*/
	scalerOrbitMemInfo.orbit_enable = 1;
	scalerOrbitMemInfo.orbit_mode = vbe_disp_oled_orbit_mode;
...
void vbe_disp_orbit_frc_overscan()
{
	...
	inSize.nLength =Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_MEM_ACT_LEN); //scaledown_ich1_sdnr_cutout_range_ver0_reg.ver_height;// Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_MEM_ACT_LEN);
	inSize.nWidth =Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_MEM_ACT_WID); //scaledown_ich1_sdnr_cutout_range_hor_reg.hor_width;// Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_MEM_ACT_WID);
...
void vbe_disp_orbit_reset_position_overscan(void)
{
	...
	if((vbe_orbit_sub_input_size.src_wid !=0 || vbe_orbit_sub_input_size.src_height!=0))
	{
		vbe_disp_orbit_set_reset_sub(1);
		input_size.srcx = vbe_orbit_sub_input_size.srcx ;
		input_size.srcy = vbe_orbit_sub_input_size.srcy ;
		input_size.src_wid = vbe_orbit_sub_input_size.src_wid;
		input_size.src_height = vbe_orbit_sub_input_size.src_height;
...
void vbe_disp_orbit_reset_rotate_overscan(void)
{
	ORBIT_SIZE inSize;
...
/*api which have protect by forcebg_semaphore*/
void vbe_disp_orbit_position_update(unsigned int tableIdx,unsigned char force_update)
{
//	ppoverlay_double_buffer_ctrl2_RBUS double_buffer_ctrl2_reg;
	static unsigned int previous_tableIdx=0x100;
//	ppoverlay_display_timing_ctrl2_RBUS ppoverlay_display_timing_ctrl2_reg;
//	ppoverlay_memcdtg_control_4_RBUS 	ppoverlay_memcdtg_control_4_reg;
