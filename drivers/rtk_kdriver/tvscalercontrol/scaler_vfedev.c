...
static struct semaphore AVD_ChannelChangeSemaphore;/*This semaphore is set avd channel change flag*/
	...
	if ((VBI_support_type_get() !=KADP_VFE_AVD_CC_IS_SUPPORTED) &&((vdc_mode == VDC_MODE_PALI)||(vdc_mode == VDC_MODE_NTSC50)||(vdc_mode == VDC_MODE_SECAM))) {//tt setting
		...
		if (vdc_mode==VDC_MODE_PALN) {
			if ((drvif_module_vdc_read_coch_detect_status() == 1)&&
				...
				vdcf_outl(VBI_TT_VBI_Control_Register_reg, 0x00702713);
			} else {
...
{
	rtd_printk(KERN_NOTICE, TAG_NAME, "#####[%s(%d)] VSC_INPUTSRC %d online measure main Error Ack\n",__func__,__LINE__,Get_DisplayMode_Src(SLR_MAIN_DISPLAY));
...
int vfe_module_init(void)
{
	...
	sema_init(&AVD_ChannelChangeSemaphore, 1);
...
struct semaphore* get_avd_channelchangeSemaphore(void)
{
	return &AVD_ChannelChangeSemaphore;
}
