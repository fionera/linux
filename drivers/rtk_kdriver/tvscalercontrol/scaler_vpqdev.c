...
UINT8 g_LGE_POD_Enable_flag = 0;
UINT8 g_color_temp_init =0;
...
//extern webos_info_t  webos_tooloption;
extern webos_strInfo_t webos_strToolOption;
extern void scalerAI_obj_info_passed_from_NN(AIInfo *nninfo);
CHIP_CM_REGION_T g_cm_rgn_adv = {{
	{{	TRUE,
		...
		{0, 182, 384, 512, 720, 1024, 1536, 2048, 2564, 3072, 3584, 4096 }, //SAT_index //0~4095
		{128, 215, 230, 240, 220, 210, 210, 210, 200, 201, 180, 130 }, //SAT_gain //0~255
		{0, 585, 1170, 1752, 2340, 2925, 3510, 4096 }, //ITN_index //0~4095
		{128, 250, 250, 250, 250, 180, 180, 160 }, //ITN_gain //0~255
		{0, 0, 0, 0, 0, 0, 0, 0, }, //R offset by hue,	   //0~2048 ==> -1024~1023
		{0, 0, 0, 0, 0, 0, 0, 0, }, //G offset by hue,	   //0~2048 ==> -1024~1023
		{0, 0, 0, 0, 0, 0, 0, 0, }, //B offset by hue,	   //0~2048 ==> -1024~1023
	},
	...
	{128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128},	/* sat by sat gain */
	{128, 128, 128, 128, 128, 128, 128, 128},	/* sat by Int gain */
	{32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32}, //gain by sat for offset by hue,	   //0~64
...
CHIP_CM_RESULT_T g_cm_v4l2;
 VPQ_SAT_LUT_T satLUTData_in_CM = {{0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80},
...
COLORTEMP_ELEM_T curColorTemp = {512, 512, 512, 512, 512, 512};
...
// lesley 0905
UINT8 v4l2_vpq_stereo_face = 0;//0: off, 1: on, 2: demo
UINT8 v4l2_vpq_stereo_face_pre = 0;//0: off, 1: on, 2: demo
// end lesley 0905
...
unsigned char vpq_set_Gamut_3D_Lut_LGDB(GAMUT_3D_LUT_17x17x17_T *pLut, unsigned int iEnable, unsigned char bOnlySetEnable)
{
	...
	if (bOnlySetEnable && iEnable) {
		...
		if (iEnable == 1)
			fwif_color_set_DDomain_3dLUT_16_TV006(pArray, 1);
		else
...
extern sld_work_struct sld_work;
extern unsigned char BFI_init;
...
void vpq_boot_init(void)
{
	...
	/* HDR10+ Ctrl ini*/
	fwif_color_HDR10p_Ctrl_Structure_ini();
	...
	drvif_module_yc_boot_init();
        fwif_color_colorwrite_InvOutput_gamma(2);
...
void vpq_do_resume(void)
{
	...
	if (g_LGE_POD_Enable_flag) {
		drvif_color_enable_LGD_POD_CLK();	/* if POD need to enable, set POD CLK */
...
static int VPQ_detect_tsk(void *p)
{
    ...
    while (1)
    {
		...
		if( LD_virAddr == NULL && MEMC_First_Run_force_SetInOutUseCase_Done == 1 )
		{
...
/*========================= for INNX demura =================================*/
		...
		case VPQ_IOC_SET_AIinfo:
		{
			AIInfo AIinfo_ioctrl[6];
			//rtd_printk(KERN_EMERG, TAG_NAME, "[henry][ioctrl] ap addr = %p\n", (int __user *)arg);
			//rtd_printk(KERN_EMERG, TAG_NAME, "[henry][ioctrl] kernel addr = %p\n", AIinfo_ioctrl);

			if (copy_from_user(AIinfo_ioctrl, (const void __user *)arg, 6*sizeof(AIInfo))) {
				rtd_printk(KERN_ERR, TAG_NAME, "kernel copy VPQ_IOC_SET_AIinfo fail\n");
				ret = -1;
			} else {

				/*int i;
				for(i=0;i<6;i++) {
					rtd_printk(KERN_EMERG, TAG_NAME, "[henry][ioctrl] face[%d]\n", i);
					rtd_printk(KERN_EMERG, TAG_NAME, "[henry][ioctrl]	pv8 = %d\n", AIinfo_ioctrl[i].pv8);
					rtd_printk(KERN_EMERG, TAG_NAME, "[henry][ioctrl]	cx12, cy12 = %d, %d\n", AIinfo_ioctrl[i].cx12, AIinfo_ioctrl[i].cy12);
					rtd_printk(KERN_EMERG, TAG_NAME, "[henry][ioctrl]	w12, h12 = %d, %d\n", AIinfo_ioctrl[i].w12, AIinfo_ioctrl[i].h12);
					rtd_printk(KERN_EMERG, TAG_NAME, "[henry][ioctrl]	range12 = %d\n", AIinfo_ioctrl[i].range12);
				}*/
				rtd_printk(KERN_ERR, TAG_NAME, "kernel copy VPQ_IOC_SET_AIinfo success\n");
				scalerAI_obj_info_passed_from_NN(AIinfo_ioctrl);
...
static unsigned char vpq_V4L2_CID_EXT_VPQ_BLACK_LEVEL_Mapping(struct v4l2_ext_vpq_black_level_info* v4l2_blackLevel)
{
	...
	if (v4l2_blackLevel != NULL) {
		if (v4l2_blackLevel->curr_input == HAL_VPQ_INPUT_ATV || v4l2_blackLevel->curr_input == HAL_VPQ_INPUT_AV) {
			level = RGB2YUV_BLACK_LEVEL_LOW;
		} else if (v4l2_blackLevel->ui_value == Blk_Lv_UI_LEVEL_AUTO && v4l2_blackLevel->curr_input != HAL_VPQ_INPUT_ATV && v4l2_blackLevel->curr_input != HAL_VPQ_INPUT_AV) {
            ...
            rtd_printk(KERN_EMERG, TAG_NAME, " func:%s [error] is null\r\n",__FUNCTION__);
	}
...
static int status = 0; // 0: finished. -1: not ready.
	...
	else if(stereo_face == V4L2_VPQ_EXT_STEREO_FACE_ON)
	{
		...
		scalerAI_ai_pq_on(stereo_face, dcValue);
	}
	else if(stereo_face == V4L2_VPQ_EXT_STEREO_FACE_DEMO)
	{
		printk("aipq path demo\n");
		scalerAI_ai_pq_on(stereo_face, 4);
	}
...
struct v4l2_ext_led_lut_RTK stReal_db_lut[LD_Table_NUM];
extern KADP_LED_DB_LUT_T LED_DB_LUT[LD_Table_NUM];
...
struct v4l2_ext_vpq_color_temp v4l2_color_temp={{192,192,192,},{64,64,64}};
...
static void assemble_winbox_win_attr(inner_pattern_winbox_win_attr *dest, struct v4l2_vpq_ext_pattern_winbox_win_attr *src)
{
	dest->x = src->x;
	dest->y = src->y;
	dest->w = src->w;
	dest->h = src->h;
	dest->fill_R = src->fill_R;
	dest->fill_G = src->fill_G;
	dest->fill_B = src->fill_B;
...
/*char vpq_V4L2_CID_EXT_VPQ_SHARPNESS_DC_check(CHIP_SHARPNESS_T *pCHIP_SHARPNESS_T)
{
	//char ret = 0;
	//unsigned int i;
	//short *ptemp;
...
char vpq_v4l2_hdrtype_compare_in_active(enum v4l2_ext_hdr_mode_RTK v4l2_hdr_type, unsigned char vpq_hdr_type)
{
	char ret_diff = 0;
	unsigned char v4l2_to_VPQ_hdrType;
	if (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) {
		if (v4l2_hdr_type == RTK_V4L2_EXT_HDR_MODE_HDR10) {
			v4l2_to_VPQ_hdrType = HAL_VPQ_HDR_MODE_HDR10;
		} else if (v4l2_hdr_type == RTK_V4L2_EXT_HDR_MODE_HLG) {
			v4l2_to_VPQ_hdrType = HAL_VPQ_HDR_MODE_HLG;
		} else {
			if (get_support_vo_force_v_top(support_sdr_max_rgb))
				v4l2_to_VPQ_hdrType = HAL_VPQ_HDR_MODE_SDR_MAX_RGB;
			else
				v4l2_to_VPQ_hdrType = HAL_VPQ_HDR_MODE_SDR;
		}

		if (vpq_hdr_type == v4l2_to_VPQ_hdrType) {
			ret_diff = 0;
		} else {
			ret_diff = 1;
			rtd_printk(KERN_INFO, TAG_NAME, "DM_freeze, compare, v4l2=%d, vpq=%d,\n", v4l2_hdr_type, vpq_hdr_type);
		}
	} else {
		ret_diff = 0 ;
	}
...
#else
char vpq_v4l2_hdrtype_compare(enum v4l2_ext_hdr_mode_RTK v4l2_hdr_type, unsigned char vpq_hdr_type)
{
	char ret_diff = 0;
	unsigned char v4l2_to_VPQ_hdrType;
	if (v4l2_hdr_type == RTK_V4L2_EXT_HDR_MODE_HDR10) {
	...
	switch(cmd)
        {
                ...
                case V4L2_CID_EXT_VPQ_SHARPNESS:
                {
						...
						rtd_printk(KERN_INFO, TAG_NAME, "get table from V4L2_CID_EXT_VPQ_SHARPNESS\n" );
                                }
                ...
                case V4L2_CID_EXT_VPQ_DYNAMIC_CONTRAST:
                {
                        ...
                        else
                        {
                               ...
                               /* rtd_printk(KERN_EMERG, TAG_NAME, "rord   dynamic_ctrl.uDcVal=%d \n", dynamic_ctrl.uDcVal);

                                rtd_printk(KERN_EMERG, TAG_NAME, "sizeof chroma_gain_en=%d \n",sizeof(tFreshContrast_coef.stChromaGain.chroma_gain_en));
                                rtd_printk(KERN_EMERG, TAG_NAME, "sizeof chroma_gain_mode=%d \n",sizeof(tFreshContrast_coef.stChromaGain.chroma_gain_mode));

                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_en=%d \n",tFreshContrast_coef.stChromaGain.chroma_gain_en);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_mode=%d \n",tFreshContrast_coef.stChromaGain.chroma_gain_mode);

                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_lookup_mode=%d \n",tFreshContrast_coef.stChromaGain.chroma_gain_lookup_mode);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_base=%d \n",tFreshContrast_coef.stChromaGain.chroma_gain_base);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_enhance_above_tab_select_main=%d \n",tFreshContrast_coef.stChromaGain.chroma_enhance_above_tab_select_main);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_enhance_below_tab_select_main=%d \n",tFreshContrast_coef.stChromaGain.chroma_enhance_below_tab_select_main);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC saturation_byy_u_tab_select_main=%d \n",tFreshContrast_coef.stChromaGain.saturation_byy_u_tab_select_main);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC saturation_byy_v_tab_select_main=%d \n",tFreshContrast_coef.stChromaGain.saturation_byy_v_tab_select_main);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_limit_en=%d \n",tFreshContrast_coef.stChromaGain.chroma_gain_limit_en);

                               for(i=0;i<21;i++){

                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_limit_seg[%d]=%d \n",i,tFreshContrast_coef.stChromaGain.chroma_gain_limit_seg[i]);
                                }

                                for(i=0;i<17;i++){
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_tab0_seg[%d]=%d \n",i,tFreshContrast_coef.stChromaGain.chroma_gain_tab0_seg[i]);
                                        }

                                for(i=0;i<17;i++){
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC chroma_gain_tab1_seg[%d]=%d \n",i,tFreshContrast_coef.stChromaGain.chroma_gain_tab1_seg[i]);
                                        }

                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y0_center =%d \n",tFreshContrast_coef.stSkinTone.range.y0_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u0_center =%d \n",tFreshContrast_coef.stSkinTone.range.u0_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v0_center =%d \n",tFreshContrast_coef.stSkinTone.range.v0_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y0_range =%d \n",tFreshContrast_coef.stSkinTone.range.y0_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u0_range =%d \n",tFreshContrast_coef.stSkinTone.range.u0_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v0_range =%d \n",tFreshContrast_coef.stSkinTone.range.v0_range);

                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y1_center =%d \n",tFreshContrast_coef.stSkinTone.range.y1_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u1_center =%d \n",tFreshContrast_coef.stSkinTone.range.u1_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v1_center =%d \n",tFreshContrast_coef.stSkinTone.range.v1_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y1_range =%d \n",tFreshContrast_coef.stSkinTone.range.y1_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u1_range =%d \n",tFreshContrast_coef.stSkinTone.range.u1_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v1_range =%d \n",tFreshContrast_coef.stSkinTone.range.v1_range);


                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y2_center =%d \n",tFreshContrast_coef.stSkinTone.range.y2_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u2_center =%d \n",tFreshContrast_coef.stSkinTone.range.u2_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v2_center =%d \n",tFreshContrast_coef.stSkinTone.range.v2_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y2_range =%d \n",tFreshContrast_coef.stSkinTone.range.y2_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u2_range =%d \n",tFreshContrast_coef.stSkinTone.range.u2_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v2_range =%d \n",tFreshContrast_coef.stSkinTone.range.v2_range);

                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y3_center =%d \n",tFreshContrast_coef.stSkinTone.range.y3_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u3_center =%d \n",tFreshContrast_coef.stSkinTone.range.u3_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v3_center =%d \n",tFreshContrast_coef.stSkinTone.range.v3_center);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC y3_range =%d \n",tFreshContrast_coef.stSkinTone.range.y3_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC u3_range =%d \n",tFreshContrast_coef.stSkinTone.range.u3_range);
                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC v3_range =%d \n",tFreshContrast_coef.stSkinTone.range.v3_range);

                                rtd_printk(KERN_EMERG, TAG_NAME, "DCC D_DCC_SKIN_EN =%d \n",tFreshContrast_coef.stSkinTone.value.D_DCC_SKIN_EN);

                                for(i=0;i<4;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC dcc_skin_region_en[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.dcc_skin_region_en[i]);
                                }
                                for(i=0;i<4;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC dcc_skin_region_enhance[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.dcc_skin_region_enhance[i]);
                                }

                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC Y0_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.Y0_seg[i]);
                                }
                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC Y1_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.Y1_seg[i]);
                                }
                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC Y2_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.Y2_seg[i]);
                                }
                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC Y3_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.Y3_seg[i]);
                                }

                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC c0_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.C0_seg[i]);
                                }
                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC c1_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.C1_seg[i]);
                                }
                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC c2_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.C2_seg[i]);
                                }
                                for(i=0;i<8;i++){
                                        rtd_printk(KERN_EMERG, TAG_NAME, "DCC c3_seg[%d]=%d \n",i,tFreshContrast_coef.stSkinTone.value.C3_seg[i]);
                                }*/
                ...
                case V4L2_CID_EXT_VPQ_CM_DB_DATA:
				{
					...
					if(!ext_control.ptr) {
						...
						if(cm_info.use_internal_cm_db) {
							ret = 0;
							rtd_printk(KERN_EMERG, TAG_NAME, "LG DB not ready, workaround  V4L2_CID_EXT_VPQ_CM_DB_DATA \n" );
						...
						rtd_printk(KERN_INFO, TAG_NAME, "LG DB mode V4L2_CID_EXT_VPQ_CM_DB_DATA \n" );
						if(copy_from_user((void *)&g_cm_v4l2, to_user_ptr(cm_info.dbInfo), sizeof(CHIP_CM_RESULT_T)))
						{
							rtd_printk(KERN_EMERG, TAG_NAME, "func:%s line:%d [error] copy_from_user controls fail [g_cm_v4l2]\r\n",__FUNCTION__,__LINE__);
							ret = -EFAULT;
							return ret;
						}
						/* LG ICM mode */
						...
						rtd_printk(KERN_INFO, TAG_NAME, "LG DB mode V4L2_CID_EXT_VPQ_CM_DB_DATA copy success\r\n");
						/* set data to RTK */
		...
		case V4L2_CID_EXT_VPQ_BYPASS_BLOCK:
		{
			...
			else
			{
				...
				else
				{
					...
					rtd_printk(KERN_INFO, TAG_NAME, "V4L2_CID_EXT_VPQ_BYPASS_BLOCK, MASK=%x, OnOff=%d,\n", v4l2_blockbypass.bypassMask, v4l2_blockbypass.bOnOff);
					// for HDR part
			...
			if (get_support_vo_force_v_top(support_sdr_max_rgb)) {
                ...
                case V4L2_CID_EXT_VPQ_GAMMA_DATA:
                {
                        ...
                        else
                        {
                                ...
                                rtd_printk(KERN_INFO, TAG_NAME, "V4L2_CID_EXT_VPQ_GAMMA_DATA, %d,\n", g_InvGammaPowerMode);
                ...
                break;
#if 0	// "V4L2_CID_EXT_VPQ_DYNAMIC_CONTRAST_COLOR_GAIN" is in vpq_v4l2_main_ioctl_s_ctrl().
					...
					if( odExtInfo.ext_type == 0 ) { /*POD*/
						...
						if( odExtInfo.ext_length != PCID_TBL_LEN*16 ) {
							...
							if( copy_from_user(pLUT, (int __user *)odExtInfo.p_ext_data, PCID_TBL_LEN*16*sizeof(UINT8)) ) {
								...
								for(i=0;i<16;i++) {
									fwif_color_swap_pcid_XY_TV006(pLUT+(i*PCID_TBL_LEN*sizeof(unsigned char)));
									fwif_color_set_pcid_RgnTableValue(pLUT+(i*PCID_TBL_LEN*sizeof(unsigned char)), i, _PCID2_COLOR_ALL);
								}
#endif
								//drvif_color_set_LGD_POD_enable(0);
                ...
                case V4L2_CID_EXT_VPQ_LOCALCONTRAST_DATA:
                {
                       ...
                       // CHIP_LOCAL_CONTRAST_T lc_param;
                        ...
                        else
                        {
							...
							/*rtd_printk(KERN_EMERG, TAG_NAME, "rord lc Comp_En :%d,LC_Tone_Mapping_En: %d,LC_tmap_slope_unit:%d,LC_tmap_blend_factor:%d \n",
							...
							*/
							...
							rtd_printk(KERN_INFO, TAG_NAME, "Set LC through V4L2_CID_EXT_VPQ_LOCALCONTRAST_DATA\n" );
							fwif_color_set_LocalContrast_table_TV006(&glc_param);
                ...
                case V4L2_CID_EXT_VPQ_GAMUT_MATRIX_POST:
                {
                        ...
                        else
                        {
                                ...
                                else
                                {
                                        ...
                                        rtd_printk(KERN_INFO, TAG_NAME, "gamutPost.gamma=%d,gamutPost.degamma=%d,\n",
		...
		case V4L2_CID_EXT_HDR_3DLUT:
		{
			...
			else
			{
				...
				else
				{
					...
					if (((v4l2_blockbypass.bypassMask & V4L2_VPQ_BYPASS_MASK_HDR_EXCEPT_PCC_RTK) != 0) && (v4l2_blockbypass.bOnOff == 1)) {
						rtd_printk(KERN_EMERG, TAG_NAME, "V4L2_CID_EXT_HDR_3DLUT skip, bypassMask=%x, onOff=%d,\n", v4l2_blockbypass.bypassMask, v4l2_blockbypass.bOnOff);
						ret = -EFAULT;
						return ret;
					}
					...
					if (vip_sysInfo->DolbyHDR_flag == 0) {
						...
						if (vpq_v4l2_hdrtype_compare_in_active(st3dlut.hdr_mode, vip_sysInfo->HDR_flag) == 1) {
							rtd_printk(KERN_INFO, TAG_NAME, "DM_freeze, freeze flag CSC3,\n");
						...
						if (st3dlut.hdr_mode != CSC3_HDR_flag_v4l2) {
							...
							rtd_printk(KERN_INFO, TAG_NAME, "DM_3dLUT_DMA, mode=%d,\n", CSC3_HDR_flag_v4l2);
						}
		...
		case V4L2_CID_EXT_HDR_EOTF:
		{
			...
			else
			{
				...
				if (((v4l2_blockbypass.bypassMask & V4L2_VPQ_BYPASS_MASK_HDR_EXCEPT_PCC_RTK) != 0) && (v4l2_blockbypass.bOnOff == 1)) {
						rtd_printk(KERN_EMERG, TAG_NAME, "V4L2_CID_EXT_HDR_EOTF skip, bypassMask=%x, onOff=%d,\n", v4l2_blockbypass.bypassMask, v4l2_blockbypass.bOnOff);
						ret = -EFAULT;
						return ret;
				}
		...
		case V4L2_CID_EXT_HDR_HLG_Y_GAIN_TBL:
		{
			...
			else
			{
				...
				else
				{
					...
					if (((v4l2_blockbypass.bypassMask & V4L2_VPQ_BYPASS_MASK_HDR_EXCEPT_PCC_RTK) != 0) && (v4l2_blockbypass.bOnOff == 1)) {
							rtd_printk(KERN_EMERG, TAG_NAME, "Y_Gain skip, bypassMask=%x, onOff=%d,\n", v4l2_blockbypass.bypassMask, v4l2_blockbypass.bOnOff);
							ret = -EFAULT;
							return ret;
					}
					...
					if (vip_sysInfo->DolbyHDR_flag == 0) {
						...
						if ((vip_sysInfo->HDR_flag != HAL_VPQ_HDR_MODE_HLG) &&
							...
							rtd_printk(KERN_INFO, TAG_NAME, "DM_freeze, freeze flag Y gain,\n");
							RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] = 2; /*  set freeze flag, freez in isr*/
						}
#else
						if (vip_sysInfo->HDR_flag != HAL_VPQ_HDR_MODE_HLG) {
						...
						if (size_byte == sizeof(short)*OETF_PROG_SIZE_TV006) {	//   programmable index table
							rtd_printk(KERN_INFO, TAG_NAME, "DM_OETF_DMA, PID, DM_DMA_mode=%d\n", DM_DMA_mode);
							...
							rtd_printk(KERN_INFO, TAG_NAME, "DM_OETF_DMA, 1025, DM_DMA_mode=%d,\n", DM_DMA_mode);
		...
		case V4L2_CID_EXT_HDR_TONEMAP:
		{
			...
			else
			{
				...
				if (((v4l2_blockbypass.bypassMask & V4L2_VPQ_BYPASS_MASK_HDR_EXCEPT_PCC_RTK) != 0) && (v4l2_blockbypass.bOnOff == 1)) {
						rtd_printk(KERN_EMERG, TAG_NAME, "ToneMapping skip, bypassMask=%x, onOff=%d,\n", v4l2_blockbypass.bypassMask, v4l2_blockbypass.bOnOff);
						ret = -EFAULT;
						return ret;
				}
	...
	rtd_printk(KERN_EMERG, "LDV", "blk_Hnum=%d, blk_Vnum=%d, ld_blk_Hsize=%d, ld_blk_Vsize=%d,\n",
		stReal_db_lut[i].blk_Hnum, stReal_db_lut[i].blk_Vnum, stReal_db_lut[i].ld_blk_Hsize, stReal_db_lut[i].ld_blk_Vsize);
                                        ...
                                        rtd_printk(KERN_EMERG, TAG_NAME, " sizeof(stReal_db_lut)=%d ,LED_DB_LUT=%d ,sizeof(KADP_LED_DB_LUT_T)=%d\n",sizeof(stReal_db_lut),sizeof(LED_DB_LUT),sizeof(KADP_LED_DB_LUT_T));
#endif
		...
		case V4L2_CID_EXT_VPQ_EXTRA_PATTERN:
		{
			...
			struct v4l2_vpq_ext_pattern_info_v2 *stRealInfo = get_v4l2_vpq_cmn_data(&pqData, ext_control.ptr);
#else
			struct v4l2_vpq_ext_pattern_info *stRealInfo = get_v4l2_vpq_cmn_data(&pqData, ext_control.ptr);
#endif
        ...
        switch(cmd)
        {
			...
			if (Get_Factory_SelfDiagnosis_Mode() || (Get_LivezoomOffMainRunScaler() == TRUE) || (Get_Live_zoom_mode()==LIVE_ZOOM_ON)) {
				...
				if (rpc_vip_sysInfo == NULL) {
					rtd_printk(KERN_EMERG, TAG_NAME, "rpc system info = NULL,.V4L2_CID_EXT_VPQ_LOW_DELAY_MODE\n");
				} else {
                ...
                case V4L2_CID_EXT_MEMC_MOTION_PRO:
                {
					...
					//rtd_printk(KERN_INFO, TAG_NAME, "webos_tooloption.eBackLight=%d\n", webos_tooloption.eBackLight);
					rtd_printk(KERN_INFO, TAG_NAME, "webos_tooloption.eBackLight=%s\n", webos_strToolOption.eBackLight);
			...
			rtd_printk(KERN_INFO, TAG_NAME, "V4L2 LD_LUT=%d,\n", vpq_led_LD_lutTableIndex);
                ...
                case V4L2_CID_EXT_VPQ_STEREO_FACE_CTRL:
                {
						...
						rtd_printk(KERN_EMERG, TAG_NAME, "aipq V4L2_CID_EXT_VPQ_STEREO_FACE_CTRL mode: %d, eBackLight %s\n", controls.value, webos_strToolOption.eBackLight);

						v4l2_vpq_stereo_face_pre = v4l2_vpq_stereo_face;
						v4l2_vpq_stereo_face = controls.value;
						ret = vpq_v4l2_ai_ctrl(controls.value, dynamic_ctrl.uDcVal);

						if(ret == -1)
							v4l2_vpq_stereo_face = v4l2_vpq_stereo_face_pre;
				}
                break;
...
int vpq_v4l2_main_ioctl_g_ctrl(struct file *file, void *fh, struct v4l2_control *ctrl)
{
	...
	if (vpq_v4l2_main_get_stop_run(cmd))
        ...
        switch (cmd)
        {
                ...
                case V4L2_CID_EXT_VPQ_STEREO_FACE_CTRL:
                {
                        ctrl->value = v4l2_vpq_stereo_face;
                }
                break;
