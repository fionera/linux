...
long HAL_VPQ_MEMC_LowDelayMode(UINT8 type){
	...
	if(!(((drv_memory_get_game_mode_dynamic() == _ENABLE) && (get_new_game_mode_condition() || (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC))) || drv_memory_get_vdec_direct_low_latency_mode())){
		vpq_memc_set_lowDelayMode(type);
		Scaler_SetMEMCLowDelay(type);
	}
	else{
		pr_notice("Game mode keep MEMC low delay settings!");
	}
	...
	if(type == 1){
		if((Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY) &&	//main source
			...
			((vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY))||(scaler_vsc_get_force_pst_lowdelay_mode()==1)) &&
...
unsigned char Scaler_MEMC_CLK_Check(void){
	...
	if( (((sys_reg_sys_clken3_reg.regValue>>29)&0x7) != 7) || (((sys_reg_sys_srst3_REG.regValue>>29)&0x7) != 7) ){
		printk(KERN_NOTICE "[MEMC][%s] Warning !! CLK not enable!!( %d,%d )", __func__,((sys_reg_sys_clken3_reg.regValue>>29)&0x7),((sys_reg_sys_srst3_REG.regValue>>29)&0x7));
	}

#if 0 //FIXME: remove CLKEN_DISP_MEMC_IN (0x18000208[6])
	if((sys_dispclksel_reg.clken_disp_memc_in != 1) || (sys_dispclksel_reg.clken_disp_memc_out != 1) || (((sys_reg_sys_clken3_reg.regValue>>29) != 7) && ((sys_reg_sys_clken3_reg.regValue>>29) != 5) && ((sys_reg_sys_clken3_reg.regValue>>29) != 1)) || (((sys_reg_sys_srst3_REG.regValue>>29) != 7) && ((sys_reg_sys_srst3_REG.regValue>>29) != 5) && ((sys_reg_sys_srst3_REG.regValue>>29) != 1))){
		//printk(KERN_NOTICE " Warning !! CLK not enable!!( %d, %d, %d,%d )",sys_dispclksel_reg.clken_disp_memc_in, sys_dispclksel_reg.clken_disp_memc_out, (sys_reg_sys_clken3_reg.regValue>>29),(sys_reg_sys_srst3_REG.regValue>>29));
		return FALSE;
	}else
#endif
	{		
...
void vpq_memc_disp_resume_instanboot(void){
	...
	rtd_printk(KERN_DEBUG, TAG_NAME, "[jerry_resume_DDD][%x, %x, %x, %x, %x]\n\r", rtd_inl(0xb8099414), rtd_inl(0xb8099484), rtd_inl(0xb809d400), rtd_inl(0xb809d5fc), rtd_inl(0xb809d7fc));
