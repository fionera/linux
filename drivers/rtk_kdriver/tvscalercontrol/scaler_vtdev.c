...
/*ARGB8888_FORMAT for roku
  RGB888 for LGE*/
static VT_CAP_FMT VT_Pixel_Format = VT_CAP_NV12;
...
#else  /*bypass memc, use memc memory*/
...
unsigned char do_vt_reqbufs(unsigned int buf_cnt)
{
	...
	{			
		down(&VT_Semaphore);
		/*WOSQRTK-14372 if AP not set requbufs(0),get_vt_function()=TRUE, means memory no need allocate again */
		//if(get_vt_function() == FALSE)/*if not allocate memoy previous,do allocate  memory*/
		//{
		//}
...
unsigned char do_vt_capture_streamon(void)
{
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME, "vt_StreamOn_flag FALSE\n");
...
void carvdout_mem_for_vt_2k1k_hfr(unsigned int tsize) /* actual size */
{
	...
	carvedout_size = (unsigned int)carvedout_buf_query(CARVEDOUT_VT, (void *)&carvdout_phyaddr_hfr);
