...
static void drvif_self_awb_changegain(UINT8 color, UINT16 delta, UINT8 inc, int src, ADCGainOffset * ptADC_info)
{
	...
	if(delta >= 2)
	{
	    if (color & SELECT_RED) {//forster modified 050728
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "drvif_self_awb_changegain RED delta=%d, inc=%d,  ptADCGainOffset->Gain_R=%d\n", delta, inc, ptADC_info->Gain_R);
	    ...
	    if (color & SELECT_GREEN) {//forster modified 050728
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "drvif_self_awb_changegain GREEN delta=%d, inc=%d, ptADCGainOffset->Gain_G=%d\n", delta, inc,  ptADC_info->Gain_G);
	    ...
	    if (color & SELECT_BLUE) {//forster modified 050728
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "drvif_self_awb_changegain BLUE delta=%d, inc=%d, ptADCGainOffset->Gain_B=%d\n", delta, inc,  ptADC_info->Gain_B);
	...
	UINT16 *r_voltage_diffresult, UINT16 *g_voltage_diffresult, UINT16 *b_voltage_diffresult, int src, ADCGainOffset * ptADC_info)
{
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]HIGH=r_ready=%d,g_ready=%d,b_ready=%d\n ", __FUNCTION__, __LINE__, r_ready, g_ready, b_ready);
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]HIGH=r(%3d) g(%3d) b(%3d)\r\n ", __FUNCTION__, __LINE__, temp_r_high, temp_g_high, temp_b_high);
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]LOW=r_ready=%d,g_ready=%d,b_ready=%d\n ", __FUNCTION__, __LINE__, r_ready, g_ready, b_ready);
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]LOW=r(%3d) g(%3d) b(%3d)\n", __FUNCTION__, __LINE__, temp_r_low, temp_g_low, temp_b_low);
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]Diff=r_ready=%d,g_ready=%d,b_ready=%d\n ", __FUNCTION__, __LINE__, r_ready, g_ready, b_ready);
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]Diff=r(%3d) g(%3d) b(%3d)\n", __FUNCTION__, __LINE__, *r_voltage_diffresult, *g_voltage_diffresult, *b_voltage_diffresult);
	...
	if(src ==_SRC_YPBPR)
	{
		...
		if(platform == PLATFORM_KXLP)
		{
			rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]\n", __func__, __LINE__);
...
//	auto_soy_soych0_a2_RBUS autosoy_soych0_a2_reg;
	...
	IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_FIELD_reg, smartfit_auto_field_reg.regValue);
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]auto_field_reg.rgb_in_one_en=%x\n",__func__,__LINE__,smartfit_auto_field_reg.rgb_in_one_en);
	...
	//rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]@@Software_Auto_Phase",__func__,__LINE__);
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "####[%s(%d)]phase:0x%02x SOD:0x%08x####\r\n",__func__,__LINE__,phase,SOD_VALUE);
...
static unsigned char ypp_auto_phase_waitfinish(unsigned char display)
{
	...
	do {
		smartfit_auto_adj_reg.regValue = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg);
		if(smartfit_auto_adj_reg.now_af == 0){
			rtd_printk(KERN_INFO, TAG_NAME_ADC, "#####[%s(%d)]auto phase finished, Wait_Time_Cnt=%d!",__func__,__LINE__,Wait_Time_Cnt);
			break;
		}
...
unsigned char auto_phasesearch_WaitFinish(unsigned char *a_pucPhaseResult)
{
	...
	*a_pucPhaseResult = smartfit_auto_phase_ctrl0_reg.autophase_max_step;
	rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "autophase_max_step=0x%02x\n", smartfit_auto_phase_ctrl0_reg.autophase_max_step);
...
void drvif_self_awb_initial_setting(void)//Auto color initial seeting at beginning
{
	...
	if(get_ic_version() >= VERSION_C){
		...
		dds_pll_pllphase_ctrl_reg.pllsph0path = 0; //merlin5 must short path (0: short path, 1: long path)
	}
	...
	adc_adc_vbias_reg.adc_vbias0_4_2 = 0;
	...
	adc_adc_clock_reg.adc_clock_6 = 1; //RGB Output Clock Divider, 0:Normal, 1:Inverted
	adc_adc_clock_reg.adc_2x_sample_0 = 1;//Online Auto SOY ADC clock select 1: CKAPLL<1:0>
	adc_adc_clock_reg.adc_2x_sample_5 = 0;//RGB 1X Output Clock (ADC_CLK_1X) Polarity, 0:Negative, 1:Positive
	IoReg_Write32(ADC_ADC_CLOCK_reg, adc_adc_clock_reg.regValue);
...
//	adc_adc_clamp_ctrl3_RBUS adc_adc_clamp_ctrl3_reg;
	...
	do
	{
		...
		if(pr_ready == FALSE)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "Pr: voltage_diff=%d, target_diff=%d, diff=%d\n", diffvalue_pr, difftarget_pr, ABS(diffvalue_pr, difftarget_pr));
		...
		if(y_ready == FALSE)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "Y: voltage_diff=%d, target_diff=%d, diff=%d\n", diffvalue_y, difftarget_y, ABS(diffvalue_y, difftarget_y));
		...
		if(pb_ready == FALSE)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "Pb: voltage_diff=%d, target_diff=%d, diff=%d\n", diffvalue_pb, difftarget_pb, ABS(diffvalue_pb, difftarget_pb));
...
void OTP_Initial_Setting(void)
{
    ...
    IoReg_Write32(ADC_ADC_VBIAS_reg, 0x01b3cca1);
...
unsigned char drvif_auto_tunedeviceoffset(unsigned char display, unsigned char ucColor, unsigned short *pMargin, unsigned short ucMin_Target)
{
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]Error ABORT!!!\n", __FUNCTION__, __LINE__);
		return ERROR_ABORT;
	}
	...
	if(*pMargin > ucMin_Target){
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]Decrease Color= %x, Offset0 BIOS = %x pMargin=%x  mintarget=%x!\n", __FUNCTION__, __LINE__,ucColor, temp,*pMargin,ucMin_Target);
		auto_adcoffsetchange(display, ucColor, ((temp > 32) ? 32 : temp), 0);

	}else if(*pMargin < ucMin_Target){

		temp = ucMin_Target - *pMargin;
		rtd_printk(KERN_NOTICE, TAG_NAME_ADC, "#####[%s(%d)]Increase Color= %x, Offset1 BIOS = %x pMargin=%x mintarget=%x!\n", __FUNCTION__, __LINE__,ucColor, temp,*pMargin, ucMin_Target);
