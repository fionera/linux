...
void framesync_fixlastline_set_precision_vtotal(void){
	...
	for(i=0; i<5; i++){
		...
		rtd_printk(KERN_DEBUG, TAG_NAME_VBE," [framesync_fixlastline_set_precision_vtotal] ivs1:%x, ivs2:%x\n",
	...
	if(ucThrIP_Flag==TRUE){
		...
		if((ppoverlay_ivs_cnt_1_reg.ivs_cnt_even==0) || (ppoverlay_ivs_cnt_2_reg.ivs_cnt_odd==0)){
			...
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE," [framesync_fixlastline_set_precision_vtotal] interlace is zero. set ivs3:%x\n", ppoverlay_ivs_cnt_3_reg.ivs_cnt_ori);
		}else{
			intputVsyncCount = (ppoverlay_ivs_cnt_1_reg.ivs_cnt_even+ppoverlay_ivs_cnt_2_reg.ivs_cnt_odd)/2;
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE," [framesync_fixlastline_set_precision_vtotal] ivs1:%x, ivs2:%x\n",
			ppoverlay_ivs_cnt_1_reg.ivs_cnt_even, ppoverlay_ivs_cnt_2_reg.ivs_cnt_odd);
		}
		intputFrameRate = 2700000000UL/intputVsyncCount;	//multiple 100
	}else{
		intputFrameRate = Get_Input_vfreq_by_vsync_cnt();
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE," [framesync_fixlastline_set_precision_vtotal] intputFrameRate:%d\n", intputFrameRate);
	}
...
*/
	...
	if((outputFrameRate<SUPPORT_DISPLAY_FRAMERATE_60HZ_PANEL_MIN)||
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE," [framesync_fixlastline_set_precision_vtotal] OutputFrameRate(%d) not support.\n",outputFrameRate);
		return;
	}
	//}
...
//	verticalTotal = ((Get_DISPLAY_CLOCK_TYPICAL()/Get_DISP_HORIZONTAL_TOTAL())*100)/outputFrameRate;
	...
	{
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE," verticalTotal(%d) out of boundary (upper=%d, lower=%d)\n",
		...
		if(verticalTotal <= Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN()){
			...
			verticalTotal = Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN();
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE," verticalTotal set as minimum (%d)\n", verticalTotal);
			dv_total_reg.regValue = IoReg_Read32(PPOVERLAY_DV_total_reg);
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE," removeRatio:%d, multipleRatio:%d\n", removeRatio,multipleRatio);
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE," [framesync_fixlastline_set_precision_vtotal] intputFrameRate:%d, outputFrameRate:%d, verticalTotal:%d\n",
...
*/
	...
	if(_TRUE == ucThrIP_Flag)
	{
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"ivs_cnt1=%x, ivs_cnt2=%x, target_ivs_cnt=%x, dist1=%x, dist2=%x, measureBased=%x \n",
			ivs_cnt1, ivs_cnt2, target_ivs_cnt, distance1, distance2, measureBased);
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[iv2dv] Input htotal:%d, Input vtotal=%d\n", Scaler_DispGetInputInfo(SLR_INPUT_H_LEN),
		...
		rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[iv2dv] Input htotal:%d, Input vtotal=%d\n", Scaler_DispGetInputInfo(SLR_INPUT_H_LEN),
			Scaler_DispGetInputInfo(SLR_INPUT_V_LEN));
		rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[iv2dv] xtalCountPerFrame:%d, temp=%d, targetPixel:%d\n", xtalCountPerFrame, temp, targetPixel);
			...
			Scaler_DispGetInputInfo(SLR_INPUT_H_LEN))
		{
			...
			rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[iv2dv] DI htotal < Input Htotal \n");
			...
			rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[iv2dv] shift_1st_line=%d, shift_2nd_line=%d\n", shift_1st_line, shift_2nd_line);
			...
			for(i=0; i<8; i++){
				...
				line_compensation++;
				rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[iv2dv] targetPixel=%d, line_compensation=%d\n", targetPixel, line_compensation);
			}
...
#endif
		rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[iv2dv] line_compensation:%d, pixel_compensation=%d\n", line_compensation, pixel_compensation);
		rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[iv2dv] fs_iv_dv_fine_tuning5_reg:%x, tuning2_reg=%x\n", fs_iv_dv_fine_tuning5_reg.regValue, tuning2_reg.regValue);
...
unsigned char drvif_framesync_detect(void)
{
	...
	//check ibuff status when full gatting enabled
	if ((Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC) == TRUE) && (IoReg_Read32(VODMA_VODMA_CLKGEN_reg) & VODMA_VODMA_CLKGEN_en_fifo_full_gate_mask)) {
		...
		if(mdomain_cap_ich12_ibuff_status_reg.ibuff_ovf | mdomain_cap_ich12_ibuff_status_reg.fsync_ovf | mdomain_cap_ich12_ibuff_status_reg.fsync_udf)
		{
			...
			{ //data fsync err check debounced
				if(fsync_debounced_count<DATAFSYNC_FLL_DEBOUNCED_TIMES){
					...
					rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[Debounced=%d]Frame Sync check error\n", fsync_debounced_count);
					return TRUE;
				}else{
			...
			if(fwif_color_get_osd_Outer() ==1) /* it's for playback photo patch . rord.tsao */
			{
				...
				mdomain_cap_ich12_ibuff_status_reg.fsync_udf=1;
				IoReg_Write32(MDOMAIN_CAP_ICH12_IBUFF_STATUS_reg,mdomain_cap_ich12_ibuff_status_reg.regValue);
				rtd_printk(KERN_ERR, TAG_NAME_VBE,"A ch1 AAA Frame Sync check error=%d %d %d\n", mdomain_cap_ich12_ibuff_status_reg.ibuff_ovf, mdomain_cap_ich12_ibuff_status_reg.fsync_ovf, mdomain_cap_ich12_ibuff_status_reg.fsync_udf);
				mdomain_cap_ich12_ibuff_status_reg.regValue = IoReg_Read32(MDOMAIN_CAP_ICH12_IBUFF_STATUS_reg);
				if(mdomain_cap_ich12_ibuff_status_reg.ibuff_ovf | mdomain_cap_ich12_ibuff_status_reg.fsync_ovf | mdomain_cap_ich12_ibuff_status_reg.fsync_udf)
				{
					rtd_printk(KERN_ERR, TAG_NAME_VBE,"A ch1 Frame Sync check error=%d %d %d\n", mdomain_cap_ich12_ibuff_status_reg.ibuff_ovf, mdomain_cap_ich12_ibuff_status_reg.fsync_ovf, mdomain_cap_ich12_ibuff_status_reg.fsync_udf);
				} else {
	...
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) > DATAFS_DISABLE_GATTING_WIDTH) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) > DATAFS_DISABLE_GATTING_HEIGHT)){
		...
		//if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) {
			if(multiple > 0)
				...
				rtd_printk(KERN_EMERG, TAG_NAME_VBE, "[ERR] wrong multiple value (%d)!!!\n", multiple);
			if ((vodmapll/64*63) > (sample_rate * 10000)) {
				ratio_n_off = 64 - (sample_rate*64/(vodmapll/10000)) - 1;
			}
			else
				ratio_n_off = 0;
		} else if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) > 1920) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) > 1080)){
	...
	while(count2){
		if(drv_framesync_check_iv2dv_phaseErr_on_fixlastline()==TRUE){
			...
			//pr_emerg("line count 1 : 8248(%x), 8258(%x)\n", IoReg_Read32(0xb8028248), IoReg_Read32(0xb8028258));
			{
				...
				if(timeout==0){
					rtd_printk(KERN_NOTICE, "FLL", "[drv_framesync_check_iv2dv_phaseErr_on_fixlastline] dreg_dbuf_set timeout. \n");
				}
			}
			//pr_emerg("line count 2 : 8248(%x), 8258(%x)\n", IoReg_Read32(0xb8028248), IoReg_Read32(0xb8028258));
			...
			//pr_emerg("line count 4 : 8248(%x), 8258(%x)\n", IoReg_Read32(0xb8028248), IoReg_Read32(0xb8028258));
			modestate_set_fll_running_flag(FALSE);
			rtd_printk(KERN_NOTICE, "FLL", "### set fll new mode @drvif_framesync_FLL_timing \n");
			break;
		}
		count2--;
	}
	if(count2 == 0){
		rtd_printk(KERN_NOTICE, "FLL", "count timeout !!!\n");
...
unsigned char drv_framesync_check_iv2dv_phaseErr_on_fixlastline(){
	...
	if(dVFreq == 0){
		rtd_printk(KERN_NOTICE, "FLL","dVFreq=%d\n", dVFreq);
		return FALSE;
	}
   ...
   // pr_emerg("[ben] temp1=%d, temp2=%d, uzudtg_fractional_fsync_reg.uzudtg_multiple_vsyn=%d, uzudtg_fractional_fsync_reg.uzudtg_remove_half_ivs_mode=%d\n",
...
//    frameDelay_upper = (inputFramePeroid*FRAME_LATENCY)/100+(inputFramePeroid*LOWDELAY_MARGIN)/100;  //ms * 1000000
    ...
    if(checkResult==TRUE){
		...
		rtd_printk(KERN_NOTICE, "FLL","voHtotal=%d, voVtotal=%d, voClock=%d, dHtotal=%d, dCLK=%d\n", voHtotal, voVtotal, voClock, dHtotal, dCLK);
		rtd_printk(KERN_NOTICE, "FLL","inputFramePeroid=%d, uzuDtgdisplayFramePeroid=%d\n", inputFramePeroid, uzuDtgdisplayFramePeroid);
//		rtd_printk(KERN_NOTICE, "FLL","vgip_lcnt=%d, uzu_lcnt=%d, memc_lcnt=%d, dtgM=%d\n", vgip_chn1_lc_reg.ch1_line_cnt, new_meas1_linecnt_real_reg.uzudtg_line_cnt_rt, new_meas1_linecnt_real_reg.memcdtg_line_cnt_rt,
		// [RL6543-298] read after delay uzudtg real time line count
		rtd_printk(KERN_NOTICE, "FLL","vgip_lcnt=%d, uzu_dly_lcnt=%d, memc_lcnt=%d, dtgM=%d\n", vgip_chn1_lc_reg.ch1_line_cnt, new_meas2_linecnt_real_reg.uzudtg_dly_line_cnt_rt, new_meas1_linecnt_real_reg.memcdtg_line_cnt_rt,
        IoReg_Read32(PPOVERLAY_new_meas0_linecnt_real_reg));
        rtd_printk(KERN_NOTICE, "FLL","id2dv_delay=%d, frameLatency=%d\n", id2dv_delay, frameLatency);
        rtd_printk(KERN_NOTICE, "FLL","idx1=%d, idx2=%d\n", idx1, idx2);
        rtd_printk(KERN_NOTICE, "FLL","timeStamp1=%d, timeStamp2=%d, timeStamp3=%d\n", timeStamp1, timeStamp2, timeStamp3);
        rtd_printk(KERN_NOTICE, "FLL","frameDelay_lower=%d, frameDelay_upper=%d\n", frameDelay_lower, frameDelay_upper);
        rtd_printk(KERN_NOTICE, "FLL","frameDelay_Result=%d\n", frameDelay_Result);
        rtd_printk(KERN_NOTICE, "FLL","dtgm2uzuvs_line=%d\n", ppoverlay_uzudtg_control1_reg.dtgm2uzuvs_line);
//		rtd_printk(KERN_NOTICE, "FLL","uzu_lcnt=%d, uzu_vtotal=%d\n", new_meas1_linecnt_real_reg.uzudtg_line_cnt_rt, uzudtg_dv_total_reg.uzudtg_dv_total);
		// [RL6543-298] read after delay uzudtg real time line count
		rtd_printk(KERN_NOTICE, "FLL","uzu_dly_lcnt=%d, uzu_vtotal=%d\n", new_meas2_linecnt_real_reg.uzudtg_dly_line_cnt_rt,
			uzudtg_dv_total_reg.uzudtg_dv_total);
        rtd_printk(KERN_NOTICE, "FLL","checkResult=%d\n", checkResult);
#endif
    }
...
//	UINT32 id2dv_delay=0;
	...
	if(dVFreq == 0){
		printk(KERN_NOTICE "[Game Mode][%s][ERROR] dVFreq = %d \n", __FUNCTION__, dVFreq);
		return 0;
	}
...
#if 1  // For debug
        //pr_notice("[ben] vgip_lcnt=%d, uzu_lcnt=%d, memc_lcnt=%d, dtgM=%d\n", vgip_chn1_lc_reg.ch1_line_cnt, new_meas1_linecnt_real_reg.uzudtg_line_cnt_rt, new_meas1_linecnt_real_reg.memcdtg_line_cnt_rt,
        //IoReg_Read32(PPOVERLAY_new_meas0_linecnt_real_reg));
        //pr_notice("[ben] timeStamp1=%d, timeStamp2=%d, timeStamp3=%d\n", timeStamp1, timeStamp2, timeStamp3);
		if((Get_DISPLAY_PANEL_MPLUS_RGBW() != TRUE)&&(Get_DISPLAY_PANEL_BOW_RGBW() != TRUE)){
...
unsigned int drv_framesync_get_gamemode_iv2dv_linedelay(void){
	...
	printk(KERN_NOTICE "[Game Mode][%s] frameDelay = %d \n", __FUNCTION__, frameDelay);
...
FRAMESYNC_REDO:
		...
		rtd_printk(KERN_NOTICE, "FLL","[gating] ######## increase vo clk\n");
#endif
		...
		if(timeout==0){
			rtd_printk(KERN_NOTICE, "FLL","[gating] Data FS change to FLL timeout!\n");
		}else{
			rtd_printk(KERN_NOTICE, "FLL","[gating] Data FS change to FLL done.\n");
		}
		rtd_printk(KERN_NOTICE, "FLL","[gating] profile1 (%d)ms\n", after_time-before_time);
		...
		if(ori_voclk_ratio_num == 0)
		{
			...
			{ //1080i/P case
				...
				if(Scaler_InputSrcGetMainChType() == _SRC_VO){
					...
					ratio_num = 0;
					divider_num = 0;
					rtd_printk(KERN_DEBUG, TAG_NAME_VBE,"################ div=%d, ratio_num=%d\n", divider_num, ratio_num);
				}else
...
void scaler_update_vo_vstat_htotal(unsigned char a_bEnableVCrop, unsigned char a_bUpdateReg)
{
	...
	if(multiple > 0)
		...
		rtd_printk(KERN_EMERG, TAG_NAME_VBE,"[Warning ]Wrong multiple value %d!!!\n", multiple);
	...
	{
		rtd_printk(KERN_EMERG, TAG_NAME_VBE,"Warning, scaler_update_vo_vstat_htotal(), ulVCropSize=%d, ulDVAct=%d\n", ulVCropSize, ulDVAct);

		if (0 == ulDVAct)
		{
			ulDVAct = Get_DISP_ACT_END_VPOS() - Get_DISP_ACT_STA_VPOS();
			rtd_printk(KERN_EMERG, TAG_NAME_VBE,"Set ulDVAct = DISP_LEN=%d\n", ulDVAct);
		}
	}
	//=========================================================================
	//=========================No crop===============================================
	...
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"old vo htotal=%d, vstart=%d, vend=%d, VoClk=%d, ulVoVsyncStar=%d\n",
									ulVoHtotal, ulVoVStart, ulVoVEnd, ulVoClk, ulVoVsyncStart);

	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"DHTotal=%d, DVStar=%d, ulDHStar=%d, DVAct=%d, ulDClk=%d\n",
									ulDHTotal, ulDVStar, ulDHStar, ulDVAct, ulDClk);

	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"ulVCropSize=%d, ulDisableEdgeSmooth=%d\n", ulVCropSize, ulDisableEdgeSmooth);
	...
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"ulTemp1=%lld, ulTemp2=%lld\n", ulTemp1, ulTemp2);
	...
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"New vo htotal=%d, vstart=%d, vend=%d, v_sync start=%d\n",
	...
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"update h total 0xb8005078=%x\n", rtd_inl(VODMA_VODMA_V1SGEN_reg));
	...
	//rtd_printk(KERN_EMERG, TAG_NAME_VBE,"0  dTemp3=%lld\n", ullTemp3);

	//ullTemp3 = ullTemp3 +350 * ulVoClk * MULTIPLE/1000 + 1920 * MULTIPLE;
	ullTemp3 = ullTemp3 +350 * ulVoClk /10000 + 1920 * MULTIPLE;

	//rtd_printk(KERN_EMERG, TAG_NAME_VBE,"1  dTemp3=%lld\n", ullTemp3);

	do_div(ullTemp3, ulVoHtotal);

	//rtd_printk(KERN_EMERG, TAG_NAME_VBE,"2  dTemp3=%lld\n", ullTemp3);
	...
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"dTemp3=%lld\n", ullTemp3);
	...
	if (ullTemp3 > ulVoVStart * MULTIPLE)
	{
		...
		//rtd_printk(KERN_EMERG, TAG_NAME_VBE,"new ulVoVStart=%d\n", ulVoVStart);

		ulIv2Dv_delay = (ulVoVStart + vo_start_offset + 2) * MULTIPLE - ullTemp3;

		//rtd_printk(KERN_EMERG, TAG_NAME_VBE,"ulIv2Dv_delay=%d/%d\n", ulIv2Dv_delay, MULTIPLE);
	...
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"new vo htotal=%d, ulVoVStart=%d\n", ulVoHtotal, ulVoVStart);
#if 0
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"iv2dv_line=%d, ullparam1=%lld, ulparam2=%d\n", iv2dv_line, ullparam1, (unsigned int)MULTIPLE);
	rtd_printk(KERN_EMERG, TAG_NAME_VBE,"iv2dv_pixel=%d, ullparam1=%lld, ulparam2=%d\n", iv2dv_pixel, ullparam1, (unsigned int)MULTIPLE);
#endif

	//rtd_printk(KERN_EMERG, TAG_NAME_VBE,"iv2dv_line=%d, iv2dv_pixel=%d\n", iv2dv_line, iv2dv_pixel);
