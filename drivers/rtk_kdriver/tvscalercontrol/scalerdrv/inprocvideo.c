...
unsigned char drvif_inprocvideo_setup_NotMeasure(unsigned char a_ucSrc, StructDisplayInfo* a_ptDispInfo, ModeInformationType* a_ptTimingInfo, unsigned char ucModeCur, unsigned char supportColor)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME_AVD,"#####[%s(%d)] ucVideoMode=%x\n", __func__, __LINE__, ucVideoMode);
	...
	// if(drvif_mode_offlinemeasure(_OFFLINE_MS_SRC_VDC, ANALOG_MODE_MEASUREMENT, a_ptDispInfo, a_ptTimingInfo) == _MODE_SUCCESS)
	{
		up(get_offline_semaphore());
			...
			if (g_usIsVtotalNoGood && g_ucSrcType == _SRC_TV)
			{
				do
				{
					...
					if (0==ulTimeout) {
						rtd_printk(KERN_EMERG, TAG_NAME_AVD, "** usVHalfTotal*2=%d, usVLineOut=%d, vstate=%d\n", usVHalfTotal*2, usVLineOut, drvif_module_vdc_Read_Vstate());
					}
					...
					if (ABS(usVLineOut, usVHalfTotal*2 ) <=2 && (usVLineOut!=526 && usVLineOut!=524 && usVLineOut!=624 && usVLineOut!=626) && ulTimeout<20) {
						if(19==ulTimeout){
							rtd_printk(KERN_EMERG, TAG_NAME_AVD, "!! usVHalfTotal*2=%d, usVLineOut=%d, ulTimeout=%d, vstate=%d\n", usVHalfTotal*2, usVLineOut, ulTimeout, drvif_module_vdc_Read_Vstate());
						}
		...
		if (_MODE_480I == a_ptDispInfo->ucMode_Curr)
		{
			...
			rtd_printk(KERN_INFO, TAG_NAME_AVD,"NTSC a_ptDispInfo->IPV_ACT_LEN_PRE=%d\n", a_ptDispInfo->IPV_ACT_LEN_PRE);
			rtd_printk(KERN_INFO, TAG_NAME_AVD,"g_usCurVHaldTotal=%d\n", g_usCurVHaldTotal);
#endif
			Scaler_SetDispStatusFromSource(a_ptDispInfo, SLR_DISP_INTERLACE,TRUE);
			Scaler_SetDispStatusFromSource(a_ptDispInfo, SLR_DISP_422,FALSE);
			Scaler_SetDispStatusFromSource(a_ptDispInfo, SLR_DISP_10BIT,TRUE);
		...
		else
		{
			...
			if (0 != usVHalfTotal && g_ucSrcType == _SRC_TV)
			{
				...
				rtd_printk(KERN_INFO, TAG_NAME_AVD,"a_ptDispInfo->IPV_ACT_LEN_PRE=%d\n", a_ptDispInfo->IPV_ACT_LEN_PRE);
				if (usVHalfTotal > 25)
					g_usCurVHaldTotal = usVHalfTotal;
			}
			else
