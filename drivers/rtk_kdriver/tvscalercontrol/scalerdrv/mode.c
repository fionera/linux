...
extern struct semaphore SVP_Semaphore;
...
{//This function is to set onlinemeasure watchdog and interrupt setting
	...
	pr_debug("\r\n###func%s channel:%d src:%d wdg:%d int:%d###\r\n",__func__, channel, src, wdgenable, interruptenable);
	...
	if(_ENABLE == interruptenable)
	{
		if(_CHANNEL1 == channel)
		{
			...
			if (src == _SRC_HDMI) {
				...
				{
					if(drvif_Hdmi_GetFrlMode() == 0)//hdmi 2.0
