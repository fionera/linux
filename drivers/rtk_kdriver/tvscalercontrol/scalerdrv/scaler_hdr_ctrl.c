...
static void reset_dolby_md_crc_error_cnt(void)
{
...
void set_cap_block_index(void);
void set_cap_double_buffer(void);

static void state_machine_for_crc_error(int dolby_result)
{
	...
	if (flag_unfreeze && unfreeze_count == 1) {
		set_cap_block_index();
	}

	if (flag_unfreeze && unfreeze_count == 2) {
		set_cap_double_buffer();
	}
	...
	if ((dolby_result & _BIT0) == 0) {
		...
		if (dolby_md_crc_error_cnt) {
			pr_info("[%s:%d][Dolby] md crc error  un freeze\n", __func__, __LINE__);
			flag_unfreeze = true;
			//set_cap_block_index();
		}
...
unsigned char HDR_DolbyVision_HDMI_check_mode(void)
{
	...
	if(rtd_inl(HDMI_P0_MDD_CR_reg) & _BIT0){
		...
		if((get_HDMI_Dolby_VSIF_mode() != DOLBY_HDMI_VSIF_LL) && (get_HDMI_HDR_mode() == HDR_DOLBY_HDMI))
			state_machine_for_crc_error(bDolbyVisionDetResult);
...
void set_cap_block_index(void)
{
	int disp_block_sel, cap_block_sel;

	mdomain_disp_ddr_mainstatus_RBUS mdomain_disp_ddr_mainstatus_reg;
	mdomain_cap_ddr_in1ctrl_RBUS mdomain_cap_ddr_in1ctrl_reg;

	mdomain_disp_ddr_mainstatus_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainStatus_reg);
	disp_block_sel =  mdomain_disp_ddr_mainstatus_reg.main_block_select;
	cap_block_sel = (disp_block_sel == 2) ? 0 : disp_block_sel + 1;
	...
	mdomain_cap_ddr_in1ctrl_reg.regValue = rtd_inl(MDOMAIN_CAP_DDR_In1Ctrl_reg);
	mdomain_cap_ddr_in1ctrl_reg.in1_single_buffer_select = cap_block_sel;
...
void mdomain_capture_freeze(int enable)
{
	...
	if (enable) {
		...
		mdomain_cap_ddr_in1ctrl_reg.in1_freeze_control = 1;
		mdomain_cap_ddr_in1ctrl_reg.in1_freeze_enable = 1;
		//set_cap_block_index();
	} else {
		mdomain_cap_ddr_in1ctrl_reg.in1_freeze_enable = 0;
