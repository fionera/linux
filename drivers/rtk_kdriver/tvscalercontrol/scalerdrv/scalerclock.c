...
unsigned char drvif_clock_set_lowdelay_dtg_uzu(unsigned char _channel, unsigned char low_delay_en)
{
		...
		if(low_delay_en == 1){
			...
			if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO)>0) && (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO)>0))
			{
	        		dclk_fract_a = Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO)-1;
				dclk_fract_b = Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO)-1;
			}
...
unsigned char drvif_clock_set_lowdelay_dtg_memc_div(unsigned char low_delay_en)
{
	...
	if(Get_DISPLAY_REFRESH_RATE() == 120){
		...
		else if((get_hdmi_4k_hfr_mode() != HDMI_NON_4K120) || force_enable_two_step_uzu())
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 0;//two pixel mode
		else
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 1;
		sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f1p_gate_sel = 0;
...
//	vgip_chn2_status_RBUS vgip_chn2_status_reg;
	...
	*/
	...
	if(get_ic_version() >= VERSION_C){
		...
		dds_pll_pllphase_ctrl_reg.pllsph0path = PLLPH0PATH_SHORT_PATH; //merlin5 must short path (0: short path, 1: long path)
	}
