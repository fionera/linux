...
void scalerdisplay_set_main_act_window_byOrbit(UINT16 DispHSta, UINT16 DispHEnd, UINT16 DispVSta, UINT16 DispVEnd)
{
	...
	drvif_scalerdisplay_set_dbuffer_en(SLR_MAIN_DISPLAY, FALSE);
	up(get_forcebg_semaphore());/*semaphore must added for not effect mute function when setting b8028300*/
	rtd_printk(KERN_WARNING, TAG_NAME_VBE,"[%s] set main_active_h=%x, main_active_v=%x\n", __FUNCTION__,main_active_h_start_end_reg.regValue,main_active_v_start_end_reg.regValue);
...
void scalerdisplay_vo_ratio_change(VO_VIDEO_PLANE plane)
 {
	...
	if(panel_ratio > source_ratio){//Change panel width
		...
		pr_err("[scaler][error] Change panel width with INVALID VALUE!!!\n");
		return;
	    }
		...
		pr_err("[scaler][error] Change panel height with INVALID VALUE!!!\n");
		return;
	    }
...
*/
	...
	if((Get_GOSUBandVODMA()==TRUE)&&((get_rotate_mode()==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_90)||(get_rotate_mode()==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_270))&&(Scaler_InputSrcGetMainChType() == _SRC_VO))
	{
	...
	if ((Scaler_InputSrcGetMainChType() == _SRC_VO) && (Scaler_VOFrom(Scaler_Get_CurVoInfo_plane())==1)){
		...
		Vstart = VRatioOffset + VStartOffset;
/*
		scalerdisplay_set_main_act_window_byOrbit((unsigned short)HStartOffset,
										(unsigned short)Scaler_DispGetInputInfo(SLR_INPUT_DISP_WID) + HStartOffset ,
										(unsigned short)VRatioOffset + VStartOffset,
										(unsigned short)Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN) + VRatioOffset + VStartOffset );
*/		return;
	...
	rtd_printk(KERN_WARNING, TAG_NAME_VBE,"[%s] first set Hstart=%x, Vstart=%x\n", __FUNCTION__,Hstart,Vstart);
...
void drvif_scalerdisplay_set_bg_color(unsigned char display, eDISPLAYBORDERTYPE BorderType, unsigned int ColorR, unsigned int ColorG, unsigned int ColorB)
{
	...
	if(SLR_MAIN_DISPLAY == display)
	{
		...
		else if(BorderType == _DISPLAY_HB)
		{
			...
			{
				rtd_printk(KERN_DEBUG, TAG_NAME_VBE, "#########Main DISPLAY_HB Color is the same\n");
				return;
			}
	...
	else// if(SLR_SUB_DISPLAY == display)
	{
		...
		if(BorderType == _DISPLAY_BG)
		{
			...
			{
				rtd_printk(KERN_DEBUG, TAG_NAME_VBE, "#########Sub DISPLAY_BG Color is the same\n");
				return;
			}
		...
		else if(BorderType == _DISPLAY_EB)
		{
			...
			{
				rtd_printk(KERN_DEBUG, TAG_NAME_VBE, "#########Sub DISPLAY_EB Color is the same\n");
				return;
			}
		...
		else if(BorderType == _DISPLAY_HB)
		{
			...
			{
				rtd_printk(KERN_DEBUG, TAG_NAME_VBE, "#########Sub DISPLAY_HB Color is the same\n");
				return;
			}
...
StructSrcRect cal_new_inputRegion_base_on_hal_orgInput(unsigned char display)
{
	...
	if((NewInput2OldInput.srcx + NewInput2OldInput.src_wid) > Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID)){
		pr_err("[scaler][WARNING]new adaptive stream, wid is over size\n");
	...
	if((NewInput2OldInput.srcy + NewInput2OldInput.src_height) > Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_LEN)){
		pr_err("[scaler][WARNING]new adaptive stream, len is over size\n");
...
unsigned char fw_scaler_cal_sdnr_capture_for_hal_orgInput(unsigned char display)
{
	...
	rtd_printk(KERN_DEBUG, TAG_NAME_VBE,"[new hal orgInput]display:%d,inpu base scalerTiming:x:%d, y:%d, wid:%d, len:%d\n",display,
	...
	{	//
		...
		if ((SCALER_DISP_CHANNEL)display == SLR_SUB_DISPLAY)
		{
			if (Get_Live_zoom_mode() == LIVE_ZOOM_ON) {
				...
				if (Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY,SLR_INPUT_IPH_ACT_WID) > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
			...
			if (ucSDNR_H+ucSDNR_X > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
			{
				...
				pr_err("[warning] sub doesn't have di, but size is over 4096\n");
			}
		}
		else// SLR_MAIN_DISPLAY
		{
			...
			if (Scaler_Get_di_hsd_flag()&&(SLR_MAIN_DISPLAY==display)){
				...
				if ((VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display))&&(Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD)&&
					...
					pr_warning("[HSD] new sdnr size:x :%d, wid:%d\n",temp_x,temp_w);
				}
			} else {
				if (Get_Live_zoom_mode() == LIVE_ZOOM_ON) {
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA, 0);
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA, 0);
					if (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_IPH_ACT_WID) > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
						Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD);
					else
				...
				if (ucSDNR_H+ucSDNR_X > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD){
					...
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, ucSDNR_H);
					pr_err("[warning] di and rtnr off, but size is over 4096\n");
				}
			}
		}
	...
	if (Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_SDNRH_ACT_WID) > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
		Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD);
...
#if 1
	//rtd_printk(KERN_EMERG, TAG_NAME_VBE,"function=%s, line=%d\n",__FUNCTION__, __LINE__);
...
unsigned char fw_scaler_cal_sdnr_capture(unsigned char display)
{
	...
	if(Scaler_InputRegionType_Get(display) == INPUT_BASE_ON_DISP_WIN){
		...
		if((Scaler_I2rnd_get_timing_enable() && ((vsc_i2rnd_dualvo_sub_stage == I2RND_DUALVO_STAGE_RERUN_MAIN_FOR_S1) || (vsc_i2rnd_sub_stage == I2RND_STAGE_MAIN_GET_RPC)))){
			...
			printk(KERN_EMERG"[crixus][sdnr]newInput.srcx = %d\n", newInput.srcx);
			printk(KERN_EMERG"[crixus][sdnr]newInput.srcy = %d\n", newInput.srcy);
			printk(KERN_EMERG"[crixus][sdnr]newInput.src_wid = %d\n", newInput.src_wid);
			printk(KERN_EMERG"[crixus][sdnr]newInput.src_height = %d\n", newInput.src_height);
		}
#endif
		...
		if ((SCALER_DISP_CHANNEL)display == SLR_SUB_DISPLAY)
		{
			...
			if (Get_Live_zoom_mode() == LIVE_ZOOM_ON) {
				...
				if (Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY,SLR_INPUT_IPH_ACT_WID) > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
					...
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY,SLR_INPUT_DI_WID));
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_DI_LEN));
			}else if(Get_sub_from_DI_mux_path()){
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DI_WID));
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_LEN, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DI_LEN));
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, newInput.srcx);
...
unsigned char fw_scaler_cal_vgip_capture(unsigned char display)
{
	...
	if ((SCALER_DISP_CHANNEL)display == SLR_SUB_DISPLAY) {
		...
		if (Get_Live_zoom_mode() == LIVE_ZOOM_ON || Get_sub_from_DI_mux_path())
		{
		...
		if (Get_Live_zoom_mode() == LIVE_ZOOM_ON)
		{
			...
			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA_PRE, Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_IPH_ACT_STA_PRE));
			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA_PRE, Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_IPV_ACT_STA_PRE));
			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, 0);
			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA, 0);
			...
			{
	  			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE)<<1);
	  			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN_PRE, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE)<<1);
			}
			else
			{
	  			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE));
	  			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN_PRE, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE));
			}

			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID_PRE));
			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID_PRE, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID_PRE));
#else
			Scaler_DispSetInputInfo(SLR_INPUT_IPH_ACT_STA, Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE));
			Scaler_DispSetInputInfo(SLR_INPUT_IPH_ACT_WID, Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE));
			Scaler_DispSetInputInfo(SLR_INPUT_IPV_ACT_STA, Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_STA_PRE));
...
//	if(vertical_total==0){
	if (0 != h_total)
		...
		pr_err("DISP_HORIZONTAL_TOTAL = 0\n");
...
void scalerdisplay_force_bg_enable(unsigned int channel, unsigned char enable)
{
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"%s,Disp[%d]=%d\n", __FUNCTION__, channel, enable);
	...
	if (!enable) {
		for (i = 0; i < SLR_FORCE_BG_TYPE_MAX; i++) {
			if (display_forcebg_mask[channel][i]) {
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"\r### mask(%d)(%d) is enable, so can not disable bg ###\r\n", channel, i);
				return;
			}
		}
		if(Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"\r### scaler(%d) is not active, so can not disable bg ###\r\n", channel);
			return;
		}
	}
  ...
  if(enable) //different algo. enable:new   disable:old
  {
	...
	if(channel == SLR_MAIN_DISPLAY){//main path
		...
		#ifdef CONFIG_RTK_KDRV_AI // lesley 1022
		extern bool bg_flag;
  ...
  else
  {
	if(channel == SLR_MAIN_DISPLAY){//main path
		...
		if (enable == main_display_control_rsv_reg.m_force_bg) {
			...
			ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_read_sel = read_level;
			IoReg_Write32(PPOVERLAY_Double_Buffer_CTRL3_reg, ppoverlay_double_buffer_ctrl3_reg.regValue);
			rtd_printk(KERN_INFO, TAG_NAME_VBE,"n####[FBG MAIN]Already disable forceBG, Scaler_MEMC_Get_CheckMEMC_Outbg=%d####\r\n",Scaler_MEMC_Get_CheckMEMC_Outbg());
...
//#endif
		...
		if(Get_DisplayMode_Src(SLR_SUB_DISPLAY) == VSC_INPUTSRC_VDEC)
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"####RTK Disable Sub ForceBG avsyn:%d !!!####\r\n", get_vo_avsync_flag(1));
		else
			rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"####RTK Disable Sub ForceBG !!!####\r\n");
...
{//use spin_lock_irqsave and spin_unlock_irqrestore for normal task
	...
	{//internal mute mask control
		rtd_printk(KERN_INFO, TAG_NAME_VBE,"###internal set_display_forcebg_mask display(%d) type(%d) value(%d)###\r\n", display, masktype, maskvalue);
	}
}
