...
#define HDMI_TO_VO_FRC_60_MAX_WIDTH_60_PANEL 1920
#define HDMI_TO_VO_FRC_60_MAX_LENTH_60_PANEL 1080
	...
	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
	{
		...
		{//HDMI 2.0
			if(Get_DISPLAY_REFRESH_RATE() == 120)
			{
				...
				/* no need becasue Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) decide max frame rate
		...
		{//hdmi 2.1
			if(Get_DISPLAY_REFRESH_RATE() == 120)
			{
				...
				//return get_HFR_mode()? HDMI_4K120_2_1 : HDMI_NON_4K120;// HDMI_4K120_2_1 is two pixel mode  no need becasue Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) decide max frame rate
			}
		}
...
void drvif_scaler_ddomain_DTG_line_compare_irq(unsigned char enable, unsigned char vlc_mode, unsigned int vlc_src_sel, unsigned int dtg_vln)
{
	...
	dtg_ie_reg.memcdtgv_den_end_event_ie = enable;
...
#endif
				{
					//[WOSQRTK-14508] set to default iv2dv delay line, to avoid the vo iv2dv-delayed vsync can not enter the fsync interval.
					fs_iv_dv_fine_tuning5_reg.iv2dv_line = 2;
					fs_iv_dv_fine_tuning5_reg.iv2dv_line_2 = 2;
				}
			}
		...
		{//hdmi no scaling down
			...
			if (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_FRAMESYNC) == FALSE) {
				...
				if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC){
					//VRR needs do v scale down to 1k, for mdisp 4k1k120Hz run at 600M
	...
	if((webos_tooloption.eModelModuleType == module_LGD) && (webos_tooloption.eBackLight == direct ) && (webos_tooloption.eLEDBarType == local_dim_block_32)){
		...
		if((m_adjust_vtotal > (vertical_total-1)) && ((m_adjust_vtotal-vertical_total+1)>ADJUST_VTOTAL_VALUE)){//2700 to 2250
			...
			while(((m_adjust_vtotal > (vertical_total-1)) &&((m_adjust_vtotal-vertical_total+1)>ADJUST_VTOTAL_VALUE)) && (--m_loop_timeout)){
				...
				//rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"-m_adjust_vtotal=%d, b8028014=%x,b802850c=%x (loop %d\n", m_adjust_vtotal, IoReg_Read32(0xb8028014),IoReg_Read32(0xb8028504), m_loop_timeout);
			}
			if(m_loop_timeout == 0)
...
void modestate_set_fractional_framesync()
{
	...
	if((vbe_disp_get_adaptivestream_fs_mode()) || (Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC))){
		uzudtg_fractional_fsync_reg.uzudtg_remove_half_ivs_alignmode = 1; // align to ivs
	}else{
...
void modestate_set_lowdelay_fractional_framesync(unsigned char _channel, unsigned int lowdelay_en)
{
	...
	{
		if(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO) > 0)
			dtg_m_remove_input_vsync_reg.remove_half_ivs_mode2 = (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO)-1);
		else
			dtg_m_remove_input_vsync_reg.remove_half_ivs_mode2 = 0;
	}
	...
	{
		if (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO) > 0)
			dtg_m_multiple_vsync_reg.dtg_m_multiple_vsync = (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO)-1);
		else
			dtg_m_multiple_vsync_reg.dtg_m_multiple_vsync = 0;
	}
...
unsigned char drivf_i3ddma_config(unsigned char display, unsigned char src)
{
	...
	else
	{
        ...
        if(get_vsc_run_pc_mode() && ((src == VGIP_SRC_TMDS) || (src == VGIP_SRC_ADC))){
            ...
            if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && drvif_i3ddma_triplebuf_flag() &&
			...
			(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) >= 2160))
            {
				...
				pr_notice("[%s][%d] i3ddma pc mode 3 buffer 4k COMPRESSION_12_BITS. \n",__FUNCTION__,__LINE__);
			}
			else
			{
            	dvrif_i3ddma_set_compression_bits(COMPRESSION_20_BITS);//pc mode go 20bit only YPP and hdmi
            	pr_notice("[%s][%d] i3ddma pc mode COMPRESSION_20_BITS. \n",__FUNCTION__,__LINE__);
			}
        }
		else
        {
	...
	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
	{
		...
		{//120 panel
				...
				&& Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= HDMI_TO_VO_FRC_60_MAX_LENTH_120_PANEL)
			{
				...
				else if(Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 601)//60.2Hz~99.4Hz //to be compatible with OLED panel
				{
					rtd_printk(KERN_INFO, TAG_NAME_VSC, "input frame big than 60 and VO force to 60hz\n");
					return TRUE;//i domain go 60
				}
				else if((webos_tooloption.eBackLight == 2) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 240) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 245))//to be compatible with OLED panel
				{
					rtd_printk(KERN_INFO, TAG_NAME_VSC, "[OLED] input frame bigger than 24, VO force to 24hz\n");
					return TRUE;//i domain go 24
				}
				else if((webos_tooloption.eBackLight == 2) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 300) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 305))//to be compatible with OLED panel
				{
		...
		else
		{
			if( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) <= HDMI_TO_VO_FRC_60_MAX_WIDTH_60_PANEL
				...
				&& Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 601)//60.2Hz~99.4Hz //to be compatible with OLED panel
			{
				rtd_printk(KERN_INFO, TAG_NAME_VSC, "input frame big than 60 and VO force to 60hz\n");
				return TRUE;
			}
...
{//return TRUE: change.  return FALSE: no change
	...
	extern bool Scaler_AVD_GetTimingModeChanged(void);
	extern void Scaler_AVD_SetTimingModeChanged(bool flag);
	...
	if(Scaler_AVD_GetTimingModeChanged()||(avd_info->ucMode_Curr != Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_MODE_CURR)))
	{
		if(Scaler_AVD_GetTimingModeChanged())
			Scaler_AVD_SetTimingModeChanged(false);
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		switch(hdrtype)
		{
			...
			case HDR_DM_SDR_MAX_RGB:
				...
				drvif_DM2_B0203_noSram_Enable(1, 0);
				//fwif_color_set_HDR_Dolby_mode_Enable(hdrtype);	// set by scaler_vtop_dolby_mode_setting
...
VSC_DRIVER_PATTERN vsc_driver_pattern_table[] ={
	...
	{HDMI_4K2K_30, 		1, 2, 0, 0, 0, 1, 0},
	{HDMI_4K2K_60, 		1, 2, 0, 0, 0, 1, 0},
	...
	{HDMI_HDR_4K2K_60, 	3, 2, 0, 0, 0, 4, 0},
	...
	switch(info)
	{
		...
		case DRIVER_FREERUN_PATTERN:
			...
			if (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI ||
				...
				return 0;
			} else if (vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY)&& (pVOInfo->is2KCP == 0)) {
				return 1;
			} else {
...
{//return TRUE: rerun //return: FALSE: rerun
