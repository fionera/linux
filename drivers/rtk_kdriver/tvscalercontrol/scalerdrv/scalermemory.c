...
static void memory_set_sub_for_pip(unsigned char display)
{
	...
	else if (MEMCAPTYPE_LINE == dvrif_memory_get_cap_mode(display))
	{
		...
		pr_debug("Sub not compres, cap frame mode\n");
	...
	else
	{
				...
				|| (modestate_get_pr3d_in_fp_3d_mode() && ((Scaler_InputSrcGetMainChType() == _SRC_VO)|| (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 310))))
		{
			...
			{//Will add condition. only main can set for WOSQRTK-2978
				//temp1 |= _BIT3;
				if(Get_DISPLAY_REFRESH_RATE() != 120) //120 panel don't need go fast mode
...
void dvrif_memory_handler(unsigned char display)
{
	...
	//if(!((get_mainsubonpr_fuc()==TRUE)&&(get_MainSubonPR_curState()==MAINSUB_SHOWONPR_INIT_SUB)))
	{
		...
		/*else if((Get_HDMI_SingleBuffer_GameMode()==TRUE) && (Scaler_InputSrcGetMainChType() == _SRC_HDMI))
			...
			if(Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE() &&
				...
				(Scaler_DispGetStatus(display,SLR_DISP_VFLIP3))&&
	...
	if(drv_memory_Get_multibuffer_flag() && (display== SLR_MAIN_DISPLAY)){
...
//remove in k4lp
		...
		if(((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_ADC)
			...
			&&(!drvif_i3ddma_triplebuf_flag()) && (display == SLR_MAIN_DISPLAY)
			){
...
void drv_memory_I_De_XC_DMA_Force_Bnd(void)
{
	de_xcxl_wrrd_bndaddr1_RBUS de_xcxl_wrrd_bndaddr1_reg;
	de_xcxl_wrrd_bndaddr2_RBUS de_xcxl_wrrd_bndaddr2_reg;

	unsigned int addr;
	addr = get_query_VIP_DMAto3DTABLE_start_address(DMAto3DTABLE_TC_C_LUT);
	...
	de_xcxl_wrrd_bndaddr1_reg.dexcxl_wrrd_downlimit_adr = (addr>>3)&0x1fffffff;
	de_xcxl_wrrd_bndaddr2_reg.dexcxl_wrrd_uplimit_adr = ((addr++0x10)>>3)&0x1fffffff;
...
//	_system_setting_info* system_info_structure_table = NULL;
	...
	if(get_platform() != PLATFORM_KXLP) { 	// use CMA memory
		if (Force_Clr == 1) {
			//drv_memory_I_De_XC_DMA_Force_Bnd();
			drv_memory_Set_I_De_XC_Mem_Ready_Flag(DI_MEM_IDX, 0);
			// clear addr in share memory first for videoFW
			Pli_addr = mem_ctrl[DI_MEM_IDX]->phyaddr;
			...
			rtd_printk(KERN_INFO, TAG_VPQ_MEM, "De-XC mem force clear, return memroy from CMA = %x,t0~2=%x,%x,%x,\n", Pli_addr, t0, t1, t2);
		} else {
			if (DbgSclrFlgTkr.Main_Scaler_Stop_flag == FALSE) {	/* connection check. */
				if (mem_ctrl[DI_MEM_IDX]->phyaddr == 0) {
					phyaddr = (unsigned int)pli_malloc(de_xc_size, GFP_DCU2_FIRST);
					if ( phyaddr != 0) {
						...
						rtd_printk(KERN_INFO, TAG_VPQ_MEM, "De-XC mem alloc from CMA\n");
						...
						mem_ctrl[DI_MEM_IDX]->getsize = 0;
						mem_ctrl[DI_MEM_IDX]->Borrow_Mode = De_XC_MEM_UNKNOW;
						rtd_printk(KERN_EMERG, TAG_VPQ_MEM, "De-XC Memory alloc fail,addr=%x\n", phyaddr);
					}
				} else {
					rtd_printk(KERN_INFO, TAG_VPQ_MEM, "De-XC mem already get from %d, CMA\n", mem_ctrl[DI_MEM_IDX]->Borrow_Mode);
				}
			} else {
				//drv_memory_I_De_XC_DMA_Force_Bnd();
				drv_memory_Set_I_De_XC_Mem_Ready_Flag(DI_MEM_IDX, 0);
				...
				t2 = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
				rtd_printk(KERN_INFO, TAG_VPQ_MEM, "De-XC mem disconnect, return memroy from CMA = %x,t0~t2=%x,%x,%x,\n", Pli_addr, t0, t1, t2);
			}
		}
	} else {	// use VBM co-buffer
		if (Force_Clr == 1) {
...
unsigned int  drv_memory_I_De_XC_DMA_Config(unsigned int DI_phyAddr, unsigned char deXC_mode)
{
	...
	info_bit = (isInfo_Compress == 0)?(22):(16);	// for m4, // normal case 16, worth case 24 for info compression per 2 pixel
	info_len = 10;
	dat_len = 32;
