...
void panel_parameter_send_info(void)
{
	...
	if ((webos_tooloption.eModelModuleType == module_LGD) && (webos_tooloption.eBackLight == direct ) && (webos_tooloption.eLEDBarType == local_dim_block_32)){
		pr_notice("[ben dbg]send specific panel index to video fw \n");
		parameter_info.specific_panel_index = 1;	// LGD specific panel, go hdmi data frc
	}else{
...
void Check_smooth_toggle_update_flag(unsigned char display)
{
	...
	{//VO and Not JPEG case, jpag goes kernel smooth toggle
		...
		if((IoReg_Read32(PPOVERLAY_Display_Timing_CTRL1_reg) & PPOVERLAY_Display_Timing_CTRL1_disp_en_mask) == 0) {
			pr_info("power off,don't need to wait smoothtoggle\n");
