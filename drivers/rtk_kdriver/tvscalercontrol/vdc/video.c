...
//	VDC_POWER_SETTING ucPowerMode = VDC_Power_off;
	...
	if (on_off == ON) {
		...
		//fwif_color_SetAutoMAFlag(0); //remove for fix KTASKWBS-10486
	}
...
void drvif_module_vdc_ResetReg(void)
{
	...
	drvif_module_vdc_SECAM_y_fir_adj(VDC_secam_y_fir_enable, VDC_hpf_bypass, VDC_y_lpf_L1p8_H6p6_MHz/*VDC_y_lpf_L2p0_H6p4_MHz*/);
				...
				if (result == 0) {
					...
					if (vdc_hlock == 1) {
						...
						VDC_DEBUG_MSG(VDC_MSG_TVSCAN, " oV:%x, nV:%x, dg:%x, ns:%x\n",
									  vdc_vlock, vdc_vlock_new,
									  vdc_dgain, vdc_noise_status);
						if ((vdc_vlock == 1) && (vdc_noise_status <= 0xf0)) {
							unsigned int line_1 = drvif_module_vdc_read_V_1st_line();
							unsigned int line_2 = drvif_module_vdc_read_V_2nd_line();
							unsigned int line_diff = ABS(line_1, line_2);
...
void drvif_module_vdc_CheckInputStatus(unsigned char ucInputSrc, unsigned char ucCheckStart)
{
	...
	if (ucInputSrc == _SRC_CVBS && display_timing_ctrl1_REG.disp_fix_last_line == 0x00) {
		//rtd_printk(KERN_EMERG, TAG_NAME_VDC,"ADC3_test_0\n");
		...
		if (vd_ifd_input_vd_adc_lsb_ctrl_reg.adc3_lsb_force0_num != 0x02){
			...
			rtd_printk(KERN_EMERG, TAG_NAME_VDC,"ADC3_test_1\n");
		}
	}else{
		//rtd_printk(KERN_EMERG, TAG_NAME_VDC,"ADC3_test_2\n");
		vd_ifd_input_vd_adc_lsb_ctrl_reg.regValue = vdcf_inl(VD_IFD_INPUT_VD_ADC_LSB_CTRL_reg);
		if (vd_ifd_input_vd_adc_lsb_ctrl_reg.adc3_lsb_force0_num == 0x02){
			vd_ifd_input_vd_adc_lsb_ctrl_reg.adc3_lsb_force0_num = 0x00;
			vdcf_outl(VD_IFD_INPUT_VD_ADC_LSB_CTRL_reg, vd_ifd_input_vd_adc_lsb_ctrl_reg.regValue);
			rtd_printk(KERN_EMERG, TAG_NAME_VDC,"ADC3_test_3\n");
		}
	}
	//[end]
#endif
...
//	unsigned char mode;
	...
	rtd_printk(KERN_INFO, TAG_NAME_VDC,"_PRINT_AVD_COST_TIME: %s(%d) time = %d\n",__FUNCTION__,__LINE__,IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)/90);
#endif
...
void  drvif_module_vdc_big_signal_detect_function(bool enable)
{
  ...
  if(vdtop_noise_status_reg.status_noise<=0x0a)
    {
                   printk("[VDC] big signal function \n ");
                                ...
                                if(vdtop_adc_over_under_flag_reg.ad4_value_big==0)
                                        {
                                                ...
                                                printk("[VDC] big signal function  ,count_big_signal_fail=%d\n ",count_big_signal_fail);
                                    ...
                                    if(count_big_signal_fail>2)
                                        {
                                              ...
                                              printk("[VDC] big signal is not coming ,change the normal signal \n");
                                                count_big_signal_fail=0;
                                ...
                                if(vdtop_adc_over_under_flag_reg.ad4_value_big==1)
                                        {
                                                                        ...
                                                                        count_big_signal_ok=count_big_signal_ok+1;
																						   printk("[VDC] big signal function  ,count_big_signal_ok=%d\n ",count_big_signal_ok);
                                         ...
                                         if(count_big_signal_ok>3)
                                        {
                                                ...
                                                printk("[VDC] big signal  is coming \n");
                                                 count_big_signal_ok=0;
