...
void drvif_IESM_frontporch_set(unsigned char game_mode_check_flag)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "front_porch_num=vlen-actlen-actsta: %d=%d-%d-%d, game_mode=%d,size=%d-%d,flag=%d,lineMode=%d,fmt_422to444=%d,\n",
...
void drvif_color_Set_Block_DeContour_Ctrl(DRV_Block_DeContour_Ctrl *ptr)
{
	...
	blk_de_contour_ctrl_reg.regValue = IoReg_Read32(IPQ_DECONTOUR_BLK_De_Contour_CTRL_reg);
        ...
        if( Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE) == TRUE)//k6 not support i mode block contour:
        {

                blk_de_contour_ctrl_reg.bdecont_en = 0;
                blk_de_contour_ctrl_reg.bdecont_en_c = 0;
        }
        else
        {
