...
FILM_FWMode DETECTION_scalerVIP_film_FWmode()
{
	...
	if(FrameDiff>(PrevDiff<<7)&&FrameDiff>(NextDiff<<7)){
        	if(im_di_si_film_final_result_reg.regValue&0x400)
                {
        		rtd_printk(KERN_EMERG, TAG_NAME, "[fw_film_framediff_big]\t");
        	}
		...
		hw_StatusNextFieldMotion_film22_All|=0x80;
        	...
        	if(im_di_si_film_final_result_reg.regValue&0x400)
                {
        		rtd_printk(KERN_EMERG, TAG_NAME, "[fw_film_prevdiff_big]\t");
        	}
		...
		hw_StatusNextFieldMotion_film22_All|=0x80;
        	...
        	if(im_di_si_film_final_result_reg.regValue&0x400)
                {
        		rtd_printk(KERN_EMERG, TAG_NAME, "[fw_film_nextdiff_big]\t");
        	}
        ...
        else
        {
        	if(im_di_si_film_final_result_reg.regValue&0x400)
                {
        		rtd_printk(KERN_EMERG, TAG_NAME, "[fw_film_not_big]\t");
        	}
        }
	if(fw_film22_pair){
...
void APPLY_scalerVIP_film_FWmode(FILM_FWMode DetInformation)
{
	...
	*/
	...
	if(fw_film_suspend == 1)
	{
        ...
        if((film_index == FILM22)&&(RPC_system_info_structure_table !=NULL))
        {
        	
                if( RPC_system_info_structure_table->VIP_source ==VIP_QUALITY_DTV_576I)
                {
        	        im_di_film_new_function_main_reg.fw_film_mixedfilmdetected = 1;
                        DetInformation.Fw_TopLine1=252;// 288*7/8;
                        DetInformation.Fw_BotLine1=300;
                        DetInformation.Fw_TopLine2=1024;
                        DetInformation.Fw_BotLine2=1024;
                        DetInformation.Fw_TopLine3=1024;
                        DetInformation.Fw_BotLine3=1024;
                }
        }
