...
#include <tvscalercontrol/vip/hdr10plus.h>
#include <memc_isr/scalerMEMC.h>
//#include <tvscalercontrol/vip/libosr.h>//k6, gsr
//#include "../libosr.h"//k6, gsr,
...
#include <Platform_Lib/TVScalerControl/vip/fcic.h>
...
VIP_RGB2YUV_COEF_CSMatrix RGB2YUV_COEF_CSMatrix_s0;
VIP_RGB2YUV_COEF_CSMatrix RGB2YUV_COEF_CSMatrix_s1;
extern unsigned char vsc_i2rnd_sub_stage;
extern unsigned char vsc_i2rnd_dualvo_sub_stage;
extern void scalerVIP_Write_s0_RGB2YUV(unsigned long arg);
...
unsigned short pApl[34][60] = {0}; // report to GSR
unsigned short LC_APL[2040] = {0}; // read from LC, maximum size = 60*34
...
/*========== POD & PCID ============*/
unsigned int PCID_RgnReadTbl[PCID_TBL_LEN] = {0};
unsigned int PCID_RgnWriteTbl[PCID_TBL_LEN] = {0};
unsigned char PCID_SampleRgnTbl[PCID_TBL_LEN] = {0};
/*==================================*/
...
//extern int LG_GSR_Status;/*gsr, .ko*/
//extern unsigned short LG_GSR_GetAdaptiveRgbGain(unsigned int * pstParams, unsigned short pApl[34][60], unsigned short maxGain);/*gsr, .ko*/
...
extern UINT8 v4l2_vpq_stereo_face;//0: off, 1: on, 2: demo
void fwif_color_set_dcc_Color_Independent_driver(unsigned char src_idx, unsigned char value)
{
	...
	if(v4l2_vpq_stereo_face != V4L2_VPQ_EXT_STEREO_FACE_OFF)
		drvif_color_set_dcc_Color_Independent_AI(Color_Independent_Blending_Table);
	else
...
void fwif_color_Icm_table_select_By_DMA(unsigned char display, unsigned short *tICM_ini, unsigned char which_table)
{
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		rtd_printk(KERN_INFO, TAG_NAME, "ICM, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
		...
		if(ret ==false)
		{
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "ICM : load icm fail set icm=disable \n");
...
void fwif_color_D_LUT_By_DMA(unsigned char display, unsigned char enable, unsigned int *pArray)
{
	...
	if (addr == 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_D_LUT_By_DMA, addr = NULL\n");
		return;
	}
#if 0
	ret = drvif_Get_VO_Active_Info(&v_sta, &v_end, &line_cnt);
	if (ret < 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_D_LUT_By_DMA is not ready, ret = %d\n", ret);
		//return -1;
	} else {
		...
		if (ret < 0) {
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_D_LUT_By_DMA fail to wait for active, ret = %d, v_end=%d, v_sta=%d, delay1=%d, delay2=%d,\n",
	...
	rtd_printk(KERN_INFO, TAG_NAME, "D_3D LUT, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
		addr, pVir_addr, addr_aligned, pVir_addr_aligned, size, size_aligned);
...
char fwif_color_D_LUT_By_DMA_16(unsigned char display, unsigned char enable, unsigned short *pArray)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "D_3D_LUT_16, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
		addr, pVir_addr, addr_aligned, pVir_addr_aligned, size, size_aligned);
...
/*cal edge_shp*/
	...
	/*LV gain fixed value*/
	sharp_gain.Edge_Level.LV_Gain = gVip_Table->Ddomain_SHPTable[table].Edge_Shp_set.LV_Max;
...
/*cal Texture_shp*/
	...
	/*LV gain fixed value*/
	sharp_gain.Texture_Level.LV_Gain = gVip_Table->Ddomain_SHPTable[table].Texture_Shp_set.LV_Max;
...
/*cal vertical*/
	...
	/*LV gain fixed value*/
	sharp_gain.Vertical_Level.LV_Gain = gVip_Table->Ddomain_SHPTable[table].Vertical.LV_Max;
	...
	/*LV gain fixed value*/
	sharp_gain.Vertical_Edge_Level.LV_Gain = gVip_Table->Ddomain_SHPTable[table].Vertical_edg.LV_Max;
...
{
        static DRV_D_DLTI_Table drv_shp_Dlti;
...
void fwif_color_set_sharpness_table(unsigned char src_idx, unsigned char value)
{
	...
	sharp.SR_Control.pndu = gVip_Table->Ddomain_SHPTable[value].SR_Control.pndu;
...
void fwif_color_VIPShpTable_to_DRVShpTable(VIP_Sharpness_Table *VIPShpTable, DRV_Sharpness_Table *DRVShpTable)
{
	...
	DRVShpTable->SR_Control.pndu = VIPShpTable->SR_Control.pndu;
	DRVShpTable->Vertical.vpk_sr_tex = VIPShpTable->Vertical.vpk_sr_tex;
	DRVShpTable->Vertical.vpk_sr_edg = VIPShpTable->Vertical.vpk_sr_edg;
...
void fwif_color_set_scaleuph_8tap_driver(unsigned char src_idx, unsigned char display, unsigned char TableIdx)
{
	...
	if (VIP_system_info_structure_table != NULL) {
		if ((get_vsc_run_pc_mode()) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) &&
			(VIP_system_info_structure_table->Mem_Width) <= 2880)
			TableIdx = 41;
	}
...
void fwif_color_set_scaleupv_6tap_driver(unsigned char src_idx, unsigned char display, unsigned char TableIdx)
{
	_system_setting_info *VIP_system_info_structure_table = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	_RPC_system_setting_info *VIP_RPC_system_info_structure_table = (_RPC_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_RPC_system_info_structure);
	extern DRV_SU_table_V8tap_t SU_table_v8tap[17];
	...
	if (TableIdx >= 17) TableIdx = 12;		//20150804 roger protect the index overflow

	if ((get_vsc_run_pc_mode()) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) &&
		(VIP_system_info_structure_table->Mem_Height) < 2048)
		TableIdx = 16;

	if ((drvif_color_ultrazoom_scalerup_PR_mode(VIP_system_info_structure_table->Mem_Width) == NOT_PR_MODE) &&
	    ((VIP_system_info_structure_table->Mem_Width) <= 1920))
...
unsigned char fwif_color_get_colorspacergb2yuvtransfer_table_SDR_MAX_RGB(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, bool bUse_HDMI_Range_Detect)
{
	...
	if (colorFMT_info == COLORSPACE_MAX) {
	...
	rtd_printk(KERN_INFO, TAG_NAME, "CSC_SMR,ch=%d, Src=%d,HD=%d,.EC=%d,.C=%d,colour_primaries=%d,ucFormat=%d,\n",
		channel, nSrcType, nSD_HD, pAviInfo.EC, pAviInfo.C, pVOInfo->colour_primaries, ucFormat);
	...
	if(pVOInfo != NULL){
		rtd_printk(KERN_INFO, TAG_NAME, "CSC_SMR,Hdmi_Mode()=%d,HDMI_CSC_DataRange=%d,IsRGB_YUV=%d,C=%d,EC=%d,colour_primaries=%d,InputInfo()=%d,VOFrom()=%d,vsc_source=%d,\n",
...
unsigned char fwif_color_get_colorspacergb2yuvtransfer_table_SDR_MAX_RGB_TV006(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, bool bUse_HDMI_Range_Detect)
{
...
#if 0	// "Input_Data_Mode" always use limited, D-doamin will always use "limit to full", output of DM is RGB full
	// "Input_Data_Mode" always use limited, D-doamin will always use "limit to full", output of DM is RGB full
	if (colorFMT_info == COLORSPACE_MAX) {
		table_num = RGB2YUV_COEF_BYPASS_0_255_TO_16_235;
		VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_601_Limted;
	...
	rtd_printk(KERN_INFO, TAG_NAME, "CSC_SMR06,ch=%d, Src=%d,HD=%d,.EC=%d,.C=%d,colour_primaries=%d,ucFormat=%d,\n",
		channel, nSrcType, nSD_HD, pAviInfo.EC, pAviInfo.C, pVOInfo->colour_primaries, ucFormat);
}
...
void fwif_color_colorspacergb2yuvtransfer(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, unsigned char VO_Jpeg_Back_YUV, unsigned char useSendRPC)
{
	...
	if (vpq_project_id == 0x00060000 && vpqex_project_id == 0x00060000) {
		if ((VIP_system_info_structure_table ->HDR_flag == HAL_VPQ_HDR_MODE_SDR) || (channel == SLR_SUB_DISPLAY)) {		//20160107 roger, sub == non-HDR in K2L
		...
		if ((VIP_system_info_structure_table ->HDR_flag == HAL_VPQ_HDR_MODE_SDR) || (channel == SLR_SUB_DISPLAY)) {		//20160107 roger, sub == non-HDR in K2L
			...
			if (VIP_system_info_structure_table ->HDR_flag == HAL_VPQ_HDR_MODE_HDR10p)
				table_index = &(tRGB2YUV_COEF[RGB2YUV_COEF_RGB_0_255_TO_YCC709_16_235][0]);
			else
				table_index = &(tRGB2YUV_COEF[RGB2YUV_COEF_RGB_16_235_TO_YCC709_16_235][0]);
			VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_709_Limted;
		}
...
//	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "rgb2yuv,display=%d,tab_num=%d,input_mode=%d,M+=%d,BOE=%d, M+_En=%d,RGB444=%d,Mode_src=%d,HDR_en=%d, HDR_csc=%d,hdr_flag=%d,\n",
	...
	if (NULL !=  table_index) {
		...
		if (VIP_system_info_structure_table !=  NULL) {
#ifdef CONFIG_I2RND_ENABLE
			//[ANDROIDTV-3549] i2run SDR+HDR case need to use different matrix table. @Crixus 20171110
			if((channel == SLR_MAIN_DISPLAY) && (Scaler_I2rnd_get_timing_enable() == _ENABLE) && ((vsc_i2rnd_dualvo_sub_stage == I2RND_DUALVO_STAGE_RERUN_MAIN_FOR_S1) || (vsc_i2rnd_sub_stage == I2RND_STAGE_MAIN_GET_RPC))){
				memcpy((unsigned short *)(&RGB2YUV_COEF_CSMatrix_s1), table_index, sizeof(VIP_RGB2YUV_COEF_CSMatrix));
				fwif_color_ChangeUINT16Endian((unsigned short *)&RGB2YUV_COEF_CSMatrix_s1, sizeof(VIP_RGB2YUV_COEF_CSMatrix)/sizeof(short), 0);
				//printk(KERN_EMERG"[crixus]copy to s1 matrix!!!\n");
			}
			else{
				memcpy((unsigned short *)(&RGB2YUV_COEF_CSMatrix_s0), table_index, sizeof(VIP_RGB2YUV_COEF_CSMatrix));
				fwif_color_ChangeUINT16Endian((unsigned short *)&RGB2YUV_COEF_CSMatrix_s0, sizeof(VIP_RGB2YUV_COEF_CSMatrix)/sizeof(short), 0);
				//printk(KERN_EMERG"[crixus]copy to s0 matrix!!!\n");
			}
#else
#ifdef CONFIG_ARM64
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "rgb2yuv, VIP_system_info_structure_table = NULL, 11\n");
		}
...
#ifdef CONFIG_I2RND_ENABLE
			//[ANDROIDTV-3549] i2run SDR+HDR case need to use different matrix table. @Crixus 20171110
			//Eric@20171221 s1 matrix table move before rerun main done to avoid i2rnd cmd table switch issue
			if(!((channel == SLR_MAIN_DISPLAY) && (Scaler_I2rnd_get_timing_enable() == _ENABLE) && ((vsc_i2rnd_dualvo_sub_stage == I2RND_DUALVO_STAGE_RERUN_MAIN_FOR_S1) || (vsc_i2rnd_sub_stage == I2RND_STAGE_MAIN_GET_RPC)))){
				scalerVIP_Write_s0_RGB2YUV(1);
				//printk(KERN_EMERG"[crixus]trigger to run s0 matrix!!!\n");
			}
#else
		...
		rtd_printk(KERN_EMERG, TAG_NAME, "rgb2yuv, ERROR, table_index == NULL !!\n");
	}

	if (VIP_system_info_structure_table == NULL || VIP_RPC_system_info_structure_table == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "rgb2yuv, ERROR, VIP_system_info_structure_table == NULL !!\n");
	} else {
...
void fwif_color_set_LD_Global_Ctrl(unsigned char src_idx, unsigned char TableIdx)
{
	...
	{
	        data.ld_comp_en = 0; //oled modle should be off local dimming
	}
        else
        {
...
{
	#if 0
		extern unsigned int LD_Backlight_Profile_Interpolation_table_H[BL_Profile_Table_NUM][BL_Profile_Table_COLUMN];
		extern unsigned int LD_Backlight_Profile_Interpolation_table_V[BL_Profile_Table_NUM][BL_Profile_Table_COLUMN];
		drvif_color_set_LD_BL_Profile_Interpolation_Table_HV((unsigned int *) &LD_Backlight_Profile_Interpolation_table_H, (unsigned int *) &LD_Backlight_Profile_Interpolation_table_V);
...
void fwif_color_set_LD_BL_Profile_Interpolation_Table_HV(unsigned char src_idx, unsigned char TableIdx)
{
	...
	extern SLR_VIP_TABLE_BLPF m_defaultVipTableBLPF[3];
	drvif_color_set_LD_BL_Profile_Interpolation_Table_HV((unsigned int *) &m_defaultVipTableBLPF[TableIdx].LD_Backlight_Profile_Interpolation_table_H, (unsigned int *) &m_defaultVipTableBLPF[TableIdx].LD_Backlight_Profile_Interpolation_table_V);
...
void fwif_color_set_LC_Enable(unsigned char src_idx, bool enable)
{
	...
	{
		rtd_printk(KERN_DEBUG, TAG_NAME, "[%s][%d] Hdmi PCmode cannot enable LC\n", __FUNCTION__, enable);
...
void fwif_color_set_LC_Global_Ctrl(unsigned char src_idx,unsigned char TableIdx)
{
	...
	//data.lc_3d_mode = Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_3d_mode;/*merlin5, delete*/
	...
	//data.lc_gcon_mode = Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_gcon_mode;/*merlin5, delete*/
...
void fwif_color_set_LC_Backlight_Profile_Interpolation(unsigned char src_idx,unsigned char TableIdx)
{
	...
	//data.lc_hinitphase3_right = Local_Contrast_Table[TableIdx].LC_Backlight_Profile_Interpolation.lc_hinitphase3_right;/*merlin5, delete*/
	...
	//data.lc_vinitphase3_right = Local_Contrast_Table[TableIdx].LC_Backlight_Profile_Interpolation.lc_vinitphase3_right;/*merlin5, delete*/
...
void fwif_color_set_LC_Global_Region_Num_Size(unsigned char TableIdx, unsigned char TotalBlkNum)
{
	...
	switch(TotalBlkNum)
	{
		...
		case LC_BLOCK_60x34:
			...
			break;
		default:
			rtd_printk(KERN_EMERG, TAG_NAME, "[LC]block mode error, set mode=%d\n", TotalBlkNum);
			nBlkHNum = 16;
			nBlkVNum = 16;
			break;
	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[%s] GetSize %d %d\n", __FUNCTION__, nImageHsize, nImageVsize);
	rtd_printk(KERN_INFO, TAG_NAME, "[%s] Info2VideoFW Tbl=%d TabH/V=%d/%d NumH/V=%d/%d\n",
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[%s][H] Num=%d, Size=%d, Tab=%d, Factor=%d\n",
		__FUNCTION__, nBlkHNum, LC_Region_Num_Size.nBlk_Hsize, tab_hsize, LC_Region_Num_Size.lc_hfactor);
	rtd_printk(KERN_INFO, TAG_NAME, "[%s][V] Num=%d, Size=%d, Tab=%d, Factor=%d\n",
		__FUNCTION__, nBlkVNum, LC_Region_Num_Size.nBlk_Vsize, tab_vsize, LC_Region_Num_Size.lc_vfactor);
...
)
{
...
#if 0
	else if(nImageHsize == 3840 && nImageVsize == 2160)
	{
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] 4k Case\n", __FUNCTION__);
	    nBlkHNum = 59;
	...
	else if(nImageHsize == 1920 && nImageVsize == 1080)
	{
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] 2k Case\n", __FUNCTION__);
		...
		for( i=60; i>=16; i-- )
		{
			...
			if( i<=(nBlkHNum+1) && ValidHBlk==0 )
			{
				...
				if( fwif_color_LC_check_Hblk_size(nRegionHSize) == 0 )
					continue;
				if( nImageHsize-(nRegionHSize*(i-1)) < 40 )
					continue;
				if( nRegionHSize%2 == 1 )
					continue;
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[%s][%d][H] Num = %d, Tab = %d, Factor = %d, RgnSize = %d\n",
		__FUNCTION__, ValidHBlk, nBlkHNum, tab_hsize, h_factor, nRegionHSize);
	rtd_printk(KERN_INFO, TAG_NAME, "[%s][%d][V] Num = %d, Tab = %d, Factor = %d, RgnSize = %d\n",
...
void fwif_color_set_LC_saturation(unsigned char src_idx,unsigned char TableIdx)
{
	...
	DRV_LC_saturation data;
	if( TableIdx >= LC_Table_NUM )
		TableIdx = LC_Table_NUM - 1;

	data.LC_saturation_en= Local_Contrast_saturation_Table[TableIdx].LC_saturation_en;
	data.LC_no_saturation_enhance_thl= Local_Contrast_saturation_Table[TableIdx].LC_no_saturation_enhance_thl;
	data.LC_saturation_blend_factor= Local_Contrast_saturation_Table[TableIdx].LC_saturation_blend_factor;
	data.LC_saturation_Gain= Local_Contrast_saturation_Table[TableIdx].LC_saturation_Gain;

	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m11= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m11;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m12= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m12;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m13= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m13;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m21= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m21;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m22= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m22;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m23= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m23;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m31= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m31;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m32= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m32;
	data.LC_saturation_r2y.LC_saturation_rgb2yuv_m33= Local_Contrast_saturation_Table[TableIdx].LC_saturation_r2y.LC_saturation_rgb2yuv_m33;

	data.LC_saturation_y2r.LC_saturation_yuv2rgb_k11= Local_Contrast_saturation_Table[TableIdx].LC_saturation_y2r.LC_saturation_yuv2rgb_k11;
	data.LC_saturation_y2r.LC_saturation_yuv2rgb_k12= Local_Contrast_saturation_Table[TableIdx].LC_saturation_y2r.LC_saturation_yuv2rgb_k12;
	data.LC_saturation_y2r.LC_saturation_yuv2rgb_k13= Local_Contrast_saturation_Table[TableIdx].LC_saturation_y2r.LC_saturation_yuv2rgb_k13;
	data.LC_saturation_y2r.LC_saturation_yuv2rgb_k22= Local_Contrast_saturation_Table[TableIdx].LC_saturation_y2r.LC_saturation_yuv2rgb_k22;
	data.LC_saturation_y2r.LC_saturation_yuv2rgb_k23= Local_Contrast_saturation_Table[TableIdx].LC_saturation_y2r.LC_saturation_yuv2rgb_k23;
	data.LC_saturation_y2r.LC_saturation_yuv2rgb_k32= Local_Contrast_saturation_Table[TableIdx].LC_saturation_y2r.LC_saturation_yuv2rgb_k32;
	data.LC_saturation_y2r.LC_saturation_yuv2rgb_k33= Local_Contrast_saturation_Table[TableIdx].LC_saturation_y2r.LC_saturation_yuv2rgb_k33;

	drvif_color_set_LC_saturation((DRV_LC_saturation*) &data);
...
extern DRV_LC_decontour Local_Contrast_Decontour_Table[LC_Table_NUM][LC_De_Cont_Level];
void fwif_color_set_LC_decontour(unsigned char src_idx,unsigned char TableIdx,unsigned char level)
{
	if(TableIdx>=LC_Table_NUM)
		TableIdx=LC_Table_NUM-1;
	if(level>=LC_De_Cont_Level)
		TableIdx=LC_De_Cont_Level-1;

    drvif_color_set_LC_decontour( &(Local_Contrast_Decontour_Table[TableIdx][level]) );
}
		...
		if ( (size > COLOR_HISTOGRAM_LEVEL)||((COLOR_HISTOGRAM_LEVEL%size)!=0)) {
			...
			for (i = 0; i < size; i++) {
			        Y_Hist_distr_cnt[i] = luma_distribution[i];
                                ...
                                /*if(i>=31){  //the method is wrong.there are not distribution need check it
					Y_Hist_distr_cnt[i] = smartPic_clue->Y_Main_Hist[127];
				}else{
					Y_Hist_distr_cnt[i] = smartPic_clue->Y_Main_Hist[i * 4];
				}*/
			}
...
bool fwif_color_od_init(UINT8 bits, UINT8 dma_mode, UINT8 freq)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "OD_Height=%d,OD_Width=%d,freq=%d, oled_panel=%d,\n",
		dispV,dispH,Get_DISPLAY_REFRESH_RATE(), strcmp(webos_strToolOption.eBackLight, "oled"));
...
#if 1 // henry modified in K6
	if (platform == PLATFORM_KXLP)  // 60 hz
		bODInited = drvif_color_od_pqc(dispV, dispH, bit_sel, dma_mode, 4, 1, 3);
	else // 120 hz
		bODInited = drvif_color_od_pqc_120hz(dispV, dispH/2, 2, dma_mode, 8, 1, 1);
#else
...
void fwif_color_set_pcid(unsigned char bpcid)
{
	...
	VIPprintf("[%s][Verify] set %d, hcic %d %d\n", __func__, bpcid, sys_reg_sys_clken3_reg.clken_disp_lg_hcic, ppoverlay_display_timing_ctrl2_reg.hcic_enable);
	...
	if(bpcid!=0)
	{
		...
		for(TblIdx=0; TblIdx<16; TblIdx++)
		{
			for(i=0; i<PCID_TBL_LEN; i++)
			{
				PCID_SampleRgnTbl[i] = TblIdx*8;
			}
			fwif_color_set_pcid_RgnTableValue(PCID_SampleRgnTbl, TblIdx, _PCID2_COLOR_ALL);
		}
		drvif_color_set_pcid_enable(1);
	}
	else
	{
		drvif_color_set_pcid2_enable(0);
...
void fwif_color_set_pcid_pixel_setting(void)
{
	...
	sPcid2Setting.pixel_ref.line1.even_r.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line1.even_g.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line1.even_b.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line2.even_r.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line2.even_g.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line2.even_b.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line3.even_r.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line3.even_g.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line3.even_b.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line4.even_r.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line4.even_g.line_sel  = _REF_LINE_CURR;
	...
	sPcid2Setting.pixel_ref.line4.even_b.line_sel  = _REF_LINE_CURR;
...
void fwif_color_set_pcid2(unsigned char bpcid2)
{
	...
	sys_reg_sys_clken3_reg.regValue = IoReg_Read32(SYS_REG_SYS_CLKEN3_reg);
	ppoverlay_display_timing_ctrl2_reg.regValue = IoReg_Read32(PPOVERLAY_Display_Timing_CTRL2_reg);
	VIPprintf("[%s][Verify] set %d, hcic %d %d\n", __func__, bpcid2, sys_reg_sys_clken3_reg.clken_disp_lg_hcic, ppoverlay_display_timing_ctrl2_reg.hcic_enable);
	...
	if(bpcid2!=0)
	{
		...
		if (drvif_color_get_pcid_enable() == 1 ) {
			...
			if (timeout_cnt <= 10) {
				rtd_printk(KERN_EMERG, TAG_NAME, "kernel copy VPQ_IOC_SET_OdExtentions_pData(PCID1) disable time out\n");
			}
		}
		...
		if (drvif_color_get_pcid2_enable() == 1 ) {
			...
			if (timeout_cnt <= 10) {
				rtd_printk(KERN_EMERG, TAG_NAME, "kernel copy VPQ_IOC_SET_OdExtentions_pData(PCID2) disable time out\n");
			}
		}
		...
		for(TblIdx=0;TblIdx<16;TblIdx++)
		{
			...
			fwif_color_set_pcid_RgnTableValue(PCID_SampleRgnTbl+(TblIdx*PCID_TBL_LEN*sizeof(unsigned char)), TblIdx, _PCID2_COLOR_ALL);
		}
...
void fwif_color_set_pcid2_pixel_setting(void)
{
	...
	sPcid2Setting.pixel_ref.line1.even_r.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line1.even_r.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line1.even_r.color_sel = _REF_COLOR_G;

	sPcid2Setting.pixel_ref.line1.even_g.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line1.even_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line1.even_g.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line1.even_b.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line1.even_b.point_sel = _REF_POINT_NEXT;
	sPcid2Setting.pixel_ref.line1.even_b.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line1.odd_r.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line1.odd_r.point_sel = _REF_POINT_PREV;
	sPcid2Setting.pixel_ref.line1.odd_r.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line1.odd_g.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line1.odd_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line1.odd_g.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line1.odd_b.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line1.odd_b.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line1.odd_b.color_sel = _REF_COLOR_G;

	/* line 2 pixel setting */
	sPcid2Setting.pixel_ref.line2.even_r.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line2.even_r.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line2.even_r.color_sel = _REF_COLOR_G;

	sPcid2Setting.pixel_ref.line2.even_g.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line2.even_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line2.even_g.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line2.even_b.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line2.even_b.point_sel = _REF_POINT_NEXT;
	sPcid2Setting.pixel_ref.line2.even_b.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line2.odd_r.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line2.odd_r.point_sel = _REF_POINT_PREV;
	sPcid2Setting.pixel_ref.line2.odd_r.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line2.odd_g.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line2.odd_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line2.odd_g.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line2.odd_b.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line2.odd_b.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line2.odd_b.color_sel = _REF_COLOR_G;

	/* line 3 pixel setting */
	sPcid2Setting.pixel_ref.line3.even_r.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line3.even_r.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line3.even_r.color_sel = _REF_COLOR_G;

	sPcid2Setting.pixel_ref.line3.even_g.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line3.even_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line3.even_g.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line3.even_b.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line3.even_b.point_sel = _REF_POINT_NEXT;
	sPcid2Setting.pixel_ref.line3.even_b.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line3.odd_r.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line3.odd_r.point_sel = _REF_POINT_PREV;
	sPcid2Setting.pixel_ref.line3.odd_r.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line3.odd_g.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line3.odd_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line3.odd_g.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line3.odd_b.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line3.odd_b.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line3.odd_b.color_sel = _REF_COLOR_G;

	/* line 4 pixel setting */
	sPcid2Setting.pixel_ref.line4.even_r.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line4.even_r.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line4.even_r.color_sel = _REF_COLOR_G;

	sPcid2Setting.pixel_ref.line4.even_g.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line4.even_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line4.even_g.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line4.even_b.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line4.even_b.point_sel = _REF_POINT_NEXT;
	sPcid2Setting.pixel_ref.line4.even_b.color_sel = _REF_COLOR_R;

	sPcid2Setting.pixel_ref.line4.odd_r.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line4.odd_r.point_sel = _REF_POINT_PREV;
	sPcid2Setting.pixel_ref.line4.odd_r.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line4.odd_g.line_sel  = _REF_LINE_CURR;
	sPcid2Setting.pixel_ref.line4.odd_g.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line4.odd_g.color_sel = _REF_COLOR_B;

	sPcid2Setting.pixel_ref.line4.odd_b.line_sel  = _REF_LINE_PREV;
	sPcid2Setting.pixel_ref.line4.odd_b.point_sel = _REF_POINT_CURR;
	sPcid2Setting.pixel_ref.line4.odd_b.color_sel = _REF_COLOR_G;
...
{
	DRV_pcid2_RgnTbl_t sRgnTblSetting;
	unsigned int i=0;

	sRgnTblSetting.RgnTblEnable = 1;

	for(i=0; i<3; i++)
	{
		sRgnTblSetting.HorIdx[i] = (3840/16) * (i+1);
		sRgnTblSetting.VerIdx[i] = (2160/16) * (i+1);
		sRgnTblSetting.HorBldStep[i] = 3;
		sRgnTblSetting.VerBldStep[i] = 3;
	}

	drvif_color_set_pcid_regional_table_setting(&sRgnTblSetting);
}
...
void fwif_color_refresh_pcid_RgnTable(void)
{
	...
	for(i=0; i<9; i++)
	{
		drvif_color_set_pcid_regional_table_valuetable(PCID_RgnWriteTbl, i ,3);
	}

}

void fwif_color_swap_pcid_XY_TV006(unsigned char* pTblValue)
{
	unsigned int x, y, i;
	unsigned char PCID_table_tmp[PCID_TBL_LEN] = {0};

	for (x = 0; x < 17; x++) {
		for(y = 0; y < 17; y++) {
			PCID_table_tmp[x * 17 + y] = pTblValue[y * 17 + x];
		}
	}

	for (i = 0; i < PCID_TBL_LEN; i++) {
		pTblValue[i] = PCID_table_tmp[i];
	}
}


void fwif_color_set_pcid_RgnTableValue(unsigned char* pTblValue, unsigned int ucTblIdx, unsigned char ucChannel)
{
	unsigned int i=0, j=0, Tbl1Idx=0, Tbl2Idx=0, Tbl3Idx=0, Tbl4Idx=0;
	unsigned char ucRgnIdx=0, ucWriteChannel=0;
	DRV_Tbl2SRAM_Val SRAMTblVal;
	bool isSameTbl = true;
	...
	if(isSameTbl == false)
	{
		if(ucChannel < 3)
		{
			for(i=0;i<4;i++)
			{
				ucRgnIdx = RgnMappingTbl[ucTblIdx][i];
				if(ucRgnIdx == RGN_NA)
					continue;
				...
				{
					SRAMTblVal.b32_24 = pcidRgnTbl[Tbl1Idx][ucChannel][j];
					SRAMTblVal.b23_16 = pcidRgnTbl[Tbl2Idx][ucChannel][j];
					SRAMTblVal.b15_08 = pcidRgnTbl[Tbl3Idx][ucChannel][j];
					SRAMTblVal.b07_00 = pcidRgnTbl[Tbl4Idx][ucChannel][j];
					PCID_RgnWriteTbl[j] = SRAMTblVal.b32Value;
				}
		...
		else
		{
			...
			for(i=0;i<4;i++)
			{
				...
				for(ucWriteChannel=0; ucWriteChannel<3; ucWriteChannel++)
				{

					for(j=0; j<PCID_TBL_LEN; j++)
					{
						SRAMTblVal.b32_24 = pcidRgnTbl[Tbl1Idx][ucWriteChannel][j];
						SRAMTblVal.b23_16 = pcidRgnTbl[Tbl2Idx][ucWriteChannel][j];
						SRAMTblVal.b15_08 = pcidRgnTbl[Tbl3Idx][ucWriteChannel][j];
						SRAMTblVal.b07_00 = pcidRgnTbl[Tbl4Idx][ucWriteChannel][j];
						PCID_RgnWriteTbl[j] = SRAMTblVal.b32Value;
					}
...
{
	//bool isGetTbl = false, isCorrectTbl = true;
	unsigned int i=0, j=0;
	unsigned char ucRgnIdx=0;
	char TblValue=0;
	DRV_Tbl2SRAM_Val SRAMTblVal;
	...
	for(i=0; i<4; i++)
	{
		...
		if(ucRgnIdx != RGN_NA)
		{
			...
			for(j=0; j<PCID_TBL_LEN; j++)
			{
				SRAMTblVal.b32Value = PCID_RgnReadTbl[j];
				switch(i)
				{
					case 0:
						TblValue = SRAMTblVal.b07_00;
						break;
					case 1:
						TblValue = SRAMTblVal.b15_08;
						break;
					case 2:
						TblValue = SRAMTblVal.b23_16;
						break;
					case 3:
						TblValue = SRAMTblVal.b32_24;
						break;
					default:
						TblValue = 0;
						break;
				}
				pTblValue[j] = TblValue;
				/*
				if(isGetTbl == false)
				{
					pTblValue[j] = TblValue;
				}
				else if( isGetTbl && pTblValue[j]!=TblValue )
				{
					VIPprintf("[VPQ][PCID] Rgn table not consist between regions\n");
					isCorrectTbl = false;
				}
				*/
			}
...
void fwif_color_set_valc(unsigned char bValc)
{
	// Just for ip firmware verification
	int i=0;
	DRV_valc_hpf_ctrl_t ValcHPFCtrl;
	DRV_valc_sat_ctrl_t ValcSATCtrl;

	for(i=0; i<16; i++)
	{
		ValcHPFCtrl.ClipSel = 0;
		ValcHPFCtrl.Weight[i] = i;
		ValcSATCtrl.Weight[i] = i;
	}

	if( bValc != 0 )
	{
		drvif_color_set_valc_enable(1);
		drvif_color_set_valc_SpatialPattern(_VALC_PAT_H1V1);
		drvif_color_set_valc_HPF_protect(&ValcHPFCtrl);
		drvif_color_set_valc_SAT_protect(&ValcSATCtrl);

		// write sample exp table
		for(i=0; i<VALC_TBL_LEN; i++)
		{
			VALC_SampleTbl[i] = 1023;//i*50;
		}
		fwif_color_set_valc_table(VALC_SampleTbl, 0, _VALC_COLOR_R);
		for(i=0; i<VALC_TBL_LEN; i++)
		{
			VALC_SampleTbl[i] = 0;// - (i*50);
		}
...
void fwif_color_set_valc_table(unsigned int* pTbl, unsigned char ucTblIdx, unsigned char ucChannel)
{
	...
	for(i=0; i<3; i++)
	{
		for(j=0; j<2; j++)
		{
			fwif_color_get_valc_table(valcTbl[i][j], j, i);
		}
	}
	...
	{
		SRAMVal.dummy0 = 0;
		SRAMVal.tbl1Value = (ucTblIdx==0)? pTbl[i] : valcTbl[ucChannel][0][i];
		SRAMVal.dummy1 = 0;
		SRAMVal.tbl2Value = (ucTblIdx==1)? pTbl[i] : valcTbl[ucChannel][1][i];
		VALC_WriteTbl[i] = SRAMVal.b32Value;
	}
...
void fwif_color_get_valc_table(unsigned int* pTbl, unsigned char ucTblIdx, unsigned char ucChannel)
{
	...
	for(i=0; i<VALC_TBL_LEN; i++)
	{
		SRAMVal.b32Value = VALC_ReadTbl[i];
		if( ucTblIdx == 0 )
		{
			pTbl[i] = SRAMVal.tbl1Value;
		}
		else
		{
			pTbl[i] = SRAMVal.tbl2Value;
		}
	}
}
...
static unsigned char WB_MEMC_forcebg_flag = 0;
void set_WB_MEMC_forcebg_flag(unsigned char enable)
{
	WB_MEMC_forcebg_flag = enable;
}
unsigned char get_WB_MEMC_forcebg_flag(void)
{
	return WB_MEMC_forcebg_flag;
}
...
void fwif_color_set_WB_Pattern_IRE(unsigned char ucType, unsigned char ucIRE)
{
	...
	static unsigned int uzu_4k120_pre;
	...
	if(timeoutcount_orbit_wb == 0)
		rtd_printk(KERN_NOTICE, TAG_NAME,"[WB]Orbit timeout error!!!\n");
	...
	if (ucType){
		...
		Scaler_MEMC_output_force_bg_enable(1);
		...
		//set 120fps
	    uzudtg_control1_reg.regValue = rtd_inl(PPOVERLAY_uzudtg_control1_reg);
		uzu_4k120_pre = uzudtg_control1_reg.uzu_4k_120_mode;
		if((uzu_4k120_pre == 0) && Get_DISPLAY_REFRESH_RATE()==120){
			uzudtg_control1_reg.uzu_4k_120_mode = 1;
		}
		rtd_outl(PPOVERLAY_uzudtg_control1_reg, uzudtg_control1_reg.regValue);
		...
		Scaler_MEMC_output_force_bg_enable(0);
	} else if(TV006_WB_Pattern_En){
		Scaler_MEMC_output_force_bg_enable(1);
		set_WB_MEMC_forcebg_flag(1);
		/*exit WB*/
	...
	//	if (platform == PLATFORM_KXLP)  // 60 hz,120hz no need bypass memc
		{
	        rtd_clearbits(0xb809d700, _BIT9);
		...
		if(uzu_4k120_pre == 0){
		    uzudtg_control1_reg.regValue = rtd_inl(PPOVERLAY_uzudtg_control1_reg);
			uzudtg_control1_reg.uzu_4k_120_mode = 0;
			rtd_outl(PPOVERLAY_uzudtg_control1_reg, uzudtg_control1_reg.regValue);
		}
...
void fwif_color_set_LDSetLUT(UINT8 lutTableIndex)
{
	...
	fwif_color_set_LD_Global_Ctrl(0, lutTableIndex);
...
bool fwif_color_set_LED_Initialize(HAL_LED_PANEL_INFO_T panelInfo)
{
	...
	/*rtd_printk(KERN_DEBUG, TAG_NAME, "ic=%d, bl_type=%d, bar_type=%d\n",panelInfo.hal_icType,panelInfo.hal_bl_type,panelInfo.hal_bar_type);*/
	//if( webos_tooloption.eBackLight == 2 ) // bypass settings on OLED from tool option
	if (strcmp(webos_strToolOption.eBackLight, "oled") == 0)
	{
		pass = 0;
	...
	if (panelInfo.hal_icType == 1) {
		if (panelInfo.hal_bl_type == HAL_BL_EDGE_LED) {
			...
			blu_ld_inter_vctrl2_RBUS blu_ld_inter_vctrl2;/* for ld_vinitphase1*/
			...
			for (i=0;i<LD_Table_NUM;i++)
				Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_type = 1; /*update ld_blk_type info in pq misc*/

			if (panelInfo.hal_bar_type == HAL_LED_BAR_6) {
				fwif_color_Set_BLPF_type(BLPF_EDGE_6);
				blu_ld_global_ctrl2.ld_blk_hnum = 5;
				for (i=0;i<LD_Table_NUM;i++)
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 5; /*update ld_blk_hnum info in pq misc*/
			} else if (panelInfo.hal_bar_type == HAL_LED_BAR_12) {
				fwif_color_Set_BLPF_type(BLPF_EDGE_12);
				blu_ld_global_ctrl2.ld_blk_hnum = 11;
				for (i=0;i<LD_Table_NUM;i++)
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 11; /*update ld_blk_hnum info in pq misc*/
			} else {
				pass = 0;
			}
			...
			blu_ld_inter_hctrl1.ld_hfactor = 1048576*(Local_Dimming_Table[0].LD_Backlight_Profile_Interpolation.ld_tab_hsize+1)*(blu_ld_global_ctrl2.ld_blk_hnum+1)/dispWidth;
			blu_ld_inter_vctrl1.ld_vfactor = 1048576*(Local_Dimming_Table[0].LD_Backlight_Profile_Interpolation.ld_tab_vsize)/dispHeight;
			blu_ld_inter_vctrl2.ld_vinitphase1 = 0;

			for (i=0;i<LD_Table_NUM;i++){
				Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = blu_ld_global_ctrl2.ld_blk_vnum; /*update ld_blk_vnum info in pq misc*/
				Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hsize = blu_ld_global_ctrl3.ld_blk_hsize; /*update ld_blk_hsize info in pq misc*/
				Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vsize = blu_ld_global_ctrl3.ld_blk_vsize; /*update ld_blk_vsize info in pq misc*/
				Local_Dimming_Table[i].LD_Backlight_Final_Decision.ld_blu_spitotal = blu_ld_backlight_ctrl1.ld_blu_spitotal; /*update ld_blu_spitotal info in pq misc*/
				Local_Dimming_Table[i].LD_Backlight_Profile_Interpolation.ld_hfactor = blu_ld_inter_hctrl1.ld_hfactor; /*update ld_hfactor info in pq misc*/
				Local_Dimming_Table[i].LD_Backlight_Profile_Interpolation.ld_vfactor = blu_ld_inter_vctrl1.ld_vfactor; /*update ld_vfactor info in pq misc*/
				Local_Dimming_Table[i].LD_Backlight_Profile_Interpolation.ld_vinitphase1 = blu_ld_inter_vctrl2.ld_vinitphase1;  /*update ld_vinitphase1 info in pq misc*/
			}
		...
		} else if (panelInfo.hal_bl_type == HAL_BL_DIRECT_L || panelInfo.hal_bl_type == HAL_BL_DIRECT_VI || panelInfo.hal_bl_type == HAL_BL_DIRECT_SKY) {
			...
			for (i=0;i<LD_Table_NUM;i++)
				Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_type = 0; /*update ld_blk_type info in pq misc*/
			...
			if (panelInfo.hal_bar_type == HAL_LED_DIRECT_96_6x16) { /*LD_Global_Ctrl_Table[5];*/
				...
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 15; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 5; /*update ld_blk_vnum info in pq misc*/
				...
				blu_ld_global_ctrl2.ld_blk_vnum = 5;
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 19; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 5; /*update ld_blk_vnum info in pq misc*/
				...
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 9; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 9; /*update ld_blk_vnum info in pq misc*/
				...
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 9; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 11; /*update ld_blk_vnum info in pq misc*/
				...
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 19; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 7; /*update ld_blk_vnum info in pq misc*/
				...
				pass = 0;
			#else
			if (panelInfo.hal_bar_type == HAL_LED_BAR_32) {
				fwif_color_Set_BLPF_type(BLPF_DIRECT_32);
				blu_ld_global_ctrl2.ld_blk_hnum = 3;
				blu_ld_global_ctrl2.ld_blk_vnum = 7;
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 3; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 7; /*update ld_blk_vnum info in pq misc*/

				}
			} else if (panelInfo.hal_bar_type == HAL_LED_BAR_50) {
				fwif_color_Set_BLPF_type(BLPF_DIRECT_50);
				blu_ld_global_ctrl2.ld_blk_hnum = 4;
				blu_ld_global_ctrl2.ld_blk_vnum = 9;
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 4; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 9; /*update ld_blk_vnum info in pq misc*/

				}
			} else if (panelInfo.hal_bar_type == HAL_LED_BAR_90) {
				fwif_color_Set_BLPF_type(BLPF_DIRECT_90);
				blu_ld_global_ctrl2.ld_blk_hnum = 5;
				blu_ld_global_ctrl2.ld_blk_vnum = 14;
				for (i=0;i<LD_Table_NUM;i++) {
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_hnum = 5; /*update ld_blk_hnum info in pq misc*/
					Local_Dimming_Table[i].LD_Global_Ctrl.ld_blk_vnum = 14; /*update ld_blk_vnum info in pq misc*/

				}
			} else { /*default :  4x8 zones*/
				fwif_color_Set_BLPF_type(BLPF_DIRECT_32);
				blu_ld_global_ctrl2.ld_blk_hnum = 3;
				blu_ld_global_ctrl2.ld_blk_vnum = 7;
				for (i=0;i<LD_Table_NUM;i++) {
			...
			blu_ld_global_ctrl3.ld_blk_hsize = dispWidth/(blu_ld_global_ctrl2.ld_blk_hnum + 1);/*LD_Global_Ctrl_Table[13];*/
			blu_ld_global_ctrl3.ld_blk_vsize = dispHeight/(blu_ld_global_ctrl2.ld_blk_vnum + 1);/*LD_Global_Ctrl_Table[14];*/
			blu_ld_backlight_ctrl1.ld_blu_spitotal = (blu_ld_global_ctrl2.ld_blk_hnum + 1) * (blu_ld_global_ctrl2.ld_blk_vnum + 1) - 1;
			blu_ld_inter_hctrl1.ld_hfactor = 1048576*(Local_Dimming_Table[0].LD_Backlight_Profile_Interpolation.ld_tab_hsize+1)*(blu_ld_global_ctrl2.ld_blk_hnum+1)/dispWidth;
			blu_ld_inter_vctrl1.ld_vfactor = 1048576*(Local_Dimming_Table[0].LD_Backlight_Profile_Interpolation.ld_tab_vsize+1)*(blu_ld_global_ctrl2.ld_blk_vnum+1)/dispHeight;
			blu_ld_inter_vctrl2.ld_vinitphase1 = 1;
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_set_LED_Initialize , hal_bl_type=%d,hal_bar_type=%d\n", panelInfo.hal_bl_type, panelInfo.hal_bar_type);
		}
	}
...
void fwif_color_set_LD_CtrlSPI_init(HAL_LED_PANEL_INFO_T panelInfo)
{
	...
	if (panelInfo.hal_icType == 1) { /*internal IC LD*/
		...
		if (panelInfo.hal_bl_type == HAL_BL_EDGE_LED) {
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_set_LD_CtrlSPI_init , panelInfo.hal_bl_type=%d,\n", panelInfo.hal_bl_type);
		}
...
#if !defined(CONFIG_FIRMWARE_IN_KERNEL)		//common not defined	rock 20150911 sync TvManager
void fwif_color_HDR10_init(unsigned char table, unsigned int H_Size, unsigned int V_Size, unsigned char EOTF_MODE)
{
	DRV_HDR10_top_init_CTL tDRV_HDR10_top_init_CTL;
	DRV_HDR10_submodule_CTL tDRV_HDR10_submodule_CTL;

	tDRV_HDR10_top_init_CTL.dolby_hsize = H_Size;
	tDRV_HDR10_top_init_CTL.dolby_vsize = V_Size;
	tDRV_HDR10_top_init_CTL.dolby_h_total = (IoReg_Read32(0xb8005070)&0x00001FFF)+1;
	tDRV_HDR10_top_init_CTL.dolby_h_den_sta = (IoReg_Read32(0xb800508C)&0x00000FFF);//+3;
	tDRV_HDR10_top_init_CTL.dolby_v_den_sta = (IoReg_Read32(0xb8005090)&0x00000FFF);
	tDRV_HDR10_top_init_CTL.dolby_mode = 1;
	tDRV_HDR10_top_init_CTL.hdmi_in_mux = 1;

	VIPprintf("dolby_hsize=%d\n",tDRV_HDR10_top_init_CTL.dolby_hsize);
	VIPprintf("dolby_vsize=%d\n",tDRV_HDR10_top_init_CTL.dolby_vsize);
	VIPprintf("dolby_h_total=%d\n",tDRV_HDR10_top_init_CTL.dolby_h_total);
	VIPprintf("dolby_h_den_sta=%d\n",tDRV_HDR10_top_init_CTL.dolby_h_den_sta);
	VIPprintf("dolby_v_den_sta=%d\n",tDRV_HDR10_top_init_CTL.dolby_v_den_sta);
	VIPprintf("dolby_mode=%d\n",tDRV_HDR10_top_init_CTL.dolby_mode);
	VIPprintf("hdmi_in_mux=%d\n",tDRV_HDR10_top_init_CTL.hdmi_in_mux);

	memcpy(&tDRV_HDR10_submodule_CTL, &t_HDR10_submodule_CTL[table], sizeof(DRV_HDR10_submodule_CTL));

	drvif_HDR10_init(&tDRV_HDR10_submodule_CTL, &tDRV_HDR10_top_init_CTL);

	if((table != 255) || (tDRV_HDR10_submodule_CTL.HDR10_EN != 0))
	{
		drvif_HDR_EOTF_OETF(EOTF_MODE);
		drvif_HDR_YUV2RGB(1, hdr_YUV2RGB[(hdr_table[table][HDR_YUV2RGB_t_sel])]);
		fwif_HDR_RGB2OPT(1, hdr_RGB2OPT[(hdr_table[table][HDR_RGB2OPT_t_sel])]);
		fwif_color_set_DM_HDR_3dLUT(1, HDRlut3D);
	}
...
char fwif_color_set_DM2_OETF_Set_Prog_Index(unsigned char tbl_sel)
{
	...
	if (tbl_sel >= OETF_prog_idx_tbl_num) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_set_DM2_OETF_Set_Prog_Index error, tbl_sel=%d,\n", tbl_sel);
		tbl_sel = 0;
	}
...
void fwif_color_VIP_get_VO_Info(unsigned char input_src_type)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "get VO info, input_src_type=%d,video_full_range_flag=%d,transfer_characteristics=%d,colour_primaries=%d,matrix_coef=%d,EOTF_Mode=%d,bt2020_Enable=%d,BT2020.Mode=%d\n",
...
void fwif_color_VIP_get_DRM_Info(unsigned char input_src_type)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "get DRM info, input_src_type=%d,drm.len=%d,drm.eEOTFtype=%d,EOTF_Mode=%d,bt2020_Enable=%d,BT2020.Mode=%d\n",
...
unsigned char  fwif_color_set_force_I_De_XC_Disable(unsigned char value)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "Force_DEXC, value=%d, mem_ready=%d, ret=%d,\n", value, mem_ready, ret);
...
extern CDS_skin_DCC_Ctrl_table CDS_SKIN_DCC_TBL[10];
...
unsigned char  fwif_color_Set_AI_Ctrl(unsigned char value_icm, unsigned char value_dcc, unsigned char value_cds, unsigned char value_ctrl)
{
	...
	if (value_ctrl > 10)
        ...
        drvif_color_AI_DCC_table_set(&AI_DCC_TBL[value_dcc],&CDS_SKIN_DCC_TBL[value_cds]); //set AI dcc

        scaler_AI_Ctrl_Set(&AI_Ctrl_TBL[value_ctrl]); //set dynamic vale;
        return 0;
...
void fwif_color_ScalerVIP_set_TC_v130_metadata2reg_frameSync(int display_OETF, int display_Brightness, int proc_mode, int width, int height, int yuv_range, _RPC_system_setting_info *RPC_sys_info)
{
	...
	if((printf_Flag&_BIT0)!=0) {
		if(PRINT_CNT%print_delay==0) {
			rtd_printk(KERN_INFO, TAG_NAME, "juwen,fwif_color_ScalerVIP_set_TC_v130_metadata2reg_frameSync\n\n");
			printk("\n");
...
//metadata init for v140 if do not get metadata
		          ...
		          sl_hdr_metadata.sdrDisplayColourSpace = sl_hdr_metadata.sdrDisplayColourSpace;
...
char fwif_color_ScalerVIP_TC_metadata_frameSync(void)
{
	...
	if (RPC_sys_info == NULL || pTC_HDR_CTRL == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "TC frame sync, point = NULL, syste, info = %p, TC_HDR_CTRL = %p\n", RPC_sys_info, pTC_HDR_CTRL);
	...
	if((printf_Flag&_BIT1)!=0) {
		...
		if (print_delay == 0)
			print_delay = 100;
		if(PRINT_CNT%print_delay==0) {
			rtd_printk(KERN_INFO, TAG_NAME, "TC frame sync, 90k clock diff = %d, VGIP lint cnt diff = %d\n", clk_1-clk_0, line_cnt_1-line_cnt_0);
		}
	}
	PRINT_CNT++;
...
void fwif_color_TC_Flow(unsigned char TC_En)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "fwif_color_TC_Flow, TC_En=%d, I3DDMA_Color_format=%d, DisplayMode_Src=%d,\n", TC_En, I3DDMA_Color_format, DisplayMode_Src);
...
char fwif_color_set_TC_FrameSync_Enable(unsigned char Enable_flag)
{
	...
	if (RPC_sys_info == NULL || pVOInfo == NULL || pTC_HDR_CTRL == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "TC frame sync, point = NULL, syste, info = %p, pVOInfo = %p, pTC_HDR_CTRL= %p\n", RPC_sys_info, pVOInfo, pTC_HDR_CTRL);
		return -1;
	}
...
char fwif_color_Reg_TC_DMA_addr(void)
{
	...
	if (TC_HDR_CTRL == NULL || addr_C_LUT == 0 || addr_I_LUT == 0 || addr_S_LUT == 0 || addr_P_LUT == 0 || addr_D_LUT == 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "get TC DMA error, TC_HDR_CTRL= %p, addr_C_LUT = %x, addr_I_LUT = %x, addr_S_LUT = %x, addr_P_LUT = %x, addr_D_LUT = %x\n",
	...
	rtd_printk(KERN_INFO, TAG_NAME, "TC ini,Clut[%x,%p,%d,%d],Ilut[%x,%p,%d,%d],Slut[%x,%p,%d,%d],Plut[%x,%p,%d,%d],Dlut[%x,%p,%d,%d]\n",
	...
	if (C_LUT_remap == 1) {
		...
		if ((TC_HDR_CTRL->C_LUT_DMA.phy_addr_align == 0) || (TC_HDR_CTRL->C_LUT_DMA.pVir_addr_align == NULL)) {
			rtd_printk(KERN_EMERG, TAG_NAME, "TC, C_LUT 96 aligned fail, addr=%x, pVir_addr=%p,\n",
	...
	if (I_LUT_remap == 1) {
		...
		if ((TC_HDR_CTRL->I_LUT_DMA.phy_addr_align == 0) || (TC_HDR_CTRL->I_LUT_DMA.pVir_addr_align == NULL)) {
			rtd_printk(KERN_EMERG, TAG_NAME, "TC, I_LUT 96 aligned fail, addr=%x, pVir_addr=%p,\n",
	...
	if (S_LUT_remap == 1) {
		...
		if ((TC_HDR_CTRL->S_LUT_DMA.phy_addr_align == 0) || (TC_HDR_CTRL->S_LUT_DMA.pVir_addr_align == NULL)) {
			rtd_printk(KERN_EMERG, TAG_NAME, "TC, S_LUT 96 aligned fail, addr=%x, pVir_addr=%p,\n",
	...
	if (P_LUT_remap == 1) {
		...
		TC_HDR_CTRL->P_LUT_DMA.pVir_addr_align = (unsigned int*)dvr_memory_alignment((unsigned long)TC_HDR_CTRL->P_LUT_DMA.pVir_addr_align, TC_HDR_CTRL->P_LUT_DMA.size);
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "TC, P_LUT 96 aligned fail, addr=%x, pVir_addr=%p,\n",
	...
	if (D_LUT_remap == 1) {
		...
		TC_HDR_CTRL->D_LUT_DMA.pVir_addr_align = (unsigned int*)dvr_memory_alignment((unsigned long)TC_HDR_CTRL->D_LUT_DMA.pVir_addr_align, TC_HDR_CTRL->D_LUT_DMA.size);
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "TC, D_LUT 96 aligned fail, addr=%x, pVir_addr=%p,\n",
				TC_HDR_CTRL->D_LUT_DMA.phy_addr_align, TC_HDR_CTRL->D_LUT_DMA.pVir_addr_align);
			BUG();
		}
	}

	rtd_printk(KERN_INFO, TAG_NAME, "TC ini, 96B aligned,Clut[%x,%p,%d,%d],Ilut[%x,%p,%d,%d],Slut[%x,%p,%d,%d],Plut[%x,%p,%d,%d],Dlut[%x,%p,%d,%d]\n",
		TC_HDR_CTRL->C_LUT_DMA.phy_addr_align, TC_HDR_CTRL->C_LUT_DMA.pVir_addr_align, TC_HDR_CTRL->C_LUT_DMA.size, C_LUT_remap,
		TC_HDR_CTRL->I_LUT_DMA.phy_addr_align, TC_HDR_CTRL->I_LUT_DMA.pVir_addr_align, TC_HDR_CTRL->I_LUT_DMA.size, I_LUT_remap,
		TC_HDR_CTRL->S_LUT_DMA.phy_addr_align, TC_HDR_CTRL->S_LUT_DMA.pVir_addr_align, TC_HDR_CTRL->S_LUT_DMA.size, S_LUT_remap,
		TC_HDR_CTRL->P_LUT_DMA.phy_addr_align, TC_HDR_CTRL->P_LUT_DMA.pVir_addr_align, TC_HDR_CTRL->P_LUT_DMA.size, P_LUT_remap,
...
/*========================== HDR10 +==========================================*/
#if 1
char fwif_color_set_HDR10p_FrameSync_Enable(unsigned char Enable)
{
	VIP_HDR10p_CTRL* HDR10p_CTRL = NULL;
	extern VIP_HDR10p_TBL HDR10p_TBL;
	HDR10p_CTRL = drvif_fwif_color_Get_HDR10p_Ctrl();

	if (HDR10p_CTRL == NULL ) {
		rtd_printk(KERN_EMERG, TAG_NAME, "get HDR10p CTRL error, HDR10p_CTRL=%p\n",
			HDR10p_CTRL);
		return -1;
	}

	HDR10p_CTRL->Enable = Enable;

	if (Enable == 1) {
		drvif_fwif_color_Set_HDR10p_demo_flag(HDR10p_Demo_OFF, HDR10p_flow_layer);
		memcpy((unsigned char*)drvif_fwif_color_Get_HDR10p_TBL(),  (unsigned char*)&HDR10p_TBL, sizeof(VIP_HDR10p_TBL));
	}
	return 0;
}

int fwif_color_ScalerVIP_HDR10p_demo_flow(void)
{
	...
	extern unsigned short OETF_HDR10p_demo_hdr10[OETF_size];
	extern unsigned int EOTF_HDR10p_demo_hdr10[EOTF_size];
	...
	int hdr10p_demo_mode[VIP_HDR10p_layer_MAX] = {HDR10p_Demo_OFF};
	...
	VIP_system_info_structure_table = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	gVIP_Table = fwif_colo_get_AP_vip_table_gVIP_Table();
	hdr10p_demo_mode[HDR10p_flow_layer] = drvif_fwif_color_Get_HDR10p_demo_flag(HDR10p_flow_layer);
	hdr10p_demo_mode[HDR10p_engMenu_layer] = drvif_fwif_color_Get_HDR10p_demo_flag(HDR10p_engMenu_layer);

	CNT++;

	if (HDR10p_CTRL == NULL || RPC_sys_info == NULL || gVIP_Table == NULL || VIP_system_info_structure_table == NULL) {
		if ((CNT % 100) == 0)
			rtd_printk(KERN_EMERG, TAG_NAME, "HDR10p demo flow, HDR10p_CTRL=%p, RPC_sys_info=%p, gVIP_Table=%p,VIP_system_info_structure_table=%p \n",
				HDR10p_CTRL, RPC_sys_info, gVIP_Table, VIP_system_info_structure_table);
		return -1;
	}

	Debug_Log = HDR10p_CTRL->Debug_Log;
	Debug_log_delay = HDR10p_CTRL->Debug_Delay;

	if (hdr10p_demo_mode[HDR10p_engMenu_layer] != hdr10p_demo_mode[HDR10p_flow_layer]) {
		if (hdr10p_demo_mode[HDR10p_engMenu_layer] == HDR10p_Demo_OFF) {
			...
			Scaler_Set_HDR_YUV2RGB(HAL_VPQ_HDR_MODE_HDR10p, HAL_VPQ_COLORIMETRY_BT2020);
		...
		} else if (hdr10p_demo_mode[HDR10p_engMenu_layer] == HDR10p_Demo_ON_HDR10) {
			//pVOInfo = Scaler_VOInfoPointer(Scaler_Get_CurVoInfo_plane());
			drvif_DM2_EOTF_Set_By_DMA(EOTF_HDR10p_demo_hdr10, EOTF_HDR10p_demo_hdr10, EOTF_HDR10p_demo_hdr10,
				HDR10p_CTRL->EOTF_DMA.phy_addr_align, HDR10p_CTRL->EOTF_DMA.pVir_addr_align);
			drvif_DM2_OETF_Set_By_DMA(OETF_HDR10p_demo_hdr10, OETF_HDR10p_demo_hdr10, OETF_HDR10p_demo_hdr10,
				HDR10p_CTRL->OETF_DMA.phy_addr_align, HDR10p_CTRL->OETF_DMA.pVir_addr_align);
			drvif_color_Set_HDR10plus_3Dlut_CInv(1024, 0);
			...
			drvif_fwif_color_Set_HDR10p_demo_flag(HDR10p_Demo_ON_HDR10, HDR10p_flow_layer);
	...
	if((Debug_Log&_BIT2)!=0) {
		if (CNT >= Debug_log_delay) {
			CNT = 0;
			rtd_printk(KERN_INFO, TAG_NAME, "HDR10+ demo mode, flow =%d, eng=%d \n", hdr10p_demo_mode[HDR10p_flow_layer], hdr10p_demo_mode[HDR10p_engMenu_layer]);
		}
	}

	return hdr10p_demo_mode[HDR10p_engMenu_layer];

}

char fwif_color_ScalerVIP_HDR10p_frameSync(void)
{
	extern VIP_HDR10p_TBL HDR10p_TBL;
	VIP_HDR10p_CTRL* HDR10p_CTRL = NULL;
	static unsigned char CNT = 0;
	...
	if (HDR10p_CTRL == NULL || RPC_sys_info == NULL) {
		if ((CNT % 100) == 0)
			rtd_printk(KERN_EMERG, TAG_NAME, "HDR10p_frameSync error, HDR10p_CTRL=%p, RPC_sys_info=%p \n", HDR10p_CTRL, RPC_sys_info);
		return -1;
	}

	if (HDR10p_CTRL->Enable != 1)	/* is not HDR10p case */
		return 1;

	Debug_Log = HDR10p_CTRL->Debug_Log;
	Debug_log_delay = HDR10p_CTRL->Debug_Delay;

	if (fwif_color_ScalerVIP_HDR10p_demo_flow() == HDR10p_Demo_OFF){	/* default setting, demo mode is enabled by engineer menu or tool*/
		...
		drvif_fwif_color_ScalerVIP_set_HDR10p_metadata2reg_frameSync(0, RPC_sys_info, HDR10p_CTRL);

		if((Debug_Log&_BIT2)!=0) {
			clk_end = drvif_Get_90k_Lo_clk();
			drvif_Get_VGIP_Active_Info(&v_sta, &v_len, &line_end);

			HDR10p_CTRL->Debug_VGIP_Line_sta = line_start;
			HDR10p_CTRL->Debug_VGIP_Line_end = line_end;
			HDR10p_CTRL->Debug_VGIP_Line_diff = line_end-line_start;
			HDR10p_CTRL->Debug_90clk_sta = clk_start;
			HDR10p_CTRL->Debug_90clk_end = clk_end;
			HDR10p_CTRL->Debug_90clk_diff = clk_end-clk_start;

			if (CNT >= Debug_log_delay) {
				CNT = 0;
				rtd_printk(KERN_INFO, TAG_NAME, "HDR10+ frame sync, 90k clock diff = %d, VGIP lint cnt diff = %d,v_sta=%d, v_len=%d, \n",
...
char fwif_color_HDR10p_Ctrl_Structure_ini(void)
{
	VIP_HDR10p_CTRL* HDR10p_CTRL = NULL;
	...
	if (HDR10p_CTRL == NULL || eotf_phy_addr == 0 || oetf_phy_addr == 0 || HDR_3D_LUT_phy_addr == 0 || RPC_sys_info == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "get HDR10p DMA error, HDR10p_CTRL=%p, eotf_phy_addr=%x, oetf_phy_addr=%x, HDR_3D_LUT_phy_addr=%x,RPC_sys_info=%p\n",
			HDR10p_CTRL, eotf_phy_addr, oetf_phy_addr, HDR_3D_LUT_phy_addr,RPC_sys_info);
		return -1;
	}

	/* ctrl ini*/
	HDR10p_CTRL->Enable = 0;
	HDR10p_CTRL->Debug_Log = 0;
	HDR10p_CTRL->Debug_Delay = 50;

	/* EOTF DMA */
	if (HDR10p_CTRL->EOTF_DMA.pVir_addr_align == NULL) {
		...
		HDR10p_CTRL->EOTF_DMA.phy_addr_align = eotf_phy_addr;
		HDR10p_CTRL->EOTF_DMA.size = size;
		pVir_addr = dvr_remap_uncached_memory(eotf_phy_addr, size, __builtin_return_address(0));
		HDR10p_CTRL->EOTF_DMA.pVir_addr_align = pVir_addr;
		EOTF_remap = 1;
	}

	/* OETF DMA */
	if (HDR10p_CTRL->OETF_DMA.pVir_addr_align == NULL) {
		...
		HDR10p_CTRL->OETF_DMA.phy_addr_align = oetf_phy_addr;
		HDR10p_CTRL->OETF_DMA.size = size;
		pVir_addr = dvr_remap_uncached_memory(oetf_phy_addr, size, __builtin_return_address(0));
		HDR10p_CTRL->OETF_DMA.pVir_addr_align = pVir_addr;
		OETF_remap = 1;
	}

	rtd_printk(KERN_INFO, TAG_NAME, "HDR10p_ini, EOTF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d, OETF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d,\n",
		HDR10p_CTRL->EOTF_DMA.phy_addr_align, HDR10p_CTRL->EOTF_DMA.pVir_addr_align, HDR10p_CTRL->EOTF_DMA.size, EOTF_remap,
		HDR10p_CTRL->OETF_DMA.phy_addr_align, HDR10p_CTRL->OETF_DMA.pVir_addr_align, HDR10p_CTRL->OETF_DMA.size, OETF_remap);

#ifdef CONFIG_BW_96B_ALIGNED
	/* EOTF DMA */
	if (EOTF_remap == 1) {
		HDR10p_CTRL->EOTF_DMA.size = dvr_size_alignment(HDR10p_CTRL->EOTF_DMA.size);
		HDR10p_CTRL->EOTF_DMA.phy_addr_align = (unsigned int)dvr_memory_alignment((unsigned long)HDR10p_CTRL->EOTF_DMA.phy_addr_align, HDR10p_CTRL->EOTF_DMA.size);
		HDR10p_CTRL->EOTF_DMA.pVir_addr_align = (unsigned int*)dvr_memory_alignment((unsigned long)HDR10p_CTRL->EOTF_DMA.pVir_addr_align, HDR10p_CTRL->EOTF_DMA.size);
		if ((HDR10p_CTRL->EOTF_DMA.phy_addr_align == 0) || (HDR10p_CTRL->EOTF_DMA.pVir_addr_align == NULL)) {
			rtd_printk(KERN_EMERG, TAG_NAME, "HDR10+, EOTF 96 aligned fail, addr=%x, pVir_addr=%p,\n",
				HDR10p_CTRL->EOTF_DMA.phy_addr_align, HDR10p_CTRL->EOTF_DMA.pVir_addr_align);
			BUG();
		}
	}
	...
	/* OETF DMA */
	if (OETF_remap == 1) {
		HDR10p_CTRL->OETF_DMA.size = dvr_size_alignment(HDR10p_CTRL->OETF_DMA.size);
		HDR10p_CTRL->OETF_DMA.phy_addr_align = (unsigned int)dvr_memory_alignment((unsigned long)HDR10p_CTRL->OETF_DMA.phy_addr_align, HDR10p_CTRL->OETF_DMA.size);
		HDR10p_CTRL->OETF_DMA.pVir_addr_align = (unsigned int*)dvr_memory_alignment((unsigned long)HDR10p_CTRL->OETF_DMA.pVir_addr_align, HDR10p_CTRL->OETF_DMA.size);
		if ((HDR10p_CTRL->OETF_DMA.phy_addr_align == 0) || (HDR10p_CTRL->OETF_DMA.pVir_addr_align == NULL)) {
			rtd_printk(KERN_EMERG, TAG_NAME, "HDR10+, OETF 96 aligned fail, addr=%x, pVir_addr=%p,\n",
				HDR10p_CTRL->OETF_DMA.phy_addr_align, HDR10p_CTRL->OETF_DMA.pVir_addr_align);
			BUG();
		}
	}

	rtd_printk(KERN_INFO, TAG_NAME, "HDR10p_ini, 96B aligned, EOTF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d, OETF:addr=%x, pVir_addr=%p, size_aliged=%d, remap=%d,\n",
		HDR10p_CTRL->EOTF_DMA.phy_addr_align, HDR10p_CTRL->EOTF_DMA.pVir_addr_align, HDR10p_CTRL->EOTF_DMA.size, EOTF_remap,
		HDR10p_CTRL->OETF_DMA.phy_addr_align, HDR10p_CTRL->OETF_DMA.pVir_addr_align, HDR10p_CTRL->OETF_DMA.size, OETF_remap);
#endif
...
#if 0
	/* 3D LUT DMA */
	if (HDR10p_CTRL->HDR_3D_LUT_DMA.pVir_addr_align == NULL) {
		...
		HDR10p_CTRL->HDR_3D_LUT_DMA.phy_addr_align = HDR_3D_LUT_phy_addr;
		HDR10p_CTRL->HDR_3D_LUT_DMA.size = size;
		pVir_addr = dvr_remap_uncached_memory(HDR_3D_LUT_phy_addr, size, __builtin_return_address(0));
		HDR10p_CTRL->HDR_3D_LUT_DMA.pVir_addr_align = pVir_addr;
	}
#endif
	return 0;
}
...
char fwif_color_rtice_DM2_ToneMapping_Set(short *TM_table12_R, short *TM_table12_G, short *TM_table12_B)
{
	...
	if (drvif_DM2_ToneMapping_Set(TM_table12_R, TM_table12_G, TM_table12_B) < 0)
		return -1;
#else
	fwif_color_DM2_ToneMapping_Set_By_DMA(TM_table12_R, TM_table12_G, TM_table12_B);
#endif
	return 0;
}
...
char fwif_color_rtice_DM2_3D_17x17x17_LUT_Set(unsigned char enable, unsigned short *pArray)
{
	...
	ret = fwif_color_set_DM_HDR_3dLUT_17x17x17_16_TV006(&args);
...
char fwif_color_DM2_EOTF_Set_By_DMA(unsigned int *EOTF_table32_R, unsigned int *EOTF_table32_G, unsigned int *EOTF_table32_B)
{
	...
	if (ret < 0) {
		...
		ret = drvif_Wait_VO_start(delay1, v_len, 0x032500);
		if (ret < 0) {
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM2_EOTF_Set_By_DMA fail to wait for active, ret = %d, v_end=%d, v_sta=%d, delay1=%d, delay2=%d,\n",
				ret, v_end, v_sta, delay1, delay2);
			//return -1;
		}
	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "DM2_EOTF, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
		addr, pVir_addr, addr_aligned, pVir_addr_aligned, size, size_aligned);
...
char fwif_color_DM2_OETF_Set_By_DMA(unsigned short *OETF_table16_R, unsigned short *OETF_table16_G, unsigned short *OETF_table16_B)
{
	...
	if (ret < 0) {
		...
		ret = drvif_Wait_VO_start(delay1, v_len, 0x032500);
		if (ret < 0) {
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM2_OETF_Set_By_DMA fail to wait for active, ret = %d, v_end=%d, v_sta=%d, delay1=%d, delay2=%d,\n",
				ret, v_end, v_sta, delay1, delay2);
			return -1;
		}
	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "DM2_OETF, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
		addr, pVir_addr, addr_aligned, pVir_addr_aligned, size, size_aligned);
...
char fwif_color_DM2_ToneMapping_Set_By_DMA(short *TM_table12_R, short *TM_table12_G, short *TM_table12_B)
{
	...
	addr = drvif_memory_get_data_align(addr, (1 << 12));
	size = drvif_memory_get_data_align(VIP_DMAto3DTable_HDR_TM_SIZE, (1 << 12));

	if (addr == 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM2_ToneMapping_Set_By_DMA, addr = NULL\n");
		return -1;
	}

	ret = drvif_Get_VO_Active_Info(&v_sta, &v_end, &line_cnt);
	if (ret < 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM2_ToneMapping_Set_By_DMA is not ready, ret = %d\n", ret);
		//return -1;
	} else {
		/* wait for active area to write DMA*/
		v_len = ((v_end - v_sta)<0)?(0):((v_end - v_sta));
		...
		ret = drvif_Wait_VO_start(delay1, v_len, 0x032500);
		if (ret < 0) {
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM2_ToneMapping_Set_By_DMA fail to wait for active, ret = %d, v_end=%d, v_sta=%d, delay1=%d, delay2=%d,\n",
				ret, v_end, v_sta, delay1, delay2);
			//return -1;
		}
	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "DM2_TM, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
		addr, pVir_addr, addr_aligned, pVir_addr_aligned, size, size_aligned);
...
char fwif_color_DM2_3D_24x24x24_LUT_Set_By_DMA(unsigned char enable, unsigned short *pArray)
{
	...
	if (ret < 0) {
		...
		ret = drvif_Wait_VO_start(delay1, v_len, 0x032500);
		if (ret < 0) {
			rtd_printk(KERN_EMERG, TAG_NAME, "fwif_color_DM2_3D_24x24x24_LUT_Set_By_DMA fail to wait for active, ret = %d, v_end=%d, v_sta=%d, delay1=%d, delay2=%d,\n",
				ret, v_end, v_sta, delay1, delay2);
			//return -1;
		}
	}
	...
	if (print_cnt >= HDR_DMA_Status_MAX) {
		rtd_printk(KERN_INFO, TAG_NAME, "DM2_3D LUT, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
			addr, pVir_addr, addr_aligned, pVir_addr_aligned, size, size_aligned);
		print_cnt = 0;
	}
	print_cnt++;
...
void fwif_color_DI_IP_ini(void)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "DI_Bnd_setting, sta=%x, end=%x\n", addr_sta, addr_end);
...
void fwif_color_DI_IEGSM_ini(void)
{
	...
	addr_sta = get_query_start_address(QUERY_IDX_DI);
	addr_end = addr_sta + VIP_DI_1_BUFFER_SIZE;
	drvif_color_iEdgeSmooth_Addr_Boundary_ini(addr_sta, addr_end);
}
...
unsigned int get_query_VIP_DMAto3DTABLE_start_address(unsigned char idx)
{
	...
	if (base_addr == 0) {
		...
		else if (idx == DMAto3DTABLE_TC_C_LUT)
			addr = base_addr + VIP_DMAto3DTable_HDR_3D_LUT_SIZE + VIP_DMAto3DTable_HDR_EOTF_SIZE + VIP_DMAto3DTable_HDR_OETF_SIZE + VIP_DMAto3DTable_D_3D_LUT_SIZE +
		...
		else if (idx == DMAto3DTABLE_HDR_TM)
			addr = base_addr + VIP_DMAto3DTable_HDR_3D_LUT_SIZE + VIP_DMAto3DTable_HDR_EOTF_SIZE + VIP_DMAto3DTable_HDR_OETF_SIZE + VIP_DMAto3DTable_D_3D_LUT_SIZE +
				VIP_DMAto3DTable_ICM_SIZE + VIP_DMAto3DTable_TC_C_LUT_SIZE + VIP_DMAto3DTable_TC_I_LUT_SIZE + VIP_DMAto3DTable_TC_S_LUT_SIZE + VIP_DMAto3DTable_TC_P_LUT_SIZE + VIP_DMAto3DTable_TC_D_LUT_SIZE;
		else
...
char fwif_color_DeMura_init(unsigned char bLogEn)
{
	...
	if (base_addr == 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "[%s] addr = NULL\n", __func__);
		return -1;
	}
	...
	if(bLogEn!=0)
	{
		rtd_printk(KERN_INFO, TAG_NAME, "Demura, addr=%x, pVir_addr=%p, addr_aliged=%x, pVir_addr_aliged=%p, size=%d, size_aliged=%d,\n",
...
static unsigned int fwif_color_INNX_Demura_crcArray_sumAll(unsigned char* data, unsigned int crcStart, unsigned int crcLength)
{
			...
			{
				rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura---lut checksum error---\n");
				rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura---lut checksum golden %x---\n", (unsigned int)(data[ INNX_Demura_addr_LUTCheckSum ]<<8) +  (unsigned int)(data[ INNX_Demura_addr_LUTCheckSum+1 ])   );
				rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura---lut checksum compute %x---\n", crc_out   );
				return 1;
			}
...
// 1.check whether header is valid
	...
	{
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura----------ASIC error ----------\n");
		return 2;
	}
...
//compute header checksum
	...
	if(compute_header_Sum !=  (unsigned int)(arr[ INNX_Demura_addr_HeaderCheckSum ]<<8) +  (unsigned int)(arr[ INNX_Demura_addr_HeaderCheckSum+1 ])   )
	{
		...
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura---header checksum error---\n");
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura---header checksum golden %x---\n", (unsigned int)(arr[ INNX_Demura_addr_HeaderCheckSum ]<<8) +  (unsigned int)(arr[ INNX_Demura_addr_HeaderCheckSum+1 ])  );
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura---header checksum compute %x---\n", compute_header_Sum );
		return 4;
	}
...
char fwif_color_INNX_Demura_TBL(unsigned char * pINNX_DemuraTBL, unsigned int TBL_Size, char *pDeLut)
{
	...
	if ((INNX_Demura_CodeLutALL_row == NULL) || (INNX_Demura_CodeLutAll == NULL) || (INNX_Demura_header_infor == NULL)
		...
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura, dynamic alloc fail, CodeLutALL_row=%p, CodeLutAll=%p, header_infor=%p, DataByRow=%p, pDeLut=%p\n",
	...
	if (pINNX_DemuraTBL == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura----------pINNX_DemuraTBL = NULL ----------\n");
		return -1;
	}

	rtd_printk(KERN_INFO, TAG_NAME, "INNX_Demura, set INNX Demura, INNX demura size =%d,\n", TBL_Size);
	...
	if (ret != 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura-------header check error, error code = %d ----------\n", ret);
		return ret;
	}
	...
	if (lut_crc_out != 0) {
		rtd_printk(KERN_EMERG, TAG_NAME, "INNX_Demura-------CRC check error, error code = %d ----------\n", lut_crc_out);
		return lut_crc_out;
	}
...
static int thread_set_od_bits(void *p)
{
	...
	down(&OD_Semaphore);
	drvif_color_set_od_bits(*pBits, 2); //10bit color depth
	up(&OD_Semaphore);
...
// 180209 henry 17x17 table setting process on Merlin3
	fwif_color_od_table_tv006_convert(od_table_tv006_store, od_table_store);
	fwif_color_od_table_tv006_transform(0, od_table_store, od_delta_table);
	drvif_color_od_table(od_delta_table, 0);// 0:delta mode 1:target mode(output mode)
	//fwif_color_od_table_17x17_transform(0, od_table_tv006_store, od_delta_table); rord mark, degrade issue
	//drvif_color_od_table_seperate(od_delta_table, 0, 3); // 3: write to rgb channel rord mark ,degrade issue
// end henry
	od_table_mode_store = 1;// 0:target mode 1:delta mode -1:inverse mode
	bODTableLoaded = TRUE;
...
unsigned char fwif_color_init_set_od_table(void)
{
	...
	extern unsigned int od_delta_table[OD_table_length];
	extern unsigned char od_table_tv006_store[OD_table_length];
  	unsigned char od_table_init[17][17] =
		{
		{  0,  23,  41,  57,  73,  89, 105, 126, 147, 160, 176, 192, 211, 224, 240, 255, 255, },
		{  0,  16,  33,  51,  71,  87, 103, 121, 142, 156, 172, 188, 208, 220, 236, 249, 255, },
		{  0,  14,  32,  49,  66,  83, 100, 119, 137, 153, 169, 185, 206, 217, 233, 247, 255, },
		{  0,  13,  30,  48,  65,  82,  99, 116, 135, 151, 167, 183, 201, 215, 231, 246, 255, },
		{  0,  12,  29,  46,  64,  81,  98, 115, 132, 148, 164, 180, 199, 212, 230, 245, 255, },
		{  0,  11,  28,  45,  62,  80,  97, 114, 131, 148, 164, 180, 196, 212, 229, 244, 255, },
		{  0,  10,  27,  44,  61,  78,  96, 113, 130, 147, 164, 180, 196, 212, 228, 244, 255, },
		{  0,   9,  27,  43,  60,  77,  94, 112, 129, 146, 163, 180, 196, 212, 228, 244, 255, },
		{  0,   8,  27,  43,  59,  76,  93, 110, 128, 145, 162, 179, 196, 212, 228, 244, 255, },
		{  0,   7,  26,  43,  59,  75,  92, 109, 126, 144, 161, 178, 195, 212, 228, 244, 255, },
		{  0,   6,  25,  42,  58,  75,  91, 108, 125, 142, 160, 177, 194, 211, 228, 244, 255, },
		{  0,   6,  24,  40,  56,  74,  90, 107, 124, 141, 158, 176, 193, 210, 227, 244, 255, },
		{  0,   5,  22,  38,  54,  72,  89, 106, 123, 140, 157, 174, 192, 209, 226, 243, 255, },
		{  0,   4,  20,  36,  52,  70,  88, 104, 120, 136, 156, 173, 190, 208, 225, 242, 255, },
		{  0,   3,  17,  33,  49,  65,  86, 102, 118, 134, 152, 168, 189, 206, 224, 241, 255, },
		{  0,   2,  15,  31,  47,  63,  81,  97, 113, 129, 150, 166, 182, 203, 221, 240, 255, },
		{  0,   2,  14,  30,  46,  61,  79,  95, 111, 127, 148, 164, 180, 193, 208, 224, 255, },
		};

	memcpy(od_table_tv006_store, od_table_init, sizeof(od_table_tv006_store));
...
void fwif_color_HDR_Sub_gamma_encode(unsigned int *final_R, unsigned int *final_G, unsigned int *final_B, UINT16 *Out_R, UINT16 *Out_G, UINT16 *Out_B)
{
	...
	for (i = 0; i < Bin_Num_HDR2_Gamma; i++) {
		if (i % 2 == 0) {
			d0 = (Out_R[i + 1] >=  Out_R[i]) ? (Out_R[i + 1] - Out_R[i]) : 0;
			d1 = (Out_R[i + 2] >=  Out_R[i + 1]) ? (Out_R[i + 2] - Out_R[i + 1]) : 0;
			d0 = (d0 >=  d_max) ? d_max : d0;
...
void fwif_color_read_memc_logo(unsigned char start_byte)
{
	...
	if (LD_virAddr != NULL) {
		...
		rtd_printk(KERN_INFO, TAG_NAME, "read_memc_logo fail\n");
	}
...
void Scaler_Get_Logo_Detect_VirAddr(void)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[%s] Get MEMC Logo PAddr = 0x%08x\n", __FUNCTION__, phyAddr);
	if (phyAddr == 0 || phyAddr == 0x00900000)
	{
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] Get PAddr error\n", __FUNCTION__);
		LD_virAddr = NULL;
		return;
	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[%s] Get MEMC Logo VAddr = 0x%08x\n", __FUNCTION__, (unsigned int)LD_virAddr);
...
void Scaler_Check_Logo_Detect_VirAddr(void)
{
	...
	{
		rtd_printk(KERN_INFO, TAG_NAME, "[%s][ERROR] phyAddr = 0x%08x, remapPhyAddr = 0x%08x, do unmap and remap\n",
