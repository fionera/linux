...
static RGB2YUV_BLACK_LEVEL rgb2yuv_bl_tv006 = RGB2YUV_BLACK_LEVEL_UNDEFINE;
...
unsigned char cm_data_debug = 0;	/* ICM DB debug */


// Merlin5
#define MplusRegSize 1152
#define SiwRegSize 668
...
void fwif_color_Set_MPEGNR_tv006(unsigned char level)
{
	...
	if (pMPEGNR_Table->MPEG_LEVEL < 4) {
		...
		threshold = pMPEGNR_Table->MNR_TABLE.MNR_T.Edge_Theshold;
		step = pMPEGNR_Table->MNR_TABLE.MNR_T.Edge_Theshold_Step;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd8 = pMPEGNR_Table->MNR_TABLE.MNR_T.Edge_Theshold;
                threshold = pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd8;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd7 = (threshold-step>0)?(threshold-step):0;
                threshold =pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd7;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd6 = (threshold-step>0)?(threshold-step):0;
                threshold =pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd6;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd5 = (threshold-step>0)?(threshold-step):0;
                threshold =pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd5;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd4 = (threshold-step>0)?(threshold-step):0;
                threshold =pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd4;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd3 = (threshold-step>0)?(threshold-step):0;
                threshold =pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd3;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd2 = (threshold-step>0)?(threshold-step):0;
                threshold =pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd2;

                pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd1 = (threshold-step>0)?(threshold-step):0;
                threshold =pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_mnr_edgethd1;
...
void fwif_init_pillar_by_TV006_DB(CHIP_COLOR_REGION_TYPE_T *crRgn_in, CM_HUE_SECTION *hue_section, CM_SAT_SECTION *sat_section, CM_ITN_SECTION *itn_section, COLORELEM_TAB_T *icmTab_in)
{
		...
		for (i = 0; i < CHIP_HUE_TBLPOINT; i++) {
			...
			if (crRgn_in->stColorRegion[r].hue_x[i] == 0)
			{
				if (r == CHIP_CM_COLOR_REGION_RED) /*Red region ==> first pillar*/
				{
					...
					hue_section->RGB_offset_curve[0][idx] = ((INT16)(crRgn_in->stColorRegion[r].r_offset_by_hue[i]));
					hue_section->RGB_offset_curve[1][idx] = ((INT16)(crRgn_in->stColorRegion[r].g_offset_by_hue[i]));
					hue_section->RGB_offset_curve[2][idx] = ((INT16)(crRgn_in->stColorRegion[r].b_offset_by_hue[i]));
				}
				/*other region, skip*/
				continue;
			}
	...
	memcpy(chip_sat_idx, &(crRgn_in->stColorRegion[0].sat_x), (CHIP_SAT_TBLPOINT + 1) * sizeof(unsigned short));
	memcpy(sat_section->rgn_idx, crRgn_in->stColorRegion[0].sat_x, (CHIP_SAT_TBLPOINT + 1) * sizeof(unsigned short));
	for (i = 0; i < CHIP_CM_COLOR_REGION_MAX; i++) memcpy(sat_section->rgn_gain[i], crRgn_in->stColorRegion[i].sat_g, (CHIP_SAT_TBLPOINT + 1) * sizeof(unsigned short));
	sat_section->pillar_num = CHIP_SAT_TBLPOINT + 1/* first pillar default 0*/;
	for (i = 0; i < sat_section->pillar_num; i++) sat_section->rgn_pillar[i] = i;

	/*init itn*/ /*set first pillar, 0 <== data protect*/
	memcpy(chip_itn_idx, crRgn_in->stColorRegion[0].Int_x, (CHIP_INT_TBLPOINT + 1) * sizeof(unsigned short));
	crRgn_in->stColorRegion[0].Int_x[0] = 0;	/*first pillar data protect*/
	memcpy(itn_section->rgn_idx, crRgn_in->stColorRegion[0].Int_x, (CHIP_INT_TBLPOINT + 1) * sizeof(unsigned short));
	for (i = 0; i < CHIP_CM_COLOR_REGION_MAX; i++) memcpy(itn_section->rgn_gain[i], crRgn_in->stColorRegion[i].Int_g, (CHIP_INT_TBLPOINT + 1) * sizeof(unsigned short));
	itn_section->pillar_num = CHIP_INT_TBLPOINT + 1/* first pillar default 0*/;
	...
	if (cm_data_debug) {
		...
		for (i = 0; i < CHIP_CM_HUE_SEG; i++)
		{
			...
			printk("%d, ", chip_hue_idx[i]);
		}
		for (i = 0; i < CHIP_CM_SAT_SEG; i++)
		{
			if (i == 0) printk("\n[ICM DB], pillar_sat[%d] = ", i);
			printk("%d, ", chip_sat_idx[i]);
		}
		for (i = 0; i < CHIP_CM_ITN_SEG; i++)
		{
			if (i == 0) printk("\n[ICM DB], pillar_itn[%d] = ", i);
			printk("%d, ", chip_itn_idx[i]);
		}
		printk("\n\n");
	}
...
{
	int i, j;
	UINT8 hue_pillar[CHIP_HUE_TBLPOINT] = {0};
	UINT16 hue_idx[CHIP_HUE_TBLPOINT] = {0};
	UINT8 bound_num = 0;
	UINT8 r_flag = 0;
...
unsigned char fwif_color_copy_ICM_RGB_offset_table_from_tv006_DB(CHIP_COLOR_REGION_TYPE_T *crRgn_in, CM_HUE_SECTION *hue_section, CHIP_COLOR_CONTROL_T *crCtrl_in)
{
	...
	for(i = 0; i <= CHIP_SAT_TBLPOINT; i++)
		tICM_RGB_Offset[RGB_OFFSET_TABLE_TV006].RGB_offset_S_gain[i] = crRgn_in->gain_by_sat_for_rgb_offset[i];
...
unsigned char fwif_color_calc_ICM_gain_table_driver_tv006(CHIP_COLOR_REGION_TYPE_T *crRgn_in, CHIP_COLOR_CONTROL_T *crCtrl_in, COLORELEM_TAB_T *icmTab_in, COLORELEM_TAB_T *icmTab_out)
{
	...
	for (i = 0; i < itn_section.pillar_num; i++) {
		...
		satbyint = crRgn_in->satbyInt_global[i];
		for (s = 0; s < sat_section.pillar_num; s++) {
			sP = sat_section.rgn_pillar[s];
			satbysat = crRgn_in->satbysat_global[s];
			for (h = 0; h < hue_section.pillar_num; h++) {
				...
				icmTab_out->elem[iP][sP][hP].S += ((sat_rgn_gain<0)?(0-((tmp64*sOffset)*(satbysat*satbyint)>>(37 + 14))):((tmp64*sOffset)*(satbysat*satbyint)>>(36 + 14))); /*shift 28+8 = 36*//* satbysat, satbyint are both 8bit */
	...
	for (i = 0; i < ITNSEGMAX; i++) {
		for (s = 0; s < SATSEGMAX; s++) {
			h = 1;
			while(h < HUESEGMAX) {
				if (icmTab_out->elem[i][s][h - 1].H < icmTab_out->elem[i][s][h].H) h++;	/* OK, no overlap*/
				else {	/* overlap case */
					/* sort data */
					for (h = 0; h < HUESEGMAX; h++) {
						min_tmp = 0xffffffff;
						for (index = HUESEGMAX - 1; index > h; index--) {
							if (icmTab_out->elem[i][s][index].H < min_tmp) {
								min_tmp = icmTab_out->elem[i][s][index].H;
								index_tmp = index;
							}
						}
						if (min_tmp <  icmTab_out->elem[i][s][h].H) {
							icmTab_out->elem[i][s][index_tmp].H = icmTab_out->elem[i][s][h].H;
							icmTab_out->elem[i][s][h].H = min_tmp;
						}
					}
				}
			}
	...
	for (i = 0; i < ITNSEGMAX; i++) {
		for (h = 0; h < HUESEGMAX; h++) {
			s = 1;
			while(s < SATSEGMAX) {
				if (icmTab_out->elem[i][s - 1][h].S < icmTab_out->elem[i][s][h].S) s++;	/* OK, no overlap*/
				else {	/* overlap case */
					/* sort data */
					for (s = 0; s < SATSEGMAX; s++) {
						min_tmp = 0xffffffff;
						for (index = SATSEGMAX - 1; index > s; index--) {
							if (icmTab_out->elem[i][index][h].S < min_tmp) {
								min_tmp = icmTab_out->elem[i][index][h].S;
								index_tmp = index;
							}
						}
						if (min_tmp <  icmTab_out->elem[i][s][h].S) {
							icmTab_out->elem[i][index_tmp][h].S = icmTab_out->elem[i][s][h].S;
							icmTab_out->elem[i][s][h].S = min_tmp;
						}
					}
				}
			}
	...
	for (s = 0; s < SATSEGMAX; s++) {
		for (h = 0; h < HUESEGMAX; h++) {
			i = 1;
			while(i < ITNSEGMAX) {
				if (icmTab_out->elem[i - 1][s][h].I < icmTab_out->elem[i][s][h].I) i++;	/* OK, no overlap*/
				else {	/* overlap case */
					/* sort data */
					for (i = 0; i < ITNSEGMAX; i++) {
						min_tmp = 0xffffffff;
						for (index = ITNSEGMAX - 1; index > i; index--) {
							if (icmTab_out->elem[index][s][h].I < min_tmp) {
								min_tmp = icmTab_out->elem[index][s][h].I;
								index_tmp = index;
							}
						}
						if (min_tmp <  icmTab_out->elem[i][s][h].I) {
							icmTab_out->elem[index_tmp][s][h].I = icmTab_out->elem[i][s][h].I;
							icmTab_out->elem[i][s][h].I = min_tmp;
						}
					}
				}
			}
...
void fwif_color_set_sharpness_table_TV006(CHIP_SHARPNESS_UI_T *ptCHIP_SHARPNESS_UI_T, CHIP_SR_UI_T *ptCHIP_SR_UI_T)
{
	...
	sharp_other_ctrl.edgpk_en               = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.pk_en;
        sharp.MK3_Table.Edge_Filter_Sel         = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.select_filter;//default value is 0
        sharp_other_ctrl.edg_lpf_shift          = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.lpf_shift;//default value is 8
	...
	sharp.Edge_Shp.Edg_D2_Shift_Bit         = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCurveMappingUI.Shift;
	sharp.Edge_Neg_Curve.enable             = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCurveMappingUI.Curve_en;
	...
	sharp.Edge_Neg_Curve.G14		= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCurveMappingUI.neg_g14;
      ...
      //  sharp.TwoD_tex.en_tex            = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.texpk_en;
        ...
        sharp_other_ctrl.texpk_en            = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.pk_en;

        sharp.MK3_Table.Tex_Filter_Sel   = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.select_filter;
        sharp_other_ctrl.tex_lpf_shift   = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.lpf_shift;
	...
	sharp.Texture_Shp.Edg_D2_Shift_Bit       = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureCurveMappingUI.Shift;
	sharp.Tex_Neg_Curve.enable               = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureCurveMappingUI.Curve_en;
	...
	sharp.Tex_Neg_Curve.G14			 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureCurveMappingUI.neg_g14;
        ...
        sharp.SR_H9.C0    = 0; //k6 not use//ptCHIP_SHARPNESS_UI_T->stSharpness.stCommonHPF.h9_c0;
        ...
        sharp.SR_V9_1.C0  = 0;//ptCHIP_SHARPNESS_UI_T->stSharpness.stCommonHPF.v91_c0;
	...
	sharp.Vertical.V_Tex_en                          =1;// ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalCurveMappingUI.tex_en;
	sharp.Vertical.V_Dering_En                       =1;// ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalCurveMappingUI.dering_en;
	sharp.V_Neg_Curve.enable                         = 1;//ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalCurveMappingUI.Curve_en;
	...
	// lesley 1014
	{
	extern void drvif_color_get_DB_AI_SHP(CHIP_SHARPNESS_UI_T *ptCHIP_SHARPNESS_UI_T);
	drvif_color_get_DB_AI_SHP(ptCHIP_SHARPNESS_UI_T);
	}
	// end lesley 1014
	...
	sharp.Texture_Flat_Shp.C0H2 	= ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c0_2;
	sharp.Texture_Flat_Shp.C0H1     = ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c0_1;
...
unsigned char fwif_color_set_dcc_FreshContrast_tv006(CHIP_DCC_T *ptr)
{
	...
	// lesley 1014
	{
	extern void drvif_color_get_DB_AI_DCC(CHIP_DCC_T *ptr);
	drvif_color_get_DB_AI_DCC(ptr);
	}
	// end lesley 1014
...
char fwif_color_set_DDomain_3dLUT_16_TV006(unsigned short *p, unsigned char enable)
{
	...
	if (enable == 0 || p == NULL) {
		...
		rtd_printk(KERN_INFO, TAG_NAME, "DD_3dLUT_DMA, off\n");
		ret = 0;
		...
		ret = drvif_color_D_3dLUT(1, pLUT3D);
		if (ret < 0)
			rtd_printk(KERN_EMERG, TAG_NAME, "DD_3dLUT_DMA, SRAM ret = %d,\n", ret);
		else
			rtd_printk(KERN_INFO, TAG_NAME, "DD_3dLUT_DMA, SRAM done\n");
	} else {
		check_times = 0;
		check_times_max = 30;
		do {
		...
		if (check_times >= check_times_max) {
			rtd_printk(KERN_EMERG, TAG_NAME, "DD_3dLUT_DMA, can't reset DD Err status, check times = %d,\n", check_times);
			drvif_DD_DMA_Err_Check(1);
			ret = -1;
		} else {
			ret = fwif_color_D_LUT_By_DMA_16(SLR_MAIN_DISPLAY, 1, p);
			if (ret < 0) {
				rtd_printk(KERN_EMERG, TAG_NAME, "DD_3dLUT_DMA, DMA ret = %d,\n", ret);
			} else {
				if (drvif_DD_Wait_for_DMA_Apply_Done() < 0)
					rtd_printk(KERN_INFO, TAG_NAME, "DD_3dLUT_DMA, ERR ~= 0\n");
				else
					rtd_printk(KERN_INFO, TAG_NAME, "DD_3dLUT_DMA, DMA done\n");
			}
		}
	}
	up(&VPQ_DDomain_DMA_TBL_Semaphore);
...
{
	drvif_color_set_SiW_ModeChange(mode);
}
...
char fwif_color_set_DM_HDR_3dLUT_24x24x24_16_TV006(void *p, unsigned char check_err)
{
	...
	if (enable == 0 || pArray == NULL) {
		...
		rtd_printk(KERN_INFO, TAG_NAME, "DM_3dLUT_DMA, off\n");
		ret = 0;
	} else if (check_err == 2) {
		fwif_HDR_compare_3DLUT(p, NEW_24x24x24);
		rtd_printk(KERN_INFO, TAG_NAME, "DM_3dLUT_DMA, copy\n");
		ret = -2;
...
{
	#if 0 /* set by scaler_vtop_dolby_mode_setting */
	//Decide HDR setting
...
//#endif
	...
	if (EOTF_table32 == NULL) {
		...
		rtd_printk(KERN_INFO, TAG_NAME, "DM_EOTF_DMA, off\n");
	} else if ((drvif_DM_HDR10_enable_Get() == 0) || ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))) {
		//drvif_DM2_EOTF_Enable(0, 1); // use sRam
		rtd_printk(KERN_INFO, TAG_NAME, "DM_EOTF_DMA, use Sram\n");
		...
		if (ret_apply < 0) {
			...
			if (ret < 0) {
				...
				drvif_DM2_EOTF_Enable(1, 1);
				rtd_printk(KERN_INFO, TAG_NAME, "DM_EOTF_DMA, write done 0\n");
			}
		}
	} else {
		if (check_err == 1) {	/* check err status while DMA write */
			...
			if (ret_apply < 0) {
				...
				ret = fwif_color_DM2_EOTF_Set_By_DMA(EOTF_table32, EOTF_table32, EOTF_table32);
				rtd_printk(KERN_INFO, TAG_NAME, "DM_EOTF_DMA, write done 1\n");
			}
		}
	}
	up(&VPQ_DM_DMA_TBL_Semaphore);
...
unsigned short pre_OETF_TBL16[OETF_size] = {0};
char fwif_color_set_DM2_OETF_TV006(UINT16* OETF_table16, unsigned char check_err)
{
	...
	if (OETF_table16 == NULL){
		...
		rtd_printk(KERN_INFO, TAG_NAME, "DM_OETF_DMA, off\n");
	} else if (check_err == 2) {
		memcpy(pre_OETF_TBL16, OETF_table16, sizeof(short)*OETF_size);
		rtd_printk(KERN_INFO, TAG_NAME, "DM_OETF_DMA, copy\n");
	} else if ((drvif_DM_HDR10_enable_Get() == 0) || ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))) {
...
char fwif_color_set_DM2_OETF_Conv_TV006(UINT16* in_OETF_table16, UINT16* out_OETF_table16, unsigned char tbl_sel)
{
	...
	for (i=0;i<(OETF_PROG_SIZE_TV006>>1);i++)
		OOTF_Y_006[i] = OOTF_Y_006[i] <<4; // Y is 12 bit, convert to 16 bit
	...
	if (OOTF_X_006[0] != 0 || OOTF_Y_006[0] != 0 || OOTF_X_006[(OETF_PROG_SIZE_TV006>>1)-1] != 0xFFFF || OOTF_X_006[(OETF_PROG_SIZE_TV006>>1)-1] != 0xFFFF) {
		...
		OOTF_Y_006[(OETF_PROG_SIZE_TV006>>1)-1] = 0xFFFF;
...
char fwif_color_set_DM2_ToneMapping_TV006(UINT16* TM_Table)
{
	...
	if (TM_Table == NULL){
		...
		rtd_printk(KERN_INFO, TAG_NAME, "DM_TM_DMA, off\n");
	} else if ((drvif_DM_HDR10_enable_Get() == 0) || ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))) {
		//rtd_printk(KERN_INFO, TAG_NAME, "DM_TM_DMA, use sRam\n");
		b02b03_no_sram_mode = drvif_Get_DM2_B0203_noSram_clk_flip_flop();
		if (b02b03_no_sram_mode == 0) {
			drvif_DM2_B0203_noSram_Enable(1, 0);
			ret_apply = WaitDM_DB_Apply_Clean();
		...
		if (ret_apply < 0) {
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "DM_TM_DMA, Sram, wait DB apply fail.\n");
			ret = -1;
		} else {
			ret = drvif_DM2_ToneMapping_Set(TM_Table, TM_Table, TM_Table);
			if (ret < 0) {
				drvif_DM2_ToneMapping_Enable(0, 1);
				rtd_printk(KERN_EMERG, TAG_NAME, "DM_TM_DMA, Sram, write fail.\n");
			} else {
				drvif_DM2_ToneMapping_Enable(1, 1);
				rtd_printk(KERN_INFO, TAG_NAME, "DM_TM_DMA, write done sram\n");
			}
		}
	} else {
		do {
		...
		if (check_times >= check_times_max) {
			rtd_printk(KERN_EMERG, TAG_NAME, "DM_TM_DMA, can't reset DM2 DMA Err status, check times = %d,\n", check_times);
			drvif_DM2_EOTF_OETF_3DLUT_DMA_Err_Check(1);
			ret = -1;
		} else {
			drvif_Get_DM2_DB_Status(&db_en, &db_lv, &db_apply);
			...
			if (ret_apply < 0) {
				drvif_DM2_ToneMapping_Enable(0, 1);
				rtd_printk(KERN_EMERG, TAG_NAME, "DM_TM_DMA, wait DB apply fail, db_apply=%d, no_sram=%d,\n", db_apply, b02b03_no_sram_mode);
				ret = -1;
			} else {
				ret = fwif_color_DM2_ToneMapping_Set_By_DMA(TM_Table, TM_Table, TM_Table);
				if (drvif_DM2_Wait_for_DMA_Apply_Done() < 0) {
					rtd_printk(KERN_EMERG, TAG_NAME, "DM_TM_DMA, DMA wait not finish.\n");
					drvif_DM2_ToneMapping_Enable(0, 1);
					ret = -1;
				} else {
					if (drvif_DM2_EOTF_OETF_3DLUT_DMA_Err_Check(1) != 0) {
						drvif_DM2_ToneMapping_Enable(0, 1);
						rtd_printk(KERN_EMERG, TAG_NAME, "DM_TM_DMA, error status ~= 0..\n");
						ret = -1;
					} else {
						drvif_DM2_ToneMapping_Enable(1, 1);
						ret = 0;
						rtd_printk(KERN_INFO, TAG_NAME, "DM_TM_DMA, write done dma\n");
					}
				}
			}
...
void hdr_resume()
{
	...
	extern dm_dm_submodule_enable_RBUS dm_submodule_temp;
	extern struct v4l2_ext_vpq_block_bypass v4l2_blockbypass;
	...
	CSC3_HDR_flag_v4l2 = 0xff;
	dm_submodule_temp.regValue = 0;	/* DM submodule flag init, HDR off setting*/

	v4l2_blockbypass.bOnOff = 0;
	v4l2_blockbypass.bypassMask = 0;
...
void fwif_color_set_LD_Spatial_Filter_Replace_TV006(KADP_LED_SPATIAL_FILTER *LED_DB_LUT_SPATIAL_FILTER, DRV_LD_Spatial_Filter *LD_Spatial_Filter)
{
	...
	LD_Spatial_Filter->ld_spatialnewcoef20 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef20;
	LD_Spatial_Filter->ld_spatialnewcoef21 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef21;
	LD_Spatial_Filter->ld_spatialnewcoef22 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef22;
	LD_Spatial_Filter->ld_spatialnewcoef23 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef23;
	LD_Spatial_Filter->ld_spatialnewcoef24 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef24;
	LD_Spatial_Filter->ld_spatialnewcoef25 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef25;
	LD_Spatial_Filter->ld_spatialnewcoef30 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef30;
	LD_Spatial_Filter->ld_spatialnewcoef31 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef31;
	LD_Spatial_Filter->ld_spatialnewcoef32 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef32;
	LD_Spatial_Filter->ld_spatialnewcoef33 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef33;
	LD_Spatial_Filter->ld_spatialnewcoef34 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef34;
	LD_Spatial_Filter->ld_spatialnewcoef35 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef35;
	LD_Spatial_Filter->ld_spatialnewcoef40 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef40;
	LD_Spatial_Filter->ld_spatialnewcoef41 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef41;
	LD_Spatial_Filter->ld_spatialnewcoef50 = LED_DB_LUT_SPATIAL_FILTER->hal_ld_spatialnewcoef50;
...
{
	if (NULL == LED_DB_LUT_GLOBAL_CTRL || NULL == LD_Global_Ctrl)
		return;

	LD_Global_Ctrl->ld_blk_hnum = LED_DB_LUT_GLOBAL_CTRL->hal_ld_blk_hnum;
	LD_Global_Ctrl->ld_blk_vnum= LED_DB_LUT_GLOBAL_CTRL->hal_ld_blk_vnum;
	LD_Global_Ctrl->ld_blk_hsize = LED_DB_LUT_GLOBAL_CTRL->hal_ld_blk_hsize;
	LD_Global_Ctrl->ld_blk_vsize = LED_DB_LUT_GLOBAL_CTRL->hal_ld_blk_vsize;
	...
	for (i=0;i<LD_Table_NUM;i++) {
		...
		if (tbl_idx >= LD_Table_NUM) {
			...
			if (strcmp(webos_strToolOption.eBackLight, "oled") != 0)
			{
				...
				fwif_color_set_LD_Global_Ctrl_TV006(&LED_DB_LUT[tbl_idx].hal_global_ctrl, &Local_Dimming_Table[tbl_idx].LD_Global_Ctrl);
			}
		}
	}
		...
		if((PQA_reg_value == DI_IM_DI_RTNR_C_TH4_TH6_reg)&&(PQA_bitup_value == 23)&&(PQA_bitlow_value == 16)&&(c_ini==0)){
			...
			for(i=0;i<7;i++){
				for(j=0;j<10;j++){
					if ((table_row+i) < 255) {
					if ( vpq_project_id == 0x00060000){
						if((VIP_system_info_structure_table->Input_src_Form == _SRC_FROM_VO)&&(VIP_system_info_structure_table->HDR_flag==HAL_VPQ_HDR_MODE_HDR10)){
					        shareMem_VIP_Table->PQA_Table[pqa_table_idx][PQA_MODE_WRITE][table_row+i][PQA_L00+j] =pNR_Table->NR_TABLE.TNR_C.Mad[i][j]/8;
						}else
	...
	if (nTableSize == 256) {
		...
		for (i=0;i<1024;i++) {
			...
			//printk("OutGamma_4CHDGA_R[%d]=%d\n",i, OutGamma_4CHDGA_R[i]);
                ...
                OutGamma_4CHDGA_R[1024] = OutGamma_4CHDGA_R[1023];

                OutGamma_4CHDGA_G[1024] = OutGamma_4CHDGA_G[1023];

                OutGamma_4CHDGA_B[1024] = OutGamma_4CHDGA_B[1023];

                OutGamma_4CHDGA_W[1024] = OutGamma_4CHDGA_W[1023];
...
char fwif_color_DM2_EOTF_OETF_3D_24x24x24_LUT_Handler(HAL_VPQ_HDR_MODE_T HDR_MODE)
{
	...
	if ((HDR_MODE != HAL_VPQ_HDR_MODE_HDR10) && (HDR_MODE != HAL_VPQ_HDR_MODE_HLG) && (HDR_MODE != HAL_VPQ_HDR_MODE_HDR10p)) {
		ret_val = 1;
		rtd_printk(KERN_INFO, TAG_NAME, "DMA_LUT_Handler, SDR\n");
	} else {
		/* DMA will write fail if dolby mode is off */
		if (drvif_DM_HDR10_enable_Get() == 0) {
			if (HDR_MODE == HAL_VPQ_HDR_MODE_HDR10p)
				fwif_color_set_HDR_Dolby_mode_Enable(HDR_DM_HDR10PLUS);
		...
		for (DMA_set_times = 0;DMA_set_times<DMA_set_times_max;DMA_set_times++) {
			...
			if (HDR_MODE == HAL_VPQ_HDR_MODE_HDR10) {
					...
					drvif_color_Set_HDR_OETF_Decrease(0);
					drvif_DM2_Wait_for_DMA_Apply_Done();
			} else if (HDR_MODE == HAL_VPQ_HDR_MODE_HDR10p) {
					args.a3dLUT_16_pArray = &HDR10_3DLUT_24x24x24[0][0];
					fwif_color_set_DM_HDR_3dLUT_24x24x24_16_TV006(&args, 0);
					fwif_color_set_DM2_EOTF_TV006(EOTF_LUT_R, 0);
...
UINT8 fwif_color_DM2_EOTF_OETF_3D_24x24x24_LUT_by_HFC(UINT8 HFC_bit, HDR_flow_control_timing HDR_timing)
{
	...
	extern unsigned char OETF_LUT_reLoad_Flag;
	extern unsigned char CSC3_HDR_flag_v4l2;
	unsigned char HFC_bit_csc3, HFC_bit_oetf;
	char ret_val = -1;
	//unsigned char check_times = 0, check_times_max = 20;
	//unsigned char DMA_set_times = 0, DMA_set_times_max = 3;
	static PQ_HDR_3dLUT_16_PARAM args;
	...
	CSC3_LUT_reLoad_Flag = 0;
	rtd_printk(KERN_INFO, TAG_NAME, "DM_3dLUT_DMA, set by HFC, HFC_bit_csc3=%d,ret_val=%d,v4l2_flag=%d\n", HFC_bit_csc3, ret_val, CSC3_HDR_flag_v4l2);
	...
	if ((HDR_control[HDR_timing].EOTF_ctrl.IPCtrlBits) & HFC_bit) {
		...
		if (tbl_index == HFC_EOTF_OFF) {
			ret_val =fwif_color_set_DM2_EOTF_TV006(NULL, 1);
		} else {
	...
	ret_val = 127;
	tbl_index = 127;
	if ((HDR_control[HDR_timing].OETF_ctrl.IPCtrlBits) & HFC_bit_oetf) {
		tbl_index = HDR_control[HDR_timing].OETF_SPtable;
		if (tbl_index == HFC_OETF_OFF) {
			ret_val =fwif_color_set_DM2_OETF_TV006(NULL, 1);
		} else if (tbl_index == HFC_OETF_CTRL_BY_MAXCLL) {	/*HFC OETF special table index, OETF table control by MAXCLL*/
			Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance(OETF_LUT_ByMaxCLL_ByPanelLuminance, 0);
			ret_val =fwif_color_set_DM2_OETF_TV006(OETF_LUT_ByMaxCLL_ByPanelLuminance, 1);
		}  else if (tbl_index == HFC_OETF_ReLoad) {
			ret_val =fwif_color_set_DM2_OETF_TV006(&pre_OETF_TBL16[0], 1);
		} else {
			ret_val =fwif_color_set_DM2_OETF_TV006(HFC_OETF_LUT_R[tbl_index], 1);
		}
	}
	OETF_LUT_reLoad_Flag = 0;
	rtd_printk(KERN_INFO, TAG_NAME, "DM_OETF_DMA, set by HFC, HFC_bit_oetf=%d,ret_val=%d,ctrl_bit=%d,tbl_idx=%d,\n",
...
void fwif_color_MplusSet(unsigned short *pRegisterSet, unsigned char panelMaker)
{
	...
	else if(panelMaker == 3)	/* BOE M+ */
	{
		regSize = SiwRegSize;	/* LG sent 32 bit data */
		regStep = 2;
		drvif_color_BOE_MplusSet(pRegisterSet, regSize, regStep);
	}
	else
		rtd_printk(KERN_ERR, TAG_NAME,"fwif_color_MplusSet, unknow nPanelMaker\n");
...
void fwif_color_set_LocalContrast_table_TV006(CHIP_LOCAL_CONTRAST_T *param)
{
	...
	if(param==NULL)
	{
		VIPprintf("[%s] get CHIP_LOCAL_CONTRAST_T table Error return\n", __FUNCTION__);
		return;
	}
...
void fwif_color_set_fcic_TV006(unsigned int *u32pTSCICTbl, unsigned int u32Tscicsize, unsigned char *u8pControlTbl, unsigned int u32Ctrlsize, unsigned char bCtrl)
{
	...
	if (drvif_color_check_fcic_table(u32pTSCICTbl, u32Tscicsize) < 0) {
		drvif_color_set_fcic_enable(bCtrl);
		drvif_color_set_fcic_table(u32pTSCICTbl, u32Tscicsize);
		drvif_color_check_fcic_table(u32pTSCICTbl, u32Tscicsize);
	}
...
void fwif_color_Set_De_Contour_tv006(RTK_DECONTOUR_T *level)
{
        ...
        DRV_Block_DeContour_Ctrl ptr_blk_de;

        drvif_color_get_LC_decontour(&ptr_LC_de);
        drvif_color_Get_I_De_Contour(&ptr_i_de);
        drvif_color_Get_Block_DeContour_Ctrl(&ptr_blk_de);
...
void fwif_color_set_BFI_En_by_InputFrameRate(unsigned char bEn)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "bBFI_InputFrameRateEn=%d,fwif_color_get_BFI_En_by_UI=%d\n", bBFI_InputFrameRateEn,fwif_color_get_BFI_En_by_UI());
	if ((bBFI_InputFrameRateEn == TRUE) && (fwif_color_get_BFI_En_by_UI()==TRUE))
		fwif_color_set_BFI_En(TRUE);
	else
		fwif_color_set_BFI_En(FALSE);
	return;
}
...
{
	bBFI_UIEn = bEn;
	rtd_printk(KERN_INFO, TAG_NAME, "bBFI_UIEn=%d,fwif_color_get_BFI_En_by_InputFrameRate=%d\n", bBFI_UIEn,fwif_color_get_BFI_En_by_InputFrameRate());
	if ((bBFI_UIEn == TRUE) && (fwif_color_get_BFI_En_by_InputFrameRate()==TRUE))
		fwif_color_set_BFI_En(TRUE);
	else
		fwif_color_set_BFI_En(FALSE);
	return;
...
{
	unsigned char ret;
	unsigned char bBFI_En_OLAY=0;
	bBFI_En_OLAY = bEn;
	rtd_printk(KERN_INFO, TAG_NAME, "bBFI_En_OLAY=%d,\n", bBFI_En_OLAY);
	if (0 != (ret = Scaler_SendRPC(SCALERIOC_VIP_RPC_BFI_ENABLE_OLED, bEn, 1))){	/*BFI enable information to video FW*/
 		pr_err("ret=%d, SCALERIOC_VIP_RPC_BFI_ENABLE_OLED RPC fail !!!\n", ret);
	}
	return;
}
