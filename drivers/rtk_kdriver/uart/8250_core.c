 ...
 */
#if defined(CONFIG_REALTEK_OF) || defined(CONFIG_ARM64)
static int serial8250_probe(struct platform_device *pdev)
{
...

    {
        ...
        for (i = 0; i < nr_uarts; i++) {
            struct uart_8250_port *up = &serial8250_ports[i];
            struct uart_port *port = &up->port;
            virq  = gic_irq_find_mapping(port->irq);
            pr_info("%s,  hwirq=%d virq=%d\n", __func__, port->irq, virq);
            port->irq = virq;
        }
...
static struct device *g_serial8250_dev = 0;
    ...
    for (i = 0; i < UART_NR; i++) {
        ...
        if (up->port.type != PORT_UNKNOWN && up->port.dev == dev) {
            ...
            if (up->port.membase == (unsigned char *)GET_MAPPED_RBUS_ADDR(ISO_MISC_OFF_UART_U0RBR_THR_DLL_reg)) {
                rtd_outl(ISO_MISC_OFF_INT_CTRL_reg, rtd_inl(ISO_MISC_OFF_INT_CTRL_reg) & (~ISO_MISC_OFF_INT_CTRL_uart0_int_to_scpu_en_mask));
            }
...
static int serial8250_resume(struct device *dev)
{
    ...
    for (i = 0; i < UART_NR; i++) {
        ...
        if (up->port.type != PORT_UNKNOWN && up->port.dev == dev) {
            ...
            if (up->port.membase == (unsigned char *)GET_MAPPED_RBUS_ADDR(ISO_MISC_OFF_UART_U0RBR_THR_DLL_reg)) {                
                rtd_outl(ISO_MISC_OFF_INT_CTRL_reg, rtd_inl(ISO_MISC_OFF_INT_CTRL_reg) | ISO_MISC_OFF_INT_CTRL_uart0_int_to_scpu_en_mask);
                rtd_outl(SYS_REG_INT_CTRL_SCPU_reg, SYS_REG_INT_CTRL_SCPU_standby_iso_misc_off_int_scpu_routing_en_mask | SYS_REG_INT_CTRL_SCPU_write_data_mask); //iso_misc_off_int_scpu_routing enable                
            }
...
static const struct dev_pm_ops rtkserial8250_pm_ops = {
	.suspend	= serial8250_suspend,
	.resume		= serial8250_resume,
#ifdef CONFIG_HIBERNATION
	.freeze_late	= serial8250_suspend,
	.thaw_early	= serial8250_resume,
	.poweroff	= serial8250_suspend,
	.restore_early	= serial8250_resume,
#endif
};
...
unsigned char _rtice_uart_config(void)
{
#ifdef CONFIG_REALTEK_PCBMGR    
    unsigned long long val = 0;    
    
    if (pcb_mgr_get_enum_info_byname("RTICE_CFG", &val)==0)    
        return val & 0x3;
