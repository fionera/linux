...
static void _rtk_serial8250_force_dma_done(struct uart_8250_port *up)
{
	...
	if((dma_write_pos - dma_read_pos) > up->rtk_dma_config.dma_tx_empty_thd) {
		unsigned int dma_reset_read_pos = dma_write_pos - up->rtk_dma_config.dma_tx_empty_thd;
		timeout = 0;
		while(1) {
    	...
    	if((dma_tx_limit - dma_read_pos + dma_write_pos - dma_tx_base) > up->rtk_dma_config.dma_tx_empty_thd) {
		unsigned int dma_reset_read_pos = dma_write_pos - up->rtk_dma_config.dma_tx_empty_thd;
		if(dma_reset_read_pos < dma_tx_base)
			dma_reset_read_pos = dma_tx_limit - up->rtk_dma_config.dma_tx_empty_thd;
		timeout = 0;
		while(1) {
			serial_port_out(port, dma_reg_ofst + UART_DMA_TXRDPTR, dma_reset_read_pos);
			ndelay(1);
...
proc_disable_print:
    ...
    serial_port_out(port, dma_reg_ofst + UART_DMA_CTRL, UR1_DMAIFEN(0));
    serial_port_out(port, dma_reg_ofst + UART_DMA_TXWRPTR, UR1_TX_WRPTR(up->rtk_dma->tx_dma_addr));
    serial_port_out(port, dma_reg_ofst + UART_DMA_TXRDPTR, UR1_TX_WRPTR(up->rtk_dma->tx_dma_addr));	
