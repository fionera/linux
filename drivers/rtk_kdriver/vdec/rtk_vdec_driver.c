...
#if defined CONFIG_RTK_KDRV_DV && defined CONFIG_DOLBY_IDK_1_6_1_1
...
unsigned int svp_flow_num = 0 ;
unsigned int is_apvr = 0 ;
...
/* for HAL_VDEC alloc ES&IB ring buffer  */
VDEC_DVR_MALLOC_STRUCT ES_RBH_info[RTKVDEC_MAX_VDEC_PORT] ;
VDEC_DVR_MALLOC_STRUCT IB_RBH_info[RTKVDEC_MAX_VDEC_PORT] ;
...
unsigned int rtkvdec_reverseInteger(unsigned int value)
{
...
void rtkvdec_fw_remote_malloc_summary(ret_info_t *info) {
	...
	list_for_each(list, &vfw_remote_malloc_info_specific_head) {
		mem_info = list_entry(list, memory_info_t, memory_list);
		if (mem_info->dcu == RTKVDEC_COMEM_DCU2_ALLOC) {
			pr_err("in DCU2 : prot(%d), phy_addr %lx, alloc_size %lx\n", mem_info->protected, mem_info->phy_addr, mem_info->malloc_size);
		}
		if (mem_info->dcu == RTKVDEC_COMEM_DCU1_ALLOC) {
			pr_err("in DCU1 : prot(%d), phy_addr %lx, alloc_size %lx\n", mem_info->protected, mem_info->phy_addr, mem_info->malloc_size);
		}
	}
...
unsigned int rtkvdec_remote_malloc_specific(unsigned long para1, unsigned long para2)
{
	...
	if (para1 == RTKVDEC_VCREATE_ALLOC) {
		...
		else if (para1 == RTKVDEC_COMEM_DCU1_ALLOC) {
			phy_addr = pli_malloc(para2, GFP_DCU1);
			if (phy_addr == INVALID_VAL) {
				pr_warn("[%s %d]alloc DCU1 memory fail\n", __func__, __LINE__);
				return 0;
			}
			DCU = RTKVDEC_COMEM_DCU1_ALLOC ;
			if (rtkvdec_svp_enable_protection((unsigned int)phy_addr, PAGE_ALIGN(para2), TYPE_SVP_PROTECT_COMEM) != 0)
				pr_err("[%s %d] ERR : failed to enable COMEM protection %lx %lx\n", __func__, __LINE__, phy_addr, PAGE_ALIGN(para2));
			else
				is_prot = 1 ;
			pr_debug("RTKVDEC_COMEM_DCU1_ALLOC %lx %lx\n", phy_addr, PAGE_ALIGN(para2));
		}
		else if (para1 == RTKVDEC_COMEM_DCU2_ALLOC) {
			phy_addr = pli_malloc(para2, GFP_DCU2);
			if (phy_addr == INVALID_VAL) {
				pr_warn("[%s %d]alloc DCU2 memory fail\n", __func__, __LINE__);
				return 0;
			}
			DCU = RTKVDEC_COMEM_DCU2_ALLOC ;
			if (rtkvdec_svp_enable_protection((unsigned int)phy_addr, PAGE_ALIGN(para2), TYPE_SVP_PROTECT_COMEM) != 0)
				pr_err("[%s %d] ERR : failed to enable COMEM protection %lx %lx\n", __func__, __LINE__, phy_addr, PAGE_ALIGN(para2));
			else
				is_prot = 1 ;
			pr_debug("RTKVDEC_COMEM_DCU2_ALLOC %lx %lx\n", phy_addr, PAGE_ALIGN(para2));
		}
		...
		if (para1 == RTKVDEC_COMEM_DCU1_ALLOC)
			phy_addr = phy_addr | 0x80000000 ;
	}
...
unsigned int rtkvdec_remote_free_specific(unsigned long para1, unsigned long para2)
{
	...
	list_for_each(del_list, &vfw_remote_malloc_info_specific_head) {
		...
		if (mem_info->phy_addr == para1) {
			if (mem_info->dcu == RTKVDEC_COMEM_DCU1_ALLOC || mem_info->dcu == RTKVDEC_COMEM_DCU2_ALLOC)
				if (mem_info->protected) {
					if (rtkvdec_svp_disable_protection((unsigned int)para1, (unsigned int)mem_info->malloc_size, TYPE_SVP_PROTECT_COMEM) != 0)
						pr_err("[%s %d] ERR : failed to disable COMEM protection %lx %lx\n", __func__, __LINE__, para1, mem_info->malloc_size);
					else
						pr_debug("RTKVDEC_COMEM free %lx %lx\n", para1, mem_info->malloc_size);
				}
			list_del(del_list);
			kfree(mem_info);
			pli_free(para1);
...
void rtkvdec_show_kstate(void)
{
	...
	for (i = 0; i < RTKVDEC_MAX_SVPMEM_NUM; i++) {
		...
		pr_err("                 pid(%d) used(%d) u8M(%d) u12M(%d) u16M(%d) u20M(%d) prot(%d)\n",
			svp_Memory[i].pid, svp_Memory[i].used, svp_Memory[i].used_8M, svp_Memory[i].used_12M, svp_Memory[i].used_16M, svp_Memory[i].used_20M, svp_Memory[i].protected);
	}
	pr_notice("\n");
...
void rtkvdec_set_dummy_apvr(void)
{
	if(is_apvr || svp_flow_num >= 2)
		rtd_outl(PLL_REG_SYS_PLL_VCPU_1_reg,0x0001601F);
	else
		rtd_outl(PLL_REG_SYS_PLL_VCPU_1_reg,0x0001401F);
}
...
int rtkvdec_cobuffer_alloc(VDEC_COBUF_TYPE type, int port)
{
	...
	switch(type)
	{
		case VDEC_SVP_BUF:
		{
			...
			else if (svp_Memory[3].used == 0 && svp_Memory[3].addr != 0 && svp_Memory[4].used == 0 && svp_Memory[4].addr != 0) {
				addr = svp_Memory[3].addr ;
				svp_Memory[3].pid = svp_Memory[4].pid = port ;
				svp_Memory[3].used = svp_Memory[4].used = 1 ;
				svp_Memory[3].used_8M = 1 ;
			}
		...
		case VDEC_SVP_BUF:
		{
			...
			for (i = 0; i < RTKVDEC_MAX_SVPMEM_NUM; i++) {
				if (svp_Memory[i].addr == phy_addr) {
					...
					svp_flow_num-- ;
					rtkvdec_set_dummy_apvr();
					if (svp_Memory[i].used_8M && i < RTKVDEC_MAX_SVPMEM_NUM - 1) {
...
void rtkvdec_alloc_ringbuf(VDEC_COBUF_TYPE type, VDEC_RBINFO_STRUCT *buf_info)
{
	unsigned char port = buf_info->nPort ;
	unsigned long vir_addr = 0 ;
	unsigned int phy_addr = 0 ;
	RINGBUFFER_HEADER *header = NULL ;

	if (port >= RTKVDEC_MAX_VDEC_PORT ) {
	...
	switch(type)
	{
		case VDEC_SVP_BUF:
		{
			/* allocate ES RB Header */
			vir_addr = (unsigned long)dvr_malloc_uncached_specific(sizeof(RINGBUFFER_HEADER), GFP_DCU1, (void **)(&ES_RBH_info[port].Memory));
			if (!vir_addr) {
				pr_err("[%s %d]alloc ES RB Header fail\n",__FUNCTION__,__LINE__);
				return ;
			}
			ES_RBH_info[port].PhyAddr = (unsigned long)dvr_to_phys((void*)vir_addr);
			ES_RBH_info[port].VirtAddr = vir_addr ;
			pr_notice("Alloc DESME v(%lx) p(%lx) vn(%lx)\n", vir_addr, ES_RBH_info[port].PhyAddr, ES_RBH_info[port].Memory);

			phy_addr = rtkvdec_cobuffer_alloc(VDEC_SVP_BUF, port);
			if (!phy_addr) {
				pr_err("[%s %d]alloc ES Buffer fail\n",__FUNCTION__,__LINE__);
				dvr_free((void*)vir_addr);
				ES_RBH_info[port].Memory = ES_RBH_info[port].PhyAddr = ES_RBH_info[port].VirtAddr = 0 ;
				return ;
			}

			header = (RINGBUFFER_HEADER*) ES_RBH_info[port].Memory;
			rtkvdec_memset(header, 0 , sizeof(RINGBUFFER_HEADER));
#ifdef CONFIG_ARM64
			header->beginAddr    = rtkvdec_reverseInteger(phy_addr);
			header->size         = rtkvdec_reverseInteger(RTKVDEC_SVPMEM_SIZE_8M);
			header->bufferID     = rtkvdec_reverseInteger(RINGBUFFER_STREAM);
			header->writePtr     = rtkvdec_reverseInteger(phy_addr);
			header->numOfReadPtr = rtkvdec_reverseInteger(1);
			header->readPtr[0]   = rtkvdec_reverseInteger(phy_addr);
			header->readPtr[1]   = rtkvdec_reverseInteger(phy_addr);
#else
			header->beginAddr    = (unsigned long)rtkvdec_reverseInteger(phy_addr);
			header->size         = (unsigned long)rtkvdec_reverseInteger(RTKVDEC_SVPMEM_SIZE_8M);
			header->bufferID     = (unsigned long)rtkvdec_reverseInteger(RINGBUFFER_STREAM);
			header->writePtr     = (unsigned long)rtkvdec_reverseInteger(phy_addr);
			header->numOfReadPtr = (unsigned long)rtkvdec_reverseInteger(1);
			header->readPtr[0]   = (unsigned long)rtkvdec_reverseInteger(phy_addr);
			header->readPtr[1]   = (unsigned long)rtkvdec_reverseInteger(phy_addr);
#endif
			buf_info->bsRingHeader = (unsigned int)ES_RBH_info[port].PhyAddr ;
			break;
		}
		case VDEC_INBAND_BUF:
		{
			/* allocate ES RB Header */
			vir_addr = (unsigned long)dvr_malloc_uncached_specific(sizeof(RINGBUFFER_HEADER), GFP_DCU1, (void **)(&IB_RBH_info[port].Memory));
			if (!vir_addr) {
				pr_err("[%s %d]alloc ES RB Header fail\n",__FUNCTION__,__LINE__);
				return ;
			}
			IB_RBH_info[port].PhyAddr = (unsigned long)dvr_to_phys((void*)vir_addr);
			IB_RBH_info[port].VirtAddr = vir_addr ;
			pr_notice("Alloc DESME v(%lx) p(%lx) vn(%lx)\n", vir_addr, IB_RBH_info[port].PhyAddr, IB_RBH_info[port].Memory);

			phy_addr = rtkvdec_cobuffer_alloc(VDEC_INBAND_BUF, port);
			if (!phy_addr) {
				pr_err("[%s %d]alloc IB Buffer fail\n",__FUNCTION__,__LINE__);
				dvr_free((void*)vir_addr);
				IB_RBH_info[port].Memory = IB_RBH_info[port].PhyAddr = IB_RBH_info[port].VirtAddr = 0 ;
				return ;
			}

			header = (RINGBUFFER_HEADER*) IB_RBH_info[port].Memory;
			rtkvdec_memset(header, 0 , sizeof(RINGBUFFER_HEADER));
#ifdef CONFIG_ARM64
			header->beginAddr    = rtkvdec_reverseInteger(phy_addr);
			header->size         = rtkvdec_reverseInteger(RTKVDEC_IBBUF_SIZE);
			header->bufferID     = rtkvdec_reverseInteger(RINGBUFFER_COMMAND);
			header->writePtr     = rtkvdec_reverseInteger(phy_addr);
			header->numOfReadPtr = rtkvdec_reverseInteger(1);
			header->readPtr[0]   = rtkvdec_reverseInteger(phy_addr);
			header->readPtr[1]   = rtkvdec_reverseInteger(phy_addr);
#else
			header->beginAddr    = (unsigned long)rtkvdec_reverseInteger(phy_addr);
			header->size         = (unsigned long)rtkvdec_reverseInteger(RTKVDEC_IBBUF_SIZE);
			header->bufferID     = (unsigned long)rtkvdec_reverseInteger(RINGBUFFER_COMMAND);
			header->writePtr     = (unsigned long)rtkvdec_reverseInteger(phy_addr);
			header->numOfReadPtr = (unsigned long)rtkvdec_reverseInteger(1);
			header->readPtr[0]   = (unsigned long)rtkvdec_reverseInteger(phy_addr);
			header->readPtr[1]   = (unsigned long)rtkvdec_reverseInteger(phy_addr);
#endif
			buf_info->ibRingHeader = (unsigned int)IB_RBH_info[port].PhyAddr ;
			break;
		}
		default:
		{
			pr_debug("rtkvdec_alloc_ringbuf: unknown type(%d)\n", type);
			break;
		}
	}
	return ;
...
void rtkvdec_free_ringbuf(VDEC_COBUF_TYPE type, VDEC_RBINFO_STRUCT *buf_info)
{
	unsigned char port = buf_info->nPort ;
	unsigned long vir_addr ;
	unsigned int phy_addr = 0 ;
	...
	switch(type)
	{
		case VDEC_SVP_BUF:
		{
			if ((unsigned int)ES_RBH_info[port].PhyAddr != buf_info->bsRingHeader) {
				pr_err("[%s %d] ES RB Header not match! port[%d] %x %x\n",__FUNCTION__,__LINE__, port, (unsigned int)ES_RBH_info[port].PhyAddr, buf_info->bsRingHeader);
				return ;
			}

			header = (RINGBUFFER_HEADER*) ES_RBH_info[port].Memory;
			phy_addr = rtkvdec_reverseInteger((unsigned int)header->beginAddr);
			rtkvdec_cobuffer_free(VDEC_SVP_BUF, phy_addr);

			if (ES_RBH_info[port].PhyAddr) {
				vir_addr = ES_RBH_info[port].VirtAddr;
				dvr_free((void*)vir_addr);
				ES_RBH_info[port].Memory = ES_RBH_info[port].PhyAddr = ES_RBH_info[port].VirtAddr = 0 ;
			}
			break;
		...
		case VDEC_INBAND_BUF:
		{
			if ((unsigned int)IB_RBH_info[port].PhyAddr != buf_info->ibRingHeader) {
				pr_err("[%s %d] IB RB Header not match! port[%d] %x %x\n",__FUNCTION__,__LINE__, port, (unsigned int)IB_RBH_info[port].PhyAddr, buf_info->ibRingHeader);
				return ;
			}

			header = (RINGBUFFER_HEADER*) IB_RBH_info[port].Memory;
			phy_addr = rtkvdec_reverseInteger((unsigned int)header->beginAddr);
			rtkvdec_cobuffer_free(VDEC_INBAND_BUF, phy_addr);

			if (IB_RBH_info[port].PhyAddr) {
				vir_addr = IB_RBH_info[port].VirtAddr;
				dvr_free((void*)vir_addr);
				IB_RBH_info[port].Memory = IB_RBH_info[port].PhyAddr = IB_RBH_info[port].VirtAddr = 0 ;
			}
			break;
...
void rtkvdec_SVP_memory_init(void)
{
	...
	pr_notice("rtkvdec_SVP_memory_init %lx %x\n", addr, size);
	if (addr == 0 || size == 0) {
	...
	for (i = 0; i < RTKVDEC_MAX_SVPMEM_NUM; i++) {
		...
		svp_Memory[i].used_20M = 0;
		svp_Memory[i].is_prot = 0;
	}
...
void rtkvdec_VBM_memory_init(void)
{
	...
	VBM_total = ALIGN(VBM_total, 1*_MB_);
...
void rtkvdec_dump_videofw_data(void)
{
	...
	unsigned long end_addr   = 0x1ac80000 ;
	unsigned long vir_addr ;
...
#ifdef CONFIG_ARM64
				magic = (unsigned long)ntohl(header->magic) ;
				size  = (unsigned long)ntohl(header->size) ;
				rd    = (unsigned long)ntohl(header->rd) ;
				wr    = (unsigned long)ntohl(header->wr) ;
#else
...
#ifdef CONFIG_ARM64
							header->rd = (unsigned int)htonl(rd + DUMP_GET_MAX_SZIE) ;
#else
...
#ifdef CONFIG_ARM64
								header->rd = (unsigned int)htonl(g_videoDumpMemPhyAddr[i] + sizeof(DEBUG_BUFFER_HEADER) + DUMP_GET_MAX_SZIE) ;
#else
...
#ifdef CONFIG_ARM64
								header->rd = (unsigned int)htonl(g_videoDumpMemPhyAddr[i] + sizeof(DEBUG_BUFFER_HEADER) + (wrPtr - limitPtr)) ;
#else
...
#ifdef CONFIG_ARM64
							header->rd = (unsigned int)htonl(rd + (wrPtr - rdPtr)) ;
#else
...
#ifdef CONFIG_ARM64
			pr_notice("Header %lx, m=%x size=%x r(%x) w(%x)\n"
			, (unsigned long)header, header->magic, header->size, header->rd, header->wr);
#else
			pr_notice("Header %lx, m=%lx size=%lx r(%lx) w(%lx)\n"
			, (unsigned long)header, header->magic, header->size, header->rd, header->wr);
#endif
		}
		else {
...
#ifdef CONFIG_ARM64
				header->rd = (unsigned int)htonl(wr) ;
#else
				header->rd = htonl(wr) ;
#endif
			}
...
#ifdef CONFIG_ARM64
		header->rd = (unsigned int)htonl(g_pDumpES.PhyAddr + sizeof(DEBUG_BUFFER_HEADER)) ;
		header->wr = (unsigned int)htonl(g_pDumpES.PhyAddr + sizeof(DEBUG_BUFFER_HEADER)) ;
#else
...
int rtkvdec_dumplog_enable(const char *pattern, int length)
{
	unsigned long return_value ;
	DEBUG_BUFFER_HEADER *header;
	unsigned long vir_addr ;
	...
	if (send_rpc_command(RPC_VIDEO, RPC_VCPU_SET_DUMPLOG, (unsigned long)g_pDumpES_Send.PhyAddr, 0, &return_value))
		pr_debug("rtkvdec: RPC_VCPU_SET_DUMPES_MEMORY fail %ld\n", return_value);
...
void rtkvdec_dumplog_disable(void)
{
	unsigned long return_value ;
	int ret = 0;
	unsigned long vir_addr ;
...
int rtkvdec_alloc(int size)
{
	...
	unsigned long is_md = 0;
	struct buffer_info *buf_info;

	if ((size & 0xff000000) == 0xdd000000) {
		is_md = 1;
		size &= 0x00ffffff;
	}
...
void rtkvdec_free_by_process(unsigned long process_id)
{
	...
	for (i = 0; i < RTKVDEC_MAX_SVPMEM_NUM; i++) {
		if (svp_Memory[i].pid == process_id) {
			if (svp_Memory[i].is_prot) {
				if (svp_Memory[i].used_8M && i < 4) {
					/* disable protection :  saddr = svp_Memory[i].addr, end addr = svp_Memory[i].addr + RTKVDEC_SVPMEM_SIZE_8M - 1 */
					svp_Memory[i].is_prot = svp_Memory[i+1].is_prot = 0 ;
				}
				else if (svp_Memory[i].used_12M && i < 3) {
					/* disable protection :  saddr = svp_Memory[i].addr, end addr = svp_Memory[i].addr + RTKVDEC_SVPMEM_SIZE_12M - 1 */
					svp_Memory[i].is_prot = svp_Memory[i+1].is_prot = svp_Memory[i+2].is_prot = 0 ;
				}
				else if (svp_Memory[i].used_16M && i < 2) {
					/* disable protection :  saddr = svp_Memory[i].addr, end addr = svp_Memory[i].addr + RTKVDEC_SVPMEM_SIZE_16M - 1 */
					svp_Memory[i].is_prot = svp_Memory[i+1].is_prot = svp_Memory[i+2].is_prot = svp_Memory[i+3].is_prot = 0 ;
				}
				else if (svp_Memory[i].used_20M && i < 1) {
					/* disable protection :  saddr = svp_Memory[i].addr, end addr = svp_Memory[i].addr + RTKVDEC_SVPMEM_SIZE_20M - 1 */
					svp_Memory[i].is_prot = svp_Memory[i+1].is_prot = svp_Memory[i+2].is_prot = svp_Memory[i+3].is_prot = svp_Memory[i+4].is_prot = 0 ;
				}
				else {
					/* disable protection :  saddr = svp_Memory[i].addr, end addr = svp_Memory[i].addr + RTKVDEC_SVPMEM_SIZE_4M - 1 */
					svp_Memory[i].is_prot = 0 ;
				}
			}
			if (svp_Memory[i].protected) {
				svp_free_size = svp_Memory[i].used_8M ? RTKVDEC_SVPMEM_SIZE_8M :
								...
								svp_Memory[i].used_16M ? RTKVDEC_SVPMEM_SIZE_16M :
								svp_Memory[i].used_20M ? RTKVDEC_SVPMEM_SIZE_20M : RTKVDEC_SVPMEM_SIZE_4M ;
				if (rtkvdec_svp_disable_protection(svp_Memory[i].addr, svp_free_size, TYPE_SVP_PROTECT_CPB) != 0)
					pr_err("[%s %d] ERR : failed to disable cpb protection %x %x\n", __func__, __LINE__, svp_Memory[i].addr, svp_free_size);
				else
					svp_Memory[i].protected = 0;
			}
			svp_Memory[i].used = 0 ;
			...
			if (svp_Memory[i].used_12M && i < 3) {
				svp_Memory[i].used_12M = 0 ;
				svp_Memory[i+1].used = svp_Memory[i+2].used = 0 ;
				svp_Memory[i+1].pid = svp_Memory[i+2].pid = 0 ;
			}
			if (svp_Memory[i].used_16M && i < 2) {
				svp_Memory[i].used_16M = 0 ;
				svp_Memory[i+1].used = svp_Memory[i+2].used = svp_Memory[i+3].used = 0 ;
				svp_Memory[i+1].pid = svp_Memory[i+2].pid = svp_Memory[i+3].pid = 0 ;
			}
			if (svp_Memory[i].used_20M && i < 1) {
				svp_Memory[i].used_20M = 0 ;
				svp_Memory[i+1].used = svp_Memory[i+2].used = svp_Memory[i+3].used = svp_Memory[i+4].used = 0 ;
				svp_Memory[i+1].pid = svp_Memory[i+2].pid = svp_Memory[i+3].pid = svp_Memory[i+4].pid = 0 ;
			}
		}
	}
	up(&svp_memory_sem);
	...
	//for MD stop flow
	down(&rtkvdec_alloc_sem);
	rtkdev_free_flow_check = 11 ;
	list_for_each_entry_safe(buf_info, tmp_buf_info,
		&rtkvdec_alloc_head, buffer_list)
	{
		if (buf_info->task_pid == process_id && buf_info->is_md_memory) {
			pr_notice("rtkvdec: resource recovery, stop dolby\n");
			dolby_adapter_dolby_vision_pause();
			dolby_adapter_dolby_vision_flush();
			dolby_adapter_dolby_vision_stop();
			pr_notice("rtkvdec: resource recovery, stop dolby done\n");
		}
	}
...
{
	pr_debug("rtkvdec_set_command(%lx/%lx)\n", para1, para2);
	if (para1 == RTKVDEC_STECMD_TYPE_APVR) {
		if (para2 == 1) {
			is_apvr = 1;
		}
		else {
...
/*
			...
			if (!VideoCreatDone) {
				...
				if ((ret = send_rpc_command(RPC_VIDEO, RPCCMD_VIDEO_CREATE, (unsigned long)init_addr, 0, &return_value))) {
#ifdef CONFIG_ARM64
					pr_debug("rtkvdec: RPCCMD_VIDEO_CREATE V1 fail %lx\n", ret);
#else
					...
					VideoCreatDone = 1 ;
#ifdef CONFIG_ARM64
					pr_debug("rtkvdec: RPCCMD_VIDEO_CREATE V1 success %lx\n", ret);
#else
					...
					if ((ret = send_rpc_command(RPC_VIDEO2, RPCCMD_VIDEO_CREATE, (unsigned long)init_addr, 0, &return_value))) {
#ifdef CONFIG_ARM64
						pr_debug("rtkvdec: RPCCMD_VIDEO_CREATE V2 fail %lx\n", ret);
#else
						...
						VideoCreatDone2 = 1 ;
#ifdef CONFIG_ARM64
						pr_debug("rtkvdec: RPCCMD_VIDEO_CREATE V2 success %lx\n", ret);
#else
...
*/
		...
		case VDEC_IOC_ALLOCSVPM:
		{
			...
			if (svpmem.size == RTKVDEC_SVPMEM_SIZE_4M ) {
				...
				else if (svp_Memory[4].used == 0 && svp_Memory[4].addr != 0) {
					svpmem.addr = svp_Memory[4].addr ;
					svp_Memory[4].pid = svpmem.pid ;
					svp_Memory[4].used = 1 ;
					i = 4;
				}
				...
				if (svp_Memory[0].used == 0 && svp_Memory[0].addr != 0 && svp_Memory[1].used == 0 && svp_Memory[1].addr != 0) {
					...
					svpmem.addr = svp_Memory[3].addr ;
					svp_Memory[3].pid = svp_Memory[4].pid = svpmem.pid ;
					svp_Memory[3].used = svp_Memory[4].used = 1 ;
					svp_Memory[3].used_8M = 1 ;
					i = 3;
				}
				...
				else if (svp_Memory[2].used == 0 && svp_Memory[2].addr != 0 && svp_Memory[3].used == 0 && svp_Memory[3].addr != 0 && svp_Memory[4].used == 0 && svp_Memory[4].addr != 0) {
					svpmem.addr = svp_Memory[2].addr ;
					svp_Memory[2].pid = svp_Memory[3].pid = svp_Memory[4].pid = svpmem.pid ;
					svp_Memory[2].used = svp_Memory[3].used = svp_Memory[4].used = 1 ;
					svp_Memory[2].used_12M = 1 ;
					i = 2;
				}
				...
				else if (svp_Memory[1].used == 0 && svp_Memory[1].addr != 0 && svp_Memory[2].used == 0 && svp_Memory[2].addr != 0 && svp_Memory[3].used == 0 && svp_Memory[3].addr != 0 && svp_Memory[4].used == 0 && svp_Memory[4].addr != 0) {
					svpmem.addr = svp_Memory[1].addr ;
					svp_Memory[1].pid = svp_Memory[2].pid = svp_Memory[3].pid = svp_Memory[4].pid = svpmem.pid ;
					svp_Memory[1].used = svp_Memory[2].used = svp_Memory[3].used = svp_Memory[4].used = 1 ;
					svp_Memory[1].used_16M = 1 ;
					i = 1;
				}
			}
			else if (svpmem.size == RTKVDEC_SVPMEM_SIZE_20M ) {
				if (svp_Memory[0].used == 0 && svp_Memory[0].addr != 0 && svp_Memory[1].used == 0 && svp_Memory[1].addr != 0
				 && svp_Memory[2].used == 0 && svp_Memory[2].addr != 0 && svp_Memory[3].used == 0 && svp_Memory[3].addr != 0
				 && svp_Memory[4].used == 0 && svp_Memory[4].addr != 0) {
					svpmem.addr = svp_Memory[0].addr ;
					svp_Memory[0].pid = svp_Memory[1].pid = svp_Memory[2].pid = svp_Memory[3].pid = svp_Memory[4].pid = svpmem.pid ;
					svp_Memory[0].used = svp_Memory[1].used = svp_Memory[2].used = svp_Memory[3].used = svp_Memory[4].used = 1 ;
					svp_Memory[0].used_20M = 1 ;
					i = 0;
				}
			}
			else {
			...
			if (cmd == VDEC_IOC_ALLOCSVPMP) {
				if (rtkvdec_svp_enable_protection(svp_Memory[i].addr, svpmem.size, TYPE_SVP_PROTECT_CPB) != 0) {
					pr_err("[%s %d] ERR : failed to enable cpb protection %x %x\n", __func__, __LINE__, svp_Memory[i].addr, svpmem.size);
					ret = -EFAULT;
					goto out;
				}
		...
		case VDEC_IOC_ECP_PROTECTION:
		{
			/* pr_debug("rtkvdec: VDEC_IOC_ECP_PROTECTION\n"); */
			VDEC_ECP_STRUCT ecp_prot ;
			int i ;
			if (copy_from_user((void *)&ecp_prot, (const void __user *)arg, sizeof(VDEC_ECP_STRUCT))) {
				ret = -EFAULT;
				pr_err("rtkvdec ioctl code=VDEC_IOC_ECP_PROTECTION failed!!!!!!!!!!!!!!!\n");
				goto out;
			}


			if (ecp_prot.type == VDEC_ECP_ES) {
				down(&svp_memory_sem);
				for (i = 0; i < RTKVDEC_MAX_SVPMEM_NUM; i++) {
					if (svp_Memory[i].addr == ecp_prot.saddr) {
						svp_Memory[i].is_prot = ecp_prot.mode ;
						if (svp_Memory[i].used_8M && i < 4) {
							svp_Memory[i+1].is_prot = ecp_prot.mode ;
						}
						if (svp_Memory[i].used_12M && i < 3) {
							svp_Memory[i+1].is_prot = svp_Memory[i+2].is_prot = ecp_prot.mode ;
						}
						if (svp_Memory[i].used_16M && i < 2) {
							svp_Memory[i+1].is_prot = svp_Memory[i+2].is_prot = svp_Memory[i+3].is_prot = ecp_prot.mode ;
						}
						if (svp_Memory[i].used_20M && i < 1) {
							svp_Memory[i+1].is_prot = svp_Memory[i+2].is_prot = svp_Memory[i+3].is_prot = svp_Memory[i+4].is_prot = ecp_prot.mode ;
						}
						break ;
					}
				}
			...
			else if (ecp_prot.type == VDEC_ECP_VBM) {
			}

			break;
		}

		case VDEC_IOC_GET_RBINFO:
		{
			/* pr_debug("rtkvdec: get ES&IB buf info\n"); */
			VDEC_RBINFO_STRUCT buf_info ;
			if (copy_from_user((void *)&buf_info, (const void __user *)arg, sizeof(VDEC_RBINFO_STRUCT))) {
				ret = -EFAULT;
				pr_err("rtkvdec ioctl code=VDEC_IOC_GET_RBINFO failed!!!!!!!!!!!!!!!\n");
				goto out;
			}

			rtkvdec_alloc_ringbuf(VDEC_SVP_BUF, &buf_info);
			rtkvdec_alloc_ringbuf(VDEC_INBAND_BUF, &buf_info);

			if (copy_to_user((void __user *)arg, (void *)&buf_info, sizeof(VDEC_RBINFO_STRUCT))) {
				ret = -EFAULT;
				goto out;
			}
			break;
		...
		case VDEC_IOC_RTN_RBINFO:
		{
			/* pr_debug("rtkvdec: get ES&IB buf info\n"); */
			VDEC_RBINFO_STRUCT buf_info ;
			if (copy_from_user((void *)&buf_info, (const void __user *)arg, sizeof(VDEC_RBINFO_STRUCT))) {
				ret = -EFAULT;
				pr_err("rtkvdec ioctl code=VDEC_IOC_RTN_RBINFO failed!!!!!!!!!!!!!!!\n");
				goto out;
			}

			rtkvdec_free_ringbuf(VDEC_SVP_BUF, &buf_info);
			rtkvdec_free_ringbuf(VDEC_INBAND_BUF, &buf_info);
...
ssize_t rtkvdec_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	} else if ((cmd_buf[0] == 'l') && (cmd_buf[1] == 'o') && (cmd_buf[2] == 'g') && (cmd_buf[3] == '_')) {
			if ((cmd_buf[4] == 'e') && (cmd_buf[5] == 'n')) {
				pr_notice("\n=== rtkvdec: enable dump log =============\n");
				rtkvdec_dumplog_enable(&cmd_buf[7], (count-7)-1);
				pr_notice("\n=======================================\n");
			}
			else if ((cmd_buf[4] == 'd') && (cmd_buf[5] == 'i')) {
				pr_notice("\n=== rtkvdec: disable dump log =============\n");
				rtkvdec_dumplog_disable();
				pr_notice("\n=======================================\n");
			}
			else {
				pr_notice("%s\n", cmd_buf);
			}
	} else	if ((cmd_buf[0] == 't') && (cmd_buf[1] == 'e') && (cmd_buf[2] == 's') && (cmd_buf[3] == 't') && (cmd_buf[4] == 'm')
					&& (cmd_buf[5] == 'o') && (cmd_buf[6] == 'd') && (cmd_buf[7] == 'e') && (cmd_buf[8] == '=')){
					// shell command as: echo testmode=1 testresult=/tmp/var/log/xxxxx > /dev/rtkvdec
				if((cmd_buf[9] == '1') && (cmd_buf[11] == 't') && (cmd_buf[20] == 't') && (cmd_buf[21] == '=')){
					pr_notice("\n=== rtkvdec: enable dump vdec log =============\n");
					rtkvdec_dumplog_enable(&cmd_buf[22], (count-22)-1);
					pr_notice("\n=======================================\n");
					} // shell command as: echo testmode=1 /tmp/var/log/xxxxx > /dev/rtkvdec
				else if ((cmd_buf[9] == '1') && (cmd_buf[11] == '/')) {
					pr_notice("\n=== rtkvdec: enable dump vdec log =============\n");
					rtkvdec_dumplog_enable(&cmd_buf[11], (count-11)-1);
					pr_notice("\n=======================================\n");
					} // shell command as: echo testmode=0 > /dev/rtkvdec
				else if (cmd_buf[9] == '0') {
					pr_notice("\n=== rtkvdec: disable dump vdec log =============\n");
					rtkvdec_dumplog_disable();
					pr_notice("\n=======================================\n");
				}
				else {
...
static int rtkvdec_init(void)
{
	...
	if (register_kernel_rpc(RPC_VCPU_ID_0x109_rtkvdec_set_command, rtkvdec_set_command) == 1)
		pr_debug("Register rtkvdec_set_command failed!\n");
