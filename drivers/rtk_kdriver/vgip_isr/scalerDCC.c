	...
	if(Temp_Main_S_Curve[Temp_index]<Temp_Main_S_Curve[Temp_index-1])
	{
		for(i=Temp_index-5;i<=Temp_index+5;i++)
		{
			if ((i >= 2) && (i < (Curve_num_Max - 2)))
			{
				...
				if (i == 0) Main_S_Curve[0] = 0;
				if (i == 1) Main_S_Curve[1] = Main_S_Curve[0];
				if (i == (Curve_num_Max - 2)) Main_S_Curve[Curve_num_Max - 2] = Main_S_Curve[Curve_num_Max - 3];
				if (i == (Curve_num_Max - 1)) Main_S_Curve[Curve_num_Max - 1] = Main_S_Curve[Curve_num_Max - 2];
			}
		}
...
//===== DCC SC avoid flash process  ===========
	...
	{
		ROSPrintf_DCCLv5("[MHH]pure_color = %d\n", pure_color);
		ROSPrintf_DCCLv5("[MHH]pure_color_weighting_for_DCC=%d \n",pure_color_weighting_for_DCC);
	}
