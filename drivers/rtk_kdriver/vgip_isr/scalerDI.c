...
/*
	int pan_detect_en=1;
	int film_mode_22flag=1;//=1;
	static unsigned char isFilm_22 = 40;
	unsigned char *FilmInfo = Scaler_MEMC_GetCadence();
	static bool pan_flag_pre;
	static int gmvx_pre2, gmvy_pre2;
	bool reg_vc_nr_MCNR_TMV_no_reset=0;
	static bool static_flag_c=0;
	static bool static_flag_p1=0;
	static bool static_flag_p2=0;
	static bool static_flag_p3=0;
	static bool static_flag_p4=0;
	static int ratio_static_c=0;
	int ratio_static_p=0;
	bool non_static_22_flag=0;
...
if(rtd_inl(0xb802e4f0)==2 || non_static_22_flag==0)
film_mode_22flag=0;

//22P
if(film_mode_22flag==1)
{
if(ratio_static_p>=static_th1 && static_flag_c==0 && ratio_static<=static_th2)
...
// chen 180308 remove
/*
if(static_flag_c==0 && ratio_static_p>=static_th1)
{
	pan_flag_pre=reg_h_pan_flag;
	gmvx_pre2=GMVx+reg_vc_nr_HMCNR_search_range_full;
	gmvy_pre2=GMVy+reg_vc_nr_MCNR_V_search_range_full;
}
else
{
	pan_flag_pre=pan_flag_pre;
...
if(film_mode_22flag==1)
{
	...
	{
		reg_h_pan_flag=1;
		mv_h_pan=15;//reg_h_pan_mv=15;
		mv_v_pan=6;//reg_vc_nr_MCNR_V_pan_mv=6;
	}
	else
	{
	}

	di_im_new_mcnr_control4.tmv_no_reset_flag=reg_vc_nr_MCNR_TMV_no_reset;


	if(sd_input ==0)
	{
		int di_pan_mvx= -(Smart_Integration_Status.SCREEN_HMC_STATUS.BIN_Rank[0]+1-17)+15;
		...
		if(reg_h_pan_flag==1 &&
			abs(max_hist_h_MV-15)<=1 &&
			abs(max_hist_v_MV-6)<=1 && recong_pan_flag==0
		)
		{
		...
		if(film_mode_22flag==0 || reg_h_pan_flag==0)
		{
				di_im_new_mcnr_gmv_penalty_ctrl.n_mcnr_pan_penalty_select=0;
			di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty1=4;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty2=3;
  ...
  if(reg_vc_nr_MCNR_TMV_no_reset==1 || rtd_inl(0xb802e4f0)==4)
{
  di_im_new_mcnr_gmv_penalty_ctrl.n_mcnr_pan_penalty_select=2;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty1=0;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty2=0;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty3=0;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty4=0;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty5=0;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty6=0;
		di_im_new_mcnr_tmv_penalty_th.n_mcnr_tmv_penalty7=0;
		di_im_new_mcnr_pan_condition.h_pan_mv_penalty=3;
}
