...
unsigned long GET_SC_RXFIFO(scd *p_this)
{
	...
	if (val == 0 && try_cnt < 0)
		SC_WARNING("GET_SC_RXFIFO retry_timeout\n");
...
static void dump_sc_tx_log(scd *p_this)
{
    int i=0;
...
int scd_set_state(scd *p_this, IFD_FSM fsm)
{
	...
	case IFD_FSM_DISABLE:
		...
		SET_SCFP(p_this, SC_CLK_EN(0) | SC_CLKDIV((p_this->clock_div - 1)) | SC_BAUDDIV2(0) |	/* fixed to 31*/
			 SC_BAUDDIV1((p_this->baud_div1 - 1)) |
			 SC_PRE_CLKDIV((p_this->pre_clock_div - 1)));

		SET_SCPCR(p_this, SC_TXGRDT(2) |
			  SC_CWI(0) |
			  SC_BWI(0) |
			  SC_WWI(0) |
			  SC_BGT(0x16) |
	...
	case IFD_FSM_DEACTIVATE:
		...
		SET_SCFP(p_this, SC_CLK_EN(1) | SC_CLKDIV((p_this->clock_div - 1)) | SC_BAUDDIV2(0) |	/* fixed to 31*/
			 SC_BAUDDIV1((p_this->baud_div1 - 1)) |
			 SC_PRE_CLKDIV((p_this->pre_clock_div - 1)));
...
int scd_set_clock(scd *p_this, unsigned long clk)
{
	...
	val = GET_SCFP(p_this) & ~SC_PRE_CLKDIV_MASK;
...
int scd_set_etu(scd *p_this, unsigned long etu)
{
	...
	SC_INFO("ETU = %lu. Baud Div2=%lu, Div1 = %lu\n", etu,
		p_this->baud_div2, p_this->baud_div1);
	SET_SCFP(p_this, val);
	return 0;
}
...
int scd_reset(scd *p_this)
{
	...
	if (p_this->atr.length <= 0) {
		SC_WARNING("SC%d - Reset ICC failed, wait ATR timeout\n",
			   p_this->id);
		scd_set_state(p_this, IFD_FSM_DEACTIVATE);
...
int scd_card_detect(scd *p_this)
{
	...
	while (try_cnt < 3) {
		if ((GET_SCIRSR(p_this) & SC_PRES)) {
			...
			SC_WARNING
				("scd_card_detect failed , please check card detect pin\n");
		ndelay(5);
		try_cnt++;
	}
	return 0;
...
int scd_xmit(scd *p_this, unsigned char *p_data, unsigned int len)
{
	...
	if ((p_this->tx_status & SC_TX_DONE) == 0) {
		...
		dump_sc_tx_log(p_this);
		return SC_FAIL;
	}

	if ((p_this->tx_status & SC_TX_ABORT)) {
		SC_WARNING("TX Aborted\n");
		dump_sc_tx_log(p_this);
...
#else
	while (len) {
		unsigned char tx_len = (len > 32) ? 32 : len;
...
{
	if (scd_card_detect(p_this) == 0) {
		SC_WARNING("Read data error scd_card_detect = 0\n");
		return SC_ERR_NO_ICC;
	}
	...
	if (p_this->fsm != IFD_FSM_ACTIVE) {
		SC_WARNING("Read data error FSM(%d)!=ACTIVATE(%d)\n",
			   p_this->fsm, IFD_FSM_ACTIVE);
		return SC_ERR_ICC_DEACTIVATE;
	}
...
static void scd_load_gpio_config(scd *p_this)
{
  ...
  /*#ifdef CONFIG_MACH_RTK_299S*/
	...
	val = 0x8E110B;
	  ...
	  rtd_outl(0xb8000810,	rtd_inl(0xb8000810) |  (0xF<<20));	/* CMD VCC : MISC GPIO 17*/
	  rtd_outl(0xb800081C,	rtd_inl(0xb800081C) |  (0xF<<28));  /* PWR SEL : ST GPIO 33*/
...
scd *scd_open(unsigned char id)
{
	...
	if (p_this) {
		...
		SC_WARNING("scd : open scd failed, out of memory\n");
