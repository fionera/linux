...
static int optee_probe(struct platform_device *pdev)
{
	...
	if (optee_msg_api_get_optee_gitversion(invoke_fn)) {
		dev_warn(&pdev->dev, "get optee git version success\n");
	}
	else
	{
		dev_warn(&pdev->dev, "get optee git version error\n");
	}
...
#ifdef CONFIG_PM
static int rtk_optee_str_suspend(struct device *dev)
{
	optee_invoke_fn *invoke_fn;
	struct arm_smccc_res res;
	int rc;
	...
	invoke_fn = optee->invoke_fn;
	if(IS_ERR(invoke_fn))
	{
		dev_err(dev, "can not get invoke_fn funtion\n");
		return -EINVAL;
	}
...
static int rtk_optee_str_resume(struct device *dev)
{
	optee_invoke_fn *invoke_fn;
	struct arm_smccc_res res;
	int rc;
...
static int rtk_optee_std_suspend(struct device *dev)
{	
	// don't need save optee to snapshot image, so don't notify optee_os suspend
	return 0;
}

static int rtk_optee_std_resume(struct device *dev)
{
	// don't need save optee to snapshot image , so don't notify optee_os resume
...
static const struct dev_pm_ops optee_pm_ops = {
	.suspend	= rtk_optee_str_suspend,
	.resume	= rtk_optee_str_resume,
#ifdef CONFIG_HIBERNATION
	.freeze	= rtk_optee_std_suspend,
	.thaw	= rtk_optee_std_resume,
	.poweroff	= rtk_optee_std_suspend,
	.restore	= rtk_optee_std_resume,
#endif
};
#endif
...
static int __init optee_driver_init(void)
{
	...
	res = platform_driver_register(&optee_driver);
	optee_carvedout_query();

#ifdef CONFIG_OPTEE_SUPPORT_MC_ALLOCATOR
	if(mc_pgtable_init()!= 0)
		pr_err("mc_pgtable_init fail");
#endif
...
#ifdef CONFIG_OPTEE_SUPPORT_MC_ALLOCATOR
subsys_initcall(optee_driver_init);
#else
