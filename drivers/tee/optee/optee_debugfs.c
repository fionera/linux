...
#define STATS_CMD_GET_CARVEDOUT_ADDR 2
...
#ifdef CONFIG_OPTEE_SUPPORT_MC_ALLOCATOR
static struct dentry *mc_pgtable_dentry = NULL;
#endif
...
static int optee_meminfo_dump(struct malloc_stats *kernel_stats, struct malloc_stats *user_stats)
{
	...
	//[CID_31910] printf_arg_mismatch: argument is incompatible with corresponding format string conversion
...
int optee_carvedout_query(void)
{
	...
	svp_module[DISP_CARVEDOUT_ID_MEMC][1] = carvedout_buf_query(CARVEDOUT_SCALER_MEMC, (void **)&svp_module[DISP_CARVEDOUT_ID_MEMC][0]);
	svp_module[DISP_CARVEDOUT_ID_MDOMAIN][1] = carvedout_buf_query(CARVEDOUT_SCALER_MDOMAIN, (void **)&svp_module[DISP_CARVEDOUT_ID_MDOMAIN][0]);
	svp_module[DISP_CARVEDOUT_ID_OD][1] = carvedout_buf_query(CARVEDOUT_SCALER_OD, (void **)&svp_module[DISP_CARVEDOUT_ID_OD][0]);
	svp_module[DISP_CARVEDOUT_ID_VBM][1] = carvedout_buf_query(CARVEDOUT_VDEC_VBM, (void **)&svp_module[DISP_CARVEDOUT_ID_VBM][0]);
	svp_module[DISP_CARVEDOUT_ID_DI_NR][1] = carvedout_buf_query(CARVEDOUT_SCALER_DI_NR, (void **)&svp_module[DISP_CARVEDOUT_ID_DI_NR][0]);

	// alloc share memory
	shm_len = sizeof(unsigned int) * 2 * DISP_CARVEDOUT_NUM;
	shm_buffer = tee_shm_alloc(meminfo_ta.ctx, shm_len, TEE_SHM_MAPPED | TEE_SHM_DMA_BUF);
	if (shm_buffer == NULL) {
		pr_err("optee_meminfo carvedout_query: no shm_buffer\n");
		ret = -ENOMEM;
		goto out;
	}
	...
	rc = tee_client_invoke_func(meminfo_ta.ctx, &arg, param);
	if (rc || arg.ret) {
		pr_err("optee_meminfo carvedout query: invoke failed ret %x arg.ret %x\n", rc, arg.ret);
		ret = -EINVAL;
		goto out;
	}
...
static int optee_mc_pgtable_open(struct inode *inode, struct file *file)
{
	return single_open(file, optee_meminfo_show, NULL);
}
static int optee_mc_pgtable_write(struct file *file, const char __user *userbuf, size_t count, loff_t *ppos)
{
	char buf[20], str[10];
	int num=-1;

	simple_write_to_buffer(buf, sizeof(buf) - 1, ppos, userbuf, count);
	if(sscanf(buf, "%6s %d", str, &num) == 2){
		if(num < 0)
		{
			pr_err("invalid module id %d\n", num);
			goto out;
		}
		if(strncmp(str, "malloc", 5)==0)
		{
			pli_mc_malloc(num);
		}
		else if(strncmp(str, "free", 4)==0)
		{
			pli_mc_free(num);
		}
		else if(strncmp(str, "dump", 4)==0)
		{
			mc_pgtable_debug(num);
		}
		else
			pr_err("mc_pgtable error command\n");
	}
	else if(sscanf(buf, "%6s", str) == 1)
	{
		if(strncmp(str, "dump", 4)==0)
			mc_pgtable_debug(0xe);
	}
	else
		pr_err("error command count str = %s id = %d",str, num);
out:
	return count;
}
static const struct file_operations optee_mc_pgtable_fops = {
	.open           = optee_mc_pgtable_open,
	.read           = seq_read,
	.write          = optee_mc_pgtable_write,
	.llseek         = seq_lseek,
	.release        = single_release,
};
#endif
static void optee_debugfs_file_add(void)
{
  ...
  // -- template begin
	name = "mc_pgtable";
	mc_pgtable_dentry = debugfs_create_file(name, S_IRUGO, optee_debugfs_root, NULL, &optee_mc_pgtable_fops);
	if (!mc_pgtable_dentry)
		pr_warn("tee:mc_pgtable debugfs file entry <%s> creation failed\n", name);
	// -- template end
#endif
}
...
#ifdef CONFIG_OPTEE_SUPPORT_MC_ALLOCATOR
	if (mc_pgtable_dentry) {
		debugfs_remove_recursive(mc_pgtable_dentry);
		mc_pgtable_dentry = NULL;
	}
#endif
}
