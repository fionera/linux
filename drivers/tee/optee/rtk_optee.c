...
static unsigned long rtk_get_fw_table_addr(void)
{
      ...
      unsigned long bootcode_size = 0, factory_ro_size = 0, eeprom_size = 0, 
        ...
        // get bootcode size
        ptr = strstr(platform_info.system_parameters, "bootcode_size=");
        if (ptr)
        {
        	sscanf(ptr, "bootcode_size=%lx", &bootcode_size);
        }

	 // get  factory_ro_size
        ptr = strstr(platform_info.system_parameters, "factory_ro_size=");
        if (ptr)
        {
        	sscanf(ptr, "factory_ro_size=%lx", &factory_ro_size);
        }

	 // get  eeprom_size
        ptr = strstr(platform_info.system_parameters, "eeprom_size=");
        if (ptr)
        {
        	sscanf(ptr, "eeprom_size=%lx", &eeprom_size);
        }
		
        // get factory size
        ptr = strstr(platform_info.system_parameters, "factory_size=");
        if (ptr)
        {
            sscanf(ptr, "factory_size=%lx", &factory_size);
        }

	 // get ss size
       ptr = strstr(platform_info.system_parameters, "ss_size=");
       if (ptr)
      	{
      		sscanf(ptr, "ss_size=%lx", &ss_size);
       }

        // get reserved size
        ptr = strstr(platform_info.system_parameters, "reserved_size=");
        if (ptr)
        {
            sscanf(ptr, "reserved_size=%lx", &reserved_size);
        }
...
int rtk_optee_reload (void)
{
    ...
    outer_flush_range((phys_addr_t)optee_phy_addr, (phys_addr_t)optee_phy_addr+optee_fw_entry->length);
