...
#define CONFIG_USB2_PORT_NUM_CHECK_WITH_USB3	3
...
static bool xhci_hub_polling_check_running = false;

#define CONFIG_USB_VBUS_GROUP_FOR_USB3	MIS_GPIO
#define CONFIG_USB_VBUS_GROUP_FOR_USB2	MIS_GPIO
#define CONFIG_USB_VBUS_CTRL_GPIO_ID_FOR_USB3	80
#define CONFIG_USB_VBUS_CTRL_GPIO_ID_FOR_USB2_2 78
#define CONFIG_USB_VBUS_CTRL_GPIO_ID_FOR_USB2_3 79
#define USB3_VBUS_CTL	    rtk_gpio_id(CONFIG_USB_VBUS_GROUP_FOR_USB3, CONFIG_USB_VBUS_CTRL_GPIO_ID_FOR_USB3)
#define USB2_2_VBUS_CTL   	rtk_gpio_id(CONFIG_USB_VBUS_GROUP_FOR_USB2, CONFIG_USB_VBUS_CTRL_GPIO_ID_FOR_USB2_2)
#define USB2_3_VBUS_CTL   	rtk_gpio_id(CONFIG_USB_VBUS_GROUP_FOR_USB2, CONFIG_USB_VBUS_CTRL_GPIO_ID_FOR_USB2_3)
#define U3PortVBusOFF()       rtk_gpio_output(USB3_VBUS_CTL, 0)
#define U3PortVBusON()        rtk_gpio_output(USB3_VBUS_CTL, 1)
...
#define NO_U3_SUPER_SPEED_PORT	BIT(8)
...
{
   /*rtd_outl(0xb8000850,0x0f0f0f0f0);*/  /* set pinmunx to GPIO */
	rtk_gpio_set_dir(USB2_2_VBUS_CTL, 1);
	rtk_gpio_output(USB2_2_VBUS_CTL, 1); /* USB PORT 1-2 (U2) */
	rtk_gpio_set_dir(USB2_3_VBUS_CTL, 1);
	rtk_gpio_output(USB2_3_VBUS_CTL, 1); /* USB PORT 1-3 (U2) */
	rtk_gpio_set_dir(USB3_VBUS_CTL, 1);
	rtk_gpio_output(USB3_VBUS_CTL, 1);   /* USB PORT 2-1 (U3) */
}
...
static void U3PortVBusControl(unsigned char on)
{
	xhci_plat_dbg("U3 port VBus = %s\n", (on) ? "On" : "Off");
	rtk_gpio_output(USB3_VBUS_CTL, (on) ? 1 : 0);
}
...
int _xhci_get_port_status_str(char* ptr, int count)
{
	...
	{			
		n = scnprintf(ptr, count, "[USB][XHCI] CMD=%03lx, STS=%04lx, U2 Port{STS=%lx, %lx, %lx, %lx}, U3 Port{STS=%lx, Link=%04lx, PWR=%d, Recovery=%d}, irq_cnt=%d\n",
    		...
    		u3_port_recovery_cnt, xhci_irq_count);
	}
	else
	{	
		n = scnprintf(ptr, count, "[USB][XHCI] CMD=%03lx, STS=%04lx, U2 Port{STS=%lx, %lx, %lx, %lx}, irq_cnt=%d\n",
    		rtd_inl(0xb8050020), rtd_inl(0xb8050024),
...
static void xhci_hub_polling_check(unsigned int arg)
{
	...
	if (poll_check_suspend || xhci_hub_polling_check_running)               // enter suspend mode
		return;
	...
	xhci_hub_polling_check_running = true;
	...
	-------------------------------------------------*/
	// case 6: U3 Port is disabled for some reason
	if ((rtd_inl(U3_PORTSC) & PORT_PLS_MASK)==XDEV_DISABLE)
	{
	...
	The b8058164 [11:9] is another way to detect the U3 device connection
	...
	if((rtd_inl(0xb8058164) & (0x3 << 9)) == (0 << 9)) { //check usb3 device plug in connector
		    ...
		    ((rtd_inl(U3_COMPANY_U2_PORTSC) & PORT_CONNECT) == 0))
		{
			...
			else
			{
				...
				{
					mdio_write(0x1, 0x02, 0x0013);
					xhci_plat_warn("[before reset vbus] %08lx=%x, %08lx=%x\n",
					            U3_PORTSC,
					            rtd_inl(U3_PORTSC),
					...
					usb3_device_plugin = 0;
					xhci_plat_warn("vbus reset device\n");
					xhci_plat_warn("[after reset vbus] %08lx=%x, %08lx=%x",
					    U3_PORTSC,
					    rtd_inl(U3_PORTSC),
					    U3_COMPANY_U2_PORTSC,
					    rtd_inl(U3_COMPANY_U2_PORTSC));
...
restart_timer:
	xhci_hub_polling_check_running = false;
	mod_timer(&xhci_hub_check_timer, jiffies + msecs_to_jiffies(polling_interval));
}
...
{
    unsigned long i, tmp, U2PortPara, val1, val2;
	xhci_plat_dbg("---------------  %s  start ---------------\n", __func__);

    for (i = 0; i < U2_PHY_CONFIG_COUNT; i++) {
		val1 = rtd_inl(0xb80380bc);
		val2 = rtd_inl(0xb8038110);
        if ((val1 & 0x1) || (val2 & 0x1)) {
			U2PortPara = ((val1 | val2) >> (16 + i * 4)) & 0xF;
		...
		val1 = rtd_inl(0xb80380c0);
		val2 = rtd_inl(0xb8038114);
        if ((val1 & 0x1) || (val2 & 0x1)) {
            U2PortPara = ((val1 | val2) >> (16 + i * 4)) & 0xF;
    ...
    for (i = 0; i < U2_PHY_CONFIG_COUNT; i++) {
		xhci_plat_info("U2Port %d Read page 0 0xE7 = 0x%x\n", i, get_usb2_phy_reg(i + 1, 0x0, 0xe7));
		xhci_plat_info("U2Port %d Read page 0 0xE4 = 0x%x\n", i, get_usb2_phy_reg(i + 1, 0x0, 0xe4));
	}
...
{		
	...
	if (1)
	{					
		xhci_config.u3_port_disabled = (is_platform_KXLP()) ? 1 : 0;	// disable it is K3LP
		return 0;
	}
#endif		
...
static int xhci_rtk_pm_resume(struct device *dev)
{
	...
	xhci_plat_dbg("=================== xhci_rtk_pm_resume ================\n");
	...
	//avoid usb2.0 port enable and can't occur disconnect////XHCI RESET
	rtd_outl(0xb8050020, 0x00000002);
	udelay(USB_PHY_DELAY);
	rtd_outl(0xb8050058, 0x00000040);    ///This register defines runtime xHC configuration parameters//set 64 device///Operational Base+ (38h)
	rtd_outl(0xb8050020, 0x00000001);    ////(default 00000000) xHCI run
	udelay(USB_PHY_DELAY);
...
static int xhci_pm_event(struct notifier_block *this,
                       unsigned long event, void *ptr)
{
	if (event==PM_SUSPEND_PREPARE)
	{
		xhci_plat_info("going to suspend, disable vbus of usb ports\n");
		U3PortVBusControl(0);                /* USB PORT 2-1 (U3) */
		rtk_gpio_output(USB2_2_VBUS_CTL, 0); /* USB PORT 1-2 (U2) */	
		rtk_gpio_output(USB2_3_VBUS_CTL, 0); /* USB PORT 1-3 (U2) */  
		msleep(500);
	}
...
static struct notifier_block xhci_pm_notifier = {
    .notifier_call = xhci_pm_event,
};

static int xhci_plat_init(void)
{
	xhci_init_driver(&xhci_plat_hc_driver, &xhci_plat_overrides);
	register_pm_notifier(&xhci_pm_notifier);	
...
{
	unregister_pm_notifier(&xhci_pm_notifier);	
...
{
	struct xhci_hcd	*xhci = hcd_to_xhci(bus_to_hcd(dev_get_drvdata(dev)));
	int  n;
	int  count = PAGE_SIZE;
	char *ptr = buf;
	int  i = 0;
	...
	if (sscanf(attr->attr.name, "u2_phy%d_config", &phy_id)!=1 || phy_id > 3)
	{
	...
	size_t			count)
{
	...
	if (sscanf(buf, "%x-%x=%x\n", &page, &addr, &val)==3)
	{
		...
		{
			pr_warn("store_u3_phy_config failed, invalid register address\n");
			goto end_proc;
		}
	}
	else
	{
		pr_warn("store_u3_phy_config failed, unknown command - %s\n", buf);
	}
    ...
    )
{
	...
	n = scnprintf(ptr, count, "no_u3_super_speed_port=%x\n", (*(unsigned int volatile*)GET_MAPPED_RBUS_ADDR(HMAC_CTR0_reg) & NO_U3_SUPER_SPEED_PORT) ? 1 : 0);
	ptr   += n;
	count -= n;
			       ...
			       const char *buf, size_t count)
{
	...
	{
		if (val)
			*(unsigned int volatile*)GET_MAPPED_RBUS_ADDR(HMAC_CTR0_reg) |= NO_U3_SUPER_SPEED_PORT;
		else
			*(unsigned int volatile*)GET_MAPPED_RBUS_ADDR(HMAC_CTR0_reg) &= ~NO_U3_SUPER_SPEED_PORT;
	}
...
// Func : xhci_plat_create_sysfs_files
//===========================================================================
int xhci_create_sysfs_files(struct xhci_hcd *xhci)
{
	...
	device_create_file(controller, &dev_attr_u3_phy_config);
...
void xhci_remove_sysfs_files(struct xhci_hcd *xhci)
{
	struct device *controller = xhci_to_hcd(xhci)->self.controller;
	// Do nothing, we don't remove the sys file, because it should never be removed.
}
