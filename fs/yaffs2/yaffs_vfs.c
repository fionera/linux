...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,12,0))
static void *yaffs_follow_link(struct dentry *dentry, void **cookie)
{
	void *ret;
#elif (YAFFS_NEW_FOLLOW_LINK == 1)
						     ...
						     void *data, int silent)
{
	...
	if (yaffs_parse_options(&options, data_str)) {
		...
		/*check inband-tags*/
		//force open inband-tags for RTK-platform
		options.inband_tags = 1;
		printk(KERN_INFO "yaffs: force set inband-tags for RTK-platform\n");
	}
	...
	if (!read_only && !(mtd->flags & MTD_WRITEABLE)) {
		...
		mtd->flags |= MTD_SWP_WRITEABLE;//Add by alexchang2131 0115-2013. For Software Write Protect mode
	}
			   ...
			   off_t offset, int count, int *eof, void *data)
{
	...
	*(int *)start = 1;
	...
	if (step == 0)
		buf +=
		    sprintf(buf,
			    "Multi-version YAFFS built:"  // "Multi-version YAFFS built:" __DATE__ " " __TIME__
			    "\n");
	else if (step == 1)
		buf += sprintf(buf, "\n");
	else {
...
static int __init init_yaffs_fs(void)
{
	...
	yaffs_trace(YAFFS_TRACE_ALWAYS,
		"yaffs built ");//"yaffs built " __DATE__ " " __TIME__ " Installing.");
...
static void __exit exit_yaffs_fs(void)
{
	...
	yaffs_trace(YAFFS_TRACE_ALWAYS,
		"yaffs built removing."); //"yaffs built " __DATE__ " " __TIME__ " removing.");
