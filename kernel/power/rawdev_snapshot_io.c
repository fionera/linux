 ...
 */
static int swsusp_rawdev_check(void)
{
	int res;
...
static int lzo_compress_threadfn(void *data)
{
	...
	while (1) {
		...
		atomic_set(&d->ready, 0);
		d->ret = crypto_comp_compress(d->tfm, d->unc, d->unc_len, d->cmp, &temp_len);
		d->cmp_len = temp_len;
		/*
...
static void print_header(struct swsusp_info *info)
{
	...
	printk("metadata start offset for compressed image = %d\n", header->metadata_start_offset_for_comp);
	printk("physical address resume func addr %x\n", header->pa_resume_func);
	printk("snapshot magic number 0x%8.8x\n", header->magic);
	if (deps->nr_dep_parts) {
		int i;
		struct dep_part_info *dp_info = &deps->dep_part_info[0];
		...
		for (i = 0; i < deps->nr_dep_parts; i++) {
			printk(" %s\t  %4d\t\t 0x%-8x\t  0x%x\n",
				lgemmc_get_partname(dp_info[i].partnum),
				dp_info[i].partnum,
				dp_info[i].value[0],
				dp_info[i].value[1]);
		}
...
static struct pfn_merge_info *merge_adjacent_pfn(struct swsusp_info *info, struct snapshot_handle *snapshot)
{
	...
	unsigned long *pfn_ptr, *compare_pfn_ptr;
	unsigned long seed_pfn = 0, nr_walked_pfn = 0;
	...
	while (nr_pages < nr_meta_pages) {
		...
		pfn_ptr = (unsigned long *)data_of(*snapshot);
		...
		while (in_page_index < (PAGE_SIZE/sizeof(long))) {
			...
			unsigned int nr_to_write, struct swsusp_info *info)
{
	...
	printk(KERN_INFO
		"PM: Using %d thread(s) for compression.\n"
		"PM: Compressing and saving image data (%d pages)...\n"
		...
		for (run_threads = thr, thr = 0; thr < run_threads; thr++) {
			...
			if (pfn_merge_info[pfn_mi_index - run_threads + thr].compressed) {
				if ((check_cmp_len[thr] != data[thr].cmp_len) ||
				    ...
				    (len != data[thr].cmp_len)) {
					printk(KERN_ERR "PM: Memory barrier issue - check_cmp_len[%d]: 0x%08zx, "
							"len: 0x%08zx, data[%d].cmp_len: 0x%08zx\n",
			...
			if (header->image_size >= snapshot_part_size) {
				printk(KERN_ERR "PM: Compressed snapshot image is too big...\n"
						"PM: Image Size: %d MB (0x%08X), Max Size: %d MB (0x%08X)\n",
						MB(header->image_size), header->image_size,
...
out_finish:
	...
	if (!ret)
		ret = err2;
	if (!ret)
		printk(KERN_INFO "PM: Image saving done. (%d MB)\n", MB(header->image_size));
