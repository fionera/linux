...
static struct psi_group *iterate_groups(struct task_struct *task, void **iter)
{
	...
	if (!*iter)
		...
		memcg = parent_mem_cgroup(*iter);

	if (memcg && parent_mem_cgroup(memcg)) {
		*iter = memcg;
		return mem_cgroup_psi(memcg);
	}
#else
	if (*iter)
		return NULL;
...
int psi_mem_cgroup_alloc(struct mem_cgroup *memcg)
{
	...
	memcg->psi.pcpu = alloc_percpu(struct psi_group_cpu);
	if (!memcg->psi.pcpu)
		return -ENOMEM;
	group_init(&memcg->psi);
	return 0;
}
...
void psi_mem_cgroup_free(struct mem_cgroup *memcg)
{
	...
	cancel_delayed_work_sync(&memcg->psi.clock_work);
	free_percpu(memcg->psi.pcpu);
}
...
int psi_show(struct seq_file *m, struct psi_group *group,
		    enum psi_res res)
{
	int full;
...
int get_psi_info(unsigned long (*avg)[3], u64 *total)
{
	...
	update_stats(&psi_system);

	if (avg)
		memcpy(avg, psi_system.avg, sizeof(psi_system.avg));

	if (total)
		memcpy(total, psi_system.total, sizeof(psi_system.total));
