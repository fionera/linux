			...
			unsigned long pgoff, unsigned long *populate)
{
	...
	*populate = 0;
	...
	if ((flags & MAP_ANONYMOUS) && addr && (addr < TASK_UNMAPPED_BASE)) {
		if (!(flags & MAP_FIXED))
...
int may_expand_vm(struct mm_struct *mm, unsigned long npages)
{
	...
	if (cur + npages > lim) {
		...
		fn_kernel_notify(FN_TYPE_MMAP_LEAK,
			"vm lack is happeded on \"process = %s(%d), thread = %s(%d), max = %dKB\"",
			ts->comm, ts->pid, current->comm, current->pid, rlimit(RLIMIT_AS) / 1024);
#endif
		return 0;
	}
	return 1;
