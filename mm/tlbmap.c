...
/* using be careful, no protection hereg. */
...
unsigned long pli_map_memory(struct file *filp, unsigned long phys_addr, int size)
{
	...
	caddr = do_mmap_pgoff(NULL, (pageno << PAGE_SHIFT)+DEF_MAP_ADDR, size, PROT_READ | PROT_WRITE,
			MAP_FIXED | MAP_SHARED | MAP_NORESERVE, 0, &populate);
...
unsigned long rtktlb_mmap(unsigned long addr)
{
	...
	startaddr = do_mmap_pgoff(NULL, addr + DEF_MAP_SIZE, 0x80000, PROT_READ | PROT_WRITE, 
		MAP_FIXED | MAP_SHARED | MAP_NORESERVE, 0, &populate);
	BUG_ON(populate);
	if (startaddr != (addr + DEF_MAP_SIZE)) {
		do_munmap(current->mm, startaddr, 0x80000);
		up_write(&current->mm->mmap_sem);
		return -EINVAL;
	} else {
		startaddr = addr;
	...
	BUG_ON(!rtk_area);
	rtk_area->vm_page_prot = pgprot_noncached(rtk_area->vm_page_prot);
	if (remap_pfn_range(rtk_area, addr + DEF_MAP_SIZE, 0x18000, 0x80000, rtk_area->vm_page_prot))
...
unsigned long rtktlb_munmap(unsigned long addr)
{
	...
	if (current->mm) {
		down_write(&current->mm->mmap_sem);
		ret = do_munmap(current->mm, addr + DEF_MAP_SIZE, 0x80000);
		up_write(&current->mm->mmap_sem);
	}
