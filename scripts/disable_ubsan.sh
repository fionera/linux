...
find ./ -name Makefile | awk -F '.' '{print "echo UBSAN_SANITIZE := n > ./tmp";print "cat ."$2" >> tmp";print "cp ./tmp ."$2;}' >> ./a.sh
...
find arch/arm/mach-rtd* -name Makefile | xargs git checkout
find drivers/staging/android -name Makefile | xargs git checkout
...
git reset --hard
#find tvscalercontrol/ -name Makefile | xargs git checkout
#find dolbyvision_edr/ -name Makefile | xargs git checkout
cd - 
